...
4. [保存] ボタンを押して、変更を保存します。
	...
	>
	> コストの計算時には、24 時間の期間内に指定されたユニットの最大数が使用されます。価格の詳細については、「[Media Services の価格](http://go.microsoft.com/fwlink/?LinkId=275107)」を参照してください。
...
資産の発行に Azure 管理ポータルを使用するとロケーターが作成され、OnDemand ベースの URL (資産に .ism ファイルが含まれている場合) または SAS URL が提供されます。
