...
8. MP4 フラグメントのタイムスタンプとインデックス (TrackFragmentExtendedHeaderBox fragment *absolute* time と fragment\_index) は昇順で配信される必要があります。Azure Media Services は、重複フラグメントに対する回復力はありますが、メディア タイムラインに従ってフラグメントの順序を変更するには機能が非常に限定されています。
