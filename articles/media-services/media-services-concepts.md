...
###オンプレミス (サード パーティ) のライブ トランスコーダー

オンプレミスのライブ トランスコーダー (またはトランスコーダー) は、カメラから伝送された音声やビデオをマルチビットレート RTMP またはスムーズ ストリーミング形式に変換します。その後、トランスコーダーはアダプティブ ビットレート RTMP またはスムーズ ストリームを Media Services のチャネルにプッシュします。そのイベントを Media Services がライブでブロードキャストします。
...
チャネルは、最大 3 つの同時実行プログラムをサポートするため、同じ受信ストリームのアーカイブを複数作成できます。これにより、1 つのイベントのさまざまな部分を必要に応じて発行したりアーカイブしたりできます。たとえば、ビジネス要件によって 1 つのプログラムの 6 時間分をアーカイブする一方、最後の 10 分間のみをブロードキャストする場合があります。これを実現するには、2 つの同時実行プログラムを作成する必要があります。1 つのプログラムは 6 時間分のイベントをアーカイブするように設定しますが、プログラムは発行されません。もう 1 つのプログラムは 10 分間のアーカイブを行うように設定します。このプログラムは公開されます。

##コンテンツの保護

###動的な暗号化

Microsoft Azure Media Services では、Advanced Encryption Standard (AES) (128 ビット暗号化キーを使用) と PlayReady DRM を使用して動的に暗号化したコンテンツを配信できます。
