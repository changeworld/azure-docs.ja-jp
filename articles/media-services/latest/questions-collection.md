...
[Azure portal](https://portal.azure.com/) を使用し、v3 のライブ イベントを管理し、v3 の資産を表示し、API へのアクセスに関する情報を取得し、コンテンツを暗号化できます。 <br/>他のすべての管理タスク (変換とジョブの管理や v3 コンテンツの分析など) には、[CLI](/cli/azure/ams)、またはサポートされているいずれかのクライアント [SDK](media-services-apis-overview.md#sdks) を使用します。
