...
1. Azure Media Services コード
  ...
  * PlayReady、Widevine、FairPlay のライセンス テンプレートを構成します。 テンプレートを使用すると、使用する各 DRM の権利とアクセス許可を構成できます。
  * JWT の要求に基づいて承認チェックのロジックを指定する、ライセンス配信の承認を定義します。
  * 使用する必要のあるコンテンツ キーとストリーミング プロトコルを指定して、DRM 暗号化を構成します。
