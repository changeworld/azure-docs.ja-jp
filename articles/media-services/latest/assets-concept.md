...
アセットは [Azure Storage アカウント](storage-account-concept.md)内の BLOB コンテナーにマップされ、アセット内のファイルはブロック BLOB としてそのコンテナーに格納されます。 ストレージ SDK クライアントを使用して、コンテナー内のアセットファイルを操作できます。
...
次の表は、これらのオプションをアセットプロパティに適用する方法をまとめたものです。 
