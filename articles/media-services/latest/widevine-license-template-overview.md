...
1.  JSON 文字列を直接構築/ハードコーディングする (エラーを招きやすい方法です)。
...
2.  JSON 属性へのプロパティのマッピングを含む必要なクラスを作成し、それらを JSON 文字列にシリアル化する前にインスタンス化する。 このようなクラスの例と、クラスをインスタンス化およびシリアル化する方法を次に示します。
