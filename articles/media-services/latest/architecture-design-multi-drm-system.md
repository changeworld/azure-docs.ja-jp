...
| **macOS** | FairPlay | FairPlay に対応した Safari (macOS X 10.11+ El Capitan 上の Safari 9+ 以降)|
| **tvOS** | FairPlay | |
...
![CENC を使用する DRM サブシステム](./media/architecture-design-multi-drm-system/media-services-generic-drm-subsystem-with-cenc.png)
...
![Media Services での CENC](./media/architecture-design-multi-drm-system/media-services-cenc-subsystem-on-AMS-platform.png)
...
3. Media Services API を使って、テスト資産用のマルチ DRM ライセンス配信サービスを構成します。 Media Services のライセンス サービスではなく、会社または会社のベンダーによるカスタム ライセンス サーバーを使用している場合は、このステップを省略できます。 ライセンス配信を構成するステップで、ライセンス取得 URL を指定できます。 承認ポリシーの制限、異なる DRM ライセンス サービスのライセンス応答テンプレートなど、一部の詳細構成を指定するには Media Services API が必要です。 現在の Azure Portal では、この構成に必要な UI は提供されていません。 API レベルの情報とサンプル コードについては、「[PlayReady または Widevine の動的共通暗号化を使用する](drm-protect-with-drm-tutorial.md)」をご覧ください。

4. Media Services API を使って、テスト資産の資産配信ポリシーを構成します。 API レベルの情報とサンプル コードについては、「[PlayReady または Widevine の動的共通暗号化を使用する](drm-protect-with-drm-tutorial.md)」をご覧ください。
...
* アプリケーションの **[構成]** タブで、Azure AD のアプリケーションへのアクセス許可を追加します。 アクセス許可は、ローカル バージョンとデプロイ バージョン両方のアプリケーションごとに必要です。
    ...
    ![アクセス許可](./media/architecture-design-multi-drm-system/media-services-perms-to-other-apps.png)
...
![カスタム Azure AD テナント ドメインのアカウント 1](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain1.png)

**スマート カードを使用する Microsoft ドメインのアカウント**:2 要素認証を使用する Microsoft 企業 IT によってカスタマイズされたサインイン ページ。

![カスタム Azure AD テナント ドメインのアカウント 2](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain2.png)

**Microsoft アカウント**:Microsoft アカウントのコンシューマー向けサインイン ページ。

![カスタム Azure AD テナント ドメインのアカウント 3](./media/architecture-design-multi-drm-system/media-services-ad-tenant-domain3.png)
...
![PlayReady に対する EME の使用](./media/architecture-design-multi-drm-system/media-services-eme-for-playready1.png)
...
![PlayReady 用のプレーヤー プラグイン](./media/architecture-design-multi-drm-system/media-services-eme-for-playready2.png)
...
![Widevine に対する EME の使用](./media/architecture-design-multi-drm-system/media-services-eme-for-widevine1.png)

Widevine では、保護されたビデオのスクリーン キャプチャが防止されません。

![Widevine 用のプレーヤー プラグイン](./media/architecture-design-multi-drm-system/media-services-eme-for-widevine2.png)
...
![権利のないユーザー](./media/architecture-design-multi-drm-system/media-services-unentitledusers.png)
...
![対称キーを使ったカスタム STS](./media/architecture-design-multi-drm-system/media-services-running-sts1.png)

次のスクリーンショットは、X509 証明書で非対称キーを使うシナリオのものです (Microsoft の最新ブラウザーを使用)。

![非対称キーを使ったカスタム STS](./media/architecture-design-multi-drm-system/media-services-running-sts2.png)
