...
project.json ファイルには、依存関係が含まれています。 以下に、NuGet からの必要な .NET Azure Media Services パッケージを含む **project.json** ファイルの例を示します。 バージョン番号は、パッケージに最新の更新プログラムがあると変更されることに注意してください。そのため、最新バージョンを確認する必要があります。
...
詳細およびカスタム コンテンツ作成ワークフローを作成するために Azure Media Services で Azure Functions と Logic Apps を使用する完全なサンプルおよびソリューションについては、[Media Services Azure Functions](https://github.com/Azure-Samples/media-services-v3-dotnet-core-functions-integration)に関する記事をご覧ください。
