...
* 高品質のデジタル メディア ファイルを資産 (入力資産) にアップロードしたら、それを処理 (エンコードまたは分析) できます。 処理されたコンテンツは別の資産 (出力資産) に渡されます。 
    * アップロードしたファイルを、さまざまなブラウザーおよびデバイスで再生できる形式に[エンコード](encode-concept.md)します。
    * アップロードしたファイルを[分析](analyze-video-audio-files-concept.md)します。 
        ...
        ビデオおよびオーディオ ファイルから分析情報を抽出できるリッチなエクスペリエンスを実現するには、Media Services v3 プリセットを使用します (「[チュートリアル: Media Services v3 を使用してビデオを分析する](analyze-videos-tutorial.md)」を参照してください)。 <br/>より詳細な分析情報が必要な場合は、Video Indexer を直接使用します。    
* コンテンツが処理されると、メディア コンテンツをクライアント プレーヤーに配信できます。 出力資産内のビデオをクライアントが再生できるようにするには、**ストリーミング ロケーター** を作成する必要があります。 **ストリーミング ロケーター** を作成するときは、**ストリーミング ポリシー** を指定する必要があります。 **ストリーミング ポリシー** を使用して、**ストリーミング ロケーター** のためのストリーミング プロトコルと暗号化オプション (該当する場合) を定義できます。
    ...
    * [パッケージとデリバリー](encode-dynamic-packaging-concept.md)
    * [フィルター](filters-concept.md)
* Advanced Encryption Standard (AES-128) または次の 3 つの主要な DRM システムのいずれかを使用して暗号化することにより、コンテンツを保護できます: コンテンツを配信できます。 コンテンツ保護を構成する方法については、[Azure portal を使用したコンテンツの暗号化](drm-encrypt-content-how-to.md)に関するクイックスタートに説明されています。
...
![Azure portal のスクリーンショット。[資産 (新規)] を選択して開いた [資産] ウィンドウと、[アップロード] ボタンを選択して追加した新しい資産が表示されています。](./media/asset-create-asset-upload-portal-quickstart/upload.png)
...
![Azure portal の [資産] ウィンドウのスクリーンショット。ignite.mp4 Media Encoded Standard encoded という資産が資産一覧に追加されています。](./media/asset-create-asset-upload-portal-quickstart/encode.png)
...
1. **[Streaming locator]\(ストリーミング ロケーター\)** セクションで、 **[Add a streaming locator]\(+ ストリーミング ロケーターの追加\)** を押します。
    ...
    > [!NOTE]
    > ストリームを暗号化する場合は、コンテンツキー ポリシーを作成し、ストリーミング ロケーターに設定する必要があります。 詳細については、[Azure portal を使用したコンテンツの暗号化](drm-encrypt-content-how-to.md)に関するページを参照してください。
1. **[Add streaming locator]\(ストリーミング ロケーターの追加\)** ウィンドウで、定義済みのストリーミング ポリシーの 1 つを選択します。 詳細については、「[ストリーミング ポリシー](stream-streaming-policy-concept.md)」を参照してください

    ![ストリーミング ロケーター](./media/asset-create-asset-upload-portal-quickstart/streaming-locator.png)

資産が発行されたら、ポータルで直接ストリーム配信することができます。 

![[再生]](./media/asset-create-asset-upload-portal-quickstart/publish.png)

または、ストリーミング URL をコピーし、クライアント プレーヤーで使用します。

> [!NOTE]
> [ストリーミング エンドポイント](stream-streaming-endpoint-concept.md)が実行されていることを確認します。 Media Service アカウントを初めて作成すると、既定のストリーミング エンドポイントが作成されます。これは停止状態になっているため、コンテンツをストリーム配信する前に開始する必要があります。<br/>ストリーミング エンドポイントが実行状態にある場合にのみ課金されます。
...
[ポータルを使用してコンテンツを暗号化する](drm-encrypt-content-how-to.md)
