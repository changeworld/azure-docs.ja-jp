...
TimeInterval|(省略可能) サンプリング レートです。既定値は 1/2 秒ごとです。<br/>JSON 形式 - HH:mm:ss.SSS (既定値は 00:00:00.500)<br/>XML 形式 – W3C XSD 期間プリミティブ (既定値は PT0.5)
DetectRegions|(省略可能) テキストを検出するビデオ フレーム内のリージョンを指定する DetectRegion オブジェクトの配列。<br/>DetectRegion オブジェクトは次の 4 つの整数で構成されます。<br/>Left: 左余白からのピクセル数<br/>Top: 上余白からピクセル数<br/>Width: リージョンの幅 (ピクセル単位)<br/>Height: リージョンの高さ (ピクセル単位)
