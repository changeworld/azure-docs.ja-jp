...
>[AZURE.NOTE] HLS コンテンツを PlayReady で暗号化する必要がある場合は、共通コンテンツ キーを作成し、それを資産に関連付ける必要があります。 また、「 [Using PlayReady dynamic common encryption](media-services-protect-with-drm.md) 」(PlayReady 動的な共通暗号化の使用) の説明に従って、コンテンツ キーの承認ポリシーを構成する必要もあります。
