<properties 
	...
	description="Azure Media Services Encoder では、カスタムのプリセット ファイルを Azure メディア エンコーダーに渡すことができます。このトピックでは、次のタスクを実現するためにプリセット ファイルをカスタマイズする方法を示します。既存のビデオでイメージをオーバーレイする、エンコーダーが生成する出力ファイル名を制御する、ビデオを結合する。" 
...
Azure Media Services Encoder では、カスタムのプリセット ファイルを Azure メディア エンコーダーに渡すことができます。このトピックでは、次のタスクを実現するためにプリセット ファイルをカスタマイズする方法を示します。
...
###Media サービス エンコーダーでのオーバーレイ

オーバーレイを表示するタイミング、オーバーレイを表示する時間、およびイメージ/ビデオのオーバーレイでは画面にオーバーレイを表示する位置を指定することができます。オーバーレイをフェードイン/フェードアウトすることもできます。オーバーレイするオーディオ/ビデオ ファイルは、複数のアセットにも 1 つのアセットにも含めることができます。オーバーレイは、エンコーダーに渡すプリセット XML で制御します。プリセット スキーマの詳細については、Azure メディア エンコーダーのスキーマを参照してください。オーバーレイは、次のプリセット スニペットに示すように <MediaFile> 要素で指定します。
...
オーバーレイは、1 つまたは複数のアセットから指定できます。複数のアセットを使用してビデオ オーバーレイを作成する場合は、オーバーレイのファイル名を %n% 構文を使用して OverlayFileName 属性で指定します。ここで n はエンコード タスクの入力アセットの 0 から始まるインデックスです。1 つのアセットを使用してビデオ オーバーレイを作成する場合は、次のプリセット スニペットに示すように、オーバーレイのファイル名は OverlayFileName 属性に直接指定します。
...
複数のアセットに格納されているオーディオ オーバーレイの場合は、オーディオ オーバーレイのファイル名を %n% 構文を使用して AudioOverlayFileName 属性で指定します。ここで n はエンコード タスクの入力アセットのコレクションの 0 から始まるインデックスです。1 つのアセットに格納されているオーディオ オーバーレイの場合は、オーバーレイのファイル名は AudioOverlayFileName 属性で直接指定します。AudioOverlayLayoutMode で、オーディオ オーバーレイを表示するタイミングを指定します。"WholeSequence" に設定すると、ビデオの時間中ずっとオーディオ トラックが表示されます。"Custom" に設定すると、AudioOverlayStartTime 属性と AudioOverlayEndTime 属性でオーディオ オーバーレイの開始時間と終了時間を指定します。OverlayStartTime と OverlayEndTime は、ソース ビデオのタイムライン内に収まるようにします。オーディオ オーバーレイのすべての属性の詳細については、Azure メディア エンコーダーのスキーマを参照してください。オーディオ オーバーレイは、次のプリセット スニペットに示すようにビデオ オーバーレイと組み合わせることができます。
...
- 入力アセット、プリセットの構成、メディア エンコーダー、および出力アセットのコレクションを指定するタスクを作成する
- ジョブを送信する
...
>[AZURE.NOTE]このスニペットは、わかりやすくするために順番に各アセットを読み込みます。実稼働環境では、アセットは一括で読み込まれます。複数のアセットを一括アップロードする方法については、「[Media Services SDK for .NET を使用するアセットの一括取り込み](media-services-dotnet-upload-files.md#ingest_in_bulk)」のページを参照してください。
...
- プリセット XML に ?OverlayFileName=”%n%” への参照が含まれている場合は、タスクの InputAssets コレクションに n + 1 以上のアセットを含める必要があります。



##ビデオのセグメントの結合

Media Services Encoder は、一連のビデオの結合をサポートします。ビデオの端と端を結合できます。また、結合するビデオの一方または両方の部分を指定することができます。結合の結果は、入力アセットの指定されたビデオを含む 1 つの出力アセットになります。結合するビデオは、複数のアセットにも 1 つのアセットにも含めることができます。結合は、エンコーダーに渡すプリセット XML によって制御します。プリセット スキーマの詳細については、「[Azure メディア エンコーダーのスキーマ](https://msdn.microsoft.com/library/azure/dn584702.aspx)」を参照してください。
...
結合する各ビデオについて、<Source> 要素を <Sources> 要素に追加します。各 <Source> 要素に <Clips> 要素を 1 つ含めます。各 <Clips> 要素には、1 つまたは複数の <Clip> 要素を含めます。この要素で開始時刻と終了時刻を指定して、出力アセットに結合するビデオの長さを指定します。<Source> 要素で、機能するアセットを参照します。参照の形式は、結合するビデオが別々のアセットと 1 つのアセットのどちらに含まれているかよって異なります。ビデオ全体を結合する場合は、<Clips> 要素を省略します。

###複数のアセットからのビデオの結合

複数のアセットのビデオを結合するときは、<Source> 要素の MediaFile 属性で 0 から始まるインデックスを使用して、<Source> 要素が対応するアセットを指定します。ゼロから始まるインデックスを指定しないと、MediaFile 属性を指定していない <Source> 要素は、最初の入力アセットを参照します。他のすべての <Source> 要素では%n% 構文を使用して参照する入力アセットの 0 から始まるインデックスを指定する必要があります。ここで n は、入力アセットの 0 から始まるインデックスです。前の例では、1 つ目の <Source> 要素で最初の入力アセット、2 つ目の <Source> 要素で 2番目の入力アセットを指定しています (以降も同様) 。参照する入力アセットの順序などの要件はありません。
...
この例では、2 番目、1 番目、3 番目の入力アセットの一部を結合します。各ビデオは 0 から始まるインデックスによって参照されるため、同じ名前を持つ 2 つのビデオを結合できることに注意してください。プリセット ファイルを作成した後は、次の処理を行う必要があります。
 
- アセットをアップロードする
- プリセットの構成を読み込む
- ジョブを作成する
- Media Services Encoder への参照を取得する
- 入力アセット、プリセットの構成、メディア エンコーダー、および出力アセットを指定するタスクを作成する
- ジョブを送信する
...
このスニペットは、わかりやすくするために順番に各アセットを読み込みます。実稼働環境では、アセットは一括で読み込まれます。複数のアセットを一括アップロードする方法については、「[Media Services SDK for .NET を使用するアセットの一括取り込み](media-services-dotnet-upload-files.md#ingest_in_bulk)」のページを参照してください。完全なサンプル コードは、「[Media Services Encoder での結合](https://code.msdn.microsoft.com/Stitching-with-Media-8fd5f203)」を参照してください。

###1 つのアセットからのビデオの結合

1 つのアセット内のビデオを結合するときは、各ビデオの名前が一意である必要があります。ビデオは、ファイル名を属性の値として、MediaFile 属性を使用して指定します。次に例を示します。
...
このプリセットでは、video1.mp4、video2.wmv、および video3.wmv の一部を結合して出力アセットにします。ジョブとタスクの作成は、複数のアセットからのビデオの結合と同じです。次のコードに示すように、1 つのアセットをアップロードするだけです。
...
オーディオ トラックの内容のほとんどが話し声であるビデオをエンコードするときに既定のエンコーダー プリセットを使用すると、バックグラウンド ノイズが発生し、エンコード先のアセットで増幅されることがあります。この現象は、NormalizeAudio 属性が true に設定されていると発生します。

###ほとんどが話し声であるプレゼンテーションのエンコード

バックグラウンド ノイズの増幅をを回避するのには、次の手順を実行します。

1. 使用するエンコーダー プリセットの内容を XML ファイルにコピーします。エンコーダー プリセットは、Azure メディア エンコーダーのスキーマにあります。
...
1. 変更したプリセット ファイルをローカル ハード ドライブに保存し、次のようなコードを使用してこのカスタム プリセットでエンコードします。
	...
	// エンコードする入力アセットを指定する task.InputAssets.Add(asset);
	 
	// ジョブの結果を含める出力アセットを追加する task.OutputAssets.AddNew("Output asset for " + asset.Name + ", encoded using " + presetName, AssetCreationOptions.None);
