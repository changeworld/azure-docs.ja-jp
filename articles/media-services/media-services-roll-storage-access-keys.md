...
新しく Azure Media Services アカウントを作成すると、メディア コンテンツの保存に使用する Azure ストレージ アカウントも選択するよう求めるメッセージが表示されます。Media Services アカウントには複数のストレージ アカウントを追加できます。
...
Media Services は、1 つのストレージ キー (プライマリまたはセカンダリ) に依存します。特に、資産のストリーミングやダウンロードに使用されるロケーターはアクセス キーに依存します。ストレージ アクセス キーをローリングする際は、ストリーミング サービスが中断されないようにロケーターも更新する必要があります。
...
30 分が経過したら、OnDemand ロケーターを再作成して、新しいセカンダリ ストレージ キーとの依存関係を確立し、既存の URL を保持できます。
