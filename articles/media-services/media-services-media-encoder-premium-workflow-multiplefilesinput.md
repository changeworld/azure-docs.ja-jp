...
ファイルから XML 構成を読み取り、ジョブ内のタスクに渡す C# のコードは次のとおりです。
...
###単純値が含まれるプロパティ
場合によっては、メディア エンコーダー プレミアム ワークフローで実行されるワークフロー ファイルと共に、コンポーネントのプロパティをカスタマイズすると便利です。ビデオにテキストを重ね合わせるワークフローを設計したとします。そのテキスト (たとえば、現在の日付) は、実行時に設定する必要があります。これを行うには、テキストを、オーバーレイ コンポーネントの text プロパティの新しい値として、エンコード タスクから送信します。このメカニズムを使用すると、ワークフロー内のコンポーネントの他のプロパティ、たとえば、オーバーレイの位置や色、AVC エンコーダーのビットレートなどを変更できます。ワークフローのコンポーネント プロパティの上書きには、**setRuntimeProperties** を使用します。
...
パスとプロパティ名を確認するには、各プロパティの横にあるアクション ボタンを使用します。このアクション ボタンをクリックし、[編集] を選択すると、プロパティの実際の名前と、その名前のすぐに上に名前空間が表示されます。
...
**メディア エンコーダー プレミアム ワークフロー**に送信するタスクごとに 2 つのアセットが必要です。

- 1 つ目は、ワークフロー ファイルを含む "ワークフロー アセット" です。ワークフロー ファイルは、[ワークフロー デザイナー](media-services-workflow-designer.md)を使用して設計できます。
- 2 つ目は、エンコードするメディア ファイルを含む "メディア アセット" です。

複数のメディア ファイルを**メディア エンコーダー プレミアム ワークフロー** エンコーダーに送信する場合は、次の制約が適用されます。

- すべてのメディア ファイルが、同じ "メディア アセット" に含まれていなければなりません。複数のメディア アセットを使用することはできません。
...
"setRuntimeProperties を使用してファイル名を設定する場合、プライマリ ファイルとメディア ファイル入力コンポーネントは接続されていません"
...
ワークフローでクリップ リスト XML を指定するには、実行時に、構成文字列 XML で **sourceTranscode** を使用します。それには、ワークフローでクリップ リスト XML ピンをメディア ソース コンポーネントに接続する必要があります。
...
このプロパティを使用して "Expressions" で出力ファイルに名前を付けるように、/primarySourceFile プロパティを指定する必要がある場合は、/primarySourceFile の設定によってクリップ リストが上書きされないように、/primarySourceFile プロパティの "後" に、クリップ リスト XML をプロパティとして渡すことをお勧めします。
...
エンコード中、入力ビデオでロゴ イメージを重ね合わせる例を見てみましょう。この例では、入力ビデオの名前は "MyInputVideo.mp4"、ロゴの名前は "MyLogo.png" です。次の手順を実行する必要があります。

- ワークフロー ファイルを使用してワークフロー アセットを作成します (以下の例)
- メディア アセットを作成します。このアセットには、MyInputVideo.mp4 (プライマリ ファイル) と MyLogo.png の 2 つのファイルが含まれます。
- 上記の入力アセットと共に、タスクをメディア エンコーダー プレミアム ワークフロー メディア プロセッサに送信し、次の構成文字列を指定します
...
上記の例では、ビデオ ファイルの名前は、メディア ファイル入力コンポーネントと primarySourceFile プロパティに送信され、ロゴ ファイルの名前は、グラフィック オーバーレイ コンポーネントに接続されているもう 1 つのメディア ファイル入力に送信されます。
...
選択されたファイルは直ちにメディア ファイル入力コンポーネントによって調査され、出力ピンに設定されます。
...
ビデオのロゴの位置を変更する (たとえば、ビデオの左上隅から 10% 離して配置する) 必要がある場合は、[手操作入力] をオフにします。このオプションをオフにできるのは、メディア ファイル入力を使用して、ロゴ ファイルをオーバーレイ コンポーネントに配置しているためです。
...
成功したら、Azure Media Services で実行できます。

まず、Azure Media Services で、ビデオ ファイルとロゴの 2 つのファイルが含まれるアセットを準備します。これを行うには、.NET または REST API を使用します。Azure ポータルまたは [Azure Media Services Explorer](https://github.com/Azure/Azure-Media-Services-Explorer) (AMSE) を使用することもできます。

このチュートリアルでは、AMSE でアセットを管理する方法について説明します。ファイルをアセットに追加する方法は 2 つあります。

1. ローカル フォルダーを作成し、そのフォルダーに 2 つのファイルをコピーします。次に、フォルダーを [アセット] タブにドラッグ アンド ドロップします
1. ビデオ ファイルをアセットとしてアップロードし、アセット情報を表示します。次に [ファイル] タブに移動し、その他のファイル (ロゴ) をアップロードします。
...
ジョブが完了するたびに、出力アセットの MP4 ファイルにオーバーレイが表示されます。
...
サンプル ワークフローは[こちら](https://github.com/Azure/azure-media-services-samples/tree/master/Encoding%20Presets/VoD/MediaEncoderPremiumWorkfows/)からダウンロードできします。
