...
* [Azure Resource Health とは](#what-is-azure-resource-health)
* [Resource Health の目的は何ですか。](#what-is-the-resource-health-intended-for)
* [Resource Health ではどのような正常性チェックが実行されますか。](#what-health-checks-are-performed-by-resource-health)
* [正常性状態はそれぞれ何を意味しますか。](#what-does-each-of-the-health-status-mean)
* [不明な状態とはどういう状態ですか。リソースに何か問題がありますか。](#what-does-the-unknown-status-mean-is-something-wrong-with-my-resource)
* [使用不可のリソースについてサポートを受けるにはどうすればよいですか。](#how-can-i-get-help-for-a-resource-that-is-unavailable)
* [Resource Health では、使用不可の状態は、プラットフォームの問題とユーザーの操作で区別されますか。](#does-resource-health-differentiate-between-unavailability-cased-by-platform-problems-versus-something-i-did)
* [Resource Health をユーザー固有の監視ツールと統合できますか。](#can-i-integrate-resource-health-with-my-monitoring-tools)
* [Resource Health はどこにありますか。](#where-do-i-find-resource-health)
* [Resource Health はすべてのリソースの種類で利用できますか。](#is-resource-health-available-for-all-resource-types)
* [リソースは使用可能と表示されているが、そのように思えない場合はどうすればよいですか。](#what-should-i-do-if-my-resource-is-showing-available-but-i-believe-it-is-not)
* [Resource Health はすべての Azure リージョンで利用できますか。](#is-resource-health-available-for-all-azure-regions)
* [Resource Health とサービス正常性ダッシュボードや Azure Portal のサービスの通知の違いは何ですか。](#how-is-resource-health-different-from-the-service-health-dashboard-or-the-azure-portal-service-notifications)
* [リソースごとに Resource Health をアクティブにする必要がありますか。](#do-i-need-to-activate-resource-health-for-each-resource)
* [自分の組織で Resource Health を有効にする必要がありますか。](#do-we-need-to-enable-resource-health-for-my-organization)
* [Resource Health は無料で使用できますか。](#is-resource-health-available-free-of-charge)
* [Resource Health で提示されている推奨事項とはどのようなものですか。](#what-are-the-recommendations-that-resource-health-provides)
...
1. 計画外のイベント。予期しないホストの再起動など。
2. 計画的なイベント。スケジュールされたホスト OS の更新プログラムなど。
3. ユーザー アクションによってトリガーされるイベント。ユーザーによる仮想マシンの再起動など。

## <a name="what-does-each-of-the-health-status-mean"></a>正常性状態はそれぞれ何を意味しますか。
正常性状態には次の 3 種類があります。
1. 使用可能: このリソースに影響を与える可能性のある既知の問題は、この Azure プラットフォームにはありません。
2. 使用不可: リソースに影響を与えている問題が Resource Health によって検出されました。
3. 不明: リソースの正常性に関する情報の受信が停止されているため、リソースの正常性を Resource Health で判断できません。 
...
1. 対象のリソースに移動します。 左側のナビゲーションで、**[リソース正常性]** をクリックします。
2. [Azure Monitor] ブレードに移動します。  左側のナビゲーションで、**[リソース正常性]** をクリックします。
3. ポータルの右上隅の疑問符をクリックし、**[ヘルプ + サポート]** を選択して **[ヘルプ + サポート]** ブレードを表示します。 ブレードが開いたら、**[リソース正常性]** をクリックします。
