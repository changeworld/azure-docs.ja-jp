<properties
   pageTitle="Active Directory を使用して Data Lake Store を認証する |Microsoft Azure"
...
# Data Lake Store で Azure Active Directory を使用して認証する
...
>[AZURE.NOTE] この方法は、Python または Java 用の Azure AD Authentication Library (ADAL) ではまだサポートされていません。
...
>[AZURE.NOTE] 上記のリンクで説明されている手順は、Azure AD Web アプリケーションのためのものですが、ネイティブ クライアント アプリケーションを作成する場合でも、手順はまったく同じです。
...
* Azure AD Web アプリケーションのクライアント ID。
	...
	>[AZURE.NOTE] Azure AD アプリケーションの作成およびクライアント ID の取得方法については、[Active Directory アプリケーションの作成に関する記事](../resource-group-create-service-principal-portal.md#create-an-active-directory-application)を参照してください。
