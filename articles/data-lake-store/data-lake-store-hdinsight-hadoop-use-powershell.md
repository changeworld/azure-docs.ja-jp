...
3. HDInsight クラスターからアクセスする Data Lake Store ファイル/フォルダーに、サービス プリンシパル アクセスを付与します。 次のスニペットにより、Data Lake Store アカウントのルートへのアクセスが得られます。

        Set-AzureRmDataLakeStoreItemAclEntry -AccountName $dataLakeStoreName -Path / -AceType User -Id $objectId -Permissions All

    プロンプトで、 **Y** を入力して確定します。

## <a name="create-an-hdinsight-cluster-with-authentication-to-data-lake-store"></a>Data Lake Store への認証を使用して HDInsight クラスターを作成する
