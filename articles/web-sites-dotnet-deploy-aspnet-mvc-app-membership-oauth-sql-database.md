...
1. 構成ウィザードでは *ContactManager* に基づいて一意の名前が提示されます。新しい App Service プランとリソース グループを作成するかどうかを決める必要があります。新しいプランまたはリソース グループを作成するかどうかを決める方法については、「[Azure App Service プランの詳細な概要](azure-web-sites-web-hosting-plans-in-depth-overview.md)」を参照してください。このチュートリアルでは、少なくとも 1 つのリソース グループを新しく作成します。近くのリージョンを選択します。[azurespeed.com](http://www.azurespeed.com/ "AzureSpeed.com") を使用して、待ち時間が最も短いデータ センターを検索することができます。次の手順でデータベースを設定します。したがって、**[OK]** はまだクリックしないでください。
...
1. **[ツール]** メニューの **[NuGet パッケージ マネージャー]**、**[パッケージ マネージャー コンソール]** を順に選択します。![Package Manager Console in Tools menu](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/SS6.png)
