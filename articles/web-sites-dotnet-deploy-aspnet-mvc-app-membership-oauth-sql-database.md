...
このチュートリアルでは、ユーザーが Facebook や Google の資格情報を使用してログインできる、安全な ASP.NET MVC 5 Web アプリケーションを構築する方法を説明します。さらに、作成したアプリケーションを[アプリ サービス](http://go.microsoft.com/fwlink/?LinkId=529714)にデプロイする方法についても学習します。
...
* 安全な ASP.NET MVC 5 プロジェクトを作成して、Azure App Service で[アプリ サービスの Web アプリ](http://go.microsoft.com/fwlink/?LinkId=529714)に発行する方法
...
>[AZURE.NOTE] このチュートリアルを完了するには、Microsoft Azure アカウントが必要です。アカウントを持っていない場合は、MSDN サブスクライバーの特典を<a href="/ja-jp/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">有効にするか、無料評価版に</a><a href="/ja-jp/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">サインアップしてください。</a>
...
1. アプリケーションに「**ContactManager**」という名前を付けて、**[OK]** をクリックします。
	...
	**メモ:** 必ず「ContactManager」と入力してください。これからコピーするコード ブロックは、ContactManager というプロジェクト名が前提となっています。

1. **[新しい ASP.NET プロジェクト]** ダイアログ ボックスで、**MVC** テンプレートを選択します。**[認証]** に **[個別ユーザー アカウント]** が設定されていること、**[クラウドでのホスト]** がオンになっていること、**[Web アプリ]** が選択されていることを確認します。

	![New ASP.NET Project dialog box](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database-vs2013/ss1.PNG)

1. 構成ウィザードが  *ContactManager* (下図を参照) に基づいて一意の名前を提示します。近くのリージョンを選択します。[azurespeed.com](http://www.azurespeed.com/ "AzureSpeed.com") を使用すると、最も待機時間が短いデータ センターを検索することができます。 
...
このチュートリアルの中で、アプリケーションを Azure にデプロイします。そこでのログオンには、Google などサード パーティの認証プロバイダーが使用されます。新しく登録されたアカウントは、 *canEdit* ロールに追加されます。アプリの URL と Google ID さえあればだれでも登録し、データベースを更新することができます。そのような操作が第三者によって行われるのを防ぐためには、サイトを停止する必要があります。だれが  *canEdit* ロールに追加されているかは、データベースを調べることによって確認できます。
...
1. **サーバー エクスプローラー**で **[Web アプリ]** に移動します。
...
2. そのアカウントにログインした状態でデータを編集できることを確認します。
 	**注:** このアプリから Google をログアウトし、同じブラウザーで異なる Google アカウントにログインすることはできません。1 つのブラウザーを使用している場合は、Google に移動してログアウトする必要があります。別のブラウザーを使用すると、同じサード パーティの認証子からの別のアカウント (Google など) でログオンすることができます。
...
**注:****SQL データベース**を展開できず、Visual Studio から **ContactDB** が表示されない場合、次の手順に従って、ファイアウォール ポートまたはポートの範囲を開く必要があります。「**Azure のファイアウォール ルールを設定する**」の指示に従ってください。ファイアウォール ルールを追加した後、データベースにアクセスするまでに数分待機する必要が生じることがあります。
...
4.	[Create an ASP.NET MVC 5 App with Facebook and Google OAuth2 (Facebook や Google の OAuth2 で、ASP.NET MVC 5 アプリを作成する)](http://www.asp.net/mvc/tutorials/mvc-5/create-an-aspnet-mvc-5-app-with-facebook-and-google-oauth2-and-openid-sign-on ) これには、ユーザー登録データベースにプロファイル データを追加する方法に関する指示と、認証プロバイダーとしての Facebook の使用に関する詳細な指示も含まれています。
...
*** チュートリアルだけでなく、その対象製品に関しても、気に入った点と、改善して欲しい点に関して、フィードバックをお寄せください。**お寄せいただいたご意見は、今後の改善に役立たせていただきます。新しいトピックについては、「[Show Me How With Code (コードの使用方法)](http://aspnet.uservoice.com/forums/228522-show-me-how-with-code)」で要求および投票することもできます。
