<properties
	pageTitle="チュートリアル: PowerApps と Logic Flows で ASP.Net Web アプリを使用してカスタム API を作成する | Microsoft Azure"
...
2. 次の画面で Web API テンプレートを選択し、**[認証なし]** を選択します。![](./media/powerapps-web-api-tutorial/noauth.png "承認なし")
	...
	>[AZURE.IMPORTANT] 認証を [認証なし] に設定したことを確認してください。
...
7. ***https://\<azure-webapp-url>/swagger/docs/v1*** に移動して、swagger json を抽出します。
	...
	> [AZURE.IMPORTANT] operation-id が重複している swagger ドキュメントは無効です。サンプルの C# テンプレートを使用した場合、operation-id "Values\_Get" が 2 回繰り返されます。片方のインスタンスを "Value\_Get" に変更してください。
...
>[AZURE.IMPORTANT] 両方のアプリは、同じディレクトリに配置される必要があります。
