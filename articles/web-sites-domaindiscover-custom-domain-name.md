...
この記事では、[DomainDiscover.com](https://domaindiscover.com) から購入したカスタム ドメイン名を Azure Websites で使用する手順を示します。DomainDiscover.com は TierraNet のサービスの 1 つです。
...
5. 次のページで、DNS レコードの値を入力します。その後、**[Add]** をクリックします。
    ...
    *  A レコードを追加する場合は、まず **[Manage DNS]** ページで **[A]** を選択する必要があります。その後、**[Host]** フィールドを、**@** (**contoso.com** などのルート ドメイン名) か、使用するサブドメイン (**www** など) のいずれかに設定します。**[IP Address]** フィールドを、Azure Website の IP アドレスに設定する必要があります。その後、Time-to-Live (TTL) の値を 1,800 (秒) などに設定します。
		...
		> 
		> * **&lt;yourwebsitename&gt;.azurewebsites.net**の **Alias Hostname** の値を使用した **www** の **Host** の値。
		> 
		> または
		> 
		> * **awverify.&lt;yourwebsitename&gt;.azurewebsites.net**の **Alias Hostname** の値を使用した **awverify.www** の **Host** の値。
		> 
