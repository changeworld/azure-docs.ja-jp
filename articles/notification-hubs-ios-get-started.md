...
<properties urlDisplayName="Get Started" pageTitle="Azure Notification Hubs の使用" metaKeywords="" description="Learn how to use Azure Notification Hubs to push notifications." metaCanonical="" services="notification-hubs" documentationCenter="Mobile" title="Get started with Notification Hubs" authors="yuaxu" solutions="" manager="dwrede" editor="" />
...
1. キーチェーンアクセス で、Quickstart アプリケーションの新しい証明書 **[自分の証明書]** を右クリックします。**[書き出す]** をクリックし、ファイルに名前をつけて、**[.p12]** 形式を選択します。次に、**[保存]** をクリックします。
...
2. [Azure の管理ポータル] にログオンし、画面下部にある **[+ 新規]** をクリックします。
...
5. 前の手順で作成した名前空間 (通常は ***通知ハブ名*-ns**) をクリックし、上部にある **[構成]** をクリックします。
