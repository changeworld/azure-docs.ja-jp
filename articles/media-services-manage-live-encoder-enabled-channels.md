...
省略可能。コマーシャルによる中断期間に既定のスレート イメージに切り替えて受信ビデオ フィードを非表示にするようにライブ エンコーダーに信号通知します。スレートの際には、オーディオもミュートされます。既定値は **false** です。
...
**注**: チャネルを作成する前に、最大解像度 1920x1080、JPEG 形式、最大サイズ 3 MB のスレート イメージを専用アセットとしてアップロードする必要があります \(他のファイルはこのアセットに含めないでください\)。ファイル名の拡張子は *.jpg にして、この AssetFile を対象アセットのプライマリ ファイルとしてマークする必要があります。このアセットを暗号化ストレージにすることはできません。
...
- [コンテンツ配信の概要](media-services-deliver-content-overview.md)
