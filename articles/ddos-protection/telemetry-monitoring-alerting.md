...
DDoS 攻撃をシミュレートしてテレメトリを検証するには、「[DDoS 検出を検証する](test-through-simulations.md)」をご覧ください。
