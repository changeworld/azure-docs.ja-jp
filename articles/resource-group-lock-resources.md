...
管理者は、サブスクリプション、リソース グループ、またはリソースにロックを適用し、組織の他のユーザーが誤って重要なリソースを削除するのを防止しなければならないことがあります。ロックを適用すると、承認済みのユーザーは引き続きリソースの読み取りと変更を実行できますが、リソースを削除することはできません。

ロールベースのアクセス制御とは異なり、管理ロックでは、すべてのユーザーとロールに制限が適用されます。ユーザーとロールのアクセス許可の設定については、[Azure のロールベースのアクセス制御](./active-directory/role-based-access-control-configure.md)に関するページをご覧ください。
