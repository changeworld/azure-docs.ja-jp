...
> [!NOTE]
> 拡張機能は、[こちらのリンク](https://visualstudiogallery.msdn.microsoft.com/e25ad307-46cf-412e-8ba5-5b555d53d2d9)から直接ダウンロードすることもできます。
> 
> 
...
3. **[ロジック アプリ]** テンプレートを選択します。ロジック アプリのひな形となる空のデプロイ テンプレートが作成されます。![Azure テンプレートの選択](./media/app-service-logic-deploy-from-vs/selectazuretemplate.png)
...
4. これでロジック アプリを作成して設計できる状態になりました。変更はデプロイ テンプレートに反映されます。 ![Designer in Visual Studio](./media/app-service-logic-deploy-from-vs/designer_in_vs.png)
...
3. ここでは、ロジック アプリをデプロイするリソース グループの詳細を選択する必要があります。![リソース グループへのデプロイ](./media/app-service-logic-deploy-from-vs/deploytoresourcegroup.png)
   ...
   > 
4. [デプロイ] ボタンをクリックします
5. デプロイの状態が **[出力]** ウィンドウに表示されます (**[Azure のプロビジョニング]** の選択が必要になる場合があります)。 ![出力](./media/app-service-logic-deploy-from-vs/output.png)
