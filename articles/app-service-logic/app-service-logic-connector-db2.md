---
title: Microsoft Azure App Service での DB2 コネクタの使用 | Microsoft Docs
...
> [!TIP]
> DB2 接続 "*EntitySet への投稿*" では ID 列の値が返され、"*API の挿入*" では処理された行が返されます。
> 
> 
...
> [!TIP]
> 次の SQL ステートメントを使用して、サンプルのテーブルおよびストアド プロシージャを作成します。
> 
> 
...
## ハイブリッド構成 (省略可能)
> [!NOTE]
> この手順は、ファイアウォールの内側にあるオンプレミスの DB2 コネクタを使用する場合にのみ、実行する必要があります。
> 
> 
