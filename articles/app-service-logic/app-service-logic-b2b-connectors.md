---
title: Logic Apps の企業間コネクタと API Apps | Microsoft Docs
...
> [!NOTE]
> BizTalk 取引先管理 (TPM) インスタンスは、AS2 コネクタ、X12 API アプリ、または EDIFACT API アプリを作成するために必要です。TPM API アプリを作成すると、TPM インスタンスが作成されます。
> 
> 
...
> [!IMPORTANT]
> 以前に作成したコネクタおよび API Apps を追加するには、同じリソース グループに Logic Apps、Mobile Apps、または Web Apps を作成します。
> 
> 
...
3. **ギャラリー**から B2B API アプリを選択します。選択した B2B API アプリがアプリに自動的に追加されます。新しい B2B API アプリを作成することもできます。
   ...
   > [!IMPORTANT]
   > AS2 コネクタ、X12 API Apps、EDIFACT API Apps には TPM インスタンスが必要です。そのため、新しい B2B API Apps を作成する場合は、最初に TPM API アプリを作成し、次に AS2 コネクタ、X12 API アプリ、または EDIFACT API アプリを作成します。
   > 
   > 
4. **[OK]** を選択して変更を保存します。
