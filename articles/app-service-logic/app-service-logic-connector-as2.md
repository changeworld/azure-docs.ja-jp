...
1. ロジック アプリを作成または編集する場合は、作成した AS2 コネクタを右側のウィンドウから選択します。<br/>![Trigger settings][3]

2. 右矢印 (→) をクリックします。<br/> ![トリガー オプション][4]

3. AS2 コネクタでは 1 つのトリガーが表示されます。*[受信およびデコード]* を選択します。<br/> ![入力の受信およびデコード][5]
...
3. AS2 のコネクタはアクションを 1 つのみサポートします。*[エンコードおよび送信]* を選択します。<br/> ![入力のエンコードおよび送信][9]
...
パラメーター | 型 | 説明
--- | --- | ---
Payload | オブジェクト| エンコードし、構成済みのエンドポイントに投稿するペイロードの内容。ペイロードは、JSON オブジェクトとして指定する必要があります。
AS2 From | 文字列 | AS2 メッセージの送信元の AS2 の ID。このパラメーターは、メッセージを送信するための適切な契約の参照に使用されます。
AS2 To | 文字列 | AS2 メッセージの受信側の AS2 の ID。このパラメーターは、メッセージを送信するための適切な契約の参照に使用されます。
Partner URL | 文字列 | 送信する必要があるメッセージの宛先となるパートナーのエンド ポイント。
Enable Archiving | ブール値 | 送信メッセージをアーカイブする必要があるかどうかを判断します。
