...
OneDrive コネクタを使用するには、まず OneDrive コネクタ API アプリのインスタンスを作成する必要があります。これは、ロジック アプリ デザイナーから直接でも、ロジック アプリ デザイナーの外からでも実行できます。デザイナーの外からインスタンスを作成する場合の手順は次のとおりです。
...
1.	新しいロジック アプリを作成し、OneDrive コネクタと同じリソース グループを選択します。詳細については、[新しいロジック アプリの作成]に関するページをご覧ください。

2.	作成したロジック アプリで [トリガーとアクション] を選択して Logic Apps デザイナーを開き、フローを構成します。

3.	OneDrive コネクタが右側のギャラリーの [このリソース グループの API アプリ] セクションに表示されます。
...
9. API の詳細については、[OneDrive API の定義]に関するページを参照してください。
