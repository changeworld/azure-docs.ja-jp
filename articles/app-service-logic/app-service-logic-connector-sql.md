...
--- | --- | ---
WHERE 句 | <ul><li>演算子: AND、OR、=、<>、<、<=、>、>=、および LIKE</li><li>複数のサブ条件は '(' および ')' で結合できます</li><li>文字列リテラル、日付と時刻 (一重引用符で囲む)、数値 (数字のみを含める必要があります)</li><li>((オペランド 演算子 オペランド) AND/OR (オペランド 演算子 オペランド))* のように厳密なバイナリ式形式にする必要があります*</li></ul> | <ul><li>演算子: Between、IN</li><li>ADD()、MAX() NOW()、POWER() などのすべての組み込み関数</li><li>*、-、+ などの算術演算子</li><li>+ を使用した文字列の連結</li><li>すべての結合</li><li>IS NULL と IS NOT Null</li><li>16 進数などの数値以外の文字を含む任意の数字</li></ul>
フィールド (選択クエリ内) |<ul><li>コンマで区切られた有効な列名。テーブル名のプレフィックスは許可されていません (コネクタは、一度に 1 つのテーブルで動作します)。</li><li>'[' と ']' を使用して名前をエスケープすることができます</li></ul> | <ul><li>TOP、DISTINCT などのキーワード</li><li>Street + City + Zip AS Address のようなエイリアス化</li><li>ADD()、MAX() NOW()、POWER() などのすべての組み込み関数</li><li>*、-、+ などの算術演算子</li><li>+ を使用した文字列の連結</li></ul>
