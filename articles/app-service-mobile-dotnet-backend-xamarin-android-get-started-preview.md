...
5. **[サービスのダウンロードとクラウドへの発行]** で **[ダウンロード]** をクリックします。
  	...
  	これにより、モバイル アプリ バックエンド コードと、モバイル アプリ バックエンドに接続されているサンプル _To do list_ クライアント アプリケーションの両方のプロジェクトが含まれるソリューションがダウンロードされます。圧縮されたプロジェクト ファイルをローカル コンピューターに保存し、保存場所を書き留めておいてください。
...
3. アプリケーションで、意味のあるテキスト (たとえば、「チュートリアルの完了」) を入力し、プラス (**+**) アイコンをクリックします。
	...
	> [AZURE.NOTE]
   	> モバイル アプリ バックエンドにアクセスして ToDoActivity.cs C# ファイルにあるデータを照会および挿入するコードを確認できます。
