...
> [!TIP]
> Azure AD からは、登録された各アプリケーションに**[サービス プリンシパル](/azure/active-directory/develop/developer-glossary#service-principal-object)** が提供されます。これはアプリケーションの ID として機能します。 このサービス プリンシパルには、ロールベースのアクセス制御 (RBAC) を介して、Key Vault などの他の Azure リソースにアクセスするための承認を与えることができます。 Key Vault は Microsoft アプリケーションなので、すべての Azure AD テナントでアプリケーション ID "cfa8b339-82a2-471a-a3c9-0fc0be7a4093" 以下に事前登録されています。
...
Key Vault が既にストレージ アカウントのロールに追加されている場合は、*"ロールの割り当ては既に存在します"* というエラーが発生します。 Azure portal でストレージ アカウントの [アクセス制御 (IAM)] ページを使用して、ロールの割り当てを確認することもできます。  
