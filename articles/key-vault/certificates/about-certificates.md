...
## <a name="certificate-use-cases"></a>証明書のユースケース

### <a name="secure-communication-and-authentication"></a>セキュリティで保護された通信と認証

TLS 証明書は、インターネット経由の通信を暗号化し、Web サイトの ID を確立するのに役立ちます。これにより、エントリポイントと通信モードがセキュリティで保護されます。  さらに、公開 CA によって署名されたチェーン証明書は、証明書を保持しているエンティティが本人であることを確認するのに役立ちます。 例として、証明書を使用して通信をセキュリティで保護し、認証を有効にする優れたユースケースを次に示します。
* イントラネットまたはインターネット Web サイト: TLS 証明書を使用して、イントラネット サイトへのアクセスを保護し、インターネット経由で暗号化されたデータ転送を保証します。
...
証明書は、ソフトウェアのコードやスクリプトをセキュリティで保護するのに役立ちます。これにより、作成者は、悪意のあるエンティティによって変更されることなく、ソフトウェアをインターネット経由で共有できます。 さらに、作成者がコード署名テクノロジを利用した証明書を使用してコードに署名すると、ソフトウェアには、作成者とその Web サイトを表示する認証スタンプでマークされます。 そのため、コード署名で使用される証明書は、ソフトウェアの信頼性を検証し、エンドツーエンドのセキュリティを促進するのに役立ちます。
