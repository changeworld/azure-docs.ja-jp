...
このクイック スタートでは、Azure Web アプリケーションを取得し、マネージド サービス ID を使用して Azure Key Vault から情報を読み取るために必要な手順を学習します。 学習内容は次のとおりです。
...
>
>Azure で実行されるアプリケーションである[マネージド サービス ID](../active-directory/managed-service-identity/overview.md) では、Azure によって自動的に管理される ID が提供されます。 これにより、*シークレット導入問題*が解決されます。ユーザーとアプリケーションはベスト プラクティスに従うことができ、最初のシークレットのローテーションについて心配する必要がありません
