---
title: すべての Azure Key Vault で論理的な削除を有効にする | Microsoft Docs
...
キー コンテナーの名前はグローバルに一意です。 キー コンテナーに格納されているシークレットの名前も一意です。 論理的に削除された状態であるキー コンテナー名またはキー コンテナー オブジェクトを再利用することはできません。 

**例 #1** アプリケーションによってプログラムで "Vault A" という名前のキー コンテナーが作成され、その後 "Vault A" が削除されたとします。 キー コンテナーは、論理的に削除された状態になります。 アプリケーションは、このキー コンテナーが論理的に削除された状態から消去されるまで、"Vault A" という名前の別のキー コンテナーを再作成できません。 

**例 #2** アプリケションがキー コンテナー A に `test key` という名前のキーを作成し、その後コンテナー A からキーを削除した場合、アプリケーションは、`test key` オブジェクトが論理的に削除された状態から消去されるまで、キー コンテナー A に `test key` という名前の新しいキーを作成できません。 

このため、キー コンテナー オブジェクトを削除し、それを論理的に削除された状態から消去せずに同じ名前で再作成しようとすると、競合エラーが発生する可能性があります。 これにより、アプリケーションまたはオートメーションで障害が発生する可能性があります。 以下の必要なアプリケーションの変更と管理の変更を行う前に、開発チームと相談してください。 
...
5. "キー コンテナーで論理的な削除が有効になっている必要がある" ポリシーを選択します。
6. [割り当て] をクリックします。
7. スコープをサブスクリプションに設定します。
8. ポリシーの効果が "監査" に設定されていることを確認します。
9. [確認と作成] を選択します。
10. 環境のフル スキャンが完了するまでに最大 24 時間かかる場合があります。
11. Azure Policy ブレードで、[コンプライアンス] をクリックします。
12. 適用したポリシーを選択します。
...
アプリケーション ロジックを変更する必要がないか確認してください。 これを確認した後、すべてのキー コンテナーで論理的な削除を有効にします。
