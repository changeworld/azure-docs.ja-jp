...
詳細については、「[非同期メッセージング パターンと高可用性][]」の「Azure データセンター内での Service Bus の障害」のセクションを参照してください。
...
[Service Bus リレー メッセージを使用した geo レプリケーション][]のサンプルでは、リレーをレプリケートする方法を説明しています。
...
[Service Bus の仲介型メッセージを使用した geo レプリケーション][]のサンプルでは、メッセージング エンティティのアクティブ レプリケーションについて説明しています。
...
[Service Bus の仲介型メッセージを使用した geo レプリケーション][]のサンプルでは、メッセージング エンティティのパッシブ レプリケーションについて説明しています。

## 永続的なクライアント側キュー

アプリケーションで Service Bus エンティティが使用不可になることは許容できるが、メッセージを失うことは許容できない場合、送信側は、Service Bus に送信できないすべてのメッセージをローカルに格納する永続的なクライアント側キューを使用できます。Service Bus エンティティが再び使用可能になると、バッファー内のすべてのメッセージがそのエンティティに送信されます。[永続的なメッセージ送信側][]のサンプルでは、MSMQ を利用してこのようなキューを実装しています。代わりに、メッセージをローカル ディスクに書き込むこともできます。
...
- [Azure のビジネス継続性テクニカル ガイダンス][]
  ...
  [非同期メッセージング パターンと高可用性]: https://msdn.microsoft.com/library/azure/dn292562.aspx
  ...
  [Azure SQL Database の継続性]: https://msdn.microsoft.com/library/azure/hh852669.aspx
