...
> [AZURE.NOTE] このサンプルは、IaaS シナリオにも適しています。IaaS シナリオでは、ローカル ディスクまたは MSMQ 用ディスクはストレージ アカウントにマッピングされ、メッセージは MSMQ によって安全に保存されます。ただし、Cloud Services や Web アプリケーションなどの PaaS シナリオには適していません。
