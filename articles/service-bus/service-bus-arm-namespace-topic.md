<properties
    pageTitle="トピックとサブスクリプションを含んだ Service Bus 名前空間を作成する | Microsoft Azure"
    description="トピックとサブスクリプションを含んだ Service Bus 名前空間を ARM テンプレートで作成する"
...
# トピックとサブスクリプションを含んだ Service Bus 名前空間を ARM テンプレートで作成する

この記事では、Azure Resource Manager (ARM) テンプレートを使用して、トピックとサブスクリプションを含んだ Service Bus の名前空間を作成する方法について説明します。さらに、デプロイメント対象のリソースを定義する方法と、デプロイの実行時に指定されるパラメーターを定義する方法について説明します。このテンプレートは、独自のデプロイに使用することも、要件に合わせてカスタマイズすることもできます。
...
トピックとサブスクリプションは、*発行とサブスクライブ*のパターンで一対多の形式の通信を実現します。

[Service Bus のトピックとサブスクリプションの詳細については、こちらを参照してください。][]

デプロイメントを自動的に実行するには、次のボタンをクリックします。

[![Azure へのデプロイ](./media/service-bus-arm-namespace-topic/deploybutton.png)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FAzure%2Fazure-quickstart-templates%2Fmaster%2F201-servicebus-create-topic-and-subscription%2Fazuredeploy.json)
