<properties title="How to Configure PHP in Azure Websites" pageTitle="Azure Websites での PHP の構成方法" metakeywords="Azure, Azure の Web サイト, 構成, PHP" description="Learn how to configure the default PHP installation or add a custom PHP installation in Azure Websites." services="Web Sites" documentationCenter="PHP" authors="cephalin" manager="wpickett" />
...
既定では、Azure Web サイトを作成すると PHP 5.4 がインストールされ、直ちに使用できる状態になります。使用可能なリリース リビジョン、既定の構成、および有効な拡張機能を確認するには、[phpinfo()] 関数を呼び出すスクリプトをデプロイします。
...
1. 'bin' ディレクトリに、'.dll' ファイル ('php_mongo.dll' など) を配置します。拡張機能は、PHP の既定バージョン (この文書の作成時点では PHP 5.4) との互換性があり、VC9 および非スレッドセーフ (nts) 互換であることを確認してください。
1. アプリケーションをデプロイします。
