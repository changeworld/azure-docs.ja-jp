...
Digital Twin API には、**Get Digital Twin**、**Update Digital Twin**、**Invoke Component Command**、および **Invoke Command** の各操作が含まれ、デジタル ツインをさらに管理できます。 [REST API](/rest/api/iothub/service/digitaltwin) は直接使用することも、[サービス SDK](../iot-develop/libraries-sdks.md) を通じて使用することもできます。
