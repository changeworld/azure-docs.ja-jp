...
これらの条件下で、*最初の実行*2015-04-09 を 14時 00分になります。\. Scheduler エンジンは、開始時刻から実行を計算します。  過去のインスタンスはすべて破棄されます。 エンジンは、将来発生する次回のインスタンスを使用します。  したがって、この場合は、*startTime* が 2015 年 4 月 7 日午後 2 時 00 分となり、次回のインスタンスは 2 日後、つまり 2015 年 4 月 9 日午後 2 時 00 分となります。

最初の実行が、注も startTime 2015-04-05 をする 14時 00分や 2015-04-01 14:00\ です。 最初の実行後に後続の実行を使用して計算 – スケジュールされたので、2015-04-11 で、2015-04-13 午後 2 時に、2時 00分 pm にし、2015-04-15 午後 2 時などです。
...
すべてのスケジュールがあると想定、*間隔*1 に設定されています。\. また、1 つ必要がありますと仮定適切な頻度内に何がに従って、*スケジュール*– など、1 つは頻度「日」を使用してならず、「月の日にち」変更、スケジュールでします。 そのような制限は、上で説明しています。
