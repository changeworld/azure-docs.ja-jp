...
このセクションでは、 `QSTodoService.cs` のオフライン同期に関するコードについて説明します。

1. Visual Studio で、「[モバイル サービスの使用]」チュートリアルでダウンロードしたプロジェクトを開きます。ファイル `QSTodoService.cs` を開きます。
...
3. テーブル操作を実行する前に、ローカル ストアを初期化する必要があります。この操作はメソッド `InitializeStoreAsync` で次のように実行します。
...
5. クラス `QSTodoService` では、メソッド `SyncAsync()` は、データを変更する操作である `InsertTodoItemAsync()` と `CompleteItemAsync` の後に呼び出されます。また、 `RefreshDataAsync()` からも呼び出されるため、ユーザーは、更新ジェスチャを実行するたびに最新のデータを取得します。 `QSTodoListViewController.ViewDidLoad()` は `RefreshDataAsync()` を呼び出すため、アプリケーションは起動時に同期も実行します。
...
1.  `QSTodoService.cs` を開きます。メソッド `InsertTodoItemAsync()` と  `CompleteItemAsync()` を編集し、 `SyncAsync()` への呼び出しをコメント アウトします。
...
3. アプリケーションの起動時に同期されないように UI の初期化を変更するには、メソッド `QSTodoListViewController.ViewDidLoad()` を編集します。このメソッドの末尾にある `RefreshAsync()` への呼び出しを削除し、 `ReloadData()` への呼び出しに置き換えます。
...
4. アプリケーションで、項目の一覧をプルダウンして更新操作を実行します。これにより、アプリケーションは `RefreshDataAsync()` を呼び出し、その結果  `SyncAsync()` が呼び出されます。これによってプッシュとプルの処理が実行されます。まず、ローカル ストアの項目がモバイル サービスに送信され、次に、新しいデータがサービスから取得されます。
...
<!-- URLs. -->
[Mobile Services のオフライン サポートでの競合の処理]に関するページ: /ja-jp/documentation/articles/mobile-services-xamarin-ios-handling-conflicts-offline-data/ 
