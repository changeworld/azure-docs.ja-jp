<properties urlDisplayName="Command-Line Tools" pageTitle="Mac および Linux 用 Azure コマンド ライン ツール" metaKeywords="Azure command-line, Azure tools Mac, Azure tools Linux" description="Learn about using the command-line tool for Mac and Linux in Azure." metaCanonical="" services="web-sites,virtual-machines,mobile-services,cloud-services" documentationCenter="" title="" authors="carolz" solutions="" manager="need to identify contact" editor="" />
...
このコマンドは、インポート済みの保存されている発行設定を削除します。このマシンでのツールを使用を完了し、ツールによってアカウントが今後使われないようにする場合に、このコマンドを使用します。
...
仮想マシン イメージは、仮想マシンの状態が **Stopped** の場合にのみ取得できます。
...
仮想マシンへの外部要求は、ロード バランサーを経由します。そのため、複数の仮想マシンの展開においては、特定の仮想マシンに対する要求は指定できません。複数の仮想マシンがあるデプロイについては、仮想マシン (vm-port) とロード バランサー (lb-port) 間のポート マッピングを構成する必要があります。
...
データ ディスクとは、仮想マシンが使用できる、BLOB ストレージ内の .vhd ファイルです。BLOB ストレージへのデータ ディスクの展開に関する詳細については、前記の Azure 技術解説図を参照してください。 

データ ディスクの接続用コマンド (azure vm disk attach および azure vm disk attach-new) は、SCSI で求められているように、接続されるデータ ディスクに論理ユニット番号 (LUN) を割り当てます。仮想マシンに接続されている最初のデータ ディスクには LUN 0、次のデータ ディスクには LUN 1 というように順に割り当てられます。
...
このコマンドは、証明書をアップロードします。パスワードで保護されていない証明書の場合は、パスワードの入力ダイアログで空白のままにします。
...
このコマンドは、Websites の [name] の構成オプションを設定します。
...
+ **-r `<sqlServer>`**   または **--sqlServer `<sqlServer>`**:   `<sqlServer>` として指定されている、既存の SQL データベース サーバーを使用します。
...
このコマンドは、`error` 以外のすべてのログをフィルターで除いたモバイル サービス ログを返します。
...
+ **-t `<type>`** または **--type `<type>`**:  `<type>`エントリにより返されるログをフィルター処理します。エントリは `information`、`warning`、または `error` です。
+ **-k `<skip>`** または **--skip `<skip>`**: `<skip>` で指定された数の行をスキップします。
+ **-p `<top>`** または **--top `<top>`**: `<top>` で指定された数の行を返します。

<div class="dev-callout"><b>注</b>
   <p><b>--query</b> パラメーターが <b>--type</b>、<b>--skip</b>、および <b>--top</b> よりも優先されます。</p>
</div>
...
+ **-p `<permissions>`** または **--permissions `<permissions>`**: `<operation>`=`<permission>` のペアのコンマ区切りリスト。`<operation>` には `insert`、`read`、`update`、または `delete` のいずれかを、`<permissions>` は `public`、`application` (既定)、`user`、または `admin` のいずれかをそれぞれ指定できます。
...
ファイル名はテーブル名と操作名で構成し、ファイルは、コマンドを実行する場所のサブフォルダーに配置する必要があります。**-f `<file>`** パラメーターまたは **--file `<file>`** パラメーターを使用して、登録する別のファイル名を指定したり、そのファイルへのパスを指定することもできます。
...
<div class="dev-callout"><b>注</b>
   <p>コンピューティング モードを "`占有`" に設定すると、同じリージョンのモバイル サービスすべてがプレミアム モードで実行されます。</p>
</div>  
...
**-p** または **--permissions** <permissions>:  method>=< と <permission> のコンマ区切りリストのペア。
...
+ **-p** または **--permissions** <permissions>: <method>=<permission>   のコンマ区切りリストのペア。
...
+ **-d** または **--destination** [destination]: ダウンロードの転送先ファイルまたはディレクトリのパス。
