...
> [AZURE.NOTE]このデータセットにフィルターを提供し、Free 価格レベルのドキュメントを 10,000 件未満に制限しました。Standard レベルを使用する場合は、この制限は適用されません。各価格レベルの容量の詳細については、「[制限および制約](search-limits-quotas-capacity.md)」を参照してください。
