...
| ファセット ナビゲーション | [ファセット ナビゲーション](search-faceted-navigation.md) ツリーでは、ユーザーがファセットを選択して、クリックするたびに結果を絞り込むことができます。 条件に一致しなくなったドキュメントを除外するフィルターによって、各ファセットがサポートされます。 |
