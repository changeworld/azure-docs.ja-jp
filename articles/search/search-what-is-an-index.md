...
1. [Get Index REST API](/rest/api/searchservice/get-index) や、[Postman](search-get-started-rest.md) などの Web テスト ツールを使用してインデックス スキーマをダウンロードします。 これで、コードに対して適応できるインデックスの JSON 表現ができました。
