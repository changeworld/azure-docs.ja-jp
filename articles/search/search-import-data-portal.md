...
Azure ポータルでは、データをインデックスに読み込む**データのインポート** コマンドが Azure Search ダッシュボードに用意されています。このコマンドを実行すると、組み込みのインデクサー機能によって既存のデータ ソースがクロールされ、データ ソースからプルされた行セットに基づいてドキュメントが作成されて、アップロードされます。
...
再構築する必要のない編集には、新しいフィールドの追加、スコアリング プロファイルの変更、サジェスターの変更、言語アナライザーの変更があります。詳細については、「[Update Index (インデックスの更新)](https://msdn.microsoft.com/library/azure/dn800964.aspx)」を参照してください。
