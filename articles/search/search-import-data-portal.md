...
Azure Search を初めて使用する場合は、 [ポータル ツールを使用するインポート、インデックス作成、クエリの実行に関するクイック スタート](search-get-started-portal.md)をステップ実行し、**データのインポート**および組み込みの不動産サンプル データ セットを使用して、インポートとインデックス作成を試してください。
