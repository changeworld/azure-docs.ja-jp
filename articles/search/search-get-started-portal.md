---
title: "Azure Search の使用 | Microsoft Docs"
...
---
# <a name="get-started-with-azure-search-in-the-portal"></a>ポータルから Azure Search を使用する
...
> [!NOTE]
> この入門チュートリアルには [Azure サブスクリプション](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A261C142F)と [Azure Search サービス](search-create-service-portal.md)が必要です。 
> 
> 
...
## <a name="create-an-index-and-load-data"></a>インデックスの作成とデータの読み込み
...
#### <a name="step-1-define-the-data-source"></a>手順 1: データ ソースを定義する
...
3. 検索ボックスを空にした状態で **[検索]** ボタンをクリックすると、すべてのドキュメントを返すワイルドカード検索が実行されます。
4. フルテキスト検索クエリをいくつか入力します。 ワイルドカード検索の結果を見て、検索対象となるアーティスト、アルバム、ジャンルを確認できます。
5. [この記事の最後に紹介されている例](https://msdn.microsoft.com/library/azure/dn798927.aspx) を参考にクエリを調整し、インデックスに存在しそうな検索文字列を使用しながら、他のクエリ構文を試すなどして応用してみてください。
...
> [!NOTE]
> インデクサーが新たに Azure Blob Storage のクロールに対応した旨が発表されましたが、これはプレビュー段階の機能であり、ポータルから利用することはできません。 そのインデクサーを試すには、コードを記述する必要があります。 詳細については、「 [Azure Blob Storage 内ドキュメントのインデックスを Azure Search で作成する](search-howto-indexing-azure-blob-storage.md) 」を参照してください。
> <a id="apdx-sampledata"></a>
> 
> 

## <a name="appendix-create-sample-data-in-documentdb"></a>付録: DocumentDB でサンプル データを作成する
...
### <a name="create-musicstoredb-for-this-tutorial"></a>このチュートリアル用の musicstoredb を作成する
...
3. **[データベースの追加]** をクリックして、`musicstoredb` という ID の新しいデータベースを作成します。 作成後にデータベースのタイル (ページの下の方) に表示されます。
4. データベース名をクリックして、データベース ブレードを開きます。
5. **[コレクションの追加]** をクリックして、`musicstorecoll` という ID のコレクションを作成します。
6. **[ドキュメント エクスプローラー]**をクリックします。
7. **[アップロード]**をクリックします。
8. **[ドキュメントのアップロード]**で、前にダウンロードした JSON ファイルがあるローカル フォルダーに移動します。 100 個以下の JSON ファイルを一括で選択します。
   * 386.json
   * 387.json
   * に関するページを参照してください。 が必要です。 に関するページを参照してください。
   * 486.json
9. 最後のファイルである 669.json をアップロードするまで、次のファイルのバッチを繰り返し取得します。
10. **[クエリ エクスプローラー]** をクリックし、ドキュメント エクスプローラーのアップロード要件を満たすようにデータがアップロードされていることを確認します。
