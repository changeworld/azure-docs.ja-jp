...
インデックスはいくつかの方法で作成できます。Azure SQL Database、Azure VM 上の SQL Server、DocumentDB など、Azure Search でクロールできるデータ ストアにデータがあれば、*インデクサー*を使用してごく簡単にインデックスを作成し、データを投入することができます。
...
5. **[ドキュメントの追加]** で、100 個以下の JSON ファイルを一括でアップロードします。
	- 386\.json
	- 387\.json
	- . . .
6. **[クエリ エクスプローラー]** をクリックし、ドキュメント エクスプローラーのアップロード要件を満たすようにデータがアップロードされていることを確認します。

このデータセット内のファイル数は合計 300 個未満であるため、上位 300 件を選択するように既定のクエリを変更するか、`select * from musicstorecoll` というクエリを作成して **[クエリの実行]** をクリックすると、この手順を簡単に実行できます。

ドキュメント番号 386 から始まり 669 で終わる JSON 出力が返されます。データの読み込みが完了したら、[このチュートリアルの手順に戻り](#defineDS)、**インポート ウィザード**を使用してインデックスを作成します。
