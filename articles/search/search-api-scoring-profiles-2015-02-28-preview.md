...
| `distance:referencePointParameter` | 参照場所として使用するためにクエリに渡されるパラメーター。scoringParameter はクエリ パラメーターです。クエリ パラメーターの説明については、「[ドキュメントの検索](search-api-2015-02-28-preview/#SearchDocs)」を参照してください。 |
...
| `tag:tagsParameter` | 特定の要求のタグを指定するためにクエリに渡されるパラメーター。`scoringParameter` はクエリ パラメーターです。クエリ パラメーターの説明については、「[ドキュメントの検索](search-api-2015-02-28-preview/#SearchDocs)」を参照してください。 |
