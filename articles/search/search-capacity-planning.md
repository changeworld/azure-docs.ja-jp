...
[Basic レベル](http://aka.ms/azuresearchbasic)または [Standard レベル](search-limits-quotas-capacity.md)のいずれかでサービスを設定すると、リソース構成を使用できます。 このレベルの課金対象のサービスについては、容量は*検索単位* (SU) ごとに購入します。各パーティションとレプリカは&1; SU としてカウントします。 
