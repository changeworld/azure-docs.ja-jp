...
容量は、[サービス レベル](search-sku-tier.md)の機能です。 各レベルでは、最大ストレージ、パーティションごとのストレージ、および作成できるオブジェクトの上限数に違いがあります。 Basic レベルは、ストレージ要件が小さい (1 つのパーティションのみ) が、高可用性構成 (3 つのレプリカ) で実行できるアプリ向けに設計されています。 その他のレベルは、マルチテナント機能などの特定のワークロードまたはパターン向けに設計されています。 内部的には、これらのレベルで作成されたサービスは、こうしたシナリオを支援するハードウェアの影響を受けます。
...
| **12 レプリカ** |12 SU |24 SU |36 SU |N/A |なし |該当なし |
