<properties
	pageTitle="Azure Search の容量計画 | Microsoft Azure | ホステッド クラウド検索サービス"
	description="Azure Search の容量は、それぞれが請求可能な検索単位である、パーティションとレプリカから形成されます。"
	services="search"
	documentationCenter=""
	authors="HeidiSteen"
	manager="mblythe"
...
Azure Search では、特定のコンピューティング リソースの容量を段階的に調整できます。クエリを強化してインデックス作成のパフォーマンスを向上させるため、ストレージと IO、またはレプリカがさらに必要な場合は、パーティションを増やすことができます。
...
両方のレベルで、容量は*検索単位* (SU) ごとに購入します。各パーティションとレプリカは 1 SU としてカウントします。

- Basic では、サービスごとに最大 3 SU を提供します。
- Standard では、サービスごとに最大 36 SU を提供します。

レベルの制限を下回っているパーティションとレプリカの組み合わせを選択する必要があります。たとえば、Standard サービスではパーティション 12 個とレプリカ 6 個にまで拡張することはできません。これを行うには 72 の検索単位 (12 x 6) が必要であり、サービスごとの検索単位の上限である 36 を超えてしまうためです。

一般的に、ほとんどの検索アプリケーションで必要なレプリカ数はパーティション数よりも多くなります。詳細については、[高可用性](#HA)に関するセクションをご覧ください。
...
SU あたりのコストはレベルによって決定されます。SU あたりのコストは、Standard レベルより Basic レベルの方が安価です。各レベルの価格は、「[Search の価格](https://azure.microsoft.com/pricing/details/search/)」をご覧ください。
...
<tr><td><b>3 つのレプリカ</b></td><td>3 SU</td></tr>
<tr><td><b>2 つのレプリカ</b></td><td>2 SU</td></tr>
<tr><td><b>1 つのレプリカ</b></td><td>1 SU</td></tr>
<tr><td></td><td><b>1 個のパーティション</b></td></tr>
</table>
