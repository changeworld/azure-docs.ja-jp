...
インデックスの作成を検証してドキュメントをアップロードするには、[Azure クラシック ポータル](https://portal.azure.com)の Search サービス ダッシュボードを開きます。[使用状況] のインデックスは 1 つずつ増えます。データベース内の 1 製品につき 1 個のドキュメントがあるため、合計 294 個のドキュメントがあります。
