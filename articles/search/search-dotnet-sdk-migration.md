<properties
   pageTitle="Azure Search .NET SDK バージョン 1.0.0-preview へのアップグレード | Microsoft Azure | ホスト型クラウド検索サービス"
   description="Azure Search .NET SDK バージョン 1.0.0-preview へのアップグレード"
   services="search"
   documentationCenter=""
...
# Azure Search .NET SDK バージョン 1.0.0-preview へのアップグレード

バージョン 0.13.0-preview 以前の [Azure Search .NET SDK](https://msdn.microsoft.com/library/azure/dn951165.aspx) を使用している場合、この記事を参考にして、最新バージョンの 1.0.0-preview を使用するようにアプリケーションをアップグレードできます。

例を含む SDK の一般的なチュートリアルについては、「[.NET アプリケーションから Azure Search を使用する方法](search-howto-dotnet-sdk.md)」を参照してください。

Azure Search .NET SDK のバージョン 1.0.0-preview には、以前のバージョン (0.13.0-preview) との互換性が維持されない変更がいくつかあります。ほとんどは小さなものなので、コードの変更に必要な作業は最小限で済みます。新しいバージョンの SDK を使用するようにコードを変更する方法については、「[アップグレードの手順](#UpgradeSteps)」を参照してください。

<a name="WhatsNew"></a>
## 1\.0.0-preview の新機能

バージョン 1.0.0-preview の対象となる REST API のバージョンは古いバージョンの Azure Search .NET SDK (2015-02-28) と同じなので、このリリースでの新しいサービス機能はありません。ただし、SDK の上級ユーザー向けの新しいクライアント側機能が 1 つあります。
...
> [AZURE.NOTE]Visual Studio を使用している場合は [プレリリースを含める] を選択することで、パッケージ マネージャー コンソールを使用している場合は `-IncludePrerelease` スイッチを使用して、プレリリース パッケージを参照できるようにします。

NuGet が新しいパッケージとその依存関係をダウンロードした後、プロジェクトをリビルドします。次のようなビルド エラーが表示されます。
...
次のステップとして、ビルド エラーを 1 つずつ修正します。ほとんどの修正では、SDK で名前が変更されたクラスとメソッドの名前を変更する必要があります。このような名前変更の一覧については、「[1\.0.0-preview における互換性が維持されない変更の一覧](#ListOfChanges)」を参照してください。

カスタム クラスを使用してドキュメントをモデル化していて、それらのクラスに null 非許容プリミティブ型のプロパティ (たとえば、C# での `int` や `bool`) がある場合、1.0.0-preview バージョンの SDK で行われたバグ修正について認識しておく必要があります。詳細については、「[1\.0.0-preview でのバグ修正](#BugFixes)」を参照してください。

最後に、ビルド エラーを修正した後は、必要に応じて、新しい機能を利用するようにアプリケーションを変更できます。新しい SDK のカスタム シリアル化機能の詳細については、「[1\.0.0-preview の新機能](#WhatsNew)」を参照してください。

<a name="ListOfChanges"></a>
## 1\.0.0-preview における互換性が維持されない変更の一覧
...
Azure Search .NET SDK の各操作は、同期および非同期の呼び出し元に対するメソッドのオーバーロードのセットとして公開されます。これらのメソッド オーバーロードのシグネチャと要素分解は、バージョン 1.0.0-preview で変更されました。
...
1\.0.0-preview バージョンでの同じ操作のメソッド シグネチャは、次のようになります。
...
バージョン 1.0.0-preview 以降では、Azure Search .NET SDK での操作メソッドの編成が異なります。
...
この問題は、SDK のバージョン 1.0.0-preview で解決されました。次のようなモデル クラスがあるものとします。
