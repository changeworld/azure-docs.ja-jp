...
ms.locfileid: "123537979"
---
# <a name="quickstart-create-a-knowledge-store-in-the-azure-portal"></a>クイック スタート: Azure portal でナレッジ ストアを作成する

ナレッジ ストアは、Azure Cognitive Search の機能です。[AI エンリッチメント パイプライン](cognitive-search-concept-intro.md)からの出力を、その後の分析やダウンストリーム処理に使用できるよう Azure Storage に送信するものです。

エンリッチメント パイプラインでは、非構造化テキストと画像コンテンツが入力され、Cognitive Services によって AI を利用した処理が適用されて、以前は存在しなかった新しい構造と情報が出力されます。 パイプラインによって作成される物理データ構造の 1 つは[ナレッジ ストア](knowledge-store-concept-intro.md)です。これには、Azure Storage に接続する任意のツール、アプリ、またはプロセスを介してアクセスできます。

このクイックスタートでは、データを設定し、**データのインポート** ウィザードを実行して、ナレッジ ストアも生成するエンリッチメント パイプラインを作成します。 このナレッジ ストアには、ソースから取得された元のテキスト コンテンツに加えて、AI によって生成されたコンテンツ (センチメント ラベル、キー フレーズ抽出、および非英語圏の顧客のコメントのテキスト翻訳を含む) が含まれます。

> [!NOTE]
> このクイックスタートは、Azure Storage でナレッジ ストアを最も速く完成させる方法です。 詳細な説明については、代わりに [REST でのナレッジ ストアの作成](knowledge-store-create-rest.md)に関する記事を参照してください。
...
+ ドキュメント テーブルには、ドキュメントのエンリッチメント ツリーの第 1 レベルのノードがすべて含まれます。

+ "ページ" または "文" レべルの粒度を指定すると、ページ テーブル (文テーブル) が作成されます。 ページまたは文レベルで実行されるスキルには、このテーブルに投影される出力があります。

+ キー フレーズやエンティティなど、コレクション (配列) を出力するスキルでは、スタンドアロン テーブルに出力があります。

同じプロジェクション グループ内のすべてのテーブルには、他のツールやアプリでテーブル リレーションシップをサポートするための相互参照情報が含まれています。
...
| テーブル | 説明 |
|-------|-------------|
| hotelReviewssDocument | CSV から引き継がれたフィールド (reviews_date や reviews_text など) が含まれます。 |
| hotelReviewssPages | スキルセットによって作成された、エンリッチされたフィールド (センチメント ラベルや翻訳されたテキストなど) が含まれます。 |
| hotelReviewssKeyPhrases | キー フレーズのみの長い一覧が含まれます。 |
...
> [!TIP]
> ここに示した手順をもう一度やってみたい場合や、別の AI エンリッチメントに関するチュートリアルに挑戦してみたい場合には、**hotel-reviews-idxr** インデクサーと関連オブジェクトを削除し、再作成してください。 インデクサーを削除すると、1 日あたりの無料トランザクションのカウンターがリセットされ、ゼロに戻ります。
