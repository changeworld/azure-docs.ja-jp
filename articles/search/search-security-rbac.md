...
> [!Note]
> サービス上のコンテンツをセキュリティで保護するためのロールベースのアクセス制御 (RBAC) はありません。 サービス自体に対する認証済み要求には、管理 API キーまたはクエリ API キーを使用します。 検索結果に対する ID ベースのアクセスの場合は、セキュリティ フィルターを作成して ID 別に結果に手を入れ、要求者がアクセスしてはいけないドキュメントを削除します。 詳細については、[セキュリティ フィルター](search-security-trimming-for-azure-search.md)に関する記事を参照してください。
...
RBAC のアクセス許可は、ポータルの操作とサービス管理 (サービスまたはその API キーの作成、削除、または変更) に適用されます。 API キーは、サービスが存在するようになった後に作成され、サービス上でのコンテンツ操作に適用されます。 また、オブジェクトの作成や削除など、ポータルでのコンテンツ関連の操作については、RBAC の所有者または共同作成者が、暗黙の管理 API キーを使用して、サービスとやりとりします。

| 操作 | 何によって制御されるか |
|-----------|-------------------------|
| サービスの作成 | RBAC アクセス許可: 所有者または共同作成者 |
| サービスのスケーリング | RBAC アクセス許可: 所有者または共同作成者|
| サービスの削除 | RBAC アクセス許可: 所有者または共同作成者 |
| 管理キーまたはクエリ キーの管理 | RBAC アクセス許可: 所有者または共同作成者|
| ポータルまたは管理 API でのサービス情報の表示 | RBAC アクセス許可: 所有者、共同作成者、または閲覧者  |
| ポータルまたは管理 API でのオブジェクト情報およびメトリックの表示 | RBAC アクセス許可: 所有者または共同作成者 |
| サービス上のオブジェクトの作成、変更、削除: <br>インデックスとコンポーネント部分 (アナライザーの定義、スコアリング プロファイル、CORS オプションなど)、インデクサー、データ ソース、シノニム、サジェスター | API を使用する場合は管理キー、ポータルを使用する場合は RBAC 所有者または共同作成者 |
| インデックスのクエリ | API を使用する場合は管理キーまたはクエリ キー、ポータルを使用する場合は RBAC 所有者または共同作成者 |
| オブジェクトに関するクエリ システム情報 (オブジェクトの統計、カウント、一覧を返す操作など) | API を使用する場合は管理キー、ポータルを使用する場合は RBAC 所有者または共同作成者 |
