<properties 
	pageTitle="Azure Search Indexer のカスタマイズ" 
	description="Azure Search インデクサーの設定とポリシーをカスタマイズする方法について説明します。" 
	services="search" 
	documentationCenter="" 
	authors="chaosrealm" 
	manager="pablocas" 
...
この記事では、Azure Search のインデクサーを使用して、これらのシナリオを実装する方法を学習します。

- データソースとターゲット インデックス間のフィールド名を変更する 
- データベース テーブルの文字列を文字列のコレクションに変換する
- データ ソースの変更検出ポリシーを切り換える 
