...
description: C#、および .Net SDK の Microsoft.Azure.Search (バージョン 10) を使用して、.NET アプリケーションの検索オブジェクトを作成および管理する方法について説明します。 コード スニペットには、サービスへの接続、インデックスの作成、およびクエリの例を示します。
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
ms.devlang: dotnet
...
## <a name="about-version-10"></a>バージョン 10 について
...
この SDK では、Search サービスの作成とスケーリングや API キーの管理などの[管理操作](/rest/api/searchmanagement/)はサポートされていません。 .NET アプリケーションから Search リソースを管理する必要がある場合は、[Azure Cognitive Search .NET Management SDK](/dotnet/api/overview/azure/search/management) を使用できます。

## <a name="upgrade-to-v10"></a>v10 にアップグレードする
