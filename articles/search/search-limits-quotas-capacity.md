...
ほとんどのリージョンで、Azure Search 価格レベル (Basic、S1、S2、S3、S3 HD) では、2017 年 11 月/12 月の後に作成されたすべてのサービスについて、ドキュメント数が無制限です。 このセクションでは、制限が適用されるリージョンと、使用しているサービスが影響を受けるかどうかを確認する方法を示します。 
...
### <a name="regions-and-services-having-document-limits"></a>ドキュメントの制限があるリージョンとサービス

制限があるサービスは、2017 年末の前に作成されたか、容量の小さなクラスターを使用して Azure Search サービスをホストしているデータ センターで実行されているものです。 影響を受けるデータ センターのリージョンは次のとおりです。
