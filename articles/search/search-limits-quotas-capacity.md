...
すべてのレベルが [ポータルでプロビジョニング](search-create-service-portal.md)できます。 サービスには最初に 1 つのパーティションと 1 つのレプリカが割り当てられますが、サービスが作成されたらリソースの割り当てを増やすことができます。

サービスは特定のレベルでプロビジョニングされます。 レベルを超えてさらに容量が必要な場合は、新しいサービスをプロビジョニングする必要があります (インプレース アップグレードがない場合)。 レベルの詳細については、[SKU または価格レベルの選択](search-sku-tier.md)に関するページをご覧ください。 プロビジョニングしたサービス内での容量調整の詳細については、[クエリとインデックスのワークロードに応じたリソース レベルのスケール](search-capacity-planning.md)に関するページをご覧ください。
