...
S2 を除き、すべてのレベルが[ポータルでプロビジョニング](search-create-service-portal.md)されます。S2 では、サポート チケットが必要になります。S2 を開始するには azuresearch_contact@microsoft.com にメールを送信します。
...
料金ページと上記の[レベル制限](#TierLimits)グラフに概算がありますが、実際の QPS (秒間クエリ) は決定が難しくなります。特に、利用できる帯域幅とシステム リソースの競合にスループットが依存する無料の共有サービスの場合にそれが当てはまります。共有サービスを支えるコンピューティング リソースとストレージ リソースは複数のサブスクライバーにより共有されます。そのため、お使いのソリューションの QPS は常に、同時に実行されるその他のワークロードの数によって変わります。
