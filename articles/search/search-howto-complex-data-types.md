...
>
> 検索ソリューションがコレクション内でフラット化されたデータセットの以前の回避策に基づいている場合は、インデックスを変更して、最新の API バージョンでサポートされている複合型を含めることをお勧めします。 API バージョンのアップグレードの詳細については、[最新の REST API バージョンへのアップグレード](search-api-migration.md)または[最新の .NET SDK バージョンへのアップグレード](search-dotnet-sdk-migration.md)に関する記事を参照してください。
