...
1. 新しいプロジェクトで appsettings.json ファイルを開き、次の名前とキーを使って既定の行を置き換えます。 ここに示す API キーはキーの例ではありません。API キーは、ホテルのデータにアクセスするために必要になる_正確な_キーです。 appsettings.json ファイルは、次のようになります。
