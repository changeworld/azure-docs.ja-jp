...
Azure Search は、Microsoft Azure のホスト型クラウド検索サービスです。このサービスは一般提供が開始されており、[Search サービス REST API バージョン 2015-02-28](https://msdn.microsoft.com/library/azure/dn798935.aspx) でサポートされる構成に対して、99.9% の可用性を保証するサービス レベル アグリーメント (SLA) を提供します。
...
機能|リリース日|状態|詳細
-------|--------|------|-------
[カスタム アナライザー](https://azure.microsoft.com/blog/custom-analyzers-in-azure-search/)|2016 年 1 月|[プレビュー](search-api-2015-02-28-preview.md)|トークナイザーとトークン フィルターのユーザー定義の構成MSDN の「[Azure Search での分析](https://msdn.microsoft.com/library/azure/mt605304.aspx)」を参照してください。
...
[.NET SDK 1.0.1-preview](https://msdn.microsoft.com/library/azure/dn951165.aspx)|2016 年 1 月|プレビュー|これは、.NET クライアント ライブラリ Microsoft.Azure.Search.dll の、3 番目のイテレーションです。このバージョンには重大な変更があります。移行のガイドラインについては、「[Azure Search .NET SDK バージョン 1.0.1-preview へのアップグレード](search-dotnet-sdk-migration.md)」を参照してください。
...
fieldMapping コンストラクト|2015 年 4 月|プレビュー|インデクサーは、fieldMapping コンストラクトをサポートするようになりました。これは実際のフィールド名が外部データベースと Azure Search インデックスとの間で異なる場合に、フィールド割り当てを提供します。`2015-02-28-preview` バージョンのインデクサーの資料については、「[インデクサー操作](search-api-indexers-2015-02-28-Preview.md)」を参照してください。
