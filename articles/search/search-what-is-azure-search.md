...
# Azure Search とは
Azure Search は、サーバーとインフラストラクチャの管理を Microsoft に委任するクラウドの Search-as-a-service (サービスとしての検索) ソリューションであり、ユーザーにはデータを入力して検索できる、すぐに使用できるサービスが提供されます。Azure Search では、検索インフラストラクチャを管理して、検索のエキスパートにならなくても、簡単な REST API または .NET SDK を使用して、アプリケーションに堅牢な検索エクスペリエンスを簡単に追加できます。

## ユーザーに強力な検索エクスペリエンスを提供する

[OData 構文](https://msdn.microsoft.com/library/azure/dn798921.aspx)と[簡単なクエリ構文](https://msdn.microsoft.com/library/azure/dn798920.aspx)を使用して、論理演算子、比較式などを使用した**強力なクエリ**を形成できます。さらに、[Lucene クエリ構文](https://msdn.microsoft.com/library/azure/mt589323.aspx) (現在プレビュー段階) では、あいまい検索、近接検索、項目ブースト、および正規表現を有効にできます。さらに、Azure Search は、アプリケーションで発音照合と正規表現を使用して、複雑な検索クエリを処理できるカスタム字句アナライザーもサポートしています。

56 種類の言語について、**言語サポート**が[含まれています](https://msdn.microsoft.com/library/azure/dn879793.aspx)。Lucene アナライザーと Microsoft アナライザー (Office および Bing での長年にわたる自然言語処理によって改良された) の両方を使用することで、Azure Search は単語区切り、テキスト標準化、レンマ化などによってテキストを分析できます。これにより、アプリケーションの検索ボックスで、スペルミス、動詞の時制、不規則な複数形の名詞 (例: 'mouse' と 'mice') などをインテリジェントに処理できます。
...
**地理空間**[サポート](https://msdn.microsoft.com/library/azure/dn798921.aspx)により、地理的な位置をインテリジェントに処理、フィルター、および表示できます。Azure Search により、ユーザーは指定した場所との検索結果の近接度に基づいて、または特定の地理的領域に基づいて、データを探索できます。

**フィルター**を使用することで、ファセット ナビゲーション (カテゴリや価格でフィルター処理するなど) を簡単に組み込み、クエリ形成を拡張し、ユーザーまたは開発者が指定した条件に基づいてフィルター処理することができます。
...
Azure search では、[インデクサー](https://msdn.microsoft.com/library/azure/dn946891.aspx)を使用した**データ統合**により、Azure SQL Database または Azure DocumentDB を自動的にクロールし、検索インデックスの内容とプライマリ データ ストアを同期できます。

**ドキュメント クラッキング**を使用して、Microsoft Office のほか PDF および HTML ドキュメントを含む[主要なファイル形式を読み取り、インデックスの作成](search-howto-indexing-azure-blob-storage.md)ができます。
...
検索結果の**ページング**とスロットルは、Azure Search が検索結果に適用する微調整された制御によって簡単です。
...
サービスの構成方法に応じて、他の Azure Search サブスクライバーと共有する Free レベル、またはリソースをサービス専用にする標準[価格レベル](https://azure.microsoft.com/pricing/details/search/)を利用します。サービスをプロビジョニングするときに、サービスをホストするデータ センターのリージョンも選択します。
