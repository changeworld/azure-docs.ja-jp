...
> [!Tip]
> 検索トラフィックまたはクエリのパフォーマンスを分析するには、 クエリの量、ユーザーが検索する用語、ユーザーがインデックス内の特定のドキュメントに誘導されるときに検索結果が成功する度合いを洞察します。 ガイダンスについては、「[Azure Search の検索トラフィックの分析](search-traffic-analytics.md)」、[使用状況とクエリ メトリックの監視](search-monitor-usage.md)に関するページ、[パフォーマンスと最適化](search-performance-optimization.md)に関するページを参照してください。
