...
> [AZURE.NOTE] [GitHub の MyDriving リポジトリ](https://github.com/Azure-Samples/MyDriving)にアクセスするために、GitHub アカウントは必要ありません。
...
GitHub、Visual Studio、または別の Git クライアント上の **[ZIP をダウンロード]** ボタンを使用して、[GitHub MyDriving リポジトリ](https://github.com/Azure-Samples/MyDriving)のローカル コピーをフェッチします。
...
## Xamarin Test Cloud でのモバイル アプリのテスト
...
> [AZURE.NOTE] ほとんどの Azure サービスには、価格レベルに応じて関連するコストがあります。Azure を初めて使用する場合、[無料で試す](https://azure.microsoft.com/free/)ことができます。ただし、MyDriving システムで特定のコンポーネントを使用する予定がない場合は、コストが発生することを回避するために、それらのコンポーネントを削除してください。この記事で後述する「運用コストを見積もり」のセクションでは、一般的なサービス コストの概要を説明しています。
...
|| - HDInsight によって処理される、別の Stream Analytics ジョブから履歴データを蓄積する Blob ストレージ。                                                                                         
...
## Visual Studio Team Services でのビルドの設定と定義のテスト
...
## Application Insights を使用したテレメトリの調査
...
>[AZURE.NOTE] Azure を初めて使用する場合、[無料アカウント](https://azure.microsoft.com/free/)があります。
...
| [Machine Learning](https://azure.microsoft.com/documentation/services/machine-learning/)<br/> 適応型の応答 | 10 ドル/シート/月。<br/>+ 3 時間の実験 * 1 ドル/実験時間。<br/>+ 3.5 時間の API CPU * 2 ドル/実稼働 CPU 時間。<br/>API CPU 時間は 5 分/日の再トレーニングを想定していますが、入力データが増えると上昇します。<br/>+ 400 回の乗車/日の処理に 2 分/日のスコアリング。 | 20 ドル |
...
| Azure App Service の [SQL Database](https://azure.microsoft.com/pricing/details/sql-database/)| Basic レベル、Single Database モデル。 | 5 ドル |
