...
* 「[デバイス モデルの動作の実装](iot-suite-remote-monitoring-device-behavior.md)」では、シミュレートされたデバイスの動作を実装するために使用する JavaScript ファイルについて説明します。
* 「[新しいシミュレートされたデバイスの作成](iot-suite-remote-monitoring-test.md)」では、すべてをうまくまとめて、新しいシミュレートされたデバイスの種類をソリューションにデプロイする方法を示しています。
...
冷却装置やトラックなどの各デバイス モデルでは、リモート監視ソリューションに接続するために一種のシミュレートされたデバイスを定義します。 各デバイス モデルは、次の上位スキーマを含む JSON ファイルに格納されます。
...
デバイス状態オブジェクトを更新する JavaScript ファイルの詳細については、[デバイスモデルの動作の概要](iot-suite-remote-monitoring-device-behavior.md)に関する記事を参照してください。
...
シミュレートされたデバイスは、リモート監視ソリューションから呼び出された cloud-to-device メソッドに対応できます。 デバイス モデル スキーマ ファイルの `CloudToDeviceMethods` セクションでは、次のようにします。

* シミュレートされたデバイスが応答できるメソッドを定義します。
* 実行するロジックが含まれている JavaScript ファイルを示します。

シミュレートされたデバイスは、サポートするメソッドの一覧をリモート監視ソリューションに送信します。

デバイスの動作を実装する JavaScript ファイルの詳細については、[デバイス モデルの動作の概要](iot-suite-remote-monitoring-device-behavior.md)に関する記事を参照してください。
...
JSON スキーマについて理解できたら、次のステップとして、[シミュレートされたデバイスの動作を実装する](iot-suite-remote-monitoring-device-behavior.md)方法について学習することをお勧めします。
