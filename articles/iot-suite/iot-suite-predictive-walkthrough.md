...
灰色の項目は、"*デバイスの管理*" 機能を実装しているコンポーネントを示します。 予測メンテナンスのソリューション アクセラレータの現在のリリースでは、これらのリソースはプロビジョニングできません。 デバイスの管理の詳細については、[リモート監視の事前構成済みソリューション][lnk-remote-monitoring]に関するページを参照してください。
...
予測メンテナンスのソリューション アクセラレータの主要コンポーネントは確認しました。次は、それをカスタマイズします。 [ソリューション アクセラレータのカスタマイズのガイダンス][lnk-customize]に関するページをご覧ください。
