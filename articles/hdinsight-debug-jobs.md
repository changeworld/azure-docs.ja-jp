<properties linkid="manage-services-hdinsight-debug-error-messages" urlDisplayName="HDInsight エラーのデバッグ" pageTitle="HDInsight のデバッグ: エラー メッセージ | Azure" metaKeywords="dinsight, hdinsight サービス, hdinsight azure, デバッグ, エラー メッセージ, エラー" description="PowerShell を使用して HDInsight を管理しているときに表示されることがあるエラー メッセージと、回復するために使用できる手順について説明します。" services="hdinsight" title="HDInsight のデバッグ: エラー メッセージ" umbracoNaviHide="0" disqusComments="1" editor="cgronlun" manager="paulettm" authors="bradsev" />

#HDInsight のデバッグ: エラー メッセージ

##はじめに
このトピックで取り上げるエラー メッセージは、Azure HDInsight のユーザーが Azure PowerShell を使用してサービスを管理する際に遭遇する可能性のあるエラー メッセージを理解するのに役立ちます。エラーから回復する手順も示されています。

一部のエラー メッセージは、Azure ポータルで HDinsight クラスターを管理した場合にも表示されます。しかし、その場合に遭遇する可能性のあるエラー メッセージは、その状況で可能な対応策に制約があるため、さほどきめ細かいものではありません。その他のエラー メッセージは、対応策が明白な文脈で提供されています。たとえば、パラメーターの制約違反である場合、値が入力されたボックスの右側にポップアップ形式でメッセージが表示されます。下図は、要求されたデータ ノードの数が多すぎた場合です。対応策は、許容値である 22 以下まで数を減らすことです。

![HDI.Debugging.ErrorMessages.Portal][image-hdi-debugging-error-messages-portal]

Azure PowerShell または Azure ポータルでユーザーが遭遇する可能性のあるエラーは、「[HDInsight エラー](#hdinsight-error-messages)」セクションに名前のアルファベット順に列挙されており、各エラーは、「[エラーの説明と対応策](#discription-mitigation-errors)」セクションの項目にリンクされ、以下のエラーの情報が示されています。
...
<h2><a id="discription-mitigation-errors"></a>エラーの診断と対応策</h2>
...
- **対応策**: 現在サポートされているリージョン (北ヨーロッパ、米国東部、米国西部) にクラスターを作成する必要があります。
...
- **対応策**: 重複しない名前をコンテナーに付けて、作成操作をやり直します。
...
- **説明**: リージョン *yourRegionName* およびサブスクリプション ID *yourSubscriptionIdx* ではリージョン能力が利用できません。
- **対応策**: HDInsight クラスターをサポートするリージョンを指定します。パブリックにサポートされている地域は、米国東部、米国西部、北ヨーロッパです。
