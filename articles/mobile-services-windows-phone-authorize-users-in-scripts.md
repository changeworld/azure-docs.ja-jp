...
<p>このチュートリアルは、モバイル サービスのクイック スタートと、1 つ前のチュートリアル「 <a href="/ja-jp/develop/mobile/tutorials/get-started-with-users-wp8">認証の使用</a>」の内容を前提としています。このため、このチュートリアルの前に、「 <a href="/ja-jp/develop/mobile/tutorials/get-started-with-users-wp8">認証の使用</a>」を完了している必要があります。</p>
</div>
...
4. 既存のスクリプトを次の関数に置き換え、**[保存]** をクリックします。
	...
	<p>挿入スクリプトを初めて実行するときには、動的スキーマを必ず有効にしてください。動的スキーマが有効になっていると、モバイル サービスでは最初の実行で <strong>userId</strong> 列を <strong>TodoItem</strong> テーブルに自動的に追加します。動的スキーマは、新しいモバイル サービスでは既定で有効になっているため、アプリケーションを Windows Phone ストアに発行する前に無効にする必要があります。</p>
    </div>
