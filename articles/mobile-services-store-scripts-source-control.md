<properties linkid="develop-mobile-tutorials-store-scripts-in-source-control" urlDisplayName="ソース管理へのサーバー スクリプトの保存" pageTitle="ソース管理へのサーバー スクリプトの保存- Azure のモバイル サービス" metaKeywords="" description="コンピューターのローカル Git リポジトリにサーバー スクリプト ファイルとモジュールを格納する方法について説明します。" metaCanonical="" services="" documentationCenter="Mobile" title="ソース管理へのサーバー スクリプトの保存" authors="glenga" solutions="" manager="" editor="" />
...
<h2><a name="enable-source-control"></a><span class="short-header">ソース管理の有効化</span>モバイル サービスでソース管理を有効にする</h2>
...
<h2><a name="clone-repo"></a><span class="short-header">レポジトリの複製</span>Git をインストールし、ローカル リポジトリを作成する</h2>
...
<h2><a name="deploy-scripts"></a><span class="short-header">スクリプトの展開</span>更新されたスクリプト ファイルをモバイル サービスに展開する</h2>
...
<h2><a name="use-npm"></a><span class="short-header">共有コードとモジュール</span>サーバー スクリプトで共有コードと Node.js モジュールを活用する</h2>
...
2. コマンド プロンプトで、ローカル Git リポジトリの `.\service` フォルダーに移動し、次のコマンドを実行します。
	...
	<strong>注</strong>
	<p>ディレクトリ階層の中に <code>node_modules</code> ディレクトリが既に存在している場合、リポジトリに新しい <code>node_modules</code> が作成されるのではなく、その既存のディレクトリの中に <code>\node-uuid</code> サブディレクトリが作成されます。この場合、既存の <code>node_modules</code> ディレクトリを削除してください。</p>
	</div>
