<properties linkid="hdinsight-use-time-based-oozie-coordinator-with-hdinsight" urlDisplayName="HDInsight での時間ベースの Oozie コーディネーターの使用" pageTitle="HDInsight での時間ベースの Oozie コーディネーターの使用 | Microsoft Azure" metaKeywords="" description="時間ベースの Oozie コーディネーターを HDInsight と組み合わせて使用し、ビッグ データ ソリューションを構築します。Oozie のワークフローとコーディネーターを定義し、Oozie コーディネーター ジョブを送信する方法について説明します。" metaCanonical="" services="hdinsight" documentationCenter="" title="HDInsight での時間ベースの Oozie コーディネーターの使用" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />


# HDInsight での時間ベースの Oozie コーディネーターの使用

ワークフローとコーディネーターを定義する方法、および時刻に基づいてコーディネーター ジョブを起動する方法について説明します。この記事を読む前に、「[HDInsight での Oozie の使用][hdinsight-oozie]」を読むと役に立ちます。
...
- **コンピューター**。Azure PowerShell がインストールされ構成されている必要があります。手順については、[Azure PowerShell のインストールおよび構成に関するページ][powershell-install-configure]を参照してください。PowerShell スクリプトを実行するには、Azure PowerShell を管理者として実行し、実行ポリシーを *RemoteSigned* に設定する必要があります。[Windows PowerShell スクリプトの実行に関するページ][powershell-script]を参照してください。
...
HDInsight はデータ ストレージとして Azure BLOB ストレージを使用します。これは *WASB* または *Windows Azure ストレージ - BLOB* と呼ばれています。WASB は、HDFS を Azure BLOB ストレージ上で Microsoft が実装したものです。詳細については、「[HDInsight での Azure BLOB ストレージの使用][hdinsight-storage]」を参照してください。
...
3. スクリプトの末尾に次のコードを追加します。この部分は、Oozie ペイロードを定義します。
	...
	>[WACOM.NOTE] ワークフロー送信ペイロード ファイルと比較した場合の主な相違点は、**oozie.coord.application.path** 変数です。ワークフロー ジョブを送信する場合は、代わりに 
**oozie.wf.application.path** を使用します。
...
7. HDinsight クラスターがバージョン 2.1 の場合は、"https://$clusterName.azurehdinsight.net:443/oozie/v2/" を、"https://$clusterName.azurehdinsight.net:443/oozie/v1/" で置換します。HDInsight クラスター Version 2.1 は、Web サービスの Version 2 をサポートしていません。
