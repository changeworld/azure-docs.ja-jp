...
まずは、使用するコネクタを 2 つ作成する必要があります。**具体的には、Dropbox Connector** と **Twitter Connector** です。コネクタを作成する手順は、以下のとおりです。
...
3. **App Service プラン**には、コネクタの作成時に使用したものを選択します。これにより、場所、サブスクリプション、リソース グループが自動的に選択されます。
...
1. まだ **[ロジック アプリの作成]** ブレードを開いているはずですので、そこで **[トリガーとアクション]** をクリックします。 

	フローを表示するデザイナーが全画面表示されます。右側に、トリガーを設定できるサービスが一覧表示されています。 

2. **[組み込み]** セクションで、**[繰り返し]** をクリックします。
...
4. **[コンテンツ]** フィールドは、`[...]` ボタン、**[ツイートのテキスト]** オプションの順にクリックします。 
	...
	すると、テキストボックスに "`@first(body('twitterconnector')).TweetText`" という値が入力されます。この値の各部の意味は、以下のとおりです。
...
5. これで、設計が完了しました。デザイナーの左上の **[コード ビュー]** をクリックしてみましょう。デザイナーで作成したワークフローを定義する JSON コードが確認できます。このコードについては、「[次のトピック][ロジック アプリの機能を使用する]」で詳しく説明します。
...
ここまでの手順で、ロジック アプリが実行できるようになりました。スケジュールが設定されているワークフローが実行されるたびに、  特定のハッシュタグが含まれるツイートがないかどうかが確認されます。条件に合致するツイートが見つかった場合には、Dropbox に保存されます。ここでは、最後にアプリを無効にしたり、動作内容を確認したりする方法を見ていきます。 

1. 画面左側の **[参照]** をクリックし、**[Logic Apps]** を選択します。 
 
2. 新たに作成したロジック アプリをクリックすると、現在の状態や全般的な情報を確認できます。 

3. ロジック アプリを編集するときは、**[トリガーとアクション]** をクリックします。 
