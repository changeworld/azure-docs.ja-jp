...
このガイドは、"queue1" という名前のキューが含まれているサービス バス名前空間が既にあることを前提としています。まだない場合は、[Azure 管理ポータル][Azure 管理ポータル]を使用して名前空間とキューを作成できます。サービス バス名前空間とキューの作成方法の詳細については、「[サービス バス キューの使用方法][サービス バス キューの使用方法]」を参照してください。

## AMQP 1.0 JMS クライアント ライブラリのダウンロード

Apache Qpid JMS AMQP 1.0 クライアント ライブラリの最新バージョンをダウンロードする場所については、「[][]<http://people.apache.org/~rgodfrey/qpid-java-amqp-1-0-client-jms.html></a>」を参照してください。
...
**メモ**:パスワードは手動で URL エンコードする必要があります。便利な URL エンコード ユーティリティは、[][1]<http://www.w3schools.com/tags/ref_urlencode.asp></a> で入手できます。
...
**メモ**:Service Bus トピック サブスクリプションから受信した場合は、JNDI で指定された物理名がトピックの名前になります。サブスクリプション名は、持続性の高いサブスクリプションが JMS アプリケーション コードで作成されるときに指定されます。「[サービス バス AMQP: 開発者ガイド][サービス バス AMQP: 開発者ガイド]」では、JMS からのサービス バス トピック サブスクリプションの使用の詳細について説明しています。
...
ここで示したサンプル JMS アプリケーションと、「[.NET サービス バス API で AMQP 1.0 を使用する方法][.NET サービス バス API で AMQP 1.0 を使用する方法]」に示されている類似の .NET アプリケーションを使用すると、.NET と Java の間でメッセージ交換を行うことができます。

サービス バスと AMQP 1.0 を使ったクロスプラットフォーム メッセージングの詳細については、「[Service Bus AMQP 1.0 開発者ガイド][サービス バス AMQP: 開発者ガイド]」を参照してください。
...
サービス バス AMQP 1.0 のサポートは、.NET、C、Python、PHP など、その他の言語からも使用できます。Service Bus で AMQP 1.0 サポートを使用すると、これらのさまざまな言語を使って作成されたコンポーネントで高い信頼性と完全な忠実度のメッセージ交換が実現されます。詳細については、「[Service Bus AMQP: 開発者ガイド][サービス バス AMQP: 開発者ガイド]」を参照してください。

## 詳細情報

-   [Azure のサービス バスの AMQP 1.0 のサポートに関するページ][Azure のサービス バスの AMQP 1.0 のサポートに関するページ]
-   [.NET サービス バス API で AMQP 1.0 を使用する方法][.NET サービス バス API で AMQP 1.0 を使用する方法]
-   [サービス バス AMQP 1.0: 開発者ガイド][サービス バス AMQP: 開発者ガイド]
-   [サービス バス キューの使用方法][2]
  ...
  [サービス バス AMQP: 開発者ガイド]: http://msdn.microsoft.com/ja-jp/library/windowsazure/jj841071.aspx
  [.NET サービス バス API で AMQP 1.0 を使用する方法]: http://aka.ms/lym3vk
  [Azure のサービス バスの AMQP 1.0 のサポートに関するページ]: http://aka.ms/pgr3dp
  [2]: http://www.windowsazure.com/ja-jp/develop/net/how-to-guides/service-bus-queues/
