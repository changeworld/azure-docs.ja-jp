...
4. **[DNS ゾーンの作成]** ページで次の値を入力してから、**[作成]** をクリックします。
   ...
   | **設定** | **値** | **詳細** |
   |---|---|---|
   |**名前**|contoso.xyz|このクイック スタートの例の DNS ゾーンの名前には、Azure DNS サーバーでまだ構成されていない限り、どのような値でも使用できます。 実際の値は、ドメイン名レジストラーから購入したドメインになります。|
...
ここで、新しいアドレス レコード (「A」レコード) を作成します。 "A" レコードは、ホスト名を IPv4 アドレスに解決するために使用されます。
