...
> [!NOTE]
> 各ネーム サーバーのアドレスをコピーするとき、アドレスの末尾にあるピリオドをコピーしてください。 末尾のピリオドは、完全修飾ドメイン名の終わりを示します。 NS 名の末尾にピリオドがない場合、レジトスラーによってはピリオドが追加されることがあります。 しかしながら、すべてのレジストラーで末尾のピリオドが自動的に追加されるわけではないため、DNS RFC に準拠するためにも、末尾のピリオドを自分で追加してください。
