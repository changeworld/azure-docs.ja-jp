...
レコード セットは、`azure network record-set create` コマンドを使用して作成します。レコード セット名、ゾーン、Time-to-Live (TTL)、レコードの種類を指定する必要があります。
...
`azure network dns-record-set list` コマンドを使用すると、DNS ゾーンの前レコードをリストできます。
