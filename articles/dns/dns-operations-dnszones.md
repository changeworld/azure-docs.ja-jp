...
次の例では、[project = demo](dns-zones-records.md#tags) と *env = test* の 2 つ *Azure Resource Manager タグ*を含む DNS ゾーンの作成方法を示します。
...
削除するゾーンを指定するには、`$zone` によって返された `Get-AzureRmDnsZone` オブジェクトを使用します。
