...
* 指定の仮想ネットワークは登録仮想ネットワークとして 1 つのプライベート ゾーンにのみリンク可能
* 指定の仮想ネットワークは解決仮想ネットワークとして最大で 10 のプライベート ゾーンにのみリンク可能
* 登録仮想ネットワークが指定された場合、プライベート ゾーンに登録されているその仮想ネットワークからの仮想マシンの DNS レコードは PowerShell/CLI/API から表示または取得できないものの、仮想マシンのレコードは実際に登録され、問題なく解決される
...
* 仮想ネットワークは、 最初から登録また解決仮想ネットワークとしてプライベート ゾーンにリンクされているときは、 空である (NIC がアタッチされている仮想マシンがない) 必要がある。 ただし、将来登録または解決仮想ネットワークとして他のプライベート ゾーンにリンクするために、空でない仮想ネットワークにすることもできます。 
