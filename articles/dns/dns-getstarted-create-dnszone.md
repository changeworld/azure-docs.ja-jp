...
Azure DNS の新しいゾーンを使用するためのドメインの委任をまだ行っていない場合は、ゾーンのネーム サーバーの 1 つに DNS クエリを直接送信する必要があります。ゾーンのネーム サーバーは、上記の Get-AzureRmDnsRecordSet によって一覧表示されたように、NS レコードに示されます。以下のコマンドの値を、ゾーンの正しい値に置き換えてください。<BR>
