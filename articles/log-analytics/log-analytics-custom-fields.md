...
> [!NOTE] カスタム フィールドは、指定した条件に一致するレコードが OMS データ ストアに追加されるときに設定されます。そのため、カスタム フィールドの作成後に収集されたレコードのデータのみが表示されます。  カスタム フィールドの作成時にデータ ストアに既にあったレコードには、カスタム フィールドは追加されません。
