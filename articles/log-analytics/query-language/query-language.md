...
- 「[Get started with Log Analytics in the Azure portal](get-started-analytics-portal.md)」(Azure portal で Log Analytics の使用を開始する) は、Azure portal でのクエリの作成との結果の使用に関するレッスンです。
...
- 「[Analyze Log Analytics data in Azure Monitor](../log-analytics-queries.md)」(Azure Monitor で Log Analytics データを分析する) では、ログ クエリの簡単な概要を提供し、Log Analytics データがどのように構成されているかについて説明しています。
- 「[Viewing and analyzing data in Log Analytics](../log-analytics-log-search-portals.md)」(Log Analytics におけるデータの表示と分析) では、Log Analytics クエリを作成して実行するポータルについて説明しています。
...
- 「[Standard properties in Log Analytics records](../log-analytics-standard-properties.md)」(Log Analytics レコードでの標準プロパティ) では、Log Analytics のすべてのデータにとって標準のプロパティについて説明しています。
- 「[Perform cross-resource log searches in Log Analytics](../log-analytics-cross-workspace-search.md)」(Log Analytics でクロス リソースのログ検索を実行する) では、複数の Log Analytics ワークスペースと Application Insights アプリケーションからデータを使用するクエリを記述する方法について説明しています。
