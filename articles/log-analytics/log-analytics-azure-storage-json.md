<properties
    pageTitle="Log Analytics を使用した Azure 診断ログの分析 | Microsoft Azure"
...
Log Analytics は、[Azure 診断ログ](../azure-portal/monitoring-overview-of-diagnostic-logs.md)を JSON 形式で Blob Storage に出力する以下の Azure サービスのログを収集することができます。
...
一部のリソースについては、前述の構成を Azure Portal から「[Azure 診断ログの概要](../azure-portal/monitoring-overview-of-diagnostic-logs.md#how-to-enable-collection-of-diagnostic-logs)」に書かれている手順で行うことができます。
...
>[AZURE.NOTE] リソースとワークスペースが異なる Azure サブスクリプションに属している場合は、`Select-AzureRmSubscription -SubscriptionId <Subscription the resource is in>` を使用して適宜切り替えてください。
...
>[AZURE.NOTE] この構成は、Azure Portal からは見えません。 構成を確認するには、 `Get-AzureRmOperationalInsightsStorageInsight` コマンドレットを使用します。  
