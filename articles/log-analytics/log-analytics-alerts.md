<properties 
   pageTitle="Log Analytics のアラート | Microsoft Azure"
...
Runbook アクションは、[Webhook](../automation/automation-webhooks.md) を使用して Runbook を開始します。アラート ルールを作成すると、Runbook に対して、"**OMS Alert Remediation**" の後に GUID が付いた名前を持つ新しい Webhook が自動的に作成されます。このアラート ルールはペイロードを送信せず、Runbook のパラメーターに値を設定できないため、パラメーターを必要としない Runbook を使用する必要があります。

また、前のセクションで説明したように、Webhook アクションを使用したアラートから Runbook を開始することもできます。Webhook アクションには、ペイロードを使用して Runbook にデータを送信できるという利点があります。Runbook アクションの利点は、Webhook を自分で作成したり、その使用方法を理解したりする必要がないことです。
...
3.	ページの上部にある **[Alert]** (アラート) をクリックして、**[Add Alert Rule]** (アラート ルールの追加) 画面を開きます。
...
4.	**[Save]** (保存) をクリックして、アラート ルールを完成させます。すぐに実行が開始されます。
...
| Run on (実行先) | Runbook を Azure クラウドで実行する場合は、**[Azure]** を選択します。Runbook をローカル環境の [Hybrid Runbook Worker](..\automation\automation-hybrid-runbook-worker.md) で実行する場合は、**[ハイブリッド worker]** を選択します。 |
