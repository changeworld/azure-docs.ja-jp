...
* ホスト ファイアウォールが必要なポートの通信をブロックしていないことを確認するには、[Network Performance Monitor] -> [Configuration]\(構成) -> [Nodes]\(ノード) のビューからソースと宛先ノードの正常性状態を表示します。 
...
これは、回線とピアリング接続が複数のサブスクリプション間で分散している場合に発生することがあります。 NPM は、ExpressRoute に接続されている VNET が NPM ワークスペースにリンクされているものと同じサブスクリプションにある ExpressRoute プライベート ピアリング接続のみ検出します。 さらに NPM は、接続されている ExpressRoute 回線が NPM ワークスペースにリンクされているものと同じサブスクリプションにある Microsoft ピアリング接続を検出します。 このことについて次の例で説明します。

 2 つの VNET があるとします。VNET A はサブスクリプション A にあり、VNET B はサブスクリプション B にあり、それぞれサブスクリプション C の ExpressRoute に接続されています。さらに、別の VNET である VNET C がサブスクリプション C にあります。ER には、サブスクリプション C に MS ピアリングもあります。 

このとき、次のようになります。

* NPM ワークスペースがサブスクリプション A にリンクされている場合、ER 経由で VNET A への接続のみを監視できます。
* NPM ワークスペースがサブスクリプション B にリンクされている場合、ER 経由で VNET B への接続のみを監視できます。
* NPM ワークスペースがサブスクリプション C にリンクされている場合、ER 経由で VNET C および MS ピアリングへの接続のみを監視できます。

サブスクリプション間接続のサポートはまもなく提供されます。 その後は、別のサブスクリプションにあるすべての ExpressRoute プライベート ピアリング接続と Microsoft ピアリング接続を 1 つのワークスペースから監視できるようになります。
...
監視に使用されるノードの正常性状態は、[Network Performance Monitor] -> [Configuration]\(構成) -> [Nodes]\(ノード) ビューに表示できます。 ノードが正常でない場合は、エラーの詳細を表示し、推奨されるアクションを実行できます。
