...
**[プローブ]** をクリックし、**[追加]** ボタンをクリックして新しいプローブを追加します。
...
* **[ホスト]** - これは、プローブに使用されるホスト名です。 Application Gateway でマルチサイトが構成されている場合にのみ適用されます。それ以外の場合は、"127.0.0.1" を使用します。 これは VM ホスト名とは異なります。
...
> [!IMPORTANT]
> ホスト名はサーバー名ではありません。 これは、アプリケーション サーバーで実行されている仮想ホストの名前です。 プローブは、http://(ホスト名):(httpsetting のポート)/urlPath に送信されます。
> 
> 
