...
この順序を確立するには、リスナーに関連付けられている要求ルーティング規則に [優先度] フィールド値を指定します。 1 から 20000 までの整数値を指定できます。1 が最高の優先度で、20000 が最低の優先度です。 受信クライアント トラフィックが複数のリスナーと一致する場合、最も優先度の高い要求ルーティング規則が要求の処理に使用されます。
