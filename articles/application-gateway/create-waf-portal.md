...
Azure Portal を使用して、[Web アプリケーション ファイアウォール](waf-overview.md) (WAF) のある[アプリケーション ゲートウェイ](overview.md)を作成することができます。 WAF は、[OWASP](https://www.owasp.org/index.php/Category:OWASP_ModSecurity_Core_Rule_Set_Project) ルールを使用してアプリケーションを保護します。 こうしたルールには、SQL インジェクション、クロスサイト スクリプティング攻撃、セッション ハイジャックなどの攻撃に対する保護が含まれます。
...
![Web アプリケーション ファイアウォールの例](./media/create-waf-portal/scenario-waf.png)
...
3. 次のアプリケーション ゲートウェイの値を入力します。
    ...
    ![新しいアプリケーション ゲートウェイの作成](./media/create-waf-portal/application-gateway-create.png)
...
5. **[仮想ネットワークの選択]**、**[新規作成]** の順にクリックし、次の仮想ネットワークの値を入力します。
    ...
    ![Create virtual network](./media/create-waf-portal/application-gateway-vnet.png)
...
2. **[サブネット]**、**[サブネット]** の順にクリックします。
    ...
    ![サブネットの作成](./media/create-waf-portal/application-gateway-subnet.png)
...
1. 対話型シェルを開いて、**PowerShell** に設定されていることを確認します。
    ...
    ![カスタム拡張機能のインストール](./media/create-waf-portal/application-gateway-extension.png)
...
3. **[ターゲットの追加]** をクリックして、作成した各仮想マシンをバックエンド プールに追加します。
    ...
    ![バックエンド サーバーの追加](./media/create-waf-portal/application-gateway-backend.png)
...
7. **[Save]** をクリックします。
    ...
    ![診断の構成](./media/create-waf-portal/application-gateway-diagnostics.png)

## <a name="test-the-application-gateway"></a>アプリケーション ゲートウェイのテスト

1. [概要] 画面で、アプリケーション ゲートウェイのパブリック IP アドレスを見つけます。 **[すべてのリソース]**、**[myAGPublicIPAddress]** の順にクリックします。

    ![アプリケーション ゲートウェイのパブリック IP アドレスの記録](./media/create-waf-portal/application-gateway-record-ag-address.png)

2. パブリック IP アドレスをコピーし、ブラウザーのアドレス バーに貼り付けます。

    ![アプリケーション ゲートウェイのテスト](./media/create-waf-portal/application-gateway-iistest.png)
