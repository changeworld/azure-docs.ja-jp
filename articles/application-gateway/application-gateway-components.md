...
Azure Application Gateway v2 SKU は静的内部 IP アドレスと静的パブリック IP アドレスの両方に対応しています。一方で v1 SKU は静的内部 IP アドレスにのみ対応しています。 アプリケーション ゲートウェイを停止し、起動すると、仮想 IP (VIP) が変更される可能性があります。
