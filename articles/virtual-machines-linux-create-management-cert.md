...
-   [publishsettings ファイルから管理証明書を取得する][publishsettings ファイルから管理証明書を取得する]
-   [Azure 管理ポータルを使用して管理証明書をインストールする][Azure 管理ポータルを使用して管理証明書をインストールする]

## <span id="publishsettings"></span></a>方法: 管理証明書の作成とアップロード

Azure の管理証明書を作成する簡単な方法が用意されています。そのためには [][1][https://windows.azure.com/download/publishprofile.aspx にアクセスします。][1]</a>
...
引用符で囲まれた部分をファイル (たとえば、encodedCert.txt) にコピーし、それを Base64 デコーダーを使用してデコードします。
...
Windows では、PowerShell または無料の Windows Base64 デコーダー ([][2]<http://www.fourmilab.ch/webtools/base64/base64.zip></a> など) を使用してデコードして、PFX ファイルを抽出できます。そのためには次のコマンドを実行します。
...
新しい証明書を生成するだけでは不十分です。10 個の証明書がすべて有効であっても、侵害された古いキーはまだ Web サイトにアクセス可能なためです。

## <span id="management"></span></a>Azure 管理ポータルを使用して管理証明書をインストールする

管理証明書は、さまざまな方法で作成できます。証明書の作成方法の詳細については、「[Azure の管理証明書の作成とアップロード][Azure の管理証明書の作成とアップロード]」を参照してください。管理証明書を作成した後、それを Azure のサブスクリプションに追加します。
...
Azure account import (File)
  ...
  [2]: http://www.fourmilab.ch/webtools/base64/base64.zip
