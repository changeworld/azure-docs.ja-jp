...
トピックへのアクセスの管理は、[ロールの割り当て](https://docs.microsoft.com/en-us/azure/role-based-access-control/role-assignments-cli)を使用して行われます。 ロールの割り当てではロールベースのアクセス確認を使用して、Azure リソースに対する操作を、特定のスコープの承認されたユーザーに制限します。
