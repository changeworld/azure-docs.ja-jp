...
決定論的な動作の場合は、[サブスクリプション再試行ポリシー](manage-event-delivery.md)の中で有効なイベントの Time-To-Live と最大配信試行回数を設定します。
...
イベント サブスクリプションを使用すると、配信されるイベントに含まれる HTTP ヘッダーを設定できます。 この機能を使用すると、配信先で必要なカスタム ヘッダーを設定できます。 イベント サブスクリプションを作成するときに、最大 10 個のヘッダーを設定できます。 各ヘッダーの値は、4,096 (4 K) バイトより大きくすることはできません。 次の配信先に配信されるイベントについて、カスタム ヘッダーを設定できます。
