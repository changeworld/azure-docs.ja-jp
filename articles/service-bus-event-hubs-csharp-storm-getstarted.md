...
+ [Maven](http://maven.apache.org/) を実行するように構成された Java 開発環境。このチュートリアルでは、[Eclipse](https://www.eclipse.org/) をお勧めします。
...
6. 上部にある **[構成]** タブをクリックし **SendRule** という名前のルールを *"送信"* 権限で、**ReceiveRule** という別のルールを *"リッスン"* 権限で追加して、**[保存]** をクリックします。
...
2.	 **Sender** プロジェクトを実行して、コンソール ウィンドウで **Enter** キーを押し、受信側ウィンドウに表示されるイベントを参照してください。
