...
ノードの 1 つを記述して、GPU がスケジュール可能であることを確認します。 これは `Capacity` セクションの下にあります。 たとえば、「 `nvidia.com/gpu:  1` 」のように入力します。 GPU が表示されない場合は、以下の「**トラブルシューティング**」のセクションを参照してください。
...
次のジョブ マニフェストには `nvidia.com/gpu: 1` のリソース制限が含まれています。 
