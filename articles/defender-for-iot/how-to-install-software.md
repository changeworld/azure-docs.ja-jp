...
| **最大帯域幅\** _ | 3 GB/秒 | 1 GB/秒 | 1 GB/秒 | 100 MB/秒 |
| _ *保護ざれる最大デバイス数** | 30,000 | 10,000 | 15,000 | 1,000 |
...
| **最大帯域幅\** _ | 150 MB/秒 | 15 MB/秒 | 3 MB/秒 |
| _ *保護ざれる最大デバイス数** | 3,000 | 300 | 100 |
...
2. **[Network sensor]\(ネットワーク センサー\)** または **[On-premises management console]\(オンプレミス管理コンソール\)** ページに移動し、ダウンロードするバージョンを選択します。
...
2. キーのディスクを前面パネルに接続します。

3. USB から起動するようにサーバーの BIOS を設定します。
...
3. NIC ポート (Gb 1) 
4. ハーフハイトの PCIe 
5. フルハイトの PCIe 拡張カード スロット 
6. 電源装置ユニット 1 
7. 電源装置ユニット 2 
8. システム ID 
9. システム状態インジケーター ケーブル ポート (CMA) ボタン 
10. USB 3.0 ポート (2) 
11. iDRAC9 専用ネットワーク ポート 
12. VGA ポート 
...
2. [BIOS 構成ファイルをインポートする](#import-the-bios-configuration-file)

#### <a name="configure-idrac-ip-address"></a>iDRAC の IP アドレスを構成する

1. センサーの電源を入れます。

2. OS が既にインストールされている場合は、F2 キーを選択して BIOS 構成に入ります。
...
5. 静的 IPv4 アドレスを **10.100.100.250** に変更します。

6. 静的サブネット マスクを **255.255.255.0** に変更します。
...
2. ブラウザーを開き、「**10.100.100.250**」と入力して iDRAC Web インターフェイスに接続します。

3. Dell の既定の管理者特権でサインインします。

   - ユーザー名: **root**

   - パスワード: **calvin**

4. アプライアンスの資格情報は次のとおりです。

   - ユーザー名: **cyberx**

   - パスワード: **xhxvhttju,@4338**
...
5. **[構成]**  >  **[Server Configuration Profile]\(サーバー構成プロファイル\)** に移動します。 次のパラメーターを設定します。
...
7. プロセスを監視するには、 **[メンテナンス]**  >  **[ジョブ キュー]** に移動します。
...
1. キーボードとスクリーンを使用してアプライアンス BIOS に直接アクセスするか、iDRAC を使用します。
   ...
   - アプライアンスが Defender for IoT アプライアンスの場合は、ユーザー名に **cyberx**、パスワードに **xhxvhttju,@4338** を使用してサインインします。

2. BIOS にアクセスした後、 **[デバイス設定]** に移動します。

3. **[Integrated RAID controller 1:Dell PERC\<PERC H330 Adapter\> Configuration Utility]** を選択して、RAID 制御構成を選択します。

4. **[構成管理]** を選択します。

5. **[Create Virtual Disk]\(仮想ディスクの作成\)** を選択します。

6. **[Select RAID Level]\(RAID レベルの選択\)** フィールドで **[RAID5]** を選択します。 **[Virtual Disk Name]\(仮想ディスク名\)** フィールドに「**ROOT**」と入力し、 **[Physical Disks]\(物理ディスク\)** を選択します。

7. **[すべて選択]** を選択し、 **[変更の適用]** を選択します。

8. **[OK]** を選択します。

9. 下にスクロールし、 **[Create Virtual Disk]\(仮想ディスクの作成\)** を選択します。

10. **[確認]** チェック ボックスをオンにし、 **[はい]** を選択します。

11. **[OK]** を選択します。

12. メイン画面に戻り、 **[System BIOS]\(システム BIOS\)** を選択します。

13. **[Boot Settings]\(ブート設定\)** を選択します。

14. **[Boot Mode]\(ブート モード\)** オプションで **[BIOS]** を選択します。

15. **[戻る]** を選択し、 **[完了]** を選択して BIOS 設定を終了します。
...
2. **[Map CD/DVD]\(CD/DVD のマップ\)** セクションで、 **[ファイルの選択]** を選択します。

3. 表示されるダイアログ ボックスから、このバージョンのバージョン ISO イメージ ファイルを選択します。

4. **[Map Device]\(デバイスのマップ\)** ボタンを選択します。
...
5. メディアがマウントされます。 **[閉じる]** を選択します。

6. アプライアンスを起動します。 iDRAC を使用する場合、 **[Consul Control]\(Consul の制御\)** ボタンを選択してサーバーを再起動できます。 次に、 **[Keyboard Macros]\(キーボード マクロ\)** で、 **[適用]** ボタンを選択します。これにより、Ctrl + Alt + Delete シーケンスが開始されます。

7. **[English]\(英語\)** を選択します。

8. **[SENSOR-RELEASE-\<version\> Enterprise]** を選択します。
...
9. アプライアンス プロファイルとネットワークのプロパティを定義します。
   ...
   | **DNS:** | - |
   | **既定のゲートウェイ IP アドレス:** | - |
   | **入力インターフェイス:** |  入力インターフェイスの一覧がシステムによって生成されます。 入力インターフェイスをミラー化するには、一覧に表示されているすべての項目をコンマ区切り記号を使用してコピーします。 ブリッジ インターフェイスを構成する必要はないことに注意してください。 このオプションは、特別なユース ケースにのみ使用されます。 |

10. 約 10 分後に、2 つの資格情報のセットが表示されます。 1 つは **CyberX** ユーザーに関するもので、もう 1 つは **Support** ユーザーに関するものです。  

11. アプライアンス ID とパスワードを保存します。 これらの資格情報は、初めてプラットフォームを使用するときにアクセスするために必要です。

12. **Enter** キーを選択して続行します。
...
2. **[システム ユーティリティ]**  >  **[システム構成]**  >  **[iLO 5 Configuration Utility]\(iLO 5 構成ユーティリティ\)**  >  **[ネットワーク オプション]** に移動します。
...
4. **Esc** キーを選択して **[iLO 5 Configuration Utility]\(iLO 5 構成ユーティリティ\)** に戻り、 **[ユーザー管理]** を選択します。

5. **[Edit/Remove User]\(ユーザーの編集/削除\)** を選択します。 管理者は、定義されている唯一の既定ユーザーです。 

6. 既定のパスワードを変更し、**F10 (保存)** キーを選択します。
...
2. **[BIOS/Platform Configuration (RBSU)]\(BIOS/プラットフォーム構成 (RBSU)\)** フォームで **[Boot Options]\(ブート オプション\)** を選択します。

3. **[Boot Mode]\(ブート モード\)** を **[Legacy BIOS Mode]\(レガシ BIOS モード\)** に変更し、**F10(保存)** キーを選択します。

4. **Esc** キーを 2 回選択して、 **[システム構成]** フォームを閉じます。
...
2. **[Create Array]\(アレイの作成\)** フォームで、すべてのオプションを選択します。 **Enterprise** アプライアンスでは、3 つのオプションを使用できます。
...
2. **[Proceed to Next Form]\(次のフォームに進む\)** を選択します。

3. **[Set RAID Level]\(RAID レベルの設定\)** フォームで、Enterprise デプロイの場合はレベルを **[RAID 5]** に、SMB デプロイの場合は **[RAID 1]** に設定します。

4. **[Proceed to Next Form]\(次のフォームに進む\)** を選択します。

5. **[Logical Drive Label]\(論理ドライブのラベル\)** フォームで「**Logical Drive 1**」と入力します。

6. **[Submit Changes]\(変更内容の送信\)** を選択します。

7. **[送信]** フォームで **[Back to Main Menu]\(メイン メニューに戻る\)** を選択します。

8. **F10(保存)** キーを選択し、**Esc** キーを 2 回押します。

9. **[システム ユーティリティ]** ウィンドウで **[One-Time Boot Menu]\(ワンタイム ブート メニュー\)** を選択します。

10. **[One-Time Boot Menu]\(ワンタイム ブート メニュー\)** フォームで **[Legacy BIOS One-Time Boot Menu]\(レガシ BIOS ワンタイム ブート メニュー\)** を選択します。

11. **[Booting in Legacy]\(レガシでブート\)** および **[Boot Override]\(ブート オーバーライド\)** ウィンドウが表示されます。 ブート オーバーライド オプションを選択します。たとえば、CD-ROM、USB、HDD、UEFI シェルなどです。
...
2. Defender for IoT ポータルの **[更新]** ページからダウンロードした ISO イメージを含む外部 CD またはキーのディスクを接続します。
...
5. **[SENSOR-RELEASE-<version> Enterprise]** を選択します。
...
6. インストール ウィザードで、アプライアンス プロファイルとネットワークのプロパティを定義します。
...
7. 約 10 分後に、2 つの資格情報のセットが表示されます。 1 つは **CyberX** ユーザーに関するもので、もう 1 つは **Support** ユーザーに関するものです。

8. アプライアンスの ID とパスワードを保存します。 プラットフォームに初めてアクセスするときに、この資格情報が必要です。

9. **Enter** キーを選択して続行します。
...
2. **[HTML5 Console]\(HTML5 コンソール\)** を選択します。

3. コンソールで、[CD] アイコンを選択し、[CD/DVD] オプションを選択します。

4. **[Local ISO file]\(ローカル ISO ファイル\)** を選択します。

5. ダイアログ ボックスで、関連する ISO ファイルを選択します。

6. 左側のアイコンに移動し、 **[Power]\(電源\)** を選択し、 **[リセット]** を選択します。

7. アプライアンスが再起動し、センサーのインストール プロセスが実行されます。
...
6. インストール ウィザードで、アプライアンス プロファイルとネットワークのプロパティを定義します。
    ...
    | **入力インターフェイス:**  | 入力インターフェイスの一覧がシステムによって生成されます。<br/><br/>入力インターフェイスをミラー化するには、一覧に表示されているすべての項目をコンマ区切り記号を使用してコピーします。<br/><br/>ブリッジ インターフェイスを構成する必要はないことに注意してください。 このオプションは、特別なユース ケースにのみ使用されます。 |

7. 約 10 分後に、2 つの資格情報のセットが表示されます。 1 つは **CyberX** ユーザーに関するもので、もう 1 つは **support** ユーザーに関するものです。

8. アプライアンスの ID とパスワードを保存します。 プラットフォームに初めてアクセスするときに、これらの資格情報が必要です。
...
2. イメージを **アップロード** し、 **[閉じる]** を選択します。

3. **[仮想マシン]** に移動してから、 **[VM の作成/登録]** を選択します。

4. **[新しい仮想マシンの作成]** を選択してから、 **[次へ]** を選択します。

5. センサー名を追加し、次のものを選択します。
...
7. 関連するデータストアを選択し、 **[次へ]** を選択します。

8. 必要なアーキテクチャに応じて、仮想ハードウェアのパラメーターを変更します。

9. **[CD/DVD Drive 1]\(CD/DVD ドライブ 1\)** については、 **[Datastore ISO file]\(データストア ISO ファイル\)** を選択し、前にアップロードした ISO ファイルを選択します。
...
2. **[format = VHDX]\(形式 = VHDX\)** を選択します。

3. **[type = Dynamic Expanding]\(種類 = 容量可変\)** を選択します。

4. VHD の名前と場所を入力します。

5. (アーキテクチャに応じて) 必要なサイズを入力します。   

6. 概要を確認し、 **[完了]** を選択します。

7. **[アクション]** メニューで、新しい仮想マシンを作成します。

8. 仮想マシンの名前を入力します。

9. **[世代の指定]**  >  **[第 1 世代]** を選択します。

10. (アーキテクチャに応じて) メモリ割り当てを指定し、動的メモリのチェック ボックスをオンにします。

11. サーバーのネットワーク トポロジに応じて、ネットワーク アダプターを構成します。

12. 以前に作成した VHDX を仮想マシンに接続します。

13. 概要を確認し、 **[完了]** を選択します。

14. 新しい仮想マシンを右クリックし、 **[設定]** を選択します。

15. **[ハードウェアの追加]** 選択し、新しいネットワーク アダプターを追加します。

16. センサー管理ネットワークに接続する仮想スイッチを選択します。

17. (アーキテクチャに応じて) CPU リソースを割り当てます。

18. 管理コンソールの ISO イメージを仮想 DVD ドライブに接続します。

19. 仮想マシンを開始します。

20. **[アクション]** メニューで、 **[接続]** を選択してソフトウェアのインストールを続行します。
...
2. VM が ISO イメージから起動し、[言語の選択] 画面が表示されます。 **[English]\(英語\)** を選択します。

3. 必要なアーキテクチャを選択します。

4. アプライアンス プロファイルとネットワークのプロパティを定義します。
...
6. サインイン資格情報が自動的に生成され、表示されます。 ユーザー名とパスワードは、サインインと管理に必要であるため、安全な場所にコピーしてください。
...
8. 以前に構成した IP アドレス `https://ip_address` を使用して管理コンソールにアクセスします。
...
9. 必要なアーキテクチャに応じて、仮想ハードウェアのパラメーターを変更します。

10. **[CD/DVD Drive 1]\(CD/DVD ドライブ 1\)** については、 **[Datastore ISO file]\(データストア ISO ファイル\)** を選択し、前にアップロードした ISO ファイルを選択します。
...
8. (アーキテクチャに応じて) 必要なサイズを入力します。

9. **[次へ]** を選択します。

10. 概要を確認し、 **[完了]** を選択します。

11. **[アクション]** メニューで、新しい仮想マシンを作成します。
...
15. **[世代]** を選択し、 **[第 1 世代]** に設定します。

16. **[次へ]** を選択します。

17. (アーキテクチャに応じて) メモリ割り当てを指定し、動的メモリのチェック ボックスをオンにします。

18. **[次へ]** を選択します。

19. サーバーのネットワーク トポロジに応じて、ネットワーク アダプターを構成します。

20. **[次へ]** を選択します。

21. 以前に作成した VHDX を仮想マシンに接続します。

22. **[次へ]** を選択します。

23. 概要を確認し、 **[完了]** を選択します。

24. 新しい仮想マシンを右クリックし、 **[設定]** を選択します。

25. **[ハードウェアの追加]** を選択し、 **[ネットワーク アダプター]** で新しいアダプターを追加します。

26. **[仮想スイッチ]** で、センサー管理ネットワークに接続するスイッチを選択します。

27. (アーキテクチャに応じて) CPU リソースを割り当てます。

28. 管理コンソールの ISO イメージを仮想 DVD ドライブに接続します。

29. 仮想マシンを開始します。

30. **[アクション]** メニューで、 **[接続]** を選択してソフトウェアのインストールを続行します。
...
2. デプロイに必要なアーキテクチャを選択します。

3. センサー管理ネットワークのネットワーク インターフェイスを定義します (インターフェイス、IP、サブネット、DNS サーバー、既定のゲートウェイ)。

4. サインイン資格情報が自動的に生成され、表示されます。 これらの資格情報は、サインインと管理に必要であるため、安全な場所に保管してください。
...
6. 以前に構成した IP アドレス `<https://ip_address>` を使用して管理コンソールにアクセスします。
...
物理アプライアンスのインストールを検証するには、いくつかのテストを実行する必要があります。 すべての種類のアプライアンスに同じ検証プロセスが適用されます。
...
2. **[システム設定]** ウィンドウで **[System Statistics]\(システムの統計\)** を選択します。
...
2. 「`system version`」と入力します。

3. 正しいバージョンが表示されることを確認します。
...
2. `network list` (Linux コマンド `ifconfig` と同等) を入力します。

3. 必要な入力インターフェイスが表示されることを確認します。 たとえば、2 つの Quad Copper NIC がインストールされている場合、一覧に 10 個のインターフェイスが表示されるはずです。
...
2. ラップトップの NIC のアドレスがアプライアンスと同じ範囲内になるように定義します。

    :::image type="content" source="media/tutorial-install-components/access-to-ui.png" alt-text="UI への管理アクセスを示すスクリーンショット。":::

3. ラップトップからアプライアンスの IP アドレスに対して ping を実行し、接続を確認します (既定:10.100.10.1)。

4. ラップトップで Chrome ブラウザーを開き、アプライアンスの IP アドレスを入力します。

5. **[この接続ではプライバシーが保護されません]** ウィンドウで、 **[詳細設定]** を選択し、続行します。

6. Defender for IoT のサインイン画面が表示されたら、テストは成功です。
...
2. GUI ネットワークが管理ポートに接続されていることを確認します。

3. アプライアンスの IP アドレスに対して ping を実行します。 ping がない場合:
...
4. ネットワーク パラメーターが正しく構成されていない場合は、次の手順を使用して変更します。
...
5. 再起動後、support ユーザーの資格情報を使用して接続し、`network list` コマンドを使用して、パラメーターが変更されたことを確認します。

6. もう一度 ping を実行し、GUI からの接続を試みます。

### <a name="the-appliance-isnt-responding"></a>アプライアンスが応答しない

1. モニターとキーボードをアプライアンスに接続するか、PuTTY を使用して CLI にリモート接続します。

2. **Support** ユーザーの資格情報を使用してサインインします。
...
vSwitch にはミラーリング機能がありませんが、単純な対処法を使用して SPAN ポートを実装できます。
...
4. ネットワーク ラベル「**SPAN Network**」を挿入し、 **[VLAN ID]**  >  **[すべて]** を選択し、 **[次へ]** を選択します。

5. **[完了]** を選択します。

6. **[SPAN Network]** > * *[編集]* を選択します。

7. **[セキュリティ]** を選択し、 **[Promiscuous Mode]\(無作為検出モード\)** ポリシーが **[Accept]\(承認\)** モードに設定されていることを確認します。

8. **[OK]** を選択し、次に **[閉じる]** を選択して vSwitch のプロパティを閉じます。

9. **[XSense VM]** プロパティを開きます。

10. **[Network Adapter 2]\(ネットワーク アダプター 2\)** で **[SPAN]** ネットワークを選択します。

11. **[OK]** を選択します。

12. センサーに接続し、ミラーリングが動作していることを確認します。
