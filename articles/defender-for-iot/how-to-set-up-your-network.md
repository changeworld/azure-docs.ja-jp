...
| 1 | グローバルを提供する。 | ☐ |  |
| 3 | アプライアンスを注文する。 | ☐ |  |
| 4 | ネットワーク内のサブネットの一覧を準備する。 | ☐ |  |
| 5 | 運用ネットワークの VLAN リストを用意する。 | ☐ |  |
| 6 | ネットワーク内のスイッチ モデルの一覧を用意する。 | ☐ |  |
| 7 | 産業機器の製造元とプロトコルの一覧を用意する。 | ☐ |  |
| 8 | センサーのネットワークの詳細 (IP アドレス、サブネット、D GW、DNS) を用意する。 | ☐ |  |
| 9 | 必要なファイアウォール規則とアクセス リストを作成する。 | ☐ |  |
| 10 | ポートの監視用のスイッチに SPAN ポートを作成するか、必要に応じてネットワーク タップを構成する。 | ☐ |  |
| 11 | センサー アプライアンス用のラック スペースを準備する。 | ☐ |  |
| 12 | スタッフ用のワークステーションを準備する。 | ☐ |  |
| 13 | Defender for IoT ラック デバイス用のキーボード、モニター、マウスを用意する。 | ☐ |  |
| 14 | アプライアンスをラックに取り付けてケーブルを接続する。 | ☐ |  |
| 15 | サイトにデプロイをサポートするリソースを割り当てる。 | ☐ |  |
| 16 | Active Directory グループまたはローカル ユーザーを作成する。 | ☐ |  |
| 17 | トレーニング (自己学習) を設定する。 | ☐ |  |
| 18 | 決行または中止する。 | ☐ |  |
| 19 | デプロイ日をスケジュールする。 | ☐ |  |
