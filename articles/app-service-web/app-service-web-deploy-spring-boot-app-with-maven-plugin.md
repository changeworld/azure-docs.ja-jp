...
1. このチュートリアルの前のセクションで説明した Azure サービス プリンシパルの設定を、次の例のように *settings.xml* ファイルの `<servers>` コレクションに追加します。
   ...
   `<environment>` | ターゲットの Azure クラウド環境を定義します。この例では `AZURE` です  (すべての環境の一覧については、「[Maven Plugin for Azure Web Apps]」(Azure Web アプリ用の Maven プラグイン) をご覧ください)。
...
Maven プラグイン用に変更できる値は複数あります。これらの各要素について詳しくは、「[Maven Plugin for Azure Web Apps]」(Azure Web アプリ用の Maven プラグイン) をご覧ください。 この記事でも、次のように重要な値については説明します。

要素 | Description
---|---|---
`<version>` | [Maven Plugin for Azure Web Apps]のバージョンを指定します。 最新バージョンを使用していることを確認するために、[Maven Central Respository](http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22azure-webapp-maven-plugin%22) で一覧表示されているバージョンを確認してください。
...
* [Azure Web Apps 用の Maven プラグインを使用して、コンテナー化された Spring Boot アプリを Azure にデプロイする方法](app-service-web-deploy-containerized-spring-boot-app-with-maven-plugin.md)
