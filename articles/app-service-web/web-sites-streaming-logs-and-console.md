...
診断は Web アプリごとに有効になります。[ポータル](https://portal.azure.com)で左側のメニュー バーにある **[参照] (1)** ボタンをクリックし、**[Web Apps] (2)** をクリックすると、既存のすべての Web アプリの**一覧 (3)** が表示されます。

構成する Web アプリの名前をクリックして移動します。
  
![][DiagnosticsLogs] 
**[設定] (1)**、**[診断ログ] (2)** の順にクリックし、**[アプリケーション ログ (ファイル システム)] (3)** を **[オン]** にします。**[レベル]** オプションでは、取り込むトレースの重要度レベルを変更できます。**[詳細]** はこの機能に詳しくなっておこうとする場合に設定してください。この設定では、トレース ステートメントのすべてがログに記録されるためです。
...
ポータル内からストリーミング ログを表示するには、**[ツール] (1)**、**[ログ ストリーム] (2)** の順にクリックします。アプリによってトレース ステートメントがアクティブに書き込まれている場合は、結果ウィンドウにそれらのステートメントがほぼリアルタイムで表示されます。
...
![][ConsoleScreenshot] 
コンソールにアクセスするには、前のセクションで説明しているように Web アプリを参照します。**[ツール]**、**[コンソール]** の順にクリックすると、コンソールが開きます。
