...
これで Drupal データベースを Azure に移行できました。Drupal コードをデプロイする前に、このコードを修正して、新しいデータベースに接続できるようにする必要があります。
...
最後に、Git または FTP を使用して、Web Apps にコードをデプロイします。
...
> [AZURE.NOTE]Git の設定によっては、.gitignore ファイル (git commit の実行後にローカル ルート ディレクトリに作成される .git フォルダーと同じ階層にある隠しファイル) の編集が必要になることがあります。これは、Drupal アプリケーションで無視されるファイルを指定します。デプロイする必要のあるファイルがここに含まれている場合は、それらのファイルを削除して無視されないようにします。

Web Apps に Drupal をデプロイした後は、引き続き Git または FTP を使用して更新をデプロイできます。
...
* ポータルからプレビュー ポータルへの変更ガイドについては、「[Azure ポータル内の移動に関するリファレンス](http://go.microsoft.com/fwlink/?LinkId=529715)」を参照してください。
  ...
  [Azure App Service で PHP-MySQL Windows Azure の Web アプリを作成して Git でデプロイする]: /develop/php/tutorials/website-w-mysql-and-git/
