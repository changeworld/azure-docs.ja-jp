<properties
	pageTitle="App Service Environment の構成方法 | Microsoft Azure"
...
リソース プールの数とサイズは、自分で設定することができます。ASE には、P1 ～ P4 の 4 つのサイズ オプションがあります。これらのサイズとその価格設定の詳細については、「[App Service の価格](../app-service/app-service-value-prop-what-is.md)」を参照してください。この数やサイズを変更することを "スケール操作" といいます。一度に実行できるスケール操作は 1 つだけです。
...
コンピューティング リソース プールのメトリックに基づいて自動スケール規則を設定する場合は、プロビジョニングの所要時間に注意してください。App Service Environment の自動スケールの詳細については、[App Service Environment で自動スケールを構成する方法][ASEAutoscale]に関する記事を参照してください。
...
データベースには、環境を定義する情報だけでなく、環境内で実行されているアプリに関する詳細情報が格納されています。これも、Azure の管理下にあるサブスクリプションの一部です。ユーザーが直接操作することはできません。仮想ネットワーク ルーティングまたはセキュリティを調整する場合も、SQL Azure へのアクセスを許可する必要があります。そうしないと、ASE が機能しません。

### ネットワーク

ASE で使用される VNet には、ASE の作成時または事前に作成したネットワークを使用できます。ASE 用とは別のリソース グループに仮想ネットワークを含める必要がある場合は、ASE の作成フローとは切り離して仮想ネットワークを作成する必要があります。ASE の作成時にサブネットを作成した場合、強制的にその仮想ネットワークと同じリソース グループ内に ASE が作成されます。
...
ASE を含んでいるホステッド サービスとは異なり、[仮想ネットワーク][virtualnetwork]とサブネットはユーザーの管理下にあります。仮想ネットワークの管理は、Virtual Network UI または PowerShell を使用して行うことができます。ASE は、クラシック VNet または Resource Manager VNet にデプロイすることができます。クラシック VNet と Resource Manager VNet とでは、ポータルと API の操作性が若干異なりますが、ASE の操作性は同じです。
...
ILB ASE の作成時には、ILB ASE で使用するサブドメインを指定することになります。また、指定したサブドメインに対しては、独自の DNS を管理する必要があります。サブドメイン名は自分で設定するため、HTTPS アクセスに使用する証明書も自分で管理する必要があります。ASE の作成後、その証明書を指定するように求められます。ILB ASE の作成と使用の詳細については、「[Using an Internal Load Balancer with an App Service Environment (App Service 環境での内部ロード バランサーの使用)][ILBASE]」を参照してください。
...
**[設定]** > **[プロパティ]**: ASE ブレードを選択すると、**[設定]** ブレードが自動的に開きます。その一番上に **[プロパティ]** が表示されます。プロパティには多数の項目があり、**Essentials** に表示される項目と重複していますが、**仮想 IP アドレス**と**発信 IP アドレス**はとても便利です。
