...
6. **[リソース グループ]** ボックスの横にある **[新規作成]** をクリックし、「MyExample」(またはお好きな名前) を入力します。
	...
	リソース グループは、API アプリ、データベース、VM など、一連の Azure リソースをひとまとめにしたものです。チュートリアルを行うときは、新しいリソース グループを作成することをお勧めします。このようにすれば、チュートリアルのために作成した Azure リソースをまとめて簡単に削除できるからです。詳細については、「[Azure Resource Manager の概要](../resource-group-overview.md)」を参照してください。
...
7. **[App Service の作成]** ダイアログ ボックスで、**[作成]** をクリックします。
	...
	**[Azure App Service アクティビティ]** ウィンドウには、Web アプリが作成されたことが表示されます。
	...
	この Web アプリは **Cloud Explorer** で確認できます。
...
1. **[ソリューション エクスプローラー]** でプロジェクトを右クリックし、**[発行]** を選択します。
	...
	発行プロファイルにはデプロイ用のユーザー名とパスワードが含まれています。これらの資格情報は自動的に生成されたものですが、好きな値に変更できます。変更方法は、このチュートリアルの Azure ポータルに関するセクションで説明します。パスワードは、`Properties\PublishProfiles` フォルダー内のユーザー固有の隠しファイルで暗号化されています。
...
> [AZURE.TIP] **[Web の 1 クリック発行]** ツール バーを有効にすると、簡単にデプロイできるようになります。**[表示]、[ツール バー]** の順にクリックし、**[Web の 1 クリック発行]** を選択します。このツール バーを使用すると、プロファイルを選択できるほか、ボタンをクリックして発行したり、ボタンをクリックして **Web の発行**ウィザードを開いたりできます。![Web One Click Publish Toolbar](./media/web-sites-dotnet-get-started/weboneclickpublish.png)
...
3. **[設定]** ブレードには、Web アプリを構成するための多数のオプションが表示されます。
	...
	**[発行]** セクションの **[デプロイ資格情報]** リンクに注目してください。ここで、デプロイに使用する新しいユーザー名とパスワードを作成します。お好きなユーザー名とパスワードを入力できます。その後、対象の Web プロジェクトで **Web の発行**ウィザードの **[接続]** タブで同じ値を入力します。
...
Web アプリを削除する簡単な方法は、Azure ポータルで **[Web アプリ]** ブレードの上部にある **[削除]** ボタンをクリックすることです。もっと良い方法は、Web アプリを含めるために作成したリソース グループを削除することです。このチュートリアルでは、リソース グループに含まれているのは Web アプリのみですが、通常、リソース グループには関連リソースのコレクションが含まれています。たとえば、Web アプリではデータベースまたは Azure ストレージ アカウントが使用されることがあります。これらは Web アプリが不要になると不要になります。リソース グループを削除すると、そのグループに含まれているすべてのリソースが削除されます。Azure ポータルを使用してリソース グループを削除するには、次の手順を実行します。
...
* 既存の Web プロジェクトを新しい Web アプリにデプロイする方法
	...
	既存の Web プロジェクトのデプロイ プロセスを開始するには、**ソリューション エクスプローラー**でプロジェクトを右クリックし、**[発行]** をクリックします。発行先として **[Microsoft Azure App Service]** を選択します。その後、対象として既存の Azure Web アプリを選択するか、**[新規作成]** をクリックして新しい Web アプリを作成します。**[新規作成]** をクリックして新しい Web アプリを作成する場合、そのプロセスは、このチュートリアルで説明した内容と同じになります。
...
* リアルタイム機能 (チャットなど) の追加方法
	...
	チャット サービス、ゲーム、株価情報などリアルタイム機能については、[WebSocket](/blog/2013/11/14/introduction-to-websockets-on-windows-azure-web-sites/) の転送方法で [ASP.NET SignalR](http://www.asp.net/signalr) を使用すると、パフォーマンスを最大限に高めることができます。詳細については、[Azure Web アプリでの SignalR の使用](http://www.asp.net/signalr/overview/signalr-20/getting-started-with-signalr-20/using-signalr-with-windows-azure-web-sites)に関するページを参照してください。
