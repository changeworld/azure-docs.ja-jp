...
5. デプロイが完了すると、次の通知メッセージが表示されます。メッセージをクリックして、デプロイメントのブレードを開きます。
...
5. [Web アプリ] ブレードに戻り、**[概要]** をクリックします。**[FTP/デプロイメント ユーザー名]** と **[FTPS ホスト名]** の横にある **[コピー]** ボタンをクリックして、これらの値をコピーします。
...
8. FTP/FTPS クライアントで、リモート サイトの **/site/wwwroot/webapps** に移動して、ローカル コンピューターにダウンロードした WAR ファイルをそのリモート ディレクトリにドラッグします。
   ...
   > [!NOTE]
   > Tomcat の既定の動作に従って、/site/wwwroot/webapps のファイル名 **ROOT.war** が、ルートの Web アプリ (http://*&lt;appname>*.azurewebsites.net) となり、ファイル名 ***&lt;anyname>*.war** が名前付き Web アプリ (http://*&lt;appname>*.azurewebsites.net/*&lt;anyname>*) になります。
   > 
   > 
