---
title: Microsoft OPC Publisher コマンドライン引数
description: この記事では、OPC Publisher コマンドライン引数の概要を紹介します。
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787472"
---
# <a name="command-line-arguments"></a>コマンドライン引数

以下に OPC Publisher のグローバル設定に使用できるコマンドライン引数を説明しています。

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>バージョン 2.5 以前の OPC Publisher コマンドライン引数

* 使用法: opcpublisher.exe \<applicationname> [\<iothubconnectionstring>] [\<options>]
    
* applicationname: 使用する OPC UA アプリケーション名、必須。アプリケーション名は、IoT Hub デバイス レジストリのこの名前の下にパブリッシャーを登録するためにも使用されます。
    
* iothubconnectionstring: IoT Hub 所有者 connectionstring、省略可能。 通常、アプリケーションを最初に開始するときにのみ、IoT Hub 所有者 connectionstring を指定します。 この接続文字列は暗号化され、プラットフォーム証明書ストアに格納されます。
それ以降の呼び出しでは、そこから読み取り、再利用されます。 開始ごとに connectionstring を指定する場合、IoT Hub デバイス レジストリ内にアプリケーション用に作成されたデバイスは削除され、毎回再作成されます。
    
いくつかの環境変数を使用して、アプリケーションを制御できます。
```
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>バージョン 2.6 以降の OPC Publisher コマンドライン引数
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
その他のリソースについては、GitHub リポジトリを参照してください。
