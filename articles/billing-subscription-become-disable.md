<properties
	pageTitle="Azure サブスクリプションが取り消された場合にどうすればよいか | Microsoft Azure"
...
# Azure サブスクリプションが取り消された場合にどうすればよいか
...
2. **[アカウント]**、**[サブスクリプション]** の順にクリックします。
3. サブスクリプションを選択します。
4. 使用制限に達したためにサブスクリプションが無効にされた場合は、"サブスクリプションの使用制限に達したので、課金されないように無効にされました。" という通知をクリックします。 それ以外の場合は、[サブスクリプションの状態] 領域の [使用制限の削除] をクリックします。
