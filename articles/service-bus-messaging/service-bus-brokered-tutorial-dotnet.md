...
1. ソリューション エクスプローラーで、プロジェクトの名前 (この例では **QueueSample**) を右クリックし、**[追加]** をクリックして、**[既存の項目]** をクリックします。
...
### <a name="create-a-receiver-and-receive-messages-from-the-queue"></a>レシーバーを作成し、キューからメッセージを受け取る
...
### <a name="end-the-queue-method-and-clean-up-resources"></a>Queue メソッドを終了し、リソースをクリーンアップする
...
### <a name="call-the-queue-method"></a>Queue メソッドを呼び出す
...
## <a name="build-and-run-the-queuesample-application"></a>QueueSample アプリケーションをビルドして実行する

前の手順が完了したら、**QueueSample** アプリケーションをビルドして実行できます。

### <a name="build-the-queuesample-application"></a>QueueSample アプリケーションをビルドする
