...
Service Bus などのメッセージ ブローカーの最も重要な機能は、メッセージを受信し、後で取得できるようにキューまたはトピックに保存することです。 *送信*という用語は、一般的に、メッセージ ブローカーにメッセージを転送することを指します。 *受信*という用語は、一般的に、メッセージを取得しているクライアントへメッセージを転送することを指します。
