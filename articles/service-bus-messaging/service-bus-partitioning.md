...
### <a name="use-sessions-with-partitioned-entities"></a>パーティション分割されたエンティティでセッションを使用する
...
- パーティション分割されたエンティティではルールの追加または削除がサポートされますが、トランザクションではサポートされません。 
- パーティション分割されたエンティティとの間のメッセージの送受信では AMQP がサポートされます。
- AMQP は、バッチの送信、バッチの受信、シーケンス番号での受信、ピーク、ロックの更新、メッセージのスケジュール設定、スケジュール済みメッセージの取り消し、ルールの追加、ルールの削除、セッション更新ロック、セッションの状態の設定、セッションの状態の取得、セッションの列挙の各操作でサポートされます。
