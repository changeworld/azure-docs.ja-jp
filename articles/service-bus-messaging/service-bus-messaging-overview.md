...
Service Bus WCF Relay には多数の利点がありますが、メッセージを送受信するには、サーバーとクライアントの両方が同時にオンラインである必要があります。 これは、通常は要求が長く続かない HTTP 型の通信でも、ブラウザーやモバイル アプリケーションなどのごくまれにしか接続しないクライアントでも、最適ではありません。 仲介型メッセージングは分離型通信をサポートしており、独自の利点があります。クライアントとサーバーが必要に応じて接続でき、非同期的に操作を実行することができます。
