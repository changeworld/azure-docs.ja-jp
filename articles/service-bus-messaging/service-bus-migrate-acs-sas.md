...
1.  **既定値を変更しない**。 自動生成される[所有者](/dotnet/api/microsoft.servicebus.sharedsecrettokenprovider)サービス ID と ACS 名前空間の秘密キーを Service Bus 名前空間と組み合わせて渡す **SharedSecretTokenProvider** オブジェクトを使用し、新しいルールを追加していない場合があります。
