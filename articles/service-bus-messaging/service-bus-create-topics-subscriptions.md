...
トピックのプロパティを設定できる [CreateTopic](https://msdn.microsoft.com/library/azure/hh293080.aspx) メソッドのオーバーロードがあることに注意してください。たとえば、トピックに送信されるメッセージの既定の有効期限 (TTL) の値を設定できます。次に、**Inventory** サブスクリプションと **Dashboard** サブスクリプションを追加します。
