...
SAS キー名とキー値のそれぞれの値は、[Azure Portal][Azure portal] から取得できます。
...
トピックおよびサブスクリプションは永続的であり、[Azure Portal][Azure portal] またはプログラムによって明示的に削除する必要があります。 次の例では、`mytopic` という名前のトピックを削除する方法を示しています。
