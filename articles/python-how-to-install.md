...
Windows では、メインの Python デベロッパー センターから入手できる [WebPI インストーラー][WebPI インストーラー]を使用して、インストールを合理化できます (このインストーラーにより www.python.org から CPython が取得されます)。
...
WebPI での Python 2.7/Python 3.4 のインストールを選択できます。
...
このリリースでは Web アプリケーションに焦点を当てていますが、いつでも [Python Package Index (PyPI)][Python Package Index (PyPI)] を参照して、豊富にあるその他のソフトウェアを確認してください。ディストリビューションをインストールすると、Web 開発から技術計算にいたるさまざまなシナリオのうち、興味のある多くのシナリオを実施することができます。
...
PTVS では、[クラウド サービスと Web サイトへのデプロイ][クラウド サービスと Web サイトへのデプロイ]をサポートしており、Microsoft Azure へのデプロイも容易になります。

PTVS は、既存の Visual Studio 2010、2012、または 2013 インストールに対応しています。詳細およびダウンロードについては、「[CodePlex の Python Tools for Visual Studio][CodePlex の Python Tools for Visual Studio]」を参照してください。
...
VM のシナリオでは、選択した Linux VM (Ubuntu、CentOS、Suse) を起動し、必要なものを実行または管理するだけです。たとえば、Windows/Mac/Linux コンピューター上で [IPython][IPython] REPL/Notebook を実行し、Azure 上で IPython エンジンを実行している Linux または Windows の複数プロセッサの VM をブラウザーで指定することができます。IPython のインストールの詳細については、IPython のチュートリアル参照してください。

Linux VM を設定する方法については、「[Linux Management (Linux の管理)][Linux Management (Linux の管理)]」のセクションを参照してください。
...
-   [IPython - an advanced REPL/Notebook for Python (IPython - 高度な Python 用 REPL/Notebook)][IPython]
-   [Azure での IPython Notebook][Azure での IPython Notebook]
-   [Python Tools for Visual Studio on CodePlex (CodePlex の Python Tools for Visual Studio)][CodePlex の Python Tools for Visual Studio]
-   [Virtualenv][Virtualenv]
  ...
  [クラウド サービスと Web サイトへのデプロイ]: /ja-jp/documentation/articles/cloud-services-web-sites-python-django-app-with-ptvs/
  ...
  [Linux Management (Linux の管理)]: /ja-jp/manage/linux/
