<properties
   pageTitle="Azure の x plat CLI を使用して複数の VM のデプロイを作成する | Azure"
   description="Azure の x plat CLI を使用して複数の VM のデプロイを作成する方法について説明します。"
...
# Azure の x plat CLI を使用して複数の VM のデプロイを作成する

次のスクリプトは、Azure クロス プラットフォーム コマンドライン インターフェイス (xplat-cli) を使用して VNET で複数仮想マシンの複数クラウド サービスのデプロイを構成する方法を示しています。
...
*The -q オプションはオブジェクト削除の相互確認を排除し、-b オプションは仮想マシンに関連付けられたディスクや BLOB をクリーンアップします。*
