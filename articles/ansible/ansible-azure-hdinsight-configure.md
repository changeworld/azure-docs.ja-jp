---
title: チュートリアル - Ansible を使用して Azure HDInsight でクラスターを構成する | Microsoft Docs
...
> [!div class="checklist"]
>
> * HDInsight 用のストレージ アカウントを作成する
> * [HDInsight Spark クラスター](/azure/hdinsight/spark/apache-spark-overview)を構成する
> * クラスターをサイズ変更する
> * クラスターの削除
...
このセクションのプレイブック コードは、Azure HDInsight クラスター名の一部として使用するランダムな postfix を作成します。
...
このセクションのプレイブック コードは、Azure リソース グループを作成します。
...
## <a name="create-a-storage-account-and-retrieve-key"></a>ストレージ アカウントを作成し、キーを取得する

Azure ストレージ アカウントは、HDInsight クラスターの既定のストレージとして使用されます。 

このセクションのプレイブック コードは、ストレージ アカウントへのアクセスに使用するキーを取得します。
...
このセクションのプレイブック コードは、Azure HDInsight クラスターを作成します。
...
インスタンスの作成には、完了までに数分かかる場合があります。

## <a name="resize-the-cluster"></a>クラスターのサイズ変更

クラスターの作成後、変更できる唯一の設定はワーカー ノードの数です。 

このセクションのプレイブック コードは、`workernode` 内の `target_instance_count` を更新してワーカー ノードの数を増分します。
...
## <a name="delete-the-cluster-instance"></a>クラスター インスタンスを削除する

HDInsight クラスターの料金は、分単位で課金されます。 

このセクションのプレイブック コードは、クラスターを削除します。
...
- [プレイブックをダウンロード](https://github.com/Azure-Samples/ansible-playbooks/blob/master/hdinsight_create.yml)して、`hdinsight_create.yml` に保存する。
- `hdinsight_create.yml` という名前の新規ファイルを作成して、次のコンテンツをコピーする。
