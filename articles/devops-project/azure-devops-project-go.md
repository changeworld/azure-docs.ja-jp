---
title: クイック スタート:Azure DevOps Starter を使用して Go プログラミング言語用の CI/CD パイプラインを作成する
description: DevOps Starter を使用すると、Azure で簡単に作業を開始できます。 いくつかの簡単な手順により、Azure サービス上で Go プログラミング言語の Web アプリを起動することができます。
ms.prod: devops
...
ms.locfileid: "91854615"
---
# <a name="create-a-cicd-pipeline-for-go-using-azure-devops-starter"></a>Azure DevOps Starter を使用して Go 用の CI/CD パイプラインを作成する

Azure DevOps Starter を使用して、Go アプリ用に継続的インテグレーション (CI) と継続的デリバリー (CD) を構成します。 DevOps Starter によって、Azure DevOps のビルドおよびリリース パイプラインの初期構成が簡略化されます。
...
1. **[シンプルな Go アプリ]** が既定のフレームワークです。 **[次へ]** を選択します。  前に選択したアプリ フレームワークによって、デプロイに使用可能な Azure サービスのデプロイ ターゲットの種類が決まります。 
...
1. Azure サブスクリプションと場所を選択し、アプリの名前を入力して、 **[完了]** を選択します。 数分後、DevOps Starter ダッシュボードが Azure portal に表示されます。 サンプル アプリが Azure DevOps 組織内のリポジトリに設定され、ビルドが実行され、アプリが Azure にデプロイされます。 
...
1. DevOps Starter の左側で、マスター分岐のリンクを選択します。 このリンクによって、新しく作成された Git リポジトリのビューが開かれます。
...
1. ブラウザーで DevOps Projects ダッシュボードに移動します。 ビルドが進行中であるはずです。 行った変更は、CI/CD パイプラインを介して自動的にビルドおよびデプロイされます。

## <a name="examine-the-cicd-pipeline"></a>CI/CD パイプラインを確認する

DevOps Starter によって、Azure Repos に完全な CI/CD パイプラインが自動的に構成されます。 パイプラインを探索し、必要に応じてカスタマイズします。 Azure DevOps のビルドおよびリリース パイプラインについて理解するには、次の手順を行います。
