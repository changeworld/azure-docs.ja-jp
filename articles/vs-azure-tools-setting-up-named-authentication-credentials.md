...
Visual Studio から Azure にアプリケーションを発行したり、既存のクラウド サービスを監視したりするには、送信する要求の本人性を Visual Studio が Azure に証明するための資格情報を指定する必要があります。Visual Studio には、こうした資格情報を指定するためのサインインのインターフェイスがいくつか設けられています。たとえば、サーバー エクスプローラーから **Azure** ノードのショートカット メニューを開いて **[Azure へ接続]** を選択できます。サインインすれば、Azure のアカウントに関連付けられたサブスクリプション情報が Visual Studio で利用できるようになるので、それ以上の設定は不要です。
...
Azure に対する認証には、次の 2 つのデータが必要です。
...
>[AZURE.NOTE]これらは、ユーザーのクラウド サービスを管理するために Visual Studio が必要とする資格情報であり、Azure ストレージ サービスに対して要求の本人性を証明するための資格情報とは異なります。
...
- サーバー エクスプローラーで **Azure** ノードのショートカット メニューを開き、**[サブスクリプションの管理]** を選択し、**[証明書]** タブをクリックしてから、**[新規]** または **[編集]** ボタンをクリックします。

- **Azure アプリケーションの発行**ウィザードから Azure クラウド サービスを発行する場合、**[サブスクリプションの選択]** の一覧から **<Manage>** を選び、[証明書] タブをクリックしてから、**[新規]** または **[編集]** をクリックします。
...
1. 証明書を管理ポータルにアップロードするために、次の手順を実行します。

    1. [Azure ポータル] リンクを選択します。

         [管理ポータル](http://go.microsoft.com/fwlink/?LinkID=213885)が開きます。

    1. Microsoft アカウントを使用して管理ポータルにサインインし、**[クラウド サービス]** をクリックします。
