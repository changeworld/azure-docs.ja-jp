...
Azure のサービス管理 API に対する認証は、管理証明書で行うことができます。これらの証明書は、各種 Azure サービスの構成とデプロイを自動化するために、今後多くのプログラムおよびツール (Visual Studio、Azure SDK など) で使用されることになります。これらの証明書は、クラウド サービスとはまったく関係がありません。

>[AZURE.WARNING] この種の証明書で本人性が確認された人物はだれであれ、関連付けられているサブスクリプションを管理できるので注意が必要です。
