...
1. Function App を展開し、**[関数]** の横にある **[+]** ボタンをクリックして、**HTTPTrigger** テンプレートをクリックします。 関数の**名前**として「`TurbineRepair`」と入力し、**[作成]** をクリックします。

    ![[Function App] ブレード、[関数 +]](media/functions-openapi-definition/add-function.png)
...
1. API がサポートする "*動詞*" (ここでは POST) のみを選択します。 これにより、生成された API 定義がさらにきれいになります。
    ...
    1. **[選択した HTTP メソッド]** で、**[POST]** 以外のオプションをすべてオフにします。
...
1. 右側のウィンドウで、**[認証の変更]** をクリックし、コピーした API キーを入力して、**[認証]** をクリックします。
...
1. **[要求の送信]** をクリックし、**[Pretty]\(整形\)** をクリックします。
