...
| API のカテゴリ | 理由 | 回避策 |
| ------------ | ------ | ---------- |
| 日付と時刻  | 現在の日付または時刻を返す API は、再生ごとに異なる値が返されるため、非決定論的です。 | .NET の `CurrentUtcDateTime` API、JavaScript の `currentUtcDateTime` API、または Python の `current_utc_datetime` API を使用します。これらは再生時に安全です。 |
