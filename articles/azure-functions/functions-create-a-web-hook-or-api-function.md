...
1. **[Function URL]** (関数 URL) と **[GitHub Secret]** (GitHub シークレット) の値をコピーします。これらの値は GitHub で webhook を作成する場合に必要になります。
2. **[実行]** まで下へスクロールして、要求の本文で問題のコメントの定義済みの JSON 本文を確認し、**[実行]** をクリックします。
