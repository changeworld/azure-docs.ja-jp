---
title: Azure Functions のローカルでの開発と実行 | Microsoft Docs
...
> [!NOTE]
> Azure Functions は現在プレビュー段階で、ツールを含めて全体的なエクスペリエンスは迅速に開発が進んでいます。この記事で説明している手順には最終的なローカル開発エクスペリエンスは反映されていませんが、ぜひ[フィードバックをお寄せ](https://feedback.azure.com/forums/355860-azure-functions)ください。
> 
> 
...
3. AzureWebJobsServiceBus という環境変数を作成し、Service Bus 接続文字列を設定します。
...
4. 必要なその他の環境変数がすべて設定されていることを確認します (前の「[条件付きの前提条件](#conditional-prerequisites)」セクションを参照してください)。
5. Visual Studio を起動し、WebJobs.Script ソリューションを開きます。
...
7. スタートアップ プロジェクトが WebJobs.Script.Host の場合は、次の手順を実行します。
...
8. スタートアップ プロジェクトが WebJobs.Script.WebHost の場合は、次の手順を実行します。
...
9. *sample\\host.json* ファイルを開き、`functions` プロパティを追加して、実行する関数を指定します。
