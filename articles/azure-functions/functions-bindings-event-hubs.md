...
`consumerGroup` は、ハブのイベントにサブスクライブするための[コンシューマー グループ](../event-hubs/event-hubs-features.md#event-consumers)の設定に使用される、省略可能なプロパティです。 省略した場合は、`$Default` コンシューマー グループが使用されます。  
