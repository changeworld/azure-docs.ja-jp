...
> - Event Grid サブスクリプションや BLOB 名を含むキュー メッセージなど、別のメカニズムを使用して BLOB 処理をトリガーする。 例については、[BLOB の入力のバインディング例](functions-bindings-storage-blob.md#input---example)に関する記事をご覧ください。
