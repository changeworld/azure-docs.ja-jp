...
1. 前のセクションで作成した **MapTruckRoute.html** ファイルを開き、次の JavaScript コードを *script* ブロックに追加して、Route Service を使用してトラックのルートを取得します。
    ...
    また、このコード スニペットで、Route Service に対してクエリが送信され、アカウントのキーに指定された起点と終点のルートが取得されます。 大型トラックのルートを表示するため、次のオプション パラメーターを使用しています。
   - パラメーター `travelMode=truck` では、トラベルのモードが *truck* である旨を指定しています。 サポートされる他のトラベルのモードは、*taxi*、*bus*、*van*、*motorcycle*、既定の *car* です。
   - パラメーター `vehicleWidth`、`vehicleHeight`、`vehicleLength` では、車両の寸法 (メートル単位) を指定しています。これらのパラメーターは、トラベルのモードが *truck* の場合にのみ考慮されます。
   - `vehicleLoadType` では、積み荷が危険物であり、一部の道路で輸送が制限されるものであることを指定しています。 現在、このパラメーターも *truck* モードの場合にのみ考慮されます。
