...
バックアップされた Azure VM に、ダイナミック ディスク上の複数のディスクにまたがるボリューム (スパン ボリュームおよびストライプ ボリューム) またはフォールト トレラント ボリューム (ミラー ボリュームおよび RAID-5 ボリューム) がある場合、その VM では実行可能スクリプトを実行できません。 代わりに、互換性のあるオペレーティング システムを使用する他のマシンで実行可能スクリプトを実行します。
