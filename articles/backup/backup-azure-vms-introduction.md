...
パフォーマンスに影響を与えるもう 1 つの要因は、 **バックアップ スケジュール**です。 すべての VM が同時にバックアップされるようなポリシーを構成すると、トラフィックが輻輳します。 バックアップ プロセスはすべてのディスクを並列でバックアップしようとします。 ストレージ アカウントからのバックアップ トラフィックを減らすには、それぞれの VM を 1 日の異なる時間帯に、重複せずにバックアップされるようにします。
