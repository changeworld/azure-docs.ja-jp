...
## ベスト プラクティス
仮想マシンのバックアップを構成するときには、ベスト プラクティスに従うことをお勧めします。

- 同時にバックアップする クラシック VM の数が 4 つを超えないようにスケジュールしてください。同じクラウド サービスからより多くの VM がバックアップされるように構成するには、バックアップ スケジュールを 1 時間ずつずらすことをお勧めします。 
- 同時にバックアップする Resource Manager VM の数が 40 を超えないようにスケジュールしてください。
- バックアップ サービスが顧客のストレージ アカウントからバックアップ コンテナーにデータを転送する場合に適切な IOPS を得ることができるように、ピーク時以外に VM に対するバックアップをスケジュールしてください。 
- ポリシーでは、VM を別々のストレージ アカウントに分散させるようにしてください。単一のストレージ アカウントに格納されている、VM のディスクの総数が 20 を超える場合は、バックアップの転送フェーズ中に必要な IOPS が得られるように VM を別々のバックアップ スケジュールに分散させることをお勧めします。
- Premium Storage で実行されている VM を同じストレージ アカウントに復元することはお勧めしません。この操作は、バックアップ操作に影響を及ぼし、バックアップ用の IOPS の数値を低下させます。 
- 各 Premium VM をそれぞれ異なる Premium Storage アカウントに保存して、適切なバックアップ時のパフォーマンスを確保することをお勧めします。 
