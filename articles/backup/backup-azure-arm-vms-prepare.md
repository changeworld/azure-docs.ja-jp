...
1. **[仮想マシンの選択]** ウィンドウが開きます。 ポリシーを使用してバックアップを作成する VM を選択します。 **[OK]** をクリックします。
   ...
   * VM は、1 つのコンテナーでのみバックアップできます。
    ...
    >[!NOTE]
    > コンテナーと同じリージョンおよびサブスクリプション内のすべての VM をバックアップの構成に使用できます。 バックアップを構成するときに、それらの VM に対して必要なアクセス許可がなくても、仮想マシン名とそのリソース グループを参照できます。  
