...
3. **セキュリティ PIN** - 資格情報コンテナーの[セキュリティ設定](https://docs.microsoft.com/azure/backup/backup-azure-security-feature)を有効にしている場合、重要なバックアップ操作を実行する際にセキュリティ PIN が必要になります。 この多要素認証により、セキュリティの別のレイヤーが追加されます。 
