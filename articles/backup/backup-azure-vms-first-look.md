...
1. バックアップ コンテナーを作成するか、*VM と同じリージョンにある*既存のバックアップ コンテナーを特定します。
2. Azure ポータルを使用して、仮想マシンを検索し、サブスクリプションに登録します。
3. 仮想マシンに VM エージェントをインストールします。
4. VM をバックアップするためのポリシーを作成して、仮想マシンを保護します。
5. バックアップを実行します。
...
6. **[資格情報コンテナーの作成]** をクリックします。バックアップ資格情報コンテナーが作成されるまで時間がかかることがあります。ポータルの下部にある状態通知を監視します。
    ...
    コンテナーが正常に作成されたことを確認するメッセージが表示されます。**[Recovery Services]** ページでは、コンテナーが **[アクティブ]** と表示されます。
...
3. **Recovery Services** コンテナーの一覧で、VM のバックアップに使用するコンテナーを選択します。
    ...
    選択したコンテナーは、**[クイック スタート]** ページで開かれます。
