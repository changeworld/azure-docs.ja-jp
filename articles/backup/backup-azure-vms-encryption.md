...
1. **[バックアップ センター]** に移動し、 **[バックアップ インスタンス]** メニュー項目を選択します。
1. **[データソースの種類]** として **[Azure 仮想マシン]** を選択し、バックアップ用に構成した VM を検索します。
1. 該当する行を右クリックするか、その他アイコン (...) を選択し、 **[今すぐバックアップ]** をクリックします。
1. **[今すぐバックアップ]** で、カレンダー コントロールを使用して復旧ポイントを保持する最終日を選択します。 **[OK]** をクリックします。
1. ポータルの通知を監視します。
   ジョブの進行状況を監視するには、 **[バックアップ センター]**  >  **[バックアップ ジョブ]** の順に移動し、 **[進行中]** のジョブの一覧をフィルター処理します。
   VM のサイズによっては、最初のバックアップの作成に時間がかかる場合があります。
...
- **ゲスト ユーザー** の場合は、キー コンテナーにアクセスするために、Azure Backup にアクセス許可を提供する必要があります。 暗号化された VM のバックアップを構成するには、キー コンテナーにアクセスできる必要があります。
...
2. 以下のいずれかのアクションを実行して、仮想マシン インスタンスを再作成します。
    ...
    1. PowerShell を使用して、復元されたディスクから新しい VM を作成します。 [詳細については、こちらを参照してください](backup-azure-vms-automation.md#create-a-vm-from-restored-disks)。
1. Linux VM の場合は、データ ディスクが開かれてマウントされるように、ADE 拡張機能を再インストールします。
