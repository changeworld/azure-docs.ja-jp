...
(たとえば、ネットワーク セキュリティ グループを使用して) デプロイにアクセス制限が適用されている場合、Azure バックアップ資格情報コンテナーへのバックアップ トラフィックが影響を受けないようにするには、追加の手順を実行する必要があります。
...
- マルチ DC 構成の一部であるドメイン コントローラー VM の復元は、PowerShell を通じてのみサポートされます。詳細については、[マルチ DC ドメイン コントローラーの復元](backup-azure-restore-vms.md#restoring-domain-controller-vms)に関するセクションを参照してください。
