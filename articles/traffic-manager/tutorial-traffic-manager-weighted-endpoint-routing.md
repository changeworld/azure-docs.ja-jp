...
ms.locfileid: "49649436"
---
# <a name="tutorial-control-traffic-routing-with-weighted-endpoints-using-traffic-manager"></a>チュートリアル: Traffic Manager を使用して重み付けされたエンドポイントを使用してトラフィックのルーティングを制御する 

このチュートリアルでは、Traffic Manager を使用して重み付けルーティング方法を使用してエンドポイント間のユーザー トラフィックのルーティングを制御する方法について説明します。 このルーティング方法では、Traffic Manager プロファイル構成で各エンドポイントに重みを割り当てます。すると、ユーザー トラフィックは、各エンドポイントに割り当てられた重みに基づいてルーティングされます。 重みは 1 から 1000 の整数です。 エンドポイントに割り当てられた重み値が大きいほど優先度が高くなります。
...
- Traffic Manager をテストするための 2 台の VM (1 台は**米国東部**内、2 台目は**西ヨーロッパ**内)。 テスト VM は、エンドポイントにより大きい重みが割り当てられた Web サイトに Traffic Manager がユーザー トラフィックをルーティングすることを示すために使用されます。
...
このセクションでは、VM *mVMEastUS* を作成します。 この VM は、より大きい重み値が割り当てられた Web サイト エンドポイントに Traffic Manager がユーザー トラフィックをルーティングすることを示すために使用されます。
...
6. **[概要]** の **[作成]** で **[作成]** を選択して、VM のデプロイを開始します。
8. VM の作成には数分かかります。 VM が作成されるまで、以降の手順に進まないでください。
...
ユーザー トラフィックのルーティング先となる、IIS サーバーを実行する 2 台の VM (*myIISVMEastUS*、*myIISVMWEurope*) を追加します。
...
4. 手順 2 と 3 を繰り返して、*myIISVMWEurope* という名前の IIS サーバー VM に関連付けられたパブリック IP アドレス *myIISVMWEurope-ip* 用の *myWestEuropeEndpoint* という名前の別のエンドポイントを追加します。**[重み]** には「**25**」と入力します。 
...
4. **[OK]** を選択します。
5. サインイン処理中に証明書の警告が表示される場合があります。 警告を受け取ったら、**[はい]** または **[続行]** を選択して接続処理を続行します。 
6. VM *myVMEastUS* の Web ブラウザーで、Traffic Manager プロファイルの DNS 名を入力して、Web サイトを表示します。 この場合のルーティング先は、IIS サーバー *myIISVMEastUS* でホストされている Web サイトになります。なぜなら、この Web サイトには、(エンドポイント重み値により小さな **25** が割り当てられている) IIS サーバー *myIISVMWEurope* と比較して **100** のより高い重みが割り当てられているためです。
...
このチュートリアルで作成したリソース グループが不要になったら、削除してください。 これを行うには、リソース グループ (**ResourceGroupTM1** または **ResourceGroupTM2**) を選択し、**[削除]** を選択します。
