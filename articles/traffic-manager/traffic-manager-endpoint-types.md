...
> [!NOTE]
> エンドポイントの無効化は、デプロイメント状態とは関係ありません。Azure サービス (VM、Web アプリなど) は、トラフィックが Traffic Manager プロファイルの DNS 名経由ではなく、そのサービスに直接アドレス指定されていれば、Traffic Manager で無効になっても稼働し続け、トラフィックを受信できます。詳細については、「[Traffic Manager のしくみ](traffic-manager-how-traffic-manager-works.md)」をご覧ください。
> 
> 
