...
10. **サービスへのトラフィックの再開** - Traffic Manager は、GET を送信し、10 秒未満で 200 OK メッセージを受け取ります。次に、Traffic Manager は、更新が要求されたときに、DNS サーバーに対するクラウド サービスの DNS 名の配信を開始します。他のエンドポイントに返されているキャッシュ済みの DNS 応答が期限切れになり、他のエンドポイントとの既存の接続が終了すると、トラフィックは元のエンドポイントに戻ります。

>[AZURE.NOTE] Traffic Manager は DNS レベルで動作するため、いずれかのエンドポイントとの既存の接続に影響を与えることはありません。フェールバック中、Traffic Manager はプライマリ エンドポイントとの間に新しい接続を確立できますが、セカンダリ エンドポイントはセッションが終了するまで既存の接続を介してトラフィックを受信し続けます。短時間でフェールバックを実行する必要がある場合、セカンダリ エンドポイントでのセッションの継続時間をアプリケーションで制限する必要があります。
