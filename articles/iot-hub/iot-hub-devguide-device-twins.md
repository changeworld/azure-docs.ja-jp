...
> [!NOTE]
> 現在のところ、デバイス ツインには MQTT プロトコルを使用して IoT Hub に接続するデバイスからのみアクセスできます。 既存のデバイス アプリを変換して MQTT を使用する方法については、[MQTT サポート](lnk-devguide-mqtt.md)の記事をご覧ください。
> 
> 
...
時系列のセンサー データやアラームなどの、タイムスタンプの付いた連続するイベントには、[デバイスからクラウドへのメッセージ](lnk-d2c.md)を使用します。 ファンの電源を入れるといった、デバイスの対話型のコントロールには、[クラウドからデバイスへのメソッド][lnk-methods]を使用します。
...
> [!NOTE]
> 上記のスニペットは、デバイス構成とその状態をエンコードするための例の 1 つであり、読みやすいように最適化されています。 IoT Hub は、デバイス ツインの必要なプロパティや報告されたプロパティに対して特定のスキーマを強制しません。
> 
> 
...
* 「[IoT Hub エンドポイント](lnk-endpoints.md)」では、各 IoT Hub がランタイムと管理の操作のために公開する、さまざまなエンドポイントについて説明します。
* 「[クォータと調整](lnk-quotas.md)」では、IoT Hub サービスに適用されるクォータと、サービスを使用するときに想定される調整の動作について説明します。
* 「[IoT Hub のデバイス SDK とサービス SDK](lnk sdk.md)」では、IoT Hub とやりとりするデバイスとサービス アプリケーションの両方を開発する際に使用できるさまざまな言語の SDK を紹介します。
* 「[Query language for twins, methods, and jobs (ツイン、メソッド、ジョブのクエリ言語)](lnk-query.md)」では、IoT Hub からデバイス ツイン、メソッド、ジョブに関する情報を取得する際に使用できるクエリ言語について説明します。
* 「[IoT Hub MQTT サポート](lnk devguide-mqtt.md)」では、MQTT プロトコルの IoT Hub サポートについて詳しく説明します。
