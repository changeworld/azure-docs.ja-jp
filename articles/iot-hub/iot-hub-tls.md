...
TLS 1.0 と 1.1 はレガシと見なされており、[非推奨となる予定](./tls-1.2-everywhere.md)です。 したがって、IoT Hub に接続するときは、TLS 1.2 を優先 TLS バージョンとして使用することを強くお勧めします。


## <a name="restrict-connections-to-tls-12-in-your-iot-hub-resource"></a>IoT Hub リソース内の TLS 1.2 への接続を制限する

セキュリティを強化するためには、TLS バージョン 1.2 を使用するクライアント接続_のみ_を許可し、[推奨される暗号](#recommended-ciphers)の使用を強制するように、IoT Hub を構成することをお勧めします。
...
`minTlsVersion` プロパティは読み取り専用であり、IoT Hub リソースの作成後は変更できないことに注意してください。 したがって、_すべての_ IoT デバイスとサービスが TLS 1.2 および[推奨される暗号](#recommended-ciphers)と互換性があることを、事前に適切にテストし、検証する必要があります。
