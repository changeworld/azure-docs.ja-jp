...
トークン サービスは、必要に応じて、トークンの有効期限を設定できます。期限が切れた時点で、IoT Hub は接続を切断するため、デバイスは、新しいトークンをトークン サービスに要求する必要があります。有効期限までの期間を短くすると、デバイスとトークン サービスの両方に対する負荷が増加するのは明らかです。
