...
より複雑な実装になると、[Azure Monitor][lnk-AM] と [Azure Resource Health][lnk-ARH] からの情報を取り込むことで、接続または通信を試みているが失敗しているデバイスを識別することが可能です。「[診断を使用した監視][lnk-devguide-mon]」ガイドを参照してください。 ハートビート パターンを実装する場合は、[IoT Hub のクォータとスロットル][lnk-quotas]を必ず確認してください。
