...
[Azure Service Bus SDK for .NET][lnk-servicebus-sdk] または [Event Hubs - イベント プロセッサ ホスト][lnk-eventprocessorhost]を使用する場合、適切なアクセス許可を持つ IoT Hub の接続文字列を使用することで、**messages/events** をイベント ハブの名前として使用できます。
...
| スロットル | ハブごとの値 |
| -------- | ------------- |
| ID レジストリの操作 (作成、取得、一覧表示、更新、削除) | 5000/分/ユニット (S3 の場合) <br/> 100/分/ユニット (S1 と S2 の場合)。 |
