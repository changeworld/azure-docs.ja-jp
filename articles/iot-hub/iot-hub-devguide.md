...
IoT ハブのデバイス ID レジストリ: - アプリケーションのメタデータは含まれていません。 - **deviceId** をキーとして使用するディクショナリと同様にアクセスできます。 - 詳細なクエリはサポートされていません。
...
IoT Hub には、通信を行うためのメッセージング プリミティブが用意されています: - [C2D](#c2d): アプリケーション バックエンド (*service* または *cloud*) から。 - [D2C](#d2c): デバイスからアプリケーション バックエンドへ。
...
| To | 宛先フィールドを指定する [C2D](#c2d) メッセージで使用されます。|
...
サービスからメッセージを送信すると、*エンキュー*と見なされます。デバイスでメッセージを*受信*する場合、同じデバイス上の他のスレッドが他のメッセージの受信を開始できるようにするために、IoT Hub によりメッセージが*ロック*されます (状態は **[非表示]** に設定されます)。デバイスのスレッドによってデバイスの処理が完了したら、メッセージを*完了*して IoT Hub にその旨を通知します。

デバイスは次の処理も実行する可能性があります: - メッセージを*拒否*し、IoT Hub が **[配信不能]** 状態に設定します。 - メッセージを*破棄*し、IoT Hub がメッセージをキューに戻し、状態を **[エンキュー]** に設定します。

スレッドは、IoT Hub に通知することなく、メッセージの処理に失敗します。この場合、既定値の 1 分で*表示 (またはロック) がタイムアウト*した後に、メッセージの状態は自動的に **[非表示]** から **[エンキュー]** に戻ります。IoT Hub の*最大配信数*のプロパティで指定された回数を限度として、メッセージの状態は **[エンキュー]** と **[非表示]** の間で遷移する可能性があります。この遷移回数の後、IoT Hub はメッセージの状態を **[配信不能]** に設定します。同様に、有効期限 (「[有効期限](#ttl)」を参照してください) が切れた後に、IoT Hub はメッセージの状態を **[配信不能]** に設定します。
