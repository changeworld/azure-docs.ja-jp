...
このようなメッセージを処理する最適な方法について詳しくは、「[チュートリアル: IoT Hub のデバイスからクラウドへのメッセージを処理する方法][lnk-guidance-d2c-processing]」をご覧ください。
...
> [AZURE.NOTE] クラウドからデバイスへのメッセージを送信するとき、プロパティ名と値に含めることができるのは、ASCII 英数字と ``{'!', '#', '$', '%, '&', "'", '*', '*', '+', '-', '.', '^', '_', '`', '|', '~'}`` のみです。
...
詳細については、「[Azure ポータルを使用した IoT Hub の管理][lnk-manage]」をご覧ください。
