...
description: "IoT Hub のエンドポイント、セキュリティ、ID レジストリ、デバイス管理、メッセージングなどについて説明する、Azure IoT Hub の開発者ガイド"
services: iot-hub
...
Azure IoT Hub は、何百万ものデバイスとアプリケーション バックエンドの間に信頼性のある保護された双方向通信を確立するのに役立つ、完全に管理されたサービスです。
...
* [IoT Hub でのメッセージの送受信][devguide-messaging]: IoT Hub が公開している (デバイスとクラウドの双方向の) メッセージング機能について説明します。 この記事には、メッセージ形式、サポートされる通信プロトコル、プロトコルで使用されるポート番号などのトピックに関する情報も含まれています。
...
* [Cloud-to-device communication guidance (クラウドからデバイスへの通信に関するガイダンス)][lnk-c2d-guidance]: ダイレクト メソッド、デバイス ツインの必要なプロパティ、およびクラウドからデバイスへのメッセージのいずれかを選択するのに役立ちます。
* [デバイスからのファイルのアップロード][devguide-upload]: デバイスからファイルをアップロードする方法について説明します。 この記事には、アップロード プロセスが送信できる通知などのトピックに関する情報も含まれています。
...
* [Reference - IoT Hub endpoints (リファレンス - IoT Hub エンドポイント)][devguide-endpoints]: 各 IoT ハブがランタイムと管理の操作のために公開する、さまざまなエンドポイントについて説明します。 この記事では、一部のデバイスを IoT Hub エンドポイントに接続できるようにするフィールド ゲートウェイの使用方法についても説明します。
...
* [リファレンス - 価格][devguide-pricing]: IoT Hub の各種 SKU に関する全般的な情報と価格、および各種の IoT Hub 機能が IoT Hub によってどのようにメッセージとして測定されるかについて詳しく説明します。
* [リファレンス - デバイス SDK とサービス SDK][devguide-sdks]: IoT ハブと対話するデバイスとサービス アプリケーションの両方を開発する際に使用できる Azure IoT SDK を一覧で示します。 この記事には、オンライン API ドキュメントへのリンクが含まれています。
