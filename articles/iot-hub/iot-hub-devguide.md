...
どちらの場合も、[IoT Hub セキュリティ トークン][lnk-sas-tokens]に関するページで説明されているように、パスワード フィールドにトークンが含まれています。
...
| ConnectionAuthMethod | IoT Hub で D2C メッセージに設定します。メッセージを送信するデバイスの認証に使用する認証方法に関する情報。詳細については、[D2C のなりすまし対策](#antispoofing)に関する記述を参照してください。|
...
> [AZURE.NOTE] HTTP を使用してデバイスからクラウドへのメッセージを送信する場合、プロパティ名と値に含めることができるのは、ASCII 英数字と ``{'!', '#', '$', '%, '&', "'", '*', '*', '+', '-', '.', '^', '_', '`', '|', '~'}`` のみです。
...
> [AZURE.NOTE] クラウドからデバイスにメッセージを送信するとき、プロパティ名と値に含めることができるのは、ASCII 英数字と ``{'!', '#', '$', '%, '&', "'", '*', '*', '+', '-', '.', '^', '_', '`', '|', '~'}`` のみです。
...
スレッドが IoT Hub に通知することなく、メッセージの処理に失敗することもあります。その場合、既定値の 1 分で*表示 (またはロック) がタイムアウト*になり、メッセージの状態が自動的に**非表示**から**エンキュー**に切り替わります。メッセージの状態は**エンキュー**と**非表示**の間で切り替わりますが、この回数が IoT Hub の*最大配信数*のプロパティで指定された上限に達すると、IoT Hub によってメッセージの状態が**配信不能**に設定されます。同様に、有効期限 (「[有効期限](#ttl)」を参照してください) が切れた後も、IoT Hub によってメッセージの状態が**配信不能**に設定されます。
...
*デバイスの接続*スロットルは、同時に接続されたデバイスの最大数ではなく、新しいデバイス接続を IoT Hub を使用して確立できる割合を制御するということを明確にしておくことが重要です。スロットルは、ハブにプロビジョニングされたユニット数に依存します。
...
ブログ投稿「[IoT Hub throttling and you][lnk-throttle-blog]」 (IoT Hub スロットルの操作) では、IoT Hub スロットルの動作について詳しく説明しています。
