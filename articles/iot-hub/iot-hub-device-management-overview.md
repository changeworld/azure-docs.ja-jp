...
ここまではサービス側の概念について説明しましたが、ここからは管理機能でデバイスを作成する方法について説明します。デバイス上の Azure IoT Hub デバイス管理クライアント ライブラリは、物理デバイスと IoT Hub 間の通信を実装するために必要なアーティファクトを提供します。

Azure IoT Hub DM クライアント ライブラリは、[LWM2M][lnk-lwm2m] 標準および CoAP ベースの要求/応答プロトコルを抽象化します。このため、ライブラリには*オブジェクト* と *リソース定義*のデバイス モデルがあります。

-   オブジェクトは、デバイスおよびファームウェアの更新など、システムに一貫性のある機能のエンティティのセットを記述します。
-   リソースは、バッテリ レベル情報および再起動アクションなど、それらのオブジェクトに含まれる属性またはアクションを記述します。

Azure IoT Hub DM クライアント ライブラリを使用する場合は、読み取り、書き込みのコールバックを実装し、物理デバイスの各リソースの操作を実行する必要があります。プロパティが変更された場合、ライブラリは IoT Hub の非同期的な更新を処理します。

次の図は、IoT Hub クライアント エージェントに必要なさまざまなコンポーネントを示しています。

![][img-client]

物理デバイスでの実装の詳細については、「[Introducing the Azure IoT Hub device management client library for C (C 用 Azure IoT Hub デバイス管理クライアント ライブラリの概要)][lnk-library-c]」を参照してください。
