...
この情報を使用して、複合型など、サポートするデータ型の範囲が含まれるモデルを定義できます (複合型の中に別の複合型を含めることもできます)。 ただし、上の例によって生成されたシリアル化された JSON は、重要なポイントを提起します。 *シリアライザー* ライブラリを使用したデータの送信 **シリアライザー** によって、JSON の形式が決定されるということです。 このポイントについては、次で説明します。
