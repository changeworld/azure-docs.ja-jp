...
- **[追加]** をクリックしてブレードを開きます。このブレードで、次の図に示すように、新しいポリシーの名前と、このポリシーに関連付けるアクセス許可を入力できます。
 	...
 	**サービス接続**ポリシーでは、クラウド側のエンドポイント (IoT Hub に接続するサービスのコンシューマー グループなど) へのアクセス許可を与え、**デバイス接続**ポリシーでは、IoT Hub のデバイス側のエンドポイントでメッセージを送受信するためのアクセス許可を与えます。
...
高レベル (S2 または S3) から低レベル (S1 または S2) への移行は、当該日に送信されるメッセージの数が競合していない場合にのみ許可されます。たとえば、1 日あたりのメッセージ数が 400,000 件を超える場合に、IoT Hub のレベルを変更することはできますが、S1 レベルに変更した場合、その日に送信できるメッセージ数が抑制されることになります。
