...
> [AZURE.NOTE]  既存の IoT ハブにはまだデバイス管理機能がないため、既存の IoT ハブをお持ちの場合でもデバイス管理機能を有効にするために新しい IoT ハブを作成する必要があります。デバイス管理が一般公開されると、既存のすべての IoT Hub がアップグレードされ、デバイス管理機能が利用できるようになります。
...
3.  **[IoT Hub]** ブレードで、IoT Hub の構成を選択します。
  ...
  -   **[場所]** で、IoT Hub をホストする場所を選択します。IoT Hub デバイス管理を使用できるのは、パブリック プレビューの段階では米国東部、北ヨーロッパ、および東アジアでのみです。将来的には、すべてのリージョンで使用可能になる予定です。
    ...
    > [AZURE.NOTE]  **[Enable Device Management (デバイス管理の有効化)]** チェック ボックスをオンにしないと、サンプルは動作しません。<br/>**[Enable Device Management (デバイス管理の有効化)]** チェック ボックスをオンにすると、プレビューの IoT ハブが作成されます。これは米国東部、北ヨーロッパ、東アジアでのみサポートされており、運用環境のシナリオを想定したものではありません。デバイス管理が有効なハブとの間でデバイスを移行することはできません。
