...
3. **[IoT Hub]** ブレードで、IoT Hub の構成を選択します。
   ...
   * **[場所]** で、IoT Hub をホストする場所を選択します。IoT Hub デバイス管理を使用できるのは、パブリック プレビューの段階では米国東部、北ヨーロッパ、および東アジアでのみです。将来的には、すべてのリージョンで使用可能になる予定です。
   ...
   > [!NOTE]
   > **[Enable Device Management (デバイス管理の有効化)]** チェック ボックスをオンにしないと、サンプルは動作しません。<br/>**[Enable Device Management (デバイス管理の有効化)]** チェック ボックスをオンにすると、プレビューの IoT ハブが作成されます。これは米国東部、北ヨーロッパ、東アジアでのみサポートされており、運用環境のシナリオを想定したものではありません。デバイス管理が有効なハブとの間でデバイスを移行することはできません。
   > 
   > 
