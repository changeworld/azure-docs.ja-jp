...
---
# <a name="comparison-of-iot-hub-and-event-hubs"></a>IoT Hub と Event Hubs の比較
デバイスからのテレメトリ データの収集は、Azure IoT Hub の主なユース ケースの 1 つです。 このため、IoT Hub はしばしば [Azure Event Hubs][Azure Event Hubs]と比較されます。 IoT Hub のように、Event Hubs は、低遅延の動作と高い信頼性を確保しながら、クラウドに対する膨大なイベントとテレメトリ データの入口として利用できるイベント処理サービスです。
...
| スケール |同時接続された数百万のデバイスをサポートするように最適化されています。 |制限された数の同時接続をサポートできます。[Azure Service Bus のクォータ][Azure Service Bus のクォータ]に基づいて最大 5,000 の AMQP 接続をサポートします。 一方、Event Hubs を使用すると、送信される各メッセージのパーティションを指定できます。 |
...
[Azure IoT Hub 開発者ガイドの「セキュリティ」セクション]: iot-hub-devguide-security.md
[Event Hub - セキュリティ]: ../event-hubs/event-hubs-authentication-and-security-model-overview.md
[Event Hub パブリッシャー ポリシー]: ../event-hubs/event-hubs-overview.md#common-publisher-tasks
[Azure Service Bus のクォータ]: ../service-bus-messaging/service-bus-quotas.md
