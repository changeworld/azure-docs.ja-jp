---
title: "IoT Hub の HA および DR | Microsoft Docs"
...
地域フェールオーバー モデルでは、ソリューション バックエンドは主に 1 つのデータセンターの場所で実行され、セカンダリの IoT ハブとバックエンドは別のデータセンターの場所にデプロイされます。 プライマリ データセンターの IoT ハブに障害が発生した場合、またはデバイスからプライマリ データセンターへのネットワーク接続が中断された場合、デバイスはセカンダリ データセンターのエンドポイントに接続します。 複数のリージョンにまたがるフェールオーバー機能を使用すると、ソリューションの可用性は、1 つのリージョンの高可用性を超えて向上させることができます。
