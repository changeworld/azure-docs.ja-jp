...
> [AZURE.NOTE] 子ジョブから "failed (失敗)" ステータスが返された場合は、シミュレーション対象デバイスがテスト システムで実行されていることを確認してください。実行されていない場合は、simulate.bat または simulate.sh スクリプトをもう一度実行して、前のセクションのデバイス ジョブの再起動手順を最初からやり直します。
