...
ルーティング エンドポイントを作成するには、[az iot hub routing-endpoint create](/cli/azure/iot/hub/routing-endpoint.md?view=azure-cli-latest#az-iot-hub-routing-endpoint-create) を使用します。 そのエンドポイントに対するメッセージ ルートを作成するには、[az iot hub route create](/cli/azure/iot/hub/route?view=azure-cli-latest#az-iot-hub-route-create) を使用します。
