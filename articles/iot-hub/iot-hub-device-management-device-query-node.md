---
title: IoT Hub デバイス管理のツインのクエリ | Microsoft Docs
...
---
# チュートリアル: node.js でクエリを使用したデバイス ツインの検索方法 (プレビュー)
...
### サンプルを開始する
サンプルを開始するには、`registry_queryDevices.js` を実行する必要があります。これにより、いくつかの異なるクエリが実行されます。次の手順に従って、サンプルを開始してください。
...
2. **azure-iot-sdks** リポジトリを複製したルート フォルダーから **azure-iot-sdks/node/service/samples** フォルダーに移動します。
3. **registry\_queryDevices.js** を開き、プレースホルダーを IoT Hub 接続文字列で置換します。
4. `node registry_queryDevices.js` を実行します。
...
JSON クエリ式を作成したら、デバイス ツインのクエリを実行できます。**queryDevices** を呼び出し、集計クエリの**結果**オブジェクトと他のすべてのクエリの**デバイス** オブジェクトを確認します。
