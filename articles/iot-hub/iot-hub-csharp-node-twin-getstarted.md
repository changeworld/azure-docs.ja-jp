...
1. **reportconnectivity** という名前の新しい空のフォルダーを作成します。 **reportconnectivity** フォルダー内に、コマンド プロンプトで次のコマンドを使用して新しい package.json ファイルを作成します。 すべての既定値をそのまま使用します。
   
    ```
    npm init
    ```
1. コマンド プロンプトで、**reportconnectivity** フォルダーに移動し、次のコマンドを実行して、**azure-iot-device** と **azure-iot-device-mqtt** パッケージをインストールします。
   
    ```
    npm install azure-iot-device azure-iot-device-mqtt --save
    ```
1. テキスト エディターを使用して、**reportconnectivity** フォルダーに新しい**ReportConnectivity.js** ファイルを作成します。
1. **ReportConnectivity.js** ファイルに次のコードを追加して、デバイス接続文字列のプレースホルダーを、**myDeviceId** デバイス ID の作成時にコピーした接続文字列に置き換えます。
