...
- **ハブ仮想ネットワーク**
   ...
   自分で作成して管理できる標準の Azure 仮想ネットワークです。 そのようなハブにセキュリティ ポリシーが関連付けられている場合は、"*ハブ仮想ネットワーク*" と呼ばれます。 現時点では、Azure ファイアウォール ポリシーのみがサポートされています。 ワークロード サーバーとサービスを含んだスポーク仮想ネットワークをピアリングすることができます。 どのスポークにもピアリングされていないスタンドアロンの仮想ネットワークでファイアウォールを管理することもできます。
...
|ファイアウォール ポリシーで IP グループがサポートされない|IP グループはパブリック プレビュー段階であり、現在は、従来のファイアウォール規則でのみサポートされます。|修正作業が進行中です。
