...
ms.sourcegitcommit: 393d4d222b260b4352f7b883b2bafed019c49e0d
ms.openlocfilehash: 313c19377d0619e3bce92399d48fa9ec32311172
...
Azure Disk Encryption は、Windows および Linux IaaS 仮想マシン ディスクを暗号化できる新機能です。 Azure Disk Encryption では、業界標準である Windows の [BitLocker](https://technet.microsoft.com/library/cc732774.aspx) 機能と Linux の [DM-Crypt](https://en.wikipedia.org/wiki/Dm-crypt) 機能を利用して、OS およびデータ ディスクのボリュームの暗号化を提供します。 ソリューションは [Azure Key](https://azure.microsoft.com/documentation/services/key-vault/) Vault と統合されています。これは、Key Vault サブスクリプションでディスク暗号化キーとシークレットを制御および管理し、Azure ストレージで保存中の仮想マシン ディスク内のすべてのデータを確実に暗号化する場合に役立ちます。
...
* 暗号化された VM を Azure バックアップ サービスでバックアップおよび復元する

**注意**:  暗号化された VM のバックアップと復元は、Key Encryption Key [KEK] 構成を使用して暗号化された VM についてのみサポートされています。 KEK を使用せずに暗号化された VM については、サポートされていません。 KEK は、VM の暗号化を有効にするための省略可能なパラメーターです。
...
3. Azure によって VM サービス モデルが更新され、IaaS VM は暗号化解除済みとしてマークされます。 VM の内容は保存時に暗号化されなくなります。
4. 暗号化を無効にしても、顧客の Key Vault や暗号化キー マテリアル (Windows システムの場合は BitLocker 暗号化キー、Linux の場合はパスフレーズ) は削除されません。
...
* Azure Disk Encryption は、Windows サーバー SKU (Windows Server 2008 R2、Windows Server 2012、Windows Server 2012 R2、および Windows Server 2016) でサポートされています。
...
* Azure CLI コマンドのいずれかを実行して、Azure サブスクリプションに関連付けるには、まず、以下のように Azure CLI バージョンをインストールする必要があります。
  ...
  * Azure Resource Manager での Mac、Linux、および Windows 用 Azure CLI の使用については、[こちら](../virtual-machines/azure-cli-arm-commands.md)を参照してください。
...
* Azure Disk Encryption を使用して暗号化が有効になっている場合、暗号化された VM を Azure バックアップ サービスでバックアップおよび復元するには、Azure Disk Encryption の Key Encryption Key 構成を使用して VM を暗号化する必要があります。 バックアップ サービスでは、Key Encryption Key [KEK] 構成を使用して暗号化された VM のみがサポートされます。 KEK を使用せずに暗号化された VM はサポートされていません。 KEK オプションを使用して VM の暗号化を有効にする方法については、下記のディスク暗号化デプロイメント シナリオをご覧ください。 

#### <a name="setup-the-azure-ad-application-in-azure-active-directory"></a>Azure Active Directory での Azure AD アプリケーションのセットアップ
...
##### <a name="create-a-new-azure-ad-app-using-azure-powershell"></a>Azure PowerShell を使用して新しい Azure AD アプリを作成する
...
##### <a name="provisioning-the-azure-ad-client-id-and-secret-from-the-azure-classic-deployment-model-portal"></a>Azure クラシック デプロイ モデルからの Azure AD のクライアント ID とシークレットのプロビジョニング
Azure AD のクライアント ID とシークレットも、Azure クラシック デプロイメント モデル ポータル (https://manage.windowsazure.com) でプロビジョニングできます。このタスクを実行するには、以下の手順に従います。
...
##### <a name="use-an-existing-app"></a>既存のアプリの使用
...
> [!NOTE]
> AAD 証明書ベースの認証は、現時点では Linux VM ではサポートされていません。
> 
> 

以降のセクションには、Azure AD の証明書ベースの認証を構成するために必要な手順が示されています。

##### <a name="create-a-new-azure-ad-app"></a>新しい Azure AD アプリの作成
...
##### <a name="use-an-existing-azure-ad-app"></a>既存の Azure AD アプリの使用
...
##### <a name="upload-a-pfx-file-to-key-vault"></a>PFX ファイルを Key Vault にアップロードする
...
##### <a name="deploy-a-certificate-in-key-vault-to-an-existing-vm"></a>既存の VM に Key Vault の証明書をデプロイする
...
#### <a name="setting-key-vault-access-policy-for-the-azure-ad-application"></a>Azure AD アプリケーションの Key Vault アクセス ポリシーの設定
...
| PS コマンドレット |[Azure PowerShell コマンドレット](/powershell/azureps-cmdlets-docs) |

### <a name="setting-and-configuring-azure-key-vault-for-azure-disk-encryption-usage"></a>Azure Disk Encryption の使用に関する Azure Key Vault の設定と構成
...
* Azure PowerShell の [Key Vault コマンドレット](https://msdn.microsoft.com/library/dn868052.aspx)を使用する。
...
#### <a name="provisioning-a-key-encryption-key-optional"></a>キー暗号化キーのプロビジョニング (省略可能)
...
#### <a name="set-key-vault-permissions-to-allow-the-azure-platform-access-to-the-keys-and-secrets"></a>Azure プラットフォームからキーとシークレットにアクセスできるように Key Vault のアクセス許可を設定する
...
## <a name="disk-encryption-deployment-scenarios-and-user-experiences"></a>ディスク暗号化のデプロイ シナリオとユーザー エクスペリエンス
ディスク暗号化を有効にできるシナリオは多数あり、手順はシナリオによって異なる場合があります。 以下のセクションでは、これらのシナリオについて詳しく説明します。

### <a name="enable-encryption-on-new-iaas-vms-created-from-the-azure-gallery"></a>Azure ギャラリーから作成された新しい IaaS VM での暗号化を有効にする
...
### <a name="enable-encryption-on-new-iaas-vms-created-from-customer-encrypted-vhd-and-encryption-keys"></a>顧客が暗号化した VHD と暗号化キーから作成された新しい IaaS VM での暗号化を有効にする
...
#### <a name="using-resource-manager-template"></a>Resource Manager テンプレートの使用
...
1. 以下のように、Key Vault でアクセス ポリシーを設定します。
   ...
   * KeyVault にシークレットを書き込むための Azure AD アプリに対するアクセス許可を設定します。`azure keyvault set-policy --vault-name <keyVaultName> --spn <aadClientID> --perms-to-keys '["wrapKey"]' --perms-to-secrets '["set"]'`
2. 既存または実行中の VM に対して暗号化を有効にするには、次のように入力します。`azure vm enable-disk-encryption --resource-group <resourceGroupName> --name <vmName> --aad-client-id <aadClientId> --aad-client-secret <aadClientSecret> --disk-encryption-key-vault-url <keyVaultURL> --disk-encryption-key-vault-id <keyVaultResourceId> --volume-type [All|OS|Data]`
3. 暗号化の状態を取得します。`azure vm show-disk-encryption-status --resource-group <resourceGroupName> --name <vmName> --json`
...
| AADClientID |Key Vault へのシークレットの書き込み権限を持つ Azure AD アプリのクライアント ID |
| AADClientSecret |Key Vault へのシークレットの書き込み権限を持つ Azure AD アプリのクライアント シークレット |
| KeyVaultName |BitLocker キーのアップロード先となる Key Vault の名前。 コマンドレット (Get-AzureRmKeyVault -ResourceGroupName <yourResourceGroupName>) を使用して取得できます。 Vaultname を使用して取得できます。 |
...
#### <a name="using-powershell-cmdlets"></a>PowerShell コマンドレットの使用
PS コマンドレットによる Azure ディスク暗号化を使用して暗号化を有効にする方法の詳細については、**Azure PowerShell を使用した Azure Disk Encryption の探索**に関するブログ投稿の[パート 1](http://blogs.msdn.com/b/azuresecurity/archive/2015/11/17/explore-azure-disk-encryption-with-azure-powershell.aspx) と[パート 2](http://blogs.msdn.com/b/azuresecurity/archive/2015/11/21/explore-azure-disk-encryption-with-azure-powershell-part-2.aspx) を参照してください。
...
### <a name="enable-encryption-on-existing-or-running-iaas-linux-vm-in-azure"></a>Azure で既存または実行中の IaaS Linux VM での暗号化を有効にする
...
### <a name="get-encryption-status-of-an-encrypted-iaas-vm"></a>暗号化された IaaS VM の暗号化状態の取得
...
#### <a name="get-encryption-status-of-an-encrypted-windows-vm-using-azure-resource-manager-portal"></a>暗号化された Windows VM の暗号化状態を、Azure Resource Manager ポータルを使用して取得する
...
#### <a name="get-encryption-status-of-an-encrypted-windowslinux-iaas-vm-using-disk-encryption-ps-cmdlet"></a>暗号化された (Windows/Linux) IaaS VM の暗号化状態を、ディスク暗号化 PS コマンドレットを使用して取得する
...
#### <a name="get-encryption-status-of-the-iaas-vm-from-disk-encryption-cli-command"></a>ディスク暗号化 CLI コマンドから IaaS VM の暗号化状態を取得する
...
##### <a name="disable-encryption-on-existingrunning-iaas-vm-in-azure-using-resource-manager-template"></a>Resource Manager テンプレートを使用して、Azure で既存/実行中の IaaS VM での暗号化を無効にする
[ここ](https://github.com/Azure/azure-quickstart-templates/tree/master/201-decrypt-running-windows-vm)で公開されている Resource Manager テンプレートを使用して、実行中の Windows IaaS VM でのディスク暗号化を無効にすることができます。 Azure クイック スタート テンプレートの [Azure にデプロイ] ボタンをクリックし、パラメーター ブレードに暗号化解除の構成を入力して [OK] をクリックします。 サブスクリプション、リソース グループ、リソース グループの場所、法律条項および契約を選択し、[作成] ボタンをクリックして新しい IaaS VM での暗号化を有効にします。
...
##### <a name="disable-encryption-on-existingrunning-iaas-vm-in-azure-using-ps-cmdlet"></a>PS コマンドレットを使用して、Azure で既存/実行中の IaaS VM での暗号化を無効にする
...
### <a name="preparing-a-pre-encrypted-windows-vhd"></a>事前に暗号化された Windows VHD の準備
...
#### <a name="prepare-os-volume-for-bitlocker-using-bdehdcfg"></a>`bdehdcfg` を使用して BitLocker の OS ボリュームを準備する
OS のパーティションを圧縮して、BitLocker のコンピューターを準備するには、次のコマンドを実行します。

    bdehdcfg -target c: shrink -quiet

#### <a name="using-bitlocker-to-protect-the-os-volume"></a>BitLocker を使用して OS ボリュームを保護する
...
#### <a name="encrypting-os-drive-on-a-running-linux-vm"></a>実行中の Linux VM での OS ドライブの暗号化
...
* (RHEL および CentOS) VM で SELinux を [無効](https://access.redhat.com/documentation/en-US/Red_Hat_Enterprise_Linux/7/html/SELinux_Users_and_Administrators_Guide/sect-Security-Enhanced_Linux-Working_with_SELinux-Changing_SELinux_Modes.html#sect-Security-Enhanced_Linux-Enabling_and_Disabling_SELinux-Disabling_SELinux)にする必要があります。 SELinux を無効にした後、VM を少なくとも 1 回再起動する必要があります。
...
> [!NOTE]
> /etc/fstab にデータ ドライブを指定するには、ブロック デバイス名 (/dev/sdb1 など) ではなく、UUID=... を使用する必要があります。 暗号化中に、ドライブの順序は、VM 上で変更されます。 VM がブロック デバイスの特定の順序に依存している場合、暗号化後のマウントは失敗します。
> 
> 
...
> [!NOTE]
> `systemd` サービスとして実行されているすべてのユーザー スペースのプロセスは、`SIGKILL` によって強制終了されます。 VM を再起動する必要があります。 実行中の VM で OS ディスクの暗号化を有効にする場合は、VM のダウンタイムを予定しておいてください。
> 
> 
...
"*再起動する前*" に、VM の[ブート診断](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/)を保存することをお勧めします。
...
2. [ブート診断](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/)を調べる。 ADE 拡張機能のメッセージには、頭に `[AzureDiskEncryption]` が付きます。
...
#### <a name="preparing-a-pre-encrypted-linux-vhd"></a>事前に暗号化された Linux VHD の準備
##### <a name="ubuntu-16"></a>Ubuntu 16
###### <a name="configure-encryption-during-distro-install"></a>ディストリビューションのインストール中に暗号化を構成します。
...
###### <a name="configure-encryption-to-work-with-azure"></a>Azure で使用する暗号化を構成する
...
3. Windows で *azure_crypt_key.sh* を編集し、Linux にコピーする場合は、必ず、*dos2unix /usr/local/sbin/azure_crypt_key.sh* を実行してください。
...
5. `update-initramfs -u -k all` を実行して initramfs を更新して、`keyscript` を有効にします。
6. 次に、VM をプロビジョニング解除できます。

![Ubuntu 16.04 のセットアップ](./media/azure-security-disk-encryption/ubuntu-1604-preencrypted-fig6.png)

7. 次の手順に進み、Azure に [VHD をアップロード](#upload-encrypted-vhd-to-an-azure-storage-account)します。
...
3. [こちらの手順](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-suse-create-upload-vhd/#prepare-opensuse-131)に従って、VM を Azure にアップロードするための準備をします。 最後の手順 (VM のプロビジョニング解除) はまだ実行しないでください。
...
5. [こちらの手順](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-create-upload-centos/#centos-70)に従って、VM を Azure にアップロードするための準備をします。 最後の手順 (VM のプロビジョニング解除) はまだ実行しないでください。
...
### <a name="upload-disk-encryption-secret-for-the-pre-encrypted-vm-to-key-vault"></a>Key Vault に事前に暗号化された VM のディスク暗号化シークレットをアップロードする
...
### <a name="specify-secret-url-when-attaching-os-disk"></a>OS ディスクをアタッチするときにシークレット URL を指定する
...
## <a name="download-this-guide"></a>このガイドのダウンロード
