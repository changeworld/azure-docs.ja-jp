...
リソース プロバイダーやアプリケーション インスタンスは、キー暗号化キーで暗号化されたデータ暗号化キーを保存します。これは多くの場合、データ暗号化キーで保護されたデータに関するメタデータとして行われます。 キー暗号化キーにアクセスできるエンティティのみが、これらのデータ暗号化キーを復号できます。 キー保存のさまざまなモデルがサポートされます。 詳細については、「[データ暗号化モデル](encryption-models.md)」を参照してください。
