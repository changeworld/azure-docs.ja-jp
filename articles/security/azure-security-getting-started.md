...
* **ロール構成ファイル**: テナントのサービス モデルに基づいて受信アクセス制御リスト (ACL) を定義します。 たとえば、テナントが特定の仮想マシンのポート 80 に Web フロント エンドを持っている場合、[Azure クラシック デプロイ モデル](../azure-resource-manager/resource-manager-deployment-model.md)内にエンドポイントを構成していると、Azure はすべての IP に対して TCP ポート 80 を開きます。 仮想マシンでバックエンド ロールまたは worker ロールが実行されている場合には、同一のテナントにある仮想マシンのみに対して、worker ロールが開かれます。
...
* [**ネットワーク セキュリティ グループ (NSG)**](../virtual-network/security-overview.md)。 NSG を使用して、仮想ネットワーク内で 1 つまたは複数の仮想マシンのインスタンスに対するトラフィックを制御できます。 NSG には、トラフィックの方向、プロトコル、ソース アドレスとポート、および送信先アドレスとポートに基づいて、トラフィックを許可または拒否するアクセス制御ルールが含まれています。
