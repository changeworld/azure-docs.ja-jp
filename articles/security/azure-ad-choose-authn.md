...
* **考慮事項**。 パススルー認証のバックアップの認証方法としてパスワード ハッシュ同期を使用していて、エージェントがユーザーの資格情報を検証できない場合があります。 そのような場合、パスワード ハッシュ同期へのフェールオーバーは自動的には行われません。 Azure AD Connect を使用して、手動でサインイン方法を切り替えてください。 

    パススルー認証は、先進認証と、特定の Exchange Online プロトコルを使用しているクラウド アプリのみをサポートします。 プロトコルの例は、ActiveSync、POP3、IMAP4 などです。 たとえば、Microsoft Office 2013 とそれ以降のバージョンでは先進認証がサポートされますが、以前のバージョンではサポートされません。 Office アプリのサポートについて詳しくは、「[Updated Office 365 modern authentication](https://blogs.office.com/en-us/2015/11/19/updated-office-365-modern-authentication-public-preview/)」(Office 365 の先進認証の更新) をご覧ください。 代替 ID のサポートなど、パススルー認証におけるその他の考慮事項については、[よく寄せられる質問](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-pass-through-authentication-faq)をご覧ください。
