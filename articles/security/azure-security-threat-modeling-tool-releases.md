---
title: Threat Modeling Tool のリリース - Microsoft Threat Modeling Tool - Azure | Microsoft Docs
description: Threat Modeling Tool のリリース ノートです
services: security
documentationcenter: na
author: jegeib
manager: jegeib
editor: jegeib
ms.assetid: na
ms.service: security
ms.workload: na
ms.tgt_pltfrm: na
ms.devlang: na
ms.topic: article
ms.date: 01/15/2019
ms.author: jegeib
ms.openlocfilehash: 12372186bf8fad10236530782b097a0fc2d5a080
ms.sourcegitcommit: a408b0e5551893e485fa78cd7aa91956197b5018
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 01/17/2019
ms.locfileid: "54360600"
---
# <a name="threat-modeling-tool-releases"></a>Threat Modeling Tool のリリース

Microsoft Threat Modeling Tool は現在、Windows 用に無料で[クリックしてダウンロードできる](https://aka.ms/threatmodelingtool)アプリケーションとしてリリースされています。 この配布のしくみでは、ユーザーがツールを開くたびに、最新の改善とバグの修正をプッシュでき、保守と使用が簡単です。

## <a name="system-requirements"></a>システム要件

- サポートされているオペレーティング システム
  - Microsoft Windows 10
- その他の要件
  - ツールとテンプレートの更新プログラムを受け取るには、インターネット接続が必要です。

## <a name="release-notes"></a>リリース ノート

- [Microsoft Threat Modeling Tool GA リリース バージョン 7.1.50911.2](./azure-security-threat-modeling-tool-releases-71509112.md) - 9/12/2018
- [Microsoft Threat Modeling Tool GA リリース バージョン 7.1.51023.1](./azure-security-threat-modeling-tool-releases-71510231.md) - 11/1/2018