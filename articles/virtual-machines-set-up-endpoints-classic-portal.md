<properties 
	...
	description="Azure の仮想マシンと通信できるようにするエンドポイントを旧ポータルで設定する方法について説明します。" 
...
周知のネットワーク プロトコルの IP プロトコルと TCP ポートまたは UDP ポートの既定値は、旧ポータルでエンドポイントを作成するときに提示されます。カスタム エンドポイントの場合、正しい IP プロトコル \(TCP または UDP\) と、パブリック ポートとプライベート ポートを指定する必要があります。着信トラフィックを複数の仮想マシンにランダムに配分するには、複数のエンドポイントで構成される負荷分散セットを作成する必要があります。
...
1.	まだサインインしていない場合は、[Azure の旧ポータル](http://manage.windowsazure.com/)にサインインします。
2.	**\[Virtual Machines\]** をクリックし、構成する仮想マシンの名前をクリックします。
3.	**\[エンドポイント\]** をクリックします。\[エンドポイント\] ページに仮想マシンの現在のすべてのエンドポイントが示されます。

	![エンドポイント](./media/virtual-machines-set-up-endpoints-classic-portal/endpointswindows.png)
...
![エンドポイントの作成に成功](./media/virtual-machines-set-up-endpoints-classic-portal/endpointwindowsnew.png)
 
PowerShell の構成については、「[Add-AzureEndpoint](https://msdn.microsoft.com/library/azure/dn495300.aspx)」をご覧ください。
...
1.	まだサインインしていない場合は、Azure の旧ポータルにサインインします。
2.	**\[Virtual Machines\]** をクリックし、構成する仮想マシンの名前をクリックします。
...
PowerShell を使用した ACL の構成については、「[PowerShell を使用したエンドポイントのアクセス制御リスト \(ACL\) の管理](https://msdn.microsoft.com/library/azure/dn376543.aspx)」をご覧ください。
