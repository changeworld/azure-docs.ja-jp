...
これは SLA に対応するために役立ちます (「[SLA の監視](#SLA-monitoring)」については、このガイダンスの中で後述します)。”ダウンタイム” の定義は、サービスによって異なります。たとえば、Visual Studio Online では、ダウンタイムを、顧客がサービスに接続しようしている時間が 120 秒を超えている期間と、その時間内に接続は確立されたがすべての基本的な読み取りと書き込みの操作が失敗した期間と定義しています。
