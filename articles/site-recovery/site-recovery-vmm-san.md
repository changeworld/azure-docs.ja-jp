...
1. SAN 記憶域が VMM に統合されたら、論理ユニット (LUN) を作成 (プロビジョニング) します。
	...
	>[AZURE.NOTE] マシンのレプリケーションを有効にした後に、Site Recovery のレプリケーション グループに配置されていない LUN にその VHD を追加してはなりません。追加された VHD は Site Recovery によって検出されません。
...
14. **初期クラウド メタデータの同期**で、コンテナーに表示されるサーバーの分かりやすい名前を指定して、VMM サーバー上のすべてのクラウドのメタデータをコンテナーと同期するかどうかを選択します。この操作は、各サーバーで 1 回のみ実行する必要があります。すべてのクラウドを同期したくない場合は、この設定をオフのままにして、VMM コンソールのクラウドのプロパティで各クラウドを個別に同期できます。
...
3. **管理者**特権でコマンド プロンプトから次のコマンドを実行して、プロバイダのインストーラーを抽出します。
