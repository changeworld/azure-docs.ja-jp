---
title: "Azure Site Recovery を使用して Azure に物理サーバーをレプリケートするための容量とスケーリングを計画する | Microsoft Docs"
description: "この記事を使用して、Azure Site Recovery を使用して Azure に Windows/Linux 物理サーバーをレプリケートするための容量とスケーリングを計画します"
...
この記事では、[Azure Site Recovery](site-recovery-overview.md) を使用してオンプレミスの Windows/Linux 物理サーバーを Azure にレプリケートする際の容量とスケーリングについて説明します。
...
## <a name="plan-deployment-capacity"></a>デプロイする容量を計画する

1. この[記事](site-recovery-plan-capacity-vmware.md)で、レプリケーションの要件を推定する方法を理解し、Site Recovery コンポーネントのサイズを決定するためのガイダンスを確認してください。
2. 次に示す考慮事項を確認して、コンポーネント サーバーをスケーリングする方法と、レプリケーされるマシンの帯域幅を制御する方法を理解してください。

## <a name="replication-considerations"></a>レプリケーションに関する注意事項

これらの考慮事項を確認して、レプリケートされるサーバーの要件を理解してください。
...
1. 追加のプロセス サーバーをセットアップするには、[この手順](site-recovery-vmware-setup-azure-ps-resource-manager.md)に従います。
2. パスフレーズを取得していない場合は、構成サーバーで **[Site Recovery インストール先フォルダー]\home\sysystems\bin\genpassphrase.exe –n** を実行して取得します。
3. プロセス サーバーをセットアップしたら、それを使用するようにソース マシンを移行します。

    1. **[設定]** > **[Site Recovery サーバー]** で、[構成サーバー] > **[プロセス サーバー]** をクリックします。
    2. 現在使用中のプロセス サーバーを右クリックし、**[切り替え]** をクリックします。
    3. **[ターゲット プロセス サーバーの選択]** で、使用するプロセス サーバーを選択し、そのサーバーが処理する VM を選択します。
...
4. 勤務時間用と勤務時間外用の制限値を設定します。 有効な範囲は、1 秒あたり 512 Kbps ～ 102 Mbps です。
    ...
    ![スロットル](./media/physical-walkthrough-capacity/throttle2.png)
