---
title: Azure Site Recovery を使用して VMware VM/物理サーバーをオンプレミス サイトに再保護する
...
- マスター ターゲットには、レプリケートされた項目の OS バージョンと同じかそれ以上の OS バージョンが必要です。
...
オンプレミス サイトにフェールバックする前に、Azure 内にプロセス サーバーが必要です。

- プロセス サーバーは、Azure 内の保護された仮想マシンからデータを受け取り、オンプレミス サイトにデータを送信します。
- プロセス サーバーと保護された仮想マシンとの間には、待ち時間が短いネットワークが必要です。 そのため、Azure にプロセス サーバーをデプロイすることをお勧めします。 レプリケーションのパフォーマンスは、プロセス サーバーが、レプリケートしている仮想マシン (Azure 内のフェールオーバーされたマシン) に近いほど高くなります。 
- 概念実証の場合、オンプレミスのプロセス サーバーと、プライベート ピアリングを使用した ExpressRoute を利用できます。
