...
2. Recovery Services コンテナーで、ソース レプリケーションの設定を指定します。 このプロセスの一環として、オンプレミスの構成サーバーをセットアップします。 このサーバーを VMware VM としてデプロイするには、準備済みの OVF テンプレートをダウンロードし、それを VMware にインポートして VM を作成します。
