---
title: "Azure Site Recovery を使用してオンプレミス サイト間に Hyper-V VM のディザスター リカバリーを設定する |Microsoft Docs"
...
---
# <a name="set-up-disaster-recovery-for-hyper-v-vms-to-your-secondary-on-premises-site"></a>セカンダリ オンプレミス サイトに対する Hyper-V VM のディザスター リカバリーを設定する
...
このチュートリアルでは、System Center Virtual Machine Manager (VMM) クラウドで管理されるオンプレミスの Hyper-V VM で、セカンダリ サイトに対するディザスター リカバリーを設定する方法について説明します。 このチュートリアルで学習する内容は次のとおりです。
...
- [シナリオのアーキテクチャとコンポーネント](concepts-hyper-v-to-secondary-architecture.md)を確認する。
- すべてのコンポーネントの[サポート要件](site-recovery-support-matrix-to-sec-site.md)を確認する。
- VMM サーバーと Hyper-V ホストが[サポート要件](site-recovery-support-matrix-to-sec-site.md#on-premises-servers)に準拠していることを確認する。
...
[ネットワーク マッピング](site-recovery-network-mapping.md)は、ソース クラウドとターゲット クラウド内のオンプレミス VMM VM ネットワーク間をマップします。 マッピングでは次が行われます。
...
3. **[サーバーの追加]** で、**[サーバーの種類]** に **System Center VMM Server** が表示されることを確認します。
...
5. インストールが完了したら、**[登録]** をクリックして、サーバーをコンテナーに登録します。
    ...
    ![インストール場所](./media/tutorial-vmm-to-vmm/provider-register.png)
...
2. Site Recovery と同期されている VMM クラウドが表示されます。 ターゲット クラウドを選択します。
   ...
   ![[ターゲット]](./media/tutorial-vmm-to-vmm/target-vmm.png)
...
[ディザスター リカバリー訓練を実行する](tutorial-dr-drill-secondary.md)
