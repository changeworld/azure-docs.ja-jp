...
* *マスター ターゲット サーバーにストレージ vMotion を使用することはできません*。 そのようにした場合、フェールバックが失敗する可能性があります。 ディスクが利用可能にならないため、この仮想マシンは起動しません。 これを回避するには、vMotion の一覧からマスター ターゲット サーバーを除外します。
...
> [!TIP]
> フェールバック時には常に Azure ベースのプロセス サーバーを使用することをお勧めします。 レプリケーションのパフォーマンスは、プロセス サーバーがレプリケートする仮想マシン (Azure でフェールオーバーするマシン) に近いほど高くなります。 ただし、概念実証やデモでは、オンプレミスのプロセス サーバーと、プライベート ピアリングを設定した ExpressRoute を使用して、高速化することができます。
...
* 仮想マシンが vCenter サーバー上でオンプレミスに存在しない場合、Site Recovery サービスは再保護時に新しい仮想マシンを作成する必要があります。 マスター ターゲットを作成する ESX ホストでこの仮想マシンは作成されます。 そのため、希望するホストにフェールバック仮想マシンが作成されるように、ESX ホストは慎重に選択します。

* *マスター ターゲット サーバーにストレージ vMotion を使用することはできません*。 そのようにした場合、フェールバックが失敗する可能性があります。 ディスクが利用可能にならないため、この仮想マシンは起動しません。

> [!WARNING]
> マスター ターゲットでストレージ vMotion の再保護が行われる場合、そのマスター ターゲットに接続されている、保護されている仮想マシンのディスクは、vMotion のターゲットに移行されます。 この後にフェールバックしようとすると、ディスクのデタッチに失敗し、ディスクを検出できなかったことを示すメッセージが表示されます。 この後でストレージ アカウントでディスクを検出することは、非常に難しくなります。 ディスクを手動で検索して仮想マシンに接続する必要があります。 その後、オンプレミスの仮想マシンを起動できるようになります。
...
> [!NOTE]
> オンプレミスの仮想マシンは、再保護の過程でオフになります。 これは、レプリケーション中のデータの整合性を確保するためです。 再保護の完了後は、仮想マシンをオンにしないでください。
