...
3. **構成サーバー** (クラシックに適用可能): オンプレミスのソース環境に構成サーバーを設定する必要があります。
...
4. **Azure Site Recovery レプリケーション アプライアンス** (プレビューに適用可能): オンプレミスのソース環境にレプリケーション アプライアンスを設定する必要があります。 このアプライアンスは、Azure Site Recovery オンプレミス インフラストラクチャ全体の基本的な構成要素です。 VMware のディザスター リカバリーの場合は、ダウンロード可能な OVF テンプレートからデプロイできる [VMware VM としてデプロイする](deploy-vmware-azure-replication-appliance-preview.md#create-azure-site-recovery-replication-appliance)ことをお勧めします。  レプリケーション アプライアンスの詳細については、[こちら](vmware-azure-architecture-preview.md)を参照してください。   
5. **ターゲット環境**:Azure サブスクリプションおよびネットワーク設定を指定して、ターゲットの Azure 環境を設定します。
6. **レプリケーション ポリシー**:レプリケーションの実行方法を指定します。 設定には、復旧ポイントを作成および保存する頻度、アプリ整合性スナップショットを作成するかどうかなどがあります。
7. **レプリケーションを有効にします**。 オンプレミス マシンのレプリケーションを有効にします。 モビリティ サービスをインストールするためのアカウントを作成した場合、マシンのレプリケーションを有効にすると、インストールされます。
