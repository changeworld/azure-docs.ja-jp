...
ソース仮想マシンで Azure Disk Encryption (ADE) が有効になっている場合、暗号化の設定に関する以下のセクションが表示されます。
...
既定値をオーバーライドし、カスタムのキー コンテナーを選択するには、暗号化設定の横の [カスタマイズ] をクリックします。
