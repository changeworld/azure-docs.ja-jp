...
|ソース リージョンには 1 TB の Premium ディスクを使った 1 つの VM がある。<br/>127 GB のデータのみが使用され、残りのディスクは空である。<br/>ディスクの種類は 200 MBps スループットの Premium。<br/>フェールオーバー後にデータは変更されない。| おおよその時間:2 時間。<br/>再保護中、Site Recovery によってデータ全体のチェックサムが生成される。 これは 25 MBps (ディスク スループットの最大 16%) で動作するため、合計所要時間は 127 GB/25 MBps で、約 87 分。<br/>Site Recovery で自動スケールを行うために、およそ 20 から 30 分のオーバーヘッド時間が必要。 |
|ソース リージョンには 1 TB の Premium ディスクを使った 1 つの VM がある。<br/>127 GB のデータのみが使用され、残りのディスクは空である。<br/>ディスクの種類は 200 MBps スループットの Premium。<br/>フェールオーバー後に 45 GB のデータが変更される。| おおよその時間:2.5 時間から 3 時間。<br/>再保護中、Site Recovery によってデータ全体のチェックサムが生成される。 これは 25 MBps (ディスク スループットの最大 16%) で動作するため、合計所要時間は 127 GB/25 MBps で、約 87 分。</br>転送速度は、スループットの約 16% (32 MBps)。 そのため、45 GB の変更を適用する転送時間は 45 GB/32 MBps で、およそ 24 分。<br/>Site Recovery で自動スケールを行うために、およそ 20 から 30 分のオーバーヘッド時間が必要。 |
