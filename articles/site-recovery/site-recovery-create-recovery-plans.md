  ...
  * デプロイメントで VMM を使用している場合は、以下に注意してください。
    ...
    * ライブラリ共有パスがリモートである (または、ローカルであるが MSCVMMLibrary と共有されていない) 場合は、次の手順を実行して共有パスを構成します (ここでは、例として \\libserver2.contoso.com\share\ が使用されています):
      ...
      * VMM サービス アカウントと同じアクセス許可が付与されているユーザー アカウントを使用してスクリプトをテストし、スタンドアロンのテスト済みスクリプトが、復旧計画で実行されるのと同じ方法で実行されることを確認します。 VMM サーバーで、次の手順を実行して、実行ポリシーを bypass に設定します。
        ...
        * 「 **Set-executionpolicy bypass**」と入力します。 詳細については、[ここ](https://technet.microsoft.com/library/ee176961.aspx)をクリックしてください。
...
1. **[復旧計画]** タブ、**[復旧計画の作成]** の順にクリックします。
   復旧計画の名前、およびソースとターゲットを指定します。 ソース サーバーには、フェールオーバーと復旧が有効になった仮想マシンが必要になります。
   
   * VMM から VMM にレプリケートする場合、**[ソースの種類]**  >  **[VMM]** を選択し、ソース VMM サーバーとターゲット VMM サーバーを選択します。 **[Hyper-V]** をクリックして、Hyper-V レプリカを使用するように構成されているクラウドを表示します。 
   * SAN を使用して VMM から VMM にレプリケートする場合、**[ソースの種類]**  >  **[VMM]** を選択し、ソース VMM サーバーとターゲット VMM サーバーを選択します。 **[SAN]** をクリックして、SAN レプリケーション用に構成されているクラウドを表示します。
   * VMM から Azure にレプリケートする場合、**[ソースの種類]**  >  **[VMM]** を選択します。  ソース VMM サーバーと、ターゲットとしての **Azure** を選択します。
   * Hyper-V サイトからレプリケートする場合、**[ソースの種類]**  >  **[Hyper-V サイト]** を選択します。 そのサイトをソースとして選択し、 **Azure **をターゲットとして選択します。
...
## <a name="customize-recovery-plans"></a>復旧計画のカスタマイズ
...
### <a name="create-an-azure-automation-runbook"></a>Azure Automation Runbook の作成
復旧計画の一部として Azure Automation Runbook を実行することにより、計画を拡張できます。 詳細については、[こちら](site-recovery-runbook-automation.md)を参照してください。

### <a name="add-custom-settings-to-a-recovery-plan"></a>復旧計画へのカスタム設定の追加
1. カスタマイズする復旧計画を開きます。
