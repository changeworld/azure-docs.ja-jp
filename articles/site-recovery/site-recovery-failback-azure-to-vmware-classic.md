...
この記事では、Azure からオンプレミス サイトへ Azure Virtual Machines をフェールバックする方法について説明します。この記事の手順は、[このチュートリアル](site-recovery-vmware-to-azure-classic.md)を使用して、オンプレミス サイトから Azure へ VMware 仮想マシンまたは Windows/Linux 物理サーバーをフェールオーバーした後、それらをフェールバックする準備ができたら使用します。
...
- Azure にフェールオーバー後、オンプレミス サイトへはいくつかの段階でフェールバックします。
	...
	- **第 3 段階**: データのフェールバックを完了したら、フェールバックしたオンプレミスの VM を Azure へのレプリケートを開始するよう再保護します。
...
- 元の VM にフェールバックする場合、次が必要です。
	- VM が vCenter サーバーによって管理されている場合、オンプレミス サイトの VM は、オンプレミスのマスター ターゲット サーバーで実行されている VM と同じデータ ストアを使用する必要があります。そうでない場合、同じ VM にフェールバックするために移行する必要があります。
	- VM が ESX ホスト上にあるが、vCenter によって管理されていない場合、VM のハード ディスクはマスター ターゲット VM と同じデータストアにある必要があります。
	- VM が ESX ホスト上にあり vCenter を使用しない場合、再保護する前に、ESX ホストの検出を完了する必要があります。これは、物理サーバーにフェールバックする場合も同様です。
...
2. セットアップを実行し、**[開始する前に]** で **[デプロイをスケールアウトするためにプロセス サーバーを追加する]** を選択します。
3. [管理サーバーをセットアップ](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)したのと同じ方法でウィザードを完了します。**[構成サーバーの詳細]** ページで、このマスター ターゲット サーバーの IP アドレスと VM にアクセスするためのパスフレーズを指定します。
...
1.	管理サーバー VM に CentOS 6.6 最小構成オペレーティング システムをインストールします。DVD ドライブに ISO イメージを保持し、システムを起動します。メディアのテストを省略し、言語として [英語 (米国)] を選択し、**[基本的な記憶域デバイス]** を選択し、ハード ドライブに重要なデータがないことを確認し、**[はい]** をクリックしてすべてのデータを破棄します。管理サーバーのホスト名を入力し、サーバーのネットワーク アダプターを選択します。**[編集システム]** ダイアログで [* * 自動的に接続する * *] を選択し、静的 IP アドレス、ネットワーク、および DNS 設定を追加します。管理サーバーにアクセスするためにタイム ゾーンとルート パスワードを指定します。 
2.	希望するインストールの種類を求められたら、パーティション用に **[カスタム レイアウトの作成]** を選択します。**[次へ]** をクリックし、**[Free]** を選択し、[作成] をクリックします。**FS Type:** **ext4** を使用し、**/**、**/var/crash** および **/home partitions** を作成します。**FS Type: swap** を使用しスワップ パーティションを作成します。
3.	既存のデバイスが検出される場合、警告メッセージが表示されます。**[形式]** をクリックして、パーティションの設定を使用してドライブをフォーマットします。**[変更をディスクに書き込む]** をクリックして、パーティションの変更を適用します。
4.	**[ブート ローダーのインストール]**、**[次へ]** を選択して、ルート パーティションにブート ローダーをインストールします。
5.	インストールが完了したら、**[再起動]** をクリックします。


#### SCSI ID を取得する

1. インストール後、VM 内の各 SCSI ハード ディスクの SCSI ID を取得します。これを実行するには、管理サーバー VM をシャットダウンし、VMware の VM のプロパティで [VM エントリ]、**[設定の編集]**、**[オプション]** を順に右クリックします。
2. **[高度]**、**[一般的な項目]**、**[構成パラメーター]** を順に選択します。このオプションは、コンピューターの実行中は無効になります。アクティブにするには、コンピューターをシャット ダウンする必要があります。
3. **disk.EnableUUID** という行が存在する場合、値が **True** (大文字と小文字は区別されます) に設定されていることを確認します。設定されている場合、キャンセルして、ゲスト オペレーティング システムの起動後に、その OS 内で SCSI コマンドをテストします。 
4.	行が存在しない場合は、**True** 値を使用し、**[行の追加]** をクリックします。二重引用符は使用しないでください。
...
6.	再保護を開始するために **[OK]** をクリックすると、VM を Azure からオンプレミスサイトにレプリケートするジョブが開始されます。進捗状況は **[ジョブ]** タブで追跡できます。
...
1.	**[復旧計画]** ページでフェールバック グループを含む復旧計画のページを選択し、**[フェールオーバー]**、**[計画されていないフェールオーバー]** をクリックします。
