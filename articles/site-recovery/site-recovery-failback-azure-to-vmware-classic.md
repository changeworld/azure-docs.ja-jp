...
2.	希望するインストールの種類を求められたら、パーティションとして **[Create Custom Layout (カスタム レイアウトを作成する)]** を選択します。**[次へ]** をクリックし、**[Free (Free)]** を選択して、[作成] をクリックします。**FS Type:** **ext4** を使用して、**/**、**/var/crash**、**/home** の各パーティションを作成します。**FS Type: swap** を使用してスワップ パーティションを作成します。
