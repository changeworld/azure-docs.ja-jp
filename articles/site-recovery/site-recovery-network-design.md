...
障害復旧ソリューションの導入を計画する管理者がまず抱く疑問のひとつは、フェールオーバーの完了後、どのようにして仮想マシンへのアクセスを確保するかです。フェールオーバー後に仮想マシンの接続先となるネットワークは、管理者が ASR のネットワーク マッピングを使用して選択できます。ネットワーク マッピングの詳細については、「[Azure Site Recovery で VMM を使用して Hyper-V 仮想マシンを保護するために、ネットワーク マッピングを準備する](site-recovery-network-mapping.md)」を参照してください。

復旧サイト用のネットワークを設計する場合、管理者には次の 2 つの選択肢があります。

- 復旧サイトのネットワーク用に異なる IP アドレス範囲を使用する。このシナリオでは、フェールオーバー後の仮想マシンに、新しい IP アドレスが割り当てられます。この場合 DNS の更新は管理者が行う必要があります。DNS の更新方法については、こちらをご覧ください。
...
このドキュメントでは、障害復旧に使用するネットワーク アドレス空間の設計に関して、技術的に重要となる 2 つの考慮事項を取り上げます。

- Azure Site Recovery (ASR) を使ってワークロードを保護するように、ファブリック マネージャー (System Center Virtual Machine Manager) のデプロイ トポロジを設計する。
- 障害復旧環境における IP アドレス管理。


## Azure Site Recovery 用に System Center VMM のデプロイを設計する

トポロジを決定するのはビジネス要件です。複数のデータ センターを単一の管理サーバーで管理するか、複数のデータ センターを複数の管理サーバーで管理するかは、組織がビジネス要件に基づいて選択します。ここでは、ASR でワークロードを保護するように System Center Virtual Machine Manager (SCVMM) のデプロイ トポロジを設計する方法について説明します。

Azure Site Recovery は、さまざまなデプロイ シナリオでの仮想マシンのレプリケーション、フェールオーバー、復旧を調整してビジネス継続性と障害復旧 (BCDR) 戦略に貢献します。ASR の設計方針は単純さです。既にあるデータ センターのトポロジ上に障害復旧ソリューションを管理者が構築できるように ASR はモデル化されています。ASR を利用することで組織は、そのビジネス ニーズに最も合ったトポロジを選ぶことができます。

全デプロイ シナリオの一覧については、「[Site Recovery とは](site-recovery-overview.md)」を参照してください。

プライマリ サイトにある VMM サーバー上のワークロードは、セカンダリ サイトを管理する VMM サーバーで保護することができます。障害が発生した場合、セカンダリ VMM サーバーによって管理されているセカンダリ サイトに、プライマリ サイトのすべてのワークロードがシームレスにフェールオーバーされます。しかし組織によっては、管理のオーバーヘッドを低く抑えるために、そのすべてのデータ センターを単一の VMM サーバーで管理した方が都合のよい場合もあります。そのようなトポロジを選ぶ場合、ワークロードを復旧する前にまず、VMM サーバーを復旧できることが不可欠となります。そのためワークロードの "耐障害性" を確保するために、VMM のトポロジは慎重に設計する必要があります。

インフラストラクチャに VMM サーバーが 1 つしかない場合は、Azure Site Recovery をデプロイして Azure に VMM クラウドの仮想マシンをレプリケートするか、単一の VMM サーバー上のクラウド間でレプリケートすることができます。復旧には、(Hyper-V Manager コンソールで Hyper-V レプリカを使用して) Azure Site Recovery コンソール以外から VMM サーバーを手動でフェールオーバーする必要があります。最善の RTO (目標復旧時間) と RPO (回復ポイントの目標) でワークロードを復旧できるよう、VMM サーバーは次のいずれかのトポロジでデプロイすることをお勧めします。

### スタンドアロン デプロイ

このトポロジでは、プライマリ サイト内の仮想マシンにスタンドアロンの VMM サーバーをデプロイしたうえで、その仮想マシンを、Azure Site Recovery と Hyper-V レプリカを使ってセカンダリ サイトにレプリケートします。VMM サーバーと、そのサポート SQL Server を同じ仮想マシンにインストールすれば、1 つの VM をインスタンス化するだけで済むので、場合によってはダウンタイムを抑えることができます。VMM サービスがリモート SQL Server を使用している場合、まず SQL Server インスタンスを復旧してから VMM サーバーを復旧する必要があります。

Hyper-V レプリカを有効にした VM に単一の VMM をデプロイする手順は次のとおりです。
...
5. HYPER-V Manager コンソールに移動し、HYPER-V レプリカを選択し、VMM VM でレプリケーションを有効にします。ASR サービスによって保護されているクラウドに VMM VM が決して追加されないようにします。これにより、Hyper-V レプリケーション設定は ASR で上書きされません。
...
このトポロジでワークロードをフェールオーバーするにはまず、ユーザーが手動で VMM VM をセカンダリ サイトにフェールオーバーする必要があります。
...
高可用性 VMM サーバーのデプロイは、VMM サービスそのものをクラスターに対応させる手法です。VMM でクリティカルなワークロードを管理している場合は、この手法をお勧めします。ワークロードの可用性を確保すると共に、VMM サーバーを確実に障害から保護することができます。VMM サーバーの常時稼働はきわめて重要な要素です。このサービスをクラスターに対応させることで、VMM の実行環境となっている VM 内で起こりうる問題だけでなく、VMM サーバーの実行環境となっているホストのハードウェア フェールオーバーからも VMM サーバーを保護することができます。高可用性 VMM サーバーをデプロイするためには、対象となる VMM サーバーが Windows Server フェールオーバー クラスター上に存在する必要があります。高可用性 VMM サーバーをデプロイする方法について詳しくは、System Center に関するこちらのブログ記事を参照してください。

ASR を使ってワークロードを保護するには、地理的に離れた複数のサイトに展開されたクラスター上で VMM サーバーをデプロイする必要があります (図 2)。VMM によって使用される SQL Server データベースは、セカンダリ サイト上にレプリカを配置して、SQL Server AlwaysOn 可用性グループで保護する必要があります。障害が発生した場合まず、VMM サーバーとそれに対応する SQL Server データベースが復旧サイトに対して自動的にフェールオーバーされ、その後ですべてのワークロードが、ASR を使ってフェールオーバーされます。

![拡張された VMM クラスター](./media/site-recovery-network-design/network-design1.png)

図 2

## 障害復旧環境におけるネットワーク アドレス空間の設計
...
障害復旧作業の観点からいえば、固定 IP アドレスを使うのが最も導入しやすいように見えますが、実際には、さまざまな問題が潜んでいるために、この方法が選ばれることはまれです。Azure Site Recovery を使用すれば、すべてのシナリオで IP アドレスを保持することができます。IP アドレスを維持する場合は、フェールオーバー機能に課せられる制約をあらかじめ十分に把握したうえで判断する必要があります。IP アドレスを維持するかどうかの判断に役立つと思われる要素について見てみましょう。このカテゴリには、拡張サブネットとサブネット フェールオーバーという、大きく 2 つのサブタイプが存在します。2 拠点間のサブネット フェールオーバーと拡張サブネットが存在するシナリオについて個別に説明したいと思います。
...
Contoso という架空の企業を例に、サブネット全体をフェールオーバーしながら、どのようにして VM を復旧場所にレプリケートできるのかを見てみましょう。まず、2 つのオンプレミス拠点間で VM をレプリケーションするときのサブネットの管理方法に注目し、その後、障害復旧サイトとして Azure を使用した場合にサブネットのフェールオーバーがどのように動作するかについて説明します。

##### サブネット フェールオーバー (企業向けの障害復旧)
...
以下の図では、プライマリ サイトと復旧サイトとの間、第 3 のサイトとプライマリ サイトとの間、そして第 3 のサイトと復旧サイトとの間のルートに適宜変更を加える必要があります。ここでは次の条件を想定しています。

- データセンターに必要なサービスは、それぞれ独自の System Center VMM インスタンスによって提供されます。データ センター間で System Center VMM データベースのレプリケーションは行われません。
- 仮想マシンの静的 IP アドレスが、それぞれのデータセンターで使用されます。
- データセンター間の接続は、インターネット経由の VPN 接続を使用するのでなく、専用回線を使用して確立されます。
...
特定の仮想マシンの保護を有効にする際に ASR は、次のワークフローに従ってネットワーク リソースを割り当てます。
...
##### オンプレミス サブネット フェールオーバー (Azure への DR)

Azure Site Recovery (ASR) では、仮想マシンの障害復旧サイトとして Microsoft Azure を使用できます。この場合、対処すべき制約はさらに増えます。
...
Woodgrove のオンプレミス インフラストラクチャは、VMM 2012 R2 サーバーによって管理されます。System Center VMM サーバーには既に、"Application Network" という VLAN ベースの論理ネットワークが作成されています。この論理ネットワークを使用して、"Application VM Network" という VM ネットワークを作成します。アプリケーション内の仮想マシンはすべて静的 IP アドレスを使用します。このため、論理ネットワークに対しても静的 IP プールを定義します。

**論理ネットワーク**

![論理ネットワーク](./media/site-recovery-network-design/network-design5.png)

Figure 6

**VM ネットワーク**

![VM ネットワーク](./media/site-recovery-network-design/network-design6.png)
...
フェールオーバーが作動し、指定した IP の仮想マシンが Recovery Network 内に作成されると、このネットワークへの接続が VNet 間接続を使って確立されます。必要であれば、この操作をスクリプト化することもできます。サブネットのフェールオーバーについて前のセクションで説明したように、Azure へのフェールオーバーの場合も、192.168.1.0/24 が Azure に移動されたことを反映するためにルートを適切に変更する必要があります。
