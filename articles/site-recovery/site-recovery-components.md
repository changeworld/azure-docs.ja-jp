...
このクラシック ポータルと Azure ポータルの両方で Site Recovery を使用することができます。Azure クラシック ポータルでは、クラシック サービス管理モデルで Site Recovery をサポートできます。Azure ポータルでは、クラシック モデルまたは Resource Manager デプロイメント モデルをサポートできます。Azure ポータルのデプロイの詳細については、[こちら](site-recovery-overview.md#site-recovery-in-the-azure-portal)をご覧ください。
...
Site Recovery は、これらの VM と物理サーバーで実行されているほとんどのアプリをレプリケートできます。サポートされるアプリの概要については、「[Azure Site Recovery で保護できるワークロード](site-recovery-workload.md)」をご覧ください。
...
- Azure ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-vmware-to-azure.md#azure-prerequisites)をご覧ください。
- クラシック ポータルの強化されたデプロイの要件の詳細については、[こちら](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment)をご覧ください。
- Azure ポータルを使用したフェールバックの詳細については、[こちら](site-recovery-failback-azure-to-vmware.md)をご覧ください。
- クラシック ポータルを使用したフェールバックの詳細については、[こちら](site-recovery-failback-azure-to-vmware-classic.md)をご覧ください。
...
- **Hyper-V ホスト**: 1 つ以上の Windows Server 2012 R2 Hyper-V ホスト サーバーが必要です。Site Recovery のデプロイ時にインストールします。
...
- Azure ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-hyper-v-site-to-azure.md#azure-prerequisites)をご覧ください。
- クラシック ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-hyper-v-site-to-azure-classic.md#azure-prerequisites)をご覧ください。
...
- Azure ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-vmm-to-azure.md#azure-requirements)をご覧ください。
- クラシック ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-vmm-to-azure-classic.md#before-you-start)をご覧ください。
...
**Azure アカウント**: InMage Scout を使用してこのシナリオをデプロイします。InMage Scout を入手するには、Azure サブスクリプションが必要です。Site Recovery コンテナーを作成した後で、InMage Scout をダウンロードし、最新の更新プログラムをインストールして、デプロイメントをセットアップします。**プロセス サーバー (プライマリ サイト)**: キャッシュ、圧縮、およびデータの最適化を処理できるように、プライマリ サイトでプロセス サーバーのコンポーネントをセットアップします。プロセス サーバーでは、保護するマシンへの統合エージェントのプッシュ インストールも処理します。**VMware ESX/ESXi および vCenter サーバー (プライマリ サイト)**: VMware VM を保護する場合、ハイパーバイザーを管理するために、VMware EXS/ESXi ハイパーバイザーと、必要に応じて VMware vCenter サーバーが必要です。
...
- Azure ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-vmm-to-vmm.md#azure-prerequisites)をご覧ください。
- - Azure クラシック ポータルを使用したデプロイの要件の詳細については、[こちら](site-recovery-vmm-to-vmm-classic.md#before-you-start)をご覧ください。
...
デプロイの要件の詳細については、[こちら](site-recovery-vmm-san.md#before-you-start)をご覧ください。
### オンプレミスの
