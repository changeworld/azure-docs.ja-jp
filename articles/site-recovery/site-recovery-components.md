...
| **プライマリ サイト** |**プロセス サーバー**: キャッシュ、圧縮、およびデータの最適化を処理できるように、プライマリ サイトでプロセス サーバーのコンポーネントをセットアップします。 プロセス サーバーでは、保護するマシンへの統合エージェントのプッシュ インストールも処理します。<br/><br/> **VMware ESX/ESXi と vCenter**: VMware EXS/ESXi ハイパーバイザーと、オプションとして VMware vCenter サーバーが必要です。<br/><br/> **VM** |
