...
この記事では、Site Recovery を使用して、異なる Azure リージョン間で Azure IaaS 仮想マシンを移行する方法について説明します。この記事のほとんどは、「[オンプレミスの VMware 仮想マシンまたは物理サーバーと Azure 間の保護の設定](site-recovery-vmware-to-azure-classic-legacy.md)」で説明した手順を使用しています。デプロイの各手順の詳細については、そちらの記事をお読みいただくことをお勧めします。
...
4. [プロセス サーバーのデプロイ](site-recovery-vmware-to-azure.md-classic-legacy#step-4-deploy-the-on-premises-process-server)。以下の点に注意してください。
...
6. [保護グループの作成](site-recovery-vmware-to-azure-classic-legacy.md#step-7-create-a-protection-group)。Site Recovery を使用して移行された仮想マシンの保護を開始するには、保護グループを設定する必要があります。グループにレプリケーション設定を指定すると、そのグループに追加するすべてのコンピューターに適用されます。 
7. [仮想マシンの設定](site-recovery-vmware-to-azure-classic-legacy.md#step-8-set-up-machines-you-want-to-protect)。モビリティ サービスを各 VM にインストールする必要があります (自動または手動で)。
8. [ステップ 8: 仮想マシンの保護の有効化](site-recovery-vmware-to-azure-classic-legacy.md#step-9-enable-protection)。VM を保護グループに追加し、VM の保護を有効にします。以下の点に注意してください。
