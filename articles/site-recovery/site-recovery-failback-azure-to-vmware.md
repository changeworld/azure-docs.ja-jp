...
* 物理サーバーをフェールオーバーした場合、常に新しい VMware VM にフェールバックは実行されます。
  * 物理マシンをフェールバックする前に以下に注意してください。
    * 保護されている物理マシンは Azure から VMware にフェールオーバーして戻った場合に、仮想マシンとして返されます。 Windows Server 2008 R2 SP1 の物理コンピューターが保護されていて、Azure にフェールオーバーした場合、これをフェールバックすることはできません。 オンプレミスの仮想マシンとして開始した WS 2008R2 SP1 はフェールバックできます
...
5. *クラシック デプロイメント* ネットワークを選択した場合、Azure ギャラリーを使用して新しい VM を作成し、プロセス サーバーをインストールするよう求められます。
   ...
   2. [こちらに記載](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server)
...
> [!NOTE]
> Linux にマスター ターゲット サーバーをインストールするには、次の手順に従います。 MT OS 向けの最小限の CentOS 6.6 オペレーティング システムのみを使用していることを確認してください。
> 
> 
...
3. 保護しているソース マシンが、ルートまたはブート デバイス用に Linux wit Reiser または XFS ファイル システムを実行している場合、次のようなパッケージを追加でインストールする必要があります。
   ...
   * # <a name="yum-install-device-mapper-multipath---this-is-required-to-enable-multipath-packages-on-the-mt-server"></a>yum install device-mapper-multipath   これは、MT サーバーでマルチパス パッケージを有効にするために必要です。
...
5. プロセス サーバーから CS にアクセスできない場合、ポート 443 での CS コンピューターへの Telnet によってプロセス サーバーからの接続を確認できます。 PS コンピューターから CS に ping を送信してみることもできます。 また通常、CS に接続されている場合、PS サーバーにハートビートがあります。
6. 代替の vCenter にフェールバックする場合は、新しい vCenter が検出されており、またマスター ターゲット サーバーも検出されていることを確認してください。 典型的な症状として、データストアが [再保護] ダイアログに表示されない、またはアクセスできないといったものがあります。
7. 物理オンプレミス コンピューターとして保護されている WS2008R2SP1 コンピューターは、Azure からオンプレミスにフェールバックすることができません。
