...
6. VMM サーバーが切断状態になっていた場合は、VMM サーバーで[クリーンアップ スクリプト](/samples/browse/?redirectedfrom=TechNet-Gallery)をダウンロードして実行します。 **[管理者として実行]** オプションを使用して PowerShell を開き、既定 (LocalMachine) のスコープの実行ポリシーを変更します。 スクリプトで、削除する VMM サーバーの ID を指定します。 このスクリプトにより、登録とクラウドのペアリングの情報がサーバーから削除されます。
