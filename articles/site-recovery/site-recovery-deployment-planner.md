...
4. 接続 VIServer がコマンドレットの名前として認識されない場合は、次のコマンドを実行するオプションが必要する可能性があります。
