---
title: "Azure Content Delivery Network のキャッシュのしくみ | Microsoft Docs"
...
この記事では、キャッシュの全般的概念の概要と、Azure Content Delivery Network (CDN) がキャッシュを使用してパフォーマンスを向上させるしくみについて説明します。 CDN エンドポイントでのキャッシュの動作をカスタマイズする方法については、[キャッシュ規則による Azure CDN キャッシュ動作の制御](cdn-caching-rules.md)に関するページ、および[クエリ文字列による Azure CDN キャッシュ動作の制御](cdn-query-string.md)に関するページをご覧ください。
...
Web ブラウザーと同様、キャッシュ ディレクティブ ヘッダーを送信することによって CDN キャッシュの実行方法を制御できます。 キャッシュ ディレクティブ ヘッダーは HTTP ヘッダーであり、通常は、配信元サーバーによって追加されます。 元来、こうしたヘッダーのほとんどはクライアント ブラウザーでのキャッシュに対応するためのものでしたが、現在は CDN などのすべての中間キャッシュでも使用されます。 2 つのヘッダー (`Cache-Control` と `Expires`) を使用して、キャッシュの更新間隔を定義できます。 `Cache-Control` が最新で、`Expires` に優先します (両方が存在する場合)。 検証に使用されるヘッダーの種類も 2 つあります (検証コントロールと呼ばれます)。これは、`ETag` と `Last-Modified` です。 `ETag` が最新で、`Last-Modified` に優先します (両方が定義されている場合)。  
...
- 要求ヘッダーで使用されている場合: 既定では、Azure CDN によって無視されます。
- 応答ヘッダーで使用されている場合: Azure CDN は、一般的な Web 配信、大きなファイルのダウンロード、および一般的なメディア ストリーミング/ビデオ オン デマンド メディア ストリーミングの最適化を使用する場合に、次の `Cache-Control` ディレクティブを優先します。  
   - `max-age`: キャッシュは、指定された秒数だけコンテンツを格納できます。 たとえば、「 `Cache-Control: max-age=5`」のように入力します。 このディレクティブは、コンテンツが新鮮であると見なされる最大時間を指定します。
   - `private`: コンテンツは 1 人のユーザーのみを対象としています。CDN など、キャッシュで共有するコンテンツは保存されません。
   - `no-cache`: コンテンツをキャッシュします。ただし、キャッシュから配信する前に、必ずそのコンテンツを検証する必要があります。 `Cache-Control: max-age=0` と同等です。
...
既定では、DSA の最適化によってこうしたヘッダーが無視されます。 Azure CDN によるこうしたヘッダーの処理方法を調整するには、CDN キャッシュ規則を使用します。 詳細については、[キャッシュ規則による Azure CDN キャッシュ動作の制御](cdn-caching-rules.md)に関するページをご覧ください。

## <a name="validators"></a>検証コントロール

キャッシュが古い場合は、HTTP キャッシュ検証コントロールを使用して、キャッシュされたバージョンのファイルと、配信元サーバーのバージョンが比較されます。 **Azure CDN from Verizon** では、ETag 検証コントロールと最終更新時検証コントロールの両方が既定でサポートされますが、**Azure CDN from Akamai** が既定でサポートするのは、最終更新時検証コントロールのみです。
...
`Last-Modified`
- **Azure CDN from Verizon のみ**については、ETag が HTTP 応答に含まれない場合、最終更新時検証コントロールが使用されます。 
...
| HTTP メソッド       | GET                    | GET                              |
| ファイル サイズ         | 300 GB                 | <ul><li>一般的な Web 配信の最適化: 1.8 GB</li> <li>メディア ストリーミングの最適化: 1.8 GB</li> <li>大きなファイルの最適化: 150 GB</li> |

## <a name="default-caching-behavior"></a>既定のキャッシュ動作

次の表では、Azure CDN 製品の既定のキャッシュ動作とその最適化について説明します。

|                    | Verizon - 一般的な Web 配信 | Verizon – 動的サイト アクセラレーション | Akamai - 一般的な Web 配信 | Akamai - 動的サイト アクセラレーション | Akamai - 大きなファイルのダウンロード | Akamai - 一般的なメディア ストリーミングまたはビデオ オン デマンド メディア ストリーミング |
|--------------------|--------|------|-----|----|-----|-----|
| **配信元を優先**   | あり    | なし   | あり | なし | 可能  | あり |
...
- キャッシュ規則を使用して CDN の既定のキャッシュ動作をカスタマイズおよび上書きする方法については、[キャッシュ規則による Azure CDN キャッシュ動作の制御](cdn-caching-rules.md)に関するページをご覧ください。 
