---
title: "Azure Content Delivery Network のキャッシュ動作をキャッシュ規則で制御する | Microsoft Docs"
...
---
# <a name="control-azure-content-delivery-network-caching-behavior-with-caching-rules"></a>Azure Content Delivery Network のキャッシュ動作をキャッシュ規則で制御する

> [!NOTE] 
> キャッシュ規則は、**Azure CDN from Verizon Standard** および **Azure CDN from Akamai Standard** に対してのみ使用できます。 **Azure CDN from Verizon Premium** の場合は、**管理**ポータルの [Azure CDN ルール エンジン](cdn-rules-engine.md)を使って同様の機能を実現できます。
 
Azure Content Delivery Network には、ファイルのキャッシュ方法を制御する方法が 2 つあります。 
...
2. 左側のウィンドウの [設定] で、**[キャッシュ]** をクリックします。
...
4. カスタム キャッシュ規則を次のように作成します。
  ...
  ![キャッシュ規則ダイアログ](./media/cdn-caching-rules/cdn-caching-rules-dialog.png)
