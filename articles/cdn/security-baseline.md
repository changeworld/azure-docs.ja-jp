---
title: Content Delivery Network の Azure セキュリティ ベースライン
description: Content Delivery Network セキュリティ ベースラインでは、Azure セキュリティ ベンチマークで指定されているセキュリティに関する推奨事項を実装するための手順のガイダンスとリソースが提供されます。
author: msmbaldwin
...
ms.locfileid: "96530971"
---
# <a name="azure-security-baseline-for-content-delivery-network"></a>Content Delivery Network の Azure セキュリティ ベースライン

このセキュリティ ベースラインにより、[Azure セキュリティ ベンチマーク バージョン 2.0](../security/benchmarks/overview.md) のガイダンスが Content Delivery Network に適用されます。 Azure セキュリティ ベンチマークには、Azure 上のクラウド ソリューションをセキュリティで保護する方法に関する推奨事項がまとめてあります。 内容は、Content Delivery Network に適用される Azure セキュリティ ベンチマークと関連ガイダンスで定義されている **セキュリティ制御** によってグループ化されています。 Content Delivery Network に適用されない **制御** は、除外されています。

Content Delivery Network を完全に Azure セキュリティ ベンチマークにマップする方法については、[完全な Content Delivery Network セキュリティ ベースライン マッピング ファイル](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines)を参照してください。
...
**ガイダンス**:国または地域によって Content Delivery Network のコンテンツへのアクセスを制限します。 CDN エンドポイントの特定のパスに対するルールを作成し、geo フィルタリング機能を使用して、選択した国または地域でコンテンツを許可またはブロックします。

- [国/地域別の Azure CDN コンテンツの制限](cdn-restrict-access-by-country.md)
...
**ガイダンス**:Content Delivery Network で Azure ロールベースのアクセス制御 (Azure RBAC) を使用して、含まれているサブスクリプションと管理グループへの特権アクセスを許可するアカウントを制限することにより、ビジネス クリティカルなシステムへのアクセスを分離します。
...
**ガイダンス**:Content Delivery Network は、Azure のロールベースのアクセス制御 (RBAC) と統合されてリソースを管理します。 Azure RBAC を使用して、ロールの割り当てによって Azure リソースへのアクセスを管理します。 これらのロールを、ユーザー、グループ、サービス プリンシパル、マネージド ID に割り当てます。 特定のリソースに対して定義済みの組み込みロールがあります。これらのロールは、Azure CLI、Azure PowerShell、Azure portal などのツールを使用してインベントリまたは照会できます。 

Azure RBAC を使用してリソースに割り当てる特権は、ロールで必要なものに応じて制限します。 これは、Azure AD Privileged Identity Management (PIM) のジャスト イン タイム (JIT) アプローチを補完するものであり、定期的に見直す必要があります。 
...
**ガイダンス**:セキュリティ チームに Azure テナントのセキュリティ閲覧者アクセス許可とサブスクリプションを付与し、Azure Security Center を使用してセキュリティ上のリスクを監視できるようにします。 
...
**ガイダンス**:ライフサイクル管理プロセスの資産のセキュリティを確保するために Content Delivery Network を使用することはできません。 影響が大きいと見なされる資産の属性とネットワーク構成を維持することは、お客様の責任です。 お客様にて、属性とネットワーク構成の変更をキャプチャするプロセスを作成し、変更の影響を測定し、必要に応じて修復タスクを作成することをお勧めします。
...
**ガイダンス**:Content Delivery Network では、リソースに関連したセキュリティ上の脅威を監視するためのネイティブ機能は提供されていません。

Content Delivery Network のログはすべて、カスタム脅威検出を設定するために使用できるセキュリティ情報イベント管理 (SIEM) ソリューションに転送します。 

潜在的な脅威や異常がないか、さまざまな種類の Azure 資産を監視します。質の高いアラートを得ることに専念し、アナリストが調べなければならない誤検知を減らします。 アラートは、ログ データ、エージェント、その他のデータを元に生成できます。
...
**ガイダンス**:Content Delivery Network は、仮想ネットワークにデプロイするためのものではありません。このため、ネットワーク セキュリティ グループのフロー ログを有効化したり、ファイアウォール経由でトラフィックをルーティングしたり、パケット キャプチャを実行したりすることはできません。

Content Delivery Network では、お客様のアクセスのために処理されるすべてのネットワーク トラフィックがログに記録されます。 デプロイされたオファリング リソース内でネットワーク フロー機能を有効にし、長期保存と監査のためにストレージ アカウントに送信されるようにこれらのログを構成します。

- [チュートリアル -  Azure Content Delivery Network で診断ログを設定する方法](cdn-azure-diagnostic-logs.md)
...
**ガイダンス**:自動的に利用できるアクティビティ ログには、読み取り操作 (GET) を除き、Content Delivery Network リソースに対するすべての書き込み操作 (PUT、POST、DELETE) が含まれています。 アクティビティ ログを使用すると、トラブルシューティング時にエラーを見つけたり、組織内のユーザーがリソースをどのように変更したかを監視したりできます。
...
**ガイダンス**:適用なし。Content Delivery Network では、セキュリティ構成機能は提供されていません。
...
一般的なリスク スコアリング プログラム (例: Common Vulnerability Scoring System) またはサードパーティのスキャン ツールによって提供される既定のリスク評価を使用して、修復の優先順位を設定します。 アプリケーションで高いセキュリティ上のリスクが生じるコンテキストと、高いアップタイムが必要となるものを使用して、環境に合わせて調整します。
