...
ルールの 3 番目の部分は、機能です。 機能では、一連の一致条件によって特定された要求の種類に適用するアクションの種類を定義します。
...
トークン認証拒否コードは、[常に] 一致条件と一緒に使用することはできません。 代わりに、**管理**ポータルの **[トークン認証]** ページの **[Custom Denial Handling]\(拒否のカスタム処理\)** セクションを使用します。 詳細については、「[トークン認証による Azure CDN 資産の保護](cdn-token-auth.md)」をご覧ください。
