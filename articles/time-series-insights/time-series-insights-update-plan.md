...
Time Series Insights 環境のタイム シリーズ モデルを構成できるようになりました。 新しいモデルにより、IoT データの検索と分析が簡単になります。 時系列データのキュレーション、メンテナンス、およびエンリッチメントが可能になり、コンシューマー対応データ セットの準備に役立ちます。 モデルでは、タイム シリーズ ID が使われ、これは、型と呼ばれる変数と階層に一意のリソースを関連付けるインスタンスにマップされます。 新しい[タイム シリーズ モデル](./concepts-model-overview.md)について参照してください。
...
- [データ インジェスト](./concepts-ingestion-overview.md)の詳細を確認します。
- [データ ストレージ](./concepts-storage.md)に関する記事を確認します。
- Time Series Insights プレビューの[データ モデリング](./concepts-model-overview.md)について学習する。
