...
認証方法の管理制御| **| *| * PIN モード| | | * 不正アクセスのアラート| | | * MFA レポート| | | * ワンタイム バイパス| | | * 音声通話に対するカスタムの案内応答| | | * 音声通話の発信元 ID のカスタマイズ| | | * イベントの確認| | | * 信頼できる IP| | | * 記憶されたデバイスに対する MFA の一時停止 (パブリック プレビュー)| | | * MFA SDK| | | * MFA サーバーを使用した、オンプレミス アプリケーション用の MFA| | | *
