...
* モバイル アプリまたは携帯電話で通知または電話呼び出しが受信されないためにユーザーがサインインできないシナリオに対処するように、テクニカル サポート スタッフをトレーニングします。 テクニカル サポート担当者は、[ワンタイム バイパスを有効に](multi-factor-authentication-whats-next.md#one-time-bypass)し、2 段階認証を "バイパス" することによってユーザーを 1 回だけ認証できるようにすることができます。 バイパスは一時的なものであり、指定された秒数が経過すると無効になります。
