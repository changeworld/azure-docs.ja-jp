---
title: Preparing and testing your offer for deployment to the Azure Marketplace (プランを準備およびテストして Azure Marketplace にデプロイする) | Microsoft Docs
...
### サンプル イメージ (省略可能)
> [!NOTE]
> サンプル イメージを含めるステップは、省略可能です。発行ポータルには複数のサンプル イメージをアップロードできますが、Azure ポータルに表示されるイメージは 1 つだけです (システムによってランダムに選択されます)。この理由から、アップロードするのは 1 つのサンプル イメージだけにすることをお勧めします。
> 
> 
