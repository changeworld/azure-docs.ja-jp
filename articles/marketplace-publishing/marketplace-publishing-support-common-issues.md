...
この記事では、次のようないくつかの一般的な Marketplace のシナリオのサポートを得たり、これに関する問題を解決したりする方法を説明します。
...
* [AZURE.NOTE]顧客または Azure リセラーから、返金またはクレジットの要求が弊社宛になされた場合、Azure サービスの提供で使用するのと同じポリシーが適用されます。それらのポリシーで返金またはクレジットが指示される場合、弊社は事前に振り出して、発行元は Microsoft が発行元に変わって行う顧客または Azure リセラー宛ての返金またはクレジットを承認することになります。ポリシーで返金もクレジットも指示されない場合、顧客または Azure リセラーに対して、直接発行元に連絡を取るように通知する場合があります。

  ![1\_RequestRefund][1]

  1. サポート ページに移動します
  2. **[顧客の管理]** を選択します
  3. **[お客様への返金の承認または要求]** を選択します
  4. **[要求の開始]** をクリックします

  ![2\_RequestRefund][2]

5. 連絡先情報を入力します
6. [どんな問題ですか?] に入力し、以下の情報を入力します。
...
その後 24 時間以内に、提供された情報を受け取った旨の受信確認電子メールが届きます。要求に関する決定が下されたらすぐに、その結果が通知されます。
...
- 新しい Chrome Incognito または IE InPrivate ブラウズ セッションを開き、個人アカウントにログインしていないことを確認します
- http://sellerdashboard.microsoft.com に移動します

![inprivate][4]

**2.会社登録の Microsoft アカウント (MSA) を使用してサインインします** (例: AzureStore@yourcompany.com)
...
次の問題については、https://portal.azure.com に問題を記録してください
...
次の問題については、http://go.microsoft.com/fwlink/?LinkId=272975 でサポート要求を作成してください

- Web サービスの管理に関する問題があります
- 自分の販売者のアカウントに関連付けられている MSA を変更する方法を教えてください。
