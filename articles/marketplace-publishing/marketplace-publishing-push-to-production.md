...
>[AZURE.IMPORTANT] Virtual Machines の場合、[Request approval to push to production (本番にプッシュするための承認を要求)] をクリックすると、次の手順が見えないところで実行されます。各手順の進行状況は、発行ポータルの [発行] タブで確認できます。このページは (ステータスが "表示済み" になるまで) 定期的にチェックして、ご自身で修正する必要があるエラーがないかどうかを確認してください。
...
## すべての Virtual Machine プラン用の運用環境チェックリスト
...
## 既に表示されている Virtual Machine プラン用の追加の運用環境チェックリスト
