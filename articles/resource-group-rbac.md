...
3. 操作 - リソースに対して実行されるアクション (読み取り、作成など) です。 
4. スコープ - サブスクリプション、リソース グループ、リソースなど、ロールが適用されるレベルです。
5. ロール割り当て - ロールにプリンシパルを関連付けてスコープを設定するプロセスです。

Azure クロスプラット フォーム (xPlat) CLI ツールまたは Azure PowerShell モジュールを使用して、サポートされている**アクション**の一覧を取得できます。

Azure クロスプラット フォーム CLI ツールでは、次のコマンドを使用して、すべてのリソース プロバイダーの**すべて**のアクションの一覧を取得します。

    azure provider operations show --operationSearchString '*';

Azure リソース マネージャー PowerShell モジュール (AzureRm) を使用している場合は、次のコマンドを使用して、**すべて**のリソース プロバイダーの**すべて**のアクションの一覧を取得します。
...
| 役割 | 実行できる操作 |
| ------- | ----------------- |
| 閲覧者 | **/読み取り (任意の内容を読み取り可能) |
| 所有者 | * (任意の内容を読み取り/書き込み可能) |
