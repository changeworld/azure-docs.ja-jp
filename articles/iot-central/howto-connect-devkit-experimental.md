...
1. **サンプル Devkit** アプリケーション テンプレートから作成された Azure IoT Central アプリケーション。 詳細については、[アプリケーションの作成のクイック スタート](quick-deploy-iot-central-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)に関するページをご覧ください。
...
1. Device Explorer から**実際のデバイス**を追加し、**[+ 新規] > [Real]\(リアル\)** をクリックして実際のデバイスを追加します。
    ...
    * デバイス名を入力するか、推奨名を使います
    ...
    ![デバイスの追加](media/howto-connect-devkit-experimental/add-device.png)

1. デバイス ページの **[接続]** をクリックして、追加したデバイスの **スコープ ID、デバイス ID、プライマリ キー**などの接続の詳細を取得します。

    ![接続の詳細](media/howto-connect-devkit-experimental/device-connect.png)
...
1. Web ブラウザーを開き、[http://192.168.0.1/start](http://192.168.0.1/start) に移動します。 次の Web ページが表示されます。
    ...
    ![デバイス構成ページ](media/howto-connect-devkit-experimental/configpage.png)
...
1. **[デバイスの構成]** を選択すると、次のページが表示されます。
    ...
    ![構成されたデバイス](media/howto-connect-devkit-experimental/deviceconfigured.png)
...
1. **Device Explorer** を使用して、追加した MXChip 実デバイスの **[Measurements] (測定)** ページに移動します。
    ...
    ![実デバイスに移動する](media/howto-connect-devkit-experimental/realdevicenew.png)

1. **[Measurements] (測定)** ページで、MXChip デバイスから来たテレメトリを表示できます。

    ![実デバイスからのテレメトリを表示する](media/howto-connect-devkit-experimental/devicetelemetrynew.png)

1. **[プロパティ]** ページで、デバイスによって報告された最後のサイコロの数字とデバイスの場所を表示できます。

    ![デバイスのプロパティを表示する](media/howto-connect-devkit-experimental/devicepropertynew.png)

1. **[設定]** ページで、MXChip デバイス上の設定を更新できます。

    ![デバイス設定を表示する](media/howto-connect-devkit-experimental/devicesettingsnew.png)

1. **[ダッシュボード]** ページで、場所のマップを確認できます

    ![デバイス ダッシュボードの表示](media/howto-connect-devkit-experimental/devicedashboardnew.png)
