...
デバイス テンプレートの変更について詳しくは、「[既存のデバイス テンプレートを編集する](howto-edit-device-template.md)」を参照してください。
...
他のインターフェイスのデバイス テンプレートをアセンブルするには、コンポーネントを使用します。 たとえば温度コントローラーのデバイス テンプレートには、複数のサーモスタット コンポーネントが含まれる可能性があります。 コンポーネントはデバイス テンプレートで直接編集できるほか、JSON ファイルとしてエクスポートしたりインポートしたりすることもできます。 デバイスは、コンポーネントのインスタンスと対話することができます。 たとえば 2 つのサーモスタットを備えるデバイスは、各サーモスタットからのテレメトリを、IoT Central アプリケーション内の異なるコンポーネントに送信できます。

## <a name="inheritance"></a>継承

インターフェイスは、継承を使用して拡張することができます。 既存のインターフェイスに機能を追加するには継承を使用します。 継承されたインターフェイスは、デバイスに対して透過的です。
...
インポートしたコンポーネントを変更したり、機能に IoT Central 固有の機能を追加したりする必要がある場合は、カスタマイズを利用します。 既存のデバイス テンプレートの機能は、任意の部分をカスタマイズできます。
