...
1. **[Generate]\(生成\)** をクリックします。
    ![アクセス トークン ページのスクリーンショット](media/howto-use-iotc-explorer-experimental/accesstokenspage.png)

1. トークン名を入力し、**[次へ]** をクリックして、**トークンの値をコピー**します。
    > [!NOTE]
    > トークンの値が表示されるのは 1 回だけであるため、ダイアログを閉じる前にコピーする必要があります。 ダイアログを閉じた後、再び表示されることはありません。

    ![アクセス トークンのコピーに関するダイアログのスクリーンショット](media/howto-use-iotc-explorer-experimental/copyaccesstoken.png)
...
![monitor-messages コマンドの出力](media/howto-use-iotc-explorer-experimental/monitormessages.png)
...
![get-twin コマンドの出力](media/howto-use-iotc-explorer-experimental/getdevicetwin.png)
...
これで IoT Central エクスプローラーの使用方法についての学習が終わりました。推奨される次の手順は、[IoT Central でのデバイスの管理](howto-manage-devices-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)について確認することです。
