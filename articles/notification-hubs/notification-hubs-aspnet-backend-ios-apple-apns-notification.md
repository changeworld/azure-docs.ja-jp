...
4. `ViewController.h` で、インポート ステートメントの後ろに次の `#define` を追加します。 `<Enter Your Backend Endpoint>` のプレースホルダーは、前のセクションでアプリのバックエンドのデプロイに使用した宛先 URL に置き換えます。 たとえば、*http://your_backend.azurewebsites.net* です。
