...
このドキュメントでは、Microsoft Sentinel がオートメーションを使用して、SOC の効果と効率を向上させる方法について学習しました。
