...
1. 使用するワークスペースを選択するか、新しいワークスペースを作成します。 複数のワークスペースで Azure Sentinel を実行できますが、データは 1 つのワークスペースに分離されます。
   ...
   >[!IMPORTANT]
   > - ワークスペースで Azure Sentinel を有効にすると、そのワークスペースを他のリソース グループまたはサブスクリプションに移動することはサポートされません。 ワークスペースを移動する必要がある場合は、[Azure Sentinel](mailto:AzureSentinel@microsoft.com) にお問い合わせください。
