---
title: Azure Sentinel プレビューで Syslog データを収集する | Microsoft Docs
description: Azure Sentinel で Syslog データを収集する方法について説明します。
services: sentinel
...
> [!NOTE]
> ご利用のアプライアンスで Syslog CEF がサポートされている場合、接続はより詳細になるので、このオプションを選択し、[CEF からのデータの収集](connect-common-event-format.md)に関するページに記載の手順に従う必要があります。

## <a name="how-it-works"></a>動作のしくみ

Syslog の収集は、Linux 用エージェントを使用して実現されます。 既定では、Linux 用エージェントは Syslog デーモンからのイベントを UDP 経由で受信しますが、Linux マシンで大量の Syslog イベントが収集されると予想される場合 (Linux エージェントが他のデバイスからイベントを受信している場合など)、Syslog デーモンとエージェントの間で TCP トランスポートを使用するように構成が変更されます。

## <a name="connect-your-syslog-appliance"></a>Syslog アプライアンスを接続する

1. Azure Sentinel ポータルで、**[データ収集]** を選択し、**[Syslog]** タイルを選択します。
...
3. Syslog コネクタの設定に戻り、**[Configure the logs to be collected]\(収集するログの構成\)** をクリックします。 
