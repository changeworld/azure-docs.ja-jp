...
1.  ホストと API キー (小文字) をサービスに有効な値で置き換え、要求ヘッダーを指定します。
...
HTTP 504 が表示された場合は、URL で HTTPS の指定を確認してください。HTTP 404 が表示された場合は、構文をダブルクリックしてください。HTTP 400 は、API キーに問題があることを意味します (キーが無効か、または API キーの指定方法に構文エラーがあります)。
