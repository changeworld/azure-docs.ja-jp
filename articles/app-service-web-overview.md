...
[App Service Web Apps](http://go.microsoft.com/fwlink/?LinkId=529714) の能力を活用することで、インフラストラクチャには二度と不安を感じることなく、使い慣れた言語やフレームワークを使用し、アプリケーションを Azure Cloud にすばやくデプロイし、コードを継続的に改善することができます。

![Web Marketplace](./media/app-service-web-overview/marketplace.png)

## 単なる Web アプリ以上のもの##

現代の企業は、これまでにないほど洗練された方法で顧客と交流しています。あらゆる種類の企業が、企業 Web サイトをビジネスに不可欠な部分と見なし、ビジネス プランにおける主要なコンポーネントであると考えています。企業はこの重要性に応えるために、機敏性、安全性、拡張性が得られるプラットフォームを探し求めています。さらに、企業は既存のビジネス システムをリンクできる機能や、新しいコードをすばやくデプロイし、世界中でインスタンスをスピン アップできる機能を必要としています。Azure App Service と Web Apps を利用することで、企業は迅速かつ費用効率に優れた方法で顧客の要求に応えることができます。
...
- **自動スケーリング** - Web Apps では、将来の顧客負荷に対処するため、スケール アップやスケール アウトをすばやく実行できます。VM の数やサイズを手動で選択するか、負荷またはスケジュールに基づきサーバーのスケーリングを行うように自動スケーリングを設定します。
- **継続的な統合** - VSO、GitHub、TeamCity、Hudson または BitBucket で継続的な統合とデプロイメントのワークフローを設定します。これにより、コードのチェックインまたは統合テストが成功するたびに Web アプリのビルド、テスト、およびデプロイを自動的に実行できます。
...
- **運用環境でのテスト** - ステージングされたデプロイメントを次のレベルに移行し、[A/B テスト][Tip]を実行して、構成可能な一部のライブ トラフィックで新しいコードを検証します。 
...
## 作業の開始 ##
Web Apps を使用するには、「[ASP.NET Web アプリケーションを作成する] [create]」のチュートリアルに従ってください。
