<properties urlDisplayName="Call a custom API from the client" pageTitle="Windows Phone クライアントからのカスタム API 呼び出し - Mobile Services" metaKeywords="" description="カスタム API を定義し、Azure Mobile Services を使用する Windows Phone アプリケーションからそれを呼び出す方法について説明します。" metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Call a custom API from the client" authors="glenga" solutions="" manager="dwrede" editor="" />
