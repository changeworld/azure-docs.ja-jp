...
この記事では、SSH を使用して HDInsight クラスターで Hadoop に接続し、Hive コマンド ライン インターフェイス (CLI) を使用して Hive クエリを対話的に実行する方法について説明します。
...
* SSH クライアントssh クライアントを備えた Linux、Unix、および Mac OSWindows ユーザーは、クライアントを  <a href="http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html" target="_blank">Putty</a> としてダウンロードする必要があります。
...
2. 接続したら、次のコマンドを使用して、Hive コマンド ライン インターフェイス (CLI) を起動します。
