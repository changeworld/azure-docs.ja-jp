...
> [WACOM.NOTE] この記事では、Azure 管理ポータル、Azure PowerShell、および Azure クロスプラットフォーム コマンド ライン インターフェイスで診断ログを使用する方法を示します。Visual Studio で診断ログを使用する方法の詳細については、「[Visual Studio での Azure Web サイトのトラブルシューティング][Visual Studio での Azure Web サイトのトラブルシューティング]」を参照してください。

## 目次

-   [Web サイト診断とは][Web サイト診断とは]
-   [方法: 診断を有効化する][方法: 診断を有効化する]
-   [方法: ログをダウンロードする][方法: ログをダウンロードする]
-   [方法: ログをストリーミングする][方法: ログをストリーミングする]
-   [方法: 診断ログを読む][方法: 診断ログを読む]
-   [次のステップ][次のステップ]
...
Azure Websites には、Web サーバーと Web アプリケーション両方からのログ情報の診断機能が備わっています。これらは論理的に**サイト診断**と**アプリケーション診断**に分けられます。
...
-   **Web サーバーのログ記録**: [W3C 拡張ログ ファイル形式][W3C 拡張ログ ファイル形式]を使用して、Web サイト上のすべての HTTP トランザクションをログに記録します。このレポートが便利なのは、全体的なサイト メトリック、たとえば、サイトで処理された要求の数や、特定の IP アドレスからの要求の数を特定するときです。

### アプリケーション診断

アプリケーション診断では、Web アプリケーションによって生成された情報を取り込むことができます。ASP.NET アプリケーションは、[System.Diagnostics.Trace][System.Diagnostics.Trace] クラスを使用して、情報をアプリケーション診断ログに記録できます。次に例を示します。
...
Visual Studio でアプリケーション診断を使用する方法の詳細については、「[Visual Studio での Azure Web サイトのトラブルシューティング][1]」を参照してください。

> [WACOM.NOTE] web.config ファイルを変更する場合と異なり、アプリケーション診断を有効にしたり、診断ログ レベルを変更したりしても、アプリケーションが実行されているアプリケーション ドメインがリサイクルされることはありません。

Azure Websites では、Web サイトにアプリケーションを発行したときにデプロイ情報もログに記録されます。これは自動的に行われ、展開ログの構成設定はありません。展開ログでは、展開が失敗した理由を特定できます。たとえば、カスタムの展開スクリプトを使用している場合は、展開ログを使用して、スクリプトでエラーが発生する理由を特定できることがあります。
...
診断は、[Azure 管理ポータル][Azure 管理ポータル]の Azure Website にある **[構成]** ページにアクセスすることで有効にできます。**[構成]** ページで、**アプリケーション診断**と**サイト診断**を使用してログを有効にします。
...
> [WACOM.NOTE] 診断を有効にするには、Azure PowerShell から **Set-AzureWebsite** コマンドレットを使用する方法もあります。Azure PowerShell をインストールしていない場合や、Azure サブスクリプションを使用するように構成していない場合は、「[Azure PowerShell の使用方法][Azure PowerShell の使用方法]」を参照してください。
...
-   **Web サーバー ログ**: /LogFiles/http/RawLogs。このフォルダーには、[W3C 拡張ログ ファイル形式][W3C 拡張ログ ファイル形式]を使用して形式が設定された 1 つ以上のテキスト ファイルが格納されます。

-   **デプロイ ログ**: /LogFiles/Git。このフォルダーには、Git デプロイのログだけでなく、Azure Websites が使用する内部デプロイ プロセスによって生成されたログも格納されます。

### FTP

FTP を使用して診断情報にアクセスするには、Azure 管理ポータルで Web サイトの**ダッシュボード**にアクセスします。**[概要]** セクションで、**[FTP 診断ログ]** リンクを使用し、FTP を使用してログ ファイルにアクセスします。**[展開/FTP ユーザー]** エントリには、FTP サイトへのアクセスに使用するユーザー名が一覧表示されます。

> [WACOM.NOTE] **[デプロイ/FTP ユーザー]** エントリが設定されていない場合や、このユーザーのパスワードを忘れた場合は、**ダッシュボード**の **[概要]** セクションで **[デプロイ資格情報のリセット]** リンクを使用することで、新しいユーザーとパスワードを作成できます。
...
> [WACOM.NOTE] Azure PowerShell をインストールしていない場合や、Azure サブスクリプションを使用するように構成していない場合は、「[Azure PowerShell の使用方法][2]」を参照してください。
...
> [WACOM.NOTE] Azure コマンド ライン ツールをインストールしていない場合や、Azure サブスクリプションを使用するように構成していない場合、「[Azure コマンド ライン ツールの使用方法][Azure コマンド ライン ツールの使用方法]」を参照してください。
...
> [WACOM.NOTE] ログのストリーミングでは、**D:\\home\\LogFiles\\** フォルダーに格納されているテキスト ファイルに書き込まれた情報がすべてストリーミングされます。
...
| ApplicationName | Web サイトの名前                             |
| InstanceId      | イベントが発生した Web サイトのインスタンス  |
...
![失敗した要求をブラウザーで表示したところ][失敗した要求をブラウザーで表示したところ]
...
Web サーバー ログは [W3C 拡張ログ形式][W3C 拡張ログ ファイル形式] (.aspx) で書式設定されます。この情報は、テキスト エディターを使用して表示したり、[Log Parser][Log Parser] などのユーティリティで解析したりできます。

> [WACOM.NOTE] Azure Websites によって生成されるログでは、**s-computername**、**s-ip**、**cs-version** のフィールドはサポートされていません。
...
-   [チュートリアル: Web サイトのトラブルシューティング][チュートリアル: Web サイトのトラブルシューティング]
-   [Visual Studio での Azure Web サイトのトラブルシューティング][3]
-   [HDInsight での Web サイト ログの分析][HDInsight での Web サイト ログの分析]
  ...
  [次のステップ]: #nextsteps
  [W3C 拡張ログ ファイル形式]: http://msdn.microsoft.com/library/windows/desktop/aa814385.aspx
  [System.Diagnostics.Trace]: http://msdn.microsoft.com/ja-jp/library/36hhw2t6.aspx
  [1]: http://www.windowsazure.com/ja-jp/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/
  [Azure 管理ポータル]: https://manage.microsoft.com
  ...
  [2]: http://www.windowsazure.com/ja-jp/develop/nodejs/how-to-guides/powershell-cmdlets/
  [Azure コマンド ライン ツールの使用方法]: http://www.windowsazure.com/ja-jp/develop/nodejs/how-to-guides/command-line-tools/
  ...
  [3]: /ja-jp/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/
