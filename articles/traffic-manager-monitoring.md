...
5. **監視対象ファイルへのアクセス試行 (4 回の試行)** - 監視システムは GET を実行していますが、10 秒以内に応答が返されません。次に、監視システムは、30 秒間隔で、さらに 3 回試行します。つまり、監視システムでサービスが使用できなくなったことを検出するまでに、最大で約 1.5 分かかります。3 回の試行のいずれかが成功した場合、試行の回数はリセットされます。図には表示されていませんが、200 OK メッセージが GET の後 10 秒を過ぎてから返された場合でも、監視システムはこのチェックを失敗としてカウントします。
...
次の表では、入れ子になったプロファイルに対する子と親のプロファイルおよび minChildEndpoints 設定に対する Traffic Manager の監視の動作について説明します。詳細については、「Traffic Manager の概要」(traffic-manager-overview.md) を参照してください。

|子プロファイル モニターの状態|親エンドポイント モニターの状態|メモ|
|---|---|---|
|無効。これは、ユーザーによってプロファイルが無効にされているためです。|停止|親エンドポイントの状態が、無効ではなく、停止になっています。無効な状態は、親プロファイルでエンドポイントを無効にしたことを示すために予約されています。|
|機能低下。少なくとも 1 つ 子エンドポイントが機能低下状態です。|子プロファイルのオンライン エンドポイントの数が、少なくとも minChildEndpoints の値である場合は、オンライン状態。子プロファイルのオンライン状態とエンドポイント チェック中状態のエンドポイントの数の合計が、少なくとも minChildEndpoints の値である場合は、エンドポイント チェック中状態。|トラフィックは、エンドポイント チェック中状態のエンドポイントにルーティングされます。minChildEndpoints を大きすぎる値に設定した場合、親エンドポイントは常に機能低下状態になります。|
|オンライン。少なくとも 1 つの子がオンラインの状態で、いずれも機能低下状態ではありません。|同上||
|エンドポイント チェック中。少なくとも 1 つが'エンドポイント チェック中'です。いずれも'オンライン'または'機能低下'状態ではありません。|同上||
|非アクティブ。すべてのエンドポイントが無効または停止のいずれかであるか、プロファイルにエンドポイントがありません。|停止||
...
[ブログの記事 - Traffic Manager の機能低下状態のトラブルシューティング](http://go.microsoft.com/fwlink/p/?LinkId=328689)
