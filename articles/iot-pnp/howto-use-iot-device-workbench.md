---
title: Azure IoT Device Workbench を使用して IoT プラグ アンド プレイ プレビュー デバイスを構築する |Microsoft Docs
description: Visual Studio Code の Azure IoT Device Workbench 拡張機能を使用して、IoT プラグ アンド プレイ デバイス モデルの作成とデバイス コードの実装を高速化します。
author: liydu
ms.author: liydu
ms.date: 07/25/2019
ms.topic: conceptual
...
ms.locfileid: "69879211"
---
# <a name="use-azure-iot-device-workbench-extension-in-visual-studio-code"></a>Visual Studio Code の Azure IoT Device Workbench 拡張機能を使用する

Visual Studio Code の Azure IoT Device Workbench 拡張機能は、デバイス機能モデル (DCM) とインターフェイスを作成し、モデル リポジトリに発行し、デバイス アプリケーションを実装するスケルトン C コードを生成するための統合環境を提供します。
...
Device Workbench 拡張機能を使用して IoT デバイスを開発する方法の詳細については、[https://github.com/microsoft/vscode-iot-workbench](https://github.com/microsoft/vscode-iot-workbench) を参照してください。

## <a name="prerequisites"></a>前提条件

[Visual Studio Code](https://code.visualstudio.com/) をインストールします。

VS Code に拡張機能をインストールするには、次の手順に従います。

1. VS Code で **[拡張機能]** タブを選択します。
1. Marketplace から **[Azure IoT Device Workbench]** を検索してインストールします。
...
1. SDK のコンパイル時にデバイス コード スタブ フォルダーが取り込まれるように、`CMakeLists.txt` ファイルの末尾に次の行を追加します。
...
1. **[チーム エクスプローラー]** で **azure-iot-sdk-c** リポジトリを開き、 **[Branches]\(ブランチ\)** を選択し、**public-preview** ブランチを検索してチェックアウトします。
    ...
    ![パブリック プレビュー](media/howto-use-iot-device-workbench/vs-public-preview.png)
...
1. **[General]\(全般\)** ツールバーで、 **[Configurations]\(構成\)** ドロップダウンを見つけます。 **[Manage Configuration]\(構成の管理\)** を選択して、プロジェクトの CMake 設定を追加します。
    ...
    ![構成を管理する](media/howto-use-iot-device-workbench/vs-manage-config.png)
...
Azure IoT Device Workbench 拡張機能は、GitHub のオープンソース プロジェクトです。 問題や機能に関する要望については、[GitHub で問題を作成](https://github.com/microsoft/vscode-iot-workbench/issues)できます。

## <a name="next-steps"></a>次の手順

このハウツー記事では、Azure IoT Device Workbench を使用して、DCM ファイルおよびインターフェイス ファイルを作成する方法について説明しました。 また、デバイス アプリケーションを実装するスケルトン C コードを生成する方法についても学習しました。 次は、[Azure IoT エクスプローラー ツールをインストールして使用する](./howto-install-iot-explorer.md)方法について学習することをお勧めします。
