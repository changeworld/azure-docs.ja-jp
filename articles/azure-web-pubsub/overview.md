...
Azure Web PubSub サービスは、リアルタイムでのコンテンツ更新を必要とするあらゆる種類のアプリケーションのために、さまざまな業界で使用されています。 Azure Web PubSub サービスを使用するのに適した例をいくつか示します。
...
Azure Web PubSub サービスは、Web ブラウザー、モバイル ブラウザー、デスクトップ アプリ、モバイル アプリ、サーバー プロセス、IoT デバイス、ゲーム コンソールなど、多様なクライアントに対応しています。 このサービスは、パブリッシュ-サブスクライブ パターンによる生の WebSocket をサポートしているため、このサービスと共に、あらゆる標準 WebSocket クライアント SDK をさまざまな言語で容易に使用することができます。 
...
* サーバーが、特定の接続やすべての接続にメッセージを送信する、または特定のユーザーに属するか、任意のグループに割り当てられた接続のサブセットにメッセージを送信する。 
* クライアントが、特定の接続やすべての接続にメッセージを送信する、または任意のグループに属する接続のサブセットにメッセージを送信する。
