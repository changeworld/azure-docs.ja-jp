...
[Azure Resource Manager を使用したデバイスへの接続](azure-stack-edge-j-series-connect-resource-manager.md)に関する記事の手順のようにします。 Azure Resource Manager 経由でローカル デバイス API にサインインしたら、デバイス上のストレージ アカウントの一覧を取得します。 次のコマンドレットを実行します。
...
2. SSL 証明書をインポートするには、 **[編集] > [SSL 証明書] > [証明書のインポート]** に移動します。
   ...
   ![証明書を Storage Explorer にインポートする](./media/azure-stack-edge-j-series-deploy-add-storage-accounts/import-cert-storage-explorer-1.png) 
