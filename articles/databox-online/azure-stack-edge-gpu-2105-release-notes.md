---
title: Azure Stack Edge 2105 リリース ノート
description: 2105 リリースを実行している Azure Stack Edge の重大な未解決の問題と解決策について説明します。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
下記のリリース ノートでは、お使いの Azure Stack Edge デバイスの 2105 リリースの、重大な未解決の問題と解決された問題について説明します。 このリリース ノートは、Azure Stack Edge Pro GPU、Azure Stack Edge Pro R、Azure Stack Edge Mini R デバイスを対象としています。 機能および問題が特定のモデルのものである場合は、常にそれを明記しています。

リリース ノートは継続的に更新されます。対応策を必要とする重大な問題が見つかった場合は、それらの問題が追加されます。 ご自分のデバイスを配置する前に、リリース ノートに含まれている情報を入念にご確認ください。

この記事は、ソフトウェア バージョン番号 **2.2.1606.3320** にマップされる **Azure Stack Edge 2105** リリースに適用されます。 このソフトウェアは、少なくとも Azure Stack Edge 2010 (2.1.1377.2170) ソフトウェアを実行している場合に、お使いのデバイスに適用できます。

## <a name="whats-new"></a>新機能

Azure Stack Edge 2105 リリースの新機能は以下のとおりです。 

- **仮想ローカル エリア ネットワーク (VLAN) の構成サポート** - このリリースでは、デバイスの PowerShell インターフェイスに接続することによって、仮想ローカル エリア ネットワーク (VLAN) の構成を変更できます。 詳細については、[仮想スイッチ上での vLAN ネットワークの作成](azure-stack-edge-gpu-create-virtual-switch-powershell.md)に関する記事を参照してください。
- **IP 転送のサポート** - このリリース以降、Virtual Machines (VM) に接続されているネットワーク インターフェイスで IP 転送がサポートされるようになりました。 
    - IP 転送を使用すると、VM のネットワーク インターフェイスに割り当てられている IP 構成のいずれにも割り当てられていない IP からのネットワーク トラフィックを VM が受信できるようになります。 
    - また、IP 転送により、VM のネットワーク インターフェイスの IP 構成に割り当てられている IP アドレスとは異なる送信元 IP アドレスを使用して、VM がネットワーク トラフィックを送信できるようになります。
        
    詳細については、「[IP 転送の有効化と無効化](../virtual-network/virtual-network-network-interface.md#enable-or-disable-ip-forwarding)」を参照してください。 
- **Kubernetes の機能強化** -　このリリースでは、Kubernetes に関連するいくつかの機能強化が行われています。
    - 以下の Kubernetes バージョンの更新プログラムを利用できます。 

        - Kubernetes サーバーのバージョン: v1.20.2
        - IoT Edge のバージョン: 0.1.0-beta14
        - Azure Arc 対応 Kubernetes バージョン: 1.1
    - Azure Arc 対応 Kubernetes では、さまざまなクラウドがサポートされるようになり、ログ記録が向上し、PowerShell インターフェイスを使用したコマンドレットの操作が変更されました。 
    - 診断およびテレメトリの修正が行われました。
    - コンピューティング ログのプロアクティブなログ収集が強化されました。 

- **Az コマンドレットのサポート** - このリリース以降、デバイスのローカル Azure Resource Manager に接続するとき、または VM ワークロードをデプロイするときに、Az コマンドレットを (プレビューで) 使用できるようになりました。 詳細については、[Az コマンドレット](/powershell/azure/new-azureps-module-az?view=azps-5.9.0&preserve-view=true)に関するページを参照してください。
- **HTTP 経由でのリモート PowerShell セッションの有効化** - このリリース以降、ローカル UI を使用して、デバイスへのリモート PowerShell セッションを *http* 経由で有効にできるようになりました。 詳細については、[http 経由でのリモート PowerShell の有効化](azure-stack-edge-gpu-manage-access-power-connectivity-mode.md#enable-device-access-via-remote-powershell-over-http)に関するページを参照してください。


## <a name="issues-fixed-in-2105-release"></a>2105 リリースで修正された問題

次の表に、以前のリリースでリリース ノートに記載され、現在のリリースでは修正された問題を示します。

| いいえ。 | 特徴量 | 問題 | 
| --- | --- | --- |
|**1.**|VM |DHCP リースの更新で障害が発生しても、ネットワーク インターフェイスのレコードが削除されなくなりました。|
|**2.**|VM | VM をプロビジョニングするときのロックに関する問題を解決するための監視機能が向上しました。| 

## <a name="known-issues-in-2105-release"></a>2105 リリースの既知の問題

次の表に、2105 リリースの既知の問題の概要を示します。
