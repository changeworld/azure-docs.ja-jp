...
1. 次の値を使用して、新しい **CNAME** レコードを作成します
   ...
   | ------------------- | ------------------------------------------- |
   | 型                | TXT                                         |
   | Host                | ルート ドメインの場合は `@`、またはサブドメインを入力 |
   | 値               | クリップボードからコードを貼り付ける          |
   | TTL (該当する場合) | 既定値のままにする                      |

> [!NOTE]
> 一部の DNS プロバイダーでは、"@" がルート ドメイン (つまり、mydomain.com) に変更されます。 これは仕様で、検証プロセスは変わらず機能します。
