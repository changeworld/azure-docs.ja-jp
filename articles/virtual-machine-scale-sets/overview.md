...
description: Azure Virtual Machine Scale Sets についてと、アプリケーションの自動スケーリングを行う方法について説明します
...
- **高可用性とアプリケーションの回復性を実現する**
    ...
    - 可用性を高めるために、[可用性ゾーン](../availability-zones/az-overview.md)を使用できます。単一のデータセンター内または複数のデータセンターで、スケール セットの VM インスタンスを自動的に分散します。

- **リソースの需要の変化に応じた、アプリケーションの自動スケーリングを可能にする**
    - アプリケーションに対する顧客の需要は、一日または一週間を通じて変化する可能性があります。 スケール セットでは、顧客の需要に対応するために、アプリケーションの需要の増加に応じて VM インスタンスの数を自動的に増やしたり、需要の減少に応じて VM インスタンスの数を減らしたりできます。
    - また、自動スケーリングを行うと、需要が少ないときに、アプリケーションを実行する不要な VM インスタンスの数を最小化できます。その一方で、需要の増加に応じて VM インスタンスが自動的に追加されるため、顧客は許容可能なレベルのパフォーマンスを利用し続けることができます。 この機能は、必要に応じてコストを削減し、Azure リソースを効率的に作成するのに役立ちます。
...
スケール セットは、仮想マシンで構成されています。 スケール セットには、アプリケーションの実行とスケーリングを行うための管理および自動化レイヤーがあります。 代わりに、個別の VM を手動で作成して管理したり、既存のツールを統合して同様のレベルの自動化を構築したりすることもできます。 次の表では、複数の VM インスタンスを手動で管理する場合と比較した、スケール セットの利点をまとめています。

| シナリオ                           | 手動の VM グループ                                                                    | 仮想マシン スケール セット |
|------------------------------------|----------------------------------------------------------------------------------------|---------------------------|
| 追加の VM インスタンスの追加        | 作成、構成、コンプライアンスの遵守が手動プロセス                             | 一元化された構成から自動で作成 |
