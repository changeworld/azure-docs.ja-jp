...
ms.locfileid: "107285549"
---
# <a name="azure-security-baseline-for-virtual-machine-scale-sets"></a>Virtual Machine Scale Sets 用の Azure セキュリティ ベースライン

このセキュリティ ベースラインにより、[Azure セキュリティ ベンチマーク バージョン 1.0](../security/benchmarks/overview-v1.md) のガイダンスが Virtual Machine Scale Sets に適用されます。 Azure セキュリティ ベンチマークでは、Azure 上のクラウド ソリューションをセキュリティで保護する方法に関する推奨事項を記載しています。その内容は、Azure セキュリティ ベンチマークで定義された **セキュリティ コントロール** と、Virtual Machine Scale Sets に適用できる関連ガイダンスにグループ分けしています。

> [!NOTE]
> Virtual Machine Scale Sets に適用されない、または Microsoft が責任を持つ **コントロール** は除外されています。 Virtual Machine Scale Sets を Azure セキュリティ ベンチマークへ完全にマップする方法については、 **[完全な Virtual Machine Scale Sets セキュリティ ベースライン マッピング ファイル](https://github.com/MicrosoftDocs/SecurityBenchmarks/raw/master/Azure%20Offer%20Security%20Baselines/1.1/virtual-machine-scale-sets-security-baseline-v1.1.xlsx)** を参照してください。
...
- [Azure 仮想マシン スケール セットのネットワーク](virtual-machine-scale-sets-networking.md)
...
**ガイダンス**: Virtual Machine Scale Set を使って Web アプリケーションをホストする場合は、使用する Virtual Machine Scale Set のサブネットでネットワーク セキュリティ グループ (NSG) を使用して、通信できるネットワーク トラフィック、ポート、およびプロトコルを制限します。 アプリケーションへの必要なトラフィックのみを許可するように NSG を構成する場合は、最小特権のネットワーク アプローチに従います。
...
**ガイダンス**: Virtual Machine Scale Sets を使用して Web アプリケーションをホストする場合、信頼できる証明書に対して HTTPS/SSL を有効にした Web アプリケーションの Azure Application Gateway をデプロイできます。 Azure Application Gateway では、リスナーをポートに割り当て、ルールを作成し、仮想マシン スケール セットなどのバックエンド プールにリソースを追加することで、アプリケーションの Web トラフィックを特定のリソースに転送します。
...
- [仮想マシン スケール セット テンプレートの詳細情報](virtual-machine-scale-sets-mvss-start.md) 
...
- [Log Analytics ワークスペースについて](/azure/azure-monitor/logs/get-started-portal)

- [Azure Monitor でカスタム クエリを実行する方法](../azure-monitor/logs/log-analytics-tutorial.md)
...
**ガイダンス**: Azure Security Center では、Azure Virtual Machines (VM) のセキュリティ イベント ログの監視機能を提供します。 自動プロビジョニングが有効になっている場合、Security Center によって、サポートされているすべての Azure VM と新しく作成されるものに Microsoft Monitoring Agent がプロビジョニングされます。またはエージェントを手動でインストールできます。  エージェントにより、プロセス作成イベント 4688 とイベント 4688 内の CommandLine フィールドが有効になります。 VM に作成された新しいプロセスは EventLog に記録され、Security Center の検出サービスによって監視されます。
...
**ガイダンス**: ユーザーのアクセスを管理する場合、Azure Active Directory (Azure AD) が推奨される方法ですが、Azure Virtual Machines にもローカル アカウントが存在する場合があります。 ローカル アカウントとドメイン アカウントの両方を、通常、最小の占有領域で、確認して管理する必要があります。 さらに、仮想マシンのリソースにアクセスするために使用される管理者アカウントには Azure Privileged Identity Management を利用します。
...
**ガイダンス**: Azure Virtual Machine Scale Sets や Azure Active Directory (Azure AD) には、デフォルトのパスワードという概念がありません。 既定のパスワードを使用する可能性のあるサード パーティ アプリケーションやマーケットプレース サービスに対する責任は、お客様が負います。
...
**ガイダンス**: IP アドレス範囲または国やリージョンにおける特定の論理グループからのアクセスのみを許可するには、Azure Active Directory (Azure AD) の条件付きアクセス ポリシーとネームド ロケーションを使用します。
...
**ガイダンス**: 監査ログとサインイン情報のログを Log Analytics ワークスペースに送信するには、Azure Active Directory の診断設定を構成します。 また、ログを確認したり、Azure 仮想マシンのログ データにクエリを実行したりするには Azure Monitor を使用します。
...
- [Azure で仮想マシンを監視する方法](/azure/azure-monitor/vm/monitor-vm-azur)
...
**ガイダンス**: Azure ロールベースのアクセス制御 (Azure RBAC) を使用すると、チーム内の職務を分離し、仮想マシン (VM) 上のユーザーがジョブを実行する上で必要な量のアクセスのみを許可できます。 すべてのユーザーに VM への無制限のアクセス許可を付与するのではなく、特定の操作のみを許可することができます。 Azure CLI または Azure PowerShell を使用して、Azure portal で VM のアクセス制御を構成できます。
...
**ガイダンス**: Virtual Machines (VM) 上の仮想ディスクは、サーバー側の暗号化または Azure Disk Encryption (ADE) のいずれかを使用して、保存時に暗号化されます。 Azure Disk Encryption では、Linux の DM-Crypt 機能を利用して、ゲスト VM 内のカスタマー マネージド キーでマネージド ディスクを暗号化します。 カスタマー マネージド キーを使用したサーバー側の暗号化では、ストレージ サービス内のデータを暗号化することで、VM に対して任意の OS の種類とイメージを使用できるため、ADE がさらに向上します。
...
- [Azure での仮想マシン スケール セットの OS の自動アップグレード](virtual-machine-scale-sets-automatic-upgrade.md)
...
**ガイダンス**: Azure Virtual Machine Scale Sets では、OS イメージの自動アップグレードを使用できます。 Virtual Machine Scale Sets の基になる仮想マシンには、Azure Desired State Configuration (DSC) 拡張機能を使用できます。 DSC は、オンラインで必要なソフトウェアを実行できるように VM を構成するときに使用されます。

- [Virtual Machine Scale Sets と Azure DSC 拡張機能の使用](virtual-machine-scale-sets-dsc.md)
...
**ガイダンス**: スクリプトの型に応じて、オペレーティング システム固有の構成またはサード パーティのリソースを使用して、Azure Compute のリソース内でユーザーがスクリプトを実行する機能を制限します。
...
- [Azure でディスクを復元し、回復した VM を作成する](../backup/tutorial-restore-disk.md)
...
- [Azure Virtual Machine Scale Sets のディスク暗号化を有効にする方法](disk-encryption-overview.md)
...
- [Azure Key Vault の論理的な削除と消去保護の概要](../key-vault/general/soft-delete-overview.md)
