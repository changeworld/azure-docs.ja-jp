...
**注:** 前の例の各 `<version>` 要素内で、この例の "n.n.n" プレースホルダーを有効なバージョン番号に置き換えてください。有効なバージョン番号は、[Maven の Azure ライブラリ リポジトリ][]から入手できます。
...
1. サイトの場所を「<http://dl.msopentech.com/eclipse>」と入力し、**Enter** キーを押します。
