...
4. Active Directory アプリケーションのアプリケーション ID を渡して、アプリケーションのサービス プリンシパルを作成します。
        ...
        New-AzureRmADServicePrincipal -ApplicationId $app.ApplicationId
5. サブスクリプションに対する権限をサービス プリンシパルに付与します。 この例では、サブスクリプション内のすべてのリソースを読み取る権限である **閲覧者** ロールを、サービス プリンシパルに付与します。 その他のロールについては、「[RBAC: 組み込みのロール](../active-directory/role-based-access-built-in-roles.md)」を参照してください。 **ServicePrincipalName** パラメーターには、アプリケーションの作成時に使用した **ApplicationId** を指定します。 
