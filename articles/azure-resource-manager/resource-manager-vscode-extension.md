...
10. **storageName** 変数を選択します。 コードは次のようになります。
...
11. `reference` はオブジェクトを返しますが、出力値は *string* に設定されているため、上記のコードは機能しません。 そのオブジェクトに値の 1 つを指定する必要があります。 reference 関数は任意のリソースの種類と共に使用できるので、VS Code はオブジェクトのプロパティを提示しません。 代わりに、[ストレージ アカウントに対して返される](/rest/api/storagerp/storageaccounts/getproperties)値の 1 つが `.primaryEndpoints.blob` であることがわかります。 

   最後のかっこの後に、そのプロパティを追加します。 右角かっこを追加します。 この outputs セクションの例を次に示します。
