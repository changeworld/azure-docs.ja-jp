...
1. storageAccountKey のパラメーター定義を次のパラメーター定義に置き換えます。 
    ...
    ![Resource Manager チュートリアルの成果物をセキュリティで保護するためのパラメーター](./media/template-tutorial-secure-artifacts/resource-manager-tutorial-secure-artifacts-parameters.png)

1. SQL 拡張リソースの 3 つの要素 (下記) の値を更新します。
...
デプロイ手順については、「[テンプレートのデプロイ](./template-tutorial-create-multiple-instances.md#deploy-the-template)」セクションを参照してください。 代わりに次の PowerShell デプロイ スクリプトを使用します。
