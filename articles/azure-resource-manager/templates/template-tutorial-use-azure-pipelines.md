...
1. **steps** セクションを次の YAML に置き換えます。
    ...
    * **deploymentScope**: デプロイのスコープを、`Management Group`、`Subscription`、および `Resource Group` の中から選択します。 このチュートリアルでは、 **[リソース グループ]** を使用します。 スコープの詳細については、「[デプロイのスコープ](deploy-rest.md#deployment-scope)」を参照してください。
