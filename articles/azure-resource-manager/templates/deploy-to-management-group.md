...
* [resourceId()](template-functions-resource.md#resourceid) 関数は、サポートされています。 管理グループ レベルのデプロイで使用されているリソースのリソース ID を取得するには、これを使用します。 たとえば、`resourceId('Microsoft.Authorization/roleDefinitions/', parameters('roleDefinition'))` と指定して、ポリシー定義のリソース ID を取得します。 リソース ID は `/providers/{resourceProviderNamespace}/{resourceType}/{resourceName}` の形式で返されます。
