...
1. **[OpenAPI 仕様から作成する]** 画面で、以下の表の値を使用して独自の API を作成します。
   ...
   |**ゲートウェイ**|**マネージド**|API を公開する API ゲートウェイ。 このフィールドは、**Developer** レベルおよび **Premium** レベルのサービスでのみ使用できます。<br/>**マネージド** ゲートウェイは、API Management サービスに組み込まれ、Microsoft によって Azure でホストされるゲートウェイを示します。 その他のゲートウェイは[セルフホステッド ゲートウェイ](self-hosted-gateway-overview.md)であり、Premium および Developer サービス レベルでのみ使用できます。 これらはオンプレミスまたは他のクラウドにデプロイできます。<br/>ゲートウェイが選択されていない場合、API は使用できず、API 要求は成功しません。|
