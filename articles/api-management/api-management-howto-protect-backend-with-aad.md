...
1. スコープが作成されたら、後の手順で使用するために、そのスコープを書き留めておきます。 
...
1. `client-app` を選択し、アプリのページの一覧で **[API のアクセス許可]** に移動します。

1. **[アクセス許可の追加]** を選択します。

1. **[API の選択]** で `backend-app` を見つけて選択します。

1. **[委任されたアクセス許可]** で、`backend-app` に最適なアクセス許可を選択し、 **[アクセス許可の追加]** をクリックします。

1. 必要に応じて、 **[API のアクセス許可]** ページでページの下部にある **[に管理者の同意を与えます <your-tenant-name>]** をクリックし、このディレクトリのすべてのユーザーに代わって同意を付与します。 
