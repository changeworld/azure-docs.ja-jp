<properties 
	pageTitle="Azure API Management ポリシーのプロパティの利用方法" 
...
| 属性 | 型 | 説明 |
|-----------|-----------------|---------------------------------------------------------------------------------------------------------|
| 名前 | string | プロパティの名前。文字、数字、ピリオド、ダッシュ、下線のみを使用できます。 |
| 値 | string | プロパティの値。空にすることはできません。スペースのみで構成することはできません。 |
...
プロパティは前の例のように完全な属性または要素値として使用できますが、例 `<set-header name = "CustomHeader{{ContosoHeader}}" ...>` のように、リテラル テキスト式に挿入することもできます。
...
>[AZURE.IMPORTANT] プロパティがポリシーに参照される場合、プロパティを使用するすべてのポリシーからプロパティを削除するまで削除は完了しません。
...
![検索][api-management-properties-search]
