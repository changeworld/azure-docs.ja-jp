...
FTP コネクタには、TriggerOnFileAvailable というトリガーが 1 つあります。このトリガーは、新しいファイルが FTP サーバーの特定のフォルダーにアップロードされるたびに発生します。
...
ロジック アプリを作成すると、この FTP トリガーにより、次の操作が実行されます。 
...
- **[存在する場合に追加]** -  'Append If Exist' を有効または無効にします。有効に設定すると、ファイルが存在する場合にファイルにデータが追加されます。無効に設定すると、ファイルが存在する場合にファイルは上書きされます。
- **[一時フォルダー]** - オプション。指定した場合、アダプターは、ファイルを  'Temporary Folder Path' にアップロードします。アップロードが完了すると、ファイルは  'Folder Path' に移動されます。移動操作がアトミックであるためには、 'Temporary Folder Path' が  'Folder Path' と同じ物理ディスクにある必要があります。一時フォルダーは、 'Append If Exist' プロパティが無効の場合にのみ使用できます。
