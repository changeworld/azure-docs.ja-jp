...
Webhook には発行された Runbook が必要です。 このチュートリアルでは、「[Azure Automation Runbook を作成する](automation-quickstart-create-runbook.md)」で作成した Runbook の変更バージョンを使用します。 先に進むには、次のコードを使用して PowerShell Runbook を編集します。
