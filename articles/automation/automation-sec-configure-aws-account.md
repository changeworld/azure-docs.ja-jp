...
AWS セキュリティ キーを作成し、コピーしたら、Azure Automation アカウントで資格情報資産を作成し、それらを安全に格納し、Runbook で参照する必要があります。記事「[Credential assets in Azure Automation (Azure Automation の資格情報資産)](../automation/automation-certificates.md/###To create a new credential with the Azure portal)」のセクション「**To create a new credential (新しい資格情報を作成するには) **」に記載の手順に従い、次の情報を入力します。
...
- AWS のタスクを自動化する Runbook を作成する方法については、記事「[Automating deployment of a VM in Amazon Web Services (アマゾン ウェブ サービスでの VM のデプロイを自動化する)](../automation/automation-scenario-aws-deployment.md)」を参照してください。
