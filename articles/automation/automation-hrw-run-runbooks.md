...
署名済み Runbook のみを使用するように Hybrid Runbook Worker が構成されました。 Hybrid Runbook Worker で使用される Runbook に署名する必要があります。 次のサンプルの PowerShell を使用して Runbook に署名します。
