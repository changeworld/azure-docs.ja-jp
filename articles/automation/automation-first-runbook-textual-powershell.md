...
1. **[発行]** をクリックして Runbook を発行し、確認を求めるメッセージが表示されたら **[はい]** をクリックします。
...
8. [出力] ウィンドウを閉じます。
9. **[すべてのログ]** をクリックして、Runbook ジョブのストリーム ウィンドウを開きます。出力ストリームでは "*Hello World*" のみが表示されますが、Runbook によって詳細やエラーに書き込みがある場合は、Runbook ジョブの他のストリームも表示できます。
...
11. **[ジョブ]** をクリックして、この Runbook のジョブ ウィンドウを開きます。この Runbook によって作成されたジョブの一覧が表示されます。ジョブを実行したのは 1 回のみのため、一覧に表示されるジョブは 1 つだけです。
...
1. *Add-AzureAccount* の後に、「*Start-AzureVM -Name 'VMName' -ServiceName 'VMServiceName'*」と入力します。ここでは、開始する仮想マシンの名前とサービス名を指定します。
