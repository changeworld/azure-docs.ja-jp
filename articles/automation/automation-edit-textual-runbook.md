...
次のサンプル コマンドでは、Runbook の既存の内容を、スクリプト ファイルの内容に置き換える方法を示します。これは「[Windows PowerShell でスクリプト ファイルから Runbook をインポートするには](../automation-creating-or-importing-a-runbook.md#ImportRunbookScriptPS)」と同じサンプル プロシージャであることに注意してください。
