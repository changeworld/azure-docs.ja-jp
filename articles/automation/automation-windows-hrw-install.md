...
ワークスペースに **Automation** ソリューションを追加するには、次の PowerShell を実行します。
...
エージェントが Log Analytics ワークスペースに正常に接続したら、数分後に次のクエリを実行して、ワークスペースにハートビート データが送信されていることを確認できます。
