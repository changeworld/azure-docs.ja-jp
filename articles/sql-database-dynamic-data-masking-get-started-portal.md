...
> [AZURE.SELECTOR]
- [動的データ マスク - Azure プレビュー ポータル](sql-database-dynamic-data-masking-get-started.md)
...
| **カスタム テキスト** | **最初と最後の文字を表示**し、その間にカスタム埋め込み文字列を追加するマスク手法。<br/>prefix[padding]suffix<br/><br/>![Navigation pane][Image2] |
...
1. Azure ポータル ([https://manage.windowsazure.com](https://manage.windowsazure.com)) を表示します。
...
4. マスクされていないデリケートなデータへのアクセスを許可する特権ログインを入力します。
	...
	>[AZURE.TIP] そのためには、アプリケーション層で、そのアプリケーションの特権を付与されたユーザーに機密データを表示したり、データベースのクエリに使用されるアプリケーション ログイン情報を追加したりすることができます。デリケートなデータの公開を最小限に抑えるには、この一覧に含めるログインの数を最小限にすることを強くお勧めします。
