---
title: Azure Virtual Desktop FSLogix プロファイル コンテナー共有 - Azure
description: 仮想マシンベースのファイル共有を使用して Azure Virtual Desktop ホスト プールの FSLogix プロファイル コンテナーを設定する方法。
author: Heidilohr
ms.topic: how-to
ms.date: 08/20/2019
ms.author: helohr
...
ms.locfileid: "111756097"
---
# <a name="create-a-profile-container-for-a-host-pool-using-a-file-share"></a>ファイル共有を使用してホスト プール用のプロファイル コンテナーを作成する

Azure Virtual Desktop サービスでは、推奨するユーザー プロファイル ソリューションとして FSLogix プロファイル コンテナーを提供します。 ユーザー プロファイル ディスク (UPD) ソリューションの使用はお勧めしていません。これは Azure Virtual Desktop の将来のバージョンでは非推奨になります。
...
1. Azure Virtual Desktop の Active Directory ユーザーを [Active Directory セキュリティ グループ](/windows/security/identity-protection/access-control/active-directory-security-groups/)に追加します。 このセキュリティ グループは、先ほど作成したファイル共有仮想マシンに対して、Azure Virtual Desktop ユーザーを認証するために使用されます。
...
6. Azure Virtual Desktop ユーザーを追加したセキュリティ グループを検索し、そのグループに **フル コントロール** があることを確認します。
