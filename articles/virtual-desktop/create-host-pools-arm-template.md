...
これらの手順を完了した後、デスクトップ アプリケーション グループに追加されたユーザーは、サポートされているリモート デスクトップ クライアントを使用して Windows Virtual Desktop にサインインして、セッション デスクトップ用のリソースを参照できます。
