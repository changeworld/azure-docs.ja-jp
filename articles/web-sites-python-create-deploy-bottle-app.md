...
Bottle Web フレームワークを使用して (このチュートリアルの [Django](web-sites-python-create-deploy-django-app.md) と [Flask](web-sites-python-create-deploy-flask-app バージョンを参照.md))、アプリケーションを作成できます。Azure ギャラリーから Web サイトを作成し、Git デプロイを設定し、リポジトリをローカルで複製します。その後、ローカルでアプリケーションを実行し、変更を行い、それらをコミットして Azure に送信します。チュートリアルでは、Windows または Mac/Linux での手順を紹介します。
...
ウィンドウが開き、ギャラリーに使用可能なアプリケーションが一覧表示されます。左側の **[APP FRAMEWORKS (アプリケーション フレームワーク)]** カテゴリをクリックし、**[Bottle]** を選択します。
