...
1. ラボの **[ユーザー]** ページに切り替えます。 
2. ツール バーの **[全員を招待]** を選択します。 
3. ユーザーへの**メッセージ**を入力します。 これは省略可能な手順です。
4. 次に、 **[送信]** を選択します。

    ![すべてのユーザーを招待する](../media/how-to-configure-student-usage/invite-all.png)

    この操作の状態は、 **[ユーザー]** 一覧の **[招待]** 列に表示されます。 招待メールには、ユーザーがラボに登録するために使用できる登録リンクが含まれます。 
...
ユーザーに対して個別のクォータを設定できます。 これを行うには、次のステップに従います。
