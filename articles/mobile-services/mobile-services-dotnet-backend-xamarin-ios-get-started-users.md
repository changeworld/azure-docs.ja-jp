---
title: Xamarin iOS アプリケーション用 Mobile Services の認証の使用 | Microsoft Docs
...
2. 次の定義を使用して、**QSTodoService** に新しいメソッド **Authenticate** を追加します。
   ...
   > [!NOTE]
   > Facebook 以外の ID プロバイダーを使用する場合は、上の例の **LoginAsync** に渡される値を *MicrosoftAccount*、*Twitter*、*Google*、*WindowsAzureActiveDirectory* のいずれかに変更します。
   > 
   > 
