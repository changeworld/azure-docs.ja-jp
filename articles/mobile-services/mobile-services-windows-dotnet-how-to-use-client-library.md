...
Mobile Services クライアントでは、Azure Notification Hubs によるプッシュ通知に登録することができます。登録すると、Windows プラットフォームから取得したチャネル URI が得られます。この値は、登録を作成するときに、任意のタグと一緒に指定します。次のコードは、Windows Notification Service (WNS) を使用する Windows ストア アプリケーションのプッシュ通知に登録します。
...
この例では、登録で 2 つのタグが含められます。詳細については、「[アプリケーションにプッシュ通知を追加する](mobile-services-dotnet-backend-windows-universal-dotnet-get-started-push.md)」を参照してください。
...
##<a name="binding"></a>方法: モバイル サービスでユーザー インターフェイスにデータをバインドする

このセクションでは、返されたデータ オブジェクトを UI 要素を使用して表示する方法について説明します。`todoTable` で未完了の項目を照会し、ごくシンプルな一覧で表示するには、次のコード例を実行して、クエリで一覧のソースをバインドします。`MobileServiceCollection` を使用すると、モバイル サービス対応のバインディング コレクションが作成されます。
...
###サーバー フロー
Mobile Services によって Windows ストア アプリまたは Windows Phone アプリの認証プロセスが管理されるようにするには、アプリケーションを ID プロバイダーに登録する必要があります。その後、モバイル サービス内で、プロバイダーから提供されたアプリケーション ID とシークレットを構成する必要があります。詳細については、チュートリアル「[アプリへの認証の追加]」を参照してください。
...
Microsoft アカウントを使用してシングル サインイン エクスペリエンスを提供する方法の例については、シングル サインインによるアプリケーションの認証に関するチュートリアル ([Windows ストア](/develop/mobile/tutorials/single-sign-on-windows-8-dotnet/)/[Windows Phone](/develop/mobile/tutorials/single-sign-on-wp8/)) を参照してください。
