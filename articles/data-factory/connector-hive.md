---
title: "Azure Data Factory を使用して Hive からデータをコピーする | Microsoft Docs"
...
| プロパティ | 説明 | 必須 |
|:--- |:--- |:--- |
| type | type プロパティを **Hive** に設定する必要があります | あり |
| host | Hive サーバーの IP アドレスまたはホスト名。複数のホストは ';' で区切ります (serviceDiscoveryMode が有効な場合のみ)。  | あり |
| ポート | Hive サーバーがクライアント接続のリッスンに使用する TCP ポート。  | いいえ |
| serverType | Hive サーバーの種類。 <br/>使用可能な値: **HiveServer1**、**HiveServer2**、**HiveThriftServer** | いいえ |
| thriftTransportProtocol | Thrift レイヤーで使用するトランスポート プロトコル。 <br/>使用可能な値: **Binary**、**SASL**、**HTTP ** | いいえ |
| authenticationType | Hive サーバーへのアクセスに使用する認証方法。 <br/>使用可能な値: **Anonymous**、**Username**、**UsernameAndPassword**、**WindowsAzureHDInsightService** | あり |
...
Hive からデータをコピーするには、データセットの type プロパティを **HiveObject** に設定します。 この種類のデータセットに追加の種類固有のプロパティはありません。
...
| プロパティ | 説明 | 必須 |
|:--- |:--- |:--- |
| type | コピー アクティビティのソースの type プロパティを **HiveSource** に設定する必要があります | あり |
