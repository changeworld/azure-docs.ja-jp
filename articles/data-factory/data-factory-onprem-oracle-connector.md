...
3. このエントリを、v4.0 フォルダーの machine.config ファイル (<システム ディスク>:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\Config\\machine.config) にコピーし、バージョンを 4.xxx.x.x に変更します。
4. `gacutil /i [provider path]` を実行して、"<ODP.NET インストール パス>\\11.2.0\\client\_1\\odp.net\\bin\\4\\Oracle.DataAccess.dll" をグローバル アセンブリ キャッシュ (GAC) にインストールします。
