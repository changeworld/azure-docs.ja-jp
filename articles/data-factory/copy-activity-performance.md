...
| データ サイズ/帯域幅 | 50 Mbps    | 100 Mbps  | 200 Mbps  | 500 Mbps  | 1 Gbps   | 10 Gbps  |
| --------------------- | ---------- | --------- | --------- | --------- | -------- | -------- |
| 1 GB                  | 2.7 分    | 1.4 分   | 0.7 分   | 0.3 分   | 0.1 分  | 0.0 分  |
| 10 GB                 | 27.3 分   | 13.7 分  | 6.8 分   | 2.7 分   | 1.3 分  | 0.1 分  |
| 100 GB                | 4.6 時間    | 2.3 時間   | 1.1 時間   | 0.5 時間   | 0.2 時間  | 0.0 時間  |
| 1 TB (テラバイト)                  | 46.6 時間   | 23.3 時間  | 11.7 時間  | 4.7 時間   | 2.3 時間  | 0.2 時間  |
| 10 TB                 | 19.4 日  | 9.7 日  | 4.9 日  | 1.9 日  | 0.9 日 | 0.1 日 |
| 100 TB                | 194.2 日 | 97.1 日 | 48.5 日 | 19.4 日 | 9.5 日 | 0.9 日 |
| 1 PB                  | 64.7 か月    | 32.4 か月   | 16.2 か月   | 6.5 か月    | 3.2 か月   | 0.3 か月   |
| 10 PB                 | 647.3 か月   | 323.6 か月  | 161.8 か月  | 64.7 か月   | 31.6 か月  | 3.2 か月   |
...
2. **1 回のコピー アクティビティのパフォーマンスを最大化する方法**:
   ...
   **コピー アクティビティが Azure Integration Runtime で実行される場合:**
   ...
   - 実行している特定のコピー パターンでは、DIU を追加してもメリットはありません。  より大きな DIU 値を指定した場合でも、実際に使用される DIU は同じままであるため、前と同じスループットが得られます。  この場合は、手順 3 に進んでください。
   ...
   **コピー アクティビティがセルフホステッド統合ランタイムで実行される場合:**
   ...
   - 一方、セルフホステッド IR ノードの CPU が高く、使用可能なメモリが少ない場合は、新しいノードを追加して、複数のノード間で負荷をスケールアウトすることができます。  手順については、[こちら](create-self-hosted-integration-runtime.md#high-availability-and-scalability)を参照してください。
...
4. **パフォーマンスを診断して最適化する。** 観測したパフォーマンスが予測どおりでない場合は、パフォーマンスのボトルネックを特定します。 次に、パフォーマンスを最適化して、ボトルネックの影響を除去するか軽減します。

   場合によっては、Azure Data Factory でコピー アクティビティを実行すると、次の例に示されているように、[コピー アクティビティの監視](copy-activity-overview.md#monitor-visually)の上部に "パフォーマンス チューニングに関するヒント" が表示されます。 このメッセージからは、特定のコピー実行で確認されたボトルネックが伝えられます。 コピー スループットを上げるために変更すべき点も指摘されます。 パフォーマンス チューニングのヒントからは現在のところ、次のような提案が表示されます。
...
コピー アクティビティの実行を支援するために許可される DIU は 2 から 256 の間です。 指定しない場合、次の表に、さまざまなコピー シナリオで使用される既定の DIU を示します。
