...
> [!NOTE]
> 次のセクションで説明する Parquet/テキスト形式の **AmazonS3Object** 型のデータセットは、下位互換性のために引き続きコピー/Lookup/GetMetadata アクティビティでそのままサポートされますが、マッピング データ フローでは機能しません。 今後は、この新しいモデルを使用することをお勧めします。ADF オーサリング UI はこれらの新しい型を生成するように切り替えられています。
