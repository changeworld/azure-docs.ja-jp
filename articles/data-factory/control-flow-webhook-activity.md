...
Azure Data Factory は、本文の追加のプロパティ "callbackuri" を url エンドポイントに渡し、この uri が指定されたタイムアウト値の前に呼び出されることを期待します。 この uri が呼び出されない場合、アクティビティは状態 'TimedOut' で失敗します。
