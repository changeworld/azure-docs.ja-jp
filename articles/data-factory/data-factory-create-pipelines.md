...
scheduler | アクティビティのスケジュールを定義します。サブプロパティは、[データセットの availability プロパティ](data-factory-create-datasets.md#Availability)にあるサブプロパティと同じです。| いいえ | 
...
|expirationTime |パイプラインの作成後に、パイプラインが有効であり、プロビジョニングされた状態である必要がある時間。パイプラインは、アクティブ、エラー、または保留中の実行がない限り、有効期限に達すると自動的に削除されます。 | いいえ | 
