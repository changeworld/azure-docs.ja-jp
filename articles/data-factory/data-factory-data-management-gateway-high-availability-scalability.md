...
4. **手動セットアップ**を選択した場合 :
    ...
    6. ノード (オンプレミス Windows コンピューター) の Data Management Gateway 構成マネージャーを確認します。接続状態、**ゲートウェイ名**、および**ノード名**が表示されます。  
        ...
        > [!NOTE]
        > Azure VM でゲートウェイをプロビジョニングしている場合は、[こちらの GitHub にある Azure Resource Manager テンプレート](https://github.com/xiaoyingLJ/vms-with-multiple-data-management-gateway)を使用できます。 このスクリプトは、論理ゲートウェイを作成し、Data Management Gateway ソフトウェアがインストールされている VM を設定し、論理ゲートウェイと共に VM を登録します。 
...
既存のゲートウェイをアップグレードして、高可用性とスケーラビリティの機能を使用することができます。 この機能は、バージョン 2.9.xxxx 以上のデータ管理ゲートウェイを備えたノードだけで動作します。 コンピューターにインストールされているデータ管理ゲートウェイのバージョンは、Data Management Gateway 構成マネージャーの **[ヘルプ]** タブで確認できます。 
