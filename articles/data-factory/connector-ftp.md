...
データを FTP からコピーする際、ADF は現在、まずファイル長の取得を試み、次にファイルを複数の部分に分割して、並列で読み取ります。 FTP サーバーが、ファイル長の取得をサポートしていない場合、または特定のオフセットから読み取ろうとする場合、エラーが発生することがあります。
