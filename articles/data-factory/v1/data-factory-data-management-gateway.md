...
たとえば、**Azure SQL Database シンクまたは Azure Synapse Analytics (旧称 SQL Data Warehouse) シンクにオンプレミスのデータ ストア**からコピーするには、次の手順を行います。
...
2. 次のコマンドを実行して Azure 資格情報を入力することにより、Azure アカウントにログインします。
    ...
    Connect-AzAccount
    ```
3. **New-AzDataFactoryGateway** コマンドレットを使用して、次のように論理ゲートウェイを作成します。
