...
4. "[コピー並列処理の次数](copy-activity-performance-features.md#parallel-copy)" では、パーティション数を制御します。この数値を大きくしすぎるとパフォーマンスが低下するため、この数値は、(DIU またはセルフホステッド IR ノードの数) × (2 ～ 4) に設定することをお勧めします。
