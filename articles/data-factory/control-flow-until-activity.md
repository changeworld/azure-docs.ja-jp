---
title: "Azure Data Factory の Until アクティビティ | Microsoft Docs"
...
プロパティ | 説明 | 使用できる値 | 必須
-------- | ----------- | -------------- | --------
name | `Until` アクティビティの名前。 | String | あり
type | **Until** に設定する必要があります。 | String | あり
expression | true または false に評価される式であることが必要です | 式。  | あり
timeout | ここに指定した時間が経過すると、do-until ループがタイムアウトします。 | 文字列 をオンにします。 `d.hh:mm:ss` または `hh:mm:ss`。 既定値は 7 日間です。 | いいえ
アクティビティ | 式が `true` に評価されるまで実行される一連のアクティビティです。 | アクティビティの配列。 |  あり
