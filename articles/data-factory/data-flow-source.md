...
**[Wildcard Pattern]\(ワイルドカード パターン\)** オプションを使用すると、ADF は、単一のソース変換で一致する各フォルダーおよびファイルをループ処理します。 これは、単一のフロー内の複数のファイルを処理する非常に効果的な方法です。 現在処理中のファイルの名前を追跡するには、[ソース オプション] の [Column to store file name]\(ファイル名を格納する列\) フィールドにフィールド名を設定します。

> [!NOTE]
> 既存のワイルドカード パターンの横にある + 記号を使用して、複数のワイルドカード一致パターンを設定し、ワイルドカード規則を追加します。
...
* **パーティションのルート パス**:```key=value``` 形式 (たとえば、year=2019) のファイル ソースにパーティション分割されたフォルダーがある場合、ADF に対して、そのパーティション フォルダー ツリーの最上位をデータ フロー データ ストリーム内の列名に割り当てるように指示できます。
...
次に、パーティションのルート パス設定を使用して、フォルダー構造の最上位レベルを ADF に指示します。 データの内容を表示すると、ADF によって、各フォルダー レベルに解決済みのパーティションが追加されることがわかります。
...
設定パネルのフィールド内をクリックすると、[動的なコンテンツの追加] というハイパーリンクが表示されます。 式ビルダーを起動することを選択した場合は、式、静的なリテラル値、またはパラメーターを使用して値を動的に設定します。
