...
ルール ベースのマッピングを選択すると、ADF に対して、受信パターン ルールに一致するように一致式を評価し、送信フィールド名を定義するように指示します。 フィールドとルール ベースのマッピングの組み合わせを追加することもできます。 フィールド名は、ソースからの受信メタデータに基づいて、実行時に ADF によって生成されます。 生成されたフィールドの名前は、デバッグ中に表示することも、[データ プレビュー] ウィンドウを使用して表示することもできます。
