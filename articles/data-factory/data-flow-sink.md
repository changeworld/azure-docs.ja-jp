...
シンク変換の **[マッピング]** タブでは、左側の受信の列を右側の変換先にマップできます。 データ フローをファイルにシンクした場合、Data Factory は常に、新しいファイルをフォルダーに書き込みます。 データベース データセットにマップする場合、 **[ポリシーを保存]** を **[上書き]** に設定することで、このスキーマを使用する新しいテーブルを生成できます。 または、既存のテーブルに新しい行を挿入し、既存のスキーマにフィールドをマップします。 
