...
| format   | 種類が `Datetime` または `Datetimeoffset` のときに使用される書式文字列。 日時の書式を設定する方法については、「[カスタム日時書式指定文字列](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings)」を参照してください。 一般に、このプロパティを指定または変更する必要はありません。 詳細については、「[データ型のマッピング](#data-type-mapping)」を参照してください。 | いいえ       |
...
| dateTimeFormat                   | 日付 (タイム ゾーン オフセットなし) と文字列の間で変換する場合の書式指定文字列 (`yyyy-MM-dd HH:mm:ss.fff` など)。  詳細については、[カスタム日時書式指定文字列](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings)に関する記事を参照してください。 | いいえ       |
| dateTimeOffsetFormat             | 日付 (タイム ゾーン オフセットあり) と文字列の間で変換する場合の書式指定文字列 (`yyyy-MM-dd HH:mm:ss.fff zzz` など)。  詳細については、[カスタム日時書式指定文字列](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings)に関する記事を参照してください。 | いいえ       |
| timeSpanFormat                   | 時間間隔と文字列の間で変換する場合の書式指定文字列 (`dd\.hh\:mm` など)。 詳細については、[カスタム TimeSpan 書式指定文字列](https://docs.microsoft.com/dotnet/standard/base-types/custom-timespan-format-strings)に関する記事を参照してください。 | いいえ       |
