...
| プロパティ | 説明 | 必須 |
| --- | --- | --- |
| query | カスタム クエリを使用してデータを読み取ります。 |いいえ (**データセット**の **tableName** が指定されている場合) |
| redshiftUnloadSettings | Amazon Redshift の UNLOAD を使用する場合のプロパティ グループ。 | いいえ |
| s3LinkedServiceName | ADF にリンクされた AwsAccessKey 種類のサービス名を指定することで、中間ストアとして使用される Amazon S3 を参照します。 | "redshiftUnloadSettings" では必須 |
| bucketName | 中間データを格納する S3 バケットを指定します。 指定しない場合、コピー アクティビティはバケットを自動生成します。 | "redshiftUnloadSettings" では必須 |

また、**RelationalSource**種類 (Amazon Redshift を含む) を typeProperties セクションの以下のプロパティで使用することもできます。 このソースの種類では Redshift の UNLOAD がサポートされないことに注意してください。
...
このサンプルのユース ケースでは、コピー アクティビティは "redshiftUnloadSettings" での構成に従って Amazon Redshift から Amazon S3 へデータを迅速にアンロードした後、Amazon S3 から "stagingSettings" に指定された Azure Blob へデータをコピーし、最後に、PolyBase を使用して SQL Data Warehouse にデータを読み込みます。 すべての中間形式が、コピー アクティビティによって正しく処理されます。

![Redshift から SQL DW へのコピーのワークフロー](media\data-factory-amazon-redshift-connector\redshift-to-sql-dw-copy-workflow.png)
