...
このプレビュー段階のサービス マップと System Center Operations Manager (SCOM) との統合を利用すると、サービス マップの動的な依存関係を基にして SCOM で分散アプリケーション ダイアグラムを自動で作成することができます。
...
4.    OMS ワークスペースに関連付けられている Azure サブスクリプションにアクセスできるサービス プリンシパル。  サービス プリンシパルの詳細については、「付録」を参照してください。
...
3. このウィザードでの最初の手順は、サービス プリンシパル名を構成する [Connection Configuration] (接続の構成) です。 サービス プリンシパルのテナント ID、アプリケーション ID (またはユーザー名かクライアント ID)、パスワードを入力します。  サービス プリンシパルの作成方法については「付録」セクションを参照してください。
