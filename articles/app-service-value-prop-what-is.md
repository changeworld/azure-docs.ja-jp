...
Azure App Service は、Azure Websites、Azure Mobile Services、Azure Biztalk Services の既存の機能をすべて含み、さらに新しい機能を追加した単一のサービスです。[これらの既存のサービスと App Service の関連の詳細については、こちら](http://azure.microsoft.com/documentation/services/app-service/)を参照してください。
...
- [**Web アプリ**](app-service-web-overview.md) - ビジネスに合わせて拡張できるミッション クリティカルな Web アプリをすばやく作成してデプロイします。
- [**モバイル アプリ**](app-service-mobile-value-prop-preview.md) - あらゆるデバイスを使用する従業員、パートナー、顧客といつでもつながります。
- [**API アプリ**](app-service-api-apps-why-best-platform.md) - クラウド API を簡単に構築して使用し、API エコノミーに参加します。
- [**ロジック アプリ**](app-service-logic-what-are-logic-apps.md) - コードを記述することなく、クラウド全体のデータへのアクセスと使用を自動化します。
...
### App Service プラン
App Service プランは、アプリ間で共有できる一連の機能と容量を表します。App Service プランでは、いくつかの料金レベル (Free、Shared、Basic、Standard など) がサポートされています。料金レベルごとに利用できる機能は異なります。アプリがホストされる App Service プランを、ダウンタイムなしですばやく変更できます。[App Service プラン](web-sites-web-hosting-plan-overview.md)の詳細に関するページを参照してください。
...
無料ですぐに、[新しい Web アプリを作成](http://go.microsoft.com/fwlink/?LinkId=523751)してください。クレジット カードもコミットメントも不要です。面倒なことはありません。
