...
* **職場または学校のアカウントを使用して Azure にログイン** - Azure Active Directory を使用して、資格情報の認証をします。CLI バージョン 0.9.9 以降では、多要素認証に対して有効になっている組織アカウントがサポートされています。ログイン後は、リソース マネージャーを使用することも、従来の (サービス管理) コマンドを使用することもできます。

* **発行設定ファイルのダウンロードと使用** - この方法では、管理タスクを実行するための証明書がインストールされますが、サブスクリプションと証明書が有効であることが必要です。この方法では、従来の (サービス管理) コマンドしか使用できません。
...
ログインによる方法は、職場または学校のアカウント (*組織アカウント*とも呼ばれます) でのみ機能します。このアカウントは組織によって管理され、組織の Azure Active Directory で定義されます。組織アカウントがない場合は、[組織アカウントを作成する](#create-an-organizational-account)ことができます。


* 職場または学校のアカウントを使用して Azure CLI から**ログインするには**、次のコマンドを使用します。
	...
	初めてこれらの資格情報を使用してログインする場合は、認証トークンをキャッシュするかどうかの確認を求められます。このプロンプトは、`azure logout` コマンド (後で説明) を以前に使用したことがある場合にも表示されます。自動化のシナリオでこのプロンプトが表示されないようにするには、`azure login` に `-q` パラメーターを付けて実行します。
...
### 多要素認証
Azure CLI バージョン 0.9.9 以降では、多要素認証 (パスワードと、その他の 1 つまたは複数の検証方法 (信頼されたデバイスの使用、またはその他の個人データの指定など) による認証) を使用する組織アカウントによってログインすることができます。

アカウントのユーザー名とパスワードで `azure login` を実行すると、開く必要のある Web ページのアドレスが CLI に表示されます。そのページにコードを入力して認証を継続するように指示されます。認証ポリシーが満たされると、Azure CLI はログインを完了します。


### 組織アカウントを作成する

現在、職場または学校のアカウントがなく、個人用アカウントを使用して Azure サブスクリプションにログインしている場合は、次の手順を使用して簡単に組織アカウントを作成できます。

1. [Azure ポータル][portal]にサインインして、**[Active Directory]** を選択します。
...
5. 最後に、Azure ポータルからログアウトし、新しい職場または学校のアカウントを使用してログインし直します。初めてこのアカウントを使用してログインする場合は、パスワードを変更するように求められます。
	...
	新しいアカウントを使用してログインしたとき、自分のサブスクリプションが表示されることを確認してください。

職場または学校のアカウントの詳細については、「[Microsoft Azure への組織としてのサインアップ][signuporg]」を参照してください。
