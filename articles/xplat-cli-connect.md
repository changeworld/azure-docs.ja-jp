...
* **職場または学校のアカウント、あるいは Microsoft アカウント ID を使用して Azure にログイン** - この場合は、どちらかの種類のアカウント ID を使用して認証を行います。CLI バージョン 0.9.9 以降では、多要素認証が有効になっているアカウントに対して対話型認証がサポートされています。対話形式でログインした後は、リソース マネージャーを使用することも、従来の (サービス管理) コマンドを使用することもできます。
...
> [AZURE.NOTE]バージョン 0.9.9 より前の Azure CLI を使用している場合、`azure login` コマンドは職場または学校のアカウント ID でのみ使用でき、Microsoft アカウント ID は機能しません。ただし、Azure CLI バージョン 0.9.9 以降では、対話型の `azure login` コマンドを使用して、任意の ID でアカウントにログオンできます。
...
> [AZURE.NOTE]どちらの場合にも、認証と承認は Azure Active Directory を使用して (Microsoft アカウントの場合は、Azure Active Directory の既定のドメインにアクセスして) 実行されます(無料試用版にサインアップした場合は、Azure Active Directory によってアカウントに既定のドメインが作成されたことに気づかないことがあります)。
