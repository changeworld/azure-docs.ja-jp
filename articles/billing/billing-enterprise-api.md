...
---
# <a name="overview-of-reporting-apis-for-enterprise-customers-preview"></a>企業ユーザー向けの Reporting API の概要 (プレビュー)
...
* **API キーの生成と取得** - エンタープライズ ポータルにログインして、「ヘルプ - Reporting APIs」のチュートリアルに従います。 このヘルプ記事の最初のセクションで、指定した加入契約に対応する API キーの生成方法、取得方法を説明しています。
...
下記の API では、[こちら](https://consumption.azure.com/v1/swagger)の Swagger エンドポイントを使用できます。これによって、API のイントロスペクションが容易になり、[AutoRest](https://github.com/Azure/AutoRest) または [Swagger CodeGen](http://swagger.io/swagger-codegen/) を使用したクライアント SDK の生成機能が有効になります。 2014 年 5 月 1 日以降のデータは、この API で使用できます。 
 ...
 **請求期間の一覧表示** - [Billing Periods API](billing-enterprise-api-billing-periods.md) を使用すると、指定された加入契約の消費量データが含まれる請求期間の一覧が逆時系列順に返されます。 各期間には、BalanceSummary、UsageDetails、Marktplace Charges および PriceSheet の 4 セットのデータの API ルートを示すプロパティが含まれています。
