...
* [部分的な自動化 (msrsync) の例](#use-the-msrsync-utility-to-populate-cloud-volumes) 
...
``msrsync`` では、ローカル ボリュームへの書き込み、およびローカル ボリュームからの書き出しのみが可能であることに注意してください。 ソースとコピー先は、クラスターの仮想ネットワーク内のローカル マウントとしてアクセス可能である必要があります。
