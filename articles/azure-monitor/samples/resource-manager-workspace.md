...
## <a name="collect-windows-events"></a>Windows イベントの収集
次のサンプルでは、[Windows イベント](../platform/data-sources-windows-events.md)のコレクションを既存のワークスペースに追加します。

### <a name="notes"></a>Notes

- 収集するイベント ログごとに **datasources** 要素を追加します。 ログごとに異なるイベントの種類のセットを指定できます。
...
## <a name="collect-syslog"></a>syslog の収集
次のサンプルでは、[syslog イベント](../platform/data-sources-syslog.md)のコレクションを既存のワークスペースに追加します。

### <a name="notes"></a>Notes

- 収集するファシリティごとに **datasources** 要素を追加します。 ファシリティごとに異なる重大度のセットを指定できます。
...
## <a name="collect-windows-performance-counters"></a>Windows パフォーマンス カウンターの収集
次のサンプルでは、[Windows パフォーマンス カウンター](../platform/data-sources-performance-counters.md)のコレクションを既存のワークスペースに追加します。

### <a name="notes"></a>Notes

- 収集するカウンターとインスタンスごとに **datasources** 要素を追加します。 カウンターとインスタンスの組み合わせごとに異なる収集速度を指定できます。
...
## <a name="collect-linux-performance-counters"></a>Linux パフォーマンス カウンターの収集
次のサンプルでは、[Linux パフォーマンス カウンター](../platform/data-sources-performance-counters.md)のコレクションを既存のワークスペースに追加します。

### <a name="notes"></a>Notes

- 収集するオブジェクトとインスタンスごとに **datasources** 要素を追加します。 オブジェクトとインスタンスの組み合わせごとに異なるカウンターのセットを指定できますが、すべてのカウンターに対して 1 つの速度しか指定できません。
...
## <a name="collect-custom-logs"></a>カスタム ログの収集
次のサンプルでは、[カスタム ログ](../platform/data-sources-custom-logs.md)のコレクションを既存のワークスペースに追加します。

### <a name="notes"></a>Notes

- 区切り記号と抽出の構成は複雑になる場合があります。 参考用に、Azure portal を使用してカスタム ログを定義した後、 **-Kind** を **CustomLog** に設定した [Get-AzOperationalInsightsDataSource](/powershell/module/az.operationalinsights/get-azoperationalinsightsdatasource) を使用してその構成を取得できます。
...
## <a name="collect-iis-log"></a>IIS ログの収集
次のサンプルでは、[IIS ログ](../platform/data-sources-iis-logs.md)のコレクションを既存のワークスペースに追加します。
...
* [エージェント データ ソースの詳細情報](../platform/agent-data-sources.md)。
