...
| |Unknown |いくつかの理由により計算できない場合、正常性状態は "*不明*" になります。 追加の詳細情報とそれらを解決するための考えられるソリューションについては、次の脚注 <sup>1</sup> を参照してください。 |

<sup>1</sup> "不明" の正常性状態は、次の問題に起因します。

- エージェントが再構成されて、Azure Monitor for VM が有効化されたときに指定されたワークスペースに、もう報告していない。 ワークスペースに報告するようにエージェントを構成するには、「[ワークスペースの追加または削除](../platform/agent-manage.md#adding-or-removing-a-workspace)」を参照してください。
- VM が削除されている。
- Azure Monitor for VMs に関連付けられたワークスペースが削除されている。 ワークスペースを復旧するには、Premier サポートの特典がある場合は、[Premier](https://premier.microsoft.com/) でサポート要求をオープンすることができます。
- ソリューションの依存関係が削除されている。 お使いの Log Analytics ワークスペースで ServiceMap と InfrastructureInsights のソリューションを再度有効にするには、提供されている [Azure Resource Manager テンプレート](vminsights-enable-at-scale-powershell.md#install-the-servicemap-and-infrastructureinsights-solutions)を使用するか、[作業開始] タブにある [ワークスペースの構成] オプションを使用して再インストールすることができます。
- VM がシャット ダウンされている。
- Azure VM サービスを使用できない、またはメンテナントが実行されている。
- ワークスペースの[毎日のデータまたは保有の制限](../platform/manage-cost-storage.md)に達している。
...
>[!NOTE]
>このアクションは、通知を受信する監視対象の各 VM に対して実行する必要があります。これは、リソース グループ内のすべての VM には適用されません。  
