...
Azure Monitor for VMs では、監視する各仮想マシンまたは仮想マシン スケール セットに、次の 2 つのエージェントをインストールする必要があります。 リソースをオンボードするには、これらのエージェントをインストールし、ワークスペースに接続します。  
