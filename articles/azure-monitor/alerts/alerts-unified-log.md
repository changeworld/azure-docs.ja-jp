...
> [!NOTE]
> 現在、1 分頻度のログ アラートに追加料金は発生しません。 プレビュー段階にある機能の価格は、後で発表され、課金が始まる前に通知されます。 通知期間後も 1 分頻度のログ アラートを引き続き使用することを選択した場合は、該当する料金が適用されます。
...
1 分頻度のアラートを使用するには、API を使用してプロパティを設定する必要があります。 API バージョン `2020-05-01-preview` でログ アラート ルールを新規作成するか、既存のものを更新する場合、`properties` セクションで、`String` 型の値 `PT1M` を使用して `evaluationFrequency` を追加します。 API バージョン `2018-04-16` でログ アラート ルールを新規作成するか、既存のものを更新する場合、`schedule` セクションで、`Int` 型の値 `1` を使用して `frequencyInMinutes` を追加します。 
