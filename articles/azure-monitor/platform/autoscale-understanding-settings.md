...
| [プロファイル] | rules | 自動スケールでは、プロファイルのルールを使用して、最大容量から最小容量までの間で自動的にスケールします。 1 つのプロファイルで複数のルールをご利用いただけます。 通常は、スケールアウトするタイミングを指定するルールと、スケールインするタイミングを指定するルールの 2 つのルールを使用します。 |
