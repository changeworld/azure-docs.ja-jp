...
* [タイルのリファレンス](view-designer-tiles.md):ご利用のカスタム ビューで使用できる各タイルの設定に関するリファレンスです。
...
| **一覧** |**> 列タイトル** |
| 名前 |最初の列の上部に表示されるテキスト。 |
| Value |2 番目の列の上部に表示されるテキスト。 |
| **一覧** |**> しきい値** |
...
| Operation |1 つの値として集計するために、値プロパティに対して実行する演算。<ul><li>合計:すべてのレコードの値を加算します。</li><li>パーセンテージ: **[中央の演算に使用される結果値]** の値によって返されるレコードの、クエリのすべてのレコードに対する割合。</li></ul> |
...
| Operation |コールアウトとして表示する 1 つの値として集計するために値プロパティに対して実行する演算。<ul><li>平均:すべてのレコードの値の平均。</li><li>カウント:クエリによって返されるすべてのレコードの数。</li><li>最後のサンプル:グラフに含まれる最後の間隔の値。</li><li>最大:グラフに含まれる間隔の最大値。</li><li>最小:グラフに含まれる間隔の最小値。</li><li>合計:すべてのレコードの値の合計。</li></ul> |
| **折れ線グラフ** |**> Y 軸** |
...
| Operation |スパークラインを有効にした場合、これは、スパークラインの値を計算するために、リストの各プロパティに対して実行する演算です。<ul><li>最後のサンプル:一定期間における系列の最後の値。</li><li>最大:一定期間における系列の最大値。</li><li>最小:一定期間における系列の最小値。</li><li>合計:一定期間における系列の値の合計。</li><li>概要: ヘッダーのクエリと同じ `measure` コマンドを使用します。</li></ul> |
