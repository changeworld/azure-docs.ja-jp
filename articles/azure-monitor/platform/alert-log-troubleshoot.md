...
そのような場合、ルールがかなりの期間 (1 週間など) 継続して実行できないとき、Azure Monitor はログ アラートを無効にし、お客様が不必要に課金されないようにします。 Azure Monitor がログ アラートを無効にした正確な時間は、[Azure アクティビティ ログ](../../azure-resource-manager/resource-group-audit.md)で調べることができます。 Azure Monitor がログ アラート ルールを無効にすると、Azure アクティビティ ログにイベントが追加されます。
...
Azure Monitor がログ アラート ルールを無効にした正確な時間は、[Azure アクティビティ ログ](../../azure-resource-manager/resource-group-audit.md)でイベントを検索すると見つけることができます。
