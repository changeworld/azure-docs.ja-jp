...
**アクション** - アラートが発生したときに特定のアクションが実行されます。 詳細については、[アクション グループ](./action-groups.md)に関する記事をご覧ください。

## <a name="what-you-can-alert-on"></a>アラートできるもの

[データ ソースの監視](./data-sources.md)に関するページで説明されているように、メトリックとログについてアラートできます。 シグナルには次が含まれていますが、これらに限定されません。
...
[Alert Management REST API](/rest/api/monitor/alertsmanagement/alerts) を使用すると、[アラート コンテキスト](alerts-common-schema-definitions.md#alert-context) フィールドなど、特定のアラートに関する詳細情報を取得できます。
