...
[リソース間](../log-query/cross-workspace-query.md)クエリを実行する[メトリック測定](./alerts-unified-log.md#calculation-of-measure-based-on-a-numeric-column-such-as-cpu-counter-value)に基づく [Scheduled Query Rules 作成](/rest/api/monitor/scheduledqueryrules/createorupdate)テンプレート (サンプル データは変数として設定):
