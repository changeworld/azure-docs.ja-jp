...
Azure Monitor は、システム割り当てのマネージド ID を使用して Azure Key Vault にアクセス権を付与します。 Log Analytics クラスターの ID はクラスター レベルでサポートされており、複数のワークスペースでカスタマー マネージド キーを使用できます。新しい Log Analytics *Cluster* リソースは、キー コンテナーと Log Analytics ワークスペース間の中間 ID 接続として機能します。 Log Analytics クラスター ストレージは、"*クラスター*" リソースに関連付けられているマネージド ID を使用して、Azure Active Directory 経由で Azure Key Vault に対する認証を行います。 
...
- プリンシパルを選択する: クラスター名またはプリンシパル ID を入力します。
