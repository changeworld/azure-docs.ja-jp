...
ログ検索アラート ルールは、使用されている構成およびカスタム分析クエリに従って、ユーザーが指定したロジックで動作します。 アラート ルールがトリガーされる正確な条件や理由を含む監視ロジックは、分析クエリにカプセル化されているため、ログ アラート ルールごとに異なる場合があります。 ログ検索アラート ルールのしきい値の条件に達したり超えたりした場合でも、Azure アラートには、特定の根本原因または評価中のシナリオに関する情報が多くありません。 したがって、ログアラートはステートレスと呼ばれます。 また、指定されたカスタム分析クエリの結果がアラート条件を満たす限り、ログ アラート ルールはトリガーされ続けます。 監視失敗の正確な根本原因のロジックは、ユーザーが提供する分析クエリ内でマスクされるため、アラートが毎回解決されるとは限りません。 現時点で、解決中の根本原因を Azure Monitor のアラートが最終的に推論するメカニズムはありません。

実際の例で同じものを見てみましょう。 [結果の数タイプのログ アラートの例](#example-of-number-of-records-type-log-alert)で指定された構成に従って、*Contoso-Log-Alert* というログ アラート ルールがあるとします。そこでは、500 の結果コードをログから探すようにカスタム アラート クエリが設計されています。

- 午後 1 時 05分に、Contoso-Log-Alert が Azure アラートによって実行されたとき、ログ検索の結果で見つかった、結果コードが 500 のレコードは 0 個でした。 0 はしきい値を下回っているため、アラートは発生しません。
- 午後 1 時 10 分の次のイテレーションで、Contoso-Log-Alert が Azure アラートによって実行されたとき、ログ検索の結果は、結果コードが 500 である 5 つのレコードを返しました。 5 はしきい値を超えているので、アラートが発生し、関連付けられたアクションがトリガーされます。
- 午後 1 時 15 分に Contoso-Log-Alert が Azure アラートによって実行されたとき、ログ検索の結果は、結果コードが 500 である 2 つのレコードを返しました。 2 はしきい値を超えているので、アラートが発生し、関連付けられたアクションがトリガーされます。
- 午後 1 時 20 分の次のイテレーションで、Contoso-Log-Alert が Azure アラートによって実行されたとき、ログ検索の結果は再び、結果コードが 500 である 0 個のレコードを返しました。 0 はしきい値を下回っているため、アラートは発生しません。

しかし、上記の午後 1 時 15 分のケースでは、Azure アラートは、午後 1 時 10 分に確認された根本的な問題が解決していないという判断と、新たな障害が発生したかどうかの判断ができません。 ユーザーによって指定されたクエリは以前のレコードを考慮に入れている可能性があるため、Azure アラートは確証を持てます。 アラートのロジックはアラート クエリにカプセル化されているため、午後 1 時 15 分に確認された結果コード 500 の 2 つのレコードが、午後 1 時 10 分に既に確認されているかどうかは分かりません。 このため慎重を期して、Contoso-Log-Alert が午後 1 時 15 分に実行されるときに、構成済みのアクションが再びトリガーされます。 さて、午後 1 時 20 分に結果コード 500 のレコードが 0 個確認された時点で、Azure アラートは、午後 1 時 10 分と午後 1 時 15 分に確認された結果コード 500 の原因が今はもう解決されていることを確信できず、Azure Monitor アラートは、500 エラーの問題が同じ理由で再び発生することはないと確実に推論できます。 そのため、Contoso-Log-Alert は Azure アラート ダッシュボードで解決済みに変更されず、アラートの解決を示す通知も送信されません。 代わりに、分析クエリに埋め込まれたロジックの正確な条件または理由を理解しているユーザーは、必要に応じて[アラートを解決済みとしてマーク](alerts-managing-alert-states.md)できます。
