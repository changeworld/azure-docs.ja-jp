...
1. アラートのペイロードが、セキュア Webhook アクションによって ITSM ツールに送信されます。
1. アラートが ITSM ツールに送られることが承認されているかどうかについて、ITSM アプリケーションが Azure AD で確認します。
1. アラートが承認されている場合、アプリケーションは次のように動作します。
   
   1. ITSM ツールで作業項目 (インシデントなど) を作成します。
   1. 構成項目 (CI) の ID を顧客管理データベース (CMDB) にバインドします。
...
## <a name="connection-with-bmc-helix"></a>BMC Helix との接続

セキュア エクスポートは BMC Helix をサポートしています。 この統合には次のような利点があります。
...
* **[共通アラート スキーマ](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-common-schema)** :ITSMC では、アラートのペイロードのスキーマは、アラートの種類によって異なります。 セキュア エクスポートでは、すべてのアラートの種類に共通のスキーマがあります。 この共通スキーマには、すべてのアラートの種類の CI が含まれています。 すべてのアラートの種類で、その CI を CMDB にバインドすることができます。
...
1. 「[Microsoft ID プラットフォームにアプリケーションを登録する](https://docs.microsoft.com/azure/active-directory/develop/quickstart-register-app)」の手順に従います。
1. Azure AD で、 **[アプリケーションの公開]** を選択します。
1. **[アプリケーション ID の URI]** に **[設定]** を選択します。
...
アクション グループの詳細については、「[Azure Portal でのアクション グループの作成および管理](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups)」を参照してください。

BMC Helix 環境で次の手順を使用します。
...
1. **[アラート]**  >  **[アクションの管理]** を選択します。
1. [[アクション グループの追加]](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups#create-an-action-group-by-using-the-azure-portal) を選択し、フィールドに入力します。
1. **[アクション グループ名]** ボックスおよび **[短い名前]** ボックスに名前を入力します。 短い名前は、通知がこのグループを使用して送信されるときに長い名前の代わりに使用されます。
1. **[セキュア Webhook]** を選択します。
1. 次の詳細を選択します。
   1. 登録した Azure Active Directory インスタンスのオブジェクト ID を選択します。
   1. URI に、BMC Helix 環境からコピーした Webhook URL を貼り付けます。
   1. **[共通アラート スキーマを有効にする]** に **[はい]** を設定します。 
...
## <a name="configure-the-partner-environment"></a>パートナー環境を構成する
