...
> [!NOTE]
> 季節性のパターン検出は、時、日、週のいずれかの間隔に設定されます。 つまり、その他のパターン (2 時間ごと、週 2 回など) は検出されない場合があります。
...
- 該当する時点で使用されていた動的しきい値を含んだアラートがトリガーされた期間のグラフ。
...
メトリックのアラート ルールが作成される前に、チャートに表示されるしきい値は、時間単位または日単位の季節性パターン (10日間) を計算するのに十分な履歴データに基づき計算されます。 アラート ルールが作成されると、動的しきい値は、利用可能な履歴データをすべて使用し、しきい値の精度を向上するために新しいデータを基に学習し、熟練し続けます。 つまり、この計算の後、グラフには、毎週のパターンも表示されるようになります。
