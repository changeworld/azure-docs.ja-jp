---
title: Azure 自動スケーリングのログ イベントのスキーマ
description: 自動スケーリング アクションを監視およびトラブルシューティングするためのログの形式
author: rboucher
...
ms.locfileid: "74152340"
---
# <a name="azure-monitor-autoscale-actions-resource-log-schema"></a>自動スケーリング アクションの Azure Monitor リソース ログ スキーマ

次に、サンプル データが含まれている、自動スケーリングのリソース ログの一般的な形式を示します。 次のすべての例が、適切な形式の JSON であるわけではありません。これは、特定のフィールドに対して有効な複数の値が含まれている可能性があるためです。 

この種類のイベントを使用して、自動スケーリングで発生している問題のトラブルシューティングを行います。 詳細については、[自動スケーリングの問題のトラブルシューティング](autoscale-troubleshoot.md)に関する記事を参照してください。


## <a name="profile-evaluation"></a>プロファイルの評価

自動スケーリングが最初に自動スケーリングプロファイルを参照するときに記録されます
...
## <a name="profile-cooldown-evaluation"></a>プロファイルのクールダウンの評価

クールダウン期間であるためスケールを実行するかどうかを評価した際に記録されます。 
...
## <a name="rule-evaluation"></a>ルールの評価

自動スケーリングが最初に特定のスケール ルールの評価を開始したときに記録されます。 
...
## <a name="metric-evaluation"></a>メトリックの評価

自動スケーリングによってスケール アクションをトリガーするために使用されているメトリックが評価されたときに記録されます。 
...
## <a name="instance-count-evaluation"></a>インスタンス数の評価

自動スケーリングによって既に準備のため実行されているインスタンスの数を評価して、開始、終了、または何もしないかを決定する際に記録されます。 
...
## <a name="scale-action-evaluation"></a>スケール アクションの評価

自動スケーリングがスケール アクションを実行する必要があるかどうかの評価を開始するときに記録されます。 
...
## <a name="instance-update-evaluation"></a>インスタンスの更新の評価

自動スケーリングによって実行されるコンピューティング インスタンスの数 (上または下) が更新されるときに記録されます。
...
## <a name="scale-action"></a>スケール アクション

自動スケーリングでスケール アクションが開始されるときに記録されます (上または下)。 
```json
{
  "time": "2018-09-10 18:12:00.6132593",
  "resourceId": "/SUBSCRIPTIONS/BA13C41D-C957-4774-8A37-092D62ACFC85/RESOURCEGROUPS/AUTOSCALETRACKING12042017/PROVIDERS/MICROSOFT.INSIGHTS/AUTOSCALESETTINGS/DEFAULTSETTING",
...
## <a name="scale-action-tracking"></a>スケール アクションの追跡

インスタンスのスケール アクションの異なる間隔で記録されます。
...
[自動スケーリング](autoscale-overview.md)について学ぶ