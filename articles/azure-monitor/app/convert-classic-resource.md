...
ワークスペースベースのリソースにより、リソース全体で共通のロールベースのアクセス制御 (RBAC) を使用できるようになり、アプリ間またはワークスペース間のクエリが不要になります。
...
クラシック リソース データは永続的に保持され、取り込まれたときのデータ保持設定が適用されます。 移行後に取り込まれるすべての新しいデータには、関連付けられている Log Analytics ワークスペースのデータ保持設定が適用されます。 
...
> [!NOTE]
> ワークスペース ベースの Application Insights リソースのデータ インジェストとリテンション期間は、データが保管されている Log Analytics ワークスペースを通じて課金されます。 ワークスペース ベースの Application Insights リソースの課金に関する[詳細を参照]( ./pricing.md#workspace-based-application-insights)してください。 (移行前に取り込まれたクラシック Application Insights リソース データには、データが保持されている期間、Application Insights のデータ保持および価格が適用され続けます。) 
...
このコマンドについての完全な Azure CLI ドキュメントについては、[Azure CLI のドキュメント](/cli/azure/ext/application-insights/monitor/app-insights/component?view=azure-cli-latest#ext-application-insights-az-monitor-app-insights-component-update)を参照してください。
