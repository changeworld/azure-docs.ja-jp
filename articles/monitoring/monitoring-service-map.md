...
![状態バッジ](media/monitoring-service-map/status-badges.png)
...
![マシン グループ](media/monitoring-service-map/machine-group.png)

### <a name="creating-a-machine-group"></a>マシン グループの作成
グループを作成するには、[マシン] の一覧で 1 つまたは複数のマシンを選択し、**[グループに追加]** をクリックします。

![グループの作成](media/monitoring-service-map/machine-groups-create.png)
...
![[グループ] タブ](media/monitoring-service-map/machine-groups-tab.png)

次に、グループ名を選択して、そのマシン グループのマップを表示します。
![マシン グループ](media/monitoring-service-map/machine-group.png) グループに属するマシンは、マップ内で白い枠線に囲まれています。

グループを展開すると、そのマシン グループを構成するマシンが一覧表示されます。

![マシン グループ内のマシン](media/monitoring-service-map/machine-groups-machines.png)
...
![グループのフィルター](media/monitoring-service-map/machine-groups-filter.png)

**[すべてのプロセス]** を選択すると、グループ内の各マシンのプロセスと接続がすべてマップに表示されます。

![マシン グループのすべてのプロセス](media/monitoring-service-map/machine-groups-all.png)

**グループ関連プロセス**のみが表示されるようビューを変更すると、マップの表示が、グループの他のマシンに直接関連するプロセスと接続のみに絞られ、簡略化されたビューになります。

![マシン グループのフィルター処理されたプロセス](media/monitoring-service-map/machine-groups-filtered.png)
...
![グループからのマシンの削除](media/monitoring-service-map/machine-groups-remove.png)

### <a name="removing-or-renaming-a-group"></a>グループの削除または名前の変更
グループの一覧にあるグループ名の横にある省略記号メニューをクリックします。

![マシン グループのメニュー](media/monitoring-service-map/machine-groups-menu.png)
...
| ![データベース サーバー](media/monitoring-service-map/role-database.png) | データベース サーバー |
| ![LDAP サーバー](media/monitoring-service-map/role-ldap.png) | LDAP サーバー |
| ![SMB サーバー](media/monitoring-service-map/role-smb.png) | SMB サーバー |

![ロール アイコン](media/monitoring-service-map/role-icons.png)
...
![失敗した接続](media/monitoring-service-map/failed-connections.png)
...
![クライアント グループ](media/monitoring-service-map/client-groups.png)

クライアント グループ内のサーバーの IP アドレスを表示するには、グループを選択します。 グループの内容が、**[Client Group Properties]\(クライアント グループのプロパティ\)** ウィンドウに一覧表示されます。

![クライアント グループのプロパティ](media/monitoring-service-map/client-group-properties.png)
...
![サーバー ポート グループ](media/monitoring-service-map/server-port-groups.png)

## <a name="context-menu"></a>コンテキスト メニュー
サーバーの右上にある省略記号 (...) をクリックすると、そのサーバーのコンテキスト メニューが表示されます。

![失敗した接続](media/monitoring-service-map/context-menu.png)
...
![[Machine Summary]\(マシンの概要\) ウィンドウ](media/monitoring-service-map/machine-summary.png)
...
![[Machine Properties]\(マシンのプロパティ\) ウィンドウ](media/monitoring-service-map/machine-properties.png)
...
![[Process Properties]\(プロセスのプロパティ\) ウィンドウ](media/monitoring-service-map/process-properties.png)
...
![[Machine Log Events]\(マシン ログ イベント\) ウィンドウ](media/monitoring-service-map/log-events.png)
...
![[Machine Service Desk]\(マシン サービス デスク\) ウィンドウ](media/monitoring-service-map/service-desk.png)
...
![[Machine Change Tracking]\(マシン変更の追跡\) ウィンドウ](media/monitoring-service-map/change-tracking.png)
...
![[Machine Performance]\(マシンのパフォーマンス\) ウィンドウ](media/monitoring-service-map/machine-performance.png)
...
[サービス マップの構成に関するドキュメントの「トラブルシューティング」]( monitoring-service-map-configure.md#troubleshooting)をご覧ください。
