...
Azure Monitor for VMs を搭載する前にワークスペースで Windows または Linux ベースの VM で[有効にしていたパフォーマンス カウンターを収集](monitoring-vminsights-onboard.md?toc=/azure/azure-monitor/toc.json#performance-counters-enabled)していなかった場合は、Windows および Linux に対してこれらのルールを[ここ](../log-analytics/log-analytics-data-sources-performance-counters.md?toc=/azure/azure-monitor/toc.json#configuring-performance-counters)で説明する手順に従って無効にする必要があります。
