<properties pageTitle="Azure 上の Linux 仮想マシンでの root 権限の使用" description="Azure 上の Linux 仮想マシンで root 権限を使用する方法について説明します。" services="virtual-machines" documentationCenter="" authors="szarkos" manager="timlt" editor=""/>
...
2. **SSH キーのみ** - 仮想マシンが証明書 (`.cer` または  `.pem` ファイル) または SSH キーを使用してプロビジョニングされ、パスワードがない場合。この場合は、コマンドを実行する前に  `sudo` からユーザーのパスワードの入力は**求められません**。
