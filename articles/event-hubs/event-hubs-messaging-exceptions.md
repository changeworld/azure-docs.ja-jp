...
- `https://sbwagn2.servicebus.windows.net/` の参照または [wget](https://www.gnu.org/software/wget/) を行います。 これは、IP フィルタリング、仮想ネットワーク、または証明書チェーンの問題 (java SDK の使用時に最も一般的) があるかどうかを確認するのに役立ちます。
...
- 前の手順で解決できない場合はネットワーク トレースを取得して分析するか、[Microsoft サポート](https://support.microsoft.com/)に問い合わせてください。 
