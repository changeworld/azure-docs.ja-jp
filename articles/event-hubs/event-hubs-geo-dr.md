...
1. プライマリ名前空間を作成します。
1. プライマリ名前空間が存在するサブスクリプションとリソース グループにセカンダリ名前空間を作成します。 この手順は省略可能です。 セカンダリ名前空間は、次の手順でペアリングを作成しているときに作成できます。 
...
1. **[ペアリングの開始]** ページで、既存のセカンダリ名前空間を選択するか、またはプライマリ名前空間が存在するサブスクリプションとリソース グループにセカンダリ名前空間を作成します。 そのうえで **[Create]\(作成\)** を選択します。 次の例では、既存のセカンダリ名前空間が選択されています。 

    :::image type="content" source="./media/event-hubs-geo-dr/initiate-pairing-page.png" alt-text="セカンダリ名前空間を選択する":::        
1. ここで、プライマリ名前空間の **[geo リカバリー]** を選択すると、次の図のような **[geo DR エイリアス]** ページが表示されます。
...
1. **[geo DR エイリアス]** ページで、 **[共有アクセス ポリシー]** を選択して、エイリアスのプライマリ接続文字列にアクセスします。 この接続文字列は、プライマリまたはセカンダリ名前空間への接続文字列を直接使用する代わりに使用します。 
