...
"*ロールベースのアクセス制御 (RBAC)* " を付与するには、作成されたマネージ ID またはサービス プリンシパルを、1 つまたは複数の Azure Maps アクセス制御ロールの定義に割り当てます。 Azure Maps に使用できる RBAC ロールの定義を表示するには、 **[アクセス制御 (IAM)]** に移動します。 **[ロール]** を選択してから、「*Azure Maps*」で始まるロールを検索します。 これらの Azure Maps ロールが、アクセス権を付与できるロールです。
