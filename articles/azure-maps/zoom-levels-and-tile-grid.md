...
Azure Maps では、球面メルカトル図法の座標系 (EPSG: 3857) が使用されます。 図法とは、球体を平面地図に変換するために使用される数学的モデルです。 球面メルカトル図法では、正方形の地図を作成するために、地図が両極で引き伸ばされます。 これにより、地図のスケールと面積は大きく歪曲されますが、この歪みを上回る 2 つの重要な性質があります。

- 正角図法であり、比較的小さなオブジェクトの形状が保持されることを意味します。 これは、建物の形状の変形を避ける必要があるため、航空映像を表示する場合に特に重要です。 正方形の建物は、長方形ではなく正方形に表示される必要があります。
- 円筒図法であり、北と南は常に上下の方向、西と東は常に左右の方向になることを意味します。 
...
![世界地図のタイル](./media/zoom-levels-and-tile-grid/world0.png)</center>
...
各ズーム レベルで使用する図法とスケールを選択したら、地理座標をピクセル座標に変換できます。 特定のズーム レベルに対する世界地図画像の完全なピクセルの幅と高さは、次のように計算できます。
...
![ピクセル寸法を示す地図](media/zoom-levels-and-tile-grid/map-width-height.png)</center>

緯度と経度の値、および詳細レベルを指定すると、ピクセル XY 座標は次のように計算できます。
...
緯度と経度の値は、WGS 84 測地系と見なされます。 Azure Maps では球面投影が使用されますが、すべての地理的座標を共通の測地系に変換することが重要であり、その測地系として WGS 84 が選択されています。 経度の値は -180 から +180 度の範囲と想定され、緯度の値は -85.05112878 から 85.05112878 の範囲にクリップされる必要があります。 これにより、極での特異点が避けられ、投影された地図が正方形になります。

## <a name="tile-coordinates"></a>タイルの座標

地図の取得と表示のパフォーマンスを最適化するため、レンダリングされる地図はタイルに分割されています。 ピクセル数は各ズーム レベルで異なるため、タイルの数は次のようになります。
