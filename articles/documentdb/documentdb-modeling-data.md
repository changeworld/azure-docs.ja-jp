...
前に示した方法を使用して、ここでは、この個人に関連したすべての情報 (連絡先詳細や住所など) を 1 つの JSON ドキュメントに**埋め込んだ**個人レコードを**非正規化**しました。また、固定スキーマに限定されているわけではないので、まったく別の形で連絡先詳細を含めるような柔軟性もあります。
...
> [AZURE.TIP]DocumentDB 内のドキュメントには、最大サイズが設けられています。詳細については、「[DocumentDB の制限](documentdb-limits.md)」を参照してください。
...
> [AZURE.NOTE]正規化されたデータ モデルでは、サーバーへの**ラウンド トリップがより多く必要になる可能性があります**。
...
###多対多のリレーションシップのモデル化
リレーショナル データベースでは、多くの場合、*多対多*のリレーションシップが結合テーブルを使用してモデル化されます。結合テーブルにより、他のテーブルのレコードが結合されます。
...
この結合テーブルで行われる処理が 2 つのデータ要素の結合だけであれば、これを削除してはどうでしょうか。 次の例を検討してみてください。
