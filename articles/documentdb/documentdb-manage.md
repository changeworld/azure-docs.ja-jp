<properties 
	pageTitle="DocumentDB - 容量 - ドキュメント ストレージ | Microsoft Azure" 
...
アプリケーションのデータ ストレージやスループット スケールの要件に合うように、何個でもコレクションを作成できます。コレクションは [Azure ポータル](https://portal.azure.com/)またはいずれかの [DocumentDB SDK](documentdb-sdk-dotnet.md) を使用して作成できます。

>[AZURE.NOTE] 各コレクションは、最大 10 GB のドキュメント データ ストレージをサポートします。大規模なデータセットを格納するために、複数のコレクションに分割する必要があります。「[.NET SDK を使用して DocumentDB 内のデータをパーティション分割する方法](documentdb-sharding.md)」を参照してください。
