<properties 
	pageTitle="DocumentDB および Power BI の視覚化チュートリアル | Microsoft Azure" 
	description="Power BI Desktop で、完全に管理された NoSQL ドキュメント データベース DocumentDB の JSON データを使用して、洞察に満ちたレポートを作成する方法について説明します。" 
	services="documentdb" 
	authors="h0n" 
	manager="jhubbard" 
	editor="mimig" 
...
# Power BI で DocumentDB データを視覚化する
...
このチュートリアルでは、Power BI Desktop で DocumentDB アカウントに接続し、ナビゲーターを使用してデータを抽出するコレクションに移動して、Power BI Desktop クエリ エディターで JSON データを表形式に変換し、レポートを作成して PowerBI.com に発行する手順について説明します。
...
この記事で説明されている手順を実行するには、以下を保持している必要があります。
...
1. Power BI クエリ エディターの中央のウィンドウに **[ドキュメント]** 列が表示されます。![Power BI Desktop Query Editor](./media/documentdb-powerbi-visualize/pbiqueryeditor.png)
...
6. 中央のウィンドウに、**List** タイプの [coordinates] 列が表示されます。このチュートリアルの冒頭に示されているように、ここで使用する GeoJSON データは、Latitude 値と Longitude 値が coordinates 配列に記録された Point タイプです。
	...
	*注: coordinates[0] 要素は経度を表し、coordinates[1] は緯度を表します。* ![Power BI Desktop Coordinates List](./media/documentdb-powerbi-visualize/pbiresultflattenlist.png)
...
## レポートを作成する
Power BI Desktop レポート ビューは、レポート作成を開始できる場所です。**[レポート]** キャンバスにフィールドをドラッグ アンド ドロップすることにより、レポートを作成できます。
