...
> [!NOTE]
> Azure アカウントを持っていない場合、別の種類のアカウントが必要であることを示すエラー メッセージが表示されます。 現在の Azure アカウントから作成する方法については、「 [Azure Active Directory で職場または学校の ID を作成する](../virtual-machines/virtual-machines-windows-create-aad-work-id.md)」を参照してください。
...
フェールオーバー優先度の値は既存の構成と一致させます。 いずれかのリージョンの failoverPriority 値は 0 にする必要があります。これは、このリージョンが [DocumentDB アカウントの書き込みリージョン][scaling-globally]として保持されることを表します。 フェールオーバー優先度の値は、各場所間で一意である必要があり、フェールオーバー優先度の最大値は、リージョンの合計数よりも小さくする必要があります。 新しいリージョンは "読み取り" リージョンとなり、フェールオーバー優先度の値は 0 より大きくする必要があります。
...
> Azure PowerShell または Windows PowerShell でこのコマンドを実行する場合、予想外のコマンドに関するエラーを受け取ります。 代わりに、Windows PowerShell コマンド プロンプトでこのコマンドを実行します。
 ...
 - `<newdatabaseaccountlocation>` は追加される新しいリージョンであり、DocumentDB が一般公開されているリージョンのいずれかである必要があります。 最新のリージョン一覧については、 [Azure のリージョン ページ](https://azure.microsoft.com/regions/#services)を参照してください。


DocumentDB アカウントの読み取りリージョンとして米国東部リージョンを追加するときの入力例は次のとおりです。 
...
現在の DocumentDB のリージョン構成と一致する、次のようなローカルのテンプレート ファイルを作成します。 "locations" 配列には、追加する新しいリージョンと共に、データベース アカウント内のすべての既存のリージョンが含まれている必要があります。 azuredeploy.json ファイルに名前を付けます。
...
上記のテンプレート ファイルは、既に 2 つのリージョンがある DocumentDB アカウントに新しいリージョンが追加される例を示します。
...
azuredeploy.parameters.json ファイルの `"databaseAccountName"` の値フィールドを、使用するデータベースの名前に更新し、ファイルを保存します。 `"databaseAccountName"` に使用できる文字は、英小文字、数字、「-」のみです。文字数は 3 ～ 50 文字にする必要があります。 `"locationName1"` と `"locationName2"` の値フィールドを、DocumentDB アカウントがあるリージョンに更新します。 `"newLocationName"` の値フィールドを、追加するリージョンに更新します。
...
現在の DocumentDB のリージョン構成と一致する、次のようなローカルのテンプレート ファイルを作成します。 "locations" 配列には、そのリージョンを削除した後も残されるリージョンだけが含まれている必要があります。 **省略された場所は DocumentDB アカウントから削除されます**。 azuredeploy.json ファイルに名前を付けます。
...
azuredeploy.parameters.json ファイルの `"databaseAccountName"` の値フィールドを、使用するデータベースの名前に更新し、ファイルを保存します。 `"databaseAccountName"` に使用できる文字は、英小文字、数字、「-」のみです。文字数は 3 ～ 50 文字にする必要があります。 `"locationName1"` の値フィールドを、リージョンの削除後に DocumentDB アカウントが存在することになるリージョンに更新します。
