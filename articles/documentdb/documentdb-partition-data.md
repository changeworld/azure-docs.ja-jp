...
DocumentDB を使用することで、いつでも作成したコレクションの追加や削除ができるほか、それを新しい受信データの格納や、既存のコレクション上で利用できるデータの再調整に使用できます。コレクションの数については、[制限](documentdb-limits.md)に関するページを確認してください。このページにある制限を引き上げる場合は、いつでも弊社にお問い合わせください。
