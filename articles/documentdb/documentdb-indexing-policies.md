...
- データ型: **String** または **Number** (各パスのデータ型ごとにエントリを 1 つだけ含めることができます)
- インデックスの種類: **Hash** (等値クエリ) または **Range** (等値、範囲、Order By クエリ)
...
![オンラインのインデックスの変換](media/documentdb-indexing-policies/index-transformations.png)
...
- 現在の DocumentDB コレクションで、Order By や、新たに導入された文字列の範囲インデックスの種類が必要な文字列の範囲クエリなど、新しいインデックス作成機能の使用を開始する場合
