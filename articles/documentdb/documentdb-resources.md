...
---|---|---
_rid|システム生成|リソースに対してシステムが生成する一意かつ階層型の識別子。
_etag|システム生成|オプティミスティック同時実行制御に必要なリソースの ETag。
_ts|システム生成|リソースの最終更新時のタイムスタンプ。
_self|システム生成|リソースに対するアドレス指定可能な一意の URI。
id|ユーザー設定可能|リソースに対するユーザー定義の一意の名前。ユーザーが ID を指定しない場合は、システムによって生成されます。
...
|/dbs/{\_rid-db} |{\_rid-db} という値と一致する ID を持つデータベース。
|/dbs/{\_rid-db}/colls/ |データベース下のコレクションのフィード。
|/dbs/{\_rid-db}/colls/{\_rid-coll} |{\_rid-coll} という値と一致する ID を持つコレクション。
|/dbs/{\_rid-db}/colls/{\_rid-coll}/docs |コレクション下のドキュメントのフィード。
|/dbs/{\_rid-db}/colls/{\_rid-coll}/docs/{\_rid-doc} |{\_rid-doc} という値と一致する ID を持つドキュメント。
|/dbs/{\_rid-db}/users/ |データベース下のユーザーのフィード。
|/dbs/{\_rid-db}/users/{\_rid-user} |{\_rid-user} という値と一致する ID を持つユーザー。
|/dbs/{\_rid-db}/users/{\_rid-user}/permissions |ユーザー下のアクセス許可のフィード。
|/dbs/{\_rid-db}/users/{\_rid-user}/permissions/{\_rid-permission} |{\_rid-permission} という値と一致する ID を持つアクセス許可。
