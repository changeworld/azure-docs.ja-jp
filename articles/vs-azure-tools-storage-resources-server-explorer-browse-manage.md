...
1. **テーブル デザイナー**で、エンティティ (単一の行) またはプロパティ (単一のセル) のショートカット メニューを開き、**[編集]** をクリックします。
    ...
    - Timestamp という名前のプロパティは作成できません。Azure Storage サービスで、同名のプロパティが使用されています。

    - DateTime 値を入力する場合、ご利用のコンピューターの地域と言語の設定に適切な形式で入力する必要があります (たとえば、英語 (U.S) の場合であれば、MM/DD/YYYY HH:MM:SS [AM|PM])。
