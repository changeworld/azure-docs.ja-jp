...
攻撃者はえてして、アカウントの資格情報を不正に利用したり (パスワードの総当たり攻撃、フィッシング、資格情報の採集など)、ユーザーを欺いて悪質な (悪質な Web サイトからドライブバイ ダウンロードで、または悪質な電子メール添付ファイルから) コードを実行させたりするなどして特権アクセスの取得を試みます。リモートから管理されるクラウド環境では、いつ、どこからアクセスされるかわからないために、アカウント侵害はリスクの増大につながります。
...
管理と運用のセキュリティは、侵入の危険のあるポイントを減らしてクライアントの攻撃対象領域を小さくすることで強化できます。その拠り所となるのが "職務の分離" と "環境の隔離" というセキュリティの原則です。つまり機密上重要な機能を他の機能から分離することによって、ある 1 つのレベルで起こったミスが他のレベルのセキュリティ侵害に波及する可能性を低く抑えます。そこで、セキュリティの侵害を招きうる作業を管理タスクから切り離すことが重要となります (たとえば管理者の電子メールにマルウェアが添付されていると、インフラストラクチャ サーバーにまで感染が広がります)。同様に、高度な機密を要する業務用のワークステーションを、インターネットの閲覧といった高いリスクを伴う用途に使用することは避けます。

管理者のワークステーションにインストールされるアプリケーションまたはサービスは、そのひとつひとつが脆弱性を悪用される危険性をはらんでおり、セキュリティ リスクとして積み重なっていきます。そのため、システムの標準インストール イメージから不要なソフトウェアを削除してシステムの攻撃対象領域を小さくすれば、クライアントが管理しやすくなると共に安定性が高まり、クライアント ソフトウェアのセキュリティ面を堅牢化できます。たとえば管理やサポート、開発に使用される標準的なワークステーションには、そのデバイスの主な目的がクラウド サービスを管理することであるならば、電子メール クライアントなどの生産性アプリケーションをインストールする必要はありません。

インフラストラクチャ コンポーネントに対する管理アクセス権を持ったクライアント システムはネットワークにおいて、インフラストラクチャ コンポーネントそのものと同様、慎重に扱う必要があります。管理者または管理者システムのセキュリティが侵害された場合、サービスの侵害につながる可能性があります。セキュリティ リスクを小さくするために、クライアントには最大限厳しいポリシーを適用してください。管理者権限を持ったクライアント デバイスに対してはセキュリティ ポリシーで審査を厳しくします。たとえば、グループ ポリシーの設定で、デバイスからのオープン インターネット アクセスを拒否したり、制限の厳しいファイアウォール構成を使用したりすることが考えられます。

その他、次のような対策を導入することができます。
...
Azure では、[多要素認証](multi-factor-authentication/multi-factor-authentication.md)や [X.509 管理証明書](https://blogs.msdn.microsoft.com/azuresecurity/2015/07/13/certificate-management-in-azure-dos-and-donts/)、ファイアウォール規則を使ってインフラストラクチャへのアクセスを制限し、プラットフォーム サービス管理を実行できます。Azure ポータルと SMAPI には、トランスポート層セキュリティ (TLS) が必要となります。一方、Azure にデプロイするサービスやアプリケーションには、アプリケーションに応じた適切な防御手段が求められます。堅牢化したワークステーションの構成が標準化されていれば、こうしたメカニズムに何度でも簡単に対応することができます。
...
- RD ゲートウェイ経由でログオンする管理者には、多要素認証 ([Azure Multi-Factor Authentication](multi-factor-authentication/multi-factor-authentication.md)) またはスマート カード認証を使用します。
...
- [Microsoft セキュリティ レスポンス センター](http://www.microsoft.com/security/msrc/default.aspx) -- このサイトでは、Azure に関する問題を含め、マイクロソフトのセキュリティの脆弱性を報告できます。電子メールで [secure@microsoft.com](mailto:secure@microsoft.com) に送信することもできます。
