...
## Description
**Set-AzureRmFrontDoor** コマンドレットでは、既存の WAF ポリシーを更新します。 入力パラメーターが指定されていない場合は、既存の WAF ポリシーの古いパラメーターが使用されます。

## 例

### 例 1: 既存の WAF ポリシーを更新する
```powershell
...
### 例 4: $resourceGroup 内のすべての WAF ポリシーを更新する
```powershell
...
$resourceGroup 内のすべての WAF ポリシーを更新します。
