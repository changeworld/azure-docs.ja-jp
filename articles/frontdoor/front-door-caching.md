...
</br>バイト範囲の要求の詳細については、[RFC 7233](http://www.rfc-base.org/rfc-7233.html) を参照してください。
...
- **クエリ文字列を無視する**: 既定のモードです。 このモードでは、Front Door は要求元からのクエリ文字列を最初の要求時にバックエンドに渡して、アセットをキャッシュします。 Front Door 環境から提供されるアセットに対する後続のすべての要求で、キャッシュされたアセットの有効期限が切れるまで、クエリ文字列が無視されます。
...
</br>エッジ ノードから消去する資産を選択します。 すべてのアセットを消去する場合は、[すべて消去] チェック ボックスをオンにします。 それ以外の場合は、消去する各アセットのパスを [パス] テキスト ボックスに入力します。 パスには次の形式がサポートされています。
