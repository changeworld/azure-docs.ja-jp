...
### 例 1: Front Door を作成するために PSRoutingRuleObject を作成します
```powershell
...
### -CustomForwardingPath
この規則に一致するリソース パスの再書き込みに使用されるカスタム パス。
受信パスを使用するには空のままにします。
...
### -DynamicCompression
キャッシュが有効になっている場合に、キャッシュされたコンテンツに対して動的圧縮を有効にするかどうか。
既定値は Enabled です
