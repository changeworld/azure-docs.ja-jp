...
## Description
**New-AzureRmFrontDoorFireWallPolicy** コマンドレットでは、現在のサブスクリプションの指定されたリソース グループで新しい Azure WAF ポリシーを作成します。
...
### -EnabledState
ポリシーが有効状態であるか無効状態であるかを示します。
使用可能な値は 'Disabled'、'Enabled' です。
...
### -Mode
ポリシー レベルの防止モードであるか検出モードであるかを説明します。
使用可能な値は 'Prevention'、'Detection' です。
...
### -Name
WebApplicationFireWallPolicy 名。
