...
description: このチュートリアルでは、Azure Front Door サービスと Web Application Firewall を使用する方法について説明します
services: frontdoor
documentationcenter: ''
author: duongau
...
ms.locfileid: "90531204"
---
# <a name="tutorial-quickly-scale-and-protect-a-web-application-using-azure-front-door-and-azure-web-application-firewall-waf"></a>チュートリアル:Azure Front Door と Azure Web Application Firewall (WAF) を使用して、Web アプリケーションをすばやくスケーリングし、保護する
...
> - Front Door を作成する。
> - Azure WAF ポリシーを作成する。
> - WAF ポリシーのルールセットを構成する。
> - Front Door に WAF ポリシーを関連付ける
> - カスタム ドメインの構成
...
## <a name="create-an-azure-front-door-afd-resource"></a>Azure Front Door (AFD) リソースを作成する
...
## <a name="add-managed-rulesets-to-this-waf-policy"></a>この WAF ポリシーにマネージド ルールセットを追加する
...
## <a name="associate-the-waf-policy-with-the-afd-resource"></a>WAF ポリシーを AFD リソースに関連付ける
...
## <a name="configure-custom-domain-for-your-web-application"></a>Web アプリケーションのカスタム ドメインを構成する
...
このチュートリアルのリソースが不要になったら、[az group delete](https://docs.microsoft.com/cli/azure/group?view=azure-cli-latest#az-group-delete) コマンドを使用して、リソース グループ、Front Door、WAF ポリシーを削除してください。

```azurecli-interactive
  az group delete \
    --name <>
```
--name このチュートリアルでデプロイしたすべてのリソースのリソース グループの名前。

## <a name="next-steps"></a>次の手順

Front Door をトラブルシューティングする方法については、引き続き攻略ガイドをご覧ください。
