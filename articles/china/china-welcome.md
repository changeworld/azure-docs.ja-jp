---
title: Azure China 21Vianet へようこそ | Microsoft Docs
description: Microsoft Azure China 21Vianet は、中国内にあるクラウド サービスの物理的に分離されたインスタンスです。 このサービスは、同等のサービス レベルで Microsoft グローバル クラウド サービスを構成する、同じ Azure、Office 365、Power BI テクノロジに基づいています。
services: china
cloud: na
documentationcenter: na
author: v-wimarc
manager: edprice
ms.assetid: na
ms.service: china
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: na
ms.workload: na
ms.date: 07/31/2018
ms.author: v-wimarc
ms.openlocfilehash: 60f847a4d708e8c5c8e9bac352a19f2386adfa4e
ms.sourcegitcommit: 8fc5f676285020379304e3869f01de0653e39466
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 05/09/2019
ms.locfileid: "65511139"
---
# <a name="welcome-to-azure-china-21vianet"></a>Azure China 21Vianet へようこそ
21Vianet (Azure China 21Vianet) が運営する Microsoft Azure は、中国内にあるクラウド サービスの物理的に分離されたインスタンスであり、Beijing 21Vianet Broadband Data Center Co., Ltd の完全子会社である Shanghai Blue Cloud Technology Co., Ltd. ("21Vianet") によって独立して運用され、処理されています。 

このサービスは、同等のサービス レベルで Microsoft グローバル クラウド サービスを構成する、同じ Azure、Microsoft Office 365、Microsoft Power BI テクノロジに基づいています。 中国の Microsoft Azure 向けの契約は、適用できる場合、顧客と 21Vianet との間で署名されます。

政府規制に準拠するように中国で提供される最初の外国のパブリック クラウド サービス プロバイダーとして、[Azure China 21Vianet](/azure/china/china-overview-operations) は、アーキテクチャに構築されたすべてのシステムとアプリケーションに対して中国の規制で必要とされる、[セキュリティ センター](https://www.trustcenter.cn/compliance/)で協議されたワールド クラスのセキュリティを提供します。 Azure には、サービスとしてのインフラストラクチャ (IaaS)、サービスとしてのプラットフォーム (PaaS)、およびサービスとしてのソフトウェア (SaaS) のコア コンポーネントがあります。 これらのコンポーネントには、ネットワーク、ストレージ、データ管理、ID 管理、その他多くのサービスが含まれます。

Azure China 21Vianet では、静止データのレプリケーションや自動スケールなど、グローバル Azure で提供されるものと同じ優れた[サービス](https://www.azure.cn/home/features/products-by-region)がほとんどサポートされます。 既にグローバル Azure サービスを使用している場合でも、中国で運用するには、ご利用のアプリケーションやサービスの一部またはすべてを[再ホスト](/azure/china/china-how-to-rehost)または[リファクター](/azure/china/china-how-to-refactor)する必要があります。 

このドキュメントには、次のトピックが含まれています。
- 中国固有のインターネット環境とポリシーを使って中国でサービスを設定しようとする組織向けの[規制に関する考慮事項](/azure/china/china-overview-policies)
- [開発者向けのガイダンス](/azure/china/china-get-started-developer-guide)
- Global Connection Toolkit など、Azure にアプリケーションとサービスを[移行するためのヘルプ](/azure/china/china-how-to-rehost)
- Azure ソリューション サービスを持つ[パートナー](/azure/china/china-resources-partners)

## <a name="operations"></a>Operations
中国[電気通信規制](http://www.china.org.cn/business/laws_regulations/2010-01/20/content_19273945.htm) (中国語) に従って、クラウド サービスのプロバイダー (IaaS および PaaS) は、付加価値電気通信の許可を有する必要があります。 外国投資が 50% 未満の国内登録企業のみがこれらの許可を受ける資格があります。 この規制に準拠するため、中国の Azure サービスは Microsoft からライセンスされたテクノロジに基づいて、21Vianet が運用しています。 

## <a name="next-steps"></a>次の手順
- [Microsoft Azure China 21Vianet のサービスと機能](https://www.azure.cn/home/features/products-by-region)
- [Azure グローバル接続センター](https://aka.ms/azcc)
- [Azure の導入事例](https://www.azure.cn/partnerancasestudy/case-studies/) (中国語)
- [GitHub の Global Connection Toolkit](https://github.com/Azure/AzureGlobalConnectionToolkit)
- [21Vianet が運営するクラウド サービス ビジネス モデル](https://wacnppe.blob.core.chinacloudapi.cn/marketing-resource/documents/Windows_Azure_and_Office_365_cloud_services_business_model_operated_by_21Vianet12.pdf) (中国語)
- [Azure China 21Vianet のセキュリティ センター: コンプライアンス](https://www.trustcenter.cn/compliance/)
- [開発者ガイド](https://www.azure.cn/documentation/articles/developerdifferences/#dev-guide) (中国語)

