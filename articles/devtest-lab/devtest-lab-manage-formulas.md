...
> [!NOTE]
> 数式 - [カスタム イメージ](devtest-lab-create-template.md)のような数式を使用すると、VHD ファイルから基本イメージを作成できます。基本イメージは、新しい VM のプロビジョニングに使用されます。どちらの方法がご使用の環境に適しているかを判断するには、「[DevTest ラボのカスタム イメージと数式の比較](devtest-lab-comparing-vm-base-image-types.md)」の記事を参照してください。
> 
> 
...
6. **[数式の作成]** ブレードで、新しい数式の**名前**と**説明**を入力します。
