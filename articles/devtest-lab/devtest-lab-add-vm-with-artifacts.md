...
> [AZURE.NOTE] この記事の付属のビデオ [How to create VMs with artifacts in a lab](/documentation/videos/how-to-create-vms-with-artifacts-in-a-devtest-lab) (ラボでアーティファクトを含む VM を作成する方法) を表示します。
...
1. ラボのブレードで、次の図に示す **[ラボ VM]** を選択します。![[+ ラボ VM] ボタン](./media/devtest-lab-add-vm-with-artifacts/devtestlab-home-blade-add-vm.png)
...
1. **[アーティファクト]** を選択し、アーティファクトの一覧から基本イメージに追加するアーティファクトを選択して構成します。**注:** DevTest ラボやアーティファクトの構成に関する経験がない場合は、「[既存のアーティファクトの VM への追加](#add-an-existing-artifact-to-a-vm)」セクションに進み、終了してからここに戻ってください。
...
1. **[追加]** を選択してアーティファクトを追加し、**[アーティファクトの追加]** ブレードに戻ります。
...
1. **[アーティファクトの追加]** ブレードの上部で、VM に追加されたアーティファクトの数を示すリンクを選択します。
...
1. **[アーティファクトの追加]** ブレードで、必要な変更を行ったら **[OK]** を選択して **[アーティファクトの追加]** ブレードを閉じます。
...
1. **[ラボ VM]** ブレードで、**[ARM テンプレートの表示]** を選択します。

1. **[Azure Resource Manager テンプレートの表示]** ブレードで、すべてのテンプレート テキストを選択します。

1. 選択したテキストをクリップボードにコピーします。

1. **[OK]** を選択して、**[Azure Resource Manager テンプレートの表示]** ブレードを閉じます。
