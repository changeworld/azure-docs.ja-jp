...
[ラボを作成](devtest-lab-create-lab.md)したら、VM テンプレートの一覧から [VM をそのラボに追加](devtest-lab-add-vm-with-artifacts.md)できます。この記事では、バーチャル ハード ディスク (VHD) イメージ ファイルをアップロードし、VM の作成に使用するテンプレートとして構成する方法について説明します。VHD イメージを使い慣れていない場合、VHD イメージの作成方法については、「[Windows Server VHD の作成と Azure へのアップロード](https://azure.microsoft.com/ja-JP/documentation/articles/virtual-machines-create-upload-vhd-windows-server/)」をご覧ください。VHD イメージを作成するか、VHD イメージへのアクセス権を取得したら、この記事の手順に従って VHD イメージをアップロードし、イメージからテンプレートを作成します。
