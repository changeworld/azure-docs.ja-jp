---
title: DevTest ラボ VM のカスタム アーティファクトの作成 | Microsoft Docs
...
2. サンプルの artifactfile.json の取得 - [GitHub リポジトリ](https://github.com/Azure/azure-devtestlab)で、Azure DevTest ラボ チームが作成したアーティファクトを確認します。このリポジトリには、独自のアーティファクトの作成に役立つ豊富なアーティファクト ライブラリが用意されています。アーティファクト定義ファイルをダウンロードし、変更を加えて独自のアーティファクトを作成します。
