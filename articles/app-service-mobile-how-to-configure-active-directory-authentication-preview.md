...
1. [Azure の管理ポータルのプレビュー]にログオンし、App Service ゲートウェイに移動します。
...
4. ディレクトリを選択し、上部の **[アプリケーション]** タブをクリックします。下部の **[追加]** をクリックして、新しいアプリケーションの登録を作成します。 
...
8. アプリケーションが追加されたら **[構成]** タブをクリックします。**[シングル サインオン]** の **[応答 URL]** を編集して、ゲートウェイの URL の末尾にパス _/signin-aad_ を追加します。例:  `https://contosogateway.azurewebsites.net/signin-aad`.HTTPS スキームを使用していることを確認します。

    ![][3]

9. **[保存]** をクリックします。その後、アプリケーションの**クライアント ID** をコピーします。

## <a name="secrets"> </a>Azure Active Directory の情報をモバイル アプリに追加します。

10. プレビュー管理ポータルからゲートウェイの **[ユーザー認証]** ブレードに戻ります。Azure Active Directory ID プロバイダーの **[クライアント ID]** 設定に貼り付けます。
  
11. **[許可されたテナント]** リストで、アプリケーションを登録したディレクトリのドメインを追加する必要があります (contoso.onmicrosoft.com など)。既定のドメイン名は、Azure Active Directory テナントで **[ドメイン]** タブをクリックして検索できます。ドメイン名を **[許可されたテナント]** リストに追加して **[保存]** をクリックします。  
