...
| 合計キー数  | 過去のレポート期間におけるキャッシュ内のキーの最大数。 この番号は、Redis INFO コマンドの `keyspace` にマップされます。 基礎となるメトリック システムの制限により、クラスタリングが有効になっているキャッシュでキー合計が返すキーの最大数は、レポート期間中にキーの数が最大のシャードのキーの最大数になります。  |
