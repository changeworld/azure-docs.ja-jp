...
* [クライアント側のトラブルシューティング](#client-side-troubleshooting) - このセクションでは、Azure Cache for Redis に接続されているアプリケーションが原因で発生した問題を特定し、解決するためのガイドラインを示します。
* [サーバー側のトラブルシューティング](#server-side-troubleshooting) - このセクションでは、Azure Cache for Redis サーバー側で発生した問題を特定し、解決するためのガイドラインを示します。
...
2. VM (クライアントおよび Azure Cache for Redis 用) のサイズを増やし、帯域幅容量を大きくして、より大きい応答のデータ転送時間を短縮します。 サーバーのみ、またはクライアントのみの帯域幅を増やすだけでは十分でない場合があります。 帯域幅使用量を測定し、現在使用している VM のサイズの容量と比較します。
...
#### <a name="resolution"></a>解決策
* 「[Azure Cache for Redis Advisor](cache-configure.md#azure-cache-for-redis-advisor)」に記載されている推奨事項とアラートを確認します。
...
* 「[Azure Cache for Redis performance](cache-faq.md#azure-cache-for-redis-performance)」(Azure Cache for Redis のパフォーマンス) の表を見て、現在のレベルの上限しきい値に近づいているかどうかを確認します。 必要に応じて、CPU 処理能力を高めた、より大きいキャッシュ レベルに[スケーリング](cache-how-to-scale.md)します。 既に Premium レベルを使用している場合は、[クラスタリングによるスケールアウト](cache-how-to-premium-clustering.md)を検討してください。
...
1. Azure Cache for Redis とクライアント アプリケーションが、Azure の同じリージョンにあることを確認します。 たとえば、キャッシュは米国東部にあるが、クライアントが米国西部にあり、要求が `synctimeout` 間隔内に完了しない場合はタイムアウトになることがあります。また、ローカル開発用コンピューターからのデバッグ時にタイムアウトになることがあります。 
   
    キャッシュとクライアントを同じ Azure リージョン内に配置することを強くお勧めします。 リージョン間呼び出しを含むシナリオの場合、接続文字列に `synctimeout` プロパティを含めることで、`synctimeout` 間隔を既定値の 1000 ミリ秒間隔より大きい値に設定する必要があります。 次の例は、`synctimeout` を 2000 ミリ秒に設定した StackExchange.Azure Cache for Redis キャッシュ接続文字列のスニペットを示しています。
