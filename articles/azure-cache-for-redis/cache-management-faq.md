...
ここでは、redis-benchmark.exe の使用例をいくつか紹介します。 正確な結果を得るために、以下のコマンドはキャッシュと同じリージョンにある VM から実行してください。
