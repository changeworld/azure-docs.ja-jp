...
-  Azure の仮想ネットワークに Active Directory ドメイン サービス (AD DS) をインストールする方法に関する概念的なガイダンスについては、「[Azure の仮想マシンでの Windows Server Active Directory のデプロイ ガイドライン](http://msdn.microsoft.com/ja-jp/library/windowsazure/jj156090.aspx)」を参照してください。
-  AD DS を含む Azure でのテスト ラボ環境を作成する手順については、「[Test Lab Guide: Base Configuration in Azure (テスト ラボ ガイド: Azure での最適な構成)](http://www.microsoft.com/ja-jp/download/details.aspx?id=41684)」を参照してください。
...
2. 仮想ネットワークを作成します。<b>[ネットワーク]</b>、<b>[仮想ネットワークの作成]</b> の順にクリックします。次の表の値を使用してウィザードの手順を完了します。 
	...
	ウィザードのページ  | 指定する値
	------------- | -------------
	**仮想ネットワークの詳細**  | <p>仮想ネットワークの名前を入力する</p><p>リージョン: 最寄りのリージョンを選択する</p>
...
2. 次の表の値を使用してウィザードの手順を完了します。
	...
	**仮想マシンの構成**  | <p>リリース日: 今日の日付</p><p>マシン名: 一意の値を指定</p><p>階層: 標準</p><p>サイズ: 任意のサイズを選択する</p><p>ユーザー名: ユーザーの名前を入力する。このユーザー アカウントは組み込みの Administrators グループのメンバーになります。</p><p>パスワード: </p><ul><li>必ず 8 文字以上で構成し、次の種類の 3 文字を含める</li><li>大文字</li><li>小文字</li><li>数字</li></ul>特殊文字
...
3. VM にログオンし、追加のディスクをフォーマットします。 
  ...
  6. Click <b>[ディスクの管理]</b> をクリックし、<b>[OK]</b> をクリックして新しいディスクを初期化します。 
...
-  [Azure での Windows Server 2012 R2 基本構成に関するテスト ラボ ガイド](http://www.microsoft.com/ja-jp/download/details.aspx?id=41684)
