...
ユーザーは Web ブラウザー ( `https://<Hadoop cluster name>.azurehdinsight.net/Home/HiveEditor`) に URL を入力してクエリ コンソール (ハイブ エディター) を使用できます (ログインするために、Hadoop クラスターの資格情報を入力するように求められます)。または、[PowerShell を使用してハイブ ジョブを送信する](hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell) こともできます。 
