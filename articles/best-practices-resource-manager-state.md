...
Azure リソース マネージャー テンプレートでは、単一値パラメーターに加えて、パラメーターとして複合オブジェクトを使用できます。複合オブジェクトを使用すると、T シャツ サイズ (仮想マシンのサイズを意味する表現)、ネットワークの設定、オペレーティング システム (OS) の設定、可用性の設定など、特定分野のデータのコレクションを実装し、参照できます。
...
- デプロイ中に、メイン テンプレートに直接提供されるパラメーター
...
容量、機能、またはエンド ツー エンドのスコープを持つソリューション テンプレートでは、リンクされたテンプレートは通常、ネットワーク上に存在するリソースを作成します。わかりやすい方法の 1 つとして、複合オブジェクトを使用してネットワークの設定を格納し、リンクされたテンプレートに渡すことが挙げられます。
