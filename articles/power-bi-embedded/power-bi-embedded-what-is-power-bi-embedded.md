...
**ワークスペース** は、Power BI コンテンツのコンテナーであり、データセット、レポート、ダッシュボードを含めることができます。 **ワークスペース** は、最初に作成された時点では空です。 プレビュー期間中は、Power BI Desktop を使用してすべてのコンテンツを作成し、[Power BI IMPORT API](https://msdn.microsoft.com/library/mt711504.aspx) を使用して、PBIX をワークスペースにプログラムでデプロイします。
