<properties
   pageTitle="Power BI Embedded REST API"
   description=""
   services="power-bi-embedded"
   documentationCenter=""
   authors="dvana"
   manager="NA"
   editor=""
   tags=""/>
<tags
   ms.service="power-bi-embedded"
   ms.devlang="NA"
   ms.topic="article"
   ms.tgt_pltfrm="NA"
   ms.workload="powerbi"
   ms.date="03/29/2016"
   ms.author="derrickv"/>

# Power BI Embedded REST API

**Microsoft Power BI Embedded** では、REST API を使用して、**Power BI レポート**を独自のアプリケーションに統合するアプリケーションをプログラムによって開発できます。

**Power BI Embedded** REST API の詳細については、[Power BI の REST 操作](https://msdn.microsoft.com/library/mt711493)に関するページをご覧ください。

## 関連項目

- [What is Microsoft Power BI Embedded (Microsoft Power BI Embedded とは)](power-bi-embedded-what-is-power-bi-embedded.md)
- [Get started with Microsoft Power BI Embedded Preview (Microsoft Power BI Embedded プレビューを使ってみる)](power-bi-embedded-get-started.md)

<!---HONumber=AcomDC_0406_2016-->