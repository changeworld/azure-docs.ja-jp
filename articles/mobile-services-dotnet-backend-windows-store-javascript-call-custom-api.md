<properties pageTitle="Windows ストア クライアントからのカスタム API の呼び出し - Mobile Services" metaKeywords="" description="カスタム API を定義し、Windows Azure モバイル サービスを使用した Windows ストア アプリから呼び出す方法について説明します。" metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Call a custom API from the client" authors="glenga"  solutions="" writer="glenga" manager="dwrede" editor=""  />
