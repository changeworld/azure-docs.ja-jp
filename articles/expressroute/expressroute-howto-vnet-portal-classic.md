<properties
   ...
   description="この記事では、クラシック デプロイ モデルを使用して ExpressRoute 用に仮想ネットワーク (VNet) をセットアップする手順について説明します。"
...
# クラシック ポータルでの ExpressRoute 用の Virtual Network の構成

この記事の手順では、クラシック デプロイ モデルとクラシック ポータルを使用して、ExpressRoute で使用する仮想ネットワークとゲートウェイを構成する手順について説明します。

リソース マネージャー デプロイ モデルでの手順については、「[PowerShell を使用した仮想ネットワークの作成](../virtual-network/virtual-networks-create-vnet-arm-ps.md)」および「[ExpressRoute 構成のリソース マネージャー VNet への VPN Gateway の追加](expressroute-howto-add-gateway-resource-manager.md)」を参照してください。
...
11. ネットワークを回線にリンクします。記事「[ExpressRoute 回線への VNet のリンク](expressroute-howto-linkvnet-classic.md)」の指示に従ってください。
