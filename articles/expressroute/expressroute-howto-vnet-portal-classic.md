...
以下に示したのは、クラシック VNet と仮想ネットワーク ゲートウェイを作成する手順です。既にクラシック VNet をご利用の場合は、この記事の「[既存のクラシック VNet を構成する](#config)」セクションを参照してください。
