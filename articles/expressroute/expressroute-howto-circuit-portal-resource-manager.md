...
- **手順 1.** 新しいリソースを作成するオプションを選択して、ExpressRoute 回線を作成できます。次の図に示すように、**[新規]****、****[ネットワーク]****、****[ExpressRoute]** の順にクリックします。 

	![](./media/expressroute-howto-circuit-portal-resource-manager/createcircuit1.png)

- **手順 2.** **[ExpressRoute]** をクリックすると、**[ExpressRoute 回線の作成]** ブレードが表示されます。このブレードに値を入力するときは、SKU レベルとデータ計測方法を正しく指定してください。
	...
	- **[データ計測方法]** によって、課金の種類が決まります。従量制課金データ プランの場合は [従量制] を、無制限データ プランの場合は [無制限] を指定できます。**注:** 課金の種類を [従量制] から [無制限] に変更することはできますが、[無制限] から [従量制] に変更することはできません。
...
- **プロパティを表示するには**
	...
	回線を選択することで、回線のプロパティを表示できます。このブレードでは、回路のサービス キーに注意してください。プロビジョニング プロセスを完了するには、回線の回線キーをコピーして、サービス プロバイダーに渡す必要があります。回線キーは回線に固有です。
...
このブレードで、[プロバイダーの状態] はサービス プロバイダー側でのプロビジョニングの現在の状態に関する情報を提供し、[回線の状態] は Microsoft 側での状態を示します。回線のプロビジョニング状態に関する詳細については、[ワークフロー](expressroute-workflows.md#expressroute-circuit-provisioning-states)に関する記事を参照してください。
...
回線を選択することで、注目している回線のプロパティを表示できます。**[プロバイダーの状態]** が **[プロビジョニング済み]** になっていることを確認してから続行します。
...
ExpressRoute 回線の特定のプロパティは、接続に影響を与えることなく変更できます。現在、Azure ポータルを使用して ExpressRoute 回線のプロパティを変更することはできません。ただし、PowerShell を使用すれば回線のプロパティを変更できます。PowerShell を使用した「[ExpressRoute 回線の変更](expressroute-howto-circuit-arm.md#modify)」を参照してください
...
**[削除]** アイコンを選択して、ExpressRoute 回線を削除できます。以下の点に注意してください。
