...
サイト間 VPN と ExpressRoute が構成可能な場合、いくつかの利点があります。ExressRoute 用にセキュリティで保護されたフェールオーバー パスとしてサイト間 VPN を構成したり、サイト間 VPN を使用して、ネットワークの一部ではないものの、ExpressRoute 経由で接続されているサイトに接続したりすることができます。この記事では、両方のシナリオを構成する手順について説明します。現時点で、この構成を作成するために使用できるのは、クラシック デプロイ モデルのみです。リソース マネージャーのデプロイ モデルに適用されるドキュメントが準備できたら、このページからリンクする予定です。
