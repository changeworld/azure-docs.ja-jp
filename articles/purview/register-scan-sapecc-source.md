---
title: SAP ECC ソースへの接続と管理
description: このガイドでは、Azure Purview で SAP ECC に接続する方法と、Purview の機能を使用して SAP ECC ソースをスキャンおよび管理する方法について説明します。
author: chandrakavya
ms.author: kchandra
ms.service: purview
...
ms.locfileid: "131023768"
---
# <a name="connect-to-and-manage-sap-ecc-in-azure-purview"></a>Azure Purview での SAP ECC への接続と管理

この記事では、Azure Purview で、SAP ECC を登録する方法、および SAP ECC を認証して操作する方法について説明します。 Azure Purview の詳細については、[概要の記事](overview.md)を参照してください。
...
このセクションでは、[Purview Studio](https://web.purview.azure.com/) を使用して Azure Purview に SAP ECC を登録する方法について説明します。
...
1. **[登録]** を選択します
1. [Register sources]\(ソースの登録\) で、 **[SAP ECC]** を選択します。 **[続行]** を選択します。
...
1. SAP ECC ソースに接続するための **アプリケーション サーバー** 名を入力します。 SAP アプリケーション サーバー ホストの IP アドレスにすることもできます。
...
次の手順に従って、SAP ECC をスキャンし、自動的に資産を識別してデータを分類します。 スキャン全般の詳細については、[スキャンとインジェストの概要](concept-scans-and-ingestion.md)に関するページを参照してください。
...
1. 次の詳細を指定します。
    ...
    1. **[クライアント ID]** : SAP クライアント ID を入力します。 これは、000 から 999 の 3 桁の数値です。

    1. **[JCo ライブラリのパス]** : JCo ライブラリが配置されているディレクトリ パス。

    1. **[Maximum memory available]\(使用可能な最大メモリ\):** スキャン プロセスで使用される、セルフホステッド統合ランタイム マシンで使用可能な最大メモリ (GB 単位)。 これは、スキャンする SAP ECC ソースのサイズによって異なります。 使用可能な大きなメモリ (100 など) を指定することをお勧めします。
