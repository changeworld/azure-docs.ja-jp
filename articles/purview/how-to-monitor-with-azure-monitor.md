...
2. ストレージ アカウントにイベントをログします。 診断ログのアーカイブには、専用のストレージ アカウントを使用することをお勧めします。 この記事に従って[ストレージ アカウントを作成します](../storage/common/storage-quickstart-create-account.md?tabs=azure-portal)。
...
新しく作成されたストレージ アカウントでログの受信が開始されるまでに最大 15 分かかります。 [Azure Storage アカウントでのリソース ログのデータ保持とスキーマを確認します](../azure-monitor/platform/resource-logs-collect-storage.md)。 診断ログが構成されると、イベントがストレージ アカウントに送られるようになります。
