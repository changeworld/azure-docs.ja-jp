...
> [!NOTE]
> 分類は直接照合されますが (社会保障番号には **社会保障番号** の分類があります)、秘密度ラベルは 1 つまたは複数の分類とシナリオが一緒に検出されたときに適用されます。 
> 
...
> [!IMPORTANT]
> 同意すると、Microsoft がラベル名とカスタムの機密情報の種類の名前を、Azure Purview と Azure Security Center (ASC) の "*両方*" に共有できるようになります。 Microsoft では、Azure Purview のラベル情報を使用して、ASC の推奨事項とアラートを強化しています。 
>
> Microsoft 365 コンプライアンス センターでの同意は、このデータを両方のサービスと共有するために適用されます。 現在、ラベル情報を Azure Purview のみと共有する選択肢はありません。
