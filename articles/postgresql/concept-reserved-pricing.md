---
title: 予約済みコンピューティングの価格 - Azure Database for PostgreSQL
...
> [!IMPORTANT]
> 予約容量の価格は、Azure Database for PostgreSQL の [Single Server](./overview.md#azure-database-for-postgresql---single-server)、[フレキシブル サーバー](flexible-server/overview.md)、および [Hyperscale Citus](./overview.md#azure-database-for-postgresql--hyperscale-citus) デプロイ オプションで使用できます。 Hyperscale (Citus) での RI の価格の詳細については、[このページ](concepts-hyperscale-reserved-pricing.md)を参照してください。
...
たとえば、1 つの汎用の Gen5 - 32 仮想コア PostgreSQL データベースと、2 つのメモリ最適化済みの Gen5 - 16 仮想コア PostgreSQL データベースを実行しているとします。 さらに、来月中に追加で汎用の Gen5 - 8 仮想コア データベース サーバーを 1 つと、メモリ最適化済みの Gen5 - 32 仮想コア データベース サーバーを 1 つデプロイする予定だとします。 少なくとも 1 年間はこれらのリソースが必要になることがわかっているとします。 この場合、単一データベースの汎用 - Gen5 用に 40 (32 + 8) 個の仮想コア 1 年予約分と、単一データベース メモリ最適化済み - Gen5 用に 64 (2x16 + 32) 個の仮想コア 1 年予約分を購入する必要があります
...
| Scope | 1 つのサブスクリプションまたは複数のサブスクリプション (共有スコープ) を仮想コアの予約のスコープにすることができます。 以下を選択した場合: </br></br> **共有** - 仮想コアの予約割引は、課金のコンテキスト内にある任意のサブスクリプションで実行されている Azure Database for PostgreSQL サーバーに適用されます。 エンタープライズのお客様の場合、共有スコープが対象の登録であり、登録内のすべてのサブスクリプションが含まれます。 従量課金制のお客様の場合、共有スコープは、アカウント管理者が作成するすべての従量課金制サブスクリプションです。</br></br>**管理グループ** では、管理グループと課金スコープの両方の一部である任意のサブスクリプションで実行されている Azure Database for PostgreSQL に予約割引が適用されます。</br></br> **単一サブスクリプション** - 仮想コアの予約割引はこのサブスクリプションの Azure Database for PostgreSQL サーバーに適用されます。 </br></br> **1 つのリソース グループ** - 予約割引は、選択したサブスクリプションおよびそのサブスクリプション内の選択したリソース グループ内の Azure Database for PostgreSQL サーバーに適用されます。
