...
```
>[!Note]
> `The parameter PrivateDnsZoneArguments is required, and must be provided by customer` というエラーが発生した場合は、古いバージョンの Azure CLI が実行されている可能性があります。 [Azure CLI をアップグレード](/cli/azure/update-azure-cli)してから、操作をやり直してください。
