---
title: クエリ ストアのシナリオ - Azure Database for PostgreSQL - フレキシブル サーバー
description: この記事では、Azure Database for PostgreSQL - フレキシブル サーバーでのクエリ ストアのいくつかのシナリオについて説明します。
author: ssen-msft
ms.author: ssen
...
ms.locfileid: "107558731"
---
# <a name="usage-scenarios-for-query-store---flexible-server"></a>クエリ ストア - フレキシブル サーバーの使用シナリオ

**適用対象:** Azure Database for PostgreSQL - 単一サーバー バージョン 11、12
...
サーバーの azure_sys データベースでクエリ ストア ビューを使用すると、実行時間が長いクエリをすばやく特定できます。 このようなクエリは、一般に、大量のリソースを消費する傾向があります。 実行時間が長いクエリを最適化すると、システムで他のクエリの実行に使用できるリソースが解放されるため、パフォーマンスを向上させることができます。 
