...
# Azure Database for PostgreSQL: .NET (C#) を使用した接続とデータの照会
...
## Visual Studio ソリューションへの Npgsql 参照のインストール
