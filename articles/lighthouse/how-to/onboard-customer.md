...
オンボードされたサブスクリプションに自分の Microsoft Partner Network (MPN) ID を関連付けると、顧客エンゲージメント全体における自分の影響を追跡し、評価が得られます。 詳しくは、「[Azure アカウントにパートナー ID をリンクする](https://docs.microsoft.com/azure/billing/billing-partner-admin-link-started)」をご覧ください。 この関連付けは、自分がリソースを管理している各顧客テナントについて、別々に実施する必要があることに注意してください。 
