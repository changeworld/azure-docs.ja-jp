...
>[AZURE.NOTE]**Visual Studio 2015 Preview を使用して構築された Cordova プロジェクトで Azure Mobile Services を使用する場合は、この[回避策](http://go.microsoft.com/fwlink/?LinkId=518765)を適用してください。以降のバージョンの Visual Studio 2015 を使用したプロジェクトではこの回避策は不要です。**
...
新しい項目をデータ テーブルに挿入します。ID (文字列型の GUID) が新しい行のプライマリ キーとして自動的に作成されます。返された [Promise]() オブジェクト上の [done]() メソッドを呼び出し、挿入されたオブジェクトのコピーを取得して、エラーがあれば処理します。
...
データ テーブルの行を更新します。このコードでは、モバイル サービスが応答すると、項目は一覧から削除されます。返された [Promise]() オブジェクト上の [done]() メソッドを呼び出し、挿入されたオブジェクトのコピーを取得して、エラーがあれば処理します。
...
**del** メソッドを使用してデータ テーブルの行を削除します。返された [Promise]() オブジェクト上の [done]() メソッドを呼び出し、挿入されたオブジェクトのコピーを取得して、エラーがあれば処理します。
