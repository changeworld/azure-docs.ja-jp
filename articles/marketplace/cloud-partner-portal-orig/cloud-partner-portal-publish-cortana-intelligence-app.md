...
この ID は製品 URL に表示されます。 小文字の英数字またはハイフン (-) のみで構成できます。 この ID はハイフンで終えることはできず、長さは最大で 50 文字です。 
>[!Note]
...
[オファーの設定] で必要な情報をすべて入力したら、**[保存]** を選択してオファーの [技術情報] の部分に移ります。 オプション。
...
登録済み Microsoft パートナーの場合は、[パートナーの Web サイト](https://partners.microsoft.com/) に移動してサインインし、組織のパートナー ID を取得します。その ID を **[Partner MPN Id]** に入力します。
...
ソリューションが、 Azure Suite コンポーネントの 顧客 1 人当たり毎月少なくとも 1,000 ドルを生み出しているか、またはソリューションが Microsoft R を使用しているかを示します。
...
- ビデオの長さは 15 分未満にする必要があります。
- ビデオは、最低限の編集を行ったソリューションの機能の録画にしなければなりません。 
