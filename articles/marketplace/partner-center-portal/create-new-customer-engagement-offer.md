...
description: Microsoft AppSource で Dynamics 365 for Customer Engagement & PowerApps オファーを作成する方法。 AppSource で、またはクラウド ソリューション プロバイダー (CSP) プログラムを使用して、オファーを一覧表示または販売します。
...
この記事では、新しい Dynamics 365 for Customer Engagement & PowerApps オファーの作成方法について説明します。 Dynamics 365 for Customer Engagement のすべてのアプリ (PowerApps、Sales、Service、Project Service、および Field Service) は、ソリューションの標準要件、互換性、および適切なプラクティスを確認する弊社の認定プロセスを通過する必要があります。 試用版エクスペリエンスでは、ユーザーがソリューションをライブの Dynamics 365 環境にデプロイできます。
...
オファーのバージョン番号を入力します。 顧客にはオファーの詳細ページにこのバージョンが表示されます。<!-- If you are only updating the version number due to marketing/descriptive changes, check the **Marketing only change** box. This option allows the offer to bypass the certification and provisioning stages.-->
