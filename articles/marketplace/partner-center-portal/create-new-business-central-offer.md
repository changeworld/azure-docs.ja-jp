---
title: Microsoft AppSource で Dynamics 365 Business Central オファーを作成する
description: Microsoft AppSource で Dynamics 365 for Operations オファーを作成する方法。 AppSource で、またはクラウド ソリューション プロバイダー (CSP) プログラムを使用して、オファーを一覧表示または販売します。
...
オファーがどのように動作するかを示すスクリーンショットを追加します。 少なくとも 3 つのスクリーンショットが必要です。最大 5 つまで追加できます。 スクリーンショットはすべて 1280 x 720 ピクセルの PNG 形式にする必要があります。
