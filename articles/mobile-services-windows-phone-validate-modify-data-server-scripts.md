...
1.  Visual Studio 2012 Express for Windows Phone で、[データの使用][データの使用]に関するチュートリアルを実行したときに変更したプロジェクトを開きます。

2.  **F5** キーを押してアプリケーションを実行し、テキスト ボックスに 10 文字を超える長さのテキストを入力して、**[保存]** をクリックします。

    モバイル サービスから返された応答 400 (正しくない要求) の結果として、処理されない **MobileServiceInvalidOperationException** が発生します。

3.  MainPage.xaml.cs ファイルを開き、既存の **InsertTodoItem** メソッドを次のコードに置き換えます。
...
2.  Visual Studio で、**F5** キーを押してアプリケーションを実行し、テキスト ボックスに 10 文字未満のテキストを入力して、**[保存]** をクリックします。
...
サーバー スクリプトは、ユーザーを認証するときに、およびプッシュ通知の送信のためにも使用されます。詳細については、次のチュートリアルを参照してください。[ページングを使用したクエリの改善][ページングを使用したクエリの改善]。
...
<!-- URLs. -->
  ...
  [チュートリアルを見る]: http://go.microsoft.com/fwlink/?LinkId=298629
