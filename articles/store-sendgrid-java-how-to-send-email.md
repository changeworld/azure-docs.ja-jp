...
1. *javax.mail.Authenticator* クラスを拡張し、*getPasswordAuthentication* メソッドの実装で SendGrid のユーザー名とパスワードを返します。  
...
3. メッセージを作成し、**To**、**From**、**Subject**、およびコンテンツの値を設定します。このコードは「[方法: 電子メールを作成する](#bkmk_HowToCreateEmail)」に示しています。
4. *javax.mail.Transport* オブジェクトを使用してメッセージを送信します。このコードは、「[方法: 電子メールを送信する](How to: Send an Email.md)」セクションに示しています。
