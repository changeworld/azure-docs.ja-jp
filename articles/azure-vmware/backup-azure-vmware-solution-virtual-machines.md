...
この記事では、Azure Backup Server を使用して、Azure VMware Solution で実行されている VMware 仮想マシン (VM) をバックアップする手順について説明します。 始める前に、[Azure VMware Solution 用の Microsoft Azure Backup Server のセットアップ](set-up-backup-server-for-azure-vmware-solution.md)が完全に済んでいることを確認してください。
