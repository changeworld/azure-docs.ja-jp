<properties
   ...
   description="データベース、テーブル、接続、および SQL Data Warehouse のクエリの最大値です。"
...
| カテゴリ | 説明 | 最大値 |
| :---------------- | :------------------------------------------- | :----------------- |
| データベース | 最大サイズ | 60 TB 圧縮<br/><br/>SQL Data Warehouse ではデータベースあたりディスク上に最大 60 TB の raw 領域を確保できます。ディスク上の領域は、パーマネント テーブルの圧縮サイズです。この領域は tempdb またはログ領域から独立しています。そのため、この領域はパーマネント テーブル専用です。クラスター化列ストア圧縮は、5X であると推定されます。これは、すべてのテーブルがクラスター化列ストア (既定のテーブル型) である場合、データベースの非圧縮サイズが約 300 TB に拡大する可能性があることを意味します。60 TB の制限は、パブリック プレビューの最後には 240 TB まで増加します。これにより、ほとんどのデータベースを非圧縮データの 1 PB 以上に拡大させる必要があります。|
...
| ワークロード管理 | 同時クエリの最大数 | 32<br/><br/> SQL Data Warehouse には同時実行スロットと呼ばれる同時実行のユニットが 32 あります。<br/><br/>すべてのクエリが 1 つの同時実行スロットの既定のリソース割り当てで実行する場合は、32 の同時実行ユーザー クエリを持つことができます。実際には、最大の同時実行クエリは、サービス レベル目標 (SLO) と各クエリのリソース要件によって異なります。リソースが利用できない場合、クエリは内部キューで待機します。詳細については、「[SQL Data Warehouse での同時実行とワークロード管理][]」をご覧ください。|
| ワークロード管理 | サービス レベル目標 (SLO) あたりの最大の同時実行スロット数 |これは、各サービス レベルが追加の CPU およびメモリ リソースを必要とするクエリを実行するために使用できる同時実行スロットの数です。ワークロード管理のために、組み込みのリソース クラスを使用して、クエリの CPU およびメモリ リソースを増やすことができます。より多くのリソースで実行することは、クエリがより多くの同時実行スロット数を必要としていることを意味します。<br/><br/>一部のクエリは、リソース クラスでは実行しません。このようなクエリでは、1 つの同時実行ユニットを使用しますが、次に示す同時実行スロットのいずれも使用しません。SQL Data Warehouse がリソース クラス内で実行する (および実行しない) クエリの一覧については、「[Concurrency and workload management (同時実行とワークロード管理)][]」を参照してください。<br/><br/>DWU100 = 4<br/><br/>DWU200 = 8<br/><br/>DWU300 = 12<br/><br/>DWU400 = 16<br/><br/>DWU500 = 20<br/><br/>DWU600 = 24<br/><br/>DWU1000 = 40<br/><br/>DWU1200 = 48<br/><br/>DWU1500 = 60 |
...
[Concurrency and workload management (同時実行とワークロード管理)]: sql-data-warehouse-develop-concurrency.md
