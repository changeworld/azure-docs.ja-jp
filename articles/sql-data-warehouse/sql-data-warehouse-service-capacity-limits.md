...
| カテゴリ | 説明 | 最大値 |
| :---------------- | :------------------------------------------- | :----------------- |
| クエリ | ユーザー テーブルに対する同時クエリ数 | 32<br/><br/>これは、同時ユーザー クエリ実行の上限です。追加のクエリは内部キューに送られ、処理の順番がくるのを待機します。同時に実行されるクエリの数に関係なく、各クエリは、超並列処理アーキテクチャを最大限に活用するように最適化されます。注: 実際の同時実行は、データベース インスタンスの DWU および実行するクエリに対して選択されたリソース クラスに基づく追加調整の対象になります。|
...
| テーブル | 行あたりのバイト数 (定義されたサイズ) | 8060 バイト<br/><br/>行あたりのバイト数は、ページ圧縮をオンにして SQL Server の場合と同じ方法で算出されます。SQL Server のように、SQL Data Warehouse は、行外にプッシュされる可変長列を可能にする行オーバーフロー ストレージをサポートします。行外にプッシュされる可変長列の場合、メイン レコードには 24 バイト ルートのみが格納されます。詳細については、SQL Server オンライン ブックの「[8 KB を超える場合の行オーバーフロー データ](https://msdn.microsoft.com/library/ms186981.aspx)」を参照してください。<br/><br/>。SQL Data Warehouse のデータ型のサイズの一覧は、「[CREATE TABLE (Azure SQL Data Warehouse)](https://msdn.microsoft.com/library/mt203953.aspx)」を参照してください。 |
| テーブル | 行あたりのバイト数 (データ移動のための内部バッファーのサイズ) | 32,768<br/><br/>注: この制限はまもなくなくなります。<br/><br/>SQL Data Warehouse では、分散型の SQL Data Warehouse システム内で行を移動するとき、内部バッファーを使用します。行を移動するサービスはデータ移動サービス (DMS) と呼ばれており、SQL Server とは異なる形式で行を格納します。<br/><br/>行が内部バッファーに収まりきれない場合は、クエリ コンパイル エラーか、または内部データ移動エラーが返されます。この問題を回避するには、「[DMS バッファー サイズの詳細](#details-about-the-dms-buffer-size)」を参照してください。|
...
DMS では、並列クエリのパフォーマンスを向上させるために、SQL データベースで定義された最大サイズに達するまであらゆる可変長データを埋め込みます。たとえば、`nvarchar(2000) NOT NULL` の値 'hello' の場合、DMS バッファーで実際に使用されるのは 4002 バイトです。2,000 文字の各文字に対して 2 バイト、さらに NULL 終端文字に 2 バイトが使用されます。
