...
## ストレージの種類を確認する
次に示す日付より前に DW を作成した場合、現在は Standard Storage を使用していることになります。このほか、Standard Storage 上にある移行予定の各 Data Warehouse は、Data Warehouse ブレードの上部の[Azure ポータル][]に、次の通知が表示されます。"*An upcoming upgrade to premium storage will require an outage. (Premium Storage への今後のアップグレードには、サービスの停止が必要になります。)詳細情報 ->*"。
...
1.	ALTER DATABASE コマンドを使用して、"MyDW" を "MyDW\_BeforeMigration" のように末尾に何かを追加した名前に変更します。これにより、すべての既存のトランザクションが終了します。この操作を成功させるには、master データベース内で行う必要があります。
```
