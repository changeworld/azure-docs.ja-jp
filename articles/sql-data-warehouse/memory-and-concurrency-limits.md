...
Gen2 は、Gen 1 と比べると 1 つのクエリに対して 2.5 倍のメモリを提供します。 この追加メモリにより、Gen2 で高速なパフォーマンスが実現可能です。  Gen2 のパフォーマンス レベルは、DW100c から DW30000c の範囲となっています。 
...
> [!NOTE]
> Gen2 の smallrc リソース クラスでは、サービス レベルが上がるにつれてメモリが動的に追加されます。サポートされる同時実行クエリは、DW1000c で最大 32 個、DW100c で 4 個のみです。  インスタンスが DW1500c を超えて拡張されると、smallrc によって使用されるコンカレンシー スロットとメモリが増加するにつれて、サービス レベルが増加します。 
>
>

次の表は、[動的なリソース クラス](resource-classes-for-workload-management.md)ごとの最大のコンカレンシー クエリ数とコンカレンシー スロット数を示しています。 Gen1 とは異なり、Gen2 の動的なリソース クラスは真に動的です。  Gen2 は、すべてのサービス レベルにおいて small-medium-large-xlarge リソース クラスに対して 3-10-22-70 のメモリ割り当て率を使用します。
