...
## トランザクション サイズ
1 回のデータ変更トランザクションには規模の面で制限があります。現在、この制限は "ディストリビューションあたり" で適用されます。そのため、数値の合計を得るには、制限とディストリビューション数を掛け算する必要があります。トランザクションの最大行数の近似値を求めるために、ディストリビューション上限を各列の合計サイズで割り算します。列の長さが異なる場合、最大サイズではなく列の平均長を利用することを検討してください。
...
ログに書き込まれるデータ量を最適化/最小化する方法については、[トランザクション ベスト プラクティス][]に関する記事を参照してください。
...
## Error\_Line() 関数
SQL Data Warehouse では、ERROR\_LINE() 関数を実装およびサポートしていないことにも注意してください。この関数がコードに含まれている場合は、SQL Data Warehouse に準拠するために削除する必要があります。代わりに、コードでクエリ ラベルを使用して同等の機能を実装します。この機能の詳細については、[クエリ ラベル] に関する記事をご覧ください。
