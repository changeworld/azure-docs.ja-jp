<properties
   pageTitle="SQL Data Warehouse に Contoso Retail Data Warehouse を読み込む |Microsoft Azure"
   description="パフォーマンスを最大にするために PolyBase を使用して SQL Data Warehouse に Contoso Retail Data Warehouse を読み込む方法について説明します。"
...
# Contoso Retail Data Warehouse の読み込み

この記事では、Contoso Retail Data Warehouse スキーマに簡単にデータを読み込む方法について説明します。

このデータセットには、代わりのデータ ソースが用意されており、それを使用すると、Azure SQL Data Warehouse について理解を深めることができます。

Contoso データセットのサイズはほぼ GB 単位です。生データは Azure Blob Storage に保持されるため、ここに直接アクセスして SQL Data Warehouse に読み込みます。

このチュートリアルでは、次のことについて説明します。

1. パブリック Blob Storage コンテナーから読み込むように PolyBase を構成する
2. [CTAS][] を使用してデータをインポートする
3. 列ストア インデックスが最適化されるようにテーブルを[再構築][]する
4. キー列の統計を作成する

## PolyBase の構成

データを読み込む際は、まず、PolyBase の前提条件であるスクリプトを構成します。

### 外部データ ソースを作成する

データはパブリック コンテナーに保持されるため、最初に、外部データ ソースを作成します。
...
この場合、マスター キーまたはデータベース スコープ資格情報を作成する必要はありません。Blob Storage コンテナーは "パブリック" として構成されているためです (つまり、このコンテナーはだれでも読み取り、列挙できます)。

> [AZURE.IMPORTANT] Azure Blob Storage コンテナーをパブリックにした場合は、データがデータ センターから発信されるときにデータ送信料金が発生し、その支払い義務が生じることに注意してください。

### 外部ファイル形式のオブジェクトを作成する

このファイル形式では、Blob Storage に保持されているテキスト ファイルの解析に必要な情報を提供します。
...
この例では、データは圧縮されず、パイプで区切られます。

## 外部テーブルの作成

Azure Blob Storage への接続が設定されたら、外部テーブルを作成できます。

情報を整理しておくために、最初に、スキーマを作成しましょう。
...
次に、外部テーブルを作成します。ここでは、データ型のマッピングの指定しているだけであることに注意してください。この定義をメタデータにバインドしてファイルを見つけます。

警告が表示されますが、このスクリプトが長いからです。
...
データを読み込むには、先ほど作成した、厳密に型指定された外部テーブルを使用することをお勧めします。そのため、単純な [CTAS][] ステートメントで十分です。

ただし、最初に、テーブルを読み込むための新しいスキーマを作成しましょう。
...
データを読み込むために、[CTAS][] ステートメントを使用します。テーブルごとに [CTAS][] が 1 つあることに注意してください。
...
読み込みの進捗状況を追跡するには、`[sys].[dm_pdw_exec_requests]` 動的管理ビュー (DMV) を使用できます。上記の CTAS ステートメントには `label` が設定されているため、その `label` を使用して進捗状況を追跡することもできます。詳細については、[ラベル][]に関する記事を参照してください。

## インデックスの再構築

最後に、テーブルを圧縮列ストア形式にするには、テーブルに対して再構築操作を実行できます。これにより、開いている行グループまたは終了した行グループは圧縮列ストア形式に変換されます。
...
クラスター化列ストアの状態を確認する方法の詳細については、[列ストア インデックスの管理][]に関する記事を参照してください。

## 統計オブジェクトの作成

すべての列の統計を作成するには、[統計][]に関する記事で説明されているストアド プロシージャのコード `prc_sqldw_create_stats` を利用できます。

出力の一部を次に示します。これには、ディメンションすべてと、Contoso Retail Data Warehouse で結合するすべての列が含まれています。これは出発点として適しています。

後から、他のファクト列にさらに統計を追加することができます。

ここでも、このスクリプトは非常に長くなります。
...
これで、すべてのデータが Azure SQL Data Warehouse に読み込まれました。すばらしい結果です。

次のようなクエリを使用すると、テーブルを照会できるようになります。
...
## 次のステップ
読み込みの概要については、「[Load data into SQL Data Warehouse (SQL Data Warehouse へのデータの読み込み)][]」を参照してください。列ストア インデックスの処理の詳細については、[列ストア インデックスの管理][]に関するページを参照してください。
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/ja-JP/library/mt203887.aspx
[再構築]: https://msdn.microsoft.com/ja-JP/library/ms188388.aspx
