...
SQL Data Warehouse は、1 日を通してデータ ウェアハウスのスナップショットを取得し、7 日間利用できる復元ポイントを作成します。 この保持期間は変更できません。 SQL Data Warehouse では、8 時間の回復ポイントの目標 (RPO) がサポートされています。 プライマリ リージョンのデータ ウェアハウスを、過去 7 日間に作成されたいずれかのスナップショットから復元することができます。
...
データ ウェアハウスの一時停止中、SQL Data Warehouse ではスナップショットが作成されないため、復元ポイントの有効期限は切れません。 データ ウェアハウスが一時停止されている間、復元ポイントは変更されません。 復元ポイントの保持は、データ ウェアハウスがオンラインになっている日数に基づきます。カレンダーの日数ではありません。

たとえば、スナップショットが 10 月 1 日午後 4 時に開始され、データ ウェアハウスが 10 月 3 日午後 4 時に一時停止された場合、復元ポイントは最大 2 日間保持された状態です。 データ ウェアハウスがオンラインに戻ったとき、復元ポイントは 2 日前のものです。 データ ウェアハウスが 10 月 5 日午後 4 時にオンラインになった場合、復元ポイントは 2 日の状態であるため、あと 5 日間保持されます。

データ ウェアハウスがオンラインに戻ると、SQL Data Warehouse は新しい復元ポイントの作成を再開し、データが 7 日以上経過すると、復元ポイントの有効期限が切れます。
