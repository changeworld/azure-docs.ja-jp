...
サービス レベル目標 (SLO) は、データ ウェアハウスのコストとパフォーマンス レベルを決定するスケーラビリティ設定です。 コンピューティング データ ウェアハウス単位 (cDWU) では、コンピューティング用に最適化されたパフォーマンス レベル スケールが測定されます (例: DW2000c)。 弾力性用に最適化されたサービス レベルは、DWU 単位で計測されます (例: DW2000)。 
