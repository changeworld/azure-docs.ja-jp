...
## <a name="what-is-workload-management"></a>ワークロード管理について

ワークロード管理により、すべてのクエリの全体的なパフォーマンスを最適化することができます。 適切に調整されたワークロードは、クエリと読み込み操作を、コンピューティング集中型であるか IO 集約型であるかに関係なく、効率的に実行します。 SQL Data Warehouse は、マルチ ユーザー環境向けのワークロード管理機能を備えています。 データ ウェアハウスは、マルチテナント ワークロードを想定していません。

データ ウェアハウスのパフォーマンス容量は、[データ ウェアハウス ユニット](what-is-a-data-warehouse-unit-dwu-cdwu.md)によって決まります。

- すべてのパフォーマンス プロファイルのメモリおよびコンカレンシーの制限を確認するには、「[Memory and concurrency limits](memory-and-concurrency-limits.md)」 (メモリとコンカレンシーの制限) を参照してください。
- パフォーマンス能力を調整するには、[スケールアップまたはスケールダウン](quickstart-scale-compute-portal.md)してください。

クエリのパフォーマンス容量は、クエリのリソース クラスによって決定されます。 この記事の残りの部分では、リソース クラスとその調整方法について説明します。
...
### <a name="gen2-dynamic-resource-classes-are-truly-dynamic"></a>Gen2 動的リソース クラスは真の意味で動的
...
-CREATE または DROP TABLE -ALTER TABLE ...SWITCH、SPLIT、または MERGE PARTITION -ALTER INDEX DISABLE -DROP INDEX -CREATE、UPDATE、または DROP STATISTICS -TRUNCATE TABLE -ALTER AUTHORIZATION -CREATE LOGIN -CREATE、ALTER、または DROP USER -CREATE、ALTER、または DROP PROCEDURE -CREATE または DROP VIEW -INSERT VALUES -システム ビューおよび DMV からの SELECT -EXPLAIN -DBCC
...
[Gen1](#stored-procedure-definition-for-gen1) または [Gen2](#stored-procedure-definition-for-gen2) で次に指定されたストアド プロシージャを使用することで、特定の SLO でのリソース クラスあたりのコンカレンシー数とメモリ割り当て量、また特定のリソース クラスでの非分割 CCI テーブルに対するメモリ消費量の多い CCI 操作で最適なリソース クラスを割り出すことができます。
...
>
>たとえば、DW100 で利用可能な最大のメモリ許可が 400 MB であり、テーブル スキーマのサイズが 400 MB の要件を超える場合です。
...
### <a name="stored-procedure-definition-for-gen1"></a>Gen1 のストアド プロシージャの定義
...
### <a name="stored-procedure-definition-for-gen2"></a>Gen2 のストアド プロシージャの定義
