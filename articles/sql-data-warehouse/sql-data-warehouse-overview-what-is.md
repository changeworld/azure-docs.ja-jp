...
- **制御ノード:** SQL Data Warehouse を任意の開発ツール、読み込みツール、またはビジネス インテリジェンス ツールと一緒に使用する場合は、制御ノードに接続します。SQL Data Warehouse のコンピューティング ノードは SQL Database です。接続時は、標準の SQL Database のような外観と操作性となります。ただし、システム内で発生するデータの移動とコンピューティングはすべてバックグラウンドで調整されます。コマンドが制御ノードに発行されると、コマンドは一連のクエリに分解され、サービスのコンピューティング ノードに渡されます。

- **コンピューティング ノード:** 制御ノードの場合と同様に、SQL Data Warehouse のコンピューティング ノードは、SQL Databases を利用します。コンピューティング ノードの仕事は、サービスのコンピューティング機能としての役割を果たすことです。バックグラウンドでは、データが SQL Data Warehouse に読み込まれると常に、サービスの当該ノード間に分散されます。制御ノードはコマンドを受信すると常に、各コンピューティング ノード用に分割し、コンピューティング ノードは対応するデータを処理します。各コンピューティング ノードは計算を完了すると、担当部分の結果を制御ノードに渡します。制御ノードは結果を集計してから答えを返します。

- **Storage:** SQL Data Warehouse のストレージはすべて、標準の Azure Storage BLOB です。すなわち、コンピューティング ノードはデータ操作時に、BLOB との間で書き込みと読み取りを直接行います。透過的に、ほぼ無制限に拡張可能な Azure Storage の機能を使用すると、ストレージを自動的に拡大縮小することができます。これはコンピューティングとは別に行うことができます。また、Azure Storage では、スケーリングまたは一時停止の間もストレージを保持し、バックアップおよび復元のプロセスを合理化し、安全でよりフォールト トレラントなストレージを使用することができます。
