...
Azure SQL Data Warehouse はエンタープライズ クラスの分散データベースであり、ペタバイト単位の量までリレーショナル データと非リレーショナル データを処理できます。業界初のクラウド データ ウェアハウスであり、実証済みの SQL の機能を使用して、数秒で拡大、縮小、および一時停止できます。
...
SQL Data Warehouse では、それぞれを個別にスケーリングできる、ストレージとコンピューティングの分離を導入しています。クラウド規模のストレージと Azure のコンピューティングを使用する SQL Data Warehouse では、必要なときに必要とするクエリのパフォーマンスに対して支払いを行うことができます。コンピューティングの動的なデプロイ、拡張、縮小、さらには一時停止も数秒で実行でき、ビジネス ニーズに関する洞察を購入できます。コンピューティングの量の変更は、スライダーを左から右に動かすような単純な操作です。

SQL Data Warehouse では、ストレージとは無関係にコンピューティングの量を完全に制御できる機能を使用して、コンピューティングを一時停止できます。動作可能な状態を維持したまま、コンピューティングを中央プールに置くことで、コストを節約できます。必要になったときに、コンピューティングを再開するだけで、ワークロードでデータとコンピューティングを利用できます。
