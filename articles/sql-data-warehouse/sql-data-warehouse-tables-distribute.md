...
すべての傾斜に修正プログラムが対応しているわけではありません。場合によっては、一部のクエリのテーブルのパフォーマンスが、データ傾斜の問題を超える可能性があります。テーブルでデータ傾斜を解決する必要があるかを決定するには、ワークロード内のデータ ボリュームとクエリについて可能な限り理解する必要があります。傾斜の影響を確認する方法の 1 つに、[クエリ監視][]に関する記事の手順を使用して、クエリのパフォーマンス、特に個々のディストリビューションでのクエリの完了時間に対する影響を監視する方法があります。
