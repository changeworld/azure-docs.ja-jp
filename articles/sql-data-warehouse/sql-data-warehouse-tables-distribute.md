...
description: "Azure SQL Data Warehouse のハッシュ分散テーブルおよびラウンド ロビン テーブルを設計するための推奨事項。"
...
この記事では、Azure SQL Data Warehouse で分散テーブルを設計するための推奨事項を示します。 ハッシュ分散テーブルは、この記事で取り上げる主題であり、大規模なファクト テーブルでのクエリ パフォーマンスを向上させます。 ラウンド ロビン テーブルは、読み込み速度の向上に役立ちます。 これらの設計の選択は、クエリおよび読み込みパフォーマンスの向上に大きな影響を与えます。
...
ハッシュ分散テーブルは、スター スキーマにある大規模なファクト テーブルを適切に操作します。 非常に大規模数の行を格納し、その上で高度なパフォーマンスを実現できます。 もちろん、パフォーマンスを得るために役立つ設計上の考慮事項の一部は、分散システムが提供するように設計されています。 適切なディストリビューション列を選択することは、この記事で説明する考慮事項の 1 つです。 
...
[Azure Storage BLOB からのデータの読み込み](load-data-from-azure-blob-storage-using-polybase.md#load-the-data-into-your-data-warehouse)に関するチュートリアルでは、ラウンド ロビン ステージング テーブルにデータを読み込む例を示しています。
...
この列の値によって行の分散方法が決まるため、ディストリビューション列の選択は、設計上の重要な決定事項です。 最適な選択肢は複数の要因によって決まり、多くの場合、トレードオフが生じます。 ただし、最初に最適な列を選択しなかった場合は、[CREATE TABLE AS SELECT (CTAS)](https://docs.microsoft.com/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) を使って別のディストリビューション列でテーブルを再作成できます。 
...
最適なパフォーマンスを得るために、すべてのディストリビューションでほぼ同じ行数を含むようにする必要があります。 1 つまたは複数のディストリビューションに含まれる行数が不均衡な場合、一部のディストリビューションでは、同時実行クエリの一部分を他の部分よりも先に終わらせます。 クエリは、すべてのディストリビューションで処理が終了するまで完了できないので、各クエリは単に最も処理が遅いディストリビューションと同じ速度になります。

- データ スキューとは、複数のディストリビューションにわたってデータが均等に分散されていないことを意味します。
- 処理の傾斜とは、クエリの並列実行を行うときに、一部のディストリビューションで他よりも処理が長引くことを意味します。 これは、データが傾斜している場合に発生する可能性があります。
...
- **NULL がない、または少ししか NULL がない。** 極端な例として、列のすべての値が NULL の場合、すべての行が同じディストリビューションに割り当てられます。 その結果、クエリ処理は 1 つのディストリビューションに傾斜され、並列処理のメリットはなくなります。 
...
- `WHERE` 句で使用*されていない*。 これにより、すべてのディストリビューションでは実行されないクエリを絞り込むことができます。 
- データ列*ではない*。 WHERE 句は多くの場合、日付別にフィルター処理されます。  この場合、すべての処理が、少数のディストリビューションのみで実行される可能性があります。
...
データ スキューをチェックする簡単な方法として、[DBCC PDW_SHOWSPACEUSED](https://docs.microsoft.com/sql/t-sql/database-console-commands/dbcc-pdw-showspaceused-transact-sql) を使用できます。 次の SQL コードは、60 の各ディストリビューションに格納されているテーブル行の数を返します。 バランスの取れたパフォーマンスを実現するには、分散テーブルの行をディストリビューション全体で均等に広げる必要があることに注意してください。
...
テーブルでデータ傾斜を解決する必要があるかを決定するには、ワークロード内のデータ ボリュームとクエリについて可能な限り理解する必要があります。 [クエリの監視](sql-data-warehouse-manage-monitor.md)に関する記事に記載された手順を使用して、クエリ パフォーマンスの傾斜の影響を監視できます。 具体的には、個々のディストリビューションで大規模なクエリの完了にかかる時間を探索します。
...
この例では、[CREATE TABLE AS SELECT](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse.md) を使用して、他のハッシュ ディストリビューション列を含むテーブルを再作成します。
