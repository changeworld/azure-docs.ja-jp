...
1.  [SharePoint への委任アクセスのためにアプリケーションを登録する][SharePoint への委任アクセスのためにアプリケーションを登録する]
2.  [モバイル サービスに SharePoint 情報を追加する][モバイル サービスに SharePoint 情報を追加する]
3.  [アクセス トークンを取得し、SharePoint API を呼び出す][アクセス トークンを取得し、SharePoint API を呼び出す]
4.  [Word 文書を作成し、アップロードする][Word 文書を作成し、アップロードする]
5.  [アプリケーションをテストする][アプリケーションをテストする]

このチュートリアルには、次のものが必要です。

-   Windows 8.1 で実行されている Visual Studio 2013
-   有効な [SharePoint Online][SharePoint Online] サブスクリプション
-   「[Active Directory 認証ライブラリのシングル サインオンによるアプリケーションの認証][Active Directory 認証ライブラリのシングル サインオンによるアプリケーションの認証]」チュートリアルの完了。SharePoint サブスクリプションによって提供されるテナントを使用する必要があります。
...
2.  ここで、次のようにユーザーのドキュメント ライブラリにファイルを追加するメソッドを作成します。
