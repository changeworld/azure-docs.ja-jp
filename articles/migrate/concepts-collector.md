...
#### <a name="internet-connectivity-with-intercepting-proxy"></a>インターセプト プロキシを使用したインターネット接続

インターネットへの接続に使用しているプロキシ サーバーがインターセプト プロキシの場合、コレクター VM にプロキシ証明書をインポートする必要があります。 コレクター VM に証明書をインポートする方法については、手順を次に従います。

1. コレクター VM で **[スタート メニュー]** へ移動し、**[コンピューター証明書の管理]** を探して開きます。
2. 左側のウィンドウにある証明書ツールの **[証明書 - ローカル コンピューター]** で、**[信頼された発行元]** を探します。 **[信頼された発行元]** で **[証明書]** をクリックして、右側のウィンドウに証明書の一覧を表示します。

    ![証明書ツール](./media/concepts-intercepting-proxy/certificates-tool.png)

3. コレクター VM にプロキシ証明書をコピーします。 この証明書を取得するために、必要に応じて、組織内のネットワーク管理者チームに連絡します。
4. 証明書をダブルクリックして、開きます。 **[証明書のインストール]** をクリックします。 これにより、証明書インポート ウィザードが表示されます。
5. 証明書インポート ウィザードで、保存場所として **[ローカル マシン]** を選択します。 **[次へ]** をクリックします。

    ![証明書ストアの場所](./media/concepts-intercepting-proxy/certificate-store-location.png)

6. **[証明書をすべて次のストアに配置する]** オプションを選択します。 **[参照]** をクリックして、表示された証明書の一覧から **[信頼された発行元]** を選択します。 **[次へ]** をクリックします。

    ![証明書ストア](./media/concepts-intercepting-proxy/certificate-store.png)
    
7. **[完了]** をクリックします。 これで、証明書がインポートされます。 
8. 必要に応じて、上述した手順 1 および 2 に従って証明書ツールを開き、証明書がインポートされていることを確認できます。
9. Azure Migrate コレクター アプリで、インターネット接続の前提条件のチェックに合格していることを確認します。
...
1. 最新の[アップグレード パッケージ](https://aka.ms/migrate/col/upgrade_9_14) (バージョン 1.0.9.14) をダウンロードします。
...
#### <a name="upgrade-to-version-10914"></a>バージョン 1.0.9.14 へのアップグレード
