...
- アクセスを制限するには、子オブジェクトへの伝達特権を持つアクセスなしロールを、子オブジェクト (vSphere ホスト、データストア、VM、ネットワーク) に割り当てます。
...
- vCenter Server の統計情報設定が、レベル 3 に設定されていません。 1 回限りの検出のモデルは vCenter Server の統計設定に依存しているため、vCenter Server での統計設定がレベル 3 未満だと、ディスクとネットワークのパフォーマンス データが vCenter Server から収集されません。 この場合、ディスクとネットワークに関して Azure Migrate から提示される推奨情報は、使用率ベースではありません。 ディスクの IOPS/スループットを考慮しないと、Azure Migrateは、Azure でプレミアム ディスクが必要であるかどうかを判断できないため、この場合、Azure Migrate はすべてのディスクに Standard ディスクを推奨します。
...
- 評価を作成しようとしている期間中、環境をプロファイルしませんでした。 たとえば、期間を 1 日に設定してパフォーマンスの評価を作成する場合は、すべてのデータ ポイントが収集されるのを、検出開始後少なくとも 1 日待つ必要があります。

**一般的理由**  

- 評価の計算対象である期間中に、いくつかの VM がシャットダウンされました。 しばらくの間電源がオフになっていた VM があった場合、その期間のパフォーマンス データは収集できなくなります。
...
> [!NOTE]
> 1 回限りの検出のモデルで、評価の信頼度レーティングが 4 つ星未満の場合は、vCenter Server の統計設定レベルを 3 に変更し、評価の考慮に入れる期間 (1 日/1 週間/1 か月) 待機してから検出と評価を行うことをお勧めします。 継続的な検出のモデルの場合、アプライアンスが環境をプロファイルするのを少なくとも 1 日待ってから、評価の *[Recalculate](再計算)* を実行します。 そのようにできない場合は、パフォーマンス ベースのサイズ設定を信頼できない可能性が生じるため、評価のプロパティを変更して、*オンプレミスと同様のサイズ設定*に切り替えることをお勧めします。
