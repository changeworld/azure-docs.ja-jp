...
VM のパフォーマンス ベースのサイズ変更の場合、Azure Migrate には CPU とメモリの使用率データが必要です。 また、VM に接続されている各ディスクについて、読み取り/書き込みの IOPS とスループットが必要です。 同様に、パフォーマンス ベースのサイズ変更を行うために、VM に接続されている各ネットワーク アダプターについても、ネットワークの入出力が必要です。 上記の使用率の数値のいずれかが vCenter Server にない場合、Azure Migrate による推奨サイズは信頼できないことがあります。 次のように、使用可能なデータ ポイントの割合に応じて、評価の信頼度レーティングが決まります。
...
- vCenter Server の統計設定がレベル 3 に設定されておらず、評価でサイズ変更の設定基準がパフォーマンス ベースになっています。 vCenter Server の統計設定がレベル 3 未満である場合、ディスクおよびネットワークのパフォーマンス データが vCenter Server から収集されません。 この場合、ディスクとネットワークに関して Azure Migrate から提示される推奨情報は、使用率ベースではありません。 ストレージについては、Azure Migrate は Standard ディスクを推奨します。ディスクの IOPS/スループットを考慮しなければ、Azure のプレミアム ディスクが必要であるかどうかを Azure Migrate が判断できないためです。
