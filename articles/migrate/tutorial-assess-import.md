...
**Total disks read ops (ディスク読み取り操作の合計)** | いいえ | すべてのディスクについての 1 秒あたりの読み取り操作数を結合したもの。 <br/> ディスクレベルのデータを提供できない場合は、このフィールドを使用します。 
**Total disks write ops (ディスク書き込み操作の合計)** | いいえ | すべてのディスクについての 1 秒あたりの書き込み操作数を結合したもの。 <br/> ディスクレベルのデータを提供できない場合は、このフィールドを使用します。
**Total disks read throughput (ディスク読み取りスループットの合計)** | いいえ | すべてのディスクから読み取られたデータ (MB/ 秒)。 <br/> ディスクレベルのデータを提供できない場合は、このフィールドを使用します。 
**Total disks write throughput (ディスク書き込みスループットの合計)** | いいえ | すべてのディスクに書き込まれたデータ (MB/ 秒)。 <br/> ディスクレベルのデータを提供できない場合は、このフィールドを使用します。
...
ディスク レベルのデータを提供できない場合は、次のフィールドを使用して、サーバーごとにディスクのパフォーマンス データを提供できます。 各フィールドの詳細については、[こちらの](#add-server-information)セクションを参照してください。
- Total disks read ops (ディスク読み取り操作の合計)
- Total disks write ops (ディスク書き込み操作の合計)
- Total disks read throughput (ディスク読み取りスループットの合計)
- Total disks write throughput (ディスク書き込みスループットの合計)
...
CSV で指定するオペレーティング システムの名前は、この一覧に記載されている名前と一致しているか、それらを含んでいる必要があります。 このことは、指定した名前が評価で有効と認識されるための要件です。
