---
title: Azure SQL の評価を作成する
description: Azure SQL Managed Instance および Azure SQL Database に移行するために、SQL インスタンスを評価する方法について説明します
author: rashi-ms
ms.author: rajosh
ms.topic: tutorial
ms.date: 02/07/2021
...
- 評価を作成するには、[VMware](how-to-set-up-appliance-vmware.md) 用の Azure Migrate アプライアンスを設定する必要があります。 このアプライアンスによって、オンプレミスのサーバーが検出され、メタデータとパフォーマンス データが Azure Migrate に送信されます。 [詳細情報](migrate-appliance.md)

## <a name="azure-sql-assessment-overview"></a>Azure SQL の評価の概要
サイズ変更の設定基準として **パフォーマンスベース** を使用して、Azure SQL の評価を作成できます。 

**サイズ変更の設定基準** | **詳細** | **データ**
--- | --- | ---
**パフォーマンスベース** | 収集されたパフォーマンス データに基づく評価 | 推奨される **Azure SQL 構成** は、SQL インスタンスおよびデータベースのパフォーマンス データ (CPU 使用率、コア数、データベース ファイルの構成とサイズ、ファイル IO、1 秒あたりのバッチ クエリ数、データベースごとのメモリのサイズと使用量など) に基づいています。

Azure SQL の評価の[詳細](concepts-azure-sql-assessment-calculation.md)をご覧ください。
...
信頼度レーティングを使うと、評価の推奨サイズの信頼性を見積もることができます。  信頼度レーティングは次のとおりです。
