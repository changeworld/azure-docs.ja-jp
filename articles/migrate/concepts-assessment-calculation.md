...
VM のサイズ変更のパフォーマンス履歴を検討せずに、VM をそのまま Azure に移行する場合は、サイズ設定基準を *[オンプレミス]* と指定すると、Azure Migrate は使用状況データを考慮せずにオンプレミス構成に基づいて VM のサイズを設定します。 この場合でも、ディスクのサイズ変更はパフォーマンス データに基づきます。
...
サイズ設定基準が *オンプレミスと同じサイズ* の場合、Azure Migrate は VM のパフォーマンス履歴を考慮せず、オンプレミスで割り当てられたサイズに基づいて VM を割り当てます。 ただし、ディスクのサイズ変更の場合、Standard または Premium ディスクの推奨にディスクのパフォーマンス履歴は考慮されます。  
- **ストレージ**: Azure Migrate は、マシンに接続されているすべてのディスクを Azure のディスクにマップします。
...
- **コンピューティング**: Azure Migrate は、オンプレミス VM のコア数とメモリ サイズを調べて、同じ構成の Azure VM を推奨します。 条件を満たす Azure VM サイズが複数ある場合、最も低コストのディスクをお勧めします。 CPU とメモリの使用率データは、オンプレミスのサイズ変更では考慮されません。
