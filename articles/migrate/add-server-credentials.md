---
title: サーバーの資格情報を提供して、アプリケーション、依存関係、SQL Server インスタンスとデータベースを検出する
...
ms.locfileid: "102054226"
---
# <a name="provide-server-credentials-to-discover-applications-dependencies-and-sql-server-instances-and-databases"></a>サーバーの資格情報を提供して、アプリケーション、依存関係、SQL Server インスタンスとデータベースを検出する
...
[Azure Migrate アプライアンス](migrate-appliance.md)は、VMware 環境で実行されているオンプレミスのサーバーを検出して、サーバー構成とパフォーマンス メタデータを Azure に送信するために Azure Migrate:Server Assessment によって使用される軽量のアプライアンスです。 また、このアプライアンスは、ソフトウェア インベントリ、エージェントレスの依存関係分析、SQL Server インスタンスとデータベースの検出にも使用されます。
