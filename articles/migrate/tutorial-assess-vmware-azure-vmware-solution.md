...
6. **[すべて表示]** をクリックして、評価のプロパティを確認します。
    ...
    ![評価の設定を選択するためのページ](./media/tutorial-assess-vmware-azure-vmware-solution/assess-servers.png)


7. **[評価のプロパティ]**  >  **[ターゲット プロパティ]** で、次の操作を行います。
...
8. **[VM サイズ]** では:
    - **[ノードの種類]** で、オンプレミス VM で実行しているワークロードに基づいてノードの種類を選択します。
        ...
        - 既定のノードの種類は AV36 です。
    - **[FTT setting, RAID level]\(FTT 設定、RAID レベル\)** で、許容するエラーと RAID の組み合わせを選択します。  選択した FTT オプションとオンプレミスの VM ディスク要件を組み合わせて、AVS で必要とされる vSAN ストレージの合計が決定されます。
...
11. 変更内容を確定する場合は **[保存]** をクリックします。

    ![評価のプロパティ](./media/tutorial-assess-vmware-azure-vmware-solution/view-all.png)

12. **[サーバーの評価]** で **[次へ]** をクリックします。
13. **[サーバーの評価]**  >  **[評価するマシンの選択]** で、評価対象サーバーのグループを新規作成します。 **[新規作成]** を選択し、グループ名を指定してください。 
14. アプライアンスを選択し、グループに追加したい VM を選択します。 続けて、 **[次へ]** をクリックします。
15. **[評価の確認と作成]** で評価の詳細を確認したら、 **[評価の作成]** をクリックしてグループを作成し、評価を実行します。
