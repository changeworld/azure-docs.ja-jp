...
1. **[サーバーの評価]**  >  **[サーバーの評価]** で、**Azure VMware Solution (AVS) プレビュー** を選択します。
...
1. **[評価のプロパティ]**  >  **[ターゲット プロパティ]** で:
    ...
    - 移行先となる Azure リージョンを **[ターゲットの場所]** で指定します。
       ...
       - 現在、4 つのリージョン (オーストラリア東部、米国東部、西ヨーロッパ、米国西部) を評価の対象にすることができます。
...
- すべての AVS ノードの使用率: すべてのノードにおける CPU、メモリ、および記憶域の使用率の予測。
    ...
    - メモリ、重複除去、圧縮では、現在、メモリの使用率が 100% に設定され、重複除去と圧縮は 1.5 に設定されています。今後のリリースでは、これらはユーザー定義の入力になり、ユーザーは必要なサイズに微調整できます。
