<properties title="Storm と HDInsight を使用したセンサー データの分析" pageTitle="Apache Storm と Microsoft Azure HDInsight (Hadoop) を使用したセンサー データの分析" description="HDInsight (Hadoop) でリアルタイムにセンサー データを処理するために Apache Storm を使用する方法を説明します。" metaKeywords="Azure hdinsight storm, Azure hdinsight realtime, azure hadoop storm, azure hadoop realtime, azure hadoop real-time, azure hdinsight real-time" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" />
...
-   Visual Studio と [Azure SDK for .NET][Azure SDK for .NET]

-   [Java と JDK][Java と JDK]

-   [Maven][Maven]

-   [Git][Git]

> [AZURE.NOTE] Java、JDK、Maven、および Git は、[Chocolatey NuGet][Chocolatey NuGet] パッケージ マネージャーからも入手できます。

## ダッシュボードの作成

ダッシュボードは、ほぼリアルタイムのセンサー情報を表示するために使用されます。ここでは、ダッシュボードは Azure Web サイトにホストされている ASP.NET アプリケーションを意味します。アプリケーションの主目的は、メッセージが処理されるときに Storm トポロジから情報を受信する [SignalR][SignalR] のハブとして機能することです。
...
4.  ソリューション エクスプローラーで、プロジェクトを右クリックし、**[追加]**、**[SignalR Hub クラス (v2)]** の順に選択します。クラスに「**DashHub.cs**」という名前を付けて、プロジェクトに追加します。これには、HDInsight とダッシュボード Web ページ間のデータ通信に使用される SignalR ハブが含まれます。
...
6.  ソリューション エクスプローラーで、プロジェクトを右クリックし、**[追加]**、**[OWIN Startup クラス]** の順に選択します。新しいクラスに「**Startup.cs**」という名前を付けます。

    > [AZURE.NOTE] Visual Studio 2012 を使用している場合、**OWIN Startup Class** テンプレートは利用できません。Startup という**クラス**を代わりに作成できます。
...
8.  ソリューション エクスプローラーで、プロジェクトを右クリックし、**[追加]**、**[HTML ページ]** の順にクリックします。新しいページに「**index.html**」という名前を付けます。このページには、このプロジェクトのリアルタイム ダッシュボードが表示されます。DashHub から情報を受信して、D3.js を使用したグラフを表示します。
...
11. ソリューション エクスプローラーで、プロジェクトを右クリックし、**[追加]**、**[HTML ページ]** の順にクリックします。新しいページに「**test.html**」という名前を付けます。このページは、メッセージの送受信による DashHub とダッシュボードのテストに使用できます。
...
1.  [Azure ポータル][Azure ポータル]から、**[新規作成]、[Service Bus]、[Event Hub]、[カスタム作成]** の順に選択します。

2.  **[新しい Event Hub の追加]** ダイアログで **Event Hub 名**を入力し、ハブを作成する **[リージョン]** を選択して、新しい名前空間を作成するか、既存の名前空間を選択します。最後に、**矢印**をクリックします。

3.  **[Event Hub の構成]** ダイアログで、**パーティション カウント**と**メッセージ保持**の値を入力します。この例では、パーティション カウントに 10 を、メッセージ保持に 1 を使用します。
...
5.  **Program.cs** ファイルで、ファイルの先頭にある次の変数の値を Azure の管理ポータルで Event Hub から取得した対応する値に設定します。
    ...
    | 設定する変数           | 設定する値                                                                            |
    |------------------------|---------------------------------------------------------------------------------------|
    | eventHubName           | イベント ハブの名前。たとえば、「**温度**」のように入力します。                       |
    | eventHubNamespace      | イベント ハブの名前空間。たとえば、「**sensors-ns**」のように入力します。             |
    | sharedAccessPolicyName | Send アクセス許可を使用して作成したポリシー。たとえば、「**デバイス**」と入力します。 |
    | sharedAccessPolicyKey  | Send アクセス許可を使用して作成したポリシーのキー。                                   |

6.  ソリューション エクスプローラーで **SendEvents** を右クリックし、**[追加]**、**[クラス]** の順にクリックします。新しいクラスに「**Event.cs**」という名前を付けます。これは、Event Hub に送信されるメッセージを表します。
...
1.  リモート デスクトップを使用して Storm クラスターに接続し、**%STORM\_HOME%\\examples\\eventhubspout\\eventhubs-storm-spout-0.9-jar-with-dependencies.jar** ファイルをローカル開発環境にコピーします。これには **events-storm-spout** が含まれています。
...
ASP.NET ダッシュボードにメッセージを送信するには、[SignalR client SDK for Java (SignalR Java クライアント SDK)][SignalR client SDK for Java (SignalR Java クライアント SDK)] を使用します。
...
**eventhubs-storm-spout** は構成情報を **Config.properties** ファイルから読み込みます。これは、接続先の Event Hub を指定します。クラスターでトポロジを開始するときに構成ファイルを指定できますが、プロジェクトに構成ファイルを 1 つを含めておくと、既知の既定の構成を使用できます。
...
-   SignalR の詳細については、「[ASP.NET SignalR][ASP.NET SignalR]」を参照してください。
