...
コンフィデンシャル コンピューティングは、コンフィデンシャル コンピューティングを定義し、その導入を促進することに特化した団体である [Confidential Computing Consortium](https://confidentialcomputing.io/) (CCC) によって定義された業界用語です。 コンフィデンシャル コンピューティングとは、計算の実行時に使用されているデータを保護する処理です。 計算は、ハードウェアベースの信頼できる実行環境 (Trusted Execution Environment: TEE) で行われます。

TEE は、承認されたコードのみが実行されるよう強制する環境です。 TEE 内のデータを、その環境外のコードで読み取ったり改ざんしたりすることはできません。

### <a name="enclaves-and-trusted-execution-environments"></a>エンクレーブと信頼できる実行環境

コンフィデンシャル コンピューティングのコンテキストにおける TEE は、一般に "*エンクレーブ*" や "*セキュア エンクレーブ*" と呼ばれます。 エンクレーブは、ハードウェアのプロセッサやメモリの、セキュリティで保護された部分です。 デバッガーを使用しても、エンクレーブ内のデータやコードを表示することはできません。 信頼されていないコードがエンクレーブ メモリの内容を変更しようとすると、環境が無効になって操作が拒否されます。
...
**ホスト**は、信頼されていない環境で実行されているエンクレーブ アプリケーションです。 ホスト内のコードは、エンクレーブに読み込まれたコードにアクセスできません。 

**エンクレーブ**は、TEE の実装内でコードとデータが実行される場所です。 シークレットと機密データを常に保護された状態にするために、セキュアな計算はエンクレーブ内で実行する必要があります。 

エンクレーブ アプリケーションの開発を始める際は、保護を必要とするコードとデータを特定しなければなりません。 信頼されたコンポーネントに配置したコードは、アプリケーションの他の部分から隔離されます。 エンクレーブが初期化されてコードがメモリに読み込まれた後は、保護環境の外から、そのコードを読み取ったり変更を加えたりすることはできなくなります。
