...
ms.locfileid: "93308343"
---
# <a name="automatic-registration-with-sql-vm-resource-provider"></a>SQL VM リソース プロバイダーへの自動登録
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

Azure portal で自動登録機能を有効にすると、Azure Virtual Machines (VM) 上の既存および今後のすべての SQL Server は軽量モードの SQL VM リソース プロバイダーに自動的に登録されます。 SQL VM リソース プロバイダーに登録すると、[SQL IaaS Agent 拡張機能](sql-server-iaas-agent-extension-automate-management.md)がインストールされます。
...
SQL VM リソース プロバイダーに SQL Server VM を登録すると、[SQL IaaS Agent 拡張機能](sql-server-iaas-agent-extension-automate-management.md)がインストールされます。 

自動登録が有効になっている場合、サブスクリプション内のすべての未登録の VM に SQL Server がインストールされているかどうかを検出するため、ジョブが毎日実行されます。 これを行うには、SQL IaaS Agent 拡張機能バイナリを VM にコピーし、SQL Server レジストリ ハイブをチェックする 1 回限りのユーティリティを実行します。 SQL Server ハイブが検出された場合、仮想マシンは軽量モードで [SQL VM リソース プロバイダー](sql-vm-resource-provider-register.md)に登録されます。 レジストリに SQL Server ハイブが存在しない場合は、バイナリが削除されます。

サブスクリプションに対して自動登録を有効にすると、SQL Server がすでにインストールされている、または今後インストールされる VM が、軽量モードで SQL VM リソース プロバイダーに登録されます。 完全な機能セットを使用するには、[完全管理モードに手動でアップグレードする](sql-vm-resource-provider-register.md#upgrade-to-full)必要があります。 
