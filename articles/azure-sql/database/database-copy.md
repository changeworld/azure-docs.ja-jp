...
> [!NOTE]
> Azure SQL Database の構成可能なバックアップ ストレージの冗長性は、現在、東南アジアの Azure リージョンでのみ一般提供されています。 プレビューでは、ソース データベースがローカル冗長またはゾーン冗長のバックアップ ストレージの冗長性を使用して作成されている場合、別の Azure リージョンのサーバーにデータベースをコピーすることはできません。 
