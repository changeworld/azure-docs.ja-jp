...
- **自動フェールオーバー ポリシー**
  ...
  既定では、フェールオーバー グループは自動フェールオーバー ポリシーを使用して構成されます。 システムはエラーが検出され、猶予期間が終了すると、フェールオーバーをトリガーします。 システムでは、影響の規模が原因で、組み込みの[高可用性インフラストラクチャ](high-availability-sla.md)によって機能停止を軽減できないかどうかを確認する必要があります。 アプリケーションから、または手動でフェールオーバー ワークフローを制御するには、自動フェールオーバーをオフにします。
