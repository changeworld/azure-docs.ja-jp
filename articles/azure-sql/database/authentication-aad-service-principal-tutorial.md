...
1. こちらのガイドに従って[アプリを登録し、アクセス許可を設定](active-directory-interactive-connect-azure-sql-db.md#register-your-app-and-set-permissions)します。
    ...
    :::image type="content" source="media/authentication-aad-service-principals-tutorial/aad-apps.png" alt-text="AAD アプリ":::
...
2. サインインに使用するクライアント シークレットも作成する必要があります。 こちらのガイドに従って、[証明書をアップロードするか、サインイン用のシークレットを作成](../../active-directory/develop/howto-create-service-principal-portal.md#upload-a-certificate-or-create-a-secret-for-signing-in)してください。
...
1. 以下のスクリプトとサービス プリンシパル *AppSP* を使用して、Azure AD サービス プリンシパル ユーザー *myapp* を作成します。
    ...
    > [!NOTE]
    > モジュール AzureRM.profile をインストールする必要がある場合は、PowerShell を管理者として開く必要があります。 次のコマンドを使用すると、最新バージョンの AzureRM.profile を自動的にインストールして、上記スクリプトの `$adalpath` を設定できます。
    > 
    > ```powershell
