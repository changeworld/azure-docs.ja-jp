...
RDP および SSH は、Azure で実行されているワークロードに接続できる、基本的な手段の一部です。インターネット経由で RDP ポートまたは SSH ポートを公開することは望ましくなく、重大な脅威面と見なされます。これは、プロトコルの脆弱性が原因であることがよくあります。この脅威面を含めるには、境界ネットワークの公開される側に要塞ホスト (踏み台サーバーとも呼ばれます) をデプロイできます。要塞ホスト サーバーは、攻撃に耐えられるように設計および構成されています。また、要塞サーバーは、要塞の背後やネットワークの奥にあるワークロードに対する RDP および SSH 接続も提供しています。
