<properties 
	pageTitle="仮想マシン上で多くのコンピューティング処理を要する .NET タスク - Azure" 
	description="Azure の仮想マシンに多くのコンピューティング処理を要する .NET アプリケーションをデプロイして実行する方法と、Service Bus キューを使用して進捗をリモート監視する方法について説明します。" 
...
5.  入力した名前が利用できることを確認できたら、名前空間をホストするリージョンを選択します (仮想マシンをホストするリージョンと同じリージョンを必ず使用してください)。
    ...
    > [AZURE.IMPORTANT] 仮想マシンが使用しているリージョンまたは使用する予定のリージョンと**同じリージョン**を選択してください。そうすることで、パフォーマンスが最高になります。
...
5. 次のコマンドを実行します。
    ...
    クライアントは、"Complete" というキュー メッセージが見つかるまで実行を続けます。注意点として、クライアントを実行しないで、問題を解くプログラムを複数実行した場合、キューを完全に空にするにはクライアントの複数回実行が必要になる場合があります。別の方法として、キューを削除して再び作成することもできます。キューを削除するには、次の **TSPSolver** を実行します (**TSPClient** ではありません)。  。
...
TSPSolver でキューの作成や削除をする代わりに、[Azure の管理ポータル](https://manage.windowsazure.com)を使用してキューの作成と削除を行うことができます。管理ポータルの  サービス バス セクションには、キューを作成し削除する機能のほかに、接続文字列や発行者、アクセス キーを取得する機能も用意されています。また、サービス バス キューのダッシュボードを表示して、発着信メッセージのメトリックを表示することもできます。 
