...
Service Fabric は、OS X ではネイティブに実行されません。Microsoft では、ローカルの Service Fabric クラスターを実行できるように、事前に構成済みの Docker コンテナー イメージを用意しています。 作業を開始する前に、以下を行う必要があります。
...
ローカル Docker コンテナーをセットアップし、その上でサービス ファブリック クラスターを実行するには、次の手順を実行します。
...
2. イメージを使用して Service Fabric One-box コンテナー インスタンスを起動します。
...
3. 対話型 ssh モードで Docker コンテナーにログインします。

    ```bash
    docker exec -it sfonebox bash
    ```

4. セットアップ スクリプトを実行します。必要な依存関係がフェッチされ、その後、コンテナーのクラスターが起動されます。
...
5. 手順 4. が正常に完了したら、Mac から ``http://localhoist:19080`` にアクセスします。Service Fabric Explorer が表示されます。
...
1. ご使用の Mac に Node.js と NPM がインストールされている必要があります。 インストールされていない場合は、次の手順に従って、Homebrew を使って Node.js と NPM をインストールしてください。 Mac にインストールされている Node.js と NPM のバージョンは、``-v`` オプションを使ってチェックできます。
