...
-	FaultDomain と UpgradeDomain: 指定されたノードにレプリカを配置することで、特定の障害ドメインまたはアップグレード ドメインにまとめられる場合は、この制約によってそのノードが除外されます。この制約について説明するいくつかの例については、「[fault and upgrade domain constraints and resulting behavior (障害ドメインとアップグレード ドメインの制約および行われる動作)](service-fabric-cluster-resource-manager-cluster-description.md)」のトピックで示します。
