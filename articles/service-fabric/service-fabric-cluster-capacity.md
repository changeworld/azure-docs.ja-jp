...
> 持続性レベルに関係なく、VM スケール セットに対する [Deallocation](https://docs.microsoft.com/en-us/rest/api/compute/virtualmachinescalesets/deallocate) 操作を行うと、クラスターは破棄されます
...
- **Azure で運用ワークロードを実行する VM インスタンスの数:** 最小プライマリ ノード タイプ サイズとして 5、信頼性レベルとして Silver を指定する必要があります。  
