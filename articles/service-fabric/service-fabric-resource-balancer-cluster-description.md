<properties
   pageTitle="リソース バランサー クラスターの記述"
   description="リソース バランサーに対するクラスターの記述の指定"
...
```
- Azure の展開で、障害ドメインとアップグレード ドメインは、Azure によって割り当てられます。したがって、Azure のインフラストラクチャ オプション内のノードとロールの定義には、障害ドメインまたはアップグレード ドメインの情報は含まれません。
