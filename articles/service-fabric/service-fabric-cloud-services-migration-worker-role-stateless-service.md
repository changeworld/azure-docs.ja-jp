<properties
   pageTitle="Web ロールと worker ロールを Service Fabric ステートレス サービスに変換する手順 | Microsoft Azure"
...
## Cloud Service プロジェクトから Service Fabric アプリケーション プロジェクトへ
...
VM の開始 | `OnStart()` | 該当なし
VM の停止 | `OnStop()` | 該当なし
クライアント要求に対するリスナーを開く | 該当なし | <ul><li> `CreateServiceInstanceListener()` (ステートレスの場合)</li><li>`CreateServiceReplicaListener()` (ステートフルの場合)</li></ul>
...
--- | --- | ---
構成の場所 | ServiceDefinition.csdef | ServiceManifest.xml
特権 | "制限付き" または "昇格された" 特権 | 任意のユーザーまたはコンピューター アカウント
