...
> [AZURE.NOTE]正常性ストアは、状態情報を保持するために使用しないでください。正常性に関連する情報のみを、エンティティの正常性評価に影響を与える正常性情報としてレポートする必要があります。正常性ストアは、汎用目的のストアとして設計されていません。これは、すべてのデータを正常性状態に集計するために、正常性評価のロジックを使用します。正常性に関係しない情報 (正常性状態に問題のないレポート状態) を送信することによって、集約される正常性状態に影響は生じませんが、状態ストアのパフォーマンスに悪影響が生じることがあります。
...
このようなすべての点についてまとめた例を見てみましょう。すべてのノードでデプロイされたマスター ステートフル永続サービスとスレーブ ステートレス サービスから構成される Service Fabric アプリケーションについて検討します (1 種類のタスクに対して 1 つのスレーブ サービス タイプ)。マスターには、スレーブによって実行されるコマンドを格納する処理キューがあります。スレーブは、受信要求を実行し、受信確認 (ACK) 信号を返信します。監視できる 1 つの条件は、マスターの処理キューの長さです。マスターのキューの長さがしきい値に達すると、警告がレポートされます。これは、スレーブが負荷を処理できないことを示します。キューが最大長に達して、コマンドが格納されなくなると、サービスが復旧できなくなり、エラーがレポートされます。**QueueStatus** プロパティについてレポートすることができます。ウォッチドッグはサービス内で動作し、マスター プライマリ レプリカに関して定期的に送信されます。有効期間は 2 分間です。30 秒間隔で定期的に送信されます。プライマリがダウンした場合、レポートはストアから自動的にクリーンアップされます。サービス レプリカが動作している状態でデッドロックまたはその他の問題がある場合、正常性ストアのレポートは有効期限が切れます。この場合は、エンティティはエラーと評価されます。

監視可能なその他の条件は、タスクの実行時間です。マスターは、タスクの種類に基づいて、タスクをスレーブに配布します。設計によっては、マスターでタスクの状態についてスレーブをポーリングすることができます。また、処理の完了時にスレーブから確認応答信号が返信されるまで待機することもできます。2 番目の場合は、スレーブが異常終了し、またはメッセージが失われた状況を検出するために、注意を払う必要があります。1 つのオプションでは、マスターが、同じスレーブに ping 要求を送信し、スレーブが状態を返信します。マスターに状態が返信されない場合、マスターはエラーと認識と、タスクのスケジュールを変更します。これは、タスクが独立していることを前提としています。
...
- タスクを実行しているスレーブでは、実行時間が想定時間よりも長いかどうかがレポートされます。プロパティ **PendingTasks** のサービス インスタンスに関してレポートされます。これは、問題のあるサービス インスタンスを示しますが、インスタンスが異常終了した状況をキャプチャしません。レポートは、その時点でクリーンアップされます。スレーブ サービスについてレポートすることもできます。スレーブでタスクが完了すると、スレーブ インスタンスによってストアからレポートがクリーンアップされます。これは、確認応答メッセージが失われた状況をキャプチャせず、タスクは、マスターの観点では終了しません。
