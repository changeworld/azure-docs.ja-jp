...
## <a name="parameter-element-defined-in-parameters"></a>Parameters 内で定義される Parameter 要素
...
## <a name="parameter-element-defined-in-section"></a>Section 内で定義される Parameter 要素
オーバーライドする設定。
...
この値は、'Value' 属性で指定した値の種類を定義します。 これには、SecretsStoreRef、Encrypted、PlainText を指定できます。
SecretsStoreRef に設定した場合は、SecretStore から参照値を取得します。
Encrypted に設定した場合、アプリケーション開発者は、証明書を作成し、Invoke-ServiceFabricEncryptSecret コマンドレットを使用して、機密情報を暗号化します。
...
## <a name="serviceplacementpolicy-element-defined-in-serviceplacementpolicies"></a>ServicePlacementPolicies 内で定義される ServicePlacementPolicy 要素
サービス配置ポリシーを定義します。このポリシーで、サービスを特定のクラスター障害ドメインで実行する必要があるか、実行してはならないかを指定します。  クラスターが地理的な距離や地政学的な地域をまたがる場合に便利です。
