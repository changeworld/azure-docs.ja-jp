...
>
> Service Fabric は、ノードの障害ドメインが変わらないことを想定しているためです。 VM の高可用性を確保する他のメカニズム ([HA-VM](https://technet.microsoft.com/en-us/library/cc967323.aspx) など) では、あるホストから別のホストへと VM の透過的な移行を使用します。 このようなメカニズムは、VM 内で実行されているコードの再構成または通知を行いません。 そのため、Service Fabric クラスターを実行する環境としては**サポートされません**。 採用する高可用性テクノロジは、Service Fabric のみにすることをお勧めします。 VM のライブ移行、SAN などメカニズムは必要ありません。 これらのメカニズムと Service Fabric と組み合わせて使用すると、アプリケーションの可用性と信頼性は_低くなります_。これは、新たな複雑さが増え、障害の集中する場所が増え、Service Fabric と競合する信頼性と可用性の戦略が利用されるためです。 
>
>
...
このレイアウトは、障害ドメインの制約の定義に違反しています。 FD0 には 2 つのレプリカがある一方で FD1 にはレプリカがありません。FD0 と FD1 の差は合計 2 になります。 この配置はクラスター リソース マネージャーによって許可されません。 同様に (N1 と N2 ではなく) N2 と N6 を選択すると、次のようになります。
...
障害ドメインの観点からは、このレイアウトはバランスがとれています。 ただし、アップグレード ドメインの制約には違反しています。 これは、UD0 のレプリカ数が 0 個で、UD1 のレプリカ数は 2 個のためです。 そのため、このレイアウトも無効であり、クラスター リソース マネージャーで選択されません。 
...
場合によっては (実際には、ほとんどの場合)、特定のワークロードが、クラスター内の特定のノードの種類だけで確実に実行されるようにしたいことがあります。 たとえば、ワークロードの中に GPU や SSD を必要とするものとしないものが混在している場合があります。 特定のワークロードをターゲットにするハードウェアの好例は、ほぼすべての n 層アーキテクチャです。 一部のコンピューターは、アプリケーションのフロント エンドまたは API 提供側として機能し、クライアントまたはインターネットに公開されます。 (多くの場合異なるハードウェア リソースを備えた) 複数のコンピューターが、コンピューティングまたはストレージ レイヤーの作業を処理します。 通常、これらはクライアントまたはインターネットに直接_公開されません_。 Service Fabric では、特定のワークロードを特定のハードウェア構成で実行する必要が生じる状況があると想定されています。 For example:
