...
次の手順では、キー コンテナー内の自己署名証明書で保護される単一ノード (単一の仮想マシン) Preview Service Fabric クラスターを作成します。 単一ノード クラスターは 1 台の仮想マシンを超えて拡張することはできず、Preview クラスターを新しいバージョンにアップグレードすることはできません。 
...
6. 証明書をダブルクリックして証明書インポート ウィザードを開きます。

7. 既定の設定を使用しますが、**[このキーをエクスポート可能にする]** チェック ボックス (**[秘密キーの保護]** 手順で) を必ずオンにしてください。 Visual Studio では、このチュートリアルの後半で、Azure Container Registry の構成時に、Service Fabric クラスター認証のために証明書をエクスポートする必要があります。

8. ブラウザーで Service Fabric Explorer を開けるようになりました。 URL は、PowerShell コマンドレットの出力の **ManagementEndpoint** で、たとえば、*https://mycluster.westeurope.cloudapp.azure.com:19080* などです。 
