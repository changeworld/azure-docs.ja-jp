...
2. IPv6 対応マネージド クラスターをデプロイします。 必要に応じて[サンプル テンプレート](https://raw.githubusercontent.com/Azure-Samples/service-fabric-cluster-templates/SF-Managed-Standard-SKU-2-NT-IPv6/AzureDeploy.json)をカスタマイズするか、独自のものを構築します。
...
2. Service Fabric リソース プロバイダー アプリケーションにロールの割り当てを追加します。 ロールの割り当ての追加は、1 回限りの操作です。 ロールを追加するには、次の PowerShell コマンドを実行するか、以下に示す Azure Resource Manager (ARM) テンプレートを構成します。 
   ...
   この手順に使用できる、[VNet サブネットを作成してロールの割り当てを行う Azure Resource Manager (ARM) テンプレート](https://raw.githubusercontent.com/Azure-Samples/service-fabric-cluster-templates/SF-Managed-Standard-SKU-2-NT-BYOVNET/SFMC-VNet-RoleAssign.json)のサンプル全体を次に示します。
...
3. 次に示すように、ロールを設定した後で、クラスター デプロイの `subnetId` プロパティを構成します。
   ...
   ```
   [Bring Your Own VNet クラスターのサンプル テンプレート](https://raw.githubusercontent.com/Azure-Samples/service-fabric-cluster-templates/SF-Managed-Standard-SKU-2-NT-BYOVNET/AzureDeploy.json)を参照するか、独自のものをカスタマイズします。
...
* 各ノード タイプは独自の VNet にデプロイされるため、ノード タイプごとに NSG ルールを構成する
 ...
 * バックエンドおよび NAT プールが既存の Azure ロード バランサーに構成されている必要があります。 例については、[こちらにある作成とロールの割り当てのサンプル](https://raw.githubusercontent.com/Azure-Samples/service-fabric-cluster-templates/SF-Managed-Standard-SKU-2-NT-BYOLB/createlb-and-assign-role)全体を参照してください。 
...
2. Service Fabric リソース プロバイダー アプリケーションにロールの割り当てを追加します。 ロールの割り当ての追加は、1 回限りの操作です。 ロールを追加するには、次の PowerShell コマンドを実行するか、以下に示す Azure Resource Manager (ARM) テンプレートを構成します。
   ...
   以下の手順では、Existing-RG リソース グループにある Existing-LoadBalancer1 という名前の既存のロード バランサーを使用します。 サブネットの名前は "default" です。
...
5. 必要に応じて、ノード タイプに適用されるマネージド クラスター NSG ルールを構成し、Azure ロード バランサーで構成した必要なトラフィックを許可します。そうしないと、トラフィックがブロックされます。
   ...
   インバウンド規則を開く方法の例については、[Bring Your Own Load Balancer の Azure Resource Manager (ARM) サンプル テンプレート](https://raw.githubusercontent.com/Azure-Samples/service-fabric-cluster-templates/SF-Managed-Standard-SKU-2-NT-BYOLB/AzureDeploy.json)を参照してください。
