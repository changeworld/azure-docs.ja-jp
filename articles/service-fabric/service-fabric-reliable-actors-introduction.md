...
Azure Service Fabric アクターは、[Service Fabric](service-fabric-technical-overview.md) 用のアクターのプログラミング モデルです。Service Fabric は、開発と管理が容易なクラウドおよびオンプレミスの両方の信頼性の高いスケーラブルなアプリケーションを構築するためのプラットフォームです。

Fabric アクターは、非同期のシングル スレッド アクター モデルを提供します。アクターは、状態と計算の単位を表します。アクターは、高い拡張性を実現するためにクラスター全体に分散されます。アクターのホスト プロセスが失敗すると、システムは別のプロセスでアクターを再作成します。同様に、アクターのホスト プロセスが実行されているノードで障害が発生すると、システムは別のノードでのホスト プロセスでアクターを再作成します。アクター ランタイムは、アクターに高可用性と一貫性のある状態管理を提供するために基になる Service Fabric プラットフォームによって提供される分散ストアを利用します。これにより、アクター ベースの分散クラウド アプリケーションの開発と維持が非常に容易になります。

## アクター
アクターは、独立したシングル スレッド コンポーネントであり、状態と動作の両方をカプセル化します。.NET オブジェクトに似ているため、開発者には自然なプログラミング モデルが提供されます。.NET オブジェクトが .NET 型のインスタンスであるように、アクターはすべてアクター型のインスタンスです。たとえば、電卓の機能を実装するアクター型がある場合や、クラスター全体のさまざまなノードで分散されるその型のアクターが多数存在する場合があります。このようなアクターはそれぞれ、アクター ID で一意に識別されます。

アクターは、要求応答パターンで非同期メッセージを渡すことによって、他のアクターを含む、システムの残りの部分と対話します。これらの対話は、非同期メソッドとしてインターフェイスで定義されます。たとえば、電卓の機能を実装するアクター型のインターフェイスを次のように定義することができます。
...
アクター型では、次のように上記のインターフェイスを実装できます。
...
各インターフェイス メソッドの引数と返されるタスクの結果の型は、[データ コントラクト シリアル化可能](service-fabric-reliable-actors-notes-on-actor-type-serialization.md)でなければなりません。

アクター ランタイムはアクター メソッドのターンごとの同時実行を提供するため、これらのメソッドの実装では、ロックや他の同時実行の問題は処理されません。詳細については、「[同時実行](#concurrency)」セクションを参照してください。
...
上記の例のように、アクター プロキシ オブジェクトの作成には、アクター ID とアプリケーション名という 2 つの情報が使用されています。アクター ID は、アクターを一意に識別する識別子です。アプリケーション名は、アクターが配置される場合の [Service Fabric アプリケーション](service-fabric-reliable-actors-platform.md#service-fabric-application-model-concepts-for-actors)の名前です。

アクターは仮想アクターです。これは、常に存在することを意味します。明示的に作成したり、破棄したりする必要はありません。アクター ランタイムは、そのアクターの要求の初回受信時に、自動的にアクターをアクティブ化します。アクターが一定時間使用されていない場合、アクター ランタイムはそれをガベージ コレクトし、必要に応じて後でアクティブ化します。詳細については、「[アクターのライフサイクルとガベージ コレクション](service-fabric-reliable-actors-lifecycle.md)」のセクションを参照してください。

アクター クライアント API では、場所の透過性とフェールオーバーも提供します。クライアント側の `ActorProxy` クラスは、必要な解決策を実行し、アクター サービスの[パーティション](service-fabric-reliable-actors-platform.md#service-fabric-partition-concepts-for-actors)を検索します。このパーティションで、指定した ID を持つアクターがホストされ、通信チャネルを開きます。`ActorProxy` は、通信エラーやフェールオーバーが発生した場合に再試行します。これは、アクターの実装で同じクライアントから重複するメッセージを取得できることを意味します。
...
### ステートレス アクター
ステートレス アクターは、名前が示すように、状態がすべてアクター ランタイムによって管理されるわけではありません。ステートレス アクターは、アクターの基本クラスから派生します。メンバー変数を使用でき、その変数は、他の .NET 型と同じように、そのアクターの有効期間にわたって保持されます。ただし、そのアクターのインスタンスは、しばらくアイドル状態になってからガベージ コレクトされた場合、そのメンバー変数に格納されている状態を失います。同様に、状態はフェールオーバーにより失われる場合があります。これは、アップグレード、リソース バランシング操作、またはアクター プロセスやアクター プロセスをホストするノードの障害などの状況で発生する可能性があります。

以下に、ステートレス アクターの例を示します。
...
### ステートフル アクター
ステートフル アクターには、アクターのガベージ コレクションとフェールオーバー全体で保持する必要がある状態があります。ステートフル アクターは `Actor<TState>` 基本クラスから派生します。ここで、`TState` はガベージ コレクションとフェールオーバー全体で保持する必要がある状態の種類です。状態には、`Actor<TState>` 基本クラスの `State` プロパティを使用して、アクター メソッドでアクセスできます。状態にアクセスするステートフル アクターの例を以下に示します。
...
アクター状態の型は、[データ コントラクト シリアル化可能](service-fabric-reliable-actors-notes-on-actor-type-serialization.md)でなければなりません。

> 注: インターフェイスおよびアクター状態の型の定義方法の詳細については、「[信頼性の高いアクターのシリアル化に関する留意事項](service-fabric-reliable-actors-notes-on-actor-type-serialization.md)」の記事を参照してください。

#### アクター状態プロバイダー
状態の格納と取得はアクター状態プロバイダーによって提供されます。状態プロバイダーは、状態プロバイダーの特定の属性を使用して、アクターごとまたはアセンブリ内のすべてのアクターに構成できます。アクター ランタイムに含まれる既定のアクター状態プロバイダーがいくつかあります。状態の持続性と信頼性は、状態プロバイダーが提供する保証によって決まります。アクターがアクティブ化されている場合、その状態はメモリに読み込まれます。アクター メソッドが完了すると、変更済みの状態は、状態プロバイダーに対してメソッドを呼び出すことで、アクター ランタイムによって自動的に保存されます。状態の保存中にエラーが発生した場合は、アクター ランタイムがそのアクター インスタンスをリサイクルします。新しいアクター インスタンスが作成され、状態プロバイダーからの最後の一貫性のある状態で読み込まれます。
...
> [AZURE.TIP]Fabric アクター ランタイムは、いくつかの[アクター状態管理に関するイベントとパフォーマンス カウンター](service-fabric-reliable-actors-diagnostics.md#actor-state-management-events-and-performance-counters)を出力します。これらは、診断やパフォーマンスの監視に役立ちます。

#### 読み取り専用メソッド
既定では、アクター ランタイムは、アクター メソッド呼び出し、タイマーのコールバックとアラームのコールバックの完了時にアクター状態を保存します。状態の保存が完了するまでは、他のアクターの呼び出しは許可されません。状態プロバイダーに応じて、状態の保存に時間がかかる場合があり、この時間にアクターで他のアクターを呼び出すことはできません。
