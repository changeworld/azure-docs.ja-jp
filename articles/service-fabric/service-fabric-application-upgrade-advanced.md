...
既にデプロイされているアプリケーションに新しいサービスを追加し、アップグレードとして発行した場合、デプロイされているアプリケーションに新しいサービスが追加されます (そのアプリケーションの既存のサービスに影響を及ぼすアップグレードは一切行われません)。ただし新しいサービスを有効にするには、追加したサービスのインスタンスを (`New-ServiceFabricService` コマンドレットを使用して) 開始する必要があります。

アップグレードの一環として、サービスをアプリケーションから削除することもできます。ただしその前に、現在実行されているサービス (アップグレードの一環として削除するサービス) のインスタンスは、`Remove-ServiceFabricService` コマンドレットを使ってすべて停止しておく必要があります。
...
Visual Studio を使ってアプリケーションをアップグレードすると、差分パッケージが自動的に発行されます。差分パッケージを手動で作成する場合 (PowerShell を使ってアップグレードする場合など) は、アプリケーションとサービスのマニフェストを更新し、最終アプリケーション パッケージには、変更のあったパッケージだけを追加するようにします。
...
ここで、PowerShell から差分パッケージを使用して、service1 のコード パッケージのみを更新すると仮定しましょう。更新後のアプリケーションは次のようになります。
