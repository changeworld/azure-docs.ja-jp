---
title: Azure Service Fabric CLI - sfctl サービス
description: Azure Service Fabric のコマンド ライン インターフェイスである sfctl について説明します。 サービス、サービスの種類、サービス パッケージの管理を行うためのコマンドの一覧が含まれています。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| 作成 | 指定された Service Fabric サービスを作成します。 |
...
| --move-cost | サービスの移動コストを指定します。 指定可能な値は\: "Zero"、"Low"、"Medium"、"High"、"VeryHigh" です。 |
...
| --service-placement-time | ビルドがスタックしていることを報告する前に、レプリカを InBuild のままにする期間。 これはステートフル サービスにのみ適用されます。 |
