---
title: Azure Service Fabric サービス モデルの XML スキーマの複合型
...
コンテナーの分離モード。 有効な値は、default、process、Hyper-V (Windows コンテナーでサポートされている場合のみ) です。
...
この値は、属性の値の保護を定義します。 これには、SecretsStoreRef、Encrypted、PlainText を指定できます。
SecretsStoreRef に設定した場合、値はクラスターの SecureStore で暗号化されたシークレットを参照します。このシークレットは、アクティブ化の際に参照および暗号化が解除されます。
...
環境変数の型。 これには、PlainText、Encrypted、SecretsStoreRef を指定できます。
...
この値は、'Password' 属性で指定したパスワードの種類を定義します。 これには、SecretsStoreRef、Encrypted、PlainText を指定できます。
SecretsStoreRef に設定した場合は、SecretStore から参照値を取得します。
Encrypted に設定した場合、アプリケーション開発者は、証明書を作成し、Invoke-ServiceFabricEncryptSecret コマンドレットを使用して、機密情報を暗号化します。
