...
Service Fabric アプリケーションの種類をクラスターにプロビジョニングします。 すべての新しいアプリケーションをインスタンス化するには、プロビジョニングが必要です。 プロビジョニング操作は、relativePathInImageStore で指定されているアプリケーション パッケージで、または外部 ".sfpkg" の URI を使って、実行することができます。 --external-provision を設定しないと、このコマンドはイメージ ストアのプロビジョニングを予期します。
...
| --compress | Service Fabric アプリケーション パッケージにのみ適用されます。 圧縮されたアプリケーション パッケージを含む新しいフォルダーを、既定の場所または圧縮場所パラメーターで指定された場所に作成し、新しく作成されたフォルダーをアップロードします。 <br><br> sfctl によって生成された圧縮ファイルが既に存在する場合は、このフラグが設定されていると上書きされます。 ディレクトリがアプリケーション パッケージでない場合は、エラーが返されます。 既に圧縮されているアプリケーション パッケージの場合は、そのままフォルダーがコピーされます。 既定では、アップロードが成功すると、新しく作成された圧縮アプリケーション パッケージは削除されます。 アップロードに失敗した場合は、必要に応じて、圧縮されたパッケージを手動でクリーンアップしてください。 圧縮された場所のパラメーターが存在しないディレクトリを参照している場合は、削除しても、作成された空のディレクトリは削除されません。 |
| --compressed-location | 圧縮されたアプリケーション パッケージを配置する場所。 <br><br> 場所が指定されていない場合、圧縮されたパッケージは、パス引数で指定された親ディレクトリの下に sfctl_compressed_temp という名前の新しく作成されたフォルダーの下に配置されます。 たとえば、パス引数に値 C\:/FolderA/AppPkg がある場合、圧縮されたパッケージは、C\:/FolderA/sfctl_compressed_temp/AppPkg に追加されます。 |
...
| --keep-compressed | アップロードが正常に完了した場合に、生成された圧縮パッケージを保持するかどうかを指定します。 <br><br> 設定されない場合、正常に完了すると、圧縮されたアプリ パッケージは削除されます。 アップロードが成功しなかった場合、アプリケーション パッケージは、再アップロードのために常に出力ディレクトリに保持されます。 |
| --show-progress | 大きいパッケージのファイル アップロードの進行状況を表示します。 |
| --timeout -t | 合計タイムアウト (秒)。 アップロード タイムアウト時間が経過すると、アップロードが失敗してエラーが返されます。 このタイムアウトは、アプリケーション パッケージ全体に適用されます。個々のファイルのタイムアウトは、タイムアウトの残り時間と等しくなります。 タイムアウトには、アプリケーション パッケージを圧縮するのに必要な時間は含まれません。  既定値\: 300。 |
