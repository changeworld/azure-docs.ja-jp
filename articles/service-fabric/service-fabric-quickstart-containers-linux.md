...
Azure 内のクラスターにアプリケーションをデプロイする場合、独自のクラスターかパーティ クラスターを使用します。

パーティ クラスターは、Azure でホストされている期間限定の無料 Service Fabric クラスターです。 Service Fabric チームによってサポートされており、誰でもアプリケーションをデプロイし、プラットフォームについて学ぶことができます。 パーティ クラスターにアクセスするには、[こちらの手順を実行します](http://aka.ms/tryservicefabric)。 
...
CLI 環境内に Service Fabric コマンド ライン (sfctl) をインストールする
...
ブラウザーを開いて、http://\<my-azure-service-fabric-cluster-url>:80 (`http://linh1x87d1d.westus.cloudapp.azure.com:80` など) で Service Fabric Explorer に移動します。 アプリケーション ノードを展開し、投票アプリケーションの種類と作成したインスタンスのエントリがあることを確認します。
