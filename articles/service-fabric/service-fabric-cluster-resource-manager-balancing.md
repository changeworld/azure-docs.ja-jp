---
title: Azure Service Fabric クラスターの均衡をとる
...
クラスター リソース マネージャーが実行する作業には、3 つのカテゴリがあります。 これらは次のとおりです。
...
![全体としてどのようにサービスのバランスを取っているかを示す図。][Image4]
</center>
...
![Cluster Resource Manager によって、どのサービス間に関係があるかが特定されることを示す図。][Image5]
</center>
...
* Cluster Resource Manager は、サブクラスター化 (配置の制約と分散を使用する場合に発生する可能性のある状況) を認識して処理できます。 サブクラスター化が負荷分散とその処理方法に与える影響については、[こちら](cluster-resource-manager-subclustering.md)を参照してください
