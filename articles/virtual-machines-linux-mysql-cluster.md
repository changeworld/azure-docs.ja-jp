...
-   [開発の準備][開発の準備]
-   [クラスターのセットアップ][クラスターのセットアップ]
-   [MySQL のセットアップ][MySQL のセットアップ]
-   [Corosync のセットアップ][Corosync のセットアップ]
-   [Pacemaker のセットアップ][Pacemaker のセットアップ]
-   [テスト][テスト]
-   [STONITH][STONITH]
-   [制限事項][制限事項]
...
Linux ベースの高可用性サービスを Microsoft Azure 上にデプロイするには、さまざまな方法が利用できます。この記事の目的はそれぞれの方法を詳細に説明することですが、MySQL サーバーの高可用性についての初歩的な知識も説明します。この記事の内容を紹介するビデオが [Channel 9][Channel 9] で視聴できます。
...
他にも NBD Cluster、Percona、Galera などの MySQL 用アーキテクチャやいくつかのミドルウェア ソリューション (VM として利用できるものが [VM Depot][VM Depot] に 1 つ以上掲載されています) が利用できる可能性があります。これらのソリューションが、(マルチキャストまたはブロードキャストではなく) ユニキャストでのレプリケーションが可能で、共有ストレージや複数のネットワーク インターフェイスに依拠しないものであれば、Microsoft Azure 上で簡単にデプロイできます。
...
このリソースのサンプル コードは [GitHub][GitHub] で入手できます。`sudo crm configure` に次のコードを追加することにより、クラスターの構成を変更する必要があります。
