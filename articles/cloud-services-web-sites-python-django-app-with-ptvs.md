<properties urlDisplayName="Django with Python Tools for Visual Studio 2.0" pageTitle="Python Tools for Visual Studio 2.0 を使用して Django アプリケーションを作成する" metaKeywords="" description="Learn how to use the Python Tools for Visual Studio to create a Django application that stores data in a SQL Database or MySQL database instance and can be deployed to a website or cloud service." metaCanonical="" services="web-sites,cloud-services" documentationCenter="Python" title="Creating Django applications with Python Tools 2.0 for Visual Studio" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
このチュートリアルでは、Python Tools for Visual Studio と Azure を使用する場合の手順を説明します。ここで作成する投票アプリケーションおよび Django の詳細については、[https://www.djangoproject.com/](https://www.djangoproject.com/) をご覧ください。
...
**注: **Azure Websites または Cloud Services を使用してこのアプリケーションを正常に実行するには、[python.org](http://www.python.org/download/) から入手した公式な CPython 2.7 ディストリビューションを使用する必要があります。他のディストリビューションは動作する場合がありますが、公式にサポートされていません。
...
Python Tools for Visual Studio は Python 仮想環境をサポートします。  Django プロジェクトを作成して、仮想環境を使用して依存関係モジュールをインストールできます。Azure Websites または Cloud Services に発行するプロジェクトを設定するときは、この方法をお勧めします。
...
1. 基になる Python インタープリターとして Python 2.7 を選択し、既定の名前 (**env**) をそのまま使用します。pip または virtualenv がまだインストールされていない場合は、これらがインストールされます。

1. **env** を右クリックし、**[Python パッケージのインストール]** を選択します。**django**
...
1. Django はファイル数が多いので、インストールに少し時間がかかります。その間、進捗状況が出力ウィンドウに表示されます。
	...
	**注: **まれに、出力ウィンドウにエラーが表示されることがあります。その場合は、エラーがクリーンアップに関連しているかどうか確認してください。クリーンアップには失敗しても、インストールは正常に行われる場合があります（出力ウィンドウを上方向にスクロールして確認してください)。これは、新しく作成した一時ファイルやフォルダーがロックされ、pip クリーンアップ処理で削除できなくなるためです。

1. **env** を右クリックし、**[Python パッケージのインストール]** を選択します。**pytz** をインストールします (必須ではありませんが、このパッケージをインストールすることをお勧めします。django でタイムゾーンをサポートするときに使用されます)。
...
1. **tutorial/settings.py** で、**INSTALLED\_APPS** に次のエントリを追加します。

		'polls',

1. **INSTALLED\_APPS** をコメント解除します。

		'django.contrib.admin',

1. **tutorial/urls.py** を次のコードに書き換えます。
...
1. **polls/models.py** を次のコードに書き換えます。
...
1. **polls/views.py** を次のコードに書き換えます。
...
1. 新しい Python ファイル **polls/urls.py** を作成し、次のコードを追加します。
...
1. 新しい **polls/admin.py** アイテムを作成し、次のコードを追加します。
...
1. ドラッグ アンド ドロップ操作または切り取りと貼り付けを使用して、**polls/templates/index.html** ファイルを **polls/templates/polls** フォルダーへ移動します。

1. **polls/templates/polls/index.html** を次のマークアップに書き換えます。
...
1. 次のように、新しい Django HTML テンプレート **polls/templates/polls/detail.html** を作成します。
...
1. 次のように、新しい Django HTML テンプレート **polls/templates/polls/results.html** を作成します。
...
これで、新しい Web アプリケーションの準備が整いました。ただし、使用する前にデータベースを構成する必要があります。Web サイトをローカルでテストするため、sqlite3 データベースを作成しましょう。非常に軽量のデータベースなので、他に何もインストールする必要はありません。データベース ファイルはプロジェクト フォルダーに作成します。

1. **tutorial/settings.py** を開き、このファイルの最上部に次の import を追加します。
...
1. **http://localhost:{port}/admin** へ移動します。ポート番号は、開発用サーバーのコンソール ウィンドウから取得できます。前の手順で作成した資格情報を使用して、ログインします。

	![Add Poll](./media/cloud-services-python-create-deploy-django-app/django-tutorial-009-admin-login.png)

1. 管理インターフェイスを使用して、アンケート項目を 1 ～ 2 件追加します。ローカル データベースにアンケート項目を追加する作業にあまり時間をかけないでください。このあとクラウド データベースに切り替えますが、その時点でもう一度データベースにアンケート項目を追加します。
...
1. 次のコードを含む新しいスタイル シート ファイル **polls/static/polls/style.css** を追加します。
...
1. **polls/static/polls/images** フォルダーに既存の画像を追加し、**background.jpg** という名前を付けます。
...
1. Websites を再実行します。  作成したスタイルシート (青色のテキストと背景画像) がインデックス ページ、投票ページ、結果ページに反映されます。
...
1. **F10** キーを押して、通常の Python コードと同じようにステップすることができます。  for ループ内で、**poll** の値を確認できます。
...
1. **env** を右クリックし、**[Python パッケージのインストール]** を選択します。**easy\_install** を使用して Python パッケージ **pyodbc** をインストールします。

1. **env** を右クリックし、**[Python パッケージのインストール]** を選択します。**pip** を使用して Python パッケージ **django-pyodbc-azure** をインストールします。
...
1. Windows Azure の管理ポータルで、**[新規]**、**[ストア]**、**[アプリ サービス]**、**[ClearDB MySQL データベース]** の順に選択します。無料プランでデータベースを作成します。

1. 次に、Django から MySQL データベースにアクセスするためのパッケージを仮想環境にインストールします。**[env]** を右クリックし、**[Python パッケージのインストール]** を選択します。**easy\_install** を使用して **mysql-python** をインストールします。

1. **tutorial/settings.py** を編集し、**DATABASES** 定義を変更します。具体的には、**NAME**、**USER**、**PASSWORD**、**HOST** を、それぞれ ClearDB コントロール パネルに表示される値に書き換えてください。
...
1. Websites を開きます。Django シェルを使って追加したアンケート項目が表示されます。
...
どちらの場合で PTVS で自動的に IIS が構成され、プロジェクトに web.config ファイルがない場合は生成すます。  settings.py で STATIC_ROOT を設定した場合に限り、静的ファイル (manage.py collectstatic) が自動的に収集されます。

仮想マシンで Django をホストする方法については、このチュートリアルで説明しません。  この方法では、必要なオペレーティング システム (Windows または Linux) で VM を作成し、Python をインストールして Django アプリケーションを手動でデプロイします。 

### Azure Websites

1. まず、Websites を作成する必要があります。Azure の管理ポータルで、**[新規]**、**[コンピューティング]**、**[Web サイト]**、**[簡易作成]** の順にクリックします。  使用できる名前を選択します。
...
1. 発行が完了すると Web ブラウザーが開き、発行した Web サイトが表示されます。

	![Web Site Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-020-website.png)

### Azure Cloud Services

1. プロジェクト ノードを右クリックして、**[Azure Cloud Service プロジェクトの追加]** を選択するか、**[変換]、[Azure Cloud Service プロジェクトに変換]** の順に選択します (お使いの Visual Studio のバージョンに応じてどちらかが表示されます)。新しいプロジェクト (拡張子 .Azure) がソリューションに追加されます。この新しいプロジェクトは、ソリューションのスタートアップ プロジェクトとなります。
...
1. コンピューティング エミュレーターで実行するには、**管理者として Visual Studio を再起動する**必要があります。
...
1. 次に、作成した Cloud Service を Azure に発行します。**tutorial.Azure** という Cloud Service プロジェクトを右クリックし、**[発行]** を選択します。
...
1. 既定値をそのまま使用し、**[発行]** をクリックします。Cloud Services の VM をプロビジョニングする必要があるため、この処理は Websites に発行する場合より長い時間がかかります。その間、進捗状況が [Azure のアクティビティ ログ] ウィンドウに表示されます。
...
このチュートリアルでは、[Python Tools for Visual Studio](http://pytools.codeplex.com) を使って Django アプリケーションを作成しました。ここで使用したデータベースは、sqlite3、SQL Server、MySQL の 3 種類です。最後に、Azure Websites と Cloud Services にこのアプリケーションを発行しました。
