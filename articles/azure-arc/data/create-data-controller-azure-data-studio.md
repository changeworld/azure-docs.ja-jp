...
現時点では、この記事で説明されている方法を使用してデータ コントローラーを作成できます。
...
1. Azure Red Hat OpenShift または Red Hat OpenShift コンテナー プラットフォームを使用している場合は、セキュリティ コンテキスト制約を適用します。 「[OpenShift で Azure Arc 対応データ サービスのセキュリティ コンテキスト制約を適用する](how-to-apply-security-context-constraint.md)」の手順に従います。
