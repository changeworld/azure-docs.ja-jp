...
この手順は Linux と Windows の両方の VM に適用されます。後で IPython を構成して notebook サーバーをポート 9999 で実行します。このポートを公開するには、Azure の管理ポータルでエンドポイントを作成する必要があります。このエンドポイントは Azure ファイアウォールでポートを開き、パブリック ポート (HTTPS、443) を VM のプライベート ポート (9999) にマップします。
