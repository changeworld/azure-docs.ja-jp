...
Azure App Service Environment は、App Service アプリを大規模かつ安全に実行するために完全に分離された専用の環境を提供する、Azure App Service の機能です。 この機能は、[Web Apps][webapps]、[Mobile Apps][mobileapps]、[API Apps][APIapps]、および [Functions][Functions] をホストすることができます。
...
ASE に関するすべての記事と作業方法は [App Service 環境の README][ASEReadme] を参照してください。
...
* 複数の ASE を使って水平方向にスケーリングすることができます。 詳細については、[地理的に分散したアプリ フットプリントの設定](https://azure.microsoft.com/documentation/articles/app-service-app-service-environment-geo-distributed-scale/)に関するページを参照してください。
* AzureCon Deep Dive に示されているようなセキュリティ アーキテクチャは、ASE を使用して構成できます。 AzureCon Deep Dive に示されたセキュリティ アーキテクチャがどのように構成されたかを確認するには、App Service 環境での[レイヤード セキュリティ アーキテクチャのインプリメント方法](https://docs.microsoft.com/en-us/azure/app-service-web/app-service-app-service-environment-layered-security)に関する記事を参照してください。
* ASE で実行されるアプリへのアクセスは、Web アプリケーション ファイアウォール (WAF) などのアップストリーム デバイスによって制限できます。 詳細については、[App Service Environment に WAF を構成する方法](https://docs.microsoft.com/en-us/azure/app-service-web/app-service-app-service-environment-web-application-firewall)に関するページを参照してください。
