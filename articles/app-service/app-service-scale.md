...
価格レベルのスケールアップ以外にも、サービスに含まれるインスタンス数を増やすことができます。これを*スケールアウト*または*スケールイン*と呼びます。*スケールアウト*と*スケールイン*の詳細については、[インスタンス数を手動または自動でスケールする方法](../azure-portal/insights-how-to-scale.md)に関する記事を参照してください。

App Service プランの詳細については、「[Azure App Service プランの詳細な概要](azure-web-sites-web-hosting-plans-in-depth-overview.md)」をご覧ください。App Service の個々のプランの価格と機能の詳細については、[App Service の価格の詳細](/pricing/details/app-service/)に関するページを参照してください。

専用の [App Service 環境](app-service-app-service-environments-readme.md)を使用する場合、スケーリングの動作は異なります。詳細については、「[App Service 環境内での Web アプリのスケーリング](../app-service-web/app-service-web-scale-a-web-app-in-an-app-service-environment.md)」を参照してください。
