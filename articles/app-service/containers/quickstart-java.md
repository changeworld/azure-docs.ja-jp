...
次に、デプロイを構成し、[コマンド プロンプト] で maven コマンド `mvn com.microsoft.azure:azure-webapp-maven-plugin:1.8.0:config` を実行し、 **[Confirm (Y/N)]\(確認 (Y/N)\)** プロンプトが表示されるまで **Enter** キーを押して既定の構成を使用し、 **'y'** キーを押して構成を完了します。
