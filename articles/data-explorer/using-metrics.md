...
1. 同じグラフにプロットされた複数のメトリックを表示する場合は、 **[メトリックの追加]** ボタンを選択します。
1. 1 つのビューに複数のグラフを表示する場合は、 **[+ New chart]\(+ 新規グラフ\)** ボタンを選択します。
...
**メトリック** | **単位** | **集計** | **メトリックの説明**
|---|---|---|---|
連続エクスポートによるエクスポートされたレコード数    | Count | SUM | クラスターからエクスポートされたレコードの合計数。 |
連続エクスポートの最大遅延分数 |    Count   | Max   | エクスポートされたレコードの最大値 (分)。|
保留中の連続エクスポートの数 | Count | Max   | 保留中のエクスポート操作の最大値。
連続エクスポートの結果    | Count |   Count   | 結果別の連続エクスポート操作の合計数。 メトリックには、連続エクスポートの名前とデータベースが含まれます。 
エクスポート使用率 |    Percent | Max   | エクスポート操作の定義済みスロットの使用。
| | | | |
...
**メトリック** | **単位** | **集計** | **メトリックの説明**
|---|---|---|---|
| (Event/IoT Hubs の) 処理されたイベント | Count | Max、Min、Sum | イベント ハブから読み取られ、クラスターによって処理されたイベントの数。 イベントは、クラスター エンジンによって拒否されたイベントと受け入れられたイベントに分類されます。 |
...
| インジェストの量 (MB 単位) | Count | Max、Sum | 圧縮前にクラスターにインジェストされたデータの合計サイズ (MB 単位)。 |
| | | | |  
...
**メトリック** | **単位** | **集計** | **メトリックの説明**
|---|---|---|---|
| クエリ実行時間 | ミリ秒 | Avg、Min、Max、Sum | クエリ結果を受け取るまでの合計時間 (ネットワーク待ち時間は含まれません)。 |
...
**メトリック** | **単位** | **集計** | **メトリックの説明**
|---|---|---|---|
ストリーミング インジェストのデータ速度 |    Count   | RateRequestsPerSecond | クラスターに取り込まれるデータの総量。 |
ストリーミング インジェストの期間   | ミリ秒  | Avg、Max、Min | すべてのストリーミング インジェスト要求の合計期間。 |
ストリーミング インジェストの要求率   | Count | Count、Avg、Max、Min、Sum | ストリーミングインジェスト要求の合計数。 |
ストリーミング インジェストの結果 | Count | Avg   | 結果の種類別のストリーミング インジェスト要求の合計数。 |
| | | | |
