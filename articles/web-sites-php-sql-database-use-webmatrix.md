...
このチュートリアルでは、Azure SQL Database を使用する PHP アプリケーションを WebMatrix で作成し、Azure Web サイトにデプロイする方法を示します。WebMatrix は、Microsoft から提供されている無料の Web 開発ツールで、Web サイトの開発に必要なものがすべて用意されています。WebMatrix では PHP がサポートされており、PHP 開発用 Intellisense が含まれています。 
...
* 管理ポータルを使用して Azure の Web サイトと SQL データベースを作成する方法。Azure の Web サイトでは PHP が既定で有効になっているため、特に何もしなくても PHP コードを実行できます。
...
Tasklist アプリケーションのファイルを1. [ダウンロード][tasklist-sqlazure-download]します。Tasklist アプリケーションは、タスク一覧のアイテムの追加、完了済みとしてのマーク付け、および削除を行うための、単純な PHP アプリケーションです。タスク一覧のアイテムは SQL データベースに保存されます (ローカル テストには SQL Server Express を使用します)。アプリケーションは、次のファイルで構成されます。
...
3. **[Web サイト]** をクリックし、**[カスタム作成]** をクリックします。
	...
	**[URL]** ボックスに値を入力し、**[データベース]** ボックスの一覧の **[新しい SQL データベースを作成します]** を選択して、**[リージョン]** ボックスの一覧で Web サイトのデータ センターを選択します。ダイアログの下部にある矢印をクリックします。
...
2. 次に、ローカル SQL Server データベースの接続情報を `taskmodel.php` ファイルに追加する必要があります。`taskmodel.php` ファイルをダブルクリックして開き、`connect` 関数内のデータベース接続情報を更新します (**メモ**:アプリケーションをローカルでテストせず、直接 Azure Websites に発行する場合は、「[アプリケーションの発行](#Publish)」に進んでください)。
...
アプリケーションを Azure Websites に発行する前に、`taskmodel.php` 内のデータベース接続情報を、先ほど (「[Create an Azure Web Site and SQL Database (Azure の Web サイトおよび SQL データベースを作成する)](#CreateWebsite)」セクションで) 取得した接続情報に更新する必要があります。
