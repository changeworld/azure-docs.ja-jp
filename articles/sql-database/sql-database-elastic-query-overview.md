...
### クエリ
外部データ ソースと外部テーブルを定義すると、エラスティック データベース クエリ機能が有効になっているデータベースに、使い慣れた SQL データベース接続文字列を使用して接続することができます。これで、外部テーブルに対して完全な読み取り専用のクエリを実行できるようになりました。ただし、以下の[制限事項のセクション](#preview-limitations)で説明する制限があります。
