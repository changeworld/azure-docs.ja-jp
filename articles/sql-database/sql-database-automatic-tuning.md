...
> - 自動チューニング設定が構成されていない既存のサーバーは、新しい Azure の既定値で自動的に構成されます。 これは、現在定義されていない状態で自動チューニングを行っているすべてのお客様に当てはまります。
> - 新しく作成されたサーバーには、新しい Azure の既定値が自動的に構成されます (新しいサーバーの作成時に自動チューニング構成が未定義の状態になっていた以前とは異なります)。
>
