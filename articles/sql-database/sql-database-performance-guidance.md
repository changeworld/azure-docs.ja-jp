...
Microsoft Azure SQL Database には 3 つの[サービス層](sql-database-service-tiers.md)があります。Basic、Standard、Premium です。Azure SQL Database とそのセカンダリ レプリカのリソース量を厳密に制御することで、Premium サービス層では、クラウド アプリケーションのパフォーマンスがさらに予測可能となります。Azure SQL Database はこの概念を Standard サービス層に拡大し、Premium サービス層よりもパフォーマンス要件が低いデータベースのパフォーマンス予測性を上げます。Basic サービス層は、低コストのデータベースのパフォーマンス要件を満たすように設計されています。
...
Microsoft はまた、自動 HA、負荷分散、組み込み管理など、さまざまな自動管理機能を Azure SQL Database に追加しています。
...
### 負荷分散 
 従来の仮想マシンとは異なり、Azure SQL Database にはまた、複数のコンピューターの間で負荷を自動分散する機構が含まれています。Load Balancer は動的にクラスターのリソース利用率を動的に監視し、データベース レプリカをクラスター内のコンピューターに移動し、多くのユーザーの間で負荷が公平に負担されるようにします。それにより、データベースのオンデマンド容量機能が強化されます。また、ロード バランサーがビジー状態のデータベースを互いから離して移行できるため、各データベースの容量要件を別々に考慮できます。多くのデータベースにまたがるソリューションを作成するとき、このロジックは、仮想マシン固有のサイズ制限ではなく、各データベースの容量ニーズに焦点を当てられる抽象化レイヤーを与えます。
...
Microsoft は Azure SQL Database の初回オンボードでお客様と緊密に連携し、サービスの利用状況を知り、将来の機能計画のために教訓をエンジニアリング チームにフィードバックしました。こうした取り組みを通じ、一連の機能が一部のお客様のニーズに合っていることを知りました。たとえば、新しいクラウド サービスを初めて開発するとき、多くの場合、オンデマンド容量と削減された管理オーバーヘッドの組み合わせが作業を簡素化し、コア ビジネスにかける時間に集中できます。厳しいパフォーマンス要件に関連する領域で課題に直面したお客様もいました。おそらくは、Azure SQL Database サービスでは現在のところ満たされない、大規模な多層データベース ソリューションの中央 API にサービスを提供するための要件でした。低い価格を重視し、高いパフォーマンス差異を積極的に受け入れるお客様もいれば、これらのデータベースの上にさらに上の価値を簡単に構築できるように特定のパフォーマンス保証に興味を示すお客様がいるとの報告を受けました。

あらゆるニーズに対応するために、Microsoft は、Basic、Standard、Premium という 3 つのサービス層を用意しました。各サービス階層には 1 つまたは複数のパフォーマンス レベルがあり、予測できる方法でデータベースを実行するためのパワーを与えます。このパワーの説明は「[DTU (Database Transaction Unit)](sql-database-technical-overview.md#understand-dtus)」にあります。
...
Premium サービス層のパフォーマンスに関する看板機能は、Premium データベースごとに秒単位でパフォーマンスを予測できることです。Premium サービス層を利用すれば、データベースのピーク負荷に基づいてデータベース アプリケーションのサイズを調整し、パフォーマンス差異に起因し、待ち時間が重要な操作で予想以上の時間が小規模クエリにかかる事態をなくすることができます。このモデルは、ピーク時のリソース ニーズ、パフォーマンス差異、クエリ待ち時間を高い確率で予測するために必要なアプリケーションの開発と製品検証の周期を大幅に単純化できます。また、一部のオンプレミス アプリケーションを大きな変更なしで移行できます。アプリケーションをもともと作成したときに想定された、従来の分離移行に近いからです。
...
Azure SQL Database では、各サーバーのマスター データベースの **sys.resource\_stats** ビューにアクティブなデータベース別のリソース利用情報が公開されます。テーブルのデータは 5 分おきに集計されます。Basic、Standard、Premium サービス層では、データをテーブルに表示するのに 5 分以上かかります。つまり、このデータはほぼリアルタイムの分析より過去の分析に適しています。**sys.resource\_stats** ビューに問い合わせると、データベースの最近の履歴が表示され、選択した予約で必要なときに望ましいパフォーマンスが届けられたか検証されます。次の例は、このビューのデータが公開されるしくみを示しています。
...
>[AZURE.NOTE]テーブルの列の一部は、スペースの関係上、切り詰められています。すべての出力を確認する場合、[sys.resource\_stats](https://msdn.microsoft.com/library/dn269979.aspx) に関するトピックを参照してください。

## リソース使用量を監視する方法

このセクションでは、Azure SQL Database のリソース利用率を監視し、現在のリソース利用率をさまざまなパフォーマンス レベルと比較する方法について説明します。

1. **sys.resource\_stats** カタログ ビューにはデータベース レベルの過去のリソース利用情報が追加され、さらに充実しています。たとえば、「userdb1」という名前のデータベースの先週のリソース利用率を調べる場合、次のクエリを実行できます。
...
### データベース間のシャーディング
Azure SQL Database は汎用ハードウェアで実行されるため、従来のオンプレミス SQL Server インストールと比べ、1 台のデータベースに対して容量限度が一般的に低くなります。そのため、Azure SQL Database でデータベース操作が 1 台のデータベースの上限に収まらないとき、多くのお客様がシャーディング手法を利用し、複数のデータベースにデータベース操作を分散しています。現在、Azure SQL Database でシャーディング手法を利用するお客様の大半は、1 つの次元のデータを複数のデータベースで分割します。この手法は、OLTP アプリケーションは、多くの場合、スキーマ内の 1 行またはほんの数行からなるグループにのみ適用されるトランザクションを実行することを理解した上で行われます。たとえば、(SQL Server 付属の従来のサンプル Northwind データベースのように) あるデータベースに顧客、注文、注文詳細が含まれる場合、注文と注文詳細が関連している顧客をグループにまとめ、1 つのデータベース内に留めることで、このデータを複数のデータベースに分割できます。アプリケーションはデータベース間で顧客を分割し、効果的に負荷を分散します。それにより、お客様がデータベース サイズの上限を回避できるだけでなく、個々のデータベースがその DTU に収まる限り、各パフォーマンス レベルの上限を大幅に超える作業負荷を Azure SQL Database で処理できます。
