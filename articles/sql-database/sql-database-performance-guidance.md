...
**ポイントインタイム リストア**は過去の特定の時点にデータベースを復元する機能です。何日前までさかのぼれるかはご利用のサービス層によって決まります。詳細については、「[ユーザー エラーからの Azure SQL Database の復旧](sql-database-user-error-recovery.md)」を参照してください。
