...
| ドキュメントの改善 | 詳細 |
| --- | --- |
| Hyperscale サービス レベルのドキュメント更新の GA リリース| 詳細については、「[最大 100 TB の Hyperscale サービス レベル](sql-database-service-tier-hyperscale.md)」を参照してください。|
|サーバーレス コンピューティング レベルのドキュメントのパブリック プレビュー リリース| 詳細については、「[SQL Database serverless (preview) (SQL Database サーバーレス (プレビュー))](sql-database-serverless.md)」を参照してください。|
| &nbsp; |
...
| サービスの機能強化 | 詳細 |
| --- | --- |
| マネージド インスタンスのパブリック エンドポイントのパブリック プレビューが開始| 詳細については、[パブリック エンドポイントで安全に Azure SQL Database マネージド インスタンスを使用する方法](sql-database-managed-instance-public-endpoint-securely.md)に関するページを参照してください。|
| マネージド インスタンスのタイム ゾーンのサポートのパブリック プレビューが開始| 詳細については、「[Azure SQL Database Managed Instance のタイム ゾーン](sql-database-managed-instance-timezone.md)」を参照してください。|
| マネージド ID を使用した Azure SQL Database のセキュリティ保護の第 2 回パブリック プレビュー リリース| 「[Securing Azure SQL Databases with managed identities just got easier](https://azure.microsoft.com/blog/securing-azure-sql-databases-with-managed-identities-just-got-easier/)」\(マネージド ID を使用した Azure SQL Databases のセキュリティ保護が簡単に\)を参照してください。|
| &nbsp; |
...
| マネージド インスタンスのパブリック エンドポイントのパブリック プレビューが開始| 詳細については、[パブリック エンドポイントで安全に Azure SQL Database マネージド インスタンスを使用する方法](sql-database-managed-instance-public-endpoint-securely.md)に関するページを参照してください。|
| マネージド インスタンスのタイム ゾーンのサポートのパブリック プレビューが開始| 詳細については、「[Azure SQL Database Managed Instance のタイム ゾーン](sql-database-managed-instance-timezone.md)」を参照してください。 |
| Azure SQL Database でのリソース ガバナンス | 詳細については、[Azure SQL Database でのリソース ガバナンス](https://azure.microsoft.com/blog/resource-governance-in-azure-sql-database/)に関するページを参照してください。 || &nbsp; |
...
| サービスの機能強化 | 詳細 |
| --- | --- |
| Azure SQL Database の読み取りスケールアウト サポートの一般提供リリース | 詳細については、[読み取りスケールアウト](sql-database-read-scale-out.md)に関する記事をご覧ください。|
| &nbsp; |
...
|SQL Database を Stream Analytics 用の参照データのソースとして使用できるようになりました。 | 詳細については、「[Azure Stream Analytics](https://azure.microsoft.com/services/stream-analytics/)」および「[Azure Stream Analytics now supports Azure SQL Database as reference data input](https://azure.microsoft.com/blog/azure-stream-analytics-now-supports-azure-sql-database-as-reference-data-input/)」\(Azure Stream Analytics で参照データ入力として Azure SQL Database のサポートを開始\)を参照してください。|
