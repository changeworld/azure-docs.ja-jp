...
たとえば、AlwaysOn はアクティブな geo レプリケーションに置き換えられます。 そのため、使用可能なグループに関連する Transact-SQL ステートメントは SQL Database でサポートされず、AlwaysOn に関連する動的管理ビューもサポートされていません。  
...
通常、SQL Server で廃止になった構文は SQL Database でサポートされていません。

## <a name="transact-sql-syntax-partially-supported-in-sql-database"></a>SQL Database で部分的にサポートされている Transact-SQL 構文
SQL Database では、該当 SQL Server 2016 Transact-SQL ステートメントに存在する引数の全部ではなく一部がサポートされています。 たとえば、`CREATE PROCEDURE` ステートメントは利用できますが、`CREATE PROCEDURE` のすべてのオプションを利用できるわけではありません。 ここで完全な構文を記述するとわかりづらく、冗長になります。 各ステートメントのサポートされる領域については、リンクされている構文トピックを参照してください。

- データベース: [CREATE](https://msdn.microsoft.com/library/dn268335.aspx)/[ALTER DATABASE](https://msdn.microsoft.com/library/ms174269.aspx)   
...
- テーブル: [CREATE](https://msdn.microsoft.com/library/dn305849.aspx)/[ALTER TABLE](https://msdn.microsoft.com/library/ms190273.aspx)   
...
- `HAS_DBACCESS`
- `KILL STATS JOB`
- `OPENQUERY`、`OPENROWSET`、`OPENDATASOURCE`、`BULK INSERT`、および 4 部構成の名前
