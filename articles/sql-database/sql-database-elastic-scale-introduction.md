...
クライアント ライブラリを使うサンプル アプリについては、「[Elastic Database ツールの概要](sql-database-elastic-scale-get-started.md)」をご覧ください。
