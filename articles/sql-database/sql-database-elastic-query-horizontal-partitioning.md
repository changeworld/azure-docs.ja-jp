...
シャード化されていないデータベースについては、「[(垂直にパーティション分割された) シャード化されたクラウド データベース間のクエリ](sql-database-elastic-query-vertical-partitioning.md)」を参照してください。
...
エラスティック クエリには、シャードへの直接アクセスを提供するストアド プロシージャも導入されています。このストアド プロシージャは **sp\_execute\_remote** と呼ばれ、リモート データベースでリモート ストアド プロシージャまたは T-SQL コードを実行するときに使用できます。使用できるパラメーターは次のとおりです。
