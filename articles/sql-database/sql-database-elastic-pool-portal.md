...
どの時点でも、特定のデータベースを選択してプールに含めることができます。新しくプールを作成する場合、Azure ではプールに含めることでメリットのあるデータベースを推奨し、含めるようマークすることをお勧めしています。Azure は、データベースの使用率の履歴を評価し、1 つのデータベースのパフォーマンス レベルを使用するよりもコスト効果が高い場合は、エラスティック プールを推奨します。エラスティック プールが推奨される場合、Azure は、プール DTU の推奨量、プール内の各データベースの最小/最大 DTU 設定、推奨されるデータベースの一覧を提供します。エラスティック プールの候補と見なされるデータベースは、14 日間以上存在し、Premium 価格レベルではないデータベースです (パブリック プレビューは Standard に制限されているため、Premium DB は有効と見なされません)。サーバーで使用可能なすべてのデータベースを追加したり、初期のリストで必要に応じてデータベースを選択、削除したりできます。
