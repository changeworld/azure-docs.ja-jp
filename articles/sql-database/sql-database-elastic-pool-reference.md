...
このリファレンスでは、エラスティック プールの記事やプログラミング情報のリンクと詳細が提供されています。

## 概要

エラスティック プールは、データベース スループット ユニット (DTU) と、複数のデータベースで共有される記憶域 (GB) のコレクションです。エラスティック データベースは、いつでも、プールに追加したり、プールから削除したりできます。プール内のエラスティック データベースでは、使用可能なリソースを解放するプールから必要なリソースのみが、それを必要とするアクティブなデータベース用にのみ使用されます。



## エラスティック プールを作成、管理するための前提条件


- エラスティック プールは、Azure SQL Database の V12 サーバーでのみ使用できます。   
- PowerShell とエラスティック プール用の REST API は、Azure リソース マネージャー (ARM) でのみサポートされています。サービス管理コマンド (RDFE) はサポートされていません。 
- エラスティック プールの作成と管理は、[Microsoft Azure ポータル](https:portal.azure.com) でのみサポートされています。 


## 現在のプレビューの制限

- 現在のプレビューのエラスティック プールに対する価格レベルは Standard です。  
...
| 記事 | 説明 |
| :-- | :-- |
| [SQL Database エラスティック プール](sql-database-elastic-pool.md) | エラスティック プールの概要 |
| [Azure ポータルを使った SQL Database エラスティック プールの作成と管理](sql-database-elastic-pool-portal.md) | Azure ポータルを使ったエラスティック プールの作成と管理方法 |
| [PowerShell を使った SQL Database エラスティック プールの作成と管理](sql-database-elastic-pool-powershell.md) | PowerShell コマンドレットを使ったエラスティック プールの作成と管理方法 |
...
プールの記憶域の上限を超えた場合、プール内のすべてのデータベースが読み取り専用になります。このような場合は、プール自体の DTU または記憶域を増やすか、プールから 1 つまたは複数のデータベースを移動します。プールの記憶域の制限を特定し、それを超えないようにするために、プールの記憶域使用率が事前設定の値を超えたときに発生させるアラートをポータルで作成することができます。
...
エラスティック プールには、Azure SQL Database V12 サーバーが必要です。サーバーは、リソース グループ内にあります。

- 各リソース グループには、最大で 800 台のサーバーを含めることができます。
- 各サーバーには、最大で 800 個のエラスティック プールを含めることができます。
- 各エラスティック プールには、最大で 100 個のデータベースを含めることができます。
