...
このチュートリアルでは、Microsoft Azure 管理ポータルを使用して、Azure SQL データ同期の基礎を学習します。
...
この手順が必要になるのは、同期グループにオンプレミス SQL Server データベースを含める場合だけです。同期グループにあるのが SQL Database のインスタンスのみである場合は、手順 4 をスキップします。
