...
## <a name="download-and-unblock-the-wingtip-tickets-saas-database-per-tenant-scripts"></a>Wingtip Tickets SaaS Database Per Tenant スクリプトのダウンロードとブロック解除
...
1. [Wingtip Tickets SaaS Database Per Tenant GitHub リポジトリ](https://github.com/Microsoft/WingtipTicketsSaaS-DbPerTenant)に移動します。
...
## <a name="working-with-the-wingtip-tickets-saas-database-per-tenant-powershell-scripts"></a>Wingtip Tickets SaaS Database Per Tenant PowerShell スクリプトの操作
...
デプロイには、接続先となる 2 つの SQL Database サーバー (*tenants1-dpt-&lt;ユーザー&gt;* サーバーと *catalog-dpt-&lt;ユーザー&gt;* サーバー) が最初に用意されています。 デモ接続を確実に成功させるために、どちらのサーバーにも、すべての IP を許可する[ファイアウォール規則](sql-database-firewall-configure.md)があります。


1. *SSMS* を開き、*tenants1-dpt-&lt;ユーザー&gt;.database.windows.net* サーバーに接続します。
...
4. 手順 2 と 3 を繰り返し、*catalog-dpt-&lt;ユーザー&gt;.database.windows.net* サーバーに接続します。
