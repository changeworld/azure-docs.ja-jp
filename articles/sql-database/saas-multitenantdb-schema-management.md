...
最初に、各テナント データベースに含まれている会場タイプを確認します。 SQL Server Management Studio (SSMS) のテナント データベースの 1 つに接続して、VenueTypes テーブルを調査します。  また、このテーブルは、データベース ページからアクセスする Azure Portal のクエリ エディターで照会できます。 

1. SSMS を開き、テナント サーバー *tenants1-dpt-&lt;user&gt;.database.windows.net* に接続します。
1. 現在、*Motorcycle Racing* および *Swimming Club* **が**含まれていないことを確認するために、*tenants1-dpt-&lt;user&gt;* サーバーで *contosoconcerthall*データベースを参照し、*VenueTypes* テーブルにクエリを実行します。
