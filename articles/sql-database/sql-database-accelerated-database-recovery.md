...
この機能のプレビュー期間中に高速データベース復元 (ADR) の詳細を確認したり、機能をお試しいただくには、[mailto:adr@microsoft.com](mailto:adr@microsoft.com) までメールでお問い合わせください。 メールには、(単一データベース、エラスティック プール、および Azure Data Warehouse の) 論理サーバーの名前を記載してください。 この機能はプレビュー版なので、運用サーバーで機能を試すことは避けてください。