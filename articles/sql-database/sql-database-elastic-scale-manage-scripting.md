...
2. すべての内容を抽出します。
    ...
    ![Extract all][1]
3. Elastic Scale クライアント DLL (Microsoft.Azure.SqlDatabase.ElasticScale.Client.dll) を取得し、手順 1. でダウンロードしたローカルの "ShardElasticityModule" フォルダーに次のファイルをコピーします。これは、2 つの方法で実行できます。1) DLL を Elastic Scale NuGet パッケージ [リンク]からダウンロードするか、2) Elastic Scale Starter Kit プロジェクト [リンク]から \bin\Debug\ に移動して DLL を取得します。
...
1. [Azure プレビュー機能のページ](http://azure.microsoft.com/services/preview/)に移動します。
...
3. **[ファイルの参照...]** をクリックし、前述の **ShardElasticityModule.zip** ファイルを見つけます。 
...
4. **[資格情報の追加]** をクリックします。 
    ...
    ![Add credential][9]
4. **[資格情報の種類]** として **[Windows PowerShell 資格情報]** を選択し、[名前] として **[ElasticScaleCredential]** を選択します。説明は省略できます。  
...
6. 変数アセットを作成するために、**[設定の追加]** をクリックし、**[変数の追加]** を選択します。 
    ...
    ![Add variable][7]
7. このチュートリアルでは、シャード マップ マネージャーとシャード化されたデータベースを配置する Azure SQL DB サーバーの完全修飾名を格納する変数を作成します。**[変数の型]** として **[文字列]** を選択し、「**SqlServerName**」と入力します。矢印をクリックして続行します。 
...
4. 残り 3 つの PowerShell Runbook (**ProvisionByDate.ps1**、**ProvisionBySize.ps1**、**ReduceServiceTier.ps1**) について、手順 2. と 3. を繰り返します。 
5. 次の手順に進みます。 
