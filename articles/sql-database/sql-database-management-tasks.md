...
この記事では、Azure SQL Database で単純な管理タスクを実行する方法を示します。

## Management Studio を使用して Azure 上の SQL Database に接続する方法

Management Studio は、単一のワークスペースで複数の SQL Server インスタンスと複数のサーバーを管理できる管理ツールです。オンプレミスの SQL Server インスタンスが既に存在する場合は、タスクをサイド バイ サイドで実行するために、オンプレミスのインスタンスと Azure 上の論理サーバーの両方に対して接続を開くことができます。

Management Studio には、構文チェック機能や、再利用の目的でスクリプトと名前付きクエリを保存する機能など、現時点では管理ポータルで使用できない機能が含まれています。SQL Database は、単純な表形式のデータ ストリーム (TDS) エンドポイントです。Management Studio を含め、TDS に対して使用できるどのツールも、SQL Database を操作するときに有効に使用できます。オンプレミス サーバーを対象にして作成したスクリプトは、SQL Database 論理サーバーで実行されます。

この手順では、Management Studio を使用して、Azure 上の論理サーバーに接続します。この手順では SQL Server Management Studio Version 2008 R2 または 2012 が必要です。Management Studio のダウンロードまたは接続に関するヘルプが必要な場合は、このサイトの [Management Studio を使用した SQL Database の管理][]に関するページをご覧ください。

接続する前に、ローカル システムのポート 1433 で送信要求を許可するファイアウォールの例外を作成する必要が生じることがあります。既定でセキュリティにより保護されたコンピューターでは通常、ポート 1433 が開かれていません。

## オンプレミス サーバーのファイアウォールの構成
...
## 方法: Azure SQL Database にログインとユーザーを追加する
...
作成するログインは、SQL Server 認証ログインであることが必要です。Windows ユーザーの ID を使用する、または ID を要求するスクリプトが既に存在している場合は、そのスクリプトは SQL Database で動作しません。
...
ここまでで、複数のログインを作成してテストしました。詳細については、「[SQL Database におけるデータベースとログインの管理][]」および「[動的管理ビューを使用した Windows Azure SQL Database の監視][]」を参照してください。

[SQL Database におけるデータベースとログインの管理]: http://msdn.microsoft.com/library/windowsazure/ee336235.aspx
[動的管理ビューを使用した Windows Azure SQL Database の監視]: http://msdn.microsoft.com/library/windowsazure/ff394114.aspx
[Management Studio を使用した SQL Database の管理]: http://www.windowsazure.com/develop/net/common-tasks/sql-azure-management/
