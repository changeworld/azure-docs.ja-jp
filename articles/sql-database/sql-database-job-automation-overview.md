...
- **SQL エージェント ジョブ**: 従来から使われている実績のある SQL Server ジョブ スケジューリング コンポーネントです。Managed Instance で利用できます。 SQL エージェント ジョブは、単一データベースでは利用できません。
- **Elastic Database ジョブ**: 1 つまたは多数の Azure SQL データベース上でカスタム ジョブを実行するジョブ スケジューリング サービスです。

SQL エージェント (SQL Database Managed Instance の構成要素としてオンプレミスで利用可能) と Database Elastic Database ジョブ エージェント (単一 SQL Database と SQL Data Warehouse で利用可能) の間には、いくつかの点で違いがあります。
