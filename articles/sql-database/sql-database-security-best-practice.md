...
> [!NOTE]
> 次で言及されています: FedRamp: AC-04、NIST: AC-5、ISO: A.6.1.2、PCI 6.4.2、SOC: CM-3、SDL-3

職務の分離では、機密性の高いタスクを、別々のユーザーに割り当てられる複数のタスクに分割する必要性が述べられています。これは、通常、データ侵害を防ぐために行います。
...
- db_owner データベース ロールのメンバーは、Transparent Data Encryption (TDE) のようなセキュリティ設定を変更したり、SLO を変更したりできるため、このメンバーシップの付与は慎重に行う必要があります。 一方、DB オプションの変更など、データベース設定を変更するような多くのタスクには、db_owner の特権が必要です。 どのソリューションでも監査が重要な役割を果たします。

- アクセス許可のみを使用して、db_owner がユーザー データを表示しないようにすることはできません。 データベース内に非常に機密性の高いデータがある場合は、Always Encrypted を使用すると、db_owner や他の DBA がそれを表示するのを安全に防止できます。
...
- コード変更のすべてのソースを必ず把握するようにします。コードは T-SQL スクリプトの場合や、ビュー、関数、トリガー、およびストアド プロシージャの形式で実行またはデプロイされるアドホック コマンドの場合があります。 また、SQL エージェント ジョブの定義 (ステップ) の一部で、SSIS パッケージ、Azure Data Factory、およびその他のサービス内から実行される場合もあります。
...
- マネージド インスタンスで、オンプレミス サーバーからの復元操作の結果としてデータベースが作成された場合は、元のデータベースの TDE 設定が使用されます。 元のデータベースで TDE が有効になっていない場合は、マネージド インスタンスに対して手動で TDE を有効にすることをお勧めします。
...
使用中のデータとは、SQL クエリの実行中にデータベース システムのメモリに格納されたデータです。 データベースに機密データが格納されている場合は、組織内の Microsoft オペレーターや DBA などの高い特権を持つユーザーが SQL Server プロセスのメモリからデータを抽出できないようにし、データベースに対してクエリを実行するときにプレーンテキスト データを表示できないようにする必要があります。

**実装方法**:

- [Always Encrypted](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-database-engine) を使用して、メモリ内にある場合や使用中であっても、機密データが Azure SQL Database にプレーンテキストで公開されないようにします。 これにより、データベース管理者 (DBA) とクラウド管理者 (または、高い特権を持つが未承認のユーザーを偽装できる悪意のあるアクター) からデータを保護し、データにアクセスできるユーザーをより細かく制御できます。

**ベスト プラクティス**:

- Always Encrypted は、保存データの暗号化 (TDE) や転送中データの暗号化 (SSL/TLS) の代用ではありません。また、パフォーマンスや機能の影響を最小限にするために、機密データ以外には使用しないでください。 保存時、転送中、使用中のデータを包括的に保護するには、TDE および TLS と組み合わせて Always Encrypted を使用することをお勧めします。 

- Always Encrypted を使用して悪意のある DBA からデータを保護する場合は、役割の分離を使って Always Encrypted キーを管理します。 役割の分離を使用して、セキュリティ管理者は物理キーを作成し、DBA は物理キーを記述して列マスター キーと列暗号化キーのメタデータ オブジェクトをデータベース内に作成します。 この処理中、セキュリティ管理者はデータベースにアクセスする必要はなく、DBA はプレーンテキストの物理キーにアクセスする必要はありません。 
...
- 複数のキー (列マスター キーまたは列暗号化キー) を使用した場合のトレードオフについて、慎重に検討してください。 キー管理コストを削減するために、キーの数は少なく保つようにしてください。 通常、(キー ローテーションの途中ではなく) 定常状態の環境では、それぞれ異なるキーを使用して異なるデータにアクセスするさまざまなユーザー グループがある場合を除き、データベースごとに 1 つの列マスターキーと 1 つの列暗号化キーで十分です。  
...
- Always Encrypted では、キー (および保護されたデータ) への一時的アクセスの付与は容易にはサポートされません。 たとえば、DBA とキーを共有し、DBA が機密および暗号化されたデータに対して何らかのクレンジング操作を実行できるようにする必要がある場合、DBA のデータへのアクセス権を確実に取り消す唯一の方法は、データを保護している列暗号化キーと列マスター キーの両方をローテーションすることですが、これは負荷の高い操作です。 

- 暗号化された列のプレーンテキスト値にアクセスするには、ユーザーは、列を保護する CMK にアクセスできる必要があります。これは、CMK が保持されているキー ストアに構成されています。 さらに、ユーザーには、[列マスター キー定義を表示する] と [列暗号化キー定義を表示する] のデータベース権限が必要です。
...
- または、Always Encrypted の使用を検討してください。ただし、以下に示す制限事項に注意してください。
...
- データの暗号化には AES (AES 256 を推奨) を使用します。 RC4、DES、TripleDES などのアルゴリズムは非推奨とされており、既知の脆弱性のため、使用すべきではありません。 
...
Always Encrypted を使用する場合は、Always Encrypted が、基本的に、使用中の機密データを Azure SQL Database の高い特権を持つユーザー (クラウド オペレーター、DBA) から保護するように設計されていることに留意してください。「[高い特権を持つ承認されていないユーザーから、使用中の機密データを保護する](#protect-sensitive-data-in-use-from-high-privileged-unauthorized-users)」を参照してください。 Always Encrypted を使用してアプリケーション ユーザーからデータを保護する場合は、次の課題に注意してください。

- 列暗号化キーと列マスター キーにアクセスするためのアクセス許可をユーザーに与えることによって機密データへのアクセス権をユーザーに付与した場合、そのアクセス権を確実に取り消すには、列暗号化キーのローテーションが必要になります。これは、列暗号化キーが保護しているすべての列の再暗号化が必要となる、負荷の高い操作です。 

- 既定で、Always Encrypted をサポートするすべての Microsoft クライアント ドライバーは、列暗号化キーのグローバル (アプリケーションごとに 1 つ) のキャッシュを保持しています。 クライアント ドライバーが、列マスター キーを保持するキー ストアに接触してプレーンテキストの列暗号化キーを取得すると、プレーンテキストの列暗号化キーがキャッシュされます。これにより、マルチユーザー アプリケーションのユーザーからデータを分離することが困難になります。 アプリケーションがキー ストア (Azure Key Vault など) と対話するときにエンド ユーザーを偽装すると、ユーザーのクエリによって列暗号化キーがキャッシュに取り込まれた後、同じキーを必要とするが、別のユーザーによってトリガーされる後続のクエリでは、キャッシュされたキーが使用されます。 ドライバーはキー ストアを呼び出さず、2 番目のユーザーが列暗号化キーにアクセスする権限を持っているかどうかはチェックされません。 その結果、ユーザーはキーにアクセスできなくても、暗号化されたデータを表示することができます。 マルチユーザー アプリケーション内でユーザーの分離を実現するには、列暗号化キーのキャッシュを無効にする必要があります。これにより、データの暗号化または暗号化解除の操作のたびにドライバーがキー ストアにアクセスする必要が生じるため、パフォーマンスのオーバーヘッドが増加します。
...
- (SQL アクセス許可、ロール、RLS を介した) 適切なアクセス制御ポリシーを使用して、マスクされた列で更新を行うユーザー アクセス許可を制限します。 列にマスクを作成しても、その列への更新は防止されません。 したがって、マスクされた列に対してクエリを実行すると、ユーザーはマスクされたデータを受け取りますが、書き込みアクセス許可があれば、同じユーザーがデータを更新できます。    

-  動的データ マスクは、マスクされた値の統計プロパティを保持しません。これは、クエリの結果 (たとえば、マスクされたデータのフィルター述語や結合を含むクエリ) に影響を与える可能性があります。
...
古いプロトコルと暗号スイートへの依存による、よく知られた脆弱性から、Azure SQL Database に接続しているクライアント コンピューターとアプリケーションを保護します。
...
- 「[TLS/SSL (Schannel SSP) の暗号スイート](https://docs.microsoft.com/windows/desktop/SecAuthN/cipher-suites-in-schannel)」で、クライアントで使用可能な暗号スイートを確認し、「[TLS 暗号スイートの順序の構成](https://docs.microsoft.com/windows-server/security/tls/manage-tls#configuring-tls-cipher-suite-order)」に従って明確に 3DES を無効にします。 

- マネージド インスタンスを使用している場合は、 **[プロキシ]** の接続の種類 (既定) を使用します。これにより、サーバー側から暗号化が実施されます。 **[リダイレクト]** の接続の種類は、現在、暗号化の実施をサポートしておらず、プライベート IP 接続でのみ使用可能です。 
  - 詳細については、[「Azure SQL の接続アーキテクチャ」の「接続ポリシー」](sql-database-connectivity-architecture.md#connection-policy)を参照してください。
  - 暗号化は、プロキシとリダイレクトの両方の接続の種類で実施されます。 

### <a name="minimize-attack-surface"></a>攻撃対象領域を最小限にする
Azure SQL Database のネットワーク アクセス制御を実装することにより、悪意のあるユーザーによって攻撃される可能性がある機能の数を最小限にします。
...
- 仮想ネットワークのサービス エンドポイントと仮想ネットワークのファイアウォール規則を使用します。
...
- (たとえば、プライベート データ パスを使用して) プライベート エンドポイントで接続することにより、Azure SQL Database へのアクセスを制限します。 
  - マネージド インスタンスは、外部アクセスを防ぐために、仮想ネットワーク内で分離することができます。 同じリージョン内の同じまたはピアリングされた仮想ネットワークに存在するアプリケーションとツールは、直接アクセスできます。 異なるリージョンにあるアプリケーションとツールは、VNet 間接続や ExpressRoute 回線のピアリングを使用して接続を確立できます。 ユーザーは、ネットワーク セキュリティ グループ (NSG) を使用して、ポート 1433 でのアクセスを、マネージド インスタンスへのアクセスを必要とするリソースのみに制限する必要があります。 
  - (単一データベースまたはエラスティック プールを含む) SQL Database サーバーの場合は、仮想ネットワーク内の SQL Database サーバーに専用のプライベート IP を提供する [Private Link](sql-database-private-endpoint-overview.md) 機能を使用します。 また、[仮想ネットワークのファイアウォール規則と共に仮想ネットワーク サービス エンドポイント](sql-database-vnet-service-endpoint-rule-overview.md)を使用して、SQL Database サーバーへのアクセスを制限することもできます。
...
- ASE を使用する Web アプリ、または SQL Database サーバー内のデータベースに接続する VNet に統合された Web アプリの場合、[VNet サービス エンドポイントと VNet ファイアウォール規則](sql-database-vnet-service-endpoint-rule-overview.md)を使用して特定の VNet とサブネットからのアクセスを制限し、 **[Azure サービスを許可する]** を [オフ] に設定できます。 また、プライベート データ パスで ASE をマネージド インスタンスに接続することもできます。  
...
- [Azure Security Center Standard レベル](../security-center/security-center-pricing.md)に切り替えることにより、サブスクリプション内の特定の SQL Database サーバーまたはマネージド インスタンス用、あるいはすべての SQL Database サーバーとマネージド インスタンス用に Azure SQL Database の [Advanced Data Security](sql-database-advanced-data-security.md#getting-started-with-ads)  を構成します。 

- 完全な調査エクスペリエンスを実現するには、 [SQL Database 監査](sql-database-auditing.md)を有効にしてデータベース イベントを追跡し、それらを Azure Storage アカウントまたは Azure Log Analytics ワークスペース内の監査ログに書き込むことをお勧めします。 

### <a name="audit-critical-security-events"></a>重要なセキュリティ イベントを監査する
データベース イベントを追跡することにより、データベース アクティビティを把握し、ビジネス上の懸念やセキュリティ違反の疑いを示す相違や異常についての洞察を得ることができます。 また、これにより、コンプライアンス基準への準拠を可能にし、促進します。 
...
- 既定で、監査ポリシーには、データベースに対するすべてのアクション (クエリ、ストアド プロシージャ、成功および失敗したログイン) が含まれます。その結果、大量の監査ログが生成される可能性があります。 ユーザーは、[PowerShell を使用してさまざまな種類のアクションおよびアクション グループの監査を構成](sql-database-auditing.md#subheading-7)して、監査されるアクションの数を制御し、イベント消失のリスクを最小化することをお勧めします。 これにより、ユーザーは真に必要な監査データのみを取り込むことができます。
...
職務の分離をサポートし、DBA を監査者から分離するには、ストレージ アカウントへのアクセスを制限する手段を講じることが重要です。 

**実装方法**:

- 監査ログを Azure Storage に保存する場合は、ストレージ アカウントへのアクセスの制御を使用して、ストレージ アカウントへのアクセスが最小限のセキュリティの原則に制限されていることを確認してください。
...
- 最初に、データベースで VA を実行し、セキュリティのベスト プラクティスに反する失敗したチェックを修正して、スキャンが "クリーン" になる (すべてのチェックに合格する) まで、許容できる構成のベースラインを設定して反復します。  

- 定期的な反復スキャンを 1 週間に 1 回実行するように構成し、関係者が概要のメールを受信するように構成します。 

- 毎週スキャンするたびに、VA の概要を確認します。 検出された脆弱性については、前回のスキャン結果からのドリフトを評価し、チェックを解決すべきかどうか、または構成の変更に正当な理由があるかどうかを判断します。   

- 該当する場合は、チェックを解決し、ベースラインを更新します。または、アクションを解決するためのチケット項目を作成し、解決されるまでそれらを追跡します。 
...
- [SQL データの検出と分類](sql-database-data-discovery-and-classification.md)を使用して、データベース内の機密データを検出、分類、ラベル付け、および保護します。 
  - SQL データの検出と分類ダッシュボードの自動検出によって作成される分類に関する推奨事項を表示し、関連する分類を受け入れて、永続的に機密データに分類ラベルのタグが付けられるようにします。 
...
- 機密データの検出規則を追跡し、分類の推奨列のドリフトを特定することにより、SQL 脆弱性評価で推奨される機密データの状態を継続的に監視します。  
...
- SQL 監査とデータ分類を組み合わせて使用します。 
  - [SQL Database 監査](sql-database-auditing.md)ログで、特に機密データへのアクセスを追跡し、アクセスされたデータやその機密ラベルなどの情報を表示できます (「[機密データへのアクセスの監査](sql-database-data-discovery-and-classification.md#subheading-3)」を参照)。 
...
**シナリオ 1**: Azure VM 上のアプリケーションが Azure SQL Database サーバー内のデータベースに接続します。 悪意のあるアクターが VM にアクセスして、不正に侵入します。 このシナリオにおけるデータ窃盗とは、承認されていない VM を使用する外部エンティティがデータベースに接続し、個人データをコピーし、それを BLOB ストレージや、別のサブスクリプションの別の SQL Database に格納することを意味します。
...
- Azure VM の NSG で許可と拒否の規則を組み合わせて使用して、VM からアクセスできるリージョンを制御します。 
- Azure SQL Database サーバー (単一データベースまたはエラスティック プールを含む) を使用している場合は、次を設定します。
...
- マネージド インスタンスの場合、既定でプライベート IP アクセスを使用することで、承認されていない VM によるデータ窃盗の最大の懸念に対応します。 マネージド インスタンスのサブネットに最も制限の厳しいポリシーを自動的に設定するサービスを提供するサブネットの委任機能をサブネットで有効にします。
...
多くのセキュリティ標準では、冗長性とフェールオーバーの機能を実装して単一障害点を回避することで実現される運用の継続性の観点からデータの可用性に対処しています。 災害シナリオでは、データおよびログ ファイルのバックアップを保持するのが一般的な方法です。 次のセクションでは、Azure に組み込まれている機能の概要と、特定のニーズを満たすために構成できる追加のオプションについて説明します。 
