...
> [!IMPORTANT]
> サンプル スクリプトについては、[アクティブ geo レプリケーションでの単一のデータベースの構成とフェールオーバー](scripts/sql-database-setup-geodr-and-failover-database-powershell.md)に関するページ、[アクティブ geo レプリケーションでのプールされたデータベースの構成とフェールオーバー](scripts/sql-database-setup-geodr-and-failover-pool-powershell.md)に関するページ、および単一データベースのフェールオーバー グループの構成とフェールオーバー (プレビュー) に関するページ (scripts/sql-database-setup-geodr-failover-database-failover-group-powershell.md) を参照してください。
>
