...
Wingtip Tickets アプリでは、各テナント データベースに、サポートされている一連の会場の種類が含まれています。 各会場には、ホストできるイベントの種類を定義し、アプリで使用される背景画像を決定する特定の会場の種類があります。 新しい種類のイベントをサポートするアプリケーションでは、この参照データを更新して、新しい会場の種類を追加する必要があります。  この演習では、すべてのテナント データベースに更新プログラムをデプロイして、次の 2 つの会場タイプを追加します: *Motorcycle Racing*、*Swimming Club*。
