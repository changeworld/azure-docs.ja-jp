...
このチュートリアルで使用されるサンプル データセットは、 **.bacpac** データベース バックアップ ファイルに保存されています。このファイルをダウンロードして使用します。 このデータセットは、[Transaction Processing Performance Council (TPC)](http://www.tpc.org/default.asp) が提供する [tpcx-bb](http://www.tpc.org/tpcx-bb/default.asp) データセットから派生したものです。
...
## <a name="separate-customers"></a>顧客を分類する
