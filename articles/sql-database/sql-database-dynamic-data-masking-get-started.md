...
動的データ マスク ポリシーは Azure プレビュー ポータルの中で設定します。設定は、データベースにアクセスするアプリケーションや他のクライアントによって使用されるセキュリティ保護接続文字列を使用することによって完了します。
