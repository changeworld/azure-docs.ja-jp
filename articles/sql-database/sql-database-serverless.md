...
#### <a name="cache-reclaiming"></a>キャッシュの回収
...
- キャッシュ サイズは、構成可能な最小仮想コアで定義されている最小メモリよりも小さくなることはありません。
...
最小仮想コア数の変更は、PowerShell の [Set-AzSqlDatabase](https://docs.microsoft.com/powershell/module/az.sql/set-azsqldatabase) コマンドと `MinVcore` 引数を使用して実行されます。

### <a name="autopause-delay"></a>自動一時停止遅延

自動一時停止遅延の変更は、PowerShell の [Set-AzSqlDatabase](https://docs.microsoft.com/powershell/module/az.sql/set-azsqldatabase) コマンドと `AutoPauseDelay` 引数を使用して実行されます。
