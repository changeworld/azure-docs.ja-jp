...
8. Q: 長期のリテンション期間は、エラスティック プールと連携しますか。
...
10. Q: データベースに対して TDE が有効化されています。 コンテナーで TDE を使用できますか。A: はい。TDE はサポートされています。 元のデータベースが存在しない場合でも、資格情報コンテナーからデータベースを復元できます。
