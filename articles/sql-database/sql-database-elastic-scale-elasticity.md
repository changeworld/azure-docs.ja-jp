...
**シャードの弾力性**により、アプリケーション開発者は必要に応じて、データベース リソースを動的に拡大および縮小することができ、アプリケーションのパフォーマンスを最大化してコストを最小限に抑えることが可能になります。Azure SQL Database の **Elastic Database ツール**と [Basic、Standard、および Premium の各サービス レベル](sql-database-service-tiers)を組み合わせることで、非常に魅力的な弾力性シナリオが実現します。Elastic Database ツールでは水平スケーリングが可能です。これは、データベース (また、["シャード" と呼びます](sql-database-elastic-scale-glossary.md)) の容量を拡大または縮小するために、これをシャード セットに追加したり、ここから削除したりする設計パターンの一種です。同様に、SQL Database のサービス層は、**垂直スケーリング**機能も提供します。これは、需要に応じて 1 つのデータベースのリソースをスケールアップまたはスケールダウンすることができる機能です。1 つのシャードの垂直スケーリングと多数のシャードの水平スケーリングを同時に実施することで、アプリケーション開発者にとって、パフォーマンス、容量、コスト最適化ニーズを満たす、非常に柔軟性の高い環境が実現します。
