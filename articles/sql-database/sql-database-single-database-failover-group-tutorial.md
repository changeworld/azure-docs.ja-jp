...
1. (省略可能) SQL Server の横にある星のアイコンを選択して **[SQL サーバー]** をお気に入りに追加し、左側のナビゲーション ウィンドウに追加します。 
    ...
    ![SQL Server を見つける](media/sql-database-single-database-create-failover-group-tutorial/all-services-sql-servers.png)

1. **[SQL サーバー]** を選択し、セクション 1 で作成したサーバー (`mysqlserver` など) を選択します。
