...
| **アクセス制御** | 少なくとも 1 つのアクセス制御を選択します。 | 条件が満たされた場合のポリシー プロセッサの対応を決める必要があります。 |
...
- フェーズ 1:セッション詳細の収集 
   - ポリシーの評価に必要なユーザーの場所やデバイス ID などのセッションの詳細を収集します。 
...
- フェーズ 2:適用 
   ...
   - ユーザーは、ポリシーが満たされるまで、フェーズ1で満たされていない追加の許可制御要件を次の順序で完了するように求められます。  
      - 多要素認証 
      - 承認されたクライアント アプリ/アプリ保護ポリシー 
      - マネージド デバイス (準拠または Hybrid Azure AD Join) 
      - 使用条件 
      - カスタム コントロール  
      - 許可の制御が満たされたら、セッション制御を適用します (アプリによって適用、Microsoft Cloud App Security、トークンの有効期間) 
