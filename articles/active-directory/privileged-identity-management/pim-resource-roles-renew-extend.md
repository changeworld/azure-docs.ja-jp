---
title: Azure リソース向けの Privileged Identity Management - ロールの延長と更新を行う | Microsoft Docs
...
---
# <a name="privileged-identity-management---resource-roles---extend-or-renew"></a>Privileged Identity Management - リソース ロール - 延長または更新

Azure リソース向けの PIM には、Azure リソースへのアクセスと割り当てのライフサイクルを管理するための新しいコントロールが導入されています。 管理者は、[開始日時] と [終了日時] プロパティを使用してメンバーシップを割り当てることができます。 割り当ての終了日時が近づくと、PIM は、適切なアクセスが維持されることを保証するために、影響を受けるメンバー (ユーザーまたはグループ) と管理者に電子メール通知を送信します。 アクションがないためにアクセスが延長されなかった場合は、割り当てを更新して、最大 30 日間、期限切れの状態のままで表示できます。
...
管理者は、期限が迫っているか期限切れになっているロールのメンバーが期限の延長または更新を要求したときに通知を受信します。 特定の管理者が要求を解決すると、他のすべての管理者に解決の決定 (承認または拒否) が通知され、要求したメンバーに決定が通知されます。 
...
左側のナビゲーション メニューから [保留中の要求] タブに移動して、要求の状態を確認します。要求をキャンセルすることもできます。
...
電子メール内のリンクに加え、管理者は、PIM の管理ポータルに移動し、左側のナビゲーション メニューから [申請の承認] を選択することで、要求を承認または拒否できます。
...
要求を承認してロールの割り当てを延長するとき、リソース管理者は、新しい開始日時、終了日時、および割り当ての種類を選択できます。 管理者が特定のタスク を完了するためのアクセス制限 を指定する (たとえば 1 日だけ延長する) 場合は、割り当ての種類の変更が必要になることがあります。 この例では、管理者は、割り当ての種類を [有資格] から [アクティブ] に変更して、アクティブ化の必要がないアクセスを要求者に提供できます。
...
ロールのメンバーシップを延長するには、PIM でリソースのロールまたはメンバー ビューに移動します。 延長する必要があるメンバーを探し、[アクション] 列の [延長] をクリックします。
...
リソースにアクセスできなくなったメンバーは、PIM の左側のナビゲーションで [自分のロール] に移動し、[Azure リソースのロール] セクションの [期限切れのロール] タブを選択することで、最大 30 日間分の有効期限が切れた割り当て履歴にアクセスできます。
...
一覧のいずれかのロールの割り当ての更新を要求するには、[更新] アクションを選択し、要求の理由を入力します。 詳しい理由に加えて延長期間を入力すると、リソース管理者が承認するか拒否するかを決定する際に役立ちます。
...
要求を承認してロールの割り当てを更新する場合、リソース管理者は、新しい開始日時、終了日時、および割り当ての種類を選択する必要があります。 

### <a name="admin-renew"></a>管理者による更新

リソース管理者は、リソースの左側のナビゲーション メニューの [メンバー] タブまたはリソース ロールの [期限切れのロール] タブで、有効期限が切れているロールの割り当てを更新できます。
