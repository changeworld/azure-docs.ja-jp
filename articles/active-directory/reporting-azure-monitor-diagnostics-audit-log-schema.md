...
ms.locfileid: "39240471"
---
# <a name="interpret-the-azure-active-directory-audit-logs-schema-in-azure-monitor-preview"></a>Azure Monitor の Azure Active Directory 監査ログ スキーマを解釈する (プレビュー)

この記事では、Azure Monitor の Azure AD 監査ログ スキーマについて説明します。 次の 2 つの例のように、個々のログ エントリは JSON BLOB 形式のテキストとして保存されます。 
...
| category | 現在サポートされている値は *Audit* のみです |
| tenantId | ログに関連付けられたテナント GUID |
| resultType | 操作の結果は、*Success* または *Failure* です |
| resultSignature |  この値はマップされていないため、このフィールドを無視しても問題ありません。 | 
| resultDescription | 結果の追加の説明です (使用可能な場合) | 
...
| identity | 要求を行う場合に提示されたトークンからの ID です。 ユーザー アカウント、システム アカウント、またはサービス プリンシパルです。 |
| level | メッセージの種類。 監査ログの場合、この値は常に *Informational* です |
| location | データセンターの場所 |
| properties | サポートされているプロパティのうち、監査ログに関連するプロパティを一覧表示します。 詳細については、以下の表を参照してください。 | 


| プロパティ名 | 説明 |
|---------------|-------------|
| AuditEventCategory | 監査イベントの種類。 *User Management*、*Application Management* などを指定できます。|
| Identity Type | *Application* または *User* です |
| 操作の種類 | *Add*、*Update*、*Delete*、または *Other* を指定できます。 |
| Target Resource Type | 操作が実行されたターゲット リソースの種類を指定します。 *Application*、*User*、*Role*、*Policy* を指定できます。 | 
| Target Resource Name | ターゲット リソースの名前。 たとえば、アプリケーション名、ロール名、ユーザー プリンシパル名、またはサービス プリンシパル名です。 |
