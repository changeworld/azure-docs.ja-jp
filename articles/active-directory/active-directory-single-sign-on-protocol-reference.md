<properties
	pageTitle="Azure でのシングル サインオンの SAML プロトコル | Microsoft Azure"
...
| ForceAuthn | 省略可能 | 指定する場合は、false にする必要+があります。それ以外の値を指定すると、エラーが発生します。|
| IsPassive | 省略可能 | 指定する場合は、false にする必要+があります。それ以外の値を指定すると、エラーが発生します。 |  
...
`SPNameQualifer` 属性は含めないでください。Azure AD は `AllowCreate` 属性を無視します。
...
- **Name 要求**: `Name` 属性の値 (`http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name`) は、認証されたユーザーのユーザー プリンシパル名です (例: `testuser@managedtenant.com`)。
- **ObjectIdentifier 要求**: `ObjectIdentifier` 属性の値 (`http://schemas.microsoft.com/identity/claims/objectidentifier`) は、Azure AD で認証されたユーザーを表すディレクトリ オブジェクトの `ObjectId` です。`ObjectId` は、認証されたユーザーの変更不可能で、グローバルに一意であり、再利用が安全な識別子です。
