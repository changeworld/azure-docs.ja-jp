...
このチュートリアルでは、フェデレーションを使用してハイブリッド ID 環境を作成する手順を説明します。  この環境を使用して、ハイブリッド ID の動作をテストしたり、ハイブリッド ID について理解を深めたりできます。
...
ハイブリッド ID 環境を稼働させるには、まず、オンプレミスの Active Directory サーバーとして使用する仮想マシンを作成する必要があります。  

>[!NOTE]
>ホスト コンピューター上で PowerShell のスクリプトを実行したことがない場合は、スクリプトを実行する前に、PowerShell で `Set-ExecutionPolicy remotesigned` を実行し、[はい] と答える必要があります。
...
## <a name="install-active-directory-pre-requisites"></a>Active Directory の前提条件をインストールする
...
1. PowerShell ISE を管理者として起動します。
2. `Set-ExecutionPolicy remotesigned` を実行し、[すべて続行] を選択します。  Enter キーを押します。
3. 次のスクリプトを実行します。
...
次に、AD FS から使用される SSL 証明書を作成します。  これは自己署名証明書であり、テスト目的にのみ使用されます。  Microsoft は、運用環境で自己署名証明書を使用することをお勧めしません。 以下の手順を実行します。
...
テナントと全体管理者が作成されたので、Azure がカスタム ドメインを確認できるようにカスタム ドメインを追加する必要があります。  以下の手順を実行します。

1. [Azure portal](https://aad.portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview) に戻って、**[すべてのユーザー]** ブレードが閉じていることを確認します。
2. 左側で **[カスタム ドメイン名]** を選択します。
3. **[カスタム ドメインの追加]** を選択します。</br>
![フェデレーション](media/tutorial-federation/custom1.png)</br>
4. **[カスタム ドメイン名]** で、ボックスにカスタム ドメインの名前を入力し、**[ドメインの追加]** をクリックします。
5. [カスタム ドメイン名] 画面では、TXT 情報または MX 情報が表示されます。  この情報を、ドメインのドメイン レジストラーの DNS 情報に追加する必要があります。  そのためには、ドメイン レジストラーに移動して、ドメインの DNS 設定の TXT 情報または MX 情報を入力します。  これにより、Azure がドメインを確認できるようになります。  Azure の確認には最大で 24 時間かかる可能性があります。  詳細については、[カスタム ドメインの追加](../../active-directory/fundamentals/add-custom-domain.md)に関するドキュメントを参照してください。</br>
![フェデレーション](media/tutorial-federation/custom2.png)</br>
6. [確認] ボタンをクリックして、確認されていることを確かめます。</br>
![フェデレーション](media/tutorial-federation/custom3.png)</br>
...
6. [ユーザー サインイン] 画面で **[AD FS とのフェデレーション]** を選択し、**[次へ]** をクリックします。
![フェデレーション](media/tutorial-federation/fed1.png)

1. [Azure AD に接続] 画面で、先ほど作成した全体管理者のユーザー名とパスワードを入力し、**[次へ]** をクリックします。
...
9. [ドメイン管理者の資格情報] ページで、contoso\Administrator のユーザー名とパスワードを入力し、**[次へ]** をクリックします。
...
2. AD FS サービス アカウント画面で、contoso\Administrator のユーザー名とパスワードを入力し、**[次へ]** をクリックします。
3. [Azure AD ドメイン] 画面で、ドロップ ダウンから検証済みのカスタム ドメインを選択し、**[次へ]** をクリックします。
4. [構成の準備完了] 画面で、 **[インストール]** をクリックします。
5. インストールが完了したら、 **[終了]** をクリックします。
6. インストールの完了後、Sychronization Service Manager または同期規則エディターを使用する前に、サインアウトしてもう一度サインインします。
...
2. 新しいテナントに作成されたユーザー アカウントでサインインします。  user@domain.onmicrosoft.com の形式を使用してサインインする必要があります。 ユーザーがオンプレミスでのサインインに使用するものと同じパスワードを使用します。
