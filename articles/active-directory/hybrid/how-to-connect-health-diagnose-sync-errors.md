...
1. 診断の最後に **[閉じる]** ボタンが表示される場合は、回答に基づいてポータルから使用可能な簡易的な修正はありません。 最後の手順に示したソリューションを参照してください。 オンプレミスからの修正はまだ有効です。 **[閉じる]** ボタンを選択します。 現在の同期エラーの状態が、**[手動修正が必要]** に切り替わります。 現在の同期サイクルの間、状態は変わりません。
