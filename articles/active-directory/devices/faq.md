...
**Q:最近、デバイスを登録しました。Azure portal のユーザー情報にデバイスが表示されないのはなぜですか? または、ハイブリッド Azure AD 参加済みデバイスのデバイス所有者が N/A とマークされるのはなぜですか?**
...
**A:** これは設計によるものです。 デバイスは、クラウドのリソースにアクセスできなくなります。 
...
**A:** はい。 Windows には、前にログインしていたユーザーがネットワークに接続していなくてもすばやくデスクトップにアクセスできるようにする、キャッシュされたログオンの機能があります。 Azure AD でデバイスが削除または無効化されても、Windows デバイスにはわかりません。 そのため、以前にログインしていたユーザーは、キャッシュされたログオンで引き続きデスクトップにアクセスできます。 ただし、デバイスは削除または無効化されているため、ユーザーはデバイス ベースの条件付きアクセスによって保護されているすべてのリソースにアクセスできません。 

まだログインしたことのないユーザーは、キャッシュされたログオンが有効になっていないため、デバイスにアクセスできません。 

---

**Q:無効にされたか削除されたユーザーは、Azure AD 参加済みデバイスにサインインできますか?**
**A:** はい、ただし限られた期間だけです。 Azure AD でユーザーが削除または無効化されても、Windows デバイスはそのことをすぐには認識しません。 そのため、以前にログインしていたユーザーは、キャッシュされたログオンでデスクトップにアクセスできます。 デバイスがユーザーの状態を認識すると (通常は 4 時間未満で)、Windows はそれらのユーザーによるデスクトップへのアクセスをブロックします。 ユーザーが Azure AD で削除または無効化されると、すべてのトークンが取り消されて、すべてのリソースにアクセスできなくなります。 

以前にログインしたことのない削除または無効化されたユーザーは、キャッシュされたログオンが有効になっていないため、デバイスにアクセスできません。 
...
**A:** 詳細については、記事 https://docs.microsoft.com/windows/client-management/connect-to-remote-aadj-pcを参照してください。
...
**A:** ユーザーがローカルのあらかじめ登録された Administrator アカウントを使用してデバイスにログインしていることが原因と考えられます。 Azure Active Directory Join を使用してセットアップを完了する前に、別のローカル アカウントを作成してください。 
