...
Azure Active Directory (Azure AD) のセキュリティの既定値群を使用すると、セキュリティを強化することが容易になり、組織のセキュリティ保護に役立ちます。 セキュリティ既定値には、一般的な攻撃に対して事前に構成されたセキュリティ設定が含まれています。 

Microsoft は、誰もがセキュリティの既定値群を利用できるよう努めています。 目標は、すべての組織が追加の費用なしで基本レベルのセキュリティを確実に有効にできるようにすることです。 セキュリティ既定値は、Azure portal で有効にします。
