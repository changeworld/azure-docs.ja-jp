...
セキュリティの既定値群を有効にした直後の 14 日以内に、一部のユーザーが出勤またはログインしない場合があることは理解しています。 ユーザー全員に Multi-Factor Authentication の登録のための十分な時間を与えられるように、14 日の期間はユーザーごとに固有です。 14 日の期間は、セキュリティ デフォルトが有効になった後、それぞれのユーザーの対話型サインインが最初に成功した時点から始まります。
