...
フィルタ―処理を構成した後で作成された新しい OU は、既定では同期されます。 これは、ボックス内のチェックマークによって示されます。 サブ OU は、明示的に選択解除することで選択解除できます。 この操作を有効にするには、白地に青のチェックマークが表示された状態になるまでボックスをクリックします (既定の状態)。 その後、同期しないサブ OU を選択解除します。

すべての サブ OU が同期される場合は、白地に青のチェックマークが表示されたボックスになります。  
...
Azure AD Connect インストール ウィザードは、常にこの構成を作成します。

### <a name="do-not-synchronize-new-ous"></a>新しい OU を同期しない
フィルター処理の構成が完了した後で作成された新しい OU を同期しないように同期エンジンを構成できます。 これは、グレー地でチェックマークの表示がないボックスの UI で示されます。 この操作を有効にするには、白地でチェックマークの表示がない状態になるまでボックスをクリックします。 その後、同期するサブ OU を選択します。
...
![](./media/active-directory-aadconnectsync-configure-filtering/scope.png) これは、**(department = IT) OR (department = Sales AND c = US)** という意味になります。
