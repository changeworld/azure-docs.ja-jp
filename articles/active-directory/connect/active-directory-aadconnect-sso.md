...
シングル サインオンは、[パスワード ハッシュ同期](active-directory-aadconnectsync-implement-password-synchronization.md)または[パススルー認証](active-directory-aadconnect-pass-through-authentication.md)のいずれかを使用して有効にする、Azure Active Directory Connect のオプションです。  有効にすると、ユーザーが会社のコンピューターを使用して企業ネットワークに接続しているときに、Azure Active Directory (Azure AD) やその他のクラウド サービスにパスワードを入力することなくサインインできます。  

シングル サインオンを有効にすると、Azure Active Directory サービス (Office 365、Microsoft Intune、CRM Online、SaaS サービスなど) に接続済みのリソースにアクセスする場合に、パスワードを入力する代わりに、オンプレミス Active Directory から自動的に取得した Kerberos チケットを使用することになります。
...
- イントラネット ゾーンの一部としてクラウドに Kerberos エンドポイントを定義している
...
シングル サインオンは Web ブラウザー ベースのクライアントと、Windows などの Kerberos 認証に対応したコンピューターの最新の認証をサポートする Office クライアントでサポートされています。  次の表は、オペレーティング システム別のブラウザー ベースのクライアントの詳細を示しています。
...
まず、ユーザーがリソースへのアクセスを試みます。  リソースには、コンピューターか URL のいずれかを指定できます。  Azure AD の場合は、SharePoint Online などのサービスの URL になります (以下では、「Azure AD リソース」として説明しています)。

1.  Azure AD は、Kerberos チケットを提供するよう、401 認証エラーを通じてクライアントに要求します。
2.  クライアントは、Active Directory から Azure AD リソースのチケットを要求します。 
3.  Active Directory は、Azure AD リソースに関連付けられたコンピューター アカウントを検索し、コンピューター アカウントの秘密キーで暗号化された Kerberos チケットをクライアントに返します。 チケットには、コンピューターにサインインしているユーザーの ID が含まれます。
...
Azure AD Connect では、パススルー認証またはパスワード ハッシュ同期を使って、シングル サインオンを簡単に有効にできます。  コンピューター アカウントで Kerberos サービス プリンシパル名 (SPN) の構成を許可するには、同期する各フォレスト内のいずれかのドメインに対するドメイン管理者権限が必要です。  ユーザー名とパスワードは Azure AD Connect や Azure AD には保存されず、この操作のみに使用されます。

Azure AD Connect をインストールするときにカスタム インストールを選択すると、ユーザーのサインイン ページでシングル サインオンを構成できます。 詳細については、「[Azure AD Connect のカスタム インストール](active-directory-aadconnect-get-started-custom.md)」を参照してください。
