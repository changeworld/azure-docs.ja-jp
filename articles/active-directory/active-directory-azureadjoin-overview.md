...
Azure AD Join は、クラウド優先またはクラウドのみの企業 (一般に、オンプレミスの Windows Server Active Directory インフラストラクチャを所有していない中小企業) 向けです。 とは言っても、Azure AD Join は従来のドメイン参加を実行できないデバイス (モバイル デバイスなど) 上で、または主に Azure AD と統合された Office 365 やその他の SaaS アプリにアクセスする必要のあるユーザー向けに、大規模な組織でも使用できます。
...
| --- | --- | --- |
| ユーザーは、仕事用の資格情報を使って Windows にサインインできます (従来の方法)。 |ユーザーは、Azure AD で管理されている仕事用の資格情報を使って Windows にサインインできる。 これは、次の 3 つのケースで会社のデバイスに該当します。 <ol><li>組織がオンプレミスの Active Directory を所有していない (小規模企業など)。</li><li>組織がすべてのユーザー アカウントを Active Directory に作成するとは限らない (たとえば、学生、コンサルタント、または季節従業員のアカウントは Active Directory に作成されない)。</li><li>組織が、モバイル SKU を実行している電話やタブレットなどの (オンプレミスの) ドメインに参加できない会社のデバイス (工場/小売部門に携帯されるセカンダリ デバイスなど) を所有している。</li></ol> Azure AD Join は、管理された組織とフェデレーションされた組織の両方で会社のデバイスの参加をサポートしています。 |ユーザーは、個人の Microsoft アカウントの資格情報を使って Windows にサインインします (変更なし)。 |
