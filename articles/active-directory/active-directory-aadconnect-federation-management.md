<properties
	pageTitle="Azure AD Connect を使用した AD FS の管理とカスタマイズ | Microsoft Azure"
...
# Azure AD Connect を使用した AD FS の管理とカスタマイズ

この記事では、Azure AD Connect を使って実行できるさまざまな AD FS 関連のタスクと、AD FS ファームの完全な構成に必要になる可能性があるその他の一般的な AD FS タスクについて説明します。
...
### 新しい AD FS WAP サーバーの追加

> [AZURE.NOTE] Azure AD Connect で WAP サーバーを追加するには、PFX 証明書ファイルが必要です。そのため、この操作を実行できるのは、Azure AD Connect を使用して AD FS ファームを構成した場合のみです。
...
次のページでは、WAP として追加するサーバーを追加します。WAP サーバーはドメインに参加している場合もあれば参加していない場合もあるため、ウィザードでは、追加されているサーバーの管理資格情報を入力するよう求められます。
...
ドメインを選択すると、ウィザードによって実行されるさらなるアクションと構成の影響に関して適切な情報が表示されます。Azure AD でまだ確認されていないドメインを選択すると、ドメインを確認するのに役立つ情報が表示される場合もあります。ドメインを確認する方法の詳細については、[Azure Active Directory でのカスタム ドメイン名の追加と確認](active-directory-add-domain-add-verify-general.md)に関するページを参照してください。
...
**規則 3: ms-ds-consistencyguid を不変 ID として発行する (存在する場合)** c:[Type == "http://contoso.com/ws/2016/02/identity/claims/msdsconcistencyguid"] => issue(Type = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier", Value = c.Value);
