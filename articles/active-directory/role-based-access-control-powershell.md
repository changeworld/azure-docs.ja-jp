...
割り当てるスコープ: スコープには 3 つのレベルがあります。 - 現在のサブスクリプション - リソース グループ (リソース グループの一覧を取得するには、`PS C:\> Get-AzureResourceGroup` と入力します) - リソース (リソースの一覧を取得するには、`PS C:\> Get-AzureResource` と入力します)
