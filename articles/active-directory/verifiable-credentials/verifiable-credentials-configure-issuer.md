...
> [!div class="checklist"]
>
> - 検証可能な資格情報構成ファイルを格納するための Azure Blob Storage を設定する。
...
次の図は、Azure AD の検証可能な資格情報アーキテクチャと構成するコンポーネントを示しています。
...
Azure Blob Storage は、Microsoft のクラウド用オブジェクト ストレージ ソリューションです。 Azure AD の検証可能な資格情報サービスでは、検証可能な資格情報の発行時に、検証可能な資格情報構成ファイルを格納するために [Azure Blob Storage](/azure/storage/blobs/storage-blobs-introduction) が使用されます。

次の手順に従って、Azure Blob Storage を作成して構成します。

1. Azure Blob Storage アカウントをお持ちでない場合は、[ストレージ アカウントを作成します](/azure/storage/common/storage-account-create)。
1. ストレージ アカウントを作成したら、コンテナーを作成します。 ストレージ アカウントの左側のメニューで、 **[データ ストレージ]** セクションまでスクロールし、 **[コンテナー]** を選択します。
1. **[+ コンテナー]** ボタンを選択します。
1. 新しいコンテナーの **[名前]** を入力します。 コンテナー名は小文字である必要があり、英文字または数字で始まる必要があり、英文字、数字、ダッシュ (-) 文字のみを含めることができます。 例: *vc-container*。
1. **[パブリック アクセス レベル]** は **[プライベート (匿名アクセスはありません)]** に設定します。
1. **［作成］** を選択します  

    次のスクリーンショットは、コンテナーを作成する方法を示しています。  
...
コンテナーを作成したら、サインインしたユーザーに適切なロール割り当てを付与して、ユーザーが Storage Blob 内のファイルにアクセスできるようにします。

1. コンテナーの一覧から *vc-container* を選択します。
...
1. **[ロールの割り当ての追加]** ページで、次の手順を実行します。
    ...
    1. 次に、これらの手順を実行するために使用しているアカウントを検索して選択します。

        ![新しいロールの割り当てを設定する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/add-role-assignment-container.png)

>[!IMPORTANT]
>既定では、コンテナーの作成者には所有者ロールが割り当てられます。 所有者ロールのままでは不十分です。 アカウントにはストレージ BLOB データ閲覧者ロールが必要です。 詳細については、「[Azure portal を使用して BLOB とキュー データへのアクセスのための Azure ロールを割り当てる](/azure/storage/blobs/assign-azure-role-data-access)」を参照してください。

### <a name="upload-the-configuration-files"></a>構成ファイルをアップロードする

Azure AD の検証可能な資格情報サービスでは、規則ファイルと表示ファイルの 2 つの JSON 構成ファイルが使用されます。 

- 規則ファイルには、検証可能な資格情報の重要なプロパティが記述されています。 特に、検証可能な資格情報を発行する前に、サブジェクト (ユーザー) が指定する必要がある要求について記述されています。 
- 表示ファイルは、資格情報のブランド化とクレームのスタイル設定を制御します。

このセクションでは、サンプルの規則と表示のファイルをストレージにアップロードします。 これらのファイルをカスタマイズする方法の詳細については、「[検証可能な資格情報をカスタマイズする方法](credential-design.md)」を参照してください。
...
1. **[アップロード]** ボタンを選択してアップロード ブレードを開き、ローカル ファイル システムを参照して、アップロードするファイルを見つけます。 **VerifiedCredentialExpertDisplay.json** と **VerifiedCredentialExpertRules.json** のファイルを選択します。 次に、 **[アップロード]** を選択して、コンテナーにファイルをアップロードします。

## <a name="create-the-verifiable-credential-expert-card-in-azure"></a>Azure で検証可能な資格情報エキスパート カードを作成する

この手順では、Azure AD の検証可能な資格情報を使用して検証可能な資格情報エキスパート カードを作成します。 検証済み資格情報を作成すると、Azure AD テナントで、検証可能な資格情報の発行プロセスを開始するユーザーにこの資格情報を発行できるようになります。

1. [Azure portal](https://portal.azure.com/) のメインの検索入力を使用して、**検証可能な資格情報** を検索します。 次に、 **[検証可能な資格情報 (プレビュー)]** を選択します。
1. [テナントを設定](verifiable-credentials-configure-tenant.md)すると、 **[新しい資格情報の作成]** のオプションが画面に表示されます。 開いていない場合、またはさらに資格情報を作成する場合は、左側のメニューで **[資格情報]** を選択します。 次に、 **[+ 資格情報]** を選択します。
1. [新しい資格情報の作成] ページで、次の操作を行います。

    1. 資格情報の **[名前]** に「**VerifiedCredentialExpert**」と入力します。 この名前は検証可能な資格情報を識別するためにポータル内で使用されます。 これは、検証可能な資格情報のコントラクトの一部として含まれます。

    1. Azure Blob Storage を作成したした Azure AD **サブスクリプション** を選択します。

    1. **[Display file]\(表示ファイル\)** で、**表示ファイルを選択** します。 [ストレージ アカウント] セクションで、**vc-container** を選択します。 次に **VerifiedCredentialExpertDisplay.json** ファイルを選択し、 **[選択]** をクリックします。

    1. **[規則ファイル]** で **規則ファイルを選択** します。 [ストレージ アカウント] セクションで、**vc-container** を選択します。 次に **VerifiedCredentialExpertRules.json** ファイルを選択し、 **[選択]** をクリックします。

    次のスクリーンショットでは、新しい資格情報を作成する方法を示しています。

    ![新しい資格情報を作成する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/how-create-new-credential.png)

1. [Create a new credential]\(新しい資格情報の作成\) 画面で、 **[作成]** を選択します。

## <a name="gather-credentials-and-environment-details-to-set-up-your-sample-application"></a>資格情報と環境の詳細を収集して、サンプル アプリケーションを設定する

新しい資格情報を作成したので、お使いの環境と作成した資格情報に関する情報を収集します。 これらの情報は、サンプル アプリケーションを設定するときに使用します。

1. 検証可能な資格情報から **[資格情報]** を選択し、資格情報の一覧から、前に作成した VerifiedCredentialExpert を選択します。

    ![新しく作成した検証済み資格情報を選択する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/select-verifiable-credential.png)
...
1. **テナント ID** をコピーして、後で使うために記録します。 次のスクリーンショットは、必要な値をコピーする方法を示しています。
...
このサンプル アプリケーションは .NET で利用でき、コードは GitHub リポジトリに保持されています。 [こちら](https://github.com/Azure-Samples/active-directory-verifiable-credentials-dotnet)の GitHub からサンプル コードをダウンロードするか、リポジトリを自分のローカル コンピューターにクローンします。
...
## <a name="issuing-your-first-verified-expert-card"></a>最初の検証済みエキスパート カードを発行する

これで、サンプル アプリケーションを実行して、最初の検証済みエキスパート カードを発行する準備ができました。
...
1. Web ブラウザーから **[Get Credential]\(資格情報の取得\)** を選択します。
     ...
     ![サンプル アプリから資格情報の取得を選択する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/get-credentials.png)
...
1. 現時点では、このアプリまたは Web サイトが危険である可能性があるという警告メッセージが表示されます。 **[詳細設定]** を選択します。

     ![危険な認証アプリの警告で [詳細設定] を選択する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/at-risk.png)

1. 危険な Web サイトの警告で、 **[Proceed anyways (unsafe)]\(このまま続行 (安全ではありません)\)** を選択します。 この警告が表示されるのは、ドメインが DID にリンクされていないためです。 ドメインを検証するには、「[ドメインを分散識別子 (DID) にリンクする](how-to-dnsbind.md)」の記事のガイダンスに従います。 このチュートリアルでは、ドメインの登録をスキップし、 **[Proceed anyways (unsafe)]\(このまま続行 (安全ではありません)\)** を選択します。

     ![危険な警告を続行する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/proceed-anyway.png)

1. QR コードをスキャンした画面に表示される PIN コードを入力するように求めるメッセージが表示されます。 この PIN は、発行ペイロードに保護レイヤーを追加するために使用されます。 PIN コードは、発行 QR コードが表示されるたびランダムに生成されます。

     ![PIN コードを入力する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/enter-verification-code.png)

1. PIN 番号を入力すると、 **[資格情報の追加]** 画面が表示されます。 画面上部に、赤色で **[Not verified]\(未確認\)** というメッセージが表示されることに注目してください。 この警告は、前述のドメイン検証に関連しています。

1. **[追加]** を 選択して、新しい検証可能な資格情報を受け入れます。

    ![新しい資格情報を追加する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/new-verifiable-credential.png)

1. おめでとうございます。 検証済み資格情報エキスパートの検証可能な資格情報を取得できました。

    ![検証可能な資格情報を新しく追加する方法を示すスクリーンショット。](media/verifiable-credentials-configure-issuer/verifiable-credential-has-been-added.png)
...
## <a name="verifying-the-verified-expert-card"></a>検証済みのエキスパート カードを検証する

これで、サンプル アプリケーションをもう一度実行して、検証済みエキスパート カードを検証する準備ができました。
...
1. このアプリまたは Web サイトが危険である可能性があるという警告メッセージが表示されたら、 **[詳細設定]** を選択します。 次に、 **[Proceed anyways (unsafe)]\(このまま続行 (安全ではありません)\)** を選択します。
...
1. プレゼンテーション要求を承認すると、要求が承認されたことを確認できます。 また、ログを確認することもできます。 ログを表示するには、検証可能な資格情報を選択します。  
    ...
    ![検証可能な資格情報エキスパート カードを示すスクリーンショット。](media/verifiable-credentials-configure-issuer/verifable-credential-info.png)
...
1. サンプル アプリに戻ります。 検証可能な資格情報のプレゼンテーションが受信されたことが表示されます。  
    ![プレゼンテーションが受信されたことを示すページ](media/verifiable-credentials-configure-issuer/verifiable-credential-expert-verification.png)
...
実際のシナリオでは、ユーザーの詳細はアプリケーションによって ID プロバイダーからプルされます。 次のコード スニペットは、サンプル アプリケーションで名前が設定されている場所を示しています。 後の例では、ID プロバイダーから検証可能な資格情報に値を直接挿入する方法について説明します。
...
> [!div class="checklist"]
> * Azure AD の検証可能な資格情報サービスを構成する
> * 同じ Azure AD テナントを使用して資格情報を発行して検証する
