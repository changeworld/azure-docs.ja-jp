...
Azure Blob Storage は、Microsoft のクラウド用オブジェクト ストレージ ソリューションです。 Azure AD の検証可能な資格情報サービスでは、検証可能な資格情報の発行時に、検証可能な資格情報構成ファイルを格納するために [Azure Blob Storage](../../storage/blobs/storage-blobs-introduction.md) が使用されます。

次の手順に従って、Azure Blob Storage を作成して構成します。

1. Azure Blob Storage アカウントをお持ちでない場合は、[ストレージ アカウントを作成します](../../storage/common/storage-account-create.md)。
