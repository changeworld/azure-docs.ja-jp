...
環境を保護された状態で維持するために、疑わしいユーザーによるサインイン アクティビティをブロックすることができます。 [Azure Active Directory (Azure AD) Identity Protection](active-directory-identityprotection.md) では、各サインインを分析して、サインイン試行がユーザー アカウントの正当な所有者によって行われなかった可能性を算出します。 可能性 (低、中、高) は、[サインイン リスク レベル](active-directory-conditional-access-conditions.md#sign-in-risk)と呼ばれる計算値の形で示されます。 サインイン リスクの条件を設定することで、特定のサインイン リスク レベルに応じた条件付きアクセス ポリシーを構成できます。 
...
![ポリシーの作成](./media/active-directory-conditional-access-app-sign-in-risk/1000.png)
...
- **Identity Protection** - このクイック スタートのシナリオでは Identity Protection を有効にする必要があります。 Identity Protection を有効にする方法がわからない場合は、「[Azure Active Directory Identity Protection の有効化](active-directory-identityprotection-enable.md)」を参照してください。

- **Tor Browser** - [Tor Browser](https://www.torproject.org/projects/torbrowser.html.en) は、オンラインでの自身のプライバシー保護に役立つことを目的としています。 Identity Protection は、Tor Browser からのサインインを、中程度のリスク レベルのある**匿名 IP アドレスからのサインイン**として検出します。 詳細については、「[Azure Active Directory リスク イベント](active-directory-reporting-risk-events.md)」を参照してください。  
...
3. **[Azure Active Directory]** ページの **[管理]** セクションで、**[条件付きアクセス]** をクリックします。
    ...
    ![条件付きアクセス](./media/active-directory-conditional-access-app-sign-in-risk/03.png)
...
6. **[割り当て]** セクションで **[ユーザーとグループ]** をクリックします。
    ...
    ![[概要]](./media/active-directory-conditional-access-app-sign-in-risk/06.png)

7. **[ユーザーとグループ]** ページで、次の手順に従います。

    ![条件付きアクセス](./media/active-directory-conditional-access-app-sign-in-risk/107.png)
...
8. **[クラウド アプリ]** をクリックします。
    ...
    ![クラウド アプリ](./media/active-directory-conditional-access-app-sign-in-risk/08.png)

9. **[クラウド アプリ]** ページで、次の手順に従います。

    ![条件付きアクセス](./media/active-directory-conditional-access-app-sign-in-risk/109.png)
...
10. **[アクセス制御]** セクションで、**[許可]** をクリックします。
    ...
    ![アクセス制御](./media/active-directory-conditional-access-app-sign-in-risk/10.png)

11. **[許可]** ページで、次の手順に従います。

    ![条件付きアクセス](./media/active-directory-conditional-access-app-sign-in-risk/105.png)
...
12. **[ポリシーを有効にする]** セクションで **[オン]** をクリックします。
    ...
    ![ポリシーを有効にする](./media/active-directory-conditional-access-app-sign-in-risk/18.png)
...
![多要素認証](./media/active-directory-conditional-access-app-sign-in-risk/118.png)
...
- ポリシーを削除するには、ポリシーを選択し、クイック アクセス ツール バーの **[削除]** をクリックします。
    ...
    ![多要素認証](./media/active-directory-conditional-access-app-sign-in-risk/33.png)
...
> [特定のアプリケーションに対して MFA を必要にする](./active-directory-conditional-access-app-based-mfa.md)
