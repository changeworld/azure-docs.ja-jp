...
コネクタ グループを作成する理由は次に示すように多数あります。

* 可用性の向上
* 複数のリージョンでアプリケーションを持つテナントの待機時間を向上させる
* リソースを編成して管理を容易にする

コネクタ グループの利点の詳細については、「[コネクタ グループを使用して別のネットワークや場所にアプリケーションを発行する](active-directory-application-proxy-connectors-azure-portal.md)」をご覧ください。
