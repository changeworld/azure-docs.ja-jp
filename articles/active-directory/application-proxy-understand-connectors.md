...
コネクタ グループの詳細については、「[コネクタ グループを使用して別のネットワークや場所にアプリケーションを発行する](active-directory-application-proxy-connectors-azure-portal.md)」をご覧ください。
...
|8|32|270|1190|
|16|64|245|1200*|
\* このマシンの接続制限は 200 です。 他のすべてのマシンで、既定の 200 接続制限を使用しました。
 
>[!NOTE]
>既定の構成制限は、200 です (2、4、および 8 コアの場合)。  16 コアでテスト中、この接続制限は 800 に変更されました。 4、8、および 16 コアのマシンの間で、最大 TPS に大きな違いはありません。 これらのマシンで主に違うのは、予想される待機時間です。  
...
送信トラフィックのファイアウォール規則を構成する方法の詳細については、「[既存のオンプレミス プロキシ サーバーと連携する](application-proxy-working-with-proxy-servers.md)」をご覧ください。
...
ネットワークの最適化の詳細については、「[Azure Active Directory アプリケーション プロキシを使用する場合のネットワーク トポロジに関する注意事項](application-proxy-network-topology-considerations.md)」をご覧ください。
...
* [コネクタ グループを使用して別のネットワークや場所にアプリケーションを発行する](active-directory-application-proxy-connectors-azure-portal.md)
