...
- [apps.dev.microsoft.com](https://apps.dev.microsoft.com) に移動して使用しているアプリケーションを選択するか、または、アプリを作成していない場合は [アプリを作成します](active-directory-v2-app-registration.md)。
...
| redirect\_uri | 必須 | 処理するアプリのレスポンスの送信先となる redirect\_uri。ポータルで登録した redirect\_uri のいずれかと完全に一致させる必要があります。 |
