...
この例では、人事部門の全メンバー (約 1,000 名) が含まれている、**人事部**という名前のセキュリティ グループがテナントに含まれています。 管理者は、Office 365 Enterprise E3 ライセンスを部署全体に割り当てる必要があります。また、この製品に含まれた Yammer Enterprise サービスを、部署での使用の準備が整うまで、一時的に無効にしておく必要があります。 管理者は、Enterprise Mobility + Security ライセンスを同じユーザー グループにデプロイする必要もあります。
