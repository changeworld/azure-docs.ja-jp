---
title: Azure Active Directory Identity Protection (更新版) でリスクを調査する方法 | Microsoft Docs
description: Azure Active Directory Identity Protection (更新版) で危険なユーザー、検出、およびサインインを調査する方法について説明します。
...
ms.locfileid: "70129487"
---
# <a name="how-to-investigate-risk"></a>方法:リスクの調査

危険なサインイン レポート、危険なユーザー レポート、および危険な検出レポートを使用して、環境内のリスクを調査し、詳細を把握できます。 危険なサインイン、ユーザー、および検出をフィルター処理して並べ替えられるため、組織内の潜在的な侵入についてより深く理解することができます。 
...
- ユーザーのアクティブなリスク検出が誤検知であると思われる場合は、ユーザー リスクを無視します。 詳細については、[Azure AD Identity Protection でリスク検出に関するフィードバックを提供する方法](howto-provide-risk-event-feedback.md)に関する記事を参照してください。
...
- 先週、匿名 IP アドレスのリスク検出が発生した正常なサインインの数はどれくらいか。
...
詳細については、[Azure AD Identity Protection でリスク検出に関するフィードバックを提供する方法](howto-provide-risk-event-feedback.md)に関する記事を参照してください。
...
リスク検出レポートは、テナント内のすべての Identity Protection のリスク検出に関する分析情報を提供します。


リスク検出レポートの既定のビューには、以下が表示されます。
...
リスト ビューをカスタマイズするには、ツール バーの **[列]** をクリックします。 列ダイアログでは、追加フィールドの表示や、既に表示されているフィールドの削除が可能です。

リスト ビュー内の項目をクリックすると、その項目に関する確認可能な詳細が横表示で表示されます。


詳細ビューには、リスク検出に関する追加情報が表示されます。

詳細ビューには次の情報へのリンクも含まれています。

- ユーザーのリスク レポートを表示する
- ユーザーのサインインを表示する
- ユーザーのリスクの高いサインインを表示する
- リンクされたリスクの高いサインインを表示する
- ユーザーのリスク検出を表示する


### <a name="filter-risk-detections"></a>リスク検出をフィルター処理する

報告されるデータを有用なものだけに絞り込むために、次の既定のフィールドを使用してリスク検出データをフィルター処理できます。
...
**[検出時間]** フィルターでは、返されるデータの期間を定義できます。 このフィルターでは、次のものを選択できます。
- 過去 90 日間
- 過去 30 日間
- 過去 7 日間
- 過去 24 時間
...
**[検出の種類]** フィルターを使用すると、リスクの検出の種類 (通常とは異なるサインイン プロパティなど) を指定できます。
...
**[ユーザー]** フィルターでは、確認したいユーザーの名前またはオブジェクト ID を指定できます。

**[ユーザー名]** フィルターでは、確認したいユーザーのユーザー プリンシパル名 (UPN) を指定できます。

**[Detection timing]\(検出のタイミング\)** フィルターを使用すると、検出がリアルタイムまたはオフラインのどちらであったかを指定できます。 注:サインイン リスク ポリシーを使用して、リアルタイム検出でサインインのチャレンジを実行できます。 

**[アクティビティ]** フィルターを使用すると、検出がサインイン (匿名 IP アドレスなど) またはユーザー (漏洩した資格情報など) にリンクされていたかどうかを指定できます。

**[ソース]** フィルターを使用すると、リスク検出のソース (Azure AD や Microsoft Cloud App Security など) を指定できます。 

**[トークンの発行元の種類]** フィルターを使用すると、トークンが発行された場所 (Azure AD や AD フェデレーション サービスなど) を指定できます。


### <a name="download-risk-detections-data"></a>リスク検出データをダウンロードする

Azure portal の外部で操作する場合は、リスク検出データをダウンロードできます。 [ダウンロード] をクリックすると、最新の 5,000 レコードの CSV ファイルが作成されます。 
