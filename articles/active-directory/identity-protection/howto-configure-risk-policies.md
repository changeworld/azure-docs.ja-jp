...
Azure AD は、ID が侵害されている可能性の指標となるリスク検出を検出します。 リスク ポリシーを構成することにより、以下のように検出結果に対して自動応答を定義できます。

- サインイン リスク ポリシーにより、ユーザーのサインイン中に検出されたリアルタイム リスク検出に対する応答を構成できます。 
...
サインイン リスク ポリシーによってトリガーされた MFA プロンプトをユーザーが正しく遂行すると、正当なユーザーによってサインインが行われたというフィードバックが Identity Protection に送られます。 その後、MFA プロンプトをトリガーしたサインイン リスク検出は自動的に終了し、このイベントがユーザー リスクの昇格につながらないよう、Identity Protection によって設定されます。 サインイン リスク ポリシーを有効にすると、MFA のダイアログが表示されたときにユーザーが自身で修正し、その後、関連する危険なサインインは自動的にクローズされるため、"危険なサインイン" ビューの表示件数を減らすことができます。
