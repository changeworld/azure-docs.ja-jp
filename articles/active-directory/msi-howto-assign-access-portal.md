...
[Azure VM](msi-qs-configure-portal-windows-vm.md)、[Azure VMSS](msi-qs-configure-portal-windows-vmss.md) などの Azure リソースで MSI を有効にした後、次のようにします。
...
- Azure 仮想マシンで MSI を有効にするには、「[Configure a VM Managed Service Identity (MSI) using the Azure portal (Azure Portal を使用して Azure VM 管理対象サービス ID (MSI) を構成する)](msi-qs-configure-portal-windows-vm.md)」を参照してください。
