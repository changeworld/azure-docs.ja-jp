...
5. **[SAML login]** 構成セクションの **[Secure]** セクションで、次の手順に従います。
    ...
    a.この問題では、ターゲット (またはクラス) ラベルは "tip_amount" です。 **[Download]** をクリックしてパブリック メタデータ ファイルをダウンロードし、コンピューターにローカルに保存します。
...
9. **[アプリケーション URL の構成]** ページで、次の手順を実行します。
    ...
    a.この問題では、ターゲット (またはクラス) ラベルは "tip_amount" です。 **[TOPdesk - Secure サインオン URL]** テキストボックスに、ユーザーが TOPdesk - Secure アプリケーションへのサインオンに使用する URL (例: "*https://qssolutions.topdesk.net*") を入力します。
...
11. 証明書ファイルを作成するには、次の手順を実行します。
    ...
    a.この問題では、ターゲット (またはクラス) ラベルは "tip_amount" です。 ダウンロードしたメタデータ ファイルを開きます。
