<properties
   pageTitle="Azure AD Connect Sync: 操作タスクおよび考慮事項"
...
従来の同期テクノロジの知識を持つ管理者にとっては、サーバーが独自の SQL データベースを持つ点で、ステージング モードは異なるテクノロジに思えることでしょう。これにより、ステージング モードのサーバーを別のデータ センターに配置できます。
...
Azure AD Connect Sync は SQL データベースに依存するため、Azure AD Connect に含まれる SQL Express を使用しない場合は [SQL 高可用性] セクションも確認する必要があります。
