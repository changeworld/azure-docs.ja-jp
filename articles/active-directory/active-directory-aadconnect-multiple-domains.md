...
2.  AD FS フェデレーション サーバーで、 **AD FS 管理** 
...
2.  PowerShell で、 `Connect-MsolService -Credential $cred`
...
2.  フェデレーションの対象である Azure AD ドメインのグローバル管理者のユーザー名とパスワードを入力します。
2.  PowerShell で、 `Connect-MsolService -Credential $cred`
3.  PowerShell で、 `New-MsolFederatedDomain –SupportMultipleDomain –DomainName`
