...
ms.locfileid: "59007681"
---
# <a name="home-realm-discovery-during-sign-in-for-microsoft-365-services"></a>Microsoft 365 サービスに対するサインイン中のホーム領域検出
...
従来、ホーム領域検出は、サインイン時に提供されるドメインによって、または一部のレガシ アプリケーションの場合はホーム領域検出ポリシーによって管理されていました。 たとえば、以前の検出動作では、Azure Active Directory ユーザーが自分のユーザー名を間違えて入力しても、組織の資格情報コレクション画面がユーザーに表示されていました。 これは、ユーザーが組織のドメイン名 "contoso.com" を正しく入力した場合に起こります。 この動作では、個々のユーザーに合わせてエクスペリエンスをカスタマイズする細分性は許可されていません。
...
サインイン ユーザー エクスペリエンスの強化に加えて、この変更には、大規模なユーザー名列挙の不正使用を軽減するのに役立つメカニズムも含まれています。

この変更は、最初にマネージド ドメインを対象とし、2019 年 5 月にロールアウトが開始されますが、フェデレーション ドメインへのロールアウトは 2019 年の末まで開始されません。 フェデレーション ドメインの正確なロールアウトの日付は、お客様のフィードバックによって決まります。

> [!IMPORTANT]
> この機能は、フェデレーションを強制するために以前のドメイン レベルのホーム領域検出に依存しているフェデレーション ドメインに大きな影響を与えます。 一部の組織では、Azure Active Directory に存在していなくても適切なドメイン名を含むユーザー名を使用してサインインするように従業員をトレーニングしてきました。これは、現時点で、ドメイン名によってユーザーが自分の組織のドメイン エンドポイントにルーティングされるからです。 新しいサインイン動作では、これは許可されません。 ユーザーは、ユーザー名を修正するように通知され、Azure Active Directory 内に存在しないユーザー名を使用してサインインすることは許されません。
>
