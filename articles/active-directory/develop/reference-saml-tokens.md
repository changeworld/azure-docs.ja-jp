...
> | EnableAdfsAuthentication | 同等の JWT 要求 | 説明 | 例 |
> | --- | --- | --- | ------------|
...
> |EnableAdfsAuthentication | `unique_name` |トークンのサブジェクトを識別する、人が判読できる値を提供します。 この値は、テナント内で一意であるとは限らず、表示目的でのみ使用されます。 | `<Attribute Name="http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name">`<br>`<AttributeValue>frankm@contoso.com<AttributeValue>`|
