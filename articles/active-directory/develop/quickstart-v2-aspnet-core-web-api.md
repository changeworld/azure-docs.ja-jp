...
このクイックスタートでは、コード サンプルを使用して、承認済みのアカウントだけがアクセスできるように ASP.NET Core Web API を保護する方法を説明します。 アカウントは、個人アカウント (hotmail.com、outlook.com など) でも、任意の Azure Active Directory (Azure AD) インスタンスの職場または学校アカウントでもかまいません。
...
> 1. **[Scope の追加]** を選択し、 **[保存してから続行]** を選択して、既定の **[アプリケーション ID URI]** をそのまま使用します。
> 1. **[Scope の追加]** ペインで、次の値を入力します。
...
>       - お使いのアプリケーションで **[この組織のディレクトリ内のアカウントのみ]** がサポートされる場合は、この値を**ディレクトリ (テナント) ID** (GUID) または**テナント名** (`contoso.onmicrosoft.com` など) に置き換えます。 **ディレクトリ (テナント) ID** は、アプリの **[概要]** ページで確認できます。
>       - アプリケーションで **[任意の組織のディレクトリ内のアカウント]** がサポートされる場合は、この値を `organizations` に置き換えます。
>       - アプリケーションで **[すべての Microsoft アカウント ユーザー]** がサポートされる場合は、この値を `common` のままにします。
>
...
| `Instance`             | ユーザーが認証するためのセキュリティ トークン サービス (STS) エンドポイント。 この値は、通常、Azure パブリック クラウドを示す `https://login.microsoftonline.com/` です。 |
