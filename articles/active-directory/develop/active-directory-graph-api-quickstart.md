---
title: Azure AD Graph API の使用方法
description: Azure Active Directory (Azure AD) Graph API を使用すると、OData REST API エンドポイントを介して Azure AD にプログラムによってアクセスできます。 アプリケーションでは、Azure AD Graph API を使って、ディレクトリのデータとオブジェクトに対して、作成、読み取り、更新、および削除 (CRUD) の各操作を実行できます。
...
* **リソース パス**:URL のこのセクションは、対話するリソース (ユーザー、グループ、特定のユーザー、特定のグループなど) を識別します。上記の例では、そのリソース セットをアドレス指定する最上位の "groups" です。 特定のエンティティ ("users/{objectId}" や "users/userPrincipalName" など) をアドレス指定することもできます。
...
"api-version" クエリ パラメーターに、Graph API 要求用のバージョンを指定します。 バージョン 1.5 以降の場合は、バージョン値を使用します (api-version=1.6)。 以前のバージョンでは、YYYY-MM-DD の形式に準拠した日付文字列を使用します (例: api-version=2013-11-08)。 プレビュー機能については、文字列 "beta" を使用します (例: api-version=beta)。 Graph API のバージョン間の相違点の詳細については、[Azure AD Graph API のバージョン管理](https://msdn.microsoft.com/Library/Azure/Ad/Graph/howto/azure-ad-graph-api-versioning)に関するページを参照してください。

## <a name="graph-api-metadata"></a>Graph API のメタデータ

Azure AD Graph API メタデータ ファイルを返すには、URL の tenant-identifier の後に "$metadata" セグメントを追加します。たとえば、`https://graph.windows.net/GraphDir1.OnMicrosoft.com/$metadata?api-version=1.6` という URL では、デモ会社のメタデータが返されます。 この URL を Web ブラウザーのアドレス バーに入力して、メタデータを表示できます。 返される CSDL メタデータ ドキュメントには、要求した Graph API バージョンによって公開されているエンティティと複合型、それらのプロパティ、および関数とアクションが記述されています。 api-version パラメーターを省略すると、最新バージョンのメタデータが返されます。
...
* 履歴を要求します。
* "me" と "myorganization" のアドレス指定エイリアスがサポートされます。 たとえば、`https://graph.windows.net/me?api-version=1.6` を使ってサインインしているユーザーのユーザー オブジェクトを返し、`https://graph.windows.net/myorganization/users?api-version=1.6` を使ってサインインしているユーザーのディレクトリのすべてのユーザーを返すことができます。
...
このクイック スタート ガイドでは、Azure AD ディレクトリに対する "書き込み" 操作の実行を練習するために、Fiddler Web Debugger を使用できます。 たとえば、ユーザーのプロファイル写真を取得してアップロードできます (これは Azure AD Graph Explorer ではできません)。 Fiddler の詳細とインストールについては、[https://www.telerik.com/fiddler](https://www.telerik.com/fiddler) をご覧ください。
