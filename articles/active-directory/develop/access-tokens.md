...
クレームは、そこに入力される値が存在する場合にのみ存在します。 アプリは要求の存在に依存することはできません。 例として、`pwd_exp` (すべてのテナントでパスワードを期限切れにする必要はない) や、`family_name` (名前のないアプリケーションに代わって、[クライアント資格情報] (v2-oauth2-client-creds-grant-flow.md) フローが使用される) などがあります。 アクセス トークンの検証に使用されるクレームは常に存在します。
