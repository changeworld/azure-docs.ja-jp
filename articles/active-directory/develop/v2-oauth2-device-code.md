...
> [!IMPORTANT]
> 現時点では、Microsoft ID プラットフォーム エンドポイントでサポートされるのは Azure AD テナントのデバイス フローのみで、個人アカウントはサポートされません。  そのため、テナントとして設定されたエンドポイントまたは `organizations` エンドポイントを使用する必要があります。  
>
