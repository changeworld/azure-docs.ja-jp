...
### <a name="to-register-a-new-application-in-the-azure-classic-portal"></a>Azure クラシック ポータルで新しいアプリケーションを登録するには
...
5. [アプリケーション情報の指定] ページでは、アプリケーションの名前と、Azure AD に登録するアプリケーションの種類を指定する必要があります。  [Web アプリケーション/クライアント](active-directory-dev-glossary.md#client-application) / [Web リソース/API](active-directory-dev-glossary.md#resource-server) アプリケーション (どちらとしても機能します) または[ネイティブ クライアント](active-directory-dev-glossary.md#native-client) アプリケーションから選択できます。 完了したら、ページの右下にある矢印アイコンをクリックします。
...
1. [Azure クラシック ポータル](https://manage.windowsazure.com)
2. 左側のメニューの [Active Directory] アイコンをクリックし、目的のディレクトリをクリックします。
3. 上部のメニューで **[アプリケーション]**をクリックし、構成するアプリケーションをクリックします。 クイック スタート ページが開き、シングル サインオンと他の構成情報が表示されます。 ページの上部にある **[構成]** リンクをクリックして、アプリケーションの構成ページに移動します。
4. Web アプリケーションの資格情報の秘密キーを追加するために、下にスクロールして [キー] セクションに移動します。  
   
   * 最初に [時間の選択] ボックスをクリックし、1 年または 2 年の期間を選択します。 
   * 次に、"有効期間の開始日" と "有効期限" の日付が入力された新しい行が表示されます。
   * 構成の変更を保存する (下記参照) と、右端の列にキーの値が格納されます。 [保存] をクリックした後にこのセクションに戻り、値をコピーしてください。この値は、クライアント アプリケーションの実行時に認証用に使用します。
5. クライアントからリソース API にアクセスするためのアクセス許可を追加するために、下にスクロールして [他のアプリケーションに対するアクセス許可] セクションに移動します。 
   
   * 最初に、[アプリケーションの追加] をクリックします。
   * [表示] ボックスの一覧を使用して、選択するリソースの種類を選択します。
   * 1 列目では、ディレクトリ内の、Web API を公開している利用可能なリソース アプリケーションの中から選択できます。 目的のリソースをクリックし、右下隅のチェック マークをクリックします。
   * 選択すると、[他のアプリケーションに対するアクセス許可] リストにそのリソースが追加されます。
   * [アプリケーションのアクセス許可] および [委任されたアクセス許可] ドロップダウン リストを使用して、クライアント アプリケーションに付与するアクセス許可を選択します。
6. 完了したら、コマンド バーの **[保存]** をクリックします。 アプリケーションのキーを指定した場合も、[保存] をクリックした後にキーが生成されます。
...
### <a name="legacy-experiences-for-granting-access"></a>アクセスを許可する際の従来のエクスペリエンス
...
#### <a name="building-the-link-that-grants-access-for-external-users-legacy"></a>外部ユーザーのアクセスを許可するリンクの作成 (旧)
...
#### <a name="grant-access-user-experience-and-response-legacy"></a>アクセスの許可のユーザー エクスペリエンスと応答 (旧)
...
#### <a name="rolling-app-keys-for-uninterrupted-access-to-the-graph-api-legacy"></a>Graph API へのアクセスが中断されないようにするためのアプリケーション キーのローリング (旧)
...
#### <a name="changing-app-properties-after-enabling-access-legacy"></a>アクセス有効化後のアプリケーションのプロパティの変更 (旧)
