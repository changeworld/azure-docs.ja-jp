...
クライアント シークレットを使用して機密クライアントを構築する場合、構成ファイルはこのようになります (GitHub の [confidential_client_secret_sample.py#L4-L15](https://github.com/AzureAD/microsoft-authentication-library-for-python/blob/0.6.1/sample/confidential_client_secret_sample.py#L4-L15) サンプルから抽出)。
...
証明書を使用して機密クライアントを構築する場合は、このようになります (GitHub の [confidential_client_certificate_sample.py#L4-L15](https://github.com/AzureAD/microsoft-authentication-library-for-python/blob/4b34fd660e5574eb876cfba63a1b927ae375efd6/sample/confidential_client_certificate_sample.py#L4-L15) サンプルからの抽出)。
...
msal4j dev サンプルでサンプルを構成するために使用されているクラス: [TestData](https://github.com/AzureAD/microsoft-authentication-library-for-java/blob/dev/src/samples/public-client/TestData.java)。
...
CONFIDENTIAL_CLIENT_ID または CONFIDENTIAL_CLIENT_SECRET のいずれかを指定します。 両方を同時に設定することはできません。
...
msal4j はパブリック プレビュー段階にあります。 署名付きアサーションはまだサポートされていません。
