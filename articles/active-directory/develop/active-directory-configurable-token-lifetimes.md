...
Microsoft ID プラットフォームによって発行されたトークンの有効期間を指定できます。 組織のすべてのアプリ、マルチテナント (複数の組織) アプリケーション、または組織の特定のサービス プリンシパルに対して、トークンの有効期間を設定できます。 
> 現時点では、マネージド ID サービス プリンシパルのトークン有効期間の構成はサポートされていないことに注意してください。
