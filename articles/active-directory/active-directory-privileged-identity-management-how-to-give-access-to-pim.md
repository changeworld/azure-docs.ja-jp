...
組織の Azure AD Privileged Identity Management (PIM) を有効にする最初のユーザーは、グローバル管理者である必要があります。ただし、他のグローバル管理者には既定で PIM へのアクセス権が付与されていないため、一時的な割り当てを管理することはできません。PIM へのアクセス権を付与するには、最初のユーザーが他のユーザーにセキュリティ管理者ロールを割り当てます。この割り当ては、PIM 内から実行する必要があり、PowerShell や他のポータルを使用して変更することはできません。
...
1 人のユーザーがロックアウトされたり、アカウントが削除されたりしたときのために、セキュリティ管理者ロールを割り当てられたユーザーが常に少なくとも 2 人いるようにしてください。
...
2. **[セキュリティ管理者]** ロールを選択します。そのロールの現在のユーザー一覧が表示されます。
...
セキュリティ管理者ロールからユーザーを削除する際は、必ず事前に、割り当てられているユーザーの人数が削除後も 2 人になるようにしてください。

1. PIM ダッシュボードで、**[セキュリティ管理者]** ロールをクリックします。そのロールの現在のユーザー一覧が表示されます。
