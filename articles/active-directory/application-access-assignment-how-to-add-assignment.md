...
特定のアプリケーションに対してユーザーが以下の操作を行えるようにするには、まず、**ユーザーをアプリケーションに割り当て**、アクセス権を付与する必要があります。
...
アプリケーションへのセルフ サービス アクセスは、ユーザーにアプリケーションの自己検出を許可したり、必要に応じてビジネス グループによるこれらのアプリケーションへのアクセス承認を許可したりする場合に優れた方法です。 ビジネス グループがアクセス パネルから直接、ユーザーに割り当てられた (パスワード シングル サインオン アプリケーションに関する)資格情報を管理できるようにすることができます。

セルフ サービス アプリケーションからアプリケーションへのアクセスを有効にするには、次の手順を実行します。
...
6.  セルフ サービスを有効にするアプリケーションを一覧から選択します。
