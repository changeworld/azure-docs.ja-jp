...
公開キーは、Azure Active Directory と Windows Server Active Directory (オンプレミスの場合) に登録されます。ID プロバイダー (IDP) は、ユーザーの公開キーを秘密キーにマップしてユーザーを検証し、One Time Password (OTP)、Phonefactor、その他の通知メカニズムを使ってログイン情報を提供します。
...
* [組織での Microsoft Passport for Work の有効化](active-directory-azureadjoin-passport-deployment.md)
