...
# チュートリアル: Azure Active Directory と BenSelect の統合
...
このリンク関係は、Azure AD の **[ユーザー名]** の値を、BenSelect の **[Username (ユーザー名)]** の値として割り当てることで確立されます。
...
1. Azure クラシック ポータルの **[BenSelect]** アプリケーション統合ページで、上部のメニューから **[属性]** をクリックします。
...
2. **[Saml トークン属性]** ダイアログで、以下の表の各行について、次の手順を実行します。
	...
	d.**[メール]** ボックスの一覧から「User.userprincipalname」と入力します。
...
4. **[ユーザーの BenSelect へのアクセスを設定してください]** ページで、**[Azure AD のシングル サインオン]** を選択し、**[次へ]** をクリックします。
...
5. **[アプリケーション設定の構成]** ダイアログ ページで、次の手順に従います。
    ...
    a.**[応答 URL]** ボックスに、`https://www.benselect.com/enroll/login.aspx?Path={<tenant name>}` の形式で URL を入力します。
...
> [AZURE.NOTE] ユーザーを手動で作成する必要がある場合は、BenSelect のサポート チーム (<mailto:support@selerix.com>) にお問い合わせください。
