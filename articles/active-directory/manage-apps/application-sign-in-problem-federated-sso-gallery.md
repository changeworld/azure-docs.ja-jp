...
8.  **[ドメインと URL]** セクションに移動します。 [識別子] テキスト ボックス内の値が、エラーで表示されている識別子の値に一致していることを確認します。
...
この問題を解決するには、Azure AD 構成に一意識別子の値を追加します。 識別子の値を追加するには、次の手順に従います。
...
8.  **[ドメインと URL]** セクションで、**[詳細な URL 設定の表示]** をクリックします。

9.  **[識別子]** ボックスに、アプリケーションの一意識別子を入力します。
...
上記のオプション 1 で問題が解決しなかった場合は、ディレクトリからアプリケーションを削除してみます。 次に、アプリケーションを追加し、再構成します。次の手順に従ってください。
...
8.  Azure AD を更新し、Azure AD ギャラリーからアプリケーションを追加します。 次に、アプリケーションを構成します

<span id="_Hlk477190176" class="anchor"></span>アプリケーションを再構成したら、アプリケーションにサインインできるようになります。
