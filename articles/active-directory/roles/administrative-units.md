...
管理単位を使用すると、付与する管理者アクセス許可を、定義した組織の部門、地域、またはその他のセグメントに制限できます。 管理単位を使用して、各地域の管理者に権限を委任したり、ポリシーを細かく設定したりできます。 たとえば、ユーザー アカウント管理者には、管理単位内のユーザーに対してのみ、プロファイル情報の更新、パスワードのリセット、およびライセンスの割り当てを実行できるようにします。

たとえば、[ヘルプデスク管理者](permissions-reference.md#helpdesk-administrator)のロールを地域のサポート スペシャリストに委任した場合、そのスペシャリストの管理対象はサポートする地域内のユーザーのみに制限されます。
...
管理単位を使用して、Azure AD リソースを論理的にグループ化することができます。 たとえば、IT 部門がグローバルに分散している組織では、地理的な境界を定義する管理単位を作成することが理にかなっている場合があります。 多国籍組織に、業務において半自律的な各種 " *サブ組織* " がある別のシナリオでは、管理単位を使用して各サブ組織を表すことができます。
...
1. **排除** : 条件を適切に定義すると、不要になった管理単位が削除されます。
1. **安定化** : 組織の構造が適切に定義されており、管理単位の数が短期間で大幅に変更されることはありません。
...
また、グループを管理単位のメンバーとして追加することもできます。 管理単位スコープのグループ管理者は、PowerShell、Microsoft Graph、Azure AD ポータルを使用してそれらを管理できます。

次の表では、さまざまな管理単位シナリオに対する現在のサポート状況について説明しています。
