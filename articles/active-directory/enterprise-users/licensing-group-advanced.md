...
1. **[割り当て]** を選択して、E3 ライセンスをユーザーに直接割り当てます。 たとえば、Yammer Enterprise 以外のすべてのサービスプランを無効にする場合などです。
...
1. 直接割り当てを使用すると、次の操作を行うことができます。
   ...
   - Yammer Enterprise は、個々のユーザーに対して無効にすることができます。 サービスはユーザーに直接割り当てられるため、変更できます。
...
![[監査ログ] ページでリスト項目が選択され、[アクティビティの詳細: 監査ログ] ウィンドウが開いていることを示すスクリーンショット。](./media/licensing-group-advanced/audit-group-license-change.png)
