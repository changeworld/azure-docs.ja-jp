...
|50078|提示された多要素認証の有効期限が切れました。アクセスするには多要素認証を更新する必要があります。|
...
|50194|アプリケーション '{appId}'({appName}) はマルチテナント アプリケーションとして構成されていません。 '{time}' より後に作成されたそのようなアプリケーションでは、/common エンドポイントの使用はサポートされていません。 テナント固有のエンドポイントを使用するか、アプリケーションをマルチテナントとして構成してください。|
...
|90094| サインインしたユーザーが同意することを許可されていないアクセス許可がアプリによって要求されたため、ユーザーがブロックされました。 |
|90095| サインインしたユーザーが同意することを許可されていないアクセス許可がアプリによって要求されたため、ユーザーに[管理者の同意要求](../manage-apps/configure-admin-consent-workflow.md)フォームが表示されました。 |
|130500|ユーザー資格情報ポリシーにより、電話でのサインインはブロックされました。|
|500011| <site address> という名前のリソース プリンシパルが <tenant ID> という名前のテナントで見つかりませんでした。 このエラーは、アプリケーションがテナントの管理者によってインストールされていない場合や、アプリケーションがテナント内のいずれのユーザーによっても同意されていない場合に発生することがあります。 間違ったテナントに認証要求を送信した可能性があります。|
|500014|リソース '{identifier}' が無効になっています。|
...
|500172|'{issuer}' によって発行された証明書 '{name}' が無効です。 現在時刻: '{curTime}'。 証明書 NotBefore: '{startTime}'。 証明書 NotAfter: '{endTime}'。|
|501291|クライアント アプリは Mam アプリであり、デバイスは登録されず、要求はブローカーを使用して送信されます。 アプリにアクセスする前にデバイスを登録するには、ワークプレースの結合を行う必要があります。|
|530003|このリソースにアクセスするには、デバイスがマネージドである必要があります。|
...
|700051|Response_type 'token'がアプリケーションに対して有効になっていません。 アプリケーションは、次の理由により、サポートされていない応答の種類を要求しました: Response_type 'token'がアプリケーションに対して有効になっていません。 アプリケーションの所有者は、Azure portal で、または MS Graph を呼び出して、暗黙のアクセス トークン許可を有効にする必要があります。|
|900432|Confidential Client はクロスクラウド要求でサポートされていません。|
|5000811|SAML トークン署名を検証できません。 署名キー識別子が、有効な登録済みキーと一致しません。|
|7000215|無効なクライアント シークレットが指定されました。|
