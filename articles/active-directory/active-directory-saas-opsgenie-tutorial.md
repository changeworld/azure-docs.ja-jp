...
* ユーザーが自分の Azure AD アカウントで自動的に OpsGenie にサインオン (シングル サインオン (SSO)) できるようにします。
...
## <a name="add-opsgenie-from-the-gallery"></a>ギャラリーからの OpsGenie の追加
...
このセクションの目的は、"Britta Simon" というテスト ユーザーに基づいて、OpsGenie で Azure AD の SSO を構成し、テストする方法について説明することです。

SSO を機能させるには、Azure AD ユーザーに対応する OpsGenie ユーザーが Azure AD で認識されている必要があります。 言い換えると、Azure AD ユーザーと OpsGenie の関連ユーザーの間で、リンク関係が確立されている必要があります。

このリンク関係を確立するには、Azure AD の **[ユーザー名]** の値を OpsGenie の **[Username]** の値として割り当てます。

OpsGenie で Azure AD の SSO を構成してテストするには、次の構成要素を完了する必要があります。
...
このセクションの目的は、Azure クラシック ポータルで Azure AD の SSO を有効にすることと、OpsGenie アプリケーションで SSO を構成することです。
...
3. **[アプリケーション設定の構成]** ダイアログ ページで、次の手順に従います。
  ...
  1. **"https://app.opsgenie.com/auth/login"**

    >[!NOTE] 
    >サインオン URL が必要な場合、 [OpsGenie サポート チーム](mailto:support@opsgenie.com) にお問い合わせください。
    >
...
4. **[OpsGenie でのシングル サインオンの構成]** ページで、次の手順を実行します。
  ...
  1. **[証明書のダウンロード]** をクリックし、コンピューターにファイルを保存します。 この証明書とメタデータ URL (エンティティ ID、SSO サインイン URL、サインアウト URL) は OpsGenie 側で SSO をセットアップするために必要です。
  2. **[次へ]**をクリックします。
...
9. [Azure Active Directory] ダイアログ ページで、次の手順に従います。
  ...
  1. Azure クラシック ポータルで、**[OpsGenie でのシングル サインオンの構成]** ダイアログ ページの **[シングル サインオン サービス URL]** の値をコピーし、**[SAML 2.0 Endpoint]** ボックスに貼り付けます。
  2. ダウンロードした証明書から base-64 でエンコードされたファイルを作成します。      
  ...
  3. base-64 でエンコードされた証明書をメモ帳で開き、その内容をクリップボードにコピーして、**[X.500 Certificate]** ボックスに貼り付けます。
  4. **[変更を保存]**をクリックします。
...
### <a name="create-a-opsgenie-test-user"></a>OpsGenie テスト ユーザーの作成
...
4. **[Add User]** ダイアログで、次の手順を実行します。
  ...
  1. **[電子メール]** ボックスに、Azure Active Directory の Britta の電子メール アドレスを入力します。
  2. **[フル ネーム]** ボックスに「**Britta Simon**」と入力します。
  3. **[保存]**をクリックします。 
...
このセクションの目的は、Britta Simon に OpsGenie へのアクセスを許可し、このユーザーが Azure の SSO を使用できるようにすることです。
