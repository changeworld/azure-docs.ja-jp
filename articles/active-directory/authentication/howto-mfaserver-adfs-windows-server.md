...
description: この記事では、Windows Server 2016 で Azure Multi-Factor Authentication と AD FS の使用を開始する方法について説明します。
...
この記事では、Windows Server 2016 以降での AD FS と Azure Multi-Factor Authentication Server の使用について説明します。 詳細については、「 [Azure Multi-Factor Authentication Server と AD FS 2.0 を使用したクラウドおよびオンプレミスのリソースのセキュリティ保護](howto-mfaserver-adfs-2.md)」を参照してください。
...
2. **WebServiceSdkPassword** の値を、該当するアカウント パスワードに設定します。 特殊文字 "&" は、**WebServiceSdkPassword** では使用できません。
...
トラブルシューティングのヘルプについては、「 [Azure Multi-Factor Authentication についてよく寄せられる質問 (FAQ)](multi-factor-authentication-faq.yml)
