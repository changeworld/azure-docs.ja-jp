...
Microsoft では、組織が条件付きアクセスを使用し、[名前付きの場所](../conditional-access/location-condition.md#named-locations)を使用してネットワークを定義することを推奨しました。 組織で Identity Protection を使用している場合は、名前付きの場所ではなく、リスクベースのポリシーを使用することを検討します。
