...
1. RADIUS クライアント (VPN、Netscaler サーバーなど) と NPS サーバー間で使用されるパスワードの暗号化アルゴリズム。
   ...
   - **CHAPV2** と **EAP** は、電話とモバイル アプリの通知をサポートします。
      ...
      >
      > さらに、VPN クライアント UX が入力フィールドをサポートしていて、ネットワーク アクセス ポリシーを構成している場合は、認証が成功する可能性があります。 ただし、ネットワーク ポリシーで構成されている RADIUS 属性は、ネットワーク アクセス デバイス (RRAS サーバーなど) や VPN クライアントには適用されません。 その結果、VPN クライアントは、必要以上に多くのアクセス権を持つか、アクセス権が足りずにアクセスできなくなる可能性があります。
