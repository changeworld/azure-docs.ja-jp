...
## <a name="enabling-the-application-integration-for-talentlms"></a>TalentLMS のアプリケーション統合の有効化
このセクションでは、TalentLMS のアプリケーション統合を有効にする方法について説明します。

### <a name="to-enable-the-application-integration-for-talentlms-perform-the-following-steps"></a>TalentLMS のアプリケーション統合を有効にするには、次の手順に従います。
...
このセクションでは、SAML プロトコルに基づくフェデレーションを使用して、ユーザーが Azure AD のアカウントで TalentLMS に対する認証を行うことができるようにする方法を説明します。 をクリックします。  
...
8. [Single Sign-On] セクションで、次の手順に従います。
    ...
    a.この問題では、ターゲット (またはクラス) ラベルは "tip_amount" です。 **[SSO integration type]** ボックスの一覧から **[SAML 2.0]** を選択します。
   
    b. Azure クラシック ポータルの **[TalentLMS でのシングル サインオンの構成]** ダイアログ ページで、**[ID プロバイダーの ID]** 値をコピーして、**[Identity provider (IdP)]** テキスト ボックスに貼り付けます。
   
    c. エクスポートした証明書から **[拇印]** の値をコピーし、**[証明書の指紋]** ボックスに貼り付けます。
    ...
    d. Azure クラシック ポータルの **[TalentLMS でのシングル サインオンの構成]** ダイアログ ページで **[リモート ログイン URL]** の値をコピーし、**[Remote sign-in URL]** テキスト ボックスに貼り付けます。
   
    e. Azure クラシック ポータルの **[TalentLMS でのシングル サインオンの構成]** ダイアログ ページで **[リモート ログアウト URL]** の値をコピーし、**[Remote sign-out URL]** テキスト ボックスに貼り付けます。
   
    f.SAML 属性の属性名またはスキーマ リファレンスを入力します。 **[TargetedID]** ボックスに、**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name** と入力します。
   
    g. **[First name]** ボックスに、**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname** と入力します。
   
    h. **[Last name]** ボックスに、**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/surname** と入力します。
   
    i. **[Email]** ボックスに、**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress** と入力します。
...
3. **[Add user]** ダイアログ ページで、次の手順に従います。
    ...
    a.この問題では、ターゲット (またはクラス) ラベルは "tip_amount" です。 Azure AD のユーザー アカウントに関連する属性の値を次のテキスト ボックスに入力します。**[First name]**、**[Last name]**、**[Email address]**。
...
### <a name="to-assign-users-to-talentlms-perform-the-following-steps"></a>ユーザーを TalentLMS に割り当てるには、次の手順に従います。
