...
ディレクトリ同期 | オンプレミス ディレクトリ オブジェクト (ユーザー、グループ、連絡先) をクラウドに同期させるのに使用され、管理オーバーヘッドの削減に役立ちます。ディレクトリ同期 (directory synchronization) は Azure AD ポータルおよび Microsoft Azure 管理ポータルでは directory sync とも呼ばれます。ディレクトリ同期を設定すると、管理者は Active Directory のオンプレミス ディレクトリ オブジェクトを Azure AD インスタンスにプロビジョニングできます。
