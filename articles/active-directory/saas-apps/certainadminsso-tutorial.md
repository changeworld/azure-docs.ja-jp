...
1. 検索ボックスに「**Certain Admin SSO**」と入力し、結果パネルで **[Certain Admin SSO]** を選択し、**[追加]** をクリックして、アプリケーションを追加します。
...
1. **[Certain Admin SSO のテスト ユーザーの作成](#create-a-certain-admin-sso-test-user)** - Certain Admin SSO で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
1. **[Certain Admin SSO Domain and URLs]\(Certain Admin SSO のドメインと URL\)** セクションで、次の手順を実行します。
...
1. **[Certain Admin SSO Configuration]\(Certain Admin SSO 構成\)** セクションで、**[Configure Certain Admin SSO]\(Certain Admin SSO の構成\)** をクリックして、**[サインオンの構成]** ウィンドウを開きます。 **[クイック リファレンス]** セクションから、**サインアウト URL、SAML エンティティ ID、SAML シングル サインオン サービス URL** をコピーします。

    ![Certain Admin SSO 構成](./media/certainadminsso-tutorial/tutorial_certainadminsso_configure.png) 

1. **Certain Admin SSO** 側にシングル サインオンを構成するには、ダウンロードされた**証明書 (Raw)**、**サインアウト URL、SAML エンティティ ID、および SAML シングル サインオン サービス URL** を [Certain Admin SSO サポート チーム](mailto:integrations@certain.com)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
