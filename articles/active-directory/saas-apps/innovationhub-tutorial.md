...
1. 検索ボックスに「**Innovation Hub**」と入力し、結果パネルで **[Innovation Hub]** を選び、**[追加]** ボタンをクリックしてアプリケーションを追加します。
...
1. **[Innovation Hub テスト ユーザーの作成](#create-an-innovation-hub-test-user)** - Innovation Hub で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
1. **[Innovation Hub Domain and URLs]\(Innovation Hub のドメインと URL\)** セクションで、次の手順を実行します。
...
1. Innovation Hub アプリケーションは、特定の形式で構成された SAML アサーションを受け入れます。 このアプリケーションには、次の要求を構成してください。 この属性の値は、アプリケーション統合ページの **[User Attributer]** セクションで管理できます。 次のスクリーンショットはその例です。
...
1. **Innovation Hub** 側でシングル サインオンを構成するには、コピーした**フェデレーション メタデータ URL** を [Innovation Hub サポート チーム](mailto:support@readify.net)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
...
1. アプリケーションの一覧で **[Innovation Hub]** を選択します。
