...
3. **[Autotask Endpoint Backup テスト ユーザーの作成](#create-a-autotask-endpoint-backup-test-user)** - Autotask Endpoint Backup で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
3. **[Autotask Endpoint Backup Domain and URLs]\(Autotask Endpoint Backup のドメインと URL\)** セクションで、次の手順に従います。
...
6. **Autotask Endpoint Backup** 側にシングル サインオンを構成するには、ダウンロードした**メタデータ XML** を [Autotask Endpoint Backup サポート チーム](https://backup.autotask.net/help/Content/0_HOME/Support_for_End_Clients.htm)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
