...
* 15Five では、**SP** Initiated SSO がサポートされています。
* 15Five では、[自動化されたユーザー プロビジョニング](15five-provisioning-tutorial.md)がサポートされます。
...
1. **[ギャラリーから追加する]** セクションで、検索ボックスに「**15Five**」と入力します。
1. 結果のパネルから **[15Five]** を選択し、アプリを追加します。 お使いのテナントにアプリが追加されるのを数秒待機します。

## <a name="configure-and-test-azure-ad-sso-for-15five"></a>15Five の Azure AD SSO の構成とテスト
...
1. **[15Five SSO の構成](#configure-15five-sso)** - アプリケーション側でシングル サインオン設定を構成します。
    1. **[15Five のテスト ユーザーの作成](#create-15five-test-user)** - 15Five で B.Simon に対応するユーザーを作成し、Azure AD の B.Simon にリンクさせます。
...
4. **[基本的な SAML 構成]** セクションで、次の手順を実行します。
    ...
    > [!NOTE]
    > これらは実際の値ではありません。 実際のサインオン URL と識別子でこれらの値を更新します。 これらの値を取得するには、[15Five クライアント サポート チーム](https://www.15five.com/contact/)に連絡してください。 Azure portal の **[基本的な SAML 構成]** セクションに示されているパターンを参照することもできます。
...
## <a name="configure-15five-sso"></a>15Five の SSO の構成

**15Five** 側でシングル サインオンを構成するには、ダウンロードした **フェデレーション メタデータ XML** と Azure portal からコピーした適切な URL を [15Five サポート チーム](https://www.15five.com/contact/)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。

### <a name="create-15five-test-user"></a>15Five のテスト ユーザーの作成
...
2. [**会社の管理**] に移動します。
    ...
    ![会社の管理](./media/15five-tutorial/profile.png "会社の管理")

3. **[ユーザー] \> [ユーザーを追加]** の順にクリックします。

    ![ユーザー](./media/15five-tutorial/account.png "ユーザー")

4. **[新しいユーザーの追加]** セクションで、次の手順に従います。

    ![新しいユーザーの追加](./media/15five-tutorial/add-person.png "新しいユーザーの追加")
...
* Microsoft マイ アプリを使用することができます。 マイ アプリで [15Five] タイルをクリックすると、15Five のサインオン URL にリダイレクトされます。 マイ アプリの詳細については、[マイ アプリの概要](https://support.microsoft.com/account-billing/sign-in-and-start-apps-from-the-my-apps-portal-2f3b1bae-0e5a-4a86-a33e-876fbd2a4510)に関するページを参照してください。

## <a name="next-steps"></a>次のステップ

15Five を構成したら、組織の機密データを流出と侵入からリアルタイムで保護するセッション制御を適用できます。 セッション制御は、条件付きアクセスを拡張したものです。 [Microsoft Cloud App Security でセッション制御を強制する方法](/cloud-app-security/proxy-deployment-aad)をご覧ください。