---
title: チュートリアル:Azure Active Directory と ExcelityGlobal の統合 | Microsoft Docs
description: Azure Active Directory と ExcelityGlobal の間でシングル サインオンを構成する方法について説明します。
...
このチュートリアルでは、ExcelityGlobal と Azure Active Directory (Azure AD) を統合する方法について説明します。
ExcelityGlobal と Azure AD の統合には、次の利点があります。

* ExcelityGlobal にアクセスする Azure AD ユーザーを制御できます。
...
ExcelityGlobal と Azure AD の統合を構成するには、次のものが必要です。
...
Azure AD への ExcelityGlobal の統合を構成するには、ギャラリーからマネージド SaaS アプリの一覧に ExcelityGlobal を追加する必要があります。

**ギャラリーから ExcelityGlobal を追加するには、次の手順に従います。**
...
2. **[ExcelityGlobal のシングル サインオンの構成](#configure-excelityglobal-single-sign-on)** - アプリケーション側でシングル サインオン設定を構成します。
3. **[Azure AD のテスト ユーザーの作成](#create-an-azure-ad-test-user)** - Britta Simon で Azure AD のシングル サインオンをテストします。
4. **[Azure AD テスト ユーザーの割り当て](#assign-the-azure-ad-test-user)** - Britta Simon が Azure AD シングル サインオンを使用できるようにします。
5. **[ExcelityGlobal のテスト ユーザーの作成](#create-excelityglobal-test-user)** - ExcelityGlobal で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
ExcelityGlobal で Azure AD シングル サインオンを構成するには、次の手順に従います。

1. [Azure portal](https://portal.azure.com/) の **ExcelityGlobal** アプリケーション統合ページで、**[シングル サインオン]** を選択します。
...
4. **[SAML でシングル サインオンをセットアップします]** ページで、次の手順を実行します。
    ...
    ![[ExcelityGlobal のドメインと URL] のシングル サインオン情報](common/idp-intiated.png)
...
6. **[SAML 署名証明書]** セクションで **[編集]** ボタンをクリックして、**[SAML 署名証明書]** ダイアログを開きます。

    ![SAML 署名証明書の編集](common/edit-certificate.png)

7. **[SAML 署名証明書]** セクションで **[Thumbprint]\(拇印\)** をコピーし、お使いのコンピューターに保存します。

    ![[Thumbprint]\(拇印\) の値をコピーする](common/copy-thumbprint.png)

8. **[ExcelityGlobal のセットアップ]** セクションで、要件に従って適切な URL をコピーします。
...
**ExcelityGlobal** 側でシングル サインオンを構成するには、**サムプリントの値**と Azure portal からコピーした適切な URL を [ExcelityGlobal サポート チーム](https://www.excelityglobal.com/contact-us/our-locations/)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
...
このセクションでは、Britta Simon に ExcelityGlobal へのアクセスを許可することで、このユーザーが Azure シングル サインオンを使用できるようにします。

1. Azure portal 上で **[エンタープライズ アプリケーション]** を選択し、**[すべてのアプリケーション]** を選択してから、**[ExcelityGlobal]** を選択します。

    ![[エンタープライズ アプリケーション] ブレード](common/enterprise-applications.png)

2. アプリケーションの一覧で **[Everbridge]** を選択します。

    ![アプリケーションの一覧の [ExcelityGlobal] リンク](common/all-applications.png)
...
このセクションでは、ExcelityGlobal で Britta Simon というユーザーを作成します。  [ExcelityGlobal サポート チーム](https://www.excelityglobal.com/contact-us/our-locations/)と協力して、ExcelityGlobal プラットフォームにユーザーを追加します。 シングル サインオンを使用する前に、ユーザーを作成し、有効化する必要があります。
