...
1. **[EmpCenter のドメインと URL]** セクションで、次の手順に従います。
    ...
    **[サインオン URL]** ボックスに、次の形式で URL を入力します。
    ...
    > [!NOTE] 
    > この値は実際のものではありません。 実際のサインオン URL でこの値を更新してください。 この値を取得するには、[EmpCenter クライアント サポート チーム](https://www.workforcesoftware.com/services/customer-support/)に問い合わせてください。 
...
1. **EmpCenter** 側にシングル サインオンを構成するには、ダウンロードした**メタデータ XML** を [EmpCenter サポート チーム](https://www.workforcesoftware.com/services/customer-support/)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
