...
3. **[Icertis Contract Management Platform のドメインと URL]** セクションで、以下の手順を実行します。
    ...
    a.[サインオン URL] ボックスに、次のパターンを使用して、ユーザーが RightScale アプリケーションへのサインオンに使用する URL を入力します。 **[サインオン URL]** ボックスに、`https://<company name>.icertis.com` のパターンを使用して URL を入力します。
