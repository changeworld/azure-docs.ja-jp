...
1. 検索ボックスに、「**Jobbadmin**」と入力します。

    ![Azure AD のテスト ユーザーの作成](./media/jobbadmin-tutorial/tutorial_jobbadmin_search.png)

1. 結果ウィンドウで **Jobbadmin** を選択し、**[追加]** をクリックして、アプリケーションを追加します。
...
1. **[Jobbadmin テスト ユーザーの作成](#creating-a-jobbadmin-test-user)** - Jobbadmin で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
1. **[Jobbadmin のドメインと URL]** セクションで、次の手順を実行します。
...
1. **Jobbadmin** 側にシングル サインオンを構成するには、ダウンロードした**メタデータ XML** を [Jobbadmin サポート チーム](https://www.jobbnorge.no/om-oss/kontakt-oss)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
...
1. アプリケーションの一覧で **[Jobbadmin]** を選択します。
