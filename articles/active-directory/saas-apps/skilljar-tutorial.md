...
1. **[Skilljar のドメインと URL]** セクションで、次の手順を実行します。
    ...
    > [!NOTE] 
    > これらは実際の値ではありません。 実際のサインオン URL と識別子でこれらの値を更新してください。 これらの値を取得するには、[Skilljar クライアント サポート チーム](https://support.skilljar.com/hc/)に連絡してください。 
...
1. **Skilljar** 側でシングル サインオンを構成するには、ダウンロードした**メタデータ XML** と、**名前識別子形式の値 - urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress** を、[Skilljar サポート チーム](https://support.skilljar.com/hc/)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
