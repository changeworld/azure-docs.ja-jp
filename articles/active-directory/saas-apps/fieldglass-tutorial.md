...
4. 検索ボックスに、「**Fieldglass**」と入力します。

    ![Azure AD のテスト ユーザーの作成](./media/fieldglass-tutorial/tutorial_fieldglass_search.png)

5. 結果ウィンドウで **Fieldglass** を選択し、**[追加]** をクリックして、アプリケーションを追加します。
...
3. **[Fieldglass のドメインと URL]** セクションで、次の手順を実行します。
...
6. **[Fieldglass 構成]** セクションで、**[Fieldglass の構成]** をクリックして、**[サインオンの構成]** ウィンドウを開きます。 **[クイック リファレンス]** セクションから **サインアウト URL と SAML エンティティ ID** をコピーします。

    ![[Configure Single Sign-On]](./media/fieldglass-tutorial/tutorial_fieldglass_configure.png) 

7. **Fieldglass** 側でシングル サインオンを構成するには、ダウンロードした**証明書 (Base64)** と **サインアウト URL および SAML エンティティ ID** を、[Fieldglass サポート チーム](http://www.fieldglass.com/solutions/support)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
