...
* Profit.co SAML App を構成したら、組織の機密データを流出と侵入からリアルタイムで保護するセッション制御を適用することができます。 セッション制御は、条件付きアクセスを拡張したものです。 [Microsoft Cloud App Security でセッション制御を強制する方法](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)をご覧ください。

## <a name="adding-profitco-saml-app-from-the-gallery"></a>ギャラリーからの Profit.co SAML App の追加
...
**B.Simon** というテスト ユーザーを使用して、Profit.co SAML App に対する Azure AD SSO を構成してテストします。 SSO を機能させるために、Azure AD ユーザーと Profit.co SAML App の関連ユーザーとの間にリンク関係を確立する必要があります。

Profit.co SAML App に対する Azure AD SSO を構成してテストするには、次の構成要素を完了します。
...
1. **[Profit.co SAML App SSO の構成](#configure-profitco-saml-app-sso)** - アプリケーション側でシングル サインオン設定を構成します。
    1. **[Profit.co SAML App テスト ユーザーの作成](#create-profitco-saml-app-test-user)** - Profit.co SAML App で B.Simon に対応するユーザーを作成し、Azure AD の B.Simon にリンクさせます。
...
1. [Azure portal](https://portal.azure.com/) の **Profit.co SAML App** アプリケーション統合ページで、 **[管理]** セクションを見つけて、 **[シングル サインオン]** を選択します。
...
**Profit.co SAML App**側でシングル サインオンを構成するには、**アプリのフェデレーション メタデータ URL** を [Profit.co SAML App サポート チーム](mailto:support@profit.co)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。

### <a name="create-profitco-saml-app-test-user"></a>Profit.co SAML App テスト ユーザーの作成

このセクションでは、Profit.co SAML App で Britta Simon というユーザーを作成します。  [Profit.co SAML App サポートチーム](mailto:support@profit.co)と協力して、Profit.co SAML App プラットフォームにユーザーを追加します。 シングル サインオンを使用する前に、ユーザーを作成し、有効化する必要があります。

## <a name="test-sso"></a>SSO のテスト 

このセクションでは、アクセス パネルを使用して Azure AD のシングル サインオン構成をテストします。

アクセス パネル上で [Profit.co SAML App] タイルをクリックすると、SSO を設定した Profit.co SAML App に自動的にサインインします。 アクセス パネルの詳細については、[アクセス パネルの概要](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)に関する記事を参照してください。
