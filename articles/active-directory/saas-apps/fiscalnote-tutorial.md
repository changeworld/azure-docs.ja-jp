...
1. **[Single sign-on]\(シングル サインオン\)** ダイアログの **[User Attributes]\(ユーザー属性\)** セクションで、上記の図に示すように SAML トークン属性を構成し、次の手順を実行します。
    ...
    d.[Tableau Server return URL]: Tableau Server ユーザーがアクセスする URL。 名前空間は空白のままにします。
...
>[!Note]
>ユーザーを手動で作成する必要がある場合は、 [FiscalNote のサポート チーム](mailto:support@fiscalnote.com)にお問い合わせください。
