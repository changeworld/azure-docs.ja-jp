...
1. 検索ボックスに「**EasyTerritory**」と入力し、結果ウィンドウで **EasyTerritory** を選び、**[追加]** をクリックして、アプリケーションを追加します。
...
1. **[EasyTerritory テスト ユーザーの作成](#create-a-easyterritory-test-user)** - EasyTerritory で Britta Simon に対応するユーザーを作成し、Azure AD の Britta Simon にリンクさせます。
...
1. **[EasyTerritory のドメインと URL]** セクションで、IDP 開始モードでアプリケーションを構成する場合は、次の手順を実行します。
...
1. **EasyTerritory** 側にシングル サインオンを構成するには、ダウンロードした**メタデータ XML** を [EasyTerritory サポート チーム](mailto:sales@easyterritory.com)に送信する必要があります。 サポート チームはこれを設定して、SAML SSO 接続が両方の側で正しく設定されるようにします。
...
1. アプリケーションの一覧で **[EasyTerritory]** を選択します。
