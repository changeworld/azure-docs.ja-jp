...
発行したアプリケーションにユーザーがアクセスできるようにするには、それらのユーザーを個別またはグループで割り当てる必要があります。 (自分自身へのアクセス権の割り当ても忘れずに行ってください)。各ユーザーを割り当てるには、Azure Basic 用、またはそれ以上のライセンスが必要があります。 ライセンスは個別に割り当てることも、グループに対して割り当てることもできます。 詳細については、次を参照してください。[をアプリケーションにユーザーを割り当てる](active-directory-applications-guiding-developers-assigning-users.md)です。 

事前認証を必要とするアプリの場合、ユーザーを割り当てると、アプリケーションを使用する権限を付与します。 必要としないアプリの事前認証をユーザーに割り当てることを意味、ユーザーがアクセス パネルで、アプリケーションにアクセスできることです。
...
アプリケーションを発行した後にアプリケーション プロキシ サービスを無効にした場合、アプリケーションは、プライベート ネットワークの外部からアクセス可能ではなくなりました。 ユーザーは通常どおりオンプレミス アプリケーション アクセスまだできます。
