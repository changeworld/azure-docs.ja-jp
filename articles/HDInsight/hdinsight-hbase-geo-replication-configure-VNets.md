<properties 
   pageTitle="2 つの Azure Virtual Network 間の VPN 接続の構成 | Azure" 
...
# 2 つの Azure Virtual Network 間の VPN 接続の構成  
...
Azure Virtual Network のサイト間接続では、VPN ゲートウェイを使用して Ipsec/IKE を使用する安全なトンネルが提供されます。VNet は異なるサブスクリプションおよび異なるリージョンであってもかまいません。マルチサイト構成と VNet 間通信を組み合わせることもできます。Vnet 間接続にはいくつかの理由があります。

- リージョン間の地理的冗長性および地理的プレゼンス 
...
**East US に Contoso-VNet-US という名前の仮想ネットワークを作成するには**
...
このチュートリアルでは、2 つの Azure Virtual Network 間に VPN 接続を構成する方法を説明しました。このシリーズの他の 2 つの記事は次のような内容です。

- [2 つの Azure Virtual Network 間の DNS の構成][hdinsight-hbase-geo-replication-dns]
