<properties pageTitle="Azure Redis Cache の使用方法" metaKeywords="" description="Learn how to create a use a cache in Azure Redis Cache" metaCanonical="" services="cache" documentationCenter="API Management" title="How to Use Azure Redis Cache" authors="sdanie" solutions="" manager="dwrede" editor="" />
...
**Azure Redis Cache**。サンプルは C\# コードで記述し、
.NET API を使用します。紹介するシナリオは、**キャッシュの作成と構成**、**キャッシュ クライアントの構成**、**キャッシュでのオブジェクトの追加と削除**、**キャッシュへの ASP.NET セッション状態の格納**などです。Azure Redis Cache の
使用方法の詳細については、「[次のステップ][]」を参照してください。
...
-	**標準** - 2 つのノード (マスター/スレーブ)、複数のサイズ、最大 53 GB99.9% の SLA。
...
-	**Standard** - 2 つのノード (マスター/スレーブ)、99.9% SLA、複数のサイズ、最大 53 GB
...
>[AZURE.NOTE] 警告: ソース コード内に資格情報を保存することは絶対に避けてください。このサンプルでは、単純化するためにあえてソース コード内に記述しています。資格情報の保存の詳細については、「[Windows Azure Websites: How Application Strings and Connection Strings Work (Windows Azure Websites: アプリケーション文字列と接続文字列の動作)][]」を参照してください。
...
>[AZURE.NOTE] 高度な接続構成オプションの詳細については、[StackExchange.Redis の構成モデル][]を参照してください。
...
項目は、`StringSet` および `StringGet` メソッドを使用して、キャッシュへの保存とキャッシュからの取得を行うことができます。
...
キャッシュ内のアイテムの有効期限を指定するには、`StringSet` の `TimeSpan` パラメーターを使用します。
...
ここまでで、Azure Redis Cache の基本について説明しました。
...
[キーの管理]: ./media/cache-dotnet-how-to-use-azure-redis-cache/redis-cache-manage-keys.png
...
[Azure Cache]: http://www.microsoft.com/ja-jp/showcase/Search.aspx?phrase=azure+caching
...
[キャッシュ内の .NET オブジェクトの操作]: http://msdn.microsoft.com/ja-jp/library/dn690521.aspx#Objects
...
[Redis データ型に関する 15 分の概要]: http://redis.io/topics/data-types-intro

[Windows Azure Websites: アプリケーション文字列と接続文字列の動作]: http://azure.microsoft.com/blog/2013/07/17/windows-azure-web-sites-how-application-strings-and-connection-strings-work/
