...
#### <a name="long-term-retention-of-backups"></a>バックアップの長期的な保有期間
35 日を超えるバックアップの長期的な保有期間は、現時点では、サービスによるネイティブなサポートはありません。 mysqldump を使用してバックアップを作成し、長期保存のために保存するという選択肢があります。 サポート チームでは、これを実現する方法を共有するために、[手順のブログ記事](https://techcommunity.microsoft.com/t5/azure-database-for-mysql/automate-backups-of-your-azure-database-for-mysql-server-to/ba-p/1791157)を作成しています。 
