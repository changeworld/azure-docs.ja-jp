...
```
> [!NOTE]
> Windows で MySQL コマンド ライン インターフェイスの新しいバージョンを使用している場合、エラー `SSL connection error: Certificate signature check failed` が表示されることがあります。 この場合、`--ssl-ca={filepath}` パラメーターを `--ssl` で置き換えます。
