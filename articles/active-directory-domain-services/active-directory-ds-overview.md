...
> [AZURE.NOTE] **パスワード同期は、ハイブリッド組織が Azure AD ドメイン サービスを使用する際に必須となります**。これは、Azure AD ドメイン サービスで提供される管理対象ドメインでは、NTLM または Kerberos 認証方法を使用してユーザーを認証する上で、それらのユーザーの資格情報が必要となることによる要件です。
