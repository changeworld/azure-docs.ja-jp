---
title: Azure Active Directory Domain Services:CentOS VM をマネージド ドメインに参加させる | Microsoft Docs
...
ここで、"contoso.com" は、マネージド ドメインの DNS ドメイン名です。 "contoso-centos" は、マネージド ドメインに参加させる CentOS 仮想マシンのホスト名です。
...
1. SSH ターミナルで次のコマンドを入力し、SSH を使用して、ドメインに参加した CentOS 仮想マシンに接続します。 マネージド ドメインに属するドメイン アカウントを使用します (例: ここでは 'bob@CONTOSO.COM')。
