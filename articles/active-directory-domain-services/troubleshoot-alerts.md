...
このエラーは回復できません。 アラートを解決するには、[既存の Azure AD DS マネージド ドメインを削除し](delete-aadds.md)、新しいディレクトリ内に再作成します。 マネージド ドメインの削除に問題がある場合は、[Azure サポート リクエストを開いて][azure-support]、追加のトラブルシューティングの支援を受けてください。
