...
# Azure AD Domain Services - トラブルシューティング ガイド
...
- SAMAccountName 形式 ('CONTOSO\\joeuser') ではなく UPN 形式 (たとえば、' joeuser@contoso.com') でサインインを試みます。場合によっては、UPN プリフィックスが極端に長いか、管理対象ドメイン上の別のユーザーと同じであるユーザーに対して SAMAccountName を自動的に生成できます。UPN 形式は、Azure AD テナント内で一意であることが保証されています。
...
- **極端に長い UPN プレフィックス:** Azure AD テナントの影響を受けているユーザーのアカウントの UPN プレフィックス (UPN の最初の部分) が 20 文字未満の長さであることを確認します。たとえば、UPN "joereallylongnameuser@contoso.com" はプレフィックス ("joereallylongnameuser") が 20 文字を超えているため、このアカウントは管理対象ドメインでは使用できません。

- **重複した UPN プレフィックス:** 影響を受けているユーザー アカウントの場合と同じ UPN プレフィックス (UPN の最初の部分) を持たないユーザー アカウントが Azure AD テナント内に他に存在していることを確認します。たとえば、"joeuser@finance.contoso.com" および "joeuser@engineering.contoso.com" という 2 つのユーザー アカウントがある場合、両方のユーザーは管理されたドメインにサインインする際に問題が発生します。この問題はユーザー アカウントのいずれかが外部アカウント (たとえば、"joeuser@live.com") である場合に発生する可能性があります。この問題の解決に取り組んでいます。
