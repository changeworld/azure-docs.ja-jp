...
このチュートリアルでは、デプロイしやすいよう単純化して、3 つのサブネットを含んだ単一の VNet を作成します。 運用環境のデプロイでは、[ハブとスポーク モデル](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/hub-spoke)を採用して、独自の VNet にファイアウォールを配置し、1 つ以上のサブネットを含む同じリージョンのピアリングされた VNet にワークロード サーバーを配置することをお勧めします。
...
> * ファイアウォールをデプロイする
> * 既定のルートを作成する
> * github.com へのアクセスを許可するようにアプリケーションを構成する
...
7. **[リソース グループ]** で、**[既存のものを使用]** > **[Test-FW-RG]** の順に選択します。
...
1. Azure portal のホーム ページで **[リソース グループ]** > **[Test-FW-RG]** の順にクリックします。
...
次の表の情報を使用して、Srv-Work 仮想マシンを構成します。 残りの構成は、Srv-Jump 仮想マシンと同じです。
...
11. **[仮想ネットワーク]** > **[Test-FW-VN]** の順にクリックします。
12. **[サブネット]** で、**[Workload-SN]** をクリックします。 必ずこのルートの **Workload-SN** サブネットのみを選択してください。それ以外の場合、ファイアウォールが正常に動作しません。

13. Click **OK**.
14. **[ルート]** > **[追加]** の順にクリックします。
...
これは、github.com へのアウトバウンド アクセスを許可するアプリケーション ルールです。

1. **Test-FW-RG** を開き、**[Test-FW01]** ファイアウォールをクリックします。
2. **Test-FW01** ページの **[設定]** で、**[ルール]** をクリックします。
3. **[アプリケーション ルール コレクション]** タブをクリックします。
4. **[アプリケーション ルール コレクションの追加]** をクリックします。
5. **[名前]** に「**App-Coll01**」と入力します。
6. **[優先度]** に「**200**」と入力します。
7. **[アクション]** で、**[許可]** を選択します。
8. **[ルール]** の **[ターゲットの FQDN]** で、**[名前]** に「**AllowGH**」と入力します。
9. **[ソース アドレス]** に「**10.0.2.0/24**」と入力します。
10. **[プロトコル:ポート]** に「**http, https**」と入力します。
11. **[ターゲットの FQDN]** に「**github.com**」と入力します
12. **[追加]** をクリックします。
...
1. **[ネットワーク ルール コレクション]** タブをクリックします。
...
6. セキュリティ アラートに対して **[OK]** > **[閉じる]** の順にクリックします。
