...
4. サブネットの名前は、既定で **GatewaySubnet** に設定されます。
   ...
   >[!IMPORTANT]
   >ゲートウェイ サブネットは特別であり、正しく動作するにはこの特定の名前である**必要があります**。
...
9. **[共有キー (PSK)]** に「**12345**」と入力します。 **[OK]** を選択します。

   >[!NOTE]
   >共有キーに別の値を使用する場合は、接続の別の端で作成した共有キーの値と一致する "*必要がある*" ことに注意してください。
...
Azure デプロイでは、ローカル ネットワーク ゲートウェイは、Azure の仮想ネットワーク ゲートウェイに接続するオンプレミスの (ユーザーの位置にある) 物理デバイスを表します。 一方、Azure Stack では、接続の両端が仮想ネットワーク ゲートウェイになります。
...
VPN 接続を確認するには、2 つの VM を作成する必要があります。1 つは Azure に作成し、もう 1 つは Azure Stack に作成します。 これらの VM を作成した後は、それらを使用して、VPN トンネル経由でデータを送受信することができます。
...
より厳密なデータ転送テストも行う必要があります。 たとえば、両方向に異なるサイズのファイルをコピーします。
