...
VPN ゲートウェイは、パブリック接続で暗号化されたトラフィックを送信する仮想ネットワーク ゲートウェイの一種です。 VPN ゲートウェイを使用すると、Azure Stack の仮想ネットワークと Azure の仮想ネットワークの間、または仮想ネットワークと VPN デバイスに接続されている別のネットワークの間で、トラフィックを安全に送信できます。

仮想ネットワーク ゲートウェイを作成する場合、作成するゲートウェイの種類を指定する必要があります。 Azure Stack では、仮想ネットワーク ゲートウェイの 1 つの種類である "Vpn" がサポートされています。

各仮想ネットワークに配置できる仮想ネットワーク ゲートウェイは 2 つですが、種類はいずれか 1 つのみになります。 選択する設定によっては、1 つの VPN ゲートウェイへの複数の接続を作成できます。 その一例は、マルチサイト接続構成です。
