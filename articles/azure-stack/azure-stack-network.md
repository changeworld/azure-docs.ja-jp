...
この /27 ネットワークは、前に説明した Azure Stack インフラストラクチャ サブネットからの小さな範囲であり、パブリック IP アドレスは必要ありませんが、NAT または透過プロキシ経由のインターネット アクセスが必要です。 このネットワークは、緊急回復コンソール システム (ERCS) に割り当てられます。ERCS VM は Azure への登録中にインターネット アクセスを必要とし、トラブルシューティングのために管理ネットワークにルーティング可能である必要があります。
