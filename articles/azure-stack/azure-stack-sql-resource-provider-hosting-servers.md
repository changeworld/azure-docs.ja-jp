...
* SQL インスタンスは、RP とユーザー ワークロードでのみ使用できるようにする必要があります。 他のコンシューマーに使用されている SQL インスタンス (App Services など) は使用できません。
* RP アダプターはドメインに参加しておらず、SQL 認証によってのみ接続できます。
* RP で使用するために、適切な権限を設定してアカウントを構成する必要があります。
* RP から SQL へのネットワーク トラフィックはポート 1433 を使用します。変更はできません。
...
* SQL インスタンスとそのホストの管理は管理者に任されています。RP は更新プログラムの適用、バックアップ、資格情報のローテーションなどを行いません。
...
sysadmin のフル権限に満たない権限の新しい管理ユーザーを作成できます。 許可が必要な特定の操作は次のとおりです。
...
SQL Server 2014 または SQL Server 2016 のいずれかのエディションを使って、スタンドアロンの (非 HA) SQL Server を使用できます。 システム管理特権を持つアカウントの資格情報があることを確認してください。
...
>[!NOTE]
SKU はポータルに表示されるまで最大 1 時間かかることがあります。 SKU の作成が完了するまで、データベースを作成することはできません。
...
* SQL Always On コンピューターだけでなく、ファイル サーバーも提供する必要がある。 この環境を独自に作成できる [Azure Stack クイックスタート テンプレート](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/sql-2016-ha)がある。 これは、独自のインスタンスを構築するためのガイドとしても役立ちます。
...
3. 使用する SQL Server インスタンスの接続詳細をフォームに入力します。必ず、Always On リスナーの FQDN または IPv4 アドレスを使用してください。 システム管理特権で構成したアカウントのアカウント情報を提供します。
...
ユーザーが SQL データベースを使用できるようにするプランとオファーを作成します。 プランに Microsoft.SqlAdapter サービスを追加し、既存のクォータを追加するか新しいクォータを作成します。 クォータを作成する場合、ユーザーに許可する容量を指定できます。
