...
```
**AzureRM.Bootstrapper** モジュールはプレビュー段階であるため、詳細事項や機能は変更されることがあります。 このモジュールの最新バージョンを PowerShell ギャラリーからダウンロードしてインストールするには、次のコマンドレットを実行します。
...
Azure Stack に必要な AzureRM モジュールをインストールするには、**Install-AzureRmProfile** コマンドレットを **2017-03-09-profile** という API バージョンのプロファイルと共に使用します。 

>[!NOTE]
>Azure Stack のクラウド管理者モジュールは、この API のバージョン プロファイルと一緒にインストールされません。 管理者モジュールは、「[PowerShell for Azure Stack のインストール](azure-stack-powershell-install.md)」の手順 3 で指定されているとおりに、個別にインストールする必要があります。
...
API バージョンのプロファイル内のモジュールを PowerShell ギャラリーで使用可能な最新バージョンのモジュールに更新するには、**Update-AzureRmProfile** コマンドレットを使用します。 モジュールをインポートするときの競合を回避するために、**Update-AzureRmProfile** コマンドレットは新しい PowerShell セッションで実行することをお勧めします。 **Update-AzureRmProfile** コマンドレットは、次のタスクを実行します。

1. 現在のスコープの特定の API バージョンのプロファイルに最新バージョンのモジュールがインストールされているかどうかをチェックします。  
2. モジュールがまだインストールされていない場合は、インストールするよう求めるプロンプトが表示されます。  
...
3. 最新バージョンのモジュールをインストールするよう求めるプロンプトが表示されます。  
4. 更新されたモジュールをインストールして現在の PowerShell セッションにインポートします。  
