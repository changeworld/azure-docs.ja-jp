...
ms.locfileid: "42889016"
---
# <a name="prepare-for-extension-host-for-azure-stack"></a>Azure Stack の拡張機能ホストを準備する

拡張機能ホストを使用することで、必要とされる TCP/IP ポートの数を減らして Azure Stack のセキュリティ保護を支援できます。 この記事では、Azure Stack で拡張機能ホストの準備を行う方法について説明します。この拡張機能ホストは、1808 更新プログラムより後の Azure Stack 更新プログラム パッケージを通じて自動的に有効になります。
...
3. ホスティング エンドポイントの証明書をインポートします。 環境に合わせてスクリプトを調整します。
...
5. 管理者ホスティング エンドポイントの証明書をインポートします。
