<properties 
	pageTitle="Microsoft Azure サービス管理の Web サイトを管理する方法" 
	description="Microsoft Azure のポータル Web サイト管理ページのリファレンス。Web サイト管理ページごとに詳細が示されています。 
	services="web-sites" 
	documentationCenter="" 
	authors="MikeWasson" 
	writer="mwasson" 
...
- **[ツールの入手]** - [WebMatrix のインストール][mswebmatrix] と [Microsoft Azure SDKのインストール][azuresdk] を行うためのリンクが表示されます。
...
**注:** ページの下部にある **[メトリックの追加]** を選択して管理ページの **[監視]** 管理ページにその他のパフォーマンス メトリックを追加できます。詳細については、[Web サイトの監視方法][監視] をご覧ください。

**[Web エンドポイントの状態]** - 監視用に構成された web エンドポイントの一覧。どのエンドポイントも構成されていない場合、**[Web エンドポイント監視の構成]** をクリックし、**[構成]** 管理ページの **[監視]** セクションに移動します。詳細については、 [Web サイトの監視方法][監視] をご覧ください。

**[自動スケールの状態]** - 標準モードでは、リソースを自動的にスケールして、必要なだけの金額を支払うようにできます。自動スケールを有効にするには、**[自動スケールの構成]** を選択します。**[スケール]** ページが表示されます。Web サイトが Free モードまたは Shared モードである場合、自動スケールを構成する前に、Standard モードに変更する必要があります (これは**[スケール]** ページで実行できます)。**[自動スケール操作のログ]** から **[管理サービス]** ポータルを開き、Web サイトの自動スケールの履歴を確認できます。既定のクエリの範囲は過去 24 時間ですが、既定のクエリは変更できます。
...
**[概要]** セクションに次の概要情報とリンクが表示されます (設定によっては、一部のオプションが表示されない場合もあります)。
 ...
 - **[適用可能なアドオンの表示]** - **[ストアから購入]** ダイアログ ボックスを開き、Web サイトに追加機能を提供するアドオンを選択して購入できます。リージョンや環境によっては、一部のアドオンが利用できない場合があります。
 ...
 - **[展開資格情報を設定する]** - FTP や Git を使用してコンテンツを Web サイトにアップロードするために必要なユーザー名とパスワードを作成する場合にクリックします。これらの資格情報を使用して、サブスクリプション内の任意の Web サイトにコンテンツをプッシュできます([FTP 資格情報] をご覧ください。)**注**: Microsoft アカウント (Live ID) の資格情報を使用した FTP ホストや Git リポジトリに対する認証はサポートされていません。
 ...
 - **[新しい展開スロットの追加]** - 標準モードのサイトで、サイトをステージング スロットに作成する場合、この機能を使用します。ステージング スロット (ステージング サイト) を使用してサイトのコンテンツと構成を検証した後で、それを運用環境にスワップできます。サイトのステージングを利用してコンテンツ更新を段階的に追加し、ステージング スロットでの更新が完了したら、サイトを運用環境にスワップすることもできます(既にステージング環境に存在するサイトにスロットを追加することはできません)。
 ...
 - **[ドロップボックスから切断]** - 展開目的でドロップボックスへの接続を設定している場合、このリンクを使って接続を切断できます。
 - **[Git リポジトリの削除]** - Git リポジトリを設定している場合、このリンクを使って削除できます。
 ...
 - **[コンピューティング モード]** - Web サイトが無料、共有、基本、標準のどのモードで実行されているかを示します。Web スケール グループ モードの詳細については、[Web サイトの規模変更方法][スケール] をご覧ください。
 - **[FTP ホスト名]** - FTP 経由での Web サイトへの発行に使用する URL を指定します ([FTP の資格情報] を参照)。
 - **[FTPS ホスト名]** - FTPS 経由での Web サイトへの発行に使用する URL を指定します ([FTP の資格情報] を参照)。
 - **[展開ユーザー/FTP ユーザー]** - FTP または Git 経由で Web サイトを Microsoft Azure にデプロイする際に使用されるアカウントを指定します ([FTP の資格情報] を参照)。
...
詳細については、[Web サイトの構成方法][構成] をご覧ください。


##スケール##
**[スケール]** 管理ページで、Web スケール グループ モード (**[無料]**、**[共有]**、**[基本]**、または **[標準]**) を指定できます。**[共有]**、**[基本]**、および **[標準]** の各モードでは、スループットとパフォーマンスが向上します。**[共有]**、**[基本]**、および **[標準]** の各モードでは、Web サイトおよび同じ Web スケール グループ内の他の Web サイトで使用する仮想マシンの数である **[インスタンス数]** を増やすことができます。
...
Web サイトのスケール オプションを構成する方法の詳細については、[Web サイトの規模の設定方法][スケール] をご覧ください。
...
- **[メトリックの追加]** - **[監視]** 管理ページの下部に表示されます。これにより、[監視] 管理ページに表示されるグラフにメトリックを追加できます。
- **[リンク]** - **[リンク済みリソース]** 管理ページの下部に表示され、他の Microsoft Azure リソースに対する管理リンク
-  を作成できます。たとえば、Web サイトで SQL データベースにアクセスする場合、**[リンク]** をクリックして、データベース リソースに対する管理リンクを作成できます。


## FTP の資格情報

使用できる FTP 資格情報のセットには、 *deployment*資格情報と *publishing profile*資格情報の 2 つがあります。主な違いは次のとおりです。
...
FTP の完全なユーザー名は、"Web サイト\ユーザー名" の形式です。これは、ポータルの **[概要]** に **[デプロイ / FTP ユーザー]** として一覧表示されます。
...
<code>publishMethod="FTP"</code> のプロファイルを探します。ユーザー名は <code>userName</code> 下に一覧表示され、パスワードは <code>userPWD</code> の下に一覧表示されます。
