...
-   [Web サイトのモード][Web サイトのモード]
-   [開発フレームワーク][開発フレームワーク]
-   [ファイル アクセス][ファイル アクセス]

    -   [ローカル ドライブ][ローカル ドライブ]
    -   [ネットワーク ドライブ ("UNC 共有" とも呼ばれます)][ネットワーク ドライブ ("UNC 共有" とも呼ばれます)]
    -   [複数のインスタンス間でのファイル アクセス][複数のインスタンス間でのファイル アクセス]
    -   [Web アプリケーションに付与されるファイル アクセスの種類][Web アプリケーションに付与されるファイル アクセスの種類]
-   [ネットワーク アクセス][ネットワーク アクセス]
-   [コードの実行、プロセス、メモリ][コードの実行、プロセス、メモリ]
-   [診断ログとイベント][診断ログとイベント]
-   [レジストリ アクセス][レジストリ アクセス]
...
クラウド サービスの性質上、UNC 共有をホストする仮想マシンはその時々で変わります。通常のクラウド運用で仮想マシンが起動または停止するたびに、必要な仮想マシンに UNC 共有が確実にマウントされます。したがって、UNC ファイル パスのマシン情報が常に変わらないことを前提として Web アプリケーションをハードコーディングしないでください。代わりに、Azure Websites に用意されている "偽" の絶対パス **D:\\home\\site** を使用してください。この "偽" の絶対パスを使用すれば、サイトとユーザーが変わっても常に目的のサイトを表すことができます。つまり、**D:\\home\\site** を使用することで、共有ファイルを別のサイトへ移動するたびに新しい絶対パスを構成する必要がなくなります。
...
Azure Websites の各 Web サイトは、独自のランダムな低権限ワーカー プロセス ID として実行されます。これを "アプリケーション プール ID" といいます (詳細については [][]<http://www.iis.net/learn/manage/configuring-security/application-pool-identities></a> を参照)。アプリケーション コードはこの ID を使用し、オペレーティング システム ドライブ (D:\\ ドライブ) に読み取り専用でアクセスします。つまり、アプリケーション コードは、オペレーティング システム ドライブ上の共通ディレクトリ構造を参照し、共通ファイルを読み取ることができます。少々広範なアクセス レベルに思えるかもしれませんが、Azure ホステッド サービスに worker ロールをプロビジョニングしてドライブ コンテンツを読み取る場合も、同じディレクトリとファイルにアクセスできます。
