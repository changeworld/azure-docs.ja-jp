...
メッセージを Service Bus トピックに送信するには、アプリケーションで **ServiceBusService** オブジェクトの **send\_topic\_message** メソッドを呼び出します。
...
パラメーター**peek\_lock** が **False** に設定されていると、メッセージが読み取られるときにサブスクリプションから削除されます。キューからメッセージを削除せずに、メッセージを読み取って (ピークして) ロックするには、
**peek\_lock** パラメーターを **True** に設定します。
