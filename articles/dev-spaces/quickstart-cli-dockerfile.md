---
title: Azure Dev Spaces を使用して Kubernetes 上に Dockerfile でアプリケーションをデプロイする
...
ms.date: 07/08/2019
ms.topic: quickstart
description: Azure Dev Spaces を使用して AKS 上に Dockerfile でマイクロサービスをデプロイする
...
ms.locfileid: "67710695"
---
# <a name="quickstart-develop-an-application-with-a-dockerfile-on-kubernetes-using-azure-dev-spaces"></a>クイック スタート:Azure Dev Spaces を使用して Kubernetes 上で Dockerfile でアプリケーションを開発する
...
この記事では、[Azure Dev Spaces サンプル アプリケーション](https://github.com/Azure/dev-spaces)を使用して、Azure Dev Spaces の使い方のデモを行います。 サンプル アプリケーションは Go で記述されていますが、アプリケーションをコンテナー化し、それを AKS で実行するのに有効な Dockerfile を提供する限り、ほぼすべての言語を使用することができます。

GitHub からアプリケーションを複製して、*dev-spaces/samples/golang/getting-started/webfrontend* ディレクトリに移動します。
...
サンプル アプリケーションには、有効な Dockerfile が含まれています。 Kubernetes でアプリケーションを実行するための Helm チャート資産を生成するには、`azds prep` コマンドを使用します。

```cmd
azds prep --public
```

Helm チャート資産を正しく生成するには、*dev-spaces/samples/golang/getting-started/webfrontend* ディレクトリから `prep` コマンドを実行する必要があります。
...
パブリック URL を開き、`/api` パスに移動すると、実行中のサービスを確認できます。 パブリック URL は、`azds up` コマンドの出力に表示されます。 この例では、パブリック URL は `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/` で、`http://default.webfrontend.1234567890abcdef1234.eus.azds.io/api` に移動します。
...
1. [`src/app/main.go` の 29 行目](https://github.com/Azure/dev-spaces/blob/master/samples/golang/getting-started/webfrontend/src/app/main.go#L29)を以下に更新します。
