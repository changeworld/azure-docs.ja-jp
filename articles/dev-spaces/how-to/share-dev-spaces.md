...
1. 各チーム メンバーに、[Azure Dev Spaces コントローラーにアクセスするための適切なアクセス許可](../troubleshooting.md#incorrect-rbac-permissions-for-calling-dev-spaces-controller-and-apis)があることを確認します。 たとえば、Azure Dev Space の**リソース グループ**を構成して、各チーム メンバーに対して[共同作成者アクセスを付与](/azure/active-directory/role-based-access-control-configure)できます。 開発空間のリソース グループは、`azds show-context` というコマンドを実行して確認できます。
