...
3. 発行先の選択を求められた場合は、**[Microsoft Azure App Service]**、**[次へ]** の順にクリックして、必要であれば Azure の資格情報でサインインします。Visual Studio によって Azure から発行設定が直接ダウンロードされ、安全に保存されます。
...
5. 次の例に示すように、App\_Start/Startup.MobileApp.cs ファイルに **MapApiControllers** 拡張メソッドの呼び出しを追加します。
...
ローカル アプリケーション構成の *authSigningKey* パラメーターでこの署名キーを使用します。モバイル バックエンドは、ローカルで実行されているときに、クライアントがクラウドベースのエンドポイントから取得するトークンを検証できるようになりました。
