...
9. 名前空間に次のクラス定義を追加します。
   ...
   > [!NOTE]
   > **<PROJECT\_NUMBER>** を、前にメモしたプロジェクト番号に置き換えます。
   > 
   > 
...
11. **OnRegistered** イベント ハンドラーをオーバーライドし、**Register** メソッドを実装する次のコードを **GcmService** クラスに追加します。
...
4. **[実行]** をクリックしてプロジェクトをビルドし、Android デバイスまたはエミュレーターでアプリを開始します。
...
3. **AppDelegate** に次の **DidReceivedRemoteNotification** イベント ハンドラーのオーバーライドも追加します。
