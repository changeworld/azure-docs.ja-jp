...
2. `npm init` を実行して、パッケージの構造を初期化します。
   ...
   ![npm init の出力][0]
3. npm リポジトリから `express` ライブラリと `azure-mobile-apps` ライブラリをインストールします。
...
6. 最初の Node.js アプリケーションを作成した後に、npm カタログの更新が必要になる場合があります。 必要に応じて、**[最新の情報に更新]** を選択します。
...
2. SQL Server 2014 構成マネージャーを実行します。
   ...
   a.[サインオン URL] ボックスに、次のパターンを使用して、ユーザーが RightScale アプリケーションへのサインオンに使用する URL を入力します。 ツリー メニューの **[SQL Server ネットワークの構成]** ノードを展開します。
...
3. SQL Server 2014 Management Studio を実行して、ローカルの SQL Server Express インスタンスに接続します。
...
6. アプリ設定が既に存在する場合は、アプリ設定の値を選択して値を編集します。
...
4. 既定の App Service プランが選択されています。 App Service プランを変更する場合:
   ...
   a.[サインオン URL] ボックスに、次のパターンを使用して、ユーザーが RightScale アプリケーションへのサインオンに使用する URL を入力します。 **[App Service プラン]** > **[+ 新規作成]** の順に選択します。 
