...
このガイドでは、クライアント側の Android SDK に注目します。  Mobile Apps 用のサーバー側 SDK の詳細については、[.NET バックエンド SDK の操作][10]または [Node.js SDK の使用方法][11]に関するページを参照してください。
...
- Andoroid アプリで使用する[モバイル アプリ バックエンドを作成する][13]。
- Android Studio で [Gradle ビルド ファイルを更新する](#gradle-build)。
- [インターネット アクセス許可を有効にする](#enable-internet)。

###<a name="<a-name="gradle-build"></a>update-the-gradle-build-file"></a><a name="gradle-build"></a>Gradle ビルド ファイルを更新する
...
###<a name="<a-name="enable-internet"></a>enable-internet-permission"></a><a name="enable-internet"></a>インターネット アクセス許可を有効にする
...
###<a name="<a-name="data-object"></a>define-client-data-classes"></a><a name="data-object"></a>クライアント データ クラスを定義する
...
Mobile Apps バックエンドに追加のテーブルを作成する方法については、「[方法: テーブル コントローラーを定義する][15]」(.NET バックエンド) または「[方法: 動的スキーマを使用してテーブルを定義する][16]」(Node.js バックエンド) を参照してください。 Node.js バックエンドでは、 **Azure ポータル** の [[テーブルの簡単操作]]設定を使用することもできます。

###<a name="<a-name="create-client"></a>how-to:-create-the-client-context"></a><a name="create-client"></a>方法: クライアント コンテキストを作成する
...
このコードで、`MobileAppUrl` をモバイル アプリ バックエンドの URL に置き換えます。この URL は、[Azure ポータル]のモバイル アプリ バックエンド用ブレードで確認できます。 このコード行をコンパイルするには、次の **import** ステートメントも追加する必要があります。
...
###<a name="<a-name="binding"></a>how-to:-bind-data-to-the-user-interface"></a><a name="binding"></a>方法: データをユーザー インターフェイスにバインドする
...
#### <a name="<a-name="layout"></a>how-to:-define-the-layout"></a><a name="layout"></a>方法: レイアウトを定義する
...
#### <a name="<a-name="adapter"></a>how-to:-define-the-adapter"></a><a name="adapter"></a>方法: アダプターを定義する
...
### <a name="<a-name="api"></a>the-api-structure"></a><a name="api"></a>API の構造
...
サンプルについては、 **Azure Portal** で Android クイックスタート プロジェクトに含まれる [ToDoActivity.java] ファイルを参照してください。

#### <a name="<a-name="use-adapter"></a>how-to:-use-the-adapter"></a><a name="use-adapter"></a>方法: アダプターを使用する
...
##<a name="<a-name="querying"></a>how-to:-query-data-from-your-mobile-app-backend"></a><a name="querying"></a>方法: モバイル アプリ バックエンドのデータを照会する
...
### <a name="<a-name="showall"></a>how-to:-return-all-items-from-a-table"></a><a name="showAll"></a>方法: テーブルからすべての項目を返す
...
### <a name="<a-name="chaining"></a>how-to:-concatenate-query-methods"></a><a name="chaining"></a>方法: クエリ メソッドを連結する
...
##<a name="<a-name="inserting"></a>how-to:-insert-data-into-the-backend"></a><a name="inserting"></a>方法: バックエンドにデータを挿入する
...
##<a name="<a-name="updating"></a>how-to:-update-data-in-a-mobile-app"></a><a name="updating"></a>方法: モバイル アプリのデータを更新する
...
##<a name="<a-name="deleting"></a>how-to:-delete-data-in-a-mobile-app"></a><a name="deleting"></a>方法: モバイル アプリのデータを削除する
...
##<a name="<a-name="lookup"></a>how-to:-look-up-a-specific-item"></a><a name="lookup"></a>方法: 特定の項目を検索する
...
##<a name="<a-name="untyped"></a>how-to:-work-with-untyped-data"></a><a name="untyped"></a>方法: 型指定のないデータを処理する
...
### <a name="<a-name="json_instance"></a>how-to:-create-an-instance-of-an-untyped-table"></a><a name="json_instance"></a>方法: 型指定のないテーブルのインスタンスを作成する
...
### <a name="<a-name="json_insert"></a>how-to:-insert-into-an-untyped-table"></a><a name="json_insert"></a>方法: 型指定のないテーブルに挿入する
...
### <a name="<a-name="json_delete"></a>how-to:-delete-from-an-untyped-table"></a><a name="json_delete"></a>方法: 型指定のないテーブルから削除する
...
### <a name="<a-name="json_get"></a>how-to:-return-all-rows-from-an-untyped-table"></a><a name="json_get"></a>方法: 型指定のないテーブルからすべての行を返す
...
##<a name="<a-name="custom-api"></a>how-to:-call-a-custom-api"></a><a name="custom-api"></a>方法: カスタム API の呼び出し
...
##<a name="<a-name="authentication"></a>how-to:-add-authentication-to-your-app"></a><a name="authentication"></a>方法: アプリに認証を追加する
...
### <a name="<a-name="caching"></a>how-to:-add-authentication-code-to-your-app"></a><a name="caching"></a>方法: アプリケーションに認証コードを追加する

次のコードでは、Google プロバイダーを使用してサーバー フローのログイン プロセスを開始します。

    MobileServiceUser user = mClient.login(MobileServiceAuthenticationProvider.Google);

**getUserId** メソッドを使用して **MobileServiceUser** からログイン ユーザーの ID を取得します。 Futures を使用して非同期ログイン API を呼び出す方法の例については、「 [Android アプリに認証を追加する]」を参照してください。

### <a name="<a-name="caching"></a>how-to:-cache-authentication-tokens"></a><a name="caching"></a>方法: 認証トークンをキャッシュする
...
## <a name="<a-name="adal"></a>how-to:-authenticate-users-with-the-active-directory-authentication-library"></a><a name="adal"></a>方法: Active Directory 認証ライブラリを使用してユーザーを認証する
...
* **INSERT-REDIRECT-URI-HERE** を、HTTPS スキームを使用して、サイトの _/.auth/login/done_ エンドポイントに置き換えます。 この値は、_https://contoso.azurewebsites.net/.auth/login/done_ のようにする必要があります。
...
##<a name="<a-name="customizing"></a>how-to:-customize-the-client"></a><a name="customizing"></a>方法: クライアントをカスタマイズする
...
### <a name="<a-name="serialization"></a>how-to:-customize-serialization"></a><a name="serialization"></a>方法: シリアル化をカスタマイズする
...
### <a name="<a-name="columns"></a>how-to:-map-different-client-and-server-names"></a><a name="columns"></a>方法: クライアントとサーバーの間で異なる名前をマップする
...
### <a name="<a-name="table"></a>how-to:-map-different-table-names-between-the-client-and-the-backend"></a><a name="table"></a>方法: クライアントとバックエンドの間で異なるテーブル名をマップする

クライアント テーブルの名前を別のモバイル サービス テーブルの名前にマップするには、[getTable()][4] メソッドのオーバーライドを使用します。

    mToDoTable = mClient.getTable("ToDoItemBackup", ToDoItem.class);

### <a name="<a-name="conversions"></a>how-to:-automate-column-name-mappings"></a><a name="conversions"></a>方法: 列名のマッピングを自動化する
...
### <a name="<a-name="complex"></a>how-to:-store-an-object-or-array-property-into-a-table"></a><a name="complex"></a>方法: オブジェクトまたは配列プロパティをテーブルに保存する
...
<!-- URLs. -->
[Azure Mobile Apps の使用]: app-service-mobile-android-get-started.md
[ASCII 制御コード C0 および C1 に関するページ]: http://en.wikipedia.org/wiki/Data_link_escape_character#C1_set
[Android のための Mobile Services SDK]: http://go.microsoft.com/fwlink/p/?LinkID=717033
