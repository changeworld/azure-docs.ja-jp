...
Azure で socket.IO ベースのチャット アプリケーションをホストする手順を説明します。Socket.IO の
...
3.  アプリケーションが適切なポートでリッスンするように、
    ...
    次の行の **3000** を **process.env.port** に変更します。
...
server.js の変更内容を保存した後、次の手順に従って
必要なモジュールをインストールし、
...
4.  4.Azure にアプリケーションをデプロイするには、
    **Publish-AzureServiceProject** コマンドレットを使用します。次に例を示します。
	...
	> 
	> 指定したサブスクリプション名がインポートされた発行プロファイルに存在しないというエラーが出力された場合は、Azure にデプロイする前に、サブスクリプションの発行プロファイルをダウンロードしてインストールする必要があります。「[Node.js アプリケーションの構築と Azure のクラウド サービスへのデプロイ](https://www.windowsazure.com/ja-jp/develop/nodejs/tutorials/getting-started/)」の「**Azure へのアプリケーションのデプロイ**」を参照してください。

    ![A browser window displaying the service hosted on Azure][completed-app]

	> [AZURE.NOTE] 指定したサブスクリプション名がインポートされた発行プロファイルに存在しないというエラーが出力された場合は、Azure にデプロイする前に、サブスクリプションの発行プロファイルをダウンロードしてインストールする必要があります。「[Node.js アプリケーションの構築と Azure のクラウド サービスへのデプロイ](https://www.windowsazure.com/ja-jp/develop/nodejs/tutorials/getting-started/)」の「**Azure へのアプリケーションのデプロイ**」を参照してください。
...
このチュートリアルでは、Azure Cloud Services でホストされる基本的なチャット アプリケーションを作成する方法を説明しました。Azure Web サイトでこのアプリケーションをホストする方法については、「[Azure Web サイトで Socket.IO を使用する Node.js チャット アプリケーションを構築する][chatwebsite]」を参照してください。
  ...
  [Azure の SLA]: http://azure.microsoft.com/support/sla/
  ...
  [Publish-AzureService コマンドの出力]: ./media/cloud-services-nodejs-chat-app-socketio/socketio-9.png
