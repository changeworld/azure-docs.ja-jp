...
ConfigurationManager を使ってボルトおよびスパウト コンポーネントから構成値を取得しないでください。null ポインター例外が発生します。 代わりに、プロジェクトの構成は、トポロジ コンテキストのキー/値ペアとして Storm トポロジに渡されます。 構成値に依存する各コンポーネントは、初期化の間にコンテキストから値を取得する必要があります。
