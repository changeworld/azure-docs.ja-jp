...
Data Lake Tools for Visual Studio では、Tez 実行エンジンが実行している Hive ジョブのパフォーマンス グラフを表示できます。 Tez の有効化の情報については、[HDInsight での Apache Hive の使用](hdinsight-use-hive.md)に関するページを参照してください。 
