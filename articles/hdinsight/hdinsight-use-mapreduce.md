...
> [!NOTE]
> 上の構文 **wasbs:///** は HDInsight クラスターの既定のストレージ コンテナーに格納されたファイルにアクセスするために使用します。クラスターをプロビジョニングするときに追加のストレージ アカウントを指定し、そのアカウントに格納されたファイルにアクセスする必要がある場合、コンテナー名とストレージ アカウント アドレスを指定することによって、データにアクセスすることができます。たとえば、**wasbs://mycontainer@mystorage.blob.core.windows.net/example/data/gutenberg/davinci.txt** のように指定します。
> 
> 
...
> [!NOTE]
> HDInsight 2.1 クラスターの場合、ファイルの場所は **wasbs:///example/jars/hadoop-examples.jar** です。
> 
> 
...
|:--- |:--- |:--- |:--- |
| [SSH](hdinsight-hadoop-use-mapreduce-ssh.md) |**SSH** を介して Hadoop コマンドを使用する |Linux |Linux、Unix、Mac OS X、または Windows |
| [Curl](hdinsight-hadoop-use-mapreduce-curl.md) |**REST** を使用してジョブをリモートで送信する |Linux または Windows |Linux、Unix、Mac OS X、または Windows |
| [Windows PowerShell](hdinsight-hadoop-use-mapreduce-powershell.md) |**Windows PowerShell** を使用してジョブをリモートで送信する |Linux または Windows |Windows |
| [リモート デスクトップ](hdinsight-hadoop-use-mapreduce-remote-desktop.md) |**リモート デスクトップ**を介して Hadoop コマンドを使用する |Windows |Windows |
