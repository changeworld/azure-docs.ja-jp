...
* __カスタム DNS__: Azure には、Azure Virtual Network にインストールされている Azure サービスの名前解決が用意されています。 この名前解決は、仮想ネットワークの外部には拡張されません。 仮想ネットワークの外部にあるリソースの名前解決を有効にするには、カスタム DNS サーバーを使用する必要があります。 カスタム DNS サーバーの使用方法の詳細については、「[VM とロール インスタンスの名前解決](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md#name-resolution-using-your-own-dns-server)」をご覧ください。

* __強制トンネリング__: HDInsight では、Azure Virtual Network の強制トンネリング構成をサポートしていません。

* __ネットワーク トラフィックの制限__: HDInsight では、ネットワーク セキュリティ グループを使用したネットワーク トラフィックの制限をサポートしていますが、いくつかの Azure IP に対する無制限のアクセスが必要です。 詳細については、「[セキュリティで保護された仮想ネットワーク](#secured-virtual-networks)」のセクションを参照してください。
...
## <a name="secured-virtual-networks"></a>セキュリティで保護された仮想ネットワーク
...
許可される IP アドレスは、HDInsight クラスターと Virtual Network が存在するリージョンに固有である必要があります。 次の表を参照して、使用しているリージョンに適した IP アドレスを見つけてください。
...
| 米国 | 米国中西部 | 52.161.23.15</br>52.161.10.167 | 443 |
...
### <a name="working-with-hdinsight-in-secured-virtual-networks"></a>セキュリティで保護された仮想ネットワークでの HDInsight の使用

インターネット アクセスをブロックすると、通常はクラスターのパブリック ゲートウェイを介して公開される HDInsight サービスを利用できません。 これには Ambari と SSH が含まれます。 代わりに、クラスターのヘッド ノードの内部 IP アドレスを使用してサービスにアクセスする必要があります。
