...
スケールアップの場合、HDInsight サービスによって、現在の CPU とメモリの要件を満たすために必要な新しいワーカー ノードの数が計算された後、必要な数のノードを追加するスケールアップ要求が発行されます。

スケールダウンの場合、ノードごとの AM コンテナーの数および現在の CPU とメモリの要件に基づき、自動スケーリングにより、特定の数のノードを削除する要求が発行されます。 また、サービスでは、現在のジョブの実行に基づいて、削除候補のノードも検出されます。 スケールダウン操作では、最初にノードの使用が停止された後、クラスターから削除されます。
...
1. **[Configuration + pricing]\(構成と価格\)** タブで、 **[Enable autoscale]\(自動スケーリングの有効化\)** チェック ボックスをオンにします。
1. **[Autoscale type]\(自動スケーリングの種類\)** で、 **[Load-based]\(負荷ベース\)** を選択します。
1. 次のプロパティに希望する値を入力します。  

    * **[Worker node]\(ワーカー ノード\)** の初期の **[Number of nodes]\(ノードの数\)** 。
    * ワーカー ノードの **[Min]\(最小\)** の数。
    * ワーカー ノードの **[Max]\(最大\)** の数。
...
1. **[Worker node]\(ワーカー ノード\)** の **[Number of nodes]\(ノードの数\)** を入力します。これは、クラスターのスケールアップの制限を制御します。
1. **[Autoscale type]\(自動スケーリングの種類\)** で、 **[Schedule-based]\(スケジュール ベース\)** オプションを選択ます。
1. **[Configure]\(構成\)** を選択して **[Autoscale configuration]\(自動スケーリングの構成\)** ウィンドウを開きます。
...
負荷ベースとスケジュール ベースの両方のスケーリングに対して、 **[Node size]\(ノード サイズ\)** の下のドロップダウン リストから VM を選択して、ワーカー ノードの VM の種類を選択します。 ノードの種類ごとに VM の種類を選択すると、クラスター全体の概算のコスト範囲を表示できるようになります。 予算に合わせて VM の種類を調整します。
...
実行中のクラスターで自動スケーリングを有効にするには、 **[Settings]\(設定\)** の **[Cluster size]\(クラスター サイズ\)** を選択します。 次に、 **[Enable autoscale]\(自動スケーリングの有効化\)** を選択します。 使用する自動スケーリングの種類を選択し、負荷ベースまたはスケジュール ベースのスケーリングのオプションを入力します。 最後に、 **[Save]\(保存\)** を選択します。
...
クラスターの現在のノード数を見るには、クラスターの **[Overview]\(概要\)** ページの **[Cluster size]\(クラスター サイズ\)** グラフに移動するか、または **[Settings]\(設定\)** の **[Cluster size]\(クラスター サイズ\)** を選択します。
