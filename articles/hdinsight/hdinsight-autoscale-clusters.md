...
ノードごとの AM コンテナーの数および現在の CPU とメモリの要件に基づいて、自動スケーリングにより、どのノードが削除の潜在的な候補であるかが指定され、特定の数のノードを削除するための要求が発行されます。 既定では、1 サイクルで 2 つのノードが削除されます。
