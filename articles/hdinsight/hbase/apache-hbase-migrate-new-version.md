---
title: "HBase クラスターを新しいバージョンに移行する - Azure HDInsight | Microsoft Docs"
...
6. 古いクラスター (https://OLDCLUSTERNAME.azurehdidnsight.net) の Ambari にログインし、HBase サービスを停止します。 サービスを停止することを確認するプロンプトが表示されたら、HBase のメンテナンス モードをオンにするためのチェックボックスをオンにします。 Ambari への接続とその使用方法の詳細については、「[Ambari Web UI を使用した HDInsight クラスターの管理](../hdinsight-hadoop-manage-ambari.md)」を参照してください。
