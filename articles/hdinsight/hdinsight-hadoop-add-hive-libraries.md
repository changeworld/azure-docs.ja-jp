...
* コンテナーへの WASB パスは、スクリプト アクションのパラメーターとして指定する必要があります。たとえば、jar が __mystorage__ という名前のストレージ アカウントの __libs__ というコンテナーに格納されている場合、パラメーターは wasb://libs@mystorage.blob.core.windows.net/__ となります。
...
2. **[オプションの構成]** ブレードで **[スクリプト アクション]** を選択し、以下のように情報を指定します。
    ...
    * __[パラメーター]__: jar が格納されているコンテナーとストレージ アカウントの WASB アドレスを入力します。たとえば、「\_\_wasb://libs@mystorage.blob.core.windows.net/__」のように入力します。
