...
クラスター外部のクライアントに Beeline がインストールされている場合は、次のコマンドを使用して接続できます。 __CLUSTERNAME__ を、使用する HDInsight クラスターの名前に置き換えます。 __PASSWORD__ を管理者 (HTTP ログイン) アカウントのパスワードに替えます。
