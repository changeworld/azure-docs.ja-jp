...
4. 次のコマンドを使用して、ファイルに含まれている HiveQL を実行します。
	...
	Hive シェルが読み込まれ、__twitter.hql__ ファイル内の HiveQL が実行され、最後に `hive >` プロンプトが返されます。
	
5. `hive >` プロンプトで、次のステートメントを使用して、__twitter.hql__ ファイル内の HiveQL によって作成された __tweets__ テーブルからデータを選択できることを確認します。
