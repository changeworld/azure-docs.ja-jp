...
HDInsight で Apache Hadoop クラスターを作成するときに、**Azure ストレージ** アカウントまたは **Data Lake Storage** のどちらかを指定します。 そのアカウントの特定のストレージ コンテナーが、作成したクラスターのファイル システム (Hadoop 分散ファイル システムなど) を保持します。 詳しい情報とガイダンスについては、次のドキュメントを参照してください。
