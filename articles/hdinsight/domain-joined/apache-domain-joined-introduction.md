---
title: HDInsight - ドメイン参加済み HDInsight クラスター - Azure
description: 詳細....
services: hdinsight
author: omidm1
manager: jhubbard
...
> [!NOTE]
> この記事で説明する新機能を使用できるクラスターの種類は、Hadoop、Spark、および対話型クエリのみです。 HBase、Storm、Kafka など、他のワークロードには今後のリリースで対応する予定です。
...
![ドメイン参加済み HDInsight クラスターのメリットの柱](./media/apache-domain-joined-introduction/hdinsight-domain-joined-four-pillars.png)が必要です。
...
HDInsight クラスター リソースを未承認のユーザーから保護し、データをセキュリティで保護すると同時に、リソースへの未承認のアクセスや誤ったアクセスを追跡するために、クラスター リソースとデータへのすべてのアクセスを監査する必要があります。 管理者は HDInsight クラスター リソースとデータへのすべてのアクセスを表示し、レポートを作成できます。 また、管理者は Apache Ranger のサポートされているエンドポイントで行われたアクセス制御ポリシーのすべての変更を表示し、レポートを作成することもできます。 ドメイン参加済み HDInsight クラスターでは、使い慣れた Apache Ranger UI を使用して監査ログを検索します。 バックエンドでは、Ranger はログの保存と検索に [Apache Solr](http://hortonworks.com/apache/solr/) を使用します。
...
* VSCode を使用してドメイン参加済みクラスターへのリンクを作成するには、[VSCode を使用した、ドメイン参加済みクラスターへのリンク](../hdinsight-for-vscode.md#linkcluster)に関する記事を参照してください。
* IntelliJ を使用して、ドメイン参加済みクラスターへのリンクを作成するには、[IntelliJ を使用した、ドメイン参加済みクラスターへのリンク](../spark/apache-spark-intellij-tool-plugin.md#linkcluster)に関する記事を参照してください。
* Eclipse を使用してドメイン参加済みクラスターへのリンクを作成するには、[Eclipse を使用した、ドメイン参加済みクラスターへのリンク](../spark/apache-spark-eclipse-tool-plugin.md#linkcluster)に関する記事を参照してください。
