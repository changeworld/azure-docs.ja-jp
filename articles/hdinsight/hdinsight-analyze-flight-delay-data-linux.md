...
> [AZURE.NOTE] このドキュメントの各部分は Windows ベースの HDInsight クラスター (Python と Hive など) で使用できますが、多くの手順は Linux ベースのクラスターに固有のものです。 Windows ベースのクラスターでの手順については、「 [HDInsight での Hive を使用したフライト遅延データの分析](hdinsight-analyze-flight-delay-data.md)
...
SQL Database が既にある場合は、サーバー名を取得していることになります。 サーバー名を確認するには、 [Azure ポータル](https://portal.azure.com) で __[SQL データベース]__を選択し、使用するデータベースの名前でフィルターをかけます。 サーバー名は __[サーバー]__ 列に表示されます。
...
8.  `exit` at the `1>` 」と入力して、tsql ユーティリティを終了します。
