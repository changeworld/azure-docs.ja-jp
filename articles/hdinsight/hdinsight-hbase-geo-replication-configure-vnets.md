---
title: "2 つの仮想ネットワーク間の VPN 接続の構成 | Microsoft Docs"
...
---
# <a name="configure-a-vpn-connection-between-two-azure-virtual-networks"></a>2 つの Azure 仮想ネットワーク間の VPN 接続の構成
...
## <a name="configure-a-vpn-connection-between-the-two-vnets"></a>2 つの Vnet 間の VPN 接続の構成
### <a name="create-local-networks"></a>ローカル ネットワークの作成
...
1. Azure クラシック ポータルで、**[新規]**、**[NETWORK SERVICES]**、**[仮想ネットワーク]**、**[ローカル ネットワークの追加]** の順にクリックします。
2. 次のように入力します。
...
### <a name="create-vpn-gateways"></a>VPN ゲートウェイの作成
...
1. Azure クラシック ポータルの左側のウィンドウで **[ネットワーク]** をクリックします。
...
1. Azure クラシック ポータルで **[ダッシュボード]** タブをクリックします。
2. ページ下部の **[ゲートウェイの作成]** をクリックし、**[動的ルーティング]** をクリックします。
3. **[はい]** をクリックして確定します。 ページ上のゲートウェイの図が黄色に変わり、"ゲートウェイを作成しています" と表示されることを確認します。 ゲートウェイが作成されるまでに、通常、15 分ぐらいかかります。
...
4. **[ゲートウェイ IP アドレス]**をコピーします。 次のセクションでは、これを使用して Contoso-VNet-EU の VPN ゲートウェイ IP アドレスを構成します。
...
### <a name="set-the-vpn-device-ip-addresses-for-local-networks"></a>ローカル ネットワークの VPN デバイス IP アドレスの設定
...
### <a name="set-vnet-gateway-keys"></a>VNet ゲートウェイ キーの設定
Vnet ゲートウェイは、共有キーを使用して仮想ネットワーク間の接続を認証します。 Azure クラシック ポータルからキーを構成することはできません。 PowerShell または .NET SDK を使用する必要があります。
...
## <a name="check-the-vpn-connection"></a>VPN 接続の確認
VNet に VM をデプロイすることなく、Azure クラシック ポータルの VNet ダッシュボード ページの仮想ネットワークのビジュアルな図を使用して、接続の状態を確認できます。
