...
* HDInsight 上の Apache Spark クラスター。 手順については、 [Azure HDInsight での Apache Spark クラスターの作成](apache-spark-jupyter-spark-sql.md)に関するページを参照してください。
...
### <a name="install-hdinsight-toolsazure-toolkit-for"></a>azure-toolkit-for の HDInsight ツール をインストールする
...
5. 既知の問題により、**[次へ]** をクリックした後でもう一度 Scala のバージョンを確認する必要があります。 Scala のバージョンが手順 4. で選択した内容に近いことを確認してください。

   ![comfirm-scala-library](./media/apache-spark-eclipse-tool-plugin/comfirm-scala-library-container.png)
6. 次のダイアログ ボックスで、**[完了]** を選択します。 
...
2. **[ジョブ]** ノードを選択します。 Java のバージョンが **1.8** より前である場合、HDInsight ツールは **E(fx)clipse** プラグインのインストールを求めるアラームを自動的に表示します。 **[OK]** を選択して続行し、ウィザードに従って Eclipse Marketplace からプラグインをインストールし、Eclipse を再起動します。 
...
![対話型のサインイン](./media/apache-spark-eclipse-tool-plugin/interactive-authentication.png)
