...
**HDInsight クラスター**: 詳しくは、「[Get started with HDInsight on Linux](hdinsight-hadoop-linux-tutorial-get-started.md)」(Linux での HDInsight の概要) をご覧ください。
...
* **認証**: 認証を行うには、クラスターの HTTP アカウント (admin) とパスワードで API を使います。 For example:
