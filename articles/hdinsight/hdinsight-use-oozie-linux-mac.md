...
* **Azure CLI**: [Azure CLI のインストール](../xplat-cli-install.md)
...
2. Sqoop アクションでは、新しい Hive テーブルの内容を Azure SQL Database のテーブルにエクスポートします。 Sqoop の詳細については、「[HDInsight での Hadoop Sqoop の使用][hdinsight-use-sqoop]」をご覧ください。
...
3. Ctrl + X キーを押した後、**Y** キーと **Enter** キーを押してファイルを保存します。
4. 次のコマンドを使用して、**workflow.xml** ファイルを **wasbs:///tutorials/useoozie/workflow.xml** にコピーします。
...
3. 次のコマンドを使用して、Oozie ジョブ定義構成を作成します。
...
4. nano エディターが開いたら、ファイルの内容として次のスクリプトを使用します。
   ...
   * **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** のすべてのインスタンスを、前に返された値に置き換えます。
     ...
     > [!WARNING]
     > パスの一部としてコンテナーとストレージ アカウントが含まれた完全な WASB パスを使用する必要があります。 短い形式 (wasbs:///) を使用すると、ジョブを開始したときに RunHiveScript アクションが失敗します。
     > 
     > 
...
1. 次のコマンドを使用して、Oozie サービスの URL を取得します。
    ...
    **http://hn0-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:11000/oozie** の部分は、Oozie コマンドで使用する URL です。
