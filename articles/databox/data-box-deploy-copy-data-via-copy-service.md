...
3. **[構成と開始]** ダイアログ ボックスで、次のように入力します。
    ...
    |ファイルの最適化              |有効にすると、取り込み時にファイルが圧縮されます。 これにより、小さなファイルのデータをより高速にコピーできるようになります。        |
...
5. 指定した設定のジョブが作成されます。 チェック ボックスをオンにしたうえで、ジョブの一時停止と再開、取り消し、再起動を実行できます。
    ...
    - ファイルの最適化を有効にした場合、コピーのパフォーマンスを高めるために、小さなファイルが取り込み時に圧縮されます。 その場合、次のスクリーンショットに示すような圧縮ファイル (GUID が名前) が表示されます。

        ![圧縮ファイルの例](media/data-box-deploy-copy-data-via-copy-service/packed-file-on-ingest.png)
