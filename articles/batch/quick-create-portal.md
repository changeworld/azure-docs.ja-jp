...
1. **[新規]**  >  **[Compute]**  >  **[Batch サービス]** の順にクリックします。 
...
3. **[ストレージ アカウント]** で、既存の汎用ストレージ アカウントを選択するか、新しいストレージ アカウントを作成します。

4. 残りの設定は既定値のままにし、**[作成]** をクリックしてアカウントを作成します。
...
1. Batch アカウントで、**[プール]** > **[追加]** の順にクリックします。
...
5. 残りの設定は既定値のままにし、**[OK]** をクリックしてプールを作成します。
...
数分後、プールの状態は **[安定]** になり、ノードが開始されます。 **[ノード]** をクリックしてノードの状態を確認します。 ノードの状態が **[アイドル]** の場合は、タスクを実行する準備が整っています。 
...
1. Batch アカウント ビューで、**[ジョブ]** > **[追加]** の順にクリックします。 

2. *myjob* という**ジョブ ID** を入力します。 **[プール]** で *mypool* を選択します。 残りの設定は既定値のままにして、**[OK]** をクリックします。
...
3. **[コマンド ライン]** に、「`cmd /c "set AZ_BATCH & timeout /t 90 > NUL"`」と入力します。 残りの設定は既定値のままにして、**[OK]** をクリックします。
...
前述のタスクの例は数分で完了します。 完了したタスクの出力を表示するには、**[ノード上のファイル]** をクリックし、`stdout.txt` ファイルを選択します。 このファイルは、タスクの標準出力を示します。 内容は次のようになります。
...
ジョブがスケジュールされていない場合でも、ノードの実行中はプールに対して料金が発生します。 プールは不要になったら、削除してください。 アカウント ビューで、**[プール]** およびプールの名前をクリックします。 その後、 **[削除]**をクリックします。  プールを削除すると、ノード上のタスク出力はすべて削除されます。 

リソース グループ、Batch アカウント、および関連するすべてのリソースは、不要になったら削除します。 これを行うには、Batch アカウントのリソース グループを選択し、**[リソース グループの削除]** をクリックしてください。
