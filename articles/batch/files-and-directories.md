...
ms.locfileid: "83790869"
---
# <a name="files-and-directories-in-azure-batch"></a>Azure Batch 内のファイルおよびディレクトリ

Azure Batch には、タスクごとに作業ディレクトリ があり、その下には 0 以上のファイルやディレクトリが作成されます。 この作業ディレクトリは、タスクによって実行されるプログラム、処理されるデータ、実行される処理の出力を格納するために使用できます。 タスクのすべてのファイルとディレクトリは、タスクのユーザーが所有者になります。

Batch サービスは、ノード上のファイル システムの一部を " *ルート ディレクトリ*" として公開します。 このルート ディレクトリには、タスクから `AZ_BATCH_NODE_ROOT_DIR` 環境変数を参照することでアクセスできます。 環境変数の使用に関する詳細については、「 [タスクの環境設定](jobs-and-tasks.md#environment-settings-for-tasks)」を参照してください。

ルート ディレクトリには、次のディレクトリ構造が含まれています。

![計算ノードのディレクトリ構造](media\files-and-directories\node-folder-structure.png)
