...
> [!IMPORTANT]
> RDP や SSH を通じてノードに接続するには、まず、ノード上にユーザーを作成する必要があります。Azure ポータルから Batch REST API を使用して[ユーザー アカウントをノードに追加][rest_create_user]し、Batch .NET の [ComputeNode.CreateComputeNodeUser][net_create_user] メソッドを呼び出すか、Batch Python モジュールの [add\_user][py_add_user] メソッドを呼び出してください。
> 
> 
