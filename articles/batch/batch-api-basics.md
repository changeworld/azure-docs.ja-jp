...
### <a name="pool-allocation-mode"></a>プール割り当てモード

Batch アカウントを作成するときに、コンピューティング ノードの[プール](#pool)の割り当て方法を指定できます。 コンピューティング ノードのプールを、Azure Batch によって管理されるサブスクリプションに割り当てるか、自分のサブスクリプションに割り当てるかを選択できます。 アカウントの "*プール割り当てモード*" プロパティによって、プールの割り当て先が決まります。 

使用するプール割り当てモードを決めるには、どちらがシナリオに適しているかを検討します。

* **Batch サービス**: Batch サービスは既定のプール割り当てモードです。このモードでは、Azure で管理されたサブスクリプションにバックグラウンドでプールが割り当てられます。 Batch サービスのプール割り当てモードについては次の点に留意してください。

    - Batch サービスのプール割り当てモードは、クラウド サービスと仮想マシン両方のプールをサポートします。
    - Batch サービスのプール割り当てモードは、共有キー認証または [Azure Active Directory 認証](batch-aad-auth.md) (Azure AD) の両方をサポートします。 
    - Batch サービスのプール割り当てモードで割り当てられるプールでは、専用のコンピューティング ノードまたは優先順位の低いコンピューティング ノードのどちらかを使用できます。
    - Azure 仮想マシン プールをカスタム VM イメージから作成する予定がある場合、または仮想ネットワークを使用する予定がある場合は、Batch サービスのプール割り当てモードを使用しないでください。 代わりに、ユーザー サブスクリプション プール割り当てモードでアカウントを作成します。
    - Batch サービスのプール割り当てモードによって作成されるアカウントでプロビジョニングされる仮想マシン プールは、[Azure Virtual Machines Marketplace][vm_marketplace] イメージから作成される必要があります。

* **ユーザー サブスクリプション**: ユーザー サブスクリプション プール割り当てモードでは、Batch プールは、アカウントが作成される Azure サブスクリプションに割り当てられます。 ユーザー サブスクリプション プール割り当てモードについては次の点に留意してください。
     
    - ユーザー サブスクリプション プール割り当てモードは、仮想マシン プールのみをサポートします。 Cloud Services プールはサポートされません。
    - カスタム VM イメージから仮想マシン プールを作成するか、仮想マシン プールを含む仮想ネットワークを使用するには、ユーザー サブスクリプション プール割り当てモードを使用する必要があります。  
    - ユーザー サブスクリプションに割り当てられているプールで [Azure Active Directory 認証](batch-aad-auth.md)を使用する必要があります。 
    - プール割り当てモードがユーザー サブスクリプションに設定されている場合、Batch アカウントに Azure キー コンテナーを設定する必要があります。 
    - ユーザー サブスクリプション プール割り当てモードを使用して作成したアカウント内のプールでは、専用のコンピューティング ノードのみを使用できます。 優先度の低いノードはサポートされていません。
    - ユーザー サブスクリプション プール割り当てモードのアカウントでプロビジョニングされる仮想マシン プールは、[Azure Virtual Machines Marketplace][vm_marketplace] イメージまたはユーザー提供のカスタム イメージから作成できます。
...
> [!IMPORTANT]
> Batch サービスのプール割り当てモードで作成された Batch アカウントには、1 つの Batch アカウントで使用できるコア数に上限を設ける既定のクォータが割り当てられています。 コア数は、コンピューティング ノードの数に対応します。 既定のクォータと、[クォータを増やす](batch-quota-limit.md#increase-a-quota)手順については、「[Azure Batch サービスのクォータと制限](batch-quota-limit.md)」を参照してください。 プールが目標のノード数に到達しない場合は、コア クォータが原因となっている可能性があります。
>
>ユーザー サブスクリプション プール割り当てモードで作成された Batch アカウントは、Batch サービス クォータを確認しません。 代わりに、指定されたサブスクリプションのコア クォータを共有します。 詳細については、「[Azure サブスクリプションとサービスの制限、クォータ、制約](../azure-subscription-service-limits.md)」の「[Virtual Machines の制限](../azure-subscription-service-limits.md#virtual-machines-limits)」をご覧ください。
>
>
...
カスタム イメージを使用して仮想マシン プールをプロビジョニングするには、ユーザー サブスクリプション プール割り当てモードで Batch アカウントを作成します。 このモードでは、Batch プールはアカウントが存在するサブスクリプションに割り当てられます。 Batch アカウントを作成する際のプール割り当てモードの設定の詳細については、「[アカウント](#account)」セクションを参照してください。

カスタム イメージを使用するには、イメージを一般化して使用できるように準備する必要があります。 Azure VM からカスタムの Linux イメージを準備する方法については、[テンプレートとして使用するための Azure Linux VM のキャプチャ](../virtual-machines/linux/capture-image-nodejs.md)に関する記事を参照してください。 Azure VM からカスタム Windows イメージを準備する方法については、「[PowerShell を使用した Azure VM のカスタム イメージの作成](../virtual-machines/windows/tutorial-custom-images.md)」を参照してください。 
...
4. **[イメージの種類]** ドロップダウンから **[カスタム イメージ (Linux/Windows)]** を選択します。 ポータルに **[カスタム イメージ]** ピッカーが表示されます。 同じコンテナーの 1 つ以上の VHD を選択し、**[選択]** ボタンをクリックします。 
    異なるストレージ アカウントと異なるコンテナーの複数の VHD のサポートは、将来的に追加される予定です。
...
- **優先順位の低いコンピューティング ノード。** 優先順位の低いノードは、Azure の余剰容量を利用して Batch ワークロードを実行します。 優先順位の低いノードは専用のノードより 1 時間あたりのコストが低く、多くのコンピューティング能力が必要なワークロードを可能にします。 詳細については、「[優先順位の低い VM で Batch を使用する](batch-low-pri-vms.md)」を参照してください。
    ...
    優先順位の低いコンピューティング ノードは、プールの割り当てモードが **[Batch Service] \(Batch サービス)** に設定された状態で作成された Batch アカウントでのみ使用できます。
...
コンピューティング ノードのプールを Azure Batch でプロビジョニングする際に、プールを Azure [仮想ネットワーク (VNet)](../virtual-network/virtual-networks-overview.md) のサブネットと関連付けることができます。 サブネットを使用した VNet の作成について詳しくは、「[サブネットを含んだ Azure 仮想ネットワークを作成する](../virtual-network/virtual-networks-create-vnet-arm-pportal.md)」を参照してください。 

 * プールに関連付けられる VNet の要件は次のとおりです。
...
* サポートされる VNet の種類は、Batch アカウントに対してプールがどのように割り当てられているかによって異なります。
    ...
    - Batch アカウントのプール割り当てモードを Batch Service に設定した場合、**Cloud Services 構成**で作成したプールにしか VNet を割り当てることができません。 さらに、指定した VNet は、クラシック デプロイ モデルで作成する必要があります。 Azure Resource Manager デプロイメント モデルで作成した VNet はサポートされていません。
 
    - Batch アカウントのプール割り当てモードをユーザー サブスクリプションに設定した場合、**仮想マシン構成**で作成したプールにしか VNet を割り当てることができません。 **クラウド サービス構成**で作成されたプールはサポートされていません。 関連付けられる VNet は、Azure Resource Manager デプロイメント モデルまたはクラシック デプロイメント モデルのいずれかで作成できます。

    プール割り当てモードに応じた VNet サポートの概要を示す表について、「[プール割り当てモード](#pool-allocation-mode)」セクションをご覧ください。

* Batch アカウントのプール割り当てモードを Batch Service に設定した場合、Batch サービス プリンシパルに対し、VNet へのアクセス許可を付与する必要があります。 VNet で、[クラシック仮想マシンの共同作業者のRBAC (ロールベースのアクセス制御)](https://azure.microsoft.com/documentation/articles/role-based-access-built-in-roles/#classic-virtual-machine-contributor) ロールを Batch サービス プリンシパルに割り当てる必要があります。 この RBAC ロールが付与されていないと、Batch サービスから 400 (無効な要求) が返されます。 Azure Portal でロールを追加するには:

    1. **VNet** を選択してから、**[アクセス制御 (IAM)]** > **[ロール]** > **[仮想マシン共同作成者]** > **[追加]** の順に選択します。
    2. **[アクセス許可の追加]** ブレードで、**[仮想マシン共同作成者]** ロールを選択します。
    3. **[アクセス許可の追加]** ブレードで、Batch API を探します。 API が見つかるまで、次の各文字列を検索します。
    ...
    3. Batch API サービス プリンシパルを選択します。 
    4. [ **Save**] をクリックします。

        ![VM 共同作成者ロールを Batch サービス プリンシパルに割り当てる](./media/batch-api-basics/iam-add-role.png)
