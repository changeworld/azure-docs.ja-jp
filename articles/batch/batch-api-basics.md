...
Batch では 2 つのアカウント構成がサポートされます。Batch アカウントを作成するときには適切な構成を選択する必要があります。 2 つのアカウント構成の違いは、アカウントの Batch [プール](#pool)の割り当て方法です。 コンピューティング ノードのプールを、Azure Batch によって管理されるサブスクリプションに割り当てるか、自分のサブスクリプションに割り当てることができます。 アカウントの "*プール割り当てモード*" プロパティによって、使用する構成が決まります。 

使用するアカウント構成を決めるには、どちらがシナリオに適しているかを検討します。

* **Batch サービス**: Batch サービスは既定のアカウント構成です。 この構成で作成されたアカウントでは、Azure で管理されたサブスクリプションにバックグラウンドで Batch プールが割り当てられます。 Batch サービス アカウント構成については次の点に留意してください。

    - Batch サービス アカウント構成は、クラウド サービスと仮想マシン両方のプールをサポートします。
    - Batch サービス アカウント構成では、共有キー認証または [Azure Active Directory 認証](batch-aad-auth.md)を使用して Batch API にアクセスできます。 
    - Batch サービス アカウント構成内のプールでは、専用のコンピューティング ノードまたは優先順位の低いコンピューティング ノードのどちらかを使用できます。
    - Azure 仮想マシン プールをカスタム VM イメージから作成する予定がある場合、または仮想ネットワークを使用する予定がある場合は、Batch サービス アカウント構成を使用しないでください。 代わりに、ユーザー サブスクリプション アカウント構成でアカウントを作成します。
    - Batch サービス サブスクリプション アカウント構成のアカウントでプロビジョニングされる仮想マシン プールは、[Azure Virtual Machines Marketplace][vm_marketplace] イメージから作成される必要があります。

* **ユーザー サブスクリプション**: ユーザー サブスクリプション アカウント構成では、Batch プールは、アカウントが作成される Azure サブスクリプションに割り当てられます。 ユーザー サブスクリプション アカウント構成については次の点に留意してください。
     
    - ユーザー サブスクリプション アカウント構成は、仮想マシン プールのみをサポートします。 Cloud Services プールはサポートされません。
    - カスタム VM イメージから仮想マシン プールを作成するか、仮想マシン プールを含む仮想ネットワークを使用するには、ユーザー サブスクリプション構成を使用する必要があります。  
    - [Azure Active Directory 認証](batch-aad-auth.md)を使用して、Batch サービスに対する要求を認証する必要があります。 
    - ユーザー サブスクリプション アカウント構成では、Batch アカウントの Azure キー コンテナーを設定する必要があります。 
    - ユーザー サブスクリプション アカウント構成を使用して作成したアカウント内のプールでは、専用のコンピューティング ノードのみを使用できます。 優先度の低いノードはサポートされていません。
    - ユーザー サブスクリプション アカウント構成のアカウントでプロビジョニングされる仮想マシン プールは、[Azure Virtual Machines Marketplace][vm_marketplace] イメージまたはユーザー提供のカスタム イメージから作成できます。
...
## コンピューティング ノード
コンピューティング ノードは、アプリケーションの一部のワークロードの処理に特化した Azure 仮想マシン (VM) またはクラウド サービス VM です。 ノードのサイズによって、CPU コアの数、メモリ容量、およびノードに割り当てられるローカル ファイル システムのサイズが決まります。 Azure Cloud Services または Virtual Machines Marketplace イメージを使用して Windows ノードまたは Linux ノードのプールを作成することができます。 これらの各オプションの詳細については、以下の「 [プール](#pool) 」セクションを参照してください。
...
> [!IMPORTANT]
> Batch サービス構成で作成された Batch アカウントには、1 つの Batch アカウントで使用できるコア数に上限を設ける既定のクォータが割り当てられています。 コア数は、コンピューティング ノードの数に対応します。 既定のクォータと、[クォータを増やす](batch-quota-limit.md#increase-a-quota)手順については、「[Azure Batch サービスのクォータと制限](batch-quota-limit.md)」を参照してください。 プールが目標のノード数に到達しない場合は、コア クォータが原因となっている可能性があります。
>
>ユーザー サブスクリプション構成で作成された Batch アカウントは、Batch サービス クォータを確認しません。 代わりに、指定されたサブスクリプションのコア クォータを共有します。 詳細については、「[Azure サブスクリプションとサービスの制限、クォータ、制約](../azure-subscription-service-limits.md)」の「[Virtual Machines の制限](../azure-subscription-service-limits.md#virtual-machines-limits)」をご覧ください。
...
仮想マシン プールでカスタム イメージを使用するには、ユーザー サブスクリプション アカウント構成で Batch アカウントを作成します。 この構成では、Batch プールはアカウントが存在するサブスクリプションに割り当てられます。 Batch アカウントを作成する際のプール割り当てモードの設定の詳細については、「[アカウント](#account)」セクションを参照してください。
...
- 現在、Standard ストレージ アカウントしかサポートされていません。 Azure Premium Storage は、将来的にサポートされる予定です。
...
>
> 1. アプリケーション パッケージを使って、Batch プール内の各ノードにアプリケーションまたはデータを分散させることができます。 アプリケーション パッケージの詳細については、「[Azure Batch アプリケーション パッケージを使用したアプリケーションのデプロイ](batch-application-packages.md)」を参照してください。
...
### Batch サービスでプロビジョニングされたプールの VNet
...
### ユーザー サブスクリプションでプロビジョニングされたプールの VNet
...
### 問題のあるコンピューティング ノードのトラブルシューティング
