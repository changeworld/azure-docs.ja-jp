...
ジョブによって、作業が実行される[プール](nodes-and-pools.md#pools)が指定されます。 ジョブごとに新しくプールを作成できるほか、多数のジョブに対して 1 つのプールを使用することもできます。 [ジョブ スケジュール](#scheduled-jobs)に関連付けられているジョブごとにプールを作成するか、ジョブ スケジュールに関連付けられているすべてのジョブに対して 1 つのプールを作成してください。
...
1 つのプールで実行されている優先順位の高いジョブは、別のプールや別の Batch アカウントで実行されているジョブのスケジューリングに影響を与えません。 ジョブの優先順位は、ジョブの送信時に作成される [autopools](nodes-and-pools.md#autopools) には適用されません。
