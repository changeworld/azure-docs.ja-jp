  ...
  <tr>
    ...
    <td><p>データ サンプルのベクターを返します。次に例を示します。</p>
        <ul>
          ...
          <p>$CPUPercent.GetSample(TimeInterval_Minute*10) は、最後の 10 分間のサンプルがすべて CPUPercent 履歴に存在する場合、200 のサンプルを返す必要があります。最後の 1 分間の履歴がまだ存在していない場合は、180 のサンプルが返されます。</p>
					<p>$CPUPercent.GetSample(TimeInterval_Minute*10, 80) は成功し、$CPUPercent.GetSample(TimeInterval_Minute*10,95) は失敗します。</p></li>
