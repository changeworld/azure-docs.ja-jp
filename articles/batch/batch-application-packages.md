...
>
> アプリケーション パッケージの作成と管理を行う API は [Batch Management .NET][api_net_mgmt] ライブラリの一部です。 コンピューティング ノードでアプリケーション パッケージをインストールするための API は [Batch .NET][api_net] ライブラリの一部です。 同等の機能は、他の言語の利用可能なバッチ API にあります。 
>
