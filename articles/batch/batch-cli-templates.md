...
Azure CLI を使用して、コードを記述せずに Batch ジョブをエンド ツー エンドで実行できます。
...
-   ジョブのタスク ファクトリにより、ジョブの複数のタスクを簡単に作成できるため、多数のタスク定義が不要になり、ジョブの送信が大幅に簡素化されます。

ジョブの入力データ ファイルを指定する必要があり、多くの場合、出力データ ファイルが生成されます。 各 Batch アカウントにストレージ アカウントが既定で関連付けられます。コーディング、ストレージ資格情報、SAS URL を必要とせずに、CLI を使用してこのストレージ アカウントとの間でファイルを簡単に転送できます。
...
-   トランスコードする一連のビデオ ファイルを持つエンド ユーザーは、まずプール テンプレートを使用し、プール ID と必要な VM の数だけを指定してプールを作成します。 これで、トランスコードするソース ファイルをアップロードできるようになります。 次に、ジョブ テンプレートを使用し、プール ID とアップロードするソース ファイルの場所だけを指定してジョブを送信できます。 生成される入力ファイルごとに 1 つのタスクを含む Batch ジョブが作成されます。 最後に、トランスコードされた出力ファイルをダウンロードできます。

## <a name="installation"></a>インストール

テンプレートとファイル転送の機能を使用するには、拡張機能をインストールする必要があります。

Azure CLI をインストールする方法については、[こちらのページ](https://docs.microsoft.com/cli/azure/install-azure-cli)をご覧ください。

Azure CLI をインストールしたら、[Azure GitHub リポジトリにある手順](https://github.com/Azure/azure-batch-cli-extensions)に従って Batch 拡張機能をインストールできます。
...
Azure Batch テンプレートは、機能と構文が Azure Resource Manager テンプレートとよく似ています。 これらのテンプレートは、アイテムのプロパティの名前と値が含まれた JSON ファイルですが、次の主要概念が追加されます。
...
-   **高度なコンストラクト**
    ...
    -   Batch API ではまだ使用できない一部の高度なコンストラクトをテンプレートで使用できます。 たとえば、共通のタスク定義を使用してジョブの複数のタスクを作成するタスク ファクトリをジョブ テンプレートで定義できます。 これらのコンストラクトにより、複数の JSON ファイル (タスクごとに 1 ファイルなど) を動的に作成するコードを記述したり、パッケージ マネージャーを使用してアプリケーションをインストールするためのスクリプト ファイルを作成したりする必要がなくなります。
...
プール、ジョブ、タスクの作成に加え、ほとんどのジョブでは入力ファイルが必要であり、出力ファイルが生成されます。入力ファイルは、クライアントからアップロードして、1 つ以上のプール ノードにコピーすることが必要な場合があります。また、プール ノードでタスクによって生成される出力ファイルは、保存してからクライアントにダウンロードする必要があります。

Azure Batch CLI 拡張機能では、ファイル転送を排除し、Batch アカウントごとに既定で作成されるストレージ アカウントを利用します。

Azure ストレージ アカウントに作成されるコンテナーに相当するファイル グループの概念が導入されました。 ファイル グループにはサブフォルダーを含めることができます。

クライアントから指定したファイル グループにファイルをアップロードすることができ、指定したファイル グループからクライアントにファイルをダウンロードする CLI コマンドが用意されています。
...
お客様からのフィードバックに応じて、今後、機能がさらに追加される可能性があります。 たとえば、Batch API、PowerShell、Portal UI でもテンプレートのサポートとファイル転送を利用できるようになる可能性があります。
-   テンプレートを定義する UI を提供できれば、JSON ファイルを作成、変更、保存、共有する必要がなくなります。

-   テンプレートに ID を割り当て、Batch サービスによって永続化し、Batch アカウントに関連付けることも考えられます。 UI、CLI、API をサポートすれば、テンプレートの作成、更新、削除が可能になります。 作成に必要なすべてのプロパティ値を指定する代わりに、プールまたはジョブの作成時にテンプレートとパラメーター値を指定することもできます。 プールとジョブを作成する UI で、パラメーター値の入力を要求する UI を動的に生成することも考えられます。

Azure CLI をお試しいただき、この記事のコメント欄または [Azure Batch MSDN フォーラム](https://social.msdn.microsoft.com/forums/azure/home?forum=azurebatch)を使用して、フィードバックや機能に関する提案をお寄せください。
