
# [Batch のドキュメント](index.yml)

# 概要
## [Azure Batch とは](batch-technical-overview.md)

# クイック スタート
## [Batch ジョブの実行 - CLI](quick-create-cli.md)
## [Batch ジョブの実行 - ポータル](quick-create-portal.md)
## [Batch ジョブの実行 - .NET](quick-run-dotnet.md)
## [Batch ジョブの実行 - Python](quick-run-python.md)


# チュートリアル

## [ファイルの並列処理 - .NET](tutorial-parallel-dotnet.md)
## [ファイルの並列処理 - Python](tutorial-parallel-python.md)
## [Arnold を使用したシーンのレンダリング](tutorial-rendering-cli.md)
## [並列 R シミュレーション](tutorial-r-doazureparallel.md)

# サンプル
## コード サンプル
### [Azure のコード サンプル](https://azure.microsoft.com/resources/samples/?service=batch)
### [Batch のサンプル リポジトリ](https://github.com/Azure/azure-batch-samples)
## [Azure CLI](cli-samples.md)

# 概念
## [開発者向け機能](batch-api-basics.md)
## [API とツール](batch-apis-tools.md)
## [クォータと制限](batch-quota-limit.md)

# ハウツー ガイド
## Batch アカウントの管理
### [ポータルで Batch アカウントを管理する](batch-account-create-portal.md)
### [Batch Management .NET で Batch アカウントを管理する](batch-management-dotnet.md)

## Azure AD による認証
### [Batch サービスでの Azure AD](batch-aad-auth.md)
### [Batch Management での Azure AD](batch-aad-auth-management.md)

## [アプリケーション パッケージの作成](batch-application-packages.md)

## プールの作成と管理
### [自動スケール コンピューティング ノード](batch-automatic-scaling.md)
### [コンピューティング集中型 VM](batch-pool-compute-intensive-sizes.md)
### [コンピューティング ノードへのアクセスの構成](pool-endpoint-configuration.md)
### [カスタム イメージを使用してプールを作成する](batch-custom-images.md)
### [仮想ネットワーク内にプールを作成する](batch-virtual-network.md)
### [Linux コンピューティング ノード](batch-linux-nodes.md)
### [優先順位の低い VM](batch-low-pri-vms.md)

## ジョブとタスクの管理
### [ジョブの準備と完了タスク](batch-job-prep-release.md)
### [同時実行ノード タスク](batch-parallel-node-tasks.md)
### [状態別にタスクをカウントする](batch-get-task-counts.md)
### [Task dependencies](batch-task-dependencies.md)
### [タスクを実行するためのユーザー アカウント](batch-user-accounts.md)
### [コードを使用せずに Batch ジョブを実行する](batch-cli-templates.md)


## [ジョブとタスク出力を保持する](batch-task-output.md)
### [Batch API で出力を保持する](batch-task-output-files.md)
### [ファイル規則ライブラリで出力を保持する](batch-task-output-file-conventions.md)

## ワークロードの実行
### [MPI ジョブ](batch-mpi.md)
### [コンテナー ワークロード](batch-docker-container-workloads.md)
### [レンダリング](batch-rendering-service.md)
### [Batch と Data Factory を使用したデータ処理](../data-factory/v1/data-factory-data-processing-using-batch.md?toc=%2fazure%2fbatch%2ftoc.json)

## リソースの監視
### [診断ログ](batch-diagnostics.md)
### [効率的なリスト クエリ](batch-efficient-list-queries.md)

## 開発者ツールの使用
### [Batch .NET の使用](batch-dotnet-get-started.md)
### [Batch Python の使用](batch-python-tutorial.md)
### [Batch Node.js の使用](batch-nodejs-get-started.md)
### [PowerShell コマンドレットの使用](batch-powershell-cmdlets-get-started.md)
### [Azure CLI の使用](batch-cli-get-started.md)

# リファレンス

## [Azure CLI](/cli/azure/batch)
## [Azure PowerShell](/powershell/module/azurerm.batch)
## [.NET](/dotnet/api/overview/azure/batch)
## [Java](/java/api/overview/azure/batch)
## [Node.js](/javascript/api/overview/azure/batch)
## [Python](/python/api/overview/azure/batch)
## REST ()
### [Batch サービス](/rest/api/batchservice)
### [Batch Management](/rest/api/batchmanagement/)
## [一括分析](batch-analytics.md)
### [プール作成イベント](batch-pool-create-event.md)
### [プール削除の開始イベント](batch-pool-delete-start-event.md)
### [プール削除の完了イベント](batch-pool-delete-complete-event.md)
### [プールのサイズ変更の開始イベント](batch-pool-resize-start-event.md)
### [プールのサイズ変更の完了イベント](batch-pool-resize-complete-event.md)
### [タスク開始イベント](batch-task-start-event.md)
### [タスク完了イベント](batch-task-complete-event.md)
### [タスク失敗イベント](batch-task-fail-event.md)
## [Batch コンピューティング ノードの環境変数](batch-compute-node-environment-variables.md)

# リソース
## [Azure のロードマップ](https://azure.microsoft.com/roadmap/)
## [Batch フォーラム](https://social.msdn.microsoft.com/Forums/en-us/home?forum=azurebatch)
## [Batch Shipyard](https://github.com/Azure/batch-shipyard)
## [ブログ](https://blogs.technet.microsoft.com/windowshpc/)
## [Azure の HPC ソリューション](../virtual-machines/linux/high-performance-computing.md?toc=%2fazure%2fbatch%2ftoc.json)
## [料金](https://azure.microsoft.com/pricing/details/batch/)
## [料金計算ツール](https://azure.microsoft.com/pricing/calculator/)
## [Resource Manager テンプレート](/azure/templates/microsoft.batch/batchaccounts)
## [サービスの更新情報](https://azure.microsoft.com/updates/?product=batch&updatetype=&platform=)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-batch)
## [ビデオ](https://azure.microsoft.com/documentation/videos/index/?services=batch)


