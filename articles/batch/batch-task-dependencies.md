...
この記事では、[Batch .NET][net_msdn] ライブラリを使用したタスクの依存関係の構成について説明します。まずジョブに対して[タスクの依存関係を有効](#enable-task-dependencies)にする方法を説明した後、[依存関係を伴うタスクの構成](#create-dependent-tasks)方法を簡単に紹介します。最後に、Batch でサポートされる[依存関係のシナリオ](#dependency-scenarios)について取り上げます。
