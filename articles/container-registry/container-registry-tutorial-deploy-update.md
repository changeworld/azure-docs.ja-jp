...
このチュートリアルでは、Web アプリケーション コンテナーの新しいバージョンを更新し、それを Geo レプリケートされたレジストリにプッシュしました。 Azure Container Registry の Webhook が App Services に更新を通知し、それがレプリケートされたレジストリからのローカル プルをトリガーしました。
