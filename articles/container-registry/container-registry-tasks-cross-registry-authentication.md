---
title: Azure Container Registry タスクでのクロスレジストリ認証
description: Azure Container Registry (ACR) タスクで Azure リソースのマネージド ID を有効にして、そのタスクで別のプライベート コンテナー レジストリにアクセスできるようにします。
