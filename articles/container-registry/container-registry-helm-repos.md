...
- グラフを検索してインストールする **Helm クライアント**。 また、Kubernetes クラスター内で開始される Helm サーバー (Tiller) も必要です。 必要に応じて、[Azure Kubernetes Service クラスターを作成][aks-quickstart]できます。 Helm のインストールと使用方法について詳しくは、[Helm のインストール][helm-install]をご覧ください。
