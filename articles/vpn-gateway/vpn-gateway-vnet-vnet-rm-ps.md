...
4. TestVNet1 のサブネット構成を作成する
	...
	次の例では、先ほど設定した変数を使用します。この例では、ゲートウェイ サブネットに /27 が使用されています。/29 のような小さなサブネットでゲートウェイ サブネットを作成することもできますが、これはお勧めできません。/27 または /26 などの比較的大きいサブネットを使用することをお勧めします。そうすることで、より大きなゲートウェイ サブネットが必要になった場合に、既存の構成または今後の構成を活用できます。
