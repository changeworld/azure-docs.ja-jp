...
1. **共有キー -** 画面下部の **[キーの管理]** をクリックします。次に、キーの隣のアイコンをクリックしてクリップボードにコピーし、キーを貼り付けてから保存します。このボタンは、単一の S2S VPN トンネルに対してのみ有効です。複数の S2S VPN トンネルが存在する場合は、*Get Virtual Network Gateway Shared Key* という API または PowerShell コマンドレットを使用します。
