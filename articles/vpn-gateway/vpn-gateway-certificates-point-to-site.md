...
4. 自己署名証明書はクライアント証明書を作成するために使用します。P2S 構成の一部として自己署名証明書の .cer ファイルをアップロードすることは、Azure に対して、クライアント コンピューターが使用している証明書を信頼するよう指示することを意味します。<br><br>適切な VPN クライアント設定で構成され、クライアント証明書がインストールされているコンピューターはすべて、P2S を介して仮想ネットワークに接続できます。そのため、クライアント証明書は必要な場合にのみ生成およびインストールし、自己署名証明書はバックアップをとって安全に保管するようにしてください。必要に応じて、後からこの自己署名証明書を別のコンピューターにインストールして、さらに多くのクライアント証明書を生成したり、.cer ファイルをエクスポートしたりできます。
