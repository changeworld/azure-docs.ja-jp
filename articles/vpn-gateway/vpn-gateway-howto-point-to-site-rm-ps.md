...
## <a name="<a-name="declare"></a>part-1---log-in-and-set-variables"></a><a name="declare"></a>パート 1 - ログインと変数の設定
...
## <a name="part-2---configure-a-vnet"></a>パート 2 - VNet の構成
1. リソース グループを作成します。
...
## <a name="part-3---add-trusted-certificates"></a>パート 3 - 信頼された証明書の追加
...
最大 20 個の信頼された証明書を Azure に追加できます。 公開キーを取得するには、Base64 でエンコードされた X.509 (.cer) ファイルとして証明書をエクスポートします。 .cer をエクスポートする 1 つの方法として、 **certmger.msc** を開き、[個人] の [証明書] にある証明書を見つけます。 この証明書を右クリックし、秘密キーなしで "Base-64 encoded X.509(.CER)" としてエクスポートします。 .cer ファイルのエクスポート先のファイル パスをメモします。 次に示すのは、証明書の Base64 文字列表現を取得するコードのサンプルです。 
...
## <a name="part-4---create-the-vpn-gateway"></a>パート 4 - VPN ゲートウェイの作成
...
## <a name="part-5---download-the-vpn-client-configuration-package"></a>パート 5 - VPN クライアント構成パッケージのダウンロード
...
## <a name="<a-name="cc"></a>part-6---generate-the-client-certificate"></a><a name="cc"></a>パート 6: クライアント証明書の生成
...
## <a name="part-7---install-the-client-certificate"></a>パート 7 - クライアント証明書のインストール
...
## <a name="part-8---connect-to-azure"></a>パート 8 - Azure への接続
...
## <a name="part-9---verify-your-connection"></a>パート 9 - 接続の確認
...
## <a name="to-add-or-remove-a-trusted-root-certificate"></a>信頼されたルート証明書を追加または削除するには
...
## <a name="to-manage-the-list-of-revoked-client-certificates"></a>失効したクライアント証明書のリストを管理するには
