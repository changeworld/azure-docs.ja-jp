...
使用料金は、サブスクリプションに対する合計**月額**料金にクレジットまたは割引を適用した金額になります。前月の利用量の未払い分が課金されます。ファイルの最上部セクションには、前月の請求サイクル中に課金されたサービスに関する詳細が表示されます。次の表は、各 .csv のバージョン ファイルの列名です。
...
サービス情報 2 | サービス情報 2 | これは省略可能なサービス固有のメタデータをキャプチャする従来のフィールドです。

いくつかの新しいフィールドと、csv Version 2 への名前の変更に加え、以下のフィールドのデータには、標準化された書式設定があります。
...
各サービスの詳細については、これら関連情報を参照してください。これらのファイルは毎月更新されます。翌月分は 25 日に発行されます。.csv Version 1 については **2015 年 5 月**以降、.csv Version 2 については **2015 年 6 月**以降から利用できます。サービス ダウンロード フィールドの基本ファイルの URL を以下に示します。
