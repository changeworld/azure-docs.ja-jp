<properties 
	pageTitle="テーブル ストレージを使用する方法 (Python) | Microsoft Azure" 
	description="Python からテーブル サービスを使用して、テーブルを作成および削除する方法、テーブルのエンティティを挿入、削除、照会する方法について説明します。" 
	services="storage" 
...
**Entity** クラスのインスタンスを insert\_entity**insert\_entity** メソッドに渡すこともできます。
 ...
 *プロジェクション*と呼ばれるこの方法では、帯域幅の使用が削減され、クエリのパフォーマンスが向上します。特に、大量のエンティティがある場合に役立ちます。**select** パラメーターを使用して、クライアントに提供するプロパティの名前を渡します。
