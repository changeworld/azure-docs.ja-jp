...
| [サブスクリプション] | 自分のサブスクリプションからデータベースを選択するか、別のサブスクリプションのデータベースへの接続情報を指定することができます。 |
...
BLOB ストレージを使用すると、大量の非構造化データをクラウドに保存する場合に、コスト効率の高いスケーラブルなソリューションを実現できます。Azure BLOB ストレージとその使用法の説明については、[BLOB ストレージの使用方法](./articles/storage-dotnet-how-to-use-blobs.md)に関するページを参照してください。
...
<tr>
<td>[サブスクリプション]</td>
<td>自分のサブスクリプションから BLOB ストレージ アカウントを選択するか、別のサブスクリプションの BLOB ストレージ アカウントへの接続情報を指定することができます。</td>
</tr>
<tr>
<td>ストレージ アカウント</td>
...
<tr>
<td>区切り記号</td>
<td>Stream Analytics は、CSV データをシリアル化するために、一般的な区切り記号をサポートしています。サポートしている値は、コンマ、セミコロン、スペース、タブ、および縦棒です。</td>
</tr>
<tr>
<td>形式</td>
...
| [サブスクリプション] | 自分のサブスクリプションからイベント ハブを選択するか、別のサブスクリプションのイベント ハブへの接続情報を指定することができます。 |
...
| 区切り記号 | Stream Analytics は、CSV 形式のデータをシリアル化するために、一般的な区切り記号をサポートしています。サポートしている値は、コンマ、セミコロン、スペース、タブ、および縦棒です。 |
...
1.	Power BI を出力として選択すると、既存の Power BI ユーザーを承認するか、新しい Power BI アカウントを作成するように求められます。  
...
[Azure テーブル ストレージ](./articles/storage-introduction.md)は高度な可用性を備えた大幅にスケーラブルなストレージであるため、アプリケーションを需要に応じて自動的に拡張できます。テーブル ストレージは Microsoft の NoSQL キー/属性ストアであり、スキーマに対する制約を抑えながら、構造化されたデータに活用できます。Azure テーブル ストレージを使用すると、永続化と効率的な取得のためにデータを保持できます。
...
| [サブスクリプション] | 自分のサブスクリプションからテーブル ストレージ アカウントを選択するか、ストレージ アカウントが別のサブスクリプションのものである場合はその接続情報を指定することができます。 |
...
| バッチ サイズ | バッチ操作のレコードの数です。通常、大部分のジョブは既定値で十分です。この設定の変更の詳細については、[TableBatchOperation クラス](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.table.tablebatchoperation.aspx)に関するページを参照してください。 |
...
| [サブスクリプション] | 自分のサブスクリプションから Service Bus 名前空間を選択するか、名前空間が別のサブスクリプションのものである場合はその接続情報を指定することができます。 |
...
| [サブスクリプション] | 自分のサブスクリプションから Service Bus 名前空間を選択するか、名前空間が別のサブスクリプションのものである場合はその接続文字列情報を指定することができます。 |
| Service Bus 名前空間 | Service Bus 名前空間は、一連のメッセージング エンティティのコンテナーです。新しいイベント ハブを作成するときは、Service Bus 名前空間も作成します。 |
