<properties
	pageTitle="Stream Analytics の出力: ストレージのオプション、分析 | Microsoft Azure"
...
# Stream Analytics の出力: ストレージのオプション、分析
...
> [AZURE.NOTE] 現在、Azure SQL Database は Stream Analytics の出力ジョブでサポートされています。ただし、データベースに接続された SQL Server を実行する Azure 仮想マシンはサポートされていません。これは、今後のリリースで変更されることがあります。
...
| バッチ サイズ | バッチ操作のレコードの数です。通常、大部分のジョブは既定値で十分です。この設定の変更の詳細については、「[TableBatchOperation Class (TableBatchOperation クラス)](https://msdn.microsoft.com/library/microsoft.windowsazure.storage.table.tablebatchoperation.aspx)」を参照してください。 |
