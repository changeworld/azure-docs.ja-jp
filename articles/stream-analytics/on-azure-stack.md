...
1. **[構成]** で、 **[ストレージ アカウントの設定]** を選択し、前の手順で作成したストレージ アカウントを選択します。
   ...
   > [ ![ジョブ ストレージ アカウントの設定を示すスクリーンショット。](media/on-azure-stack/storage-account-settings.png) ](media/on-azure-stack/storage-account-settings.png#lightbox)
...
1. 入力が Azure Stack Hub サブスクリプションのイベント ハブまたは IoT ハブである場合は、次に示すように情報を手動で入力します。
   ...
   | Service Bus 名前空間 | 名前空間は、一連のメッセージング エンティティのコンテナーです。 新しいイベント ハブを作成するときは、名前空間も作成します。 たとえば、*sb://<Event Hub Name>.eventhub.shanghai.azurestack.corp.microsoft.com* とします。 |
   ...
   > [!div class="mx-imgBorder"]
   > [ ![イベント ハブの入力を示すスクリーンショット。](media/on-azure-stack/event-hub-input.png) ](media/on-azure-stack/event-hub-input.png#lightbox)
   ...
   | --- | --- |
   | 入力のエイリアス | この入力を参照するジョブのクエリで使用するわかりやすい名前。 |
   | IoT Hub | 入力として使用する IoT Hub の名前。 たとえば、 *<IoT Hub Name>.shanghai.azurestack.corp.microsoft.com* とします。 |
   ...
   > [!div class="mx-imgBorder"]
   > [ ![IoT Hub の入力を示すスクリーンショット。](media/on-azure-stack/iot-hub-input.png) ](media/on-azure-stack/iot-hub-input.png#lightbox)
...
1. 出力が Azure Stack Hub サブスクリプションのイベント ハブまたは BLOB ストレージである場合は、次に示すように情報を手動で入力します。
   ...
   | Service Bus 名前空間 | 一連のメッセージング エンティティのコンテナーです。 新しいイベント ハブを作成したとき、Service Bus 名前空間も作成されました。 たとえば、*sb://<Event Hub Name>.eventhub.shanghai.azurestack.corp.microsoft.com* とします。 |
   ...
   > [!div class="mx-imgBorder"]
   > [ ![イベント ハブの出力を示すスクリーンショット。](media/on-azure-stack/event-hub-output.png) ](media/on-azure-stack/event-hub-output.png#lightbox)
   ...
   | ストレージ アカウント | ご自分の出力を送信するストレージ アカウントの名前です。 たとえば、 *<Storage Account Name>.blob.shanghai.azurestack.corp.microsoft.com* とします。 |
...
1. サブスクリプションと、作成した Stream Analytics Edge ジョブを選択します。 **[保存]** を選択して、 **[次: ルート]** を選択します。
   ...
   > [!div class="mx-imgBorder"]
   > [ ![モジュールの追加を示すスクリーンショット。](media/on-azure-stack/edge-modules.png) ](media/on-azure-stack/edge-modules.png#lightbox)

1. **[確認および作成 >]** を選択します。
1. **[確認および作成]** ステップで、 **[作成]** を選択します。 
   > [!div class="mx-imgBorder"]
   > [ ![マニフェストを示すスクリーンショット。](media/on-azure-stack/module-content.png) ](media/on-azure-stack/module-content.png#lightbox)
1. モジュールが一覧に追加されていることを確認します。
   > [!div class="mx-imgBorder"]
   > [ ![[デプロイ] ページを示すスクリーンショット。](media/on-azure-stack/edge-deployment.png) ](media/on-azure-stack/edge-deployment.png#lightbox)
