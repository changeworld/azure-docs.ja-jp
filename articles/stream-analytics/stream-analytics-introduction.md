...
Azure Stream Analytics は、大量のストリーミング データを検証するように設計されたイベント処理エンジンです。 パターンやリレーションシップは、デバイス、センサー、Web サイト、ソーシャル メディア フィード、アプリケーションなどのいくつかの入力ソースから抽出された情報で識別できます。 これらのパターンを使用すると、アラートを作成したり、情報をレポート ツールにフィードしたり、変換されたデータを後で使用するために格納したりするなどの、ダウンストリームの他のアクションをトリガーできます。

次のシナリオは、Azure Stream Analytics を使用できる場合の例です。

* モノのインターネット (IoT) センサー フュージョンおよびデバイス テレメトリに対するリアルタイム分析
* Web ログ/クリックストリーム分析
...
Azure Stream Analytics ジョブは、入力、変換クエリ、および出力で構成されます。 ソフトウェアまたはデバイスからのイベントが Azure Event Hubs、Azure IoT Hub、または Azure Blob Storage によって取り込まれたら、これらのサービスのうちの 1 つ以上をジョブへの入力ソースとして指定できます。 SQL クエリ言語に基づく変換クエリは、ストリーミング データの一定期間にわたるフィルター処理、並べ替え、集計、および結合を容易に行うために使用されます。 集計操作を実行するときは、イベントの順序付けのオプションや時間枠の期間を調整できます。
...
* リアルタイムの視覚化のためにデータを Power BI ダッシュボードに送信する。
...
Stream Analytics はマネージド サービスとして、分レベルで 99.9% の可用性のイベント処理が証されます。 詳細については、「[Stream Analytics の SLA](https://azure.microsoft.com/support/legal/sla/stream-analytics/v1_0/)」のページを参照してください。 
...
Stream Analytics は、1 秒間に何百万というイベントを処理できるため、短い待ち時間で結果を配信することができます。 大規模でリアルタイムの複雑なイベント処理アプリケーションに対応するために、スケールアップまたはスケールアウトを行うことができます。 Stream Analytics は、パーティション分割によってパフォーマンスをサポートしているため、複雑なクエリを並行化し、複数のストリーミング ノードで実行できます。 Azure Stream Analytics の基盤となっている [Trill](https://github.com/Microsoft/Trill) は、Microsoft Research と共同で開発された、高パフォーマンスのメモリ内ストリーミング分析エンジンです。
