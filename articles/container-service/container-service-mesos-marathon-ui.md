...
Secure Shell (SSH) トンネルを確立したら、http://localhost/ に移動します。DC/OS Web UI が読み込まれ、使用リソース、アクティブなエージェント、実行中のサービスなど、クラスターに関する情報を確認できます。
...
Marathon を使用して新しいコンテナーをデプロイするには、**[Create Application (アプリケーションの作成)]** ボタンをクリックし、次の情報をフォームに入力します。準備ができたら、**[Create Application (アプリケーションの作成)]** をクリックします。
...
DC/OS アプリ (http://localhost/) に戻ると、タスク (この場合は Docker 形式のコンテナー) が DC/OS クラスターで実行されていることがわかります。
...
Marathon UI では、コンテナーのインスタンス数を増減することができます。この操作を実行するには、Marathon ページに移動し、スケールするコンテナーを選択して、**[Scale (スケール)]** ボタンをクリックします。**[Scale Application (アプリケーションのスケール)]** ダイアログ ボックスで、任意のコンテナー インスタンス数を入力し、**[Scale Application (アプリケーションのスケール)]** を選択します。
