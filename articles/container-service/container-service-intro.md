...
Azure Container Service の使用を開始するには、Azure リソース マネージャーのテンプレートを使用して ACS クラスターをデプロイします。このデプロイは、異なるサイズと可用性のオプションで構成でき、Apache Mesos または Docker Swarm を使用して構成します。Azure リソース マネージャーのテンプレートは、Azure ポータル、Azure CLI、または PowerShell を使用してデプロイできます。また、テンプレートを変更して、Azure の追加または高度な構成を含めることもできます。デプロイと ACS クラスターの詳細については、「[Azure コンテナー サービス クラスターのデプロイ](./container-service-deployment.md)」を参照してください。
