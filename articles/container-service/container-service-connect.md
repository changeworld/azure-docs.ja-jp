...
Azure コンテナー サービスによってデプロイされている Mesos クラスターと Swarm クラスターは、REST エンドポイントを公開しています。ただし、これらのエンドポイントは外部には開放されていません。これらのエンドポイントを扱うためには、Secure Shell (SSH) トンネルを作成する必要があります。SSH トンネルが確立されると、クラスター エンドポイントに対してコマンドを実行し、独自のシステムでブラウザー経由でクラスター UI を表示できます。このドキュメントでは、Linux、OS X、Windows から SSH トンネルを作成する手順を紹介します。
...
**PORT** は、公開するエンドポイントのポートです。Swarm の場合は 2375 です。Mesos の場合は、ポート 80 を使用してください。**USERNAME** は、クラスターのデプロイ時に指定したユーザー名です。**DNSPREFIX** は、クラスターのデプロイ時に指定した DNS 接頭辞です。**REGION** は、リソース グループが置かれているリージョンです。
...
同様に、各アプリケーションの REST API にはトンネル (Marathon - `http://localhost/marathon/v2`) を経由して到達できます。公開されている各種 API の詳細については、Mesosphere の [Marathon API](https://mesosphere.github.io/marathon/docs/rest-api.html) 関連ドキュメントを参照してください。[Mesos Scheduler API](http://mesos.apache.org/documentation/latest/scheduler-http-api/) については、[Chronos API](https://mesos.github.io/chronos/docs/api.html) と Apache のドキュメントを参照してください。
...
- **[Source Port]** (ソース ポート) には、使用する優先設定を指定します (Mesos の場合は 80、Swarm の場合は 2375)。
- **[Destination]** (ターゲット) には、localhost:80 (Mesos) または localhost:2375 (Swarm) を指定します。
...
Mesos のトンネルを構成したら、関連するエンドポイントに次のアドレスでアクセスできます。
