...
**[Docker クライアント]** は、開発者が、コンテナーで実行されるアプリケーションのデプロイ、スケーリング、および管理を自動化することを支援するオープン ソース ソリューションです。
...
1. 省略可能な手順: 埋め込みの Tomcat サーバーを使用して、既定のポート 8080 ではなくポート 80 で実行する場合 (Spring Boot プロジェクトをローカルでテストする場合など)、次の手順を使用してポートを構成することができます。
...
1. [Azure Portal] を参照して、ログインします。
   ...
   Azure Portal のアカウントにログインしたら、「[Azure Portal を使用したプライベート Docker コンテナー レジストリの作成]」の記事の手順に従います。便宜上、この手順を改めて以下で説明します。
...
1. **[コンテナー レジストリの作成]** ブレードが表示されたら、**[レジストリ名]** と **[リソース グループ]** を入力して、**[有効化]** を選択し、**[管理ユーザー]** に対して **[作成]** をクリックします。
...
>
> この場合、次のようにして Docker コマンド ラインから Azure にログインする必要があります。
>
...
1. **[+ 新規]** のメニュー アイコン、**[Web + Mobile]**、**[Web App on Linux]** の順にクリックします。
   ...
   ![Azure Portal で Web アプリを新しく作成する][LX01]

1. **[Web App on Linux]** ブレードが表示されたら、次の情報を入力します。
...
> [!NOTE]
>
> Azure は、80 または 8080 の標準のポートで実行されている埋め込みの Tomcat サーバーにインターネットの要求を自動的にマップします。 ただし、埋め込みの Tomcat サーバーをカスタム ポートで実行するように構成している場合は、埋め込みの Tomcat サーバーのポートを定義する Web アプリに環境変数を追加する必要があります。 そのためには、次の手順を実行してください。
>
> 1. [Azure Portal] を参照して、ログインします。
> 
...
[Docker クライアント]: https://www.docker.com/
