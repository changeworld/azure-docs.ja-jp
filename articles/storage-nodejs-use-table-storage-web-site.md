...
> [WACOM.NOTE] 以下に示す手順の多くでは、コマンド ラインを使用します。これらの手順では、**cmd.exe** (Windows) や **Bash** (Unix Shell) など、お使いのオペレーティング システムのコマンド ラインを使用してください。OS X システムでは、ターミナル アプリケーションを使用してコマンド ラインにアクセスできます。
...
2.  次のコマンドを入力して、[azure][Azure]、[node-uuid]、[nconf]、および [async] モジュールをローカルにインストールし、これらのモジュールのエントリを **package.json** ファイルに保存します。
...
3.  app.js ファイル内で、次の行が表示されるまで下へスクロールします。
    ...
    これらの行を下のコードに置き換えます。これにより、ストレージ アカウントへの接続を使って、**Task** のインスタンスが初期化されます。これは **TaskList** に渡され、TaskList ではこれを使用してテーブル サービスを操作します。
...
3.  Web ブラウザーを開き、http://127.0.0.1:3000 に移動します。これにより次のような Web ページが表示されます。
...
2.  新しい Azure の Web サイトを作成するには、次のコマンドを使用します。
    ...
    > [WACOM.NOTE] これがサブスクリプションで最初の Azure の Web サイトである場合、ポータルを使用して Web サイトを作成するように指示するメッセージが表示されます。詳細については、「[Node.js アプリケーションの作成と Azure の Web サイトへの展開]」を参照してください。
...
3.  **[アプリ設定]** で、**[キー]** フィールドに「**STORAGE\_NAME**」と入力し、**[値]** フィールドに自分のストレージ アカウントの名前を入力します。チェックマークをクリックして、次のフィールドに移動します。次のキーと値について、この手順を繰り返します。
    ...
    -   **STORAGE\_KEY** - ストレージ アカウントのアクセス キー
...
変更内容を Azure に展開した後も、Web アプリケーションは **[アプリ設定]** のエントリから接続文字列を読み取って動作し続けます。これを確認するために、**[アプリ設定]** の **STORAGE\_KEY** のエントリを無効な値に変更します。この値を保存すると、無効なストレージ アクセス キー設定のために Web サイトは動作しなくなります。
