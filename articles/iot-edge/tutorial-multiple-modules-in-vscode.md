...
Visual Studio Code を使用して、複数のモジュールを含むご自身の IoT Edge ソリューションを開発できます。 このチュートリアルでは、Visual Studio Code でシミュレートされた IoT Edge デバイスで、センサー データをパイプ処理するだけの IoT Edge ソリューションを作成、更新、および展開する手順について説明します。 このチュートリアルで学習する内容は次のとおりです。
...
このチュートリアルでは、C# モジュールで IoT Edge ソリューションを作成してから、関数モジュールを追加し、そのソリューションのルートを更新して、シミュレートされた IoT Edge デバイスにビルドおよび展開しました。 次のチュートリアルに進み、VS Code で Azure IoT Edge を開発するときの他のシナリオを確認できます。

> [!div class="nextstepaction"]
> [VS Code で C# モジュールをデバッグする](how-to-vscode-debug-csharp-module.md)
> [VS Code で C# 関数をデバッグする](how-to-vscode-debug-azure-function.md)