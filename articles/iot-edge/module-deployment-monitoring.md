...
1. OS でのデバイスのイメージ化と [IoT Edge ランタイム](iot-edge-runtime.md)のインストールによる、新たな IoT Edge デバイスのプロビジョニングを行います。
2. [IoT Edge モジュール](iot-edge-modules.md)の起動に必要なデバイスの構成、およびその正常性を監視します。 
3. 最後に、デバイスが置き換えられたり古くなったりした場合に、デバイスを回収します。  
...
* **異常**: 1 つ以上のモジュールが正常にデプロイされなかったことが、IoT Edge デバイスからサービスに報告されたことを示します。 エラーについて詳しく調べるには、それらのデバイスにリモートで接続し、ログ ファイルを参照します。
