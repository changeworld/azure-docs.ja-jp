...
> [!NOTE]
> 最初、cameraCapture モジュールからの出力にはいくつかの接続エラーが表示されることがあります。 これは、モジュールがデプロイされてから開始されるまでの遅延が原因です。
>
> cameraCapture モジュールにより、成功するまで接続が再試行されます。 その後、以下でに説明する、予想されるイメージ分類メッセージが表示され始めるはずです。
