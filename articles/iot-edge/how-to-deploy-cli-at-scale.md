...
```
>[!NOTE]
> この多層デプロイ マニフェストの形式は、標準的なデプロイ マニフェストとは若干異なることに注意してください。 ランタイム モジュールの desired プロパティは、ドット表記を使用して階層化されます。 Azure portal が多層デプロイを認識するためには、この書式が必要となります。 次に例を示します。
>
...
>[!NOTE]
>現在、どの多層デプロイも、有効と見なされるためには、edgeAgent オブジェクトが含まれている必要があります。 多層デプロイで更新されるのがモジュールのプロパティだけだとしても、空のオブジェクトを追加してください。 たとえば、「 `"$edgeAgent":{}` 」のように入力します。 空の edgeAgent オブジェクトを含んだ多層デプロイは、edgeAgent モジュール ツインでは、**applied** ではなく **targeted** として表示されます。

まとめると、多層デプロイを作成するには、次のようにします。

- Azure CLI の create コマンドに `--layered` フラグを追加する必要があります
- システム モジュールを含めることはできません
- `$edgeAgent` と `$edgeHub` では、完全な "ドット表記 " を使用する必要があります
