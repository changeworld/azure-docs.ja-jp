---
title: デプロイのためのロジック アプリ テンプレートの作成 - Azure Logic Apps
...
> [!IMPORTANT]
> テンプレート内の接続で使用する Azure のリソース グループと場所は、ロジック アプリと同じものである必要があります。
...
LogicAppTemplate モジュールをインストールした後にテンプレートを生成するには、次の PowerShell コマンドを実行します。
