...
- [Logic Apps の Azure Policy 定義の一覧](/azure/logic-apps/policy-samples)
...
さらに、環境の種類やデータの機密度レベルなど、個々のセキュリティ ドメイン用の個別のサブスクリプションと管理グループを使用して分離を実装します。 アプリケーションやエンタープライズ環境で必要とされる Azure リソースへのアクセス レベルを制限できます。 Azure Active Directory のロールベースのアクセス制御を使用して、Azure リソースへのアクセスを制御できます。
