...
| isMessageFailed | ブール | X12 メッセージが失敗したかどうか。 (必須) |
| isTechnicalAcknowledgmentExpected | ブール | X12 契約で技術確認が構成されているかどうか。 (必須) |
| isFunctionalAcknowledgmentExpected | ブール | X12 契約で機能確認が構成されているかどうか。 (必須) |
| needAk2LoopForValidMessages | ブール | 有効なメッセージのために AK2 ループが必要かどうか。 (必須) |
...
| StatusCode | ブール | トランザクション セット受信確認の状態コード。 (必須) |
