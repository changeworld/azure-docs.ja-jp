...
1. 電子メール アクションのプロパティを設定します。 件名に **Limit** 変数を追加します。 こうすることで、次のように、変数の現在の値が指定の条件と一致していることを確認できます。
      ...
      | プロパティ | 値 | 説明 |
      | -------- | ----- | ----------- | 
      | **ターゲット** | *<email-address\@domain>* | 受信者の電子メール アドレス。 テストのために、自分の電子メール アドレスを使用します。 | 
      | **サブジェクト** | "Limit" の現在の値は **Limit** | 電子メールの件名を指定します。 この例では、**Limit** 変数が含まれていることを確認してください。 | 
