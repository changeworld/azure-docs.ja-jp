...
ロジック アプリからのすべての呼び出しは、リージョンごとの特定の IP アドレス セットから送信されます。  これらの専用 IP アドレスからの要求のみを受け入れるように、フィルターを追加できます。  これら IP アドレスの一覧については、[こちらの記事](logic-apps-limits-and-config.md#configuration)を参照してください。
