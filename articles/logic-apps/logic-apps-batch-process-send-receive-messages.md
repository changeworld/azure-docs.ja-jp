...
3. バッチの名前およびバッチをリリースするための条件を次のように指定します。
   ...
   * **[スケジュール]**: 処理のバッチ リリース スケジュール (この例では "5 分ごと")。

     ![バッチ トリガーの詳細を指定する](./media/logic-apps-batch-process-send-receive-messages/receive-batch-schedule-based.png)


4. バッチ トリガーが発動したときに電子メールを送信する別のアクションを追加します。 バッチの項目が 5 個になるたびに、または 5 分経過するたびに、ロジック アプリが電子メールを送信します。
...
5. 以前に電子メール プロバイダーの接続を作成していなかった場合は、求められたときに認証用の電子メールの資格情報を指定します。 詳しくは、[電子メールの資格情報の認証](../logic-apps/quickstart-create-first-logic-app-workflow.md)に関する記事をご覧ください。
