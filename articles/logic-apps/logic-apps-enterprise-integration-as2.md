...
* [**AS2 メッセージにエンコードする**アクション](#encode): 暗号化、デジタル署名、メッセージ処理通知 (MDN) を介した受信確認を可能にします。このことは、否認防止をサポートするのに役立ちます。 たとえば、このアクションでは、AS2/HTTP ヘッダーが適用され、構成されている場合は、次のタスクが実行されます。
...
* [**AS2 メッセージをデコードする**アクション](#decode): 暗号化の解除、デジタル署名、メッセージ処理通知 (MDN) を介した受信確認を可能にします。 たとえば、このアクションでは、次のタスクが実行されます。 
...
1. **[アクションを選択してください]** と検索ボックスで、 **[すべて]** を選択します。 検索ボックスに、「encode as2」と入力し、次のアクションを選択します。**AS2 メッセージにエンコードする**。

   ![[AS2 メッセージにエンコードする] を選択する](./media/logic-apps-enterprise-integration-as2/select-as2-encode.png)

1. ご利用の統合アカウントに対し既存の接続がない場合は、ここでその接続を作成するように求められます。 接続の名前を指定し、接続する統合アカウントを選択し、 **[作成]** を選択します。

   ![統合アカウントへの接続を作成する](./media/logic-apps-enterprise-integration-as2/as2-create-connection.png)  
 
1. ここで、以下のプロパティに関する情報を提供します。

   | プロパティ | 説明 |
   |----------|-------------|
   | **AS2-From** | AS2 契約で指定されているメッセージ送信者の識別子 |
   | **AS2-To** | AS2 契約で指定されているメッセージ受信者の識別子 |
   | **body** | メッセージ ペイロード |
   |||
...
1. **[アクションを選択してください]** と検索ボックスで、 **[すべて]** を選択します。 検索ボックスに「decode as2」と入力し、次のアクションを選択します。**AS2 メッセージをデコードする**

   ![[AS2 メッセージをデコードする] を選択する](media/logic-apps-enterprise-integration-as2/select-as2-decode.png)

1. ご利用の統合アカウントに対し既存の接続がない場合は、ここでその接続を作成するように求められます。 接続の名前を指定し、接続する統合アカウントを選択し、 **[作成]** を選択します。

   ![統合アカウントへの接続を作成する](./media/logic-apps-enterprise-integration-as2/as2-create-connection.png)  

1. **本文**および**ヘッダー**については、前のトリガー出力またはアクション出力から該当する値を選択します。
