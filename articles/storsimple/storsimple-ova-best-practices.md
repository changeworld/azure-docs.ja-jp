...
* グループ ポリシー オブジェクト (GPO) が仮想アレイに適用されないようにします。 仮想アレイ (子ノード) が親から GPO を自動的に継承しないように継承をブロックできます。 詳細については、「 [継承をブロックする](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc731076(v=ws.11))」をご覧ください。
...
* ボリュームを作成するときは、想定されるデータ消費量と将来的な増大を考慮してください。 ボリュームまたは共有を後から拡大することはできません。
* ボリュームや共有の作成後に、StorSimple 上のボリュームや共有のサイズを縮小することはできません。
...
* データが仮想アレイからクラウドに送信される前に AES-256 暗号化を適用するためにクラウド ストレージの暗号化キーを定義します。 データが最初に暗号化される場合、このキーは不要です。 キーは、 [Azure Key Vault](../key-vault/general/overview.md)などのキー管理システムを使用して生成し、安全に保管することができます。
* StorSimple Manager サービスを介してストレージ アカウントを構成する際は、必ず TLS モードを有効にし、StorSimple デバイスとクラウド間のネットワーク通信に安全なチャネルを作成します。
