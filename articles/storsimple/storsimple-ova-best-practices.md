...
次に、これまで計算した合計ローカル領域の 15% が、ローカル スナップショットを格納するための領域として必要です。つまり利用できる領域は 85% のみとなります。この例では、それに対応する領域が約 990 GB (= 0.85 × プロビジョニングするデータ ディスクの TB サイズ)。したがって、プロビジョニングするデータ ディスクは、(990(1/0.85))= 1320 GB = 1.16 TB = 約 1.25 TB (最も近い四分位数で丸める) となります。
