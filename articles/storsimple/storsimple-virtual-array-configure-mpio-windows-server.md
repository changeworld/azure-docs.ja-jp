...
* StorSimple Virtual Array に 1 つ以上のボリュームを作成する必要があります。 詳細については、StorSimple Virtual Array での[ボリュームの追加](storsimple-virtual-array-deploy3-iscsi-setup.md#step-3-add-a-volume)に関するトピックをご覧ください。 この手順では、仮想アレイに 3 つのボリューム (下図のとおり、1 つのローカル固定ボリュームと 2 つの階層化ボリューム) を作成しました。
