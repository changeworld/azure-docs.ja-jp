...
この記事では、ローカル Web UI を使用してStorSimple Virtual Array に更新プログラムをインストールするために必要な手順について説明します。各更新手順の所要時間は 2 分未満です。StorSimple Virtual Array を最新の状態に保つために、ソフトウェアの更新プログラムまたは修正プログラムを適用することが必要な場合があります。
...
## ローカル Web UI を使用する 
最新の更新プログラムをインストールするには、修正プログラムまたは更新プログラム (この時点で更新プログラムのインストールに Azure クラシック ポータルを使用できません) を適用するのにローカル Web UI を使用する必要があります。
...
3. Microsoft Update カタログの検索ボックスに、ダウンロードする修正プログラムのサポート技術情報 (KB) 番号を入力します。Update 0.2 では**3160441**、Update 0.1 では **3160441** を入力し、**検索**をクリックします。
...
4. 再起動が完了したら、サインイン ページが表示されます。ここでソフトウェアのバージョンを確認できます。**[メンテナンス]**、**[ソフトウェア更新プログラム]** の順に移動します。表示されるソフトウェアのバージョンが Update 0.2 では **10.0.10280.0**、Update 0.1 では **10.0.10279.0** になります。
