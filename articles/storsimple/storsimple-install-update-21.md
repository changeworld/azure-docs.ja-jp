...
12. デバイスで **StorSimple 8000 シリーズの Update 2.1 (6.3.9600.17704)** が実行されていることを確認します。**[最終更新日]** も変更されています。 
...
デバイスが 0.2、0.3、1.0 および 1.1 のバージョンを実行している場合は、LSI ドライバーおよびファームウェア更新をダウンロードしてインストールする必要があります。この更新プログラムは、Update 1.2 または 2 を実行している場合は既にインストールされています。
...
<br></br> ディスクのファームウェア更新もインストールしなければならない場合があります。`Get-HcsFirmwareVersion` コマンドレットを実行すると、ディスクのファームウェア更新が必要かどうかを確認することができます。`XMGG`、`XGEG`、 `KZ50`、`F6C2`、`VR08` のファームウェア バージョンを実行している場合、これらの更新プログラムをインストールする必要はありません。
...
詳細については、[Update 2.1 リリース](storsimple-update21-release-notes.md)に関するページを参照してください。
