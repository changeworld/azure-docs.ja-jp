...
| 手順 4. デバイスの最小セットアップを完了する。</br>オプション: StorSimple デバイスを更新する。 | 管理サービスを使用して、デバイスのセットアップを完了し、ストレージを提供できるようにします。 |
...
| 手順 7. ボリュームをマウント、初期化、フォーマットする。</br>オプション: MPIO を構成する。 | デバイスによって提供される iSCSI ストレージにサーバーを接続します。必要に応じて、サーバーがリンク、ネットワーク、およびインターフェイスの障害を許容できるように MPIO を構成します。 |
...
| | ネットワーク インターフェイス: DATA 0 のコントローラーの固定 IP | これらの IP は、インターネットへのルーティングが可能である必要があります。</br>コントローラー 0 の固定 IP アドレス:</br>コントローラー 1 の固定 IP アドレス:|
| | | | |
