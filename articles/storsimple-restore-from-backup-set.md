...
**\[バックアップ カタログ\]** ページを使用して、特定のバックアップからデバイスを復元できます。ただし、ボリュームを復元するとボリュームはバックアップが作成された時点の状態に戻ることに注意してください。バックアップ操作の後に追加されたすべてのデータは失われます。
...
3. バックアップ セットを展開して、関連付けられているボリュームを表示します。これらのボリュームは、復元する前にホストおよびデバイス上でオフラインにする必要があります。ボリュームをオフラインにするには、**\[ボリューム コンテナー\]** ページでボリュームにアクセスし、「[ボリュームをオフラインにする](storsimple-manage-volumes.md#take-a-volume-offline)」の手順を実行します。

    >  [AZURE.IMPORTANT]最初にホスト上のボリュームをオフラインにしてから、デバイス上のボリュームをオフラインにします。ホスト上のボリュームをオフラインにしない場合は、StorSimple Manager サービスによってホスト上のボリュームが自動的にオフラインに設定されます。その結果、データが破損する可能性があります。
