...
VNet は、1 つのリージョンの範囲内に作成します。 同じアドレス空間を持つ VNet を異なる 2 つのリージョン (たとえば米国東部と米国西部) に作成できますが、2 つを直接接続することはできません。 
