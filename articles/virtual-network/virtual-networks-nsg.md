...
| **発信元アドレスのプレフィックス** |ルールに関して一致する発信元アドレスのプレフィックスまたはタグ。 |1 つの IP アドレス (例: 10.10.10.10)、IP サブネット (例: 192.168.1.0/24)、[サービス タグ](#service-tags)、または * (すべてのアドレス)。 |ルールの数を減らすには、範囲、サービス タグ、* の使用を検討してください。 |
| **宛先アドレスのプレフィックス** |ルールに関して一致する宛先アドレスのプレフィックスまたはタグ。 | 1 つの IP アドレス (例: 10.10.10.10)、IP サブネット (例: 192.168.1.0/24)、[既定のタグ](#service-tags)、または * (すべてのアドレス)。 |ルールの数を減らすには、範囲、サービス タグ、* の使用を検討してください。 |
...
### <a name="default-tags"></a>システム タグ

サービス タグは、IP アドレスのカテゴリに対応するシステム指定の識別子です。 サービス タグは、任意のセキュリティ規則の**発信元アドレスのプレフィックス**および**宛先アドレスのプレフィックス** プロパティで使用できます。 詳細については、「[サービス タグ](security-overview.md#service-tags)」を参照してください。

### <a name="default-rules"></a>既定セキュリティ規則

すべての NSG に既定のセキュリティ規則一式が含まれています。 既定のルールは削除できませんが、これには最も低い優先順位が割り当てられているため、ルールを作成することで上書きできます。 詳しくは、「[既定セキュリティ規則](security-overview.md#default-security-rules)」をご覧ください。
...
| Azure Resource Manager テンプレート   | いいえ   | [はい](template-samples.md) |
...
* Azure Load Balancer 経由のトラフィックをフィルター処理する NSG を作成する場合、適用される発信元ポートとアドレス範囲は、ロード バランサー フロントエンドのものではなく、呼び出しを開始するコンピューターのものになります。 宛先ポートとアドレス範囲は、ロード バランサー フロントエンドのものではなく、宛先コンピューターのものになります。
* AzureLoadBalancer タグをブロックすると、Azure Load Balancer による正常性プローブが失敗し、サービスに悪影響が及ぶことがあります。
...
| ルール | Access | 優先順位 | 発信元アドレス範囲 | 発信元ポート | 宛先アドレス範囲 | 宛先ポート | プロトコル |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Allow-Inbound-HTTP-Internet | ALLOW | 100 | インターネット | * | * | 80 | TCP |
| Allow-Inbound-RDP-Internet | ALLOW | 200 | インターネット | * | * | 3389 | TCP |
| Deny-Inbound-All | 拒否 | 300 | インターネット | * | * | * | TCP |

**送信の規則**

| ルール | Access | 優先順位 | 発信元アドレス範囲 | 発信元ポート | 宛先アドレス範囲 | 宛先ポート | プロトコル |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Deny-Internet-All |拒否 |100 | * | * | インターネット | * | * |
...
> [!NOTE]
> 前述のルールの発信元アドレス範囲は**インターネット**であって、ロード バランサーの仮想 IP アドレスではありません。 発信元ポートは 500001 ではなく * です。 ロード バランサーの NAT 規則は、NSG セキュリティ規則とは異なります。 NSG セキュリティ規則は、常にトラフィックの最初の発信元と最終的な宛先に関連付けられ、両者の間にあるロード バランサーは**関係しません**。 Azure Load Balancer では、発信元の IP アドレスとポートが常に保持されます。
> 
> 
...
| ルール | Access | 優先順位 | 発信元アドレス範囲 | 発信元ポート | 宛先アドレス範囲 | 宛先ポート | プロトコル |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Deny-Inbound-RDP-Internet | 拒否 | 100 | インターネット | * | * | 3389 | TCP |
| Allow-Inbound-HTTP-Internet | ALLOW | 200 | インターネット | * | * | 80 | TCP |
...
* [NSG のトラブルシューティング](virtual-network-nsg-troubleshoot-portal.md)
