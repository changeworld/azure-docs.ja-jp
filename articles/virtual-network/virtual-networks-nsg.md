...
* **AzureLoadBalancer** (Resource Manager) (クラシックの場合は **AZURE_LOADBALANCER**): このタグは、Azure のインフラストラクチャのロード バランサーを表します。 このタグは、Azure Load Balancer の正常性プローブが開始される Azure データセンター IP に変換されます。
...
- **ロード バランサー:** Azure Load Balancer による VM とロール インスタンスの正常性プローブが許可されます。 このルールをオーバーライドすると、Azure Load Balancer による正常性プローブが失敗し、サービスに悪影響が及ぶことがあります。
