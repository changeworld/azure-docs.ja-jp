...
1. [Azure CLI のインストールと構成](../xplat-cli-install.md)に関する記事の手順に従って、Azure CLI 1.0 をインストールして構成し、`azure-login` コマンドを使用して Azure アカウントにログインします。

2. ログインして適切なサブスクリプションを選択した後で、次のコマンドを PowerShell で実行してプレビュー版に登録します (CLI では登録できません)。
