...
> [AZURE.NOTE] 最適なパフォーマンスを得るには、DNS サーバーとして Azure VM を使用する場合、IPv6 を無効にして、[インスタンスレベル パブリック IP](virtual-networks-instance-level-public-ip.md) を各 DNS サーバーの VM に割り当てる必要があります。Windows サーバーを DNS サーバーとして使用する場合、[この記事](http://blogs.technet.com/b/networking/archive/2015/08/19/name-resolution-performance-of-a-recursive-windows-dns-server-2012-r2.aspx)で、追加のパフォーマンス分析と最適化について説明しています。
