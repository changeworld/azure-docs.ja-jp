<properties 
   pageTitle="2 層アプリケーションを使用するハイブリッド接続 |Microsoft Azure"
...
- **azurevnet** ネットワークには、次の 3 つのファイアウォール仮想アプライアンスがあります。 
	...
	- **AZF2**。**azsn2** と **azsn3** 間のトラフィックを管理するために使用される内部ファイアウォール。これも 3-NIC 仮想アプライアンスです。
...
Azure の各サブネットは、そのサブネットで開始されたトラフィックをどのようにルーティングするかを定義するために使用される UDR テーブルにリンクすることができます。UDR が定義されていない場合、Azure では、既定のルートを使用してあるサブネットから別のサブネットにトラフィックが送信されることを許可します。UDR の理解を深めるには、「ユーザー定義ルートと IP 転送」を参照してください。

上記の要件に基づいて、適切なファイアウォール アプライアンスを経由して通信が行われることを保証するには、UDR を含む次のルート テーブルを **azurevnet** 内に作成する必要があります。

### azgwudr

このシナリオでは、ファイアウォールを管理するためにオンプレミスから Azure に送信されるトラフィックのみが使用され、**AZF3** に接続してそのトラフィックを内部ファイアウォール **AZF2** 経由で送信する必要があるようにします。したがって、**GatewaySubnet** には、次に示す 1 つのルートのみが必要です。

|変換先|次のホップ|説明|
|---|---|---|
|10\.0.4.0/24|10\.0.3.11|オンプレミスのトラフィックが管理ファイアウォール**AZF3** に送信されることを許可します|

### azsn2udr

|変換先|次のホップ|説明|
|---|---|---|
|10\.0.3.0/24|10\.0.2.11|アプリケーション サーバーをホストするバックエンド サブネットへのトラフィックが **AZF2** 経由で送信されることを許可します|
...
|変換先|次のホップ|説明|
|---|---|---|
|10\.0.3.0/24|192\.168.2.4|Azure 内のバックエンド サブネットへのトラフィックが **OPFW** で送信されることを許可します|
...
IP 転送の詳細については、「[ユーザー定義ルートと IP 転送](./virtual-networks-udr-overview/#ip-forwarding)」を参照してください。
...
**OPFW** に対する IP 転送が有効になっていない場合、パケットは Azure 仮想ネットワークのロジックによって削除されます。これは、パケットの VM への送信は、送信先が VM の IP アドレスである場合のみ許可されるためです。
...
- **ルート**: 10.0.0.0/16 (**onpremvnet**) へのすべてのトラフィックを、**port1** 経由で Azure ゲートウェイ IP アドレス (10.0.0.1) に送信する必要があります。
- **ポリシー**: **port1** と **port2** 間のすべての双方向トラフィックを許可します。
