...
Azure ネットワークを設計する際には、出発点として以下の質問をご利用ください。
...
VNet とサブネット リソースは、Azure で実行するワークロードのセキュリティ境界を定義する際に役立ちます。VNET は、(CIDR ブロックとして定義される) アドレス空間のコレクションを特徴としています。
...
ソリューションを設計する前に、[Azure のネットワーク サービスに関連するすべての制限事項](../azure-subscription-service-limits#networking-limits)を確認してください。一部の制限は、サポート チケットを開いて引き上げることができます。
...
VNet ごとにアドレス空間を指定する必要もあります。オンプレミスのデータ センターと Azure リージョンの間に接続が必要になるため、Azure VNet に使用されるアドレス空間がオンプレミスと競合することはできません。各 VNet で使用されるアドレス空間が他の既存の VNet と競合することもできません。下の表のアドレス空間を利用すれば、以上の要件を満たせる場合があります。
