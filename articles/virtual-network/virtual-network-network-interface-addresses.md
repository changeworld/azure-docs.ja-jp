...
Azure へのログインまたは接続に使用するアカウントは、[ネットワークの共同作業者](../role-based-access-control/built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor)ロール、または[ネットワーク インターフェイスのアクセス許可](../role-based-access-control/custom-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json)の一覧で示されている適切なアクセス許可を割り当てられた[カスタム ロール](virtual-network-network-interface.md#permissions)に、割り当てられている必要があります。

## <a name="add-ip-addresses"></a>IP アドレスを追加する

[Azure の制限](#private)に関する記事で示されている制限内であれば、必要なだけいくつでも、[プライベート](#public)および[パブリック](#ipv4)な [IPv4](../azure-resource-manager/management/azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#azure-resource-manager-virtual-networking-limits) アドレスをネットワーク インターフェイスに追加できます。 既存のネットワーク インターフェイスの 1 つの[セカンダリ IP 構成](#secondary)に、プライベート IPv6 アドレスを追加できます (既存のセカンダリ IP 構成がない場合のみ)。 各ネットワーク インターフェイスには、IPv6 プライベート アドレスを 1 つしか含めることができません。 必要に応じて、パブリック IPv6 アドレスを IPv6 ネットワーク インターフェイス構成に追加できます。 IPv6 アドレスの使用の詳細については、「[IPv6](#ipv6)」を参照してください。

1. Azure Portal 上部に "*リソースの検索*" というテキストが表示されたボックスがあります。そこに "*ネットワーク インターフェイス*"{5}{6}と入力します。 検索結果に **[ネットワーク インターフェイス]** が表示されたら、それを選択します。
...
各ネットワーク インターフェイスには、[プライベート](#primary) [IPv4](#private) アドレスが割り当てられた[プライマリ](#ipv4) IP 構成が 1 つ必要です。 1 つ以上の[セカンダリ](#secondary) IP 構成を追加することができ、各構成には IPv4 プライベート IP アドレスと、必要に応じて IPv4 [パブリック](#public) IP アドレスを割り当てます。

### <a name="ipv6"></a>IPv6

ネットワーク インターフェイスの 1 つのセカンダリ IP 構成には、プライベート [IPv6](#ipv6) アドレスを 1 つ割り当てることができます (割り当てなくてもかまいません)。 ネットワーク インターフェイスは、既存のセカンダリ IP 構成を使うことはできません。 各ネットワーク インターフェイスには、IPv6 プライベート アドレスを 1 つしか含めることができません。 必要に応じて、パブリック IPv6 アドレスを IPv6 ネットワーク インターフェイス構成に追加できます。
