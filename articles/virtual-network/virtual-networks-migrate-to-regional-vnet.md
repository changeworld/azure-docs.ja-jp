...
現在アフィニティ グループに関連付けられている VNet は、近い将来にリージョン VNet に移行可能になります。このページでは、移行のための処理が進行中であること、および移行を完了するために次の作業を行うタイミングをお知らせするために、随時情報を更新していきます。

アフィニティ グループは、VNet での使用は推奨されなくなりましたが、現在でも存在しています。このため、VNet を移行しない選択や、移行を遅らせる選択ができます。VNet は移行しなくても動作しますが、制限事項に直面する可能性があるほか、リージョン VNet を必要とする多くの新機能を利用することができません。そのため、VNet を有効にするときに、アフィニティ グループ VNet をリージョン VNet に移行することを強くお勧めします。

### 早期の移行

サポート契約をご契約のお客様は、早期の移行が可能です。早期の移行をするには、次の手順に従います。

1. **管理ポータル**で、VNet ページの右上隅にある [**Microsoft サポートに問い合わせる**] をクリックします。

	![VNet のリージョン移行の要求](./media/virtual-networks-migrate-to-regional-vnet/IC790447.png)

1. [**Microsoft サポートに問い合わせる**] ページで、次の項目を選択します。 サブスクリプション : 複数のサブスクリプションがある場合は、移行対象の VNet に対応するサブスクリプションを選択します。サポートの種類 : 技術 製品の種類 : Virtual Network (VNet) 問題の種類 : リージョン Virtual Network への Virtual Network の移行

	- **サブスクリプション :** 複数のサブスクリプションがある場合は、移行対象の VNet に対応するサブスクリプションを選択します。

	- **サポートの種類 :** *技術*

	- **製品の種類 :** *Virtual Network (VNet)*

	- **問題の種類 :** *リージョン Virtual Network への Virtual Network の移行*

1. 前提条件 : 次の手順を実行するには、VNet が既に移行可能に設定されていなければなりません。VNet が移行可能になったことがわかるのは、確認通知を受信したときか (早期の移行を要求した場合)、このページの更新後にすべての VNet が移行可能と表示されたときです。
