...
Form Recognizer 事前構築済みモデルを使用して、請求書、レシート、身分証明書、または名刺からテキスト、テーブル、キーと値のペアを抽出します。
...
4. 次のオプションを使用して、分析するファイルを選択します。
    ...
    * 身分証明書の画像の URL。 このクイックスタートでは、[サンプル ID ドキュメント](https://raw.githubusercontent.com/Azure-Samples/cognitive-services-REST-api-samples/master/curl/form-recognizer/DriverLicense.png)を使用できます。
...
> [Form Recognizer のクライアント ライブラリ SDK と REST API クイックスタートを利用する](./try-sdk-rest-api.md)