...
サブスクリプションの中でそのネットワークに接続する最初の Web アプリの場合は、仮想ネットワークでの認証のための証明書がシステムにより作成されます。証明書を確認するには、[Azure ポータル](http://go.microsoft.com/fwlink/?LinkId=529715)で仮想ネットワークに移動し、ネットワークを選択した後、[証明書] タブを選択します。  
...
App Service プランを、無料、共有、または基本など、低いレベルのプランに縮小すると、そのプランに含まれる Web アプリで使用される仮想ネットワーク接続は無効になります。そのプランを再び標準プランに拡張すると、それらのネットワーク接続は再確立されます。
