...
* **&lt;Record&gt;**: 呼び出し元の声を録音し、録音されたファイルの URL を返します。
...
アプリケーションで Twilio API の呼び出しをインスタンス化する場合 (たとえば、**CallResource.Create** メソッドを使用した場合)、Twilio は TwiML 応答を返すことが想定されている URL にユーザーの要求を送信します。 「[方法: 発信通話する](#howto_make_call)」の例では、Twilio から提供される URL [http://twimlets.com/message][twimlet_message_url] を使用して応答を返します。
