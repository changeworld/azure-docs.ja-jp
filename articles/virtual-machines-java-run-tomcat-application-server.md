<properties linkid="dev-java-vm-application-server" urlDisplayName="仮想マシンで Tomcat を実行する" pageTitle="仮想マシンで Tomcat を実行する - Azure チュートリアル" metaKeywords="Azure vm, Tomcat の vm を作成する, Tomcat の vm を構成する" description="Windows 仮想マシンを作成し、Apache Tomcat アプリケーション サーバーを実行できるように仮想マシンを構成する方法について説明します。" metaCanonical="" services="virtual-machines" documentationCenter="Java" title="Java アプリケーション サーバーを仮想マシンで実行する方法" authors="waltpo" solutions="" manager="" editor="" />
...
2. **32 ビット/64 ビット Windows サービス インストーラー**のリンクをダブルクリックします。この場合、Tomcat は Windows サービスとしてインストールされます。
