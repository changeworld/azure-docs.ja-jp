...
------------ | ------------- | ------------ | ------------ | ------------- |
| ディスク障害 | はい | 障害が発生したノードの高速置換 | [vSAN の既定のストレージ ポリシーについて](https://docs.vmware.com/en/VMware-vSphere/6.7/com.vmware.vsphere.virtualsan.doc/GUID-C228168F-6807-4C2A-9D74-E584CAF49A2A.html) |
| ファン障害 | はい | 冗長性のあるファン、障害が発生したノードの高速置換 |  |  |
| NIC の障害 | はい | 冗長性のある NIC、障害が発生したノードの高速置換
| ホストの電源障害 | はい | 冗長電源 |  |  |
| ESXi ホスト障害 | はい | 障害が発生したノードの高速置換 | [VMware vSphere の高可用性](https://www.vmware.com/products/vsphere/high-availability.html) |  |  |
| VM の障害 | はい | [ロード バランサー](load-balancers.md)  | [VMware vSphere の高可用性](https://www.vmware.com/products/vsphere/high-availability.html) | ステートレス VMware VM 用の Azure Load Balancer |
| リーフ スイッチ ポートの障害 | はい | 冗長性のある NIC |  |  |
| リーフ スイッチの障害 | はい | 冗長性のあるリーフ スイッチ |  |  |
| ラックの障害 | はい | 配置グループ |  |  |
| オンプレミスの DC へのネットワーク接続 | はい  | 冗長性のあるネットワーク サービス |  | 冗長性のある ER 回線 |
| Azure へのネットワーク接続 | はい | |  | 冗長性のある ER 回線 |
| データセンターの障害 | はい |  |  | 可用性ゾーン |
| リージョンの障害 | はい  |  |  | Azure Azure リージョン |
