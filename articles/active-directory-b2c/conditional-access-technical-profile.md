...
このサンプルを動作させるには、次の例に示す要求を拡張ファイルの ClaimsSchema セクションに追加します。
...
TrustFrameworkPolicy 要素に、次の例に示すようにこれらの SubJourneys を追加します。
...
次の例に示すように、新しい要求を使用するユーザー体験を追加します。
...
この UserJourney を参照する証明書利用者ファイルの例を次に示します。
