...
| --------- | -------- | ----------- |
| Type | はい | 要求マスクの種類です。 指定できる値: `Simple`および`Regex`。 値が `Simple` は、要求の文字列の先頭部分に単純なテキスト マスクを適用することを示します。 値が `Regex` は正規表現が全体として、要求の文字列に適用されることを示します。  `Regex`値が指定されている場合、オプショナルな属性も正規表現で定義することが必要となります。 |
