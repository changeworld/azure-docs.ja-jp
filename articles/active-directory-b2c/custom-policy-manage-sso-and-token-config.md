...
セッションの動作と SSO の構成を変更するには、**RelyingParty** 要素の内部に [UserJourneyBehaviors](relyingparty.md) 要素を追加します。  **UserJourneyBehaviors** 要素は、**DefaultUserJourney** の直後にする必要があります。 **UserJourneyBehavors** 要素の内部は、次の例のようにする必要があります。
