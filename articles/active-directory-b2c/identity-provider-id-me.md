...
Azure Active Directory B2C (Azure AD B2C) で ID.me アカウントを持つユーザーのサインインを有効にするために、[API と SDK の ID.me Developer Resources](https://developers.id.me/) でアプリケーションを作成する必要があります。 詳細については、「[OAuth 統合ガイド](https://developers.id.me/documentation/oauth/overview/kyc)」を参照してください。 まだ ID.me 開発者アカウントを持っていない場合は、[https://developers.id.me/registration/new](https://developers.id.me/registration/new) でサインアップできます。
...
1. **[新規作成]** を選択します
    ...
    1. **[Redirect URI]** に、「`https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp`」と入力します。 [カスタム ドメイン](custom-domain.md)を使用する場合は、「`https://your-domain-name/your-tenant-name.onmicrosoft.com/oauth2/authresp`」と入力します。 `your-tenant-name` を実際のテナントの名前に、`your-domain-name` を実際のカスタム ドメインに置き換えます。 
1. **[続行]** をクリックします。
...
ユーザーが ID.me アカウントを使用してサインインできるようにするには、そのアカウントを Azure AD B2C がエンドポイント経由で通信できる相手のクレーム プロバイダーとして定義する必要があります。 エンドポイントは、特定のユーザーが認証されていることを確認するために Azure AD B2C で使う一連の要求を提供します。
...
1. **[今すぐ実行]** ボタンを選択します。
1. サインアップまたはサインイン ページで、 **[ID.me]** を選択して ID.me アカウントでサインインします。
