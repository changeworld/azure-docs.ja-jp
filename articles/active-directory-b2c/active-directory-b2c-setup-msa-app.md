...
Azure Active Directory (AD) B2C で ID プロバイダーとして Microsoft アカウントを使用するには、最初に Microsoft アカウント アプリケーションを作成し、適切なパラメーターを提供する必要があります。そのためには Microsoft アカウントが必要です。ない場合は、[https://www.live.com/](https://www.live.com/) で取得できます。
...
2. **[アプリケーションの作成]** をクリックします。
    ...
    ![MSA - 新規アプリケーションの追加](./media/active-directory-b2c-setup-msa-app/msa-add-new-app.png)

3. **アプリケーション名**を入力し、**[同意する]** をクリックします。注: Microsoft サービスの利用規約に同意する必要があります。

    ![MSA - アプリケーション名](./media/active-directory-b2c-setup-msa-app/msa-app-name.png)
...
5. **[リダイレクト URL]** フィールドに「`https://login.microsoftonline.com/te/{tenant}/oauth2/authresp`」と入力します。**{tenant}** は実際のテナントの名前 (例: contosob2c.onmicrosoft.com) に置き換えます。ページの下部にある **[保存]** をクリックします。
