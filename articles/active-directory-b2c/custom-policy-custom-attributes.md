...
「[Azure Active Directory B2C: カスタム ポリシーの概要](custom-policy-get-started.md)」の手順に従った場合は、**TrustFrameworkBase.xml**、**TrustFrameworkExtensions.xml**、**SignUpOrSignin.xml**、**ProfileEdit.xml**、**PasswordReset.xml** という名前の[サンプル ファイル](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/archive/master.zip)をダウンロードして変更しました。 この手順では、これらのファイルにさらに変更を加えます。
