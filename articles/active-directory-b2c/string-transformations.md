...
| InputParameter | maximumNumber | int | [省略可能] `INTEGER` randomGeneratorType のみです。 maximute 数を指定します。 |
...
指定された書式設定文字列に従って要求の書式を設定します。 この変換は C# `String.Format` メソッドを使用します。
