...
4. Web ベースのアプリケーションを作成している場合は、**[Web アプリ/Web API を含める]** スイッチを **[はい]** に切り替えます。 **[応答 URL]** は、アプリケーションが要求したトークンを Azure AD B2C が返すエンドポイントです。 たとえば、「 `https://localhost:44316/`」のように入力します。 Web アプリケーションで、Azure AD B2C によってセキュリティ保護された Web API も呼び出す場合は、**[キーの生成]** ボタンをクリックして**アプリケーション シークレット**を作成することも必要になります。
