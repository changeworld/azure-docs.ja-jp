...
言語のカスタマイズを使用すると、顧客のニーズに合わせてユーザー体験を異なる言語に変更できます。  37 言語の翻訳を提供しています (「[追加情報](#additional-information)」を参照)。  ページが 1 つの言語でのみ提供されている場合でも、ページ上の任意のテキストをニーズに合わせてカスタマイズできます。  
...
3. **[Supported languages]\(サポートされている言語\)** ブレードに移動します。  ここでは、**[リソースの追加]** を選択できます。
4. サポートする言語をすべて選択します。  
