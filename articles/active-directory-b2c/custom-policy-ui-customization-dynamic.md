...
この記事では、カスタム ポリシーを使用して、*動的コンテンツ*を含む Azure AD B2C ユーザー インターフェイスをカスタマイズする方法に重点を置いて説明します。 作業を開始する場合は、「[Azure Active Directory B2C: カスタム ポリシーでの UI カスタマイズの構成](custom-policy-ui-customization.md)」を参照してください。

>[!NOTE]
>Azure AD B2C 記事「[Azure Active Directory B2C: カスタム ポリシーでの UI カスタマイズの構成](custom-policy-ui-customization.md)」では、以下の基本について説明しています。
...
「[Azure Active Directory B2C: カスタム ポリシーでの UI カスタマイズの構成](custom-policy-ui-customization.md)」では、HTML5 ファイルを Azure Blob Storage にアップロードします。 こうした HTML5 ファイルは静的で、各要求で同じ HTML コンテンツをレンダリングします。
...
3. アプリケーションに (*Contoso.AADB2C.UI* などの) 名前を付けて、 **[OK]** を選択します。
    ...
    ![新しい Visual Studio プロジェクトを作成する](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. **[Web アプリケーション]** テンプレートを選択します。

5. 認証を **[認証なし]** に設定します。

    ![Web アプリケーション テンプレートを選択する](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Views フォルダーの下の Home フォルダーを右クリックし、 **[追加]**  >  **[新しいアイテム]** の順に選択します。
    ...
    ![Visual Studio の新しいアイテムの追加メニュー項目](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. **[追加]** を選択します。
    ...
    ![Visual Studio の [MVC ビュー ページ] が強調表示された [新しいアイテムの追加] ダイアログ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. ファイルのコンテンツをコピーして、レイアウト定義の下に貼り付けます。 コードは次のようになります。
    ...
    ![HTML5 を追加した後の unified.cshtml ファイル](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![カスタムの background_background_image src 値を持つ img 要素](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. **Controllers\HomeController.cs** を開いて、次のメソッドを追加します。
    ...
    ![ビューをレンダリングするためにコントローラーを変更する](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. **ソリューション エクスプローラー**で **Contoso.AADB2C.UI** プロジェクトを右クリックしてから、 **[発行]** を選択します。
    ...
    ![Microsoft Azure App Service への発行](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. **[Microsoft Azure App Service]** タイルを選択してから、 **[発行]** を選択します。

    ![新しい Microsoft Azure App Service の作成](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. **[作成]** をクリックして、Azure リソースの作成を開始します。
    ...
    ![App Service プロパティの入力](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. [Azure Portal](https://portal.azure.com/) で、 **[App Services]** を選択してから、API アプリの名前を選択します。
    ...
    ![Azure Portal で API アプリを選択する](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. **[設定]** セクションの **[API]** セクションで、 **[CORS]** を選択します。

    ![Azure portal 内の [App Service] メニューで強調表示された [CORS] メニュー項目](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. **[保存]** を選択します。
    ...
    ![[許可される元のドメイン] でアスタリスクが強調表示された CORS 設定ページ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. `LoadUri` の値を _~/tenant/default/unified_ から _https://<アプリ名>.azurewebsites.net/home/unified_ に変更します。
    ...
    ![LoadUri 要素が強調表示された XML スニペットの例](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. アップロードした証明書利用者 (RP) カスタム ポリシーである **B2C_1A_signup_signin** を開いてから、 **[今すぐ実行]** を選択します。
    ...
    ![サインアップまたはサインイン ポリシー](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. ID `background_background_image` の `<img>` 要素を見つけて、`src` の値を `@ViewData["background"]` に置き換えます。
    ...
    ![src 値が強調表示された img 要素 ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. _campaignId_ クエリ文字列パラメーターを URI に追加します。 たとえば、以下のイメージのように、`&campaignId=hawaii` を追加します。
    ...
    ![campaignId クエリ文字列パラメーターが強調表示された URI](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. **Enter** キーを押して、ハワイの背景イメージを表示します。

    ![ハワイ画像のカスタム背景のサインアップ サインイン ページ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. 値を *Tokyo* に変更して **Enter** キーを押します。
    ブラウザーに東京の背景が表示されます。

    ![東京画像のカスタム背景のサインアップ サインイン ページ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
...
* 「[カスタム ポリシーの概要](custom-policy-get-started.md)」チュートリアルの完了後に、独自のカスタム ポリシー ファイルを使用してシナリオを構築することをお勧めします。 参照用に[サンプルのポリシー ファイル](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-ui-customization)が提供されています。
