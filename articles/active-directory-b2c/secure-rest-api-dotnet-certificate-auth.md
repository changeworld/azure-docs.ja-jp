---
title: クライアント証明書を使用して RESTful サービスをセキュリティで保護する
...
関連する記事では、Azure Active Directory B2C (Azure AD B2C) と対話する [RESTful サービスを作成](rest-api-claims-exchange-dotnet.md)します。
...
>[!NOTE]
>また、[HTTP 基本認証](secure-rest-api-dotnet-basic-auth.md)を使用して、RESTful サービスをセキュリティで保護することもできます。 ただし、HTTP 基本認証は、クライアント証明書より安全性が低いと見なされます。 この記事で説明するように、RESTful サービスは、クライアント証明書認証を使用してセキュリティで保護することをお勧めします。
...
* [REST API 要求交換を統合](rest-api-claims-exchange-dotnet.md)する記事の手順を完了している。
...
7. **[パスワード]** ボックスに、証明書のパスワードを入力します。
    ...
    ![Azure portal の [キーの作成] ページでポリシー キーをアップロードする](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-upload.png)
...
5. `<Metadata>` 要素の終了直後に、次の XML スニペットを追加します。
    ...
    ![ClientCertificate 認証 XML 要素の設定](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-tech-profile.png)
...
3. **[名]** ボックスに「**Test**」と入力して、プロセスをテストします。
    ...
    ![[名] テキスト ボックスが強調表示され、入力の検証エラーが表示されている](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-test.png)
...
![証明書の検証コードの追加](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-code.png)
...
この手順をトラブルシューティングする必要がある場合は、[Application Insights を使用したログの収集](troubleshoot-with-application-insights.md)に関する記事をご覧ください。

## <a name="optional-download-the-complete-policy-files-and-code"></a>完全なポリシー ファイルとコードをダウンロードする (省略可能)
* 「[カスタム ポリシーの概要](custom-policy-get-started.md)」チュートリアルの完了後に、独自のカスタム ポリシー ファイルを使用してシナリオを構築することをお勧めします。 参照用に[サンプルのポリシー ファイル](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw-secure-cert)が提供されています。
