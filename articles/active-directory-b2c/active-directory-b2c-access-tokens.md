...
アクセス トークンを要求する前に、Web API を登録し、クライアント アプリケーションに付与できるアクセス許可を発行しておく必要があります。 まず、「[Web API の登録](active-directory-b2c-app-registration.md#register-a-web-api)」の手順に従ってください。
...
1. まだであれば、クライアント アプリケーション ([Web アプリ](active-directory-b2c-app-registration.md#register-a-web-application)または[ネイティブ クライアント](active-directory-b2c-app-registration.md#register-a-mobilenative-application)) を登録します。
1. アプリケーションの [設定] ブレードで、**[API アクセス]** を選択します。
1. **[追加]** をクリックします。
...
> [!NOTE]
> この時点で、カスタム ドメインはアクセス トークンと同時にサポートされていません。 要求 URL 内で yourtenantId.onmicrosoft.com ドメインを使用する必要があります。
