...
[関連する Azure AD B2C の記事](rest-api-claims-exchange-dotnet.md)では、Azure Active Directory B2C (Azure AD B2C) のユーザー体験を認証なしで統合する RESTful サービス (Web API) を作成しました。
...
[REST API 要求交換の Azure AD B2C ユーザー体験への統合](rest-api-claims-exchange-dotnet.md)に関する記事の手順を完了してください。
...
1. *[App_Start]* フォルダー右クリックし、 **[追加]** を選択し、次に **[クラス]** を選択します。
   ...
   ![App_Start フォルダーに ClientAuthMiddleware.cs クラスを追加する](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup-auth1.png)

2. **[名前]** ボックスに「**ClientAuthMiddleware.cs**」と入力します。

   ![Visual Studio の [新しい項目の追加] ダイアログでの新しい C# クラスの作成](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup-auth2.png)
...
1. プロジェクトを右クリックし、 **[追加]**  >  **[新しい項目]** の順に選択して、**OWIN** を検索します。
   ...
   ![Visual Studio の [新しい項目の追加] ダイアログでの OWIN Startup クラスの作成](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-OWIN-startup.png)
...
![コントローラーに [承認] タグを追加する](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-authorize.png)
...
5. `<Metadata>` 要素の終了直後に、次の XML スニペットを追加します。
    ...
    ![基本認証の XML 要素を TechnicalProfile に追加する](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-secure-basic-add-1.png)
...
3. **[名]** ボックスに「**Test**」と入力して、プロセスをテストします。
    ...
    ![ID API で名前の入力検証をテストする](./media/secure-rest-api-dotnet-basic-auth/rest-api-netfw-test.png)
...
* 「[カスタム ポリシーの概要](custom-policy-get-started.md)」チュートリアルの完了後に、独自のカスタム ポリシー ファイルを使用してシナリオを構築することをお勧めします。 参照用に[サンプルのポリシー ファイル](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw-secure-basic)が提供されています。
