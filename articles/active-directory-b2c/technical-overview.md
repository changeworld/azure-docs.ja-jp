...
* **ユーザー フロー** と **カスタム ポリシー** - アプリケーションに対する組み込み (ユーザー フロー) と完全にカスタマイズ可能 (カスタム ポリシー) な ID エクスペリエンスです。
  ...
  * 組織、顧客、従業員、パートナー、市民に固有の複雑な ID ワークフローには、"*カスタム ポリシー*" を使用します。
...
* **ソーシャル** ID または **エンタープライズ** ID。ユーザーの ID は、フェデレーション ID プロバイダーによって管理されます。 たとえば、Facebook、Microsoft、ADFS、Salesforce などです。
...
詳細については、「[ローカル アカウント ID プロバイダーを設定する](identity-provider-local.md)」を参照してください。
...
![パスワードの強制リセット フロー](media/technical-overview/force-password-reset-flow.png)
