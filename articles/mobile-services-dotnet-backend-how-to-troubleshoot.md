...
10. サービスが正常に発行されたら、**サーバー エクスプローラー**を開き、**[Microsoft Azure]** ノードと **[Mobile Services]** ノードを展開します。必要に応じてサインインします。
...
モバイル サービスでは顧客からの要求を処理するときに、さまざまな役立つ診断情報を生成します。また、発生した例外も取得します。これ以外にも、各 [**TableController**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.tables.tablecontroller.aspx) の [**Services**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.tables.tablecontroller.services.aspx) プロパティで使用可能な [**Log**](http://msdn.microsoft.com/library/microsoft.windowsazure.mobile.service.apiservices.log.aspx) プロパティを利用して、追加のログでコントローラー コードをインストルメントすることができます。
...
同じログは、**Azure の管理ポータル**で、モバイル サービスの **[ログ]** タブでも使用できます。
