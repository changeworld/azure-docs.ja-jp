...
| networkReconnect          | 接続が失われ、ネットワークに再度接続しています。 | - メディア転送接続が失われると、`Poor` に設定されます <br/> - ネットワークへの接続が切れると、`Bad` に設定されます <br/> - 新しいセッションが接続済みになると、`Good` に設定されます。                                                                       | 帯域幅が狭い、インターネット接続がない                          |
