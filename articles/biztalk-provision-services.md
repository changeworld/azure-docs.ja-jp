...
4.  BizTalk サービスの設定を入力します。
    ...
    | **エディション**       | テスト/開発フェーズで作業している場合は、**開発者**を選択します。現在のフェーズが運用フェーズの場合は、[BizTalk サービス: エディションのチャート][BizTalk サービス: エディションのチャート]を使用して、**Premium**、**Standard**、**Basic** のどれが実際のビジネス シナリオに最適であるかを判断してください。 |
