...
description: Azure Network Watcher の接続モニターのスキーマについて説明します。
...
ms.locfileid: "114451035"
---
# <a name="azure-network-watcher-connection-monitor-schema"></a>Azure Network Watcher の接続モニターのスキーマ
...
ログつまり Log Analytics に取り込まれるデータには、2 つの種類があります。
テスト データ (NWConnectionMonitorTestResult クエリ) は、特定のテストグループの監視頻度に基づいて更新されます。
パス データ (NWConnectionMonitorPathResult クエリ) は、損失率またはラウンドトリップ時間が大きく変化したときに更新されます。
両者は独立しているため、テスト データは更新され続けているのに、パス データはあまり更新されない期間が発生することがあります。

## <a name="connection-monitor-tests-schema"></a>接続モニターのテスト スキーマ

接続モニターのテスト データ スキーマのフィールドとその説明を次に示します 
...
| RoundTripTimeMsThreshold  | テストに設定されているラウンドトリップしきい値 (ミリ秒) |
...
接続モニターのパス データ スキーマのフィールドとその説明を次に示します 
