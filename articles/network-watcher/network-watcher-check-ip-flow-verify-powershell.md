...
仮想マシン上の NIC の IP アドレスが必要です。 次のコマンドを使用して仮想マシンにアタッチされている NIC を取得します。 仮想マシン上のテスト対象 IP アドレスが既にわかっている場合、この手順は省略できます。
...
トラフィックをテストする `Test-AzureRmNetworkWatcherIPFlow` コマンドレットを実行します。 この例では、1 番目の NIC の最初の IP アドレスを使用します。
