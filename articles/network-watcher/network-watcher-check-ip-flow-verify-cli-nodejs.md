...
仮想マシン上の NIC の IP アドレスが必要です。 次のコマンドを使用して仮想マシンの NIC を取得します。 仮想マシン上のテスト対象 IP アドレスが既にわかっている場合、この手順は省略できます。
...
トラフィックをテストする `network watcher ip-flow-verify` コマンドレットを実行します。 この例では、1 番目の NIC の最初の IP アドレスを使用します。
