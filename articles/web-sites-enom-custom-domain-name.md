<properties 
	pageTitle="Azure の Web サイトの eNom ドメイン名の構成" 
...
4. Host Records エディターでは、**[Record Type]** フィールドを使用して特定のレコード タイプを選択できます。Azure Web サイトの場合、**[CNAME (Alias)]** または **[A (Address)]** のみを選択して使用する必要があります。

	![zone file editor](./media/web-sites-custom-domain-name/e-editrecords.png)

	> [AZURE.NOTE] エントリをゾーン ファイルに追加する前に、eNom ではルート ドメイン ('@') とワイルドカード サブドメイン (''\*') の DNS レコードが既に作成されています。ルート ドメインを Web サイトにリダイレクトするか、ワイルドカード サブドメインの A レコードを使用する場合は、新しいエントリを作成せずに、これらの既存のエントリを変更する必要があります。

	* CNAME レコードを追加するときは、**[Host Name]** フィールドを、使用するサブドメインに設定する必要があります。たとえば **www** にします。**[Address]** フィールドを、Azure Web サイトの **.azurewebsites.net** ドメイン名に設定する必要があります。たとえば **contoso.azurwebsites.net** にします。

		> [AZURE.NOTE] また、A レコードを使用する場合、次のいずれかの構成で CNAME レコードを追加する必要があります。
		> 
		> * **[Alias]** 値、**www**、**[Other host]** 値、**&lt;yourwebsitename&gt;.azurewebsites.net**。
		> 
		> または
		> 
		> * **[Alias]** 値、**awverify.www**、**[Other host]** 値、**awverify.&lt;yourwebsitename&gt;.azurewebsites.net**。
		> 
		> この CNAME レコードは Azure で使用されて、A レコードで参照されるドメインの所有者が検証されます。

	* A レコードを追加するときは、**[Host Name]** フィールドを、**@** (**contoso.com** などのルート ドメイン名) か、(複数のサブドメインに一致するワイルドカード) か、使用する特定のサブドメイン (**www**.) のいずれかに設定する必要があります。**[Address]** フィールドを、Azure Web サイトの IP アドレスに設定する必要があります。

		> [AZURE.NOTE] A レコードを追加するときは、ホストが **[awverify]**、**[Points to]** が **awverify.&lt;yourwebsitename&gt;.azurewebsites.net.の CNAME レコードも追加する必要があります。
