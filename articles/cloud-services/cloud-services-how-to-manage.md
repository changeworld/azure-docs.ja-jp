...
- スワップを実行する前に、ロールのすべてのインスタンスを実行する必要があります。 インスタンスの状態は、Azure クラシック ポータルで、または [Windows PowerShell で Get-AzureRole コマンド](/powershell/module/azure/get-azurerole?view=azuresmps-3.7.0)を使用して確認できます。
