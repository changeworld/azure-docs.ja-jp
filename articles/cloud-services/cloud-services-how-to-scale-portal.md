...
>[AZURE.NOTE] この記事では、クラウド サービスの Web ロールと worker ロールについて説明します。仮想マシンを直接作成した場合 (クラシック)、仮想マシンはクラウド サービスでホストされます。標準の仮想マシンは、[可用性セット](../virtual-machines/virtual-machines-windows-classic-configure-availability.md)に関連付けることでスケールすることができる共に、手動でオンまたはオフすることができます。
