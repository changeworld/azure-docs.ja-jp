<properties 
   pageTitle="Visual Studio の接続済みサービスを使用して Azure Storage を追加する"
...
Visual Studio 2015 では、**[接続済みサービスの追加]** ダイアログを使用して、Azure Storage に C#Cloud Services 、.NET バックエンド モバイル サービス、ASP.NET Web サイトまたはサービス、ASP.NET 5 サービス、あるいは Azure Web ジョブ サービスを接続できます。接続済みサービス機能は、必要なすべての参照と接続コードを追加し、構成ファイルを適切に変更します。また、ダイアログでは、次の手順が BLOB ストレージ、キュー、およびテーブルの開始であることを知らせるドキュメントが示されます。
...
- .NET クラウド サービスの Web ロールと ｗorker ロール プロジェクト
...
1. Azure アカウントがあることを確認します。Azure アカウントを持っていない場合は、[無料評価版](http://go.microsoft.com/fwlink/?LinkId=518146)にサインアップできます。Azure アカウントを取得したら、ストレージ アカウントを作成し、Mobile Services を作成して、Azure Active Directory を構成することができます。
...
1. **[接続済みサービスの追加]** ダイアログ ボックスで、**[Azure Storage]** を選択し、**[構成]** ボタンをクリックします。Azure にまだログインしていない場合は、ログインを促すメッセージが表示されます。

    ![[接続済みサービスの追加] ダイアログ ボックス - [Storage]](./media/vs-azure-tools-connected-services-storage/IC796703.png)
...
1. WebJob プロジェクトの [サービス参照] ノードの下に、ストレージの接続済みサービスが表示されます。
    ...
    ![Web ジョブ プロジェクト内の Azure Storage](./media/vs-azure-tools-connected-services-storage/IC796705.png)
...
ダイアログが完了すると、Visual Studio は参照を追加し、特定の構成ファイルを変更します。特定の変更はプロジェクトの種類によって異なります。
 ...
 - クラウド サービス プロジェクト (Web ロールおよびワーカー ロール) の場合は、[変更内容 – クラウド サービス プロジェクト](http://go.microsoft.com/fwlink/p/?LinkId=516965)を参照してください。 
 - WebJob プロジェクトの場合は、[変更内容 - WebJob プロジェクト](storage/vs-storage-webjobs-what-happened/)を参照してください。
