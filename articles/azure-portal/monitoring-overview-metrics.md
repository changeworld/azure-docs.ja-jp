...
>[AZURE.NOTE] ホスト レベルのメトリックには、VM (Azure Resource Manager ベース) および VM スケール セットからアクセスできます。その際、追加の診断設定は不要です。この新しいホスト レベルのメトリックは、Windows と Linux のインスタンスで使用できます。VM や VMSS で Azure 診断をオンにしている場合にアクセスできる、ゲスト OS レベルのメトリックと混同しないでください。Azure 診断の構成の詳細については、「[Microsoft Azure 診断とは](../azure-diagnostics.md)」を参照してください。
...
- [自動スケールの一般的なメトリック](insights-autoscale-common-metrics.md)の概要
- [アラート ルールの作成](insights-alerts-portal.md)方法
