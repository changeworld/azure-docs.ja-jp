...
ポータルにより、リソース グループの現在の状態を表すテンプレートを生成するか、特定のデプロイに使用されたテンプレートを取得できます。このトピックでは、両方のオプションを紹介します。
...
リソース グループのブレードから、リソース グループの現在の状態を表すテンプレートをエクスポートできます。

リソース グループのテンプレートを表示するには、**[テンプレートのエクスポート]** を選択します。

![リソース グループのエクスポート](./media/resource-group-portal/export-resource-group.png)

リソース マネージャーにより 4 つのファイルが生成されます。

1. ソリューションのインフラストラクチャを定義するテンプレート

2. デプロイ中に値を渡すために使用できるパラメーター ファイル

3. テンプレートをデプロイするために実行できる Azure PowerShell スクリプト ファイル

4. テンプレートをデプロイするために実行できる Azure CLI スクリプト ファイル

最初に、現在のリソース グループを表すテンプレートを確認します。

![show template](./media/resource-group-portal/show-rg-template.png)

**[リソース]** セクション内に、デプロイするリソースの定義が表示されます。

パラメーター ファイルには、デプロイ中に渡すパラメーター値を保存できます。

![パラメーターの表示](./media/resource-group-portal/show-parameters.png)

Azure PowerShell でテンプレートをデプロイするためのスクリプト ファイルがあります。

![Azure PowerShell の表示](./media/resource-group-portal/show-powershell.png)

また、Azure CLI でテンプレートをデプロイするためのスクリプト ファイルがあります。

![Azure CLI の表示](./media/resource-group-portal/show-cli.png)

ポータルには、このテンプレートを使用するための 3 つのオプションがあります。テンプレートを今すぐ再デプロイするには、**[デプロイ]** を選択します。すべてのファイルをローカルでダウンロードするには、**[ダウンロード]** を選択します。ポータルを介して後で使用するために Azure アカウントにファイルを保存するには、**[テンプレートの保存]** を選択します。
...
履歴からデプロイを選択すると、そのデプロイの詳細が表示されます。リソースをデプロイするたびに、リソース マネージャーは、使用したテンプレートを保持します。デプロイに使用された実際のテンプレートを取得するには、**[テンプレートの表示]** を選択します。

![export template](./media/resource-group-portal/export-template.png)

このデプロイに使用されたテンプレートが表示されます。定義したすべてのパラメーターと変数が含まれます。

![show template](./media/resource-group-portal/show-template.png)

先に述べたように、これはリソース グループを完全に表していないことがあります。このデプロイ以外でリソースを追加または削除した場合、それらの操作はこのテンプレートに反映されません。前のセクションで示したように、テンプレート、パラメーター ファイル、スクリプト ファイルを表示できます。前のセクションで示したように、テンプレートを再デプロイ、ダウンロード、保存することもできます。
...
- 監査ログの表示については、「[リソース マネージャーの監査操作](../resource-group-audit.md)」を参照してください。
- デプロイ エラーのトラブルシューティングの詳細については、「[Troubleshooting resource group deployments with Azure Portal (Azure ポータルでのリソース グループのデプロイのトラブルシューティング)](../resource-manager-troubleshoot-deployments-portal.md)」を参照してください。
