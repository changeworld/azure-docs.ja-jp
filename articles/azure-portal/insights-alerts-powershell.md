---
title: PowerShell での Azure サービス アラートの作成 | Microsoft Docs
description: PowerShell を使用して Azure アラートを作成します。このアラートにより、指定した条件が満たされたときに通知やオートメーションをトリガーできます。
author: rboucher
manager: ''
editor: ''
...
---
# PowerShell での Azure サービス アラートの作成
> [!div class="op_single_selector"]
> * [ポータル](../monitoring-and-diagnostics/insights-alerts-portal.md)
...
4. ルールを作成するには、最初に重要な情報をいくつか入手する必要があります。
   ...
   * そのリソースが使用できる**メトリック定義**
     ...
     リソース ID は、Azure ポータルを使用して取得できます。リソースが既に作成されている場合は、ポータルでそのリソースを選択し、次のブレードで "*設定*" セクションの "*プロパティ*" を選択します。次にブレードに "リソース ID" フィールドがあります。また、[Azure リソース エクスプローラー](https://resources.azure.com/)を使用することもできます。
...
## 次のステップ
* 収集および監視できる情報の種類など、[Azure での監視の概要](../monitoring-and-diagnostics/monitoring-overview.md)情報を入手します。
...
* [診断ログ収集の概要](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md)情報を入手して、サービスに関する詳細な頻度の高いメトリックを収集します。
