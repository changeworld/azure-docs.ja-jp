...
Azure リソース ID、[Azure リソースの名前付け規則](/azure/architecture/best-practices/resource-naming)に従います。 ポータルは、ダッシュボードを作成する場合、一般には GUID 形式の ID を選択しますが、プログラムでダッシュボードを作成する場合は任意の有効な名前を使用できます。 
