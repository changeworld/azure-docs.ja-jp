...
これを行うには、エンドポイントの作成に使用する `az dt endpoint create` コマンドに `--auth-type` パラメーターを追加します。 (このコマンドの詳細については、その[リファレンス ドキュメント](/cli/azure/dt/endpoint/create?view=azure-cli-latest&preserve-view=true)または [Azure Digital Twins エンドポイントを設定するための一般的な手順](how-to-manage-routes-apis-cli.md#create-the-endpoint)に関する記事を参照してください)。
