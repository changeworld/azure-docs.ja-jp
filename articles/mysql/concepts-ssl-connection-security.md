...
たとえば、[Minimum TLS setting version]\(TLS の最小設定バージョン\) の値を TLS 1.0 に設定した場合、サーバーは TLS 1.0、1.1、および 1.2+ を使用しているクライアントからの接続を許可することになります。 または、これを 1.2 に設定すると、TLS 1.2 以上を使用するクライアントからの接続だけを許可し、TLS 1.0 と TLS 1.1 を使用する接続はすべて拒否することを意味します。

> [!Note] 
> Azure Database for MySQL では既定で、すべての新しいサーバーに対して TLS が無効になっています。
>
> 現在、Azure Database for MySQL でサポートされている TLS バージョンは、TLS 1.0、1.1、1.2 です。 特定の TLS の最小バージョンに適用されると、無効に変更することはできません。
