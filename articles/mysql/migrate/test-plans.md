...
WWI は、週末に移行を行い、太平洋標準時の午後 10 時から午前 2 時までの時間枠を特定しました。 午前 2 時の目標 (4 時間のダウンタイム目標) までにすべてのテストに成功して移行が完了しなかった場合は、ロールバック手順が開始されました。 問題は今後の移行のために文書化されました。 すべての移行時間枠が前倒しされ、許容されるビジネス タイムラインに基づいて再スケジュールされました。
...
次のクエリを使用すると、すべてのテーブルと推定行数を取得できます。
...
> [!NOTE]
> `INFORMATION_SCHEMA` テーブルでは、テーブル全体の推定値セットが提供されます。 テーブルのサイズなどのメトリックをより正確に表示するには、`innodb_stats_transient_sample_pages` サーバー パラメーターを使用してページのサンプル サイズを増やします。 このサーバーの値を大きくすると、より多くのページが分析され、より正確な結果が得られます。

行の正確な数を取得するには、すべてのテーブルに対して `count(*)` SQL ステートメントを実行します。 このコマンドを実行すると、大きなテーブルでは時間がかかることがあります。 次のスクリプトでは、正確な数を取得するために実行できる SQL ステートメントのセットが生成されます。
...
データ テーブルは、継続的なアプリケーションの使用によって拡大し続ける可能性があります。 場合によっては、データが大きくなることはなくとも、削除や更新によって絶えず変更されることがあります。 その場合は、データベース ファイルに大量の断片化が発生する可能性があります。 MySQL OPTIMIZE TABLE ステートメントを使用すると、物理ストレージ領域のニーズを軽減し、I/O 効率を向上させることができます。
...
> [パフォーマンス ベースライン](./performance-baselines.md)