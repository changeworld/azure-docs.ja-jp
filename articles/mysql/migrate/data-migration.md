---
title: オンプレミスの MySQL から Azure Database for MySQL への移行ガイドのデータ移行
...
ms.locfileid: "110656984"
---
# <a name="mysql-on-premises-to-azure-database-for-mysql-migration-guide-data-migration"></a>オンプレミスの MySQL から Azure Database for MySQL への移行ガイドのデータ移行
...
### <a name="performing-the-migration"></a>移行を実行する
...
どのようなパスを使用するかにかかわらず、次のような一般的な手順を実行する必要があります。
  ...
  - サポートされている Azure MySQL バージョンへのアップグレード

  - データベース オブジェクトのインベントリ

  - ユーザーとアクセス許可のエクスポート

### <a name="migrate-to-latest-mysql-version"></a>最新の MySQL バージョンに移行する

WWI の会議データベースでは 5.5 を実行しているので、アップグレードを実行する必要があります。 CIO は、最新バージョンの MySQL (現在は 8.0) にアップグレードするように求められています。
...
アップグレード チェッカーで問題が報告されない場合は、MySQL バイナリを置き換え、インプレース アップグレードを実行しても問題はありません。 問題がある場合は、データベースをエクスポートして問題に対処する必要があります。
...
```
  ...
  - **yourusername** を root に設定し、**yourpassword** をルート ユーザーのパスワードに設定します。
...
> [MySQL Workbench を使用したデータ移行](./data-migration-with-mySQL-workbench.md)