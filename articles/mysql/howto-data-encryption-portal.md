...
```
* Azure Database for MySQL - 単一サーバーは、汎用またはメモリ最適化の価格レベルと、汎用ストレージ v2 である必要があります。 処理を進める前に、「[カスタマー マネージド キーを使用したデータ暗号化](concepts-data-encryption-mysql.md#limitations)」の制限事項を参照してください。
