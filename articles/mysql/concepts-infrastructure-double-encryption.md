...
1. **多様性が追加された暗号化実装** - ハードウェア ベースの暗号化への計画的な移行により、ソフトウェア ベースの実装に加えて、ハードウェア ベースの実装が提供されるため、実装がさらに多様化します。
...
> [!Important]
> - シナリオ 2 と 4 は、追加のインフラストラクチャ暗号化レイヤーによって Azure Database for MySQL サーバーのパフォーマンスに大きな影響があります。
