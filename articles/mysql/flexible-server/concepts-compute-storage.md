...
| コンピューティング サイズ         | 仮想コア | メモリ サイズ (GiB) | サポートされる最大 IOPS | サポートされる最大 I/O 帯域幅 (MBps)| 最大接続数
|----------------------|--------|-------------------| ------------------ |-----------------------------------|------------------
| **バースト可能**        |        |                   | 
...
プロビジョニングするストレージは、フレキシブル サーバーで使用可能なストレージ容量です。 ストレージは、データベース ファイル、一時ファイル、トランザクション ログ、および MySQL サーバー ログに使用されます。 すべてのコンピューティング レベルで、サポートされる最小ストレージは 20 GiB で、最大ストレージは 16 TiB です。 ストレージは 1 GiB 単位でスケーリングされ、サーバーの作成後にスケールアップすることができます。
