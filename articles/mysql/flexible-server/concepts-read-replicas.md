...
レプリカは、ソース Azure Database for MySQL フレキシブル サーバーと同じようにユーザーが管理する新しいサーバーです。 読み取りレプリカごとに、仮想コアでのプロビジョニング済みコンピューティングとストレージ (GB/月) に基づいて課金されます。 詳細については、「[価格](./concepts-compute-storage.md#pricing)」を参照してください。
