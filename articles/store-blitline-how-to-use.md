<properties linkid="develop-net-how-to-guides-blitline-image-processing-service" urlDisplayName="Blitline 画像処理サービス" pageTitle="Blitline を使用した画像処理の方法 - の機能ガイド " metaKeywords="" description="Azure アプリケーション内で Blitline サービスを使用して画像を処理する方法について説明します。" metaCanonical="" services="" documentationCenter=".NET" title="Azure と Azure ストレージで Blitline を使用する方法" authors="pennij" solutions="" manager="" editor="" />
...
画像処理は何度にもわたって繰り返されている作業であり、通常は Web サイトごとに最初から作り直されます。私たちもこの作業を何度も行ってきたので、状況は理解しています。あるとき私たちは、ユーザーに代わってこの処理を行うサービスを提供したらどうかと考えました。私たちは、この作業をすばやく効率的に行う方法を把握しており、ユーザーの負担を軽減することができます。
...
- Blitline には、Aviary.com のようなライブ画像処理機能はありません。
...
- Blitline は並列性がきわめて高いため、同期処理を行いません。したがって、処理の完了時に通知を受け取ることができるように、Blitline に postback_url を伝える必要があります。
...
ここでサポートされる*機能*の詳細については、<http://http://www.blitline.com/docs/functions> を参照してください。
