...
* **Vulnerability to SQL Injection (SQL インジェクションの脆弱性)**: このアラートは、データベースでアプリケーション エラーが検出された場合にトリガーされます。 このアラートは、SQL インジェクション攻撃に対する脆弱性が存在する可能性を示すものです。
* **Unusual access from unfamiliar location (未知の場所からの通常とは異なるアクセス)**: このアラートは、最後の期間に見られなかった不明な IP アドレスからのアクセス イベントがサーバーで検出された場合にトリガーされます。
