...
### <a name="code-injection-discovered"></a>コード インジェクションの検出
コード インジェクションとは、実行中のプロセスやスレッドに実行可能なモジュールを挿入することです。  この手法は、マルウェアがデータにアクセスし、その削除を隠したり防いだりするために使用されます (たとえば永続化)。 このアラートは、挿入されたモジュールがクラッシュ ダンプに存在することを示します。 正当なソフトウェア開発者は、悪意からではなく、既存のアプリケーションやオペレーティング システム コンポーネントの変更や拡張などの目的で、コード インジェクションを実行することもあります。  悪意のある挿入モジュールと悪意のない挿入モジュールを区別できるように、Security Center は、挿入されたモジュールが疑わしい動作のプロファイルに準拠しているかどうかをチェックします。 このチェックの結果は、警告の "SIGNATURE (シグネチャ)" フィールドに表示され、警告の重大度、説明、修正手順に反映されます。 

このアラートには、他にも次のフィールドがあります。

- ADDRESS (アドレス): メモリ内における挿入されたモジュールの場所。
- IMAGENAME (イメージ名): 挿入されたモジュールの名前。 イメージ名がイメージ内で指定されていない場合は、空白にすることができます。
- SIGNATURE (シグネチャ): 挿入されたモジュールが、疑わしい動作の特徴と一致しているかどうかを示します。 

次の表は、結果の例と説明を示しています。

| シグネチャの値                      | 説明                                                                                                       |
|--------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| 不審な反射ローダーのエクスプロイト | この疑わしい動作は、多くの場合、オペレーティング システム ローダーとは関係なく、挿入されたコードの読み込みに関連しています。 |
| 不審な挿入されたエクスプロイト          | 多くの場合、メモリへのコード挿入に関連する悪意のある行為を示します。                                       |
| 不審な挿入するエクスプロイト         | 多くの場合、メモリに挿入されたコードの使用に関連する悪意のある行為を示します。                                   |
| 不審な挿入されたデバッガー エクスプロイト | 多くの場合、デバッガーの検出または回避に関連する悪意のある行為を示します。                         |
| 不審な挿入されたリモート エクスプロイト   | 多くの場合、コマンド n 制御 (C2) シナリオに関連する悪意のある行為を示します。                                 |

この種類のアラートの例を次に示します。

![コード インジェクションのアラート](./media/security-center-alerts-type/security-center-alerts-type-fig21.png)

### <a name="suspicious-code-segment"></a>疑わしいコード セグメント
疑わしいコード セグメントは、反射型インジェクションやプロセス ハロウイングなど、標準以外の方法を使用してコード セグメントが割り当てられたことを示します。  さらに、このアラートは、コード セグメントの他の特性を処理して、報告されるコード セグメントの能力および動作に関するコンテキストを提供します。

このアラートには、他にも次のフィールドがあります。

- ADDRESS (アドレス): メモリ内における挿入されたモジュールの場所。
- SIZE (サイズ): 疑わしいコード セグメントのサイズ。
- STRINGSIGNATURES (文字列署名): このフィールドは、関数名がコード セグメント内に含まれている API の機能を一覧表示します。 たとえば、次のような機能です。
    - イメージ セクション記述子、x64 の動的コード実行、メモリ割り当てとローダー機能、リモート コード インジェクション機能、ハイジャック制御機能、環境変数の読み取り、任意のプロセス メモリの読み取り、トークン特権の照会または変更、HTTP/HTTPS ネットワーク通信、ネットワーク ソケット通信。
- IMAGEDETECTED (イメージ検出): このフィールドは、疑わしいコード セグメントが検出されたプロセスに PE イメージが挿入されたかどうかと、挿入されたモジュールがどのアドレスから開始されているかを示します。
- SHELLCODE (シェル コード): このフィールドは、セキュリティ上重要なオペレーティング システム機能にアクセスするために悪意のあるペイロードによって一般的に使用される動作の存在を示します。 

この種類のアラートの例を次に示します。

![疑わしいコード セグメント アラート](./media/security-center-alerts-type/security-center-alerts-type-fig22.png)
