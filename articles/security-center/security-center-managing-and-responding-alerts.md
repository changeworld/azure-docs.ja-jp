...
> [!NOTE]
> **2015-06-01 プレビュー** API バージョンを使用している場合は、上記に一覧したものの中から、どのアラーム重大度の種類をどのシナリオに適用するかにおいて違いがあります。  
...
**[説明]** フィールドで、このイベントに関する詳細な情報を確認できます。 詳しい情報を確認することで、セキュリティの警告を引き起こした原因、ターゲット リソース、(場合によっては) 発生元の IP アドレス、解決のための推奨事項を知ることができます。  Windows セキュリティ イベント ログによっては IP アドレスが含まれていないため、発生元の IP アドレスは空 (利用不可) である場合もあります。

Security Center から提案される修復方法は、セキュリティ アラートによって異なります。 場合によっては、他の Azure 機能を使用して推奨される修復方法を実行する必要があります。 たとえば、この攻撃に対する修復として、[ネットワーク ACL](../virtual-network/virtual-networks-acl.md) または[ネットワーク セキュリティ グループ](../virtual-network/security-overview.md#security-rules)規則を使用して、この攻撃を生成している IP アドレスを許可しないことが挙げられます。 さまざまなアラートの種類の詳細については、[セキュリティ アラートの種類](security-center-alerts-overview.md#security-alert-types)に関するページを参照してください。
