...
4. **[セキュリティ ポリシー]** ブレードで、**[領域ごとにストレージ アカウントを選ぶ]** を選択します。 実行している VM がある各リージョンに対し、これらの VM から収集されたデータが格納されているストレージ アカウントを選択します。 ストレージ アカウントを選択しないと、リージョンごとに自動的に作成されます。 収集されたデータは、セキュリティ上の理由で他の顧客データから論理的に分離されます。
