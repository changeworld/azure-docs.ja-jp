...
VM に対する Free レベルのセキュリティ評価のほとんどは、Standard レベルのセキュリティ アラートの多くと同様、Microsoft Monitoring Agent (MMA) 機能のインストールが必要となります。 Security Center の [Auto Provision]\(自動プロビジョニング\) を有効にすることで、Azure VM のエージェントを自動的にデプロイすることができます。
