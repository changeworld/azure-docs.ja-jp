...
Security Center では、お客様のリソースの構成を継続的に評価して、セキュリティの問題と脆弱性を特定します。 これらの評価は、お客様のセキュリティ検疫の強化に向けた推奨事項として表示されます。 規制コンプライアンス ダッシュボードでは、一連のコンプライアンス標準とそれらのすべての要件を確認できます。ここでは、サポートされている要件が適切なセキュリティ評価に対応付けられています。 そのため、これらの評価の状態に基づき、標準と照らし合わせてお客様のコンプライアンス体制を確認することができます。
