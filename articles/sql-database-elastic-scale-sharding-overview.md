<properties 
	pageTitle="シャーディングの概要" 
	description="シャーディングを利用する理由は、データベース リソースのスケールを変更して可用性またはパフォーマンスを向上させることです。" 
