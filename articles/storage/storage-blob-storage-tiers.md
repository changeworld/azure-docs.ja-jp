<properties
	pageTitle="Azure Blob Storage: クール層とホット層 | Microsoft Azure"
	description="アクセス パターンに基づいてオブジェクト データを効率的なコストで格納するための Azure Blob Storage 層の概要です。"
	services="storage"
	documentationCenter=""
...
# Azure Blob Storage: クール層とホット層

## 概要

Azure Blob Storage は、大量の非構造化データをクラウドに保存するための、コスト効率の高いスケーラブルなソリューションです。Blob Storage には大量のストレージ容量と拡張性があり、お客様ごとにペタバイトのデータと数十億のオブジェクトを簡単に低コストで格納できます。詳細については、「[Microsoft Azure Storage の概要](storage-introduction.md)」をご覧ください。

今日では、クラウドに格納されるデータは指数関数的なペースで増加しており、拡大するストレージのニーズのコストを管理するうえで、アクセスの頻度や保有期間などの属性に基づいてデータを階層化することが重要になっています。お客様のフィードバックとご利用のパターンから判明したことは、クラウドに格納されるデータは有効期間を通じてどのように生成、処理、およびアクセスされるかという点で、まったく異なる場合があるということでした。有効期間を通じて活発にアクセスおよび修正されるデータもあれば、有効期間の初期に非常に頻繁にアクセスされ、古くなるにつれて急速にアクセスが減るデータもあります。また、クラウド上で始めからあまり利用されず、格納されてからはほとんどアクセスされないデータもあります。一般に、**ホット データ**として分類されるデータは、非常に頻繁にアクセスされ、高い持続性と可用性が必要になります。これに対して、**クール データ**は、あまり頻繁にアクセスされず、長期間維持されるデータです。クール データでは、可用性が少し低くても許容できますが、高持続性は必要で、アクセスにかかる時間とスループット特性もホット データと同程度である必要があります。クール データの場合、可用性の SLA が少し低く、アクセス コストが高めであっても、ストレージ コストが大幅に低ければ許容できます。

上記の各データ アクセス シナリオは、特定のアクセス パターン用に最適化されたストレージの分化された層の利点を利用しています。Azure Blob Storage は、現在、異なるアクセス パターンと料金モデルによって、データの分化されたストレージ層のニーズに対応しています。

## BLOB ストレージ アカウント

**BLOB ストレージ アカウント**とは、Azure Storage に BLOB (オブジェクト) として非構造化データを格納するための特殊なストレージ アカウントです。BLOB ストレージ アカウントでは、現在、アクセス層としてクールまたはホットを選択できます。アクセス頻度が低いクール データは、より低いストレージ コストで格納し、アクセス頻度が高いホット データは、より低いアクセス コストで格納することができます。BLOB ストレージ アカウントは、既存の汎用ストレージ アカウントと同様で、現在使用されているすべての優れた耐久性、可用性、スケーラビリティ、およびパフォーマンス機能を共有します。たとえば、ブロック BLOB と追加 BLOB の 100% の API 整合性などです。

BLOB ストレージ アカウントは、**アクセス層**属性を公開します。この属性を使用すると、アカウントに格納されているデータに応じてアクセス層を**ホット**または**クール**として指定できます。データの使用パターンが変化した場合は、いつでもこれらのアクセス層を切り替えることができます。

> [AZURE.NOTE] アクセス層を変更すると、追加料金が発生することがあります。詳細については、後の「[価格と課金](storage-blob-storage-tiers.md#pricing-and-billing)」を参照してください。

ホット アクセス層の使用シナリオの例には、次のようなものがあります。

- 活発に使用されたり、頻繁にアクセス (読み取りまたは書き込み) されると予想されるデータ。
- 処理段階にあり、最終的にはクール層に移行されるデータ。

クール アクセス層の使用シナリオの例には、次のようなものがあります。

- バックアップ、アーカイブ、および障害復旧のデータセット。
- 既に頻繁に参照されなくなっているものの、アクセスされたときにはすぐに利用できることが期待されている、古いメディア コンテンツ。
- 今後処理するためにさらにデータが収集されている場合に、コスト効率よく格納する必要がある、大規模なデータ セット (たとえば、長期保存する科学データや、製造施設からの未加工のテレメトリ データ)。
- 最終的に使用可能な形式に処理した後も保持する必要がある、元の (生の) データ (たとえば、他の形式にコード変換した後の生のメディア ファイル)。
...
- ページ BLOB を使用する必要がある。一般に、ページ BLOB が必要な特別な理由がなければ、ブロック BLOB を使用することをお勧めします。
...
> [AZURE.NOTE] BLOB ストレージ アカウントは、現在、ほとんどの Azure リージョンでサポートされており、残りのリージョンでも順次サポートされていきます。利用できるリージョンの更新された一覧については、「[Azure のリージョン](https://azure.microsoft.com/regions/#services)」ページを参照してください。
...
<tr>
	...
	<td><strong><center>ホット アクセス層</center></strong></td>
	<td><strong><center>クール アクセス層</center></strong>&lt;/td
</tr>
<tr>
...
<tr>
	<td><strong><center>パフォーマンスと拡張性<center></strong></td>
...
- **ストレージ コスト**: データの格納のコストは、格納されているデータの量だけでなく、アクセス層によって異なります。ギガバイトあたりのコストは、クール アクセス層の方がホット アクセス層よりも低くなります。
- **データ アクセス コスト**: クール アクセス層のデータの場合、読み取りと書き込みに対して、ギガバイトあたりのデータ アクセス料金を課金されます。
- **トランザクション コスト**: どちらの層も、トランザクションごとの料金が発生します。ただし、クール アクセス層のトランザクションあたりのコストは、ホット アクセス層の場合よりも高くなります。
...
> [AZURE.NOTE] ユーザーが新しいストレージ層を試し、サービス開始後の機能を検証できるように、クールからホットへのアクセス層の変更に対する課金は 2016 年 6 月 30 日までは免除されます。2016 年 7 月 1 日からは、クールからホットへのすべての切り替えに課金が適用されます。BLOB ストレージ アカウントの料金モデルの詳細については、「[Azure Storage の料金](https://azure.microsoft.com/pricing/details/storage/)」ページを参照してください。送信データ転送料金の詳細については、「[Data Transfers (データ転送) の料金詳細](https://azure.microsoft.com/pricing/details/data-transfers/)」を参照してください。
...
4. デプロイ モデルとして **[リソース マネージャー]** を選択します。
...
9. 新しいリソース グループを指定するか、既定のリソース グループを選択します。リソース グループの詳細については、「[Azure ポータルを使用した Azure リソースのデプロイと管理](../azure-portal/resource-group-portal.md)」を参照してください。
...
このセクションの目的は、ユーザーが BLOB ストレージ アカウントにスムーズに移行できるように手助けすることです。BLOB ストレージ アカウントは、BLOB のみを格納するように特化されています。テーブル、キュー、およびファイルも格納することができる既存の汎用ストレージ アカウントは、BLOB ストレージ アカウントに変換することはできません。つまり、BLOB ストレージ アカウントで提供される機能を使用するには、適切なアクセス層の新しい BLOB ストレージ アカウントを作成し、既存のデータを新たに作成されたアカウントに移行する必要があるということです。

### 既存のデータの移行の計画

データを BLOB ストレージ アカウントに移動している場合は、クール アクセス層の利点を利用して、使用頻度の低いデータに対するストレージ コストを節約したいでしょう。クール アクセス層の BLOB ストレージ アカウントでの、データの移行計画の最初の手順は、現在の使用パターンを評価して、BLOB ストレージ アカウントへの移行によって利益が得られるかどうかを判断することです。一般に、以下のことを調べる必要があります。
...
既存のストレージ アカウントを監視し、このデータを収集するには、「[Azure のストレージ メトリックの有効化とメトリック データの表示](storage-enable-and-view-metrics.md)」を参照してください。次にこのデータを使用して、[料金計算ツール](https://azure.microsoft.com/pricing/calculator/?scenario=data-management)でコストを見積もります。

### 既存のデータの移行

オンプレミス ストレージ ソリューション、サード パーティのクラウド ストレージ プロバイダー、または Azure の既存の汎用ストレージ アカウントから、既存のデータを BLOB ストレージ アカウントに移行するには、以下の方法を使用できます。

#### AzCopy

AzCopy は、Azure Storage との間で高パフォーマンスのデータ コピーを行うように設計された Windows コマンドライン ユーティリティです。AzCopy を使用して、既存の汎用ストレージ アカウントから BLOB ストレージ アカウントにデータをコピーできます。またオンプレミス ストレージ システムから BLOB ストレージ アカウントにデータをアップロードすることもできます。
...
4.	**同じアカウントの両方のアクセス層にオブジェクトを格納することはできますか。**
    ...
    現時点ではありません。アクセス層属性はアカウント レベルで設定され、そのアカウント内のすべてのオブジェクトに適用されます。
...
7.	**BLOB ストレージ アカウントを使用するには、既存のアプリケーションを変更する必要がありますか。**

    BLOB ストレージ アカウントは、[Storage Services REST API](https://msdn.microsoft.com/library/azure/dd894041.aspx) の最新バージョン (2014-02-14) を使用していれば、汎用ストレージ アカウントと 100% の API 整合性があります。プロトコルの古いバージョンを使用している場合は、新しいバージョンを使用して両方の種類のストレージ アカウントとシームレスに動作するように、アプリケーションを更新する必要があります。一般に、どの種類のストレージ アカウントを使用するかにかかわらず、常に最新バージョンを使用することをお勧めしています。

8.	**ユーザー エクスペリエンスに変更はありますか。**

    BLOB ストレージ アカウントは BLOB だけを格納しますが、その他の点では汎用ストレージ アカウントと非常によく似ていて、高い持続性、可用性、拡張性、パフォーマンス、セキュリティなどの Azure Storage のすべての主要機能を継承しています。BLOB ストレージ アカウントに固有の機能および制限と、上に記載したアクセス層を除き、他のすべては同じままです。
