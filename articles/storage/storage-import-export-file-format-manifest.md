...
|`BlobList/PropertiesPath`|String|省略可能。 インポート操作の BLOB の一覧で、BLOB に設定される既定のプロパティを含むディスク上のファイルの相対パスを指定します。 このプロパティは、必要に応じて BLOB ごとに上書きできます。<br /><br /> この要素は、エクスポート操作ではマニフェストから省略されます。|  
