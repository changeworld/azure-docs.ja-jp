...
### <a name="upload-a-file-with-index-tags"></a>インデックス タグを使用してファイルをアップロードする
...
> * インデックス タグを持つ別のストレージ アカウントに BLOB をコピーする
...
> [!NOTE]
> ソース BLOB にインデックス タグがあり、それらのタグを保持する必要がある場合は、コピー先 BLOB に再適用する必要があります。 インデックス タグを設定する方法の詳細については、以下の **インデックス タグがある BLOB を別のストレージ アカウントにコピーする** 方法に関するセクションを参照してください。 

<a id="copy-between-accounts-and-add-index-tags"></a>

### <a name="copy-blobs-to-another-storage-account-with-index-tags"></a>インデックス タグを持つ別のストレージ アカウントに BLOB をコピーする

BLOB を別のストレージ アカウントにコピーし、[BLOB インデックス タグ (プレビュー)](../blobs/storage-manage-find-blobs.md) を対象の BLOB に追加できます。
