...
description: 管理プレーンのセキュリティ、承認、ネットワーク セキュリティ、暗号化など、Azure Storage アカウントをセキュリティで保護する方法について詳しく説明します。
services: storage
author: tamram
ms.service: storage
ms.topic: article
...
Azure Storage には包括的な一連のセキュリティ機能が用意されており、組織はそれを利用してセキュリティで保護されたアプリケーションを構築およびデプロイできます。

- Azure Storage に書き込まれるすべてのデータ (メタデータを含む) は、[Storage Service Encryption (SSE)](storage-service-encryption.md) を使用して自動的に暗号化されます。 詳細については、[Azure Blob、Files、Table、Queue Storage 用の既定の暗号化の発表](https://azure.microsoft.com/blog/announcing-default-encryption-for-azure-blobs-files-table-and-queue-storage/)に関するブログ記事を参照してください。
- Azure Active Directory (Azure AD) とロールベースのアクセス制御 (RBAC) は、リソース管理操作とデータ プレーン操作の両方でサポートされます。   
    - ストレージ アカウントを対象とする RBAC ロールをセキュリティ プリンシパルに割り当て、Azure AD を使用して、キー管理などのリソース管理操作を承認できます。
    - Azure AD の統合は、BLOB およびキュー データ操作でサポートされています。 RBAC ロールのスコープは、サブスクリプション、リソース グループ、ストレージ アカウント、個々のコンテナーまたはキューに設定できます。 ロールは、Azure リソースのセキュリティ プリンシパルまたはマネージド ID に割り当てることができます。 詳細については、「[Azure Active Directory を使用して Azure Storage へのアクセスを認証する](storage-auth-aad.md)」を参照してください。
- アプリケーションと Azure の間で送信されるデータを、[クライアント側暗号化](../storage-client-side-encryption.md)、HTTPS、または SMB 3.0 使用して保護できます。  
- Azure 仮想マシンに使用する OS とデータ ディスクは、[Azure Disk Encryption](../../security/fundamentals/encryption-overview.md) を使用して暗号化できます。
...
- ストレージ ファイアウォール、サービス エンドポイント、またはプライベート エンドポイントを使用して、アプリケーション コンポーネントとストレージ間のネットワーク層セキュリティを有効にすることができます。

この記事では、Azure Storage で使用できる各セキュリティ機能の概要について説明します。 各機能の詳細については、記事のリンクを参照してください。

この記事で取り上げている領域は次のとおりです。

* [管理プレーンのセキュリティ](#management-plane-security) - ストレージ アカウントに対するリソースレベルのアクセスのセキュリティ保護

  管理プレーンは、ストレージ アカウントの管理に使用する操作が構成されます。 このセクションでは、Azure Resource Manager デプロイ モデルと、ロールベースのアクセス制御 (RBAC) を使用してストレージ アカウントへのアクセスを制御する方法について説明します。 また、ストレージ アカウント キーの管理とその再生成方法についても説明します。

* [ネットワーク セキュリティ](#network-security) - ストレージ アカウントに対するネットワークレベルのアクセスのセキュリティ保護

  このセクションでは、ストレージ サービス エンドポイントへのネットワークレベルのアクセスをセキュリティで保護する方法について説明します。 ストレージ ファイアウォールを使用して、特定の仮想ネットワークまたは IP アドレス範囲からデータへのアクセスを許可する方法について説明します。 また、ストレージ アカウントでサービス エンドポイントとプライベート エンドポイントを使用する方法についても説明します。

* [承認](#authorization) - データに対するアクセスの承認

  このセクションでは、共有アクセス署名と保存されているアクセス ポリシーを使用したストレージ アカウントのデータ オブジェクト (BLOB、ファイル、キュー、テーブルなど) へのアクセスについて説明します。 サービスレベルの SAS とアカウントレベルの SAS の両方が対象です。 また、特定の IP アドレス (または IP アドレスの範囲) に対するアクセスを制限する方法、HTTPS に使用されるプロトコルを制限する方法、Shared Access Signature が期限切れになる前に無効にする方法についても説明します。

* [転送中の暗号化](#encryption-in-transit)

  このセクションでは、Azure Storage とのデータの送受信時にデータをセキュリティで保護する方法について説明します。 Azure ファイル共有に対して SMB 3.0 による暗号化と HTTPS の推奨される使用方法について説明します。 また、クライアント側の暗号化についても説明します。これを使うと、ストレージに転送する前にデータを暗号化し、ストレージから転送した後にデータを復号化できます。
...
ストレージ アカウント キーは、さまざまな理由から再生成します。

* セキュリティのために定期的に再生成することができます。
* アプリケーションまたはネットワークのセキュリティが侵害された場合にストレージ アカウント キーを再生成することがあります。
* キーの再生成のもう 1 つの例は、キーにアクセスできるチーム メンバーが離脱する場合です。 Shared Access Signature は、主にこのシナリオに対処するために設計されました。ほとんどの個人またはアプリケーションとアクセス キーを共有するのではなく、アカウントレベルの SAS 接続文字列またはトークンを共有するようにしてください。

#### <a name="key-regeneration-plan"></a>キーの再生成計画
使用中のアクセス キーは、無計画に再生成しないでください。 キーの再生成が突然行われると、既存のアプリケーションのストレージ アカウントへのアクセスがブロックされ、大規模な中断が発生する可能性があります。 Azure Storage アカウントには 2 つのキーが用意されているので、キーを一度に 1 つずつ再生成できます。

キーを再生成する前に、ストレージ アカウントに依存するすべてのアプリケーションと、Azure で使用しているその他のサービスの一覧を取得しておきます。 たとえば、ストレージ アカウントを使用する Azure Media Services を使用している場合は、アクセス キーを再生成した後で、そのキーをメディア サービスと再同期する必要があります。 ストレージ エクスプローラーなどのアプリケーションを使用している場合、そのアプリケーションにも新しいキーを提供する必要があります。 VHD ファイルがストレージ アカウントに格納されている VM がある場合、ストレージ アカウント キーの再生成によって影響を受けることはありません。
...
また、ストレージ アカウント キーをシークレットとして [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) に格納し、アプリケーションでそこからキーを取得する方法もあります。 次にキーを再生成し、Azure Key Vault を更新するときに、アプリケーションを再デプロイする必要はありません。Azure Key Vault から新しいキーが自動的に選択されます。 アプリケーションで必要になるたびにキーを読み取ることができます。また、アプリケーションを使ってキーをメモリにキャッシュし、使用時にエラーが発生した場合は、Azure Key Vault からキーを取得し直すこともできます。

Azure Key Vault を使用すると、ストレージ キーのセキュリティ レベルも 1 つ追加されます。 Key Vault を使用すると、アプリケーション構成ファイルにストレージ キーを記述しなくても済むようになります。 また、これらの構成ファイルにアクセスできるすべてのユーザーにキーが公開されるのを防ぐことができます。

Azure Key Vault には、Azure AD を使用してキーへのアクセスを制御できる利点もあります。 Key Vault からキーを取得する必要がある特定のアプリケーションへのアクセス権を付与できます。キーへのアクセス権が不要な他のアプリケーションには公開されません。
...
ネットワークのセキュリティを使うと、選択したネットワークから Azure Storage アカウントのデータへのアクセスを制限できます。 Azure Storage ファイアウォールを使うと、特定のパブリック IP アドレス範囲からクライアントへのアクセスを制限すること、Azure 上の仮想ネットワーク (VNet) を選択すること、特定の Azure リソースにアクセスすることができます。 また、アクセスを必要とする VNet のストレージ アカウントにプライベート エンドポイントを作成し、パブリック エンドポイントを介したすべてのアクセスをブロックするオプションもあります。

Azure portal の [[ファイアウォールと仮想ネットワーク]](storage-network-security.md) タブを使用して、ストレージ アカウントのネットワーク アクセス規則を構成できます。 ストレージ ファイアウォールを使用して、パブリック インターネット トラフィックへのアクセスを拒否し、構成されたネットワーク規則に基づいてクライアントを選択するアクセス権を付与することができます。

また、[[プライベート エンドポイント]](../../private-link/private-endpoint-overview.md) を使うと、[プライベート リンク](../../private-link/private-link-overview.md)を使用して VNet からストレージ アカウントにプライベートかつ安全に接続することもできます。

ストレージ ファイアウォール規則は、ストレージ アカウントのパブリック エンドポイントにのみ適用されます。 ストレージ アカウントのプライベート エンドポイントをホストするサブネットは、そのプライベート エンドポイントの作成を承認すると、アカウントへの暗黙的なアクセス権を獲得します。

> [!NOTE]
> ストレージ ファイアウォール規則は、Azure portal および Azure Storage Management API を介して実行されるストレージ管理操作には適用されません。

### <a name="access-rules-for-public-ip-address-ranges"></a>パブリック IP アドレス範囲のアクセス規則
Azure Storage ファイアウォールを使うと、特定のパブリック IP アドレス範囲からストレージ アカウントへのアクセスを制限することができます。 IP アドレス規則を使うと、固定パブリック IP エンドポイント上で通信される特定のインターネットベースのサービスへのアクセスを制限することや、オンプレミスのネットワークを選択することができます。

### <a name="access-rules-for-azure-virtual-networks"></a>Azure 仮想ネットワークのアクセス規則
既定では、ストレージ アカウントは、任意のネットワーク上のクライアントからの接続を受け入れます。 ストレージ ファイアウォールを使用して、ストレージ アカウント内のデータへのクライアント アクセスを、選択したネットワークに制限することができます。 [サービス エンドポイント](../../virtual-network/virtual-network-service-endpoints-overview.md)を使うと、Azure 仮想ネットワークからストレージ アカウントへのトラフィックのルーティングが可能になります。 

### <a name="granting-access-to-specific-trusted-resource-instances"></a>特定の信頼されたリソース インスタンスへのアクセス権の付与
サービス リソースの種類またはリソース インスタンスに基づく強力な認証を使用して、[Azure の信頼されたサービスの一部](storage-network-security.md#trusted-microsoft-services)に、ファイアウォールを介してストレージ アカウントにアクセスすることを許可できます。

ストレージ ファイアウォールを介したリソース インスタンスベースのアクセスをサポートするサービスの場合、選択したインスタンスのみがストレージ アカウントのデータにアクセスできます。 この場合、サービスは、システムによって割り当てられた[マネージド ID](../../active-directory/managed-identities-azure-resources/overview.md) を使用したリソースインスタンス認証をサポートする必要があります。

### <a name="using-private-endpoints-for-securing-connections"></a>接続をセキュリティで保護するためのプライベート エンドポイントの使用
Azure Storage はプライベート エンドポイントをサポートします。これを使うと、Azure 仮想ネットワークからストレージ アカウントへの安全なアクセスを可能になります。 プライベート エンドポイントによって、VNet のアドレス空間からストレージ サービスにプライベート IP アドレスが割り当てられます。 プライベート エンドポイントを使用する場合、ストレージ接続文字列によって、ストレージ アカウント宛てのトラフィックはプライベート IP アドレスにリダイレクトされます。 プライベート エンドポイントとストレージ アカウント間の接続には、プライベート リンクが使用されます。 プライベート エンドポイントを使用すると、VNet からのデータの流出をブロックできます。

また、VPN または [ExpressRoutes](../../expressroute/expressroute-locations.md) プライベート ピアリングおよび他のピアリングされた仮想ネットワーク経由で接続されたオンプレミス ネットワークは、プライベート エンドポイントを介してストレージ アカウントにアクセスすることもできます。 ストレージ アカウントのプライベート エンドポイントを任意のリージョンの VNet に作成し、セキュリティで保護されたグローバル リーチを実現することができます。 また、他の [Azure Active Directory](../../active-directory/fundamentals/active-directory-whatis.md) テナントにストレージ アカウントのプライベート エンドポイントを作成することもできます。
...
#### <a name="resources"></a>リソース
* [Windows および Linux IaaS VM の Azure ディスク暗号化](../../security/fundamentals/encryption-overview.md)
