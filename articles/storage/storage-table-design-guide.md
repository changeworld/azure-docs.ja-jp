...
-	2 番目に良い方法は、 **PartitionKey** を使用する***範囲クエリ***と、**RowKey** 値 の範囲にフィルターをかけ、1 つ以上のエンティティを返す Range Query です。 **PartitionKey** 値は特定のパーティションを識別し、**RowKey** 値はそのパーティション内のエンティティのサブセットを識別します。例: $filter=PartitionKey eq 'Sales' および RowKey ge 'S' および RowKey lt 'T'  
...
#### 関連のあるパターンとガイダンス  
このパターンを実行する場合、以下のパターンとガイダンスも関連する可能性があります：- [エンティティ グループ トランザクション](#entity-group-transactions) - [マージまたは置換](#merge-or-replace)
...
次の手順は、2 番目の方法を使用した場合に、新しい従業員を追加するときに従う必要がある手順の概要を示しています。この例では、Sales 部署で ID が 000152、姓が Jones、従業員を 1 人追加します: 1。**PartitionKey** 値 "Sales" と **RowKey** 値 "Jones"　を持つインデックスのエンティティを取得します。 手順 2.2 で使用するこのエンティティの ETag を保存します。新しい従業員エンティティを挿入するエンティティ グループ トランザクション、つまり、バッチ操作(**PartitionKey** 値 "Sales" と **RowKey** 値 "000152") を作成し、新しい従業員 ID を従業員 ID フィールドに追加することでインデックス エンティティ (**PartitionKey** 値 "Sales" と **RowKey** 値 "Jones") を更新します。エンティティ グループ トランザクションの詳細については、「[エンティティ グループ トランザクション](#entity-group-transactions)」をご覧ください。3.オプティミスティック同時実行エラー (他のユーザーがインデックス エンティティを変更したこと) が原因でエンティティ グループ トランザクションが失敗した場合は、手順 1. からまたやり直す必要があります。
...
このパターンを実装する方法を決定するときに、次の点に注意してください:-このソリューションに対応するエンティティを取得するには少なくとも 2 つのクエリが必要です: **RowKey** 値のリストを入手するためのインデックス エンティティを照会する エンティティ、およびリスト内の各エンティティを照会するエンティティ。- リストの各エンティティは最大 1 MB サイズ、ソリューションののオプション 2 と 3 では、いずれの姓の従業員 ID リストも1 MB を超えない想定です。従業員 ID リストが 1 MB を超える可能性が高い場合はオプション 1 を使用し、インデックスのデータを Blob ストレージに格納します。-2 のオプションを使用する場合 (EGT を使用して、従業員の追加、削除および従業員の姓を変更する) 場合は、トランザクション ボリュームが、既定のパーティションでスケーラビリティの制限に近づくかないか評価する必要があります。この場合、更新リクエストを処理するキューを使用し、従業員エンティティとは別のパーティションにインデックスのエンティティ格納することができる一貫性のあるソリューション (オプション 1 または 3 ) を検討する必要があります。このソリューションのオプション 2 では、部署内で姓で検索することを前提としています。例えば、Sales 部署で姓が Jones の従業員のリストを取得する場合などです。組織全体で、姓が Jones であるすべての従業員を検索したい場合は、オプション 1 または 3 のオプションのいずれかを使用します。最終的な一貫性を提供する、キュー ベースのソリューションを実装することができます (詳細については、 [最終的に一貫性のあるトランザクション パターン](#eventually-consistent-transactions-pattern) を参照してください)。
...
このパターンを実行する場合、以下のパターンとガイダンスも関連する可能性があります：- [複合キーのパターン](#compound-key-pattern) - [最終的に一貫性のあるトランザクション パターン](#eventually-consistent-transactions-pattern) - [エンティティ グループ トランザクション](#entity-group-transactions) - [異なる種類のエンティティ型の使用](#working-with-heterogeneous-entity-types)
...
#### 関連のあるパターンとガイダンス
このパターンを実行する場合、以下のパターンとガイダンスも関連する可能性があります:- [複合キーのパターン](#compound-key-pattern) - [エンティティ グループ トランザクション](#entity-group-transactions) - [異なる種類のエンティティ型の使用](#working-with-heterogeneous-entity-types)
...
このパターンを実装方法を決定する場合は、次の点に注意してしてください。 - 完全なデータ系列が1 つのエンティティに収まらない (エンティティは最大で 252 個のプロパティを設定できます) は、BloB などの代替データ ストアを使用します。- エンティティの更新を同時に行う複数のクライアントがある場合、 **ETag** オプティミスティック同時実行で実行する必要があります。クライアントがたくさんある場合は、競合が大量に発生する可能性があります。
