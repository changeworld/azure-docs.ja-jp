...
2. `dotnet add package` コマンドを使用して、.NET パッケージ用 Azure Blob ストレージ クライアント ライブラリの `12.5.0-preview.6` バージョンをインストールします。 
...
この例では、クエリがクエリ アクセラレーション API に送信され、結果が戻されています。
