...
BLOB をアーカイブ層からオンライン層にリハイドレートする 2 つ目のオプションは、[Set Blob Tier](/rest/api/storageservices/set-blob-tier) を呼び出して BLOB の層を変更する方法です。 この操作により、アーカイブされた BLOB の層をホットまたはクールに変更できます。
