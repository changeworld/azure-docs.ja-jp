...
たとえば、ユーザーが **AllowProtectedAppendWrites** プロパティを有効にして、保持間隔が 90 日の時間ベースのアイテム保持ポリシーを作成するとします。 追加 BLOB _logblob1_ は今日コンテナーに作成され、今後 10 日間、新しいログが追加 BLOB に追加されます。したがって、_logblob1_ の有効な保持期間は、今日から 100 日 (最後に追加された時刻 + 90 日) です。
