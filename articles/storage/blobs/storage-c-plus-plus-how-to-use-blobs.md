...
> [!NOTE]
> このガイドは、C++ 用 Azure Storage クライアント ライブラリ 1.0.0 以上、12.x までのバージョンを対象としています。 Microsoft では、[GitHub](https://github.com/Azure/azure-sdk-for-cpp/tree/master/sdk/storage) 経由で使用できる C++ 用の Storage クライアント ライブラリの最新バージョンを使用することをお勧めします。 詳細については、最新の[クイックスタート](quickstart-blobs-c-plus-plus.md)をご覧ください。
...
ローカルの Windows コンピューターでアプリケーションをテストするには、[Azurite ストレージ エミュレーター](../common/storage-use-azurite.md)を使用できます。 Azurite は、ローカルの開発マシンで使用できる Azure の BLOB、Queue、Table のサービスをシミュレートするユーティリティです。 次の例では、ローカルのストレージ エミュレーターに接続文字列を保持する静的フィールドを宣言する方法を示しています。
