---
title: Azure Storage v10 SDK for JavaScript を使用して BLOB のアップロード、ダウンロード、一覧表示、削除を行う (プレビュー)
...
ms.locfileid: "51278403"
---
# <a name="quickstart-upload-download-list-and-delete-blobs-using-azure-storage-v10-sdk-for-javascript-preview"></a>クイック スタート: Azure Storage v10 SDK for JavaScript を使用して BLOB のアップロード、ダウンロード、一覧表示、削除を行う (プレビュー)
...
Done
```
このクイック スタートで新しいストレージ アカウントを使用している場合は、"*Containers*" のラベルの下にコンテナー名が一覧表示されない可能性があります。
...
```
API によって行われた要求は、一定の時間後にタイムアウトに設定される場合があります。 [Aborter](/javascript/api/%40azure/storage-blob/aborter?view=azure-node-preview) クラスは要求がタイムアウトする方法を管理し、このサンプルで使用されるタイムアウトを定義するには次の定数が使用されます。
```javascript
...
- [SharedKeyCredential](/javascript/api/%40azure/storage-blob/sharedkeycredential?view=azure-node-preview) クラスでは、要求パイプラインに提供するためにストレージ アカウントの資格情報がラップされます。

- [StorageURL](/javascript/api/%40azure/storage-blob/storageurl?view=azure-node-preview) クラスでは、新しいパイプラインが作成されます。

- [ServiceURL](/javascript/api/%40azure/storage-blob/serviceurl?view=azure-node-preview) では、REST API で使用される URL がモデル化されます。 このクラスのインスタンスを使用すると、コンテナーの一覧の表示や、コンテナーの URL を生成するためのコンテキスト情報の提供といったアクションを実行できます。

*ServiceURL* のインスタンスを [ContainerURL](/javascript/api/%40azure/storage-blob/containerurl?view=azure-node-preview) および [BlockBlobURL](/javascript/api/%40azure/storage-blob/blockbloburl?view=azure-node-preview) のインスタンスと併用して、ストレージ アカウントのコンテナーと BLOB を管理します。
...
```
応答はストリームとして返されます。 この例では、ストリームは文字列に変換されてコンソールに表示されます。
...
> [JavaScript API リファレンス](https://docs.microsoft.com/en-us/javascript/api/overview/azure/storage/client?view=azure-node-preview)
