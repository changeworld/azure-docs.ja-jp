...
BLOB 層が最後に変更された時間は、BLOB の**アクセス層変更時間**プロパティを介して公開されます。 BLOB がアーカイブ層にあると、上書きできないため、このシナリオでは、同じ BLOB をアップロードすることは許可されません。 ホット層またはクール層にある BLOB を上書きすると、新しい BLOB は上書きされた BLOB の層を継承します。
