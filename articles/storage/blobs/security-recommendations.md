...
| ストレージ アカウントをロックして誤削除を防止する | サブスクリプション、リソース グループ、またはリソースにロックを適用し、組織の他のユーザーが誤って削除したり変更したりするのを防止できます。 詳細については、「[リソースのロックによる予期せぬ変更の防止](../../azure-resource-manager/management/lock-resources.md)」を参照してください。
...
| Azure Monitor でアラートを設定する | ログ アラートを構成して、設定された頻度でリソース ログを評価し、その結果に基づいてアラートを発行することができます。 詳しくは、「[Azure Monitor でのログ アラート](../../azure-monitor/platform/alerts-unified-log.md)」を参照してください。 | - |
