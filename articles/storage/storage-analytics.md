...
>[Azure.NOTE] コンテナーの一覧作成操作 ( [ListContainers](https://msdn.microsoft.com/library/azure/dd179352.aspx) メソッドなど) を実行しても、$logs コンテナーは表示されません。 直接アクセスする必要があります。 たとえば、[ListBlobs](https://msdn.microsoft.com/library/azure/dd135734.aspx) メソッドを使用して、`$logs` コンテナー内の BLOB にアクセスできます。
