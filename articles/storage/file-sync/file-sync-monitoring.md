...
| クラウドを使った階層化のキャッシュ ヒット率 | キャッシュから取得されたかクラウドからリコールされたバイトの割合 (ファイル全体ではない)。<br><br>単位: パーセンテージ<br>集計の種類:Average<br>適用可能なディメンション: サーバー エンドポイント名、サーバー名、同期グループ名 |
...
| クラウドを使った階層化のリコール成功率 | 成功したリコール要求の割合。<br><br>単位: パーセンテージ<br>集計の種類:Average<br>適用可能なディメンション: サーバー エンドポイント名、サーバー名、同期グループ名 |
