...
- 復号化の場合:
	...
	- リゾルバーが指定されていない場合にキーが指定されると、そのキーの識別子が必須キー識別子と一致すると、そのキーが使用されます。識別子が一致しなければ、エラーがスローされます。
	  ...
	  BLOB、キュー、およびテーブルや、Key Vault 統合の詳細なエンド ツー エンド シナリオについては、「[暗号化のサンプル](https://github.com/Azure/azure-storage-net/tree/master/Samples/GettingStarted/EncryptionSamples)<fix URL>」を参照してください。
...
- [Azure Storage Client Library for Java の Maven パッケージ](http://mvnrepository.com/artifact/com.microsoft.azure/azure-storage) をダウンロードする
