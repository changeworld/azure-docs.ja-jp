...
> [!NOTE]
> この例 (とこの記事のその他の例) では、Composer を使用して Azure 向け PHP クライアント ライブラリがインストールされていることを前提としています。 ライブラリを手動でインストールした場合は、 `WindowsAzure.php` オートローダー ファイルを参照する必要があります。
> 
> 
```php
...
```
**setPublicAccess(PublicAccessType::CONTAINER\_AND\_BLOBS)** の呼び出しにより、匿名の要求でコンテナーと BLOB データにアクセス可能になります。 **setPublicAccess(PublicAccessType::BLOBS_ONLY)** の呼び出しにより、匿名の要求で BLOB データにのみアクセス可能になります。 コンテナー ACL について詳しくは、「[Set Container ACL (REST API)][container-acl]」をご覧ください。

BLOB サービスのエラー コードについて詳しくは、「[BLOB サービスのエラー コード][error-codes]」をご覧ください。
...
```
前の例では、ストリームとして BLOB をアップロードしていることに注意してください。 ただし BLOB は、[file\_get\_contents][file_get_contents] 関数などを使って、文字列としてアップロードすることもできます。 前のサンプルを使用してこれを行うには `$content = fopen("c:\myfile.txt", "r");` を `$content = file_get_contents("c:\myfile.txt");` に変更します。
...
```
この例では、ストリーム リソースとして BLOB を取得していることに注意してください (既定の動作)。 ただし、[stream\_get\_contents][stream-get-contents] 関数を使って、返されたストリームを文字列に変換できます。
