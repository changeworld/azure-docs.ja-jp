...
BLOB ストレージにデータをインポートするときは、トランザクション料金は発生しません。BLOB ストレージからデータをエクスポートするときは、標準の送信料金が適用されます。トランザクション料金の詳細については、「[データ転送の料金詳細](https://azure.microsoft.com/pricing/details/data-transfers/)」をご覧ください。
...
いいえ。Azure Import/Export サービスを使用して、Resource Manager ストレージ アカウントとの間でデータを直接コピーすることはできません。サービスがサポートするのは、クラシック ストレージ アカウントだけです。Resource Manager ストレージ アカウントは、まもなくサポートされる予定です。代わりに、データをクラシック ストレージ アカウントにインポートし、[AzCopy](storage-use-azcopy.md) または CopyBlob を使用して、データを Resource Manager ストレージ アカウントに移行できます。
...
**Azure Import/Export クライアント ツールは、32 ビット オペレーティング システムと互換性がありますか。** いいえ。クライアント ツールが対応しているのは、64 ビット Windows オペレーティング システムだけです。サポートされる OS バージョンの一覧については、「[前提条件](#pre-requisites)」の「オペレーティング システム」をご覧ください。
