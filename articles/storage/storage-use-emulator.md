...
> [AZURE.NOTE]HTTPS は、Azure Storage 内のリソースにアクセスするための推奨されるプロトコルですが、ストレージ エミュレーターでは HTTPS を使用できません。
