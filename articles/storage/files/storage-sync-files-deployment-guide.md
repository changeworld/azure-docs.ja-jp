...
> [!Important]  
> 同期グループ内の任意のクラウドまたはサーバー エンドポイントに変更を加えることにより、ファイルを同期グループ内の他のエンドポイントと同期できます。 クラウド エンドポイントを直接変更する (Azure ファイル共有) 場合は、24 時間に 1 回のみクラウド エンドポイントのために初期化される、Azure File Sync の変更検出ジョブによってまず認識される必要があることに注意してください。 詳細については、[Azure Files の FAQ](storage-files-faq.md#afs-change-detection) を参照してください。
