...
* 1 つのストレージ同期サービスでサポートできるよりも多くの登録済みサーバーまたは同期グループを用意する必要がある場合。 詳細については、「[Azure File Sync のスケール ターゲット](storage-files-scale-targets.md#azure-file-sync-scale-targets)」を確認してください。
...
> [!Note]  
> Windows Server 2019 でクラウドを使った階層化が有効になっているボリュームでデータ重複除去をサポートするには、Windows 更新プログラム [KB4520062 - 2019 年 10 月](https://support.microsoft.com/help/4520062)またはそれ以降の月次ロールアップ更新プログラムをインストールし、Azure File Sync エージェント バージョン 12.0.0.0 以降が必要です。
...
詳細については、「[Azure File Sync のパフォーマンス メトリック](storage-files-scale-targets.md#azure-file-sync-performance-metrics)」と「[Azure File Sync のスケール ターゲット](storage-files-scale-targets.md#azure-file-sync-scale-targets)」を参照してください。
...
シナリオの詳細なガイダンスについては、[Azure File Sync と Azure ファイル共有の移行の概要](storage-files-migration-overview.md)に関する記事をご覧ください。
