...
- RHEL/CentOS VM の場合、システムを RHEL/CentOS 8.2 にアップグレードします。 その他の Linux VM の場合、カーネルを 5.0 以上にアップグレードします。
...
12. **アラート ルール名**、**説明**、および **重大度** などの **アラートの詳細** を指定します。
...
## <a name="how-to-create-alerts-if-a-premium-file-share-is-trending-towards-being-throttled"></a>Premium ファイル共有がスロットルされる傾向にある場合にアラートを作成する方法
...
5. ストレージ アカウントでサポートされているシグナルの一覧が表示されたら、 **[エグレス]** メトリックを選択します。
   ...
   > [!NOTE]
   > イングレス、エグレス、またはトランザクションが設定したしきい値を超えたときに発せられる 3 つの個別のアラートを作成する必要があります。 これは、アラートが発せられるのは、すべての条件が満たされた場合に限られるからです。 したがって、すべての条件を 1 つのアラートに含めた場合、イングレス、エグレス、およびトランザクションがそれぞれのしきい値を超えた場合にのみアラートが発せられます。

6. 下へスクロールします。 **[ディメンション名]** ドロップダウンをクリックし、 **[ファイル共有]** を選択します。
7. **[ディメンション値]** ドロップダウンをクリックして、アラートの対象とするファイル共有を選択します。
8. **アラート パラメーター** (しきい値、演算子、集計粒度、評価の頻度) を定義し、 **[完了]** をクリックします。

   > [!NOTE]
   > 秒あたりのエグレス、イングレス、および IOPS が与えられていますが、エグレス、イングレス、およびトランザクションのメトリックは分あたりです。 (集計の粒度について -> 分あたり = ノイズが多いので、別のものを選択してください)。したがって、たとえば、プロビジョニングされたエグレスが 90 MiB/秒であり、プロビジョニングされたエグレスの 80% にしきい値を設定する場合は、次のアラート パラメーターを選択する必要があります。**しきい値** に 75497472、**演算子** に以上 (>=)、**集計型** に平均を選択します。 アラートのノイズをどれだけにするかに応じて、集計の粒度と評価の頻度に選択する値を選択できます。 たとえば、アラートに 1 時間の期間の平均イングレスを監視させ、毎時間アラート ルールを実行させる場合、**集計の粒度** に 1 時間、**評価の頻度** に 1 時間を選択します。

9. **[アクション グループの選択]** をクリックし、既存のアクション グループを選択するか、新しいアクション グループを作成して、**アクション グループ** (メール、SMS など) をアラートに追加します。
10. **アラート ルール名**、**説明**、および **重大度** などの **アラートの詳細** を指定します。
11. **[アラート ルールの作成]** をクリックして、アラートを作成します。

    > [!NOTE]
    > プロビジョニングされたイングレスが原因で Premium ファイル共有がスロットルされそうになっている場合に通知を受けるには、手順 5 を除く同じ手順に従い、代わりに **[イングレス]** メトリックを選択します。

    > [!NOTE]
    > プロビジョニングされた IOPS が原因で Premium ファイル共有がスロットルされそうになっている場合に通知を受けるには、いくつかの変更を行う必要があります。 手順 5 で、代わりに **[トランザクション]** メトリックを選択します。 また、手順 10 では、**集計の種類** の唯一のオプションは total です。 したがって、しきい値は、選択した集計の粒度に依存します。 たとえば、プロビジョニングされたベースライン IOPS の80% にしきい値を設定し、**集計粒度** に 1 時間を選択した場合、**しきい値** はベースライン IOPS (バイト単位) x 0.8 x 3600 になります。 これらの変更以外は、上記と同じ手順に従います。 
...
* [Azure Files に関してよく寄せられる質問 (FAQ)](storage-files-faq.md)
