---
title: Unity ショーケース アプリケーション
description: ショーケース アプリケーションが格納されている GitHub リポジトリへのリンク
author: florianborn71
ms.author: flborn
ms.date: 04/01/2020
ms.topic: sample
ms.openlocfilehash: e1e91a892a1c8b69b3dc7f6c79109377e13bd7ae
ms.sourcegitcommit: 829d951d5c90442a38012daaf77e86046018e5b9
ms.translationtype: HT
ms.contentlocale: ja-JP
ms.lasthandoff: 10/09/2020
ms.locfileid: "80804454"
---
# <a name="unity-showcase-application"></a>Unity ショーケース アプリケーション

Unity ショーケース アプリケーションは、Azure Remote Rendering 機能を示す Unity ベースのサンプル アプリケーションです。 Unity エディター内と Hololens 2 の両方で実行できます。

![ショーケース アプリ](./media/showcase-app.png "ショーケース アプリ")

## <a name="source-repository"></a>ソース リポジトリ

このアプリケーションのソース コードと、ビルドおよびデプロイの手順の説明は、[ARR サンプル リポジトリ](https://github.com/Azure/azure-remote-rendering/tree/master/Unity/AzureRemoteRenderingShowcase)にあります。

## <a name="next-steps"></a>次のステップ

* [クイック スタート: Unity によるモデルのレンダリング](../quickstarts/render-model.md)
* [クイック スタート: モデルをレンダリング用に変換する](../quickstarts/convert-model.md)
