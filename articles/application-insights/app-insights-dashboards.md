...
# Application Insights ポータルのダッシュボードおよびナビゲーション
...
[Azure ポータル](https://portal.azure.com)にサインインすると、ダッシュボードが表示されます。ダッシュボードはカスタマイズしたり、全画面表示モードにしたりできます。

ダッシュボードは、複数のアプリケーションで構成されたソリューションを、ホストされている場所を問わず監視するうえで特に役立ちます。Stream Analytics などの Azure コンポーネントがシステムに組み込まれている場合は、ダッシュボードに監視グラフを追加することもできます。

![カスタマイズされたダッシュボード](./media/app-insights-dashboards/30.png)

1. 左上隅をクリックすると、いつでもダッシュボードに戻れます。
2. 新しいリソースを作成するには、**[+ 新規]** をクリックします。[Application Insights リソース](app-insights-create-new-resource.md)は、アプリのテレメトリを格納、分析する場所です。
3. ナビゲーション バーを使って既存のリソースを開きます。
4. ダッシュボードを編集、作成するには、ダッシュボードのツールバーを使用します。さまざまなアプリケーションに対して[複数のダッシュボードを作成する](#dashboards)ことができます。
...
Web アプリケーション、バックエンド サーバー、Azure サービスなど、複数のコンポーネントでシステムが構成されている場合は、すべてのコンポーネントの主要業績評価指標を 1 つの画面に表示できます。

複数のアプリケーションがある場合は、複数のダッシュボードを作成して切り替えることができます。

![[編集] をクリックします。タイルやグラフをドラッグします。ギャラリーからタイルを追加します。[完了] をクリックします。](./media/app-insights-dashboards/30.png)
...
![[概要] ブレードで [メトリック] をクリックします。](./media/app-insights-dashboards/16-metrics.png)

### グラフおよびグリッドの編集

新しいグラフをブレードに追加するには:

![メトリックス エクスプローラーで、[グラフの追加] を選択します](./media/app-insights-dashboards/04-add.png)

表示内容を編集する既存のグラフまたは新しいグラフを選択します。

![1 つまたは複数のメトリックを選択します](./media/app-insights-dashboards/08-select.png)
...
![[グループ化] をオンにし、[グループ化] でプロパティを選択します](./media/app-insights-dashboards/15-segment.png)
...
![スライダーを調整します](./media/app-insights-dashboards/18-height.png)

### データのフィルター選択

選択したプロパティ値のセットのメトリックのみを表示するには:

![[フィルター] をクリックした後、プロパティを展開し、目的の値にチェック マークを付けます](./media/app-insights-dashboards/19-filter.png)
...
![[お気に入り] を選択します](./media/app-insights-dashboards/21-favorite-save.png)

このブレードをもう一度表示するには、**概要ブレードに移動**し、[お気に入り] を開きます。

![概要ブレードで [お気に入り] を選択します](./media/app-insights-dashboards/22-favorite-get.png)
...
![メトリックス エクスプローラーの上部にあるボタン](./media/app-insights-dashboards/17-reset.png)
...
![Azure ポータルでアプリケーションの概要ブレードを開きます](./media/app-insights-dashboards/03-range.png)
...
![グラフの一部をドラッグします。](./media/app-insights-dashboards/12-drag.png)
...
![グラフ上にマウス ポインターを置きます](./media/app-insights-dashboards/02-focus.png)

特定のポイントのメトリックの値は、先行するサンプリング間隔の集計です。

サンプリング間隔または "粒度" は、ブレードの上部に表示されます。

![ブレードのヘッダー。](./media/app-insights-dashboards/11-grain.png)

[時間範囲] ブレードで粒度を調整できます。

![ブレードのヘッダー。](./media/app-insights-dashboards/grain.png)
...
![[フィルター] を選択し、テレメトリの種類を選択します](./media/app-insights-dashboards/02-filter-req.png)
...
![プロパティを展開し、値を選択します](./media/app-insights-dashboards/03-response500.png)
...
![プロパティを展開し、値を選択します](./media/app-insights-dashboards/04-failingReq.png)
...
![[お気に入り] をクリックし、名前を設定して [保存] をクリックします](./media/app-insights-dashboards/08-favorite-save.png)


検索条件をもう一度表示するには、**概要ブレードに移動**し、[お気に入り] を開きます。

![[お気に入り] タイル](./media/app-insights-dashboards/22-favorite-get.png)

相対的な時間範囲を選択して保存した場合は、ブレードを開くたびに最新のデータが表示されます。絶対的な時間範囲を選択して保存した場合は、毎回同じデータが表示されます。

## 分析

[Analytics](app-insights-analytics.md) は、アプリのパフォーマンスに関する問題を診断して理解するのに役立つ高度な検索機能です。

![Analytics example](./media/app-insights-dashboards/025.png)

[Analytics] タイルをクリックすると表示されます。
