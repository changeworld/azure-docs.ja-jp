...
`summarize` の複数の式によって複数の列が作成されます。
...
複数の数値列を選択するには、[Don't Split (分割しない)] を選択しておく必要があります。複数の数値列を表示すると同時に、文字列の列で分割することはできません。
...
Analytics レポートはダッシュボードで非常にわかりやすいものの、データをより消化しやすい形式に変換することが必要な場合もあります。 たとえば、認証されたユーザーがテレメトリでエイリアスにより識別されるとします。 結果には、ユーザーの実名を表示する必要があるとします。 このために必要なのは、エイリアスから実際の名前にマップする CSV ファイルだけです。 
...
テーブルをインポートするには、**[設定]**、**[データ ソース]** を開き、指示に従ってソースを追加します。 この定義を使用して、テーブルをアップロードします。
