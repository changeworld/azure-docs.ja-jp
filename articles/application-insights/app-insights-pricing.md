...
* クォータは、*データ ポイント*単位で測定されます。 単一のデータ ポイントは、追跡メソッドの 1 つの呼び出しで、コードで明示的に呼び出されるか、標準テレメトリ モジュールのいずれかによって呼び出されます。 
...
* データ速度は、既定では、[アダプティブ サンプリング](app-insights-sampling)によって低下します。つまり、アプリの使用量が増加するにつれて、テレメトリ速度が期待するほど増加することはありません。
