...
Application Insights Profiler は、1 時間ごとに 2 分間、または **[Application Insights Profiler を構成する]** ページの [**[今すぐプロファイル]**](app-insights-profiler-settings.md?toc=/azure/azure-monitor/toc.json) ボタンが押されたときに、プロファイラー データを収集します。 ただし、プロファイリング データがアップロードされるのは、プロファイラーの実行中に発生した要求に添付できる場合のみです。 
