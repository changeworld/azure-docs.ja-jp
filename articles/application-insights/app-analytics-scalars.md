...
スカラー式は、[クエリ](app-analytics-queries.md)とは異なります。クエリは、結果としてテーブルを返します。
...
値を切り捨てて、指定された bin サイズの倍数である整数にします。[`summarize by`](app-analytics-queries.md#summarize-operator) クエリでよく使用されます。値が分散している場合に、特定の値ごとの小さなセットにグループ化されます。
...
テキスト文字列から[正規表現](app-analytics-reference.md#regular-expressions)との一致を取得します。必要に応じて、抽出された部分文字列を、指定された型に変換することもできます。
...
* *regex:* [正規表現](app-analytics-reference.md#regular-expressions)。
