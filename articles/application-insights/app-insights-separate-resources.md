...
* コンポーネントの ID でセグメント分割およびフィルター処理できるプロパティをすべてのテレメトリに追加します。これはサーバー ロール インスタンスでは自動的に行われますが、それ以外の場合は、[テレメトリ初期化子](app-insights-api-filtering-sampling.md#add-properties)を使用してプロパティを追加できます。
...
## その他の Application Insights リソースの作成
