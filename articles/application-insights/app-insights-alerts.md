...
* アラートに応答する Web アプリを設定する場合は、[Webhook アドレス](../azure-portal/insights-webhooks-alerts.md)を設定します。アラートがアクティブ化されているとき (つまり、トリガーされている)、およびアラートが解決済みのときの両方で呼び出されます。(ただし、現時点でクエリ パラメーターは Webhook プロパティとしては渡されないという点に注意してください)。
