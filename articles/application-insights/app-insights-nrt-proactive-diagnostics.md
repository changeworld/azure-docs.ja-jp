...
要求の割合と影響を受けるユーザーの数から、問題の緊急度を判断できます。上の例では、15% という失敗率が通常の失敗率である 1.3% と比較され、異常事態が発生していることを示しています。22 名のユーザーが影響を受け、何らかの操作に失敗しました。それが自分のアプリであれば、その深刻度を評価できます。
...
![構成](./media/app-insights-nrt-proactive-diagnostics/060.png)

## 違い...

NRT のプロアクティブ診断は、Application Insights の類似しているが異なるその他の機能を補完します。

* [メトリック アラート](app-insights-alerts.md)は、ユーザーが設定するアラートで、CPU 占有率、要求率、ページの読み込み時間など、広範なメトリックを監視できます。このアラートを使用すると、リソースを追加する必要がある場合などに自分に警告することができます。これに対し、NRT のプロアクティブ診断は、重要な少数のメトリック (現在は要求失敗率のみ) を対象としており、Web アプリの要求失敗率が Web アプリの通常の動作と比較して大幅に増加すると、ほぼリアルタイムでユーザーに通知するよう設計されています。
...
*この情報に関してご意見がございましたら、ぜひお寄せください。宛先は * [ainrtpd@microsoft.com](mailto:ainrtpd@microsoft.com) です。
