<properties 
	...
	description="データ コレクション モジュールを有効または無効にし、パフォーマンス カウンターおよびその他のパラメーターを追加します" 
...
Application Insights SDK は、いくつかのモジュールで構成されます。コア モジュールは、Application Insights ポータルにテレメトリを送信する基本的な API を提供します。追加のモジュールは、アプリケーションとそのコンテキストからデータを収集します。構成ファイルを調整することによって、モジュールを有効または無効にしたり、そのいくつかのモジュールのパラメーターを設定できます。
...
アプリケーションで使用される外部のコンポーネントの応答性に関するデータを収集します。このモジュールを IIS サーバーで機能させるには、[状態モニターをインストールする][redfield]必要があります。これを Azure Web アプリまたは VM で使用するには、[Application Insights 拡張を選択します][azure]。
...
Web アプリで処理されない例外をカウントします。「[障害と例外][exceptions]」を参照してください。
...
既定では、このモジュールは、さまざまな Windows パフォーマンス カウンターを収集します。メトリック エクスプ ローラーで、フィルターのブレードを開いたときにこれらのカウンターを確認できます。
...
次のプレースホルダ―が `InstanceName` としてサポートされています。
...
これらのパラメーターは、Java SDK が収集するテレメトリ データを格納およびフラッシュする方法に影響します。
...
Application Insights に送信される各データ項目に操作 ID を追加します。Web アプリの場合、"操作" とは HTTP 要求のことです。それで、たとえば、要求、および要求処理の一部を成すカスタム イベントやトレースはすべて、同じ操作 ID を持つことになります。
