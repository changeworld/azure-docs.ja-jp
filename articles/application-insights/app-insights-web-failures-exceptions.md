<properties 
	pageTitle="Web アプリの障害と例外の検出と診断" 
	description="Web アプリの障害と例外の検出と診断" 
...
# Application Insights を利用し、Web アプリの障害と例外を診断する

[Visual Studio Application Insights][start] は、アプリケーションの障害を検出および診断するための強力なツールです。ここでは特に、Web アプリケーションについて説明します (ただし、Application Insights は [他のさまざまなプラットフォーム][platforms]にも適用できます)。
...
Web ページに大量のスクリプトがある場合、*[Application Insights を Web ページに追加することができます][track]
...
> [AZURE.NOTE]Web ブラウザーで、CDN または他のドメインからのスクリプト ファイルを組み込む場合、スクリプト タグに属性 ```crossorigin="anonymous"``` が含まれること、およびサーバーが CORS ヘッダーを送信して、スタック トレースおよびこれらのリソースからハンドルされない javascript 例外の詳細を取得することを確認してください。

ただし、.NET フレームワークが例外をキャッチする場合もあるため、必ずレポートされるというわけではありません。したがって、すべての例外を確実に表示するためには、ちょっとした例外ハンドラーを作成する必要があります。最良の対処方法は、テクノロジによって異なります。詳細については、[このブログ](http://blogs.msdn.com/b/visualstudioalm/archive/2014/12/12/application-insights-exception-telemetry.aspx)を参照してください。
