...
最新のクラウドとマイクロサービス アーキテクチャの登場により、シンプルで独立したデプロイ可能なサービスが生み出されています。これにより、可用性とスループットを増加させながら、コストを削減させることができます。 しかし、これらの動きによって、個別のサービスで全体を理解するのがより簡単になった一方、システム全体の判断やデバッグがより難しくなっています。
...
* [JavaScript を選択した場合](https://docs.microsoft.com/azure/application-insights/app-insights-javascript)
...
OpenCensus を使用して分散トレース機能をアプリケーションに追加するには、まず [Application Insights のローカル フォワーダーをインストールして構成します](./opencensus-local-forwarder.md)。 ここから、ローカル フォワーダーを使用して分散トレース データをルーティングするように、OpenCensus を構成します。 [Python](./opencensus-python.md) と [Go](./opencensus-go.md) の両方がサポートされます。
