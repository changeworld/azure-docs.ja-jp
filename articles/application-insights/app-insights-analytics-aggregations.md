...
過去 1 時間に行われた要求の数を調べます。

    requests | where timestamp > ago(1h)
    | summarize req_count = sum(itemCount) by name

> [AZURE.NOTE] 単に要求データ ポイントの数をカウントするのではなく、itemCount プロパティを使用します。これにより、[サンプリング](app-insights-sampling.md)の設定に応じた結果が返されます。たとえば、サンプリングが 33% に設定されている場合、各データ ポイントの itemCount は 3 になります。

#### Min と Max

要求ストリーム内のすべてのレコードの最小タイムスタンプと最大タイムスタンプを見つけます。group by 句はないため、以下のように出力には行が 1 つしかありません。
...
逆関数に関する「[`mvexpand` 演算子](app-insights-analytics-queries.md#mvexpand-operator)」も参照してください。
