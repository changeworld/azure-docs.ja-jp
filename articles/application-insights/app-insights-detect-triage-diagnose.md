...
## 可用性の低下を検出する
Marcela Markova は OBS チームの上級開発者で、オンライン パフォーマンスの監視を主導しています。Marcela は次のような [Web テスト][availability]を設定しました。
...
通常のしきい値を超える応答時間になった場合の[アラート][metrics]を設定しました。こうすることで、応答時間が遅くなった場合に確実に把握できるようになります。
...
また、このアラートを構成する必要がなかったことも意外でした。実は、Application Insights では、プロアクティブ診断が自動的に有効になります。この機能はアプリケーションの通常のエラー パターンに合わせて自動的に調整され、特定のページに対するエラー、高負荷、他のメトリックとの関連性に "慣れ" ます。その期待される値を上回った場合にのみ、アラートが発行されます。
...
![失敗した要求](./media/app-insights-detect-triage-diagnose/23.png)
