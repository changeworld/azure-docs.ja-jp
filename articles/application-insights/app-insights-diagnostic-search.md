...
> [AZURE.NOTE]アプリが大量のテレメトリを生成し、ASP.NET SDK バージョン 2.0.0-beta3 以降を使用している場合は、アダプティブ サンプリング モジュールが、代表的な一部のイベントのみを送信することによって、ポータルに送信される量を自動的に削減します。ただし、同じ要求に関連のあるイベントは、グループとして選択または選択解除されるので、関連するイベント間を移動できます。[サンプリングについてはこちらを参照してください](app-insights-sampling.md)。
