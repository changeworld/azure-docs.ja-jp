...
指定された分数 (既定値は 0.05) よりも多いイベントに表れるすべてのパターン。 各パターンでは、パターンで設定されていない列 (特定の値に制限されていない列) にはワイルドカード値 (既定では null 値) が含まれます (これらの値を手動で変更する方法については、次の引数のセクションをご覧ください)。
...
* 重み列の *itemCount*
    ...
    これは、アカウントのサンプリングとメトリックの事前集計を考慮する際に使用します。 各行には、この列で指定された重み属性が設定されます。 既定では、各行の重みは "1" です。 この場合、各行に既に埋め込まれているデータのバケットまたは集計が考慮されます。
  
    例: `T | evaluate basket('~', itemCount)`
