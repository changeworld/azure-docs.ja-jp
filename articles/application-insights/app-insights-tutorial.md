...
[開発時に SDK をコードに追加する](app-insights-asp-net.md)ことで開始するか、[実行時にアプリをインストルメント化する](app-insights-monitor-performance-live-website-now.md)ことで開始できます。

アプリケーションから収集されたテレメトリ データは [Azure ポータル](https://portal.azure.com/)に格納、分析され、ポータルで直感的なビューやパワフルなツールにより迅速な分析や診断を実行できます。
...
Application Insights には、異なる種類のテレメトリを収集するさまざまな[標準インストルメンテーション モジュール](app-insights-configuration-with-applicationinsights-config.md)があります。たとえば、HTTP 要求に対する Web アプリの応答を監視するモジュール、例外を監視するモジュール、依存関係を監視するモジュール、パフォーマンス カウンターを監視するモジュールなどがあります。

開発時にコードをインストルメント化する場合は、テレメトリ データをポータルに送信する[独自のカスタム コード](app-insights-api-custom-events-metrics.md)を記述することもできます。カスタム テレメトリは、ユーザーの行動を追跡する際に特に便利であり、問い合わせ、販売、特定の目標の達成などのイベントをカウントできます。

### クライアント側の監視

さらに、各 Web ページに挿入される小さなスクリプトという形をとる[クライアント側のインストルメンテーション](app-insights-javascript.md)があります。テレメトリは、エンド ユーザーのブラウザーから Azure ポータルに直接送信されます。ページの読み込み時間、AJAX 呼び出し、およびスクリプトの例外が監視されます。ボタンのクリックやその他のイベントなどのユーザーの操作を追跡するカスタム コードを記述することもできます。

クライアント側の監視は、サーバー側の監視に加えて使用することも、サーバー側の監視とは無関係に使用することもできます。
...
Application Insights は、Microsoft Azure でホストされる多数のサービスの 1 つであり、テレメトリは分析と表示のために Microsoft Azure に送信されます。このため、他の操作を行う前に、[Microsoft Azure](http://azure.com) のサブスクリプションが必要になります。サインアップは無料で、Application Insights の無料の[価格レベル](https://azure.microsoft.com/pricing/details/application-insights/)を選択できます。


## 作業開始

Web アプリの監視を開始する主な方法は 2 つあります。目的に適した方法から始めてください。

|タイミング|方法|取得内容
|---|---|---
| 開発 |SDK をコードに追加する (サーバーおよびクライアントのいずれかまたは両方)| アプリの応答時間とエラー率を監視し、開発、テスト、および運用中の問題を診断します。<br/>カスタム トレースと診断を追加するためのコードを記述します。 
|ランタイム |ホストでエージェントを実行する (オンプレミスで、または Azure で。J2EE または ASP.NET) | 応答時間とエラー率を監視し、実行時に問題を診断します。<br/>コードにアクセスする必要はありません。

これら 2 つのアプローチは互いを補完します。テレメトリをフルに活用するには両方を使用してください。

### 実行時に開始する

いろいろな意味で、実行時にアプリをインストルメント化するのが最も簡単なオプションです。コードの変更はなく、アプリのデプロイ後でも設定が可能です。(後で開発時オプションに切り替えることもできます。)

このオプションは、以下に適しています。

* カスタム テレメトリを必要としないアプリ。標準的な要求、依存関係の応答時間、エラー率、例外レポート、およびパフォーマンス カウンターを取得できます。
* 問題が発生している実行中のアプリ。診断データをすばやく取得する必要があります。

**アプリの実行時にサーバーが行うこと**

* **オンプレミスまたは VM 上の IIS**: サーバーに [Application Insights Status Monitor をインストールします](app-insights-monitor-performance-live-website-now.md)。管理アクセス権が必要です。
* **Azure Web アプリまたは VM**: アプリのコントロール パネルで [Application Insights のパフォーマンスの監視を選択します](app-insights-monitor-performance-live-website-now.md#if-your-app-runs-as-an-azure-web-app)。
* **Java**: サーバーに [Java エージェントをインストールします](app-insights-java-live.md)。

Java オプションを除くこれらのツールは、テレメトリを表示するために必要な Application Insights リソースを自動的に設定します。

[Azure ポータル](https://portal.azure.com)にサインインし、アプリ用の Application Insights の概要に移動して、データが到着するのを確認します。

後で「開発中に使用を開始する」の手順に従って[カスタム テレメトリ](app-insights-api-custom-events-metrics.md)を追加しても、何の問題も発生しません。

### 開発時に開始する

Application Insights のパワーをフルに利用するには、開発中に適切な Application Insights SDK をコードに追加します。

SDK を追加することで、以下を実行できます。

* 開発およびテスト中に診断を取得する
* 例外レポートにスタック トレースを取得する
* テレメトリをカスタマイズする。ビジネスと診断の両方を目的としてイベントとメトリックのカウントと追跡を行う独自のコードを記述します。

**使用中の開発ツール**

* **Visual Studio 2013 update 2 以降で ASP.NET**: [Application Insights を Web プロジェクトに追加する](app-insights-asp-net.md)メニュー コマンドを使用します。新しいプロジェクトを作成する場合は、Application Insights オプションをクリックします。
* **Eclipse で Java**: [Eclipse アドインを使用します](app-insights-java-eclipse.md)
* **Java とその他のツール**: [Java SDK をインストールします](app-insights-java-get-started.md)
* **その他のプラットフォーム**: [適切な SDK をインストールします](app-insights-platforms.md)


![Application Insights を Visual Studio のプロジェクトに追加する](./media/app-insights-tutorial/appinsights-03-addExisting.png)

ほとんどの場合、これらのツールは SDK をプロジェクトに追加し、Application Insights ポータル内にリソースを作成した後、この 2 つをリンクします。ただし、必要に応じて、[新しいリソースを Azure 内に手動で作成](app-insights-create-new-resource.md)できます。

### Web ページ オプション

サーバー コードをインストルメント化できない場合でも、Web ページを常にインストルメント化できます。

これによって、ユーザー、セッション、およびページ ビューの数、ページ読み込みのパフォーマンス データ、AJAX 呼び出しの応答時間、およびエラー率を取得できます。ボタンのクリックや選択などのその他のイベントを追跡するカスタム コードを記述することもできます。これは、単一ページの Web アプリケーションで特に便利です。

* **Web ページ**: [Web ページにコード スニペットを追加します](app-insights-javascript.md)。
...
![概要ブレード](./media/app-insights-tutorial/01-find.png)
...
![サーバー ブレード](./media/app-insights-tutorial/04.png)
...
![グラフの編集](./media/app-insights-tutorial/05.png)
...
![分析](./media/app-insights-tutorial/11.png)
...
![ダッシュボード](./media/app-insights-tutorial/12.png)
...
メトリックに関する[リリース注釈](app-insights-annotations.md)に、新しいビルドのデプロイ先が示されます。これにより、変更内容がアプリケーションのパフォーマンスに影響を与えたかどうかを簡単に把握できます。リリース注釈は、[Visual Studio Team Services ビルド システム](https://www.visualstudio.com/ja-JP/get-started/build/build-your-app-vs)によって自動で作成されます。[PowerShell から作成](#create-annotations-from-powershell)することもできます。
