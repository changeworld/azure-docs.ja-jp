...
セッションは、ユーザーとアプリケーション間の単一のエンカウンターを表します。最も単純な形式では、セッションは、ユーザーがアプリケーションを起動すると開始され、ユーザーがアプリを終了すると終了します。Mobile Apps の場合、セッションは、アプリが 20 秒以上中断された (背景ページに移動)した 場合に終了します。アプリが再開されると、新しいセッションが開始されます。もちろん、ユーザーは 1 日に複数のセッションを行うことも、さらには 1 時間に複数のセッションを行うこともあります。
