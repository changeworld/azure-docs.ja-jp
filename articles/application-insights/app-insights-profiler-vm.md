...
このページでは、Azure VM または Azure 仮想マシン スケール セットで Application Insights Profiler を実行するために必要な手順を説明します。 Application Insights Profiler は、VM 用の Windows の Azure 診断拡張機能と共にインストールされています。 Profiler を実行するように拡張機能を構成する必要があります。また、VM で実行されている Web アプリのプロファイルを取得するには、アプリケーションに App Insights SDK が組み込まれている必要があります。
...
1. 変更された環境のデプロイ定義をデプロイします。  
   ...
   次の PowerShell コマンドは、Azure 診断拡張機能のみを使用する既存の仮想マシン向けの代替アプローチです。  
