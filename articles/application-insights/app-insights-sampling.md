...
* **アダプティブ サンプリング**はサンプリングの割合を自動的に調整し、要求が一定の量になるようにします。Application Insights SDK for ASP.NET バージョン 2.0.0-beta3 以降の既定では、アダプティブ サンプリングは有効です。 
* SDK バージョン 2 からは、**固定レート サンプリング**も利用できます。サンプリングの割合を指定します。これは、JavaScript クライアントと ASP.NET サーバーの両方で動作します。クライアントとサーバーはサンプリングを同期するので、検索では関連のあるページ ビューと要求の間を移動できます。
* **インジェスト サンプリング**。これは、Application Insights サービスで行われます。SDK はブラウザーとサーバーからすべてのテレメトリを送信しますが、保持されるのは一部だけです。これは、アプリで送信されるテレメトリの量が最小値を超えた場合にのみ、および SDK が他の種類のサンプリングのいずれかを実行していない場合にのみ動作するアダプティブ サンプリングです。また、使用している価格レベルの影響を受けます。インジェスト サンプリングはトラフィックを削減しませんが、トラフィックを月間のクォータ (上限) 以内で維持するのに役立ちます。
...
ASP.NET SDK バージョン 2.0.0-beta3 以降を使用している場合、アダプティブ サンプリングは自動的に有効になります。使用する SDK バージョンに関係なく、SDK にサンプルが存在しない場合、インジェスト サンプリング (サーバーでの) が動作することがあります。
...
サンプリング フォームはすべて、特定のトラフィックしきい値を超えた場合にのみ動作します。たとえば、ほとんどのデバッグ セッションや使用頻度の低いサイトには影響を与えません。
...
それ以外の場合は、アダプティブ サンプリングをお勧めします。これの有効化は、既定では ASP.NET サーバー SDK によって行われます。

SDK でサンプリングを使用しない場合 (たとえば、Java を使用している場合、または ApplicationInsights.config でサンプリングを無効にした場合)、インジェスト サンプリングはデータが Application Insights サービスに到着すると同時に動作する場合があります。これはアダプティブ サンプリングのフォームです。
 ...
 * はい。アダプティブ サンプリングを構成するコード メソッドを使用します。サンプリング レートを取得するコールバックを指定できます。連続エクスポートを使用する場合は、エクスポートされたデータ ポイントに示されているサンプリング率を確認できます。
