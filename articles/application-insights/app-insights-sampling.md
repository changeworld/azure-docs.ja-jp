...
* アプリケーションが多数のテレメトリを送信する場合、サンプリングは自動的に行われます。自動サンプリングは、量が多い場合にだけ、ASP.NET Web サーバー アプリにおいてのみ実行されます。
* サンプリングは、ポータルの料金ページで (保持されるテレメトリの量を減らし、月間クォータに収まるようにするために)、または ASP.NET SDK の .config ファイルでやはりネットワーク トラフィックを減らすために、手動で設定することもできます。
* 現在のサンプリング レートは、各レコードのプロパティです。[検索] ウィンドウで、要求などのイベントを開きます。すべてのプロパティの省略記号 [...] を展開し、テレメトリの種類に応じて、[*数] プロパティ ([要求数] や [イベント数] など) を探します。値が 1 より大きい場合、サンプリングが行われています。値 3 は、サンプリングが 33% であることを意味します。保持されている各レコードは、生成された元のレコード 3 個に相当します。
...
* **アダプティブ サンプリング**は、ASP.NET アプリの SDK から送信されるテレメトリの量を自動的に調整します。SDK v 2.0.0-beta3 からの既定値です。
* **固定レート サンプリング**は、ASP.NET サーバーおよびユーザーのブラウザーから送信されるテレメトリの量を削減します。管理者がレートを設定します。
* **インジェスト サンプリング**は、Application Insights サービスによって維持されるテレメトリの量を、管理者が設定したレートで削減します。インジェスト サンプリングはテレメトリのトラフィックを削減しませんが、トラフィックを月間のクォータ (上限) 以内で維持するのに役立ちます。
...
> [AZURE.WARNING] タイルに表示される値は、インジェスト サンプリングに設定した値を示します。SDK サンプリングの実行中は、この値は実際のサンプリング レートを表しません。


## Web サーバーでのアダプティブ サンプリング
...
ほとんどの中小規模アプリケーションには、サンプリングは不要です。最も役立つ診断情報や最も正確な統計は、すべてのユーザー アクティビティからデータを収集することで入手します。
