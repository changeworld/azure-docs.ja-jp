...
デバッグ中またはデプロイの直後にすばやくテレメトリ データを表示するには、ライブ ストリームを使用します。

![概要ブレードで、[ライブ ストリーム] をクリックします](./media/app-insights-asp-net/45.png)


ライブ ストリームは、デプロイ直後にアプリが正常に動作していることを検証できるように設計されています。

ほんの数分前からのデータを表示し、データを保持しません。

2\.1.0-beta1 以降のバージョンの SDK が必要です。
...
概要グラフで集計データを探します。最初、1 つまたは 2 つのポイントだけが表示されます。次に例を示します。
...
[ライブ ストリーム](#live-stream)を使用すると、デプロイまたは再デプロイの最初の数分間を監視して、アプリが正しく動作しているかどうかを判断できます。古いバージョンを取り替えるときは特に、性能の改善が気になります。問題がある場合、古いバージョンに戻すことがあります。
