...
![この図は、テレメトリとコマンドを使用できるよう、IoT Edge デバイスの IoT Edge Stream Analytics モジュールに接続されたオイル ポンプ ジャックを示しています。 フィルター選択されたテレメトリが、クラウドにあるリモート監視ソリューション アクセラレータの IoT Edge デバイスに送信されます。 クラウドには、Deployment と Package も存在します。 Deployment によって IoT Edge ランタイムがデバイスにデプロイされます。](media/iot-accelerators-remote-monitoring-edge/overview.png)
