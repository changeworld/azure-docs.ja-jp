...
[Split][split] モジュールなど一部のモジュールでは、Python クライアント ライブラリでサポートされない `Dataset` 名の形式で出力されます。

![データセット形式][dataset-format]

[\[CSV に変換\]][convert-to-csv] などの変換モジュールを使用してサポートされる形式で出力を取得する必要があります。
...
ご利用のデータが既にシリアル化されている場合、`update_from_dataframe` の代わりに `update_from_raw_data` を使用します。同様に動作し、`dataframe` の代わりに `raw_data` を渡すだけです。
