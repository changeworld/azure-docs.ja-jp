...
- 一般に、負荷分散規則では、IP フラグメントの転送または UDP および TCP パケットの IP フラグメント化の実行はサポートされていません。  [HA ポートの負荷分散規則](load-balancer-ha-ports-overview.md)は、この一般的な説明の例外であり、既存の IP フラグメントを転送するために使用できます。
