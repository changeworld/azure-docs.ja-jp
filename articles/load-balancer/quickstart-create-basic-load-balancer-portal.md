...
このセクションでは、ポータルを使用してパブリック Basic Load Balancer を作成します。 ポータルを使用してパブリック IP とロード バランサー リソースを作成する場合、パブリック IP アドレスは ロード バランサーのフロント エンドとして自動的に構成されます。 フロント エンドの名前は **LoadBalancerFrontend** です。
