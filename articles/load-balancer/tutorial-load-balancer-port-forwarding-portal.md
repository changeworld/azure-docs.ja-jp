...
4. ページ下部にある **[次へ: フロントエンド IP の構成]** ボタンを選択します。

5. **[フロントエンド IP の構成]** で、 **[+ フロントエンド IP の追加]** を選択します。

6. **[名前]** に、「**LoadBalancerFrontend**」と入力します。

7. **[IP のバージョン]** として **[IPv4]** または **[IPv6]** を選択します。

    > [!NOTE]
    > IPv6 は現在、ルーティングの優先順位またはリージョン間負荷分散 (グローバル階層) ではサポートされていません。
...
12. **[ルーティング設定]** については、既定値の **[Microsoft ネットワーク]** のままにします。
