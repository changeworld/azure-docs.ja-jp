...
> [!IMPORTANT]
> この記事では、VM とスケール セット インスタンスの自動配置についても説明します。 自動配置は現在、パブリック プレビュー段階にあります。
> プレビューに参加するには、[https://aka.ms/vmss-adh-preview](https://aka.ms/vmss-adh-preview) でプレビューのオンボードに関するアンケートにお答えください。
> Azure portal のプレビュー機能にアクセスするには、次の URL を使用する必要があります: [https://aka.ms/vmssadh](https://aka.ms/vmssadh)。
...
1. Azure [portal](https://portal.azure.com) を開きます。 **自動配置**のプレビューを試す場合は、次の URL を使用します: [https://aka.ms/vmssadh](https://aka.ms/vmssadh)。
...
1. **自動配置**の URL を使用した場合は、このオプションを選択すると、VM とスケール セット インスタンスがこのグループの使用可能なホストに自動的に割り当てられます。
