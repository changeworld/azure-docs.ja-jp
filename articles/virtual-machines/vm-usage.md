...
| Tags| この列には、ユーザーが指定したリソース タグが含まれます。 タグは、課金記録のグループ化に使用できます。 [CLI](./linux/tag.md) または [PowerShell](./windows/tag.md) を使用して仮想マシンにタグを付ける方法について学習してください。これは、Resource Manager VM でのみ使用できます。| `{"myDepartment":"RD","myUser":"myName"}`|
