...
- Virtual Machine Scale Sets に適用されるとき、1 回のバッチでアップグレードされるのは、Virtual Machine Scale Sets の仮想マシンの 20% 以下です (バッチごとに少なくとも 1 つの仮想マシン)。
...
拡張機能のアップグレード プロセスにより、VM 上の既存の拡張機能のバージョンが、拡張機能の発行元によって発行された新しい拡張機能のバージョンに置き換えられます。 新しい拡張機能がインストールされた後、VM の正常性が監視されます。 アップグレードが完了してから 5 分以内に VM が正常な状態にならない場合、拡張機能の新しいバージョンは前のバージョンにロールバックされます。
...
2. アップグレード オーケストレーターにより、アップグレードする VM インスタンスのバッチが識別されます。いずれのバッチも、最大で合計 VM 数の 20%、最小バッチ サイズは 1 つの仮想マシンです。

3. アプリケーション正常性プローブまたはアプリケーション正常性拡張機能が構成されているスケール セットの場合、アップグレードは VM が正常になるまで 5 分間 (または定義されている正常性プローブ構成だけ) 待機した後、次のバッチのアップグレードに進みます。 アップグレード後に VM の正常性が回復しない場合は、既定で、VM の前のバージョンの拡張機能が再インストールされます。
...
- 1 つの "フェーズ" は 1 つ以上のリージョンで構成でき、更新は、フェーズ内の対象となる VM が正常に更新された場合にのみ、フェーズ間を移動します。
...
VM または仮想マシン スケールセットには、拡張機能の自動アップグレードが有効になっていない他の拡張機能に加えて、拡張機能の自動アップグレードが有効になっている複数の拡張機能があってもかまいません。  

1 つの仮想マシンで複数の拡張機能アップグレードを利用できる場合、アップグレードをまとめてバッチ処理することができます。 ただし、各拡張機能のアップグレードは、仮想マシンで個別に適用されます。 1 つの拡張機能で障害が発生しても、アップグレードされる可能性のある他の拡張機能には影響しません。 たとえば、2 つの拡張機能のアップグレードがスケジュールされていて、最初の拡張機能のアップグレードが失敗した場合でも、2 つ目の拡張機能はアップグレードされます。

VM または仮想マシン スケール セットに[拡張機能のシーケンス処理](../virtual-machine-scale-sets/virtual-machine-scale-sets-extension-sequencing.md)を使用して複数の拡張機能が構成されている場合は、拡張機能の自動アップグレードも適用できます。 拡張機能のシーケンス処理は VM の初回デプロイに適用され、拡張機能での以降の拡張機能のアップグレードは個別に適用されます。


## <a name="next-steps"></a>次のステップ
> [!div class="nextstepaction"]
> [アプリケーション正常性拡張機能について確認する](./windows/automatic-vm-guest-patching.md)
