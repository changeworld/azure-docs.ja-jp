...
参照しているリソースを特定するのに十分な情報が提供されるようにわかりやすい名前にする必要があります。
...
既にオペレーティング システムを含んでいる .VHD イメージ ファイルから仮想マシンが作成される場合、Microsoft Azure での仮想マシン名と仮想マシンの OS コンピューター名が異なる場合があります。このような状況は仮想マシンの管理が困難になるのでお勧めしません。Azure 仮想マシンのリソース名が、その仮想マシンのオペレーティング システムに割り当てたコンピューター名と同じになるようにします。
...
Azure では 2 種類のストレージ アカウントを使用できます。標準ストレージ アカウントでは、BLOB ストレージ (Azure 仮想マシン ディスクの保存に利用)、テーブル ストレージ、キュー ストレージ、ファイル ストレージにアクセスできます。Premium Storage は、AlwaysOn クラスター内の SQL Server などの高パフォーマンス アプリケーション用に設計されており、現在は Azure Virtual Machine ディスクのみをサポートします。
