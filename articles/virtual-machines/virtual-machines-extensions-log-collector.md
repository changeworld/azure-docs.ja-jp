...
収集モードは 2 つあり、どちらが使用されるかは、収集するファイルの種類によって異なります。- Azure ゲスト エージェント ログのみ (GA)。Azure のゲスト エージェントとその他の Azure コンポーネントに関連したすべてのログが対象となります。- すべてのログ (Full)。GA モードで収集されるすべてのファイルに加え、次のファイルが収集されます。
