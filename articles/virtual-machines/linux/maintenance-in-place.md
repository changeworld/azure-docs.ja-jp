...
これらの更新は、インプレース ライブ移行 ("メモリ保護更新" とも呼ばれます) を実現するテクノロジによって完了します。 ホストの更新時は仮想マシンが "一時停止" 状態になり、RAM 内のメモリが保護されます。一方、ホスティング環境 (基礎となるオペレーティング システムなど) には必要な更新プログラムと修正プログラムが適用されます。
