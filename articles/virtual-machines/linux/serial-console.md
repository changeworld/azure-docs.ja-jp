...
シリアル コンソールのテキストが画面サイズいっぱいに表示されない (テキスト エディターの使用後に多く発生) | これは、シリアル接続で画面サイズが認識されないことについての既知の問題です。 "サイズ変更" コマンドを備えたユーティリティ (xterm など) のインストールをお勧めします。 "サイズ変更" を実行すれば、この問題は解消します。
