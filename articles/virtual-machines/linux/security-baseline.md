...
**ガイダンス**:Linux Virtual Machines や Azure Active Directory には、既定のパスワードという概念がありません。 既定のパスワードを使用する可能性のあるサード パーティ アプリケーションやマーケットプレース サービスに対する責任は、お客様が負います。
...
**ガイダンス**: Linux を実行している Virtual Machines (VM) との間や、それらのマシン間の転送中のデータは、RDP または SSH セッションでの VM への接続のタイミングなどの接続の性質に応じて、いくつかの方法で暗号化されます。
...
* [Linux 仮想マシン用の診断拡張機能](../extensions/diagnostics-linux.md?toc=/azure/azure-monitor/toc.json)
...
* [VM テンプレートをダウンロードする方法に関する情報](../windows/download-template.md)
