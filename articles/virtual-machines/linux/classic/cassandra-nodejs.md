...
この記事では基本的な手法を用いて、Docker、Chef、Puppet と比較して、Cassandra クラスターの構築に関連する内容について説明します。 この手法によって、インフラストラクチャのデプロイを大幅に簡略化することができます。

## <a name="the-deployment-models"></a>デプロイ モデル
Microsoft Azure のネットワークでは、ネットワークのセキュリティのきめ細かな設定を実現するためにアクセスを制限できる、分離プライベート クラスターのデプロイが可能です。 この記事では、基本レベルでの Cassandra のデプロイを説明することを目的としているため、一貫性レベル、およびスループットに最適化したストレージ設計については取り上げません。 仮想クラスターのネットワーク要件の一覧を次に示します。
...
<table>
<tr><th>ソフトウェア</th><th>ソース</th><th>Version</th></tr>
...
デプロイを簡略化するため、必要なすべてのソフトウェアをデスクトップにダウンロードします。 その後、それらを Ubuntu テンプレート イメージにアップロードし、クラスター デプロイの前段階として作成します。
...
テンプレート VM を作成するには、Azure portal にログインして、次のシーケンスを使用します。[新規]、[COMPUTE]、[仮想マシン]、[ギャラリーから]、[UBUNTU]、[Ubuntu Server 14.04 LTS] の順にクリックしてから、右矢印をクリックします。 Linux VM の作成方法を説明したチュートリアルについては、「Linux を実行する仮想マシンの作成」を参照してください。
...
**手順 1:仮想ネットワークの作成** Azure portal にログインし、次の表に記載の属性で仮想ネットワーク (クラシック) を作成します。 プロセスの詳細な手順については、「[Azure Portal を使用した仮想ネットワーク (クラシック) の作成](../../../virtual-network/virtual-networks-create-vnet-classic-pportal.md)」をご覧ください。
...
<table>
<tr><th>Status</th><th>Address    </th><th>Load    </th><th>トークン    </th><th>所有 </th><th>ホスト ID    </th><th>ラック</th></tr>
...
### <a name="step-1-create-the-virtual-network-at-the-2nd-region"></a>手順 1:2 つ目のリージョンでの仮想ネットワークの作成
...
### <a name="step-5-update-local-networks-with-the-respective-gateway-addresses"></a>手順 5:それぞれの "ゲートウェイ" アドレスで "ローカル" ネットワークを更新
...
### <a name="step-8-create-the-virtual-machines-in-region-2"></a>ステップ 8:リージョン #2 での仮想マシンの作成
...
**手順 1:Node.js と Cassandra クライアントのインストール**
