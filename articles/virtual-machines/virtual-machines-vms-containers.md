...
- シンプルな [Git](https://git-scm.com/) スタイルのコマンドを使用して、[パブリック](https://registry.hub.docker.com/)や[プライベートの Docker レジストリにイメージを](virtual-machines-docker-registry-on-azure-blob-storage.md)プッシュ/プルし、それらを簡単に共有できる 
...
[CoreOS](virtual-machines-linux-coreos-how-to.md) は、最適なフット プリントと Docker サポート、および独自のコンテナー システム ([rkt](https://github.com/coreos/rkt)) を備えた Linux ディストリビューションで、[fleet](virtual-machines-linux-coreos-fleet-get-started.md) というコンテナー グループ管理ツールも備えています。
