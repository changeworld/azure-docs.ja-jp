...
Azure Resource Manager での仮想マシンのサポートを発表してからほぼ 1 年になります。ここでは、サポートされている機能の強化点と追加機能について詳しく説明します。さらに、仮想ネットワークへのサイト間ゲートウェイを使用して、サブスクリプションに共存する 2 つのデプロイメント モデルからのリソースの最適な接続および使用方法についても説明します。この記事では、クラシックから Resoure Manager への、サービスとしてのインフラストラクチャ (IaaS) リソースの移行を有効にする方法について説明します。
...
機能面について言うと、クラシック デプロイメント モデルのコンピューティング、ネットワーク、およびストレージ機能のほとんどが、Azure Resource Manager でもサポートされています。Azure Resource Manager でのこの新しい機能と、拡張を続けるデプロイメント ベースに応じて、クラシック デプロイメント モデルの既存のデプロイメントをお客様が移行できるようにします。

>[AZURE.NOTE] 移行サービスのパブリック プレビュー中は、Azure サブスクリプションの運用以外のワークロードのみの移行をお勧めします。
...
パブリック プレビュー中は、主にコンピューティングとネットワークを対象とする 2 つの移行範囲を提供します。シームレスに移行できるように、クラシック ストレージ アカウントを使用して Resource Manager VM 用のディスクを格納できるようになっています。

### 仮想マシンの移行 (仮想ネットワーク外)

Resource Manager デプロイメント モデルでは、既定でアプリケーションのセキュリティを適用します。すべての VM が Resource Manager モデルの仮想ネットワーク内にある必要があります。したがって、移行の一環として、VM を再起動します (`Stop`､`Deallocate`、および`Start`)。仮想ネットワークについては、2 つのオプションがあります。
- まず、新しい仮想ネットワークを作成し、仮想マシンをその新しい仮想ネットワークに移行するようにプラットフォームに要求できます。
- また、Resource Manager の既存の仮想ネットワークに仮想マシンを移行することもできます。

>[AZURE.NOTE] この移行範囲では、移行中のある期間、管理プレーンまたはデータ プレーンのいずれかの操作が許可されない場合があります。

### 仮想マシンの移行 (仮想ネットワーク内)

この範囲では、ほとんどの VM 構成について、クラシック デプロイメント モデルと Resource Manager デプロイメント モデルの間でメタデータのみを移行します。基になる VM は、同じネットワーク内の同じストレージを使用する同じハードウェアで実行されます。したがって、クラシックから Resource Manager へのメタデータの移行の場合、移行中の一定期間は管理プレーンの操作が許可されない可能性があります。ただし、データ プレーンは引き続き機能します。つまり、移行中は VM (クラシック) 上で実行されているアプリケーションでダウンタイムが発生することはありません。
...
>[AZURE.NOTE] この移行範囲では、移行中のある期間、管理プレーンが許可されない場合があります。上記の特定の構成の場合は、データ プレーン ダウンタイムが発生します。

### ストレージ アカウントと移行

このパブリック プレビューではストレージ アカウントの移行はサポートされていません。

シームレスに移行できるように、クラシック ストレージ アカウントで Resource Manager VM をデプロイする機能が有効になっています。この機能により、ストレージ アカウントとは関係なく、コンピューティングおよびネットワーク リソースを移行できます。また、移行する必要があります。
...
パブリック プレビューでは、以下の機能はサポートされていません。必要に応じて、これらの設定を削除し、VM を移行してから、Resource Manager デプロイメント モデルでその設定を再度有効にすることができます。
...
パブリック プレビューでは、以下の構成はサポートされていません。
...
移行のワークフローを次に示します。パブリック プレビューの発表に伴い、REST API、PowerShell、および Azure CLI を使用する移行のトリガー サポートを追加しました。
...
準備操作は、移行プロセスの最初の手順です。この手順の目的は、クラシックから Resource Manager リソースへの IaaS リソースの変換をシミュレートし、これを並行して示し、視覚化することです。

移行を準備する仮想ネットワークまたはホストされるサービス (仮想ネットワークでない場合) を選択します。

最初は、プラットフォームは常に移行対象のリソースのデータをバックグラウンドで分析し、リソースを移行できるかどうかを示すコード (成功/失敗) を返します。
...
問題がある場合は、いつでも移行を中止し、クラシック デプロイメント モデルに戻ることができます。戻ると、リソースに対する管理プレーン操作が開始されるため、クラシック デプロイメント モデルでその VM に対する通常の操作を再開できます。

### 中止

中止は省略可能な手順です。この手順により、クラシック デプロイメント モデルへの変更を元に戻し、移行を中止できます。コミット操作をトリガーしたら、この操作は実行できません。

### コミット

検証が完了したら、移行をコミットできます。リソースは Resource Manager デプロイメント モデルでのみ使用できるようになります。クラシックには表示されません。つまり、移行されたリソースは新しいポータルでのみ管理できます。

この操作に失敗した場合、2 ～ 3 回再試行することをお勧めします。失敗が続く場合は、サポート チケットを作成するか、[VM フォーラム](https://social.msdn.microsoft.com/Forums/azure/ja-JP/home?forum=WAVirtualMachinesforWindows)でフォーラム投稿を作成し、ClassicIaaSMigration タグを付けてください。
...
移行対象のリソースの数によって異なります。小規模なデプロイ (VM が数十台) の場合、移行に要する時間は 1 時間未満です。大規模なデプロイ (VM が数百台) の場合は、移行に数時間かかることがあります。サービスはパブリック プレビュー段階であるため、開発またはテスト サブスクリプションで実行し、その影響を評価することを強くお勧めします。
...
現時点では、移行準備中のリソースについては、準備操作で暗黙的に検証されます。プラットフォームでサポートされる移行オプションでは、移行準備の最初の手順として、リソースを移行できるかどうかを検証します。検証に失敗しても、リソースにはまったく影響はありません。
