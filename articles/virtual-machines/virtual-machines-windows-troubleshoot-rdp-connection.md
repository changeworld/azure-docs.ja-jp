...
この記事は、Windows を実行する Azure 仮想マシンに適用されます。Linux を実行する Azure Virtual Machines の場合は、「[Linux ベースの Azure Virtual Machines に対する Secure Shell (SSH) 接続のトラブルシューティング](virtual-machines-linux-troubleshoot-ssh-connection.md)」を参照してください。
