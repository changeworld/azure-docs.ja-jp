...
[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-rm-include.md)]クラシック デプロイ モデル。クラシック デプロイ モデルを使用してこのリソースをデプロイすることはできません。


Spark は、Mesos や YARN クラスター マネージャーで実行されるだけでなく、単純なスタンドアロン型のデプロイ モードも用意されています。このチュートリアルでは、サンプルの Azure リソース マネージャー テンプレートと、[Azure PowerShell](../powershell-install-configure.md) または [Azure CLI](../xplat-cli-install.md) を使用して、Ubuntu VM に Spark Cluster をデプロイする手順について説明します。
