...
Spark は、Mesos や YARN クラスター マネージャーで実行されるだけでなく、単純なスタンドアロン型のデプロイメント モードも用意されています。このチュートリアルでは、サンプルの Azure リソース マネージャー テンプレートと、[Azure PowerShell](../powershell-install-configure.md) または [Azure CLI](../xplat-cli-install.md) を使用して、Ubuntu VM に Spark Cluster をデプロイする手順について説明します。
