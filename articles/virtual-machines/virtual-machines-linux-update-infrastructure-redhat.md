...
## RHUI がデプロイされているリージョン
RHUI は、[Azure の状態ダッシュ ボード](https://azure.microsoft.com/status/)に一覧されるすべてのパブリック リージョンにデプロイされます。つまり、これらのリージョンで追加料金を支払うことなく、yum 更新サービスを取得できることを意味します。今後、この情報は更新されます。

## オンプレミスの更新リポジトリから更新プログラムを入手する (Red Hat Network Satellite など)

オンプレミスの更新リポジトリから更新プログラムを入手するには、Red Hat Cloud Access ライセンスと Azure に対する既存の Red Hat サブスクリプションを入手する必要があります。

次に、RHUI の登録を解除し、オンプレミスの更新インフラストラクチャを再登録する必要があります。詳細な手順を次に示します。
