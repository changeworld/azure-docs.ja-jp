...
SSH クライアントは、さまざまな理由で VM 上の SSH サービスに到達できない可能性があります。[一般的な SSH のトラブルシューティングの手順](virtual-machines-linux-troubleshoot-ssh-connection.md)に従った場合は、接続問題のトラブルシューティングが必要になります。この記事では、詳細なトラブルシューティング手順を説明し、SSH 接続に失敗した場所の特定とその解決方法を確認します。
...
[Azure クラシック ポータル](https://manage.windowsazure.com)で、クラシック デプロイ モデルを使用して作成された VM について、次の手順を実行します。
...
1. クラシック デプロイ モデルを使用して作成された VM の場合は、**[参照]**、**[仮想マシン (クラシック)]**、*VM 名*の順に選択します。
...
インターネットに直接接続されているコンピューターがない場合は、新しい Azure VM をリソース グループまたはクラウド サービス内に簡単に作成して使用できます。詳細については、「[Azure 上で Linux を実行する仮想マシンの作成](virtual-machines-linux-quick-create-cli.md)」を参照してください。テストの完了後に、そのリソース グループまたは VM と、クラウド サービスを削除します。
