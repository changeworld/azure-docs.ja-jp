...
エラー応答には、常に状態コード、エラー オブジェクトが含まれています。 各エラー オブジェクトには、常にエラー コード、メッセージが含まれています。 VM のデプロイメントをテンプレートから作成すると、エラー オブジェクトに詳細セクションも含まれ、それにはエラー コードとメッセージの内部レベルも含まれます。 通常、エラー メッセージの最も内側のレベルは、ルートの障害です。 


## <a name="common-virtual-machine-management-error"></a>仮想マシンの管理の一般的なエラー

このセクションでは、仮想マシンの管理についての一般的なエラー メッセージを一覧表示します
