...
> [AZURE.NOTE]ここで紹介する例には、リソース マネージャーでのテンプレート ベースの操作は含まれていません。詳細については、「[Using the Azure Command-Line Interface with the Resource Manager (リソース マネージャーでの Azure コマンドライン インターフェイスの使用)](xplat-cli-azure-resource-manager.md)」を参照してください。
