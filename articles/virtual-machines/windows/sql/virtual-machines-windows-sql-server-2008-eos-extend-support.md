...
- **SQL Server のバックアップ**:SQL Server のバックアップを使用して、リージョンまたはゾーンで障害が発生した場合に SQL Server を復旧することができます。 EOS SQL Server ではマネージド バックアップ機能がサポートされていないため、お客様はバックアップを手動で実行する必要があります。
