---
title: Azure Windows 仮想マシンでの接続された VHD のトラブルシューティング | Microsoft Docs
description: Windows VM が予期せず再起動する、接続された VHD を含むストレージ アカウントの削除でエラーが発生する、などの問題のトラブルシューティングを行う方法。
...
ms.locfileid: "30289931"
---
# <a name="troubleshoot-attached-vhds-on-azure-windows-virtual-machines"></a>Azure Windows 仮想マシンでの接続された VHD のトラブルシューティング

Azure Virtual Machines は、OS ディスクおよび接続されたデータ ディスクに仮想ハード ディスク (VHD) を使用します。 VHD は、1 つ以上の Azure ストレージ アカウントにページ BLOB として格納されます。 この記事では、VHD で発生する可能性のある一般的な問題のトラブルシューティングを行う方法について説明します。 

  * [接続された VHD による VM の予期しない再起動]
  * [Resource Manager デプロイメントでのストレージ削除エラー]

## <a name="you-are-experiencing-unexpected-reboots"></a>接続された VHD による VM の予期しない再起動

Azure 仮想マシン (VM) に、同じストレージ アカウントにある VHD が多数接続されている場合、個々のストレージ アカウントのスケーラビリティ ターゲットを超える可能性があり、これは VM が失敗する原因となります。 ストレージ アカウントの分単位メトリック (**TotalRequests**/**TotalIngress**/**TotalEgress**) で、ストレージ アカウントのスケーラビリティ ターゲットを超えるスパイクが発生していないかどうかを調べます。 ストレージ アカウントで調整が行われたかどうかを調べる際には、[メトリックによる PercentThrottlingError の増加の表示] に関するセクションを参照すると役に立ちます。
...
## <a name="storage-delete-errors-in-rm"></a>Resource Manager デプロイメントでのストレージ削除エラー

このセクションでは、Azure Resource Manager デプロイメントで Azure ストレージ アカウント、コンテナー、または BLOB を削除しようとしたときに、次のいずれかのエラーが発生した場合のトラブルシューティングのガイダンスを提供します。
...
Azure VM で使用される VHD は .vhd ファイルです。Azure では Standard または Premium Storage アカウントでページ BLOB としてこれらを格納します。  Azure ディスクについて詳しくは、[こちら](../../virtual-machines/windows/about-disks-and-vhds.md)をご覧ください。 Azure では、破損を防ぐため、VM に接続されているディスクは削除できません。 また、VM に接続されているページ BLOB があるコンテナーおよびストレージ アカウントも削除できません。 
...
[接続された VHD による VM の予期しない再起動]: #you-are-experiencing-unexpected-reboots
[Resource Manager デプロイメントでのストレージ削除エラー]: #storage-delete-errors-in-rm