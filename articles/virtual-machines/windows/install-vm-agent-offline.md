...
2.  OS ディスクをデータ ディスクとして別の VM ("_トラブルシューティング ツール_" VM と呼ばれます) に接続します。 詳細については、[Azure Portal での Windows VM へのデータ ディスクの接続](attach-managed-disk-portal.md)に関するページをご覧ください。
...
7.  VM エージェントが動作していない場合は、現在の構成をバックアップします。

    >[!NOTE]
    >VM にエージェントがインストールされていない場合は、手順 8. に進みます。 
...
8.  トラブルシューティング ツール VM 上の既存のファイルを、VM エージェントのインストール用リポジトリとして使用します。 次の手順を完了します。
    ...
    2. レジストリ ファイルを編集します。 各ファイルで、(次の図に示すように) エントリ値 **SYSTEM** を **BROKENSYSTEM** に変更し、ファイルを保存します。
...
9.  C:\windowsazure\packages の VM エージェント フォルダーを、&lt;接続した OS ディスク&gt;: \windowsazure\packages にコピーします。

    ![VM エージェント ファイルを OS ディスクにコピーする](./media/install-vm-agent-offline/copy-package.png)
      
    >[!NOTE]
    >**logs** フォルダーをコピーしないでください。 サービスが起動したら、新しいログが生成されます。

10.  **[BROKENSYSTEM]** を選択します。 メニューから、**[ファイル]** > **[ハイブのアンロード]** を選択します。

11.  **[BROKENSOFTWARE]** を選択します。 メニューから、**[ファイル]** > **[ハイブのアンロード]** を選択します。

12.  OS ディスクの接続を解除してから、OS ディスクを使用して VM を再作成します。

13.  VM にアクセスします。 RdAgent が実行されていて、ログが生成されていることがわかります。

クラシック デプロイ モデルを使用して VM を作成した場合は、これで完了です。
