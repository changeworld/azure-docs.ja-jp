...
```
詳しくは、「[Azure PowerShell Versioning](/powershell/azure/overview)」(Azure PowerShell のバージョン管理) をご覧ください。


## <a name="prepare-the-vm"></a>VM を準備する
 
新しい VM を作成するために特殊化された VHD をそのまま使用する場合は、以下のステップが完了していることを確認します。 
  * Managed Disks を使用する場合は、「[Plan for the migration to Managed Disks](on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)」(Managed Disks への移行の計画) をご確認ください。
...
特殊化された OS には、[Azure にアップロード](upload-image.md)した VHD、または[既存の Azure VM の VHD のコピー](vhd-copy.md)を使用できます。 
...
省略可能: 追加の管理ディスクをデータ ディスクとして接続します。 このオプションは、[管理データ ディスクの作成](create-managed-disk-ps.md)に関する記事に従って管理データ ディスクを作成したと想定しています。 
