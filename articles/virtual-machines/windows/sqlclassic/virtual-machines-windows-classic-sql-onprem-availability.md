...
> [!IMPORTANT]
> **Azure レプリカの追加ウィザード**でサポートされるのは、クラシック デプロイメント モデルで作成された仮想マシンのみです。 新しい VM のデプロイでは、新しい Resource Manager モデルを使用する必要があります。 Resource Manager で VM を使用する場合は、Transact-SQL コマンド (ここには示されていません) を使用して、セカンダリ Azure レプリカを手動で追加する必要があります。 このウィザードは、Resource Manager のシナリオでは機能しません。
