...
この例では、管理対象ディスクとしてデータ ディスクを作成し、後で使うために **$dataDisk2** パラメーターに割り当てます。 空のデータ ディスクは、実行中の VM に接続された後、VM にログインして diskmgmt.msc を使うか、または [WinRM とスクリプトをリモートで使って](attach-disk-ps.md#initialize-the-disk)、初期化する必要があります。
...
- 既に VM がある場合は、[データ ディスクを接続](attach-disk-portal.md)できます。
