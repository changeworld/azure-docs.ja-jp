...
> [!IMPORTANT]
> 初めて VHD を Azure にアップロードする前に Sysprep を実行する場合、Sysprep の実行前に [VM の準備](prepare-for-upload-vhd-image.md)ができていることを確認してください。 
> 
> 
...
* VM がオンプレミスの場合、[VHD を Azure にアップロード](upload-image.md)できるようになりました。
* VM が既に Azure に存在する場合、[一般化された VM からイメージを作成](capture-image.md)できるようになりました。
