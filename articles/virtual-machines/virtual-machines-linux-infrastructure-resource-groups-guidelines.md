...
リソース グループで重要なのは、ストレージ、ネットワーク、およびコンピューティング リソースを宣言する JSON ファイルを使用して、環境を構築する機能です。適用する関連カスタム スクリプトや構成を定義することもできます。これらの JSON テンプレートを使用して、一貫性があり、再現可能なアプリケーションのデプロイを作成します。このアプローチにより、開発環境を構築し、同じテンプレートを使用して運用環境のデプロイを作成できます。また、その逆も可能です。テンプレートの使用に関する理解を深めるには、[テンプレートのチュートリアル](../resource-manager-template-walkthrough.md)に関するページをご覧ください。このページでは、JSON テンプレートの構築の手順について説明しています。
