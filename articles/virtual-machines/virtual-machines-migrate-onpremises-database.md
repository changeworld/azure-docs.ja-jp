...
> [AZURE.NOTE]オンプレミスの AlwaysOn デプロイメントが使用できる場合は、[Azure のレプリカ追加ウィザード](virtual-machines-sql-server-extend-on-premises-alwayson-availability-groups.md)から Azure でレプリカを作成し、移行方法としてフェールオーバーすることもできます。
...
## Azure VM のデプロイメント ウィザードのチュートリアル
...
9. [デプロイメントの設定] ページでは、新規または既存のクラウド サービス名と仮想マシン名を指定できます。
