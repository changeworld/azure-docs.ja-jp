...
>[AZURE.TIP]Azure での完全な HPC Pack クラスターの運用デプロイの場合は、[HPC Pack IaaS デプロイメント スクリプト](virtual-machines-hpcpack-cluster-powershell-script.md)や、リソース マネージャー テンプレート (例: [Windows ワークロードの HPC Pack クラスター](https://azure.microsoft.com/marketplace/partners/microsofthpc/newclusterwindowscn/) テンプレート) などの自動化手法を利用することをお勧めします。その他のテンプレートについては、「[Microsoft HPC Pack を使用して Azure でハイパフォーマンス コンピューティング (HPC) クラスターを作成および管理するためのオプション](virtual-machines-hpcpack-cluster-options.md)」を参照してください。
...
4. ポータルを使用して設定を構成し、VM を作成します。詳細な手順については、チュートリアル「[Azure ポータルで Windows 仮想マシンを作成する](virtual-machines-windows-tutorial.md)」をご覧ください。初めてのデプロイの場合は、既定値や推奨される設定を選択することをお勧めします。

    **VNet の考慮事項**

   * **[設定]** で新しい VNet を作成する場合は、プライベート ネットワーク アドレスの範囲 (例: 10.0.0.0/16) とサブネット アドレスの範囲 (例: 10.0.0.0/24) を指定します。
    
4. VM を作成して実行したら、[その VM に接続](virtual-machines-log-on-windows-server-preview.md)します。 
...
* クラウド サービスに [Azure バースト ノード](virtual-machines-hpcpack-cluster-node-burst.md)を追加して、オンデマンドのクラスター コンピューティング能力を向上させます。 
