<properties 
   pageTitle="Azure Virtual Machines での SQL Server の自動バックアップ"
...
>[AZURE.NOTE]自動バックアップを初めて有効にすると、バックグラウンドで SQL Server IaaS エージェントが構成されます。この間、ポータルには自動バックアップが構成されていることは示されません。エージェントがインストールされ、構成されるまで数分待ちます。その後、ポータルに新しい設定が反映されます。
