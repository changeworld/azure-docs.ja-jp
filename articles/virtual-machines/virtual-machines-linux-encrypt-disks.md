...
---
# <a name="encrypt-disks-on-a-linux-vm-using-the-azure-cli"></a>Azure CLI を使って Linux VM のディスクを暗号化する
仮想マシン (VM) のセキュリティと法令遵守を強化するため、Azure の仮想ディスクに保存されるデータを暗号化できます。 ディスクは、Azure Key Vault で保護されている暗号化キーを使って暗号化されます。 これらの暗号化キーを制御し、その使用を監査することができます。 この記事では、Azure CLI と Resource Manager デプロイメント モデルを使って、Linux VM の仮想ディスクを暗号化する方法を詳しく説明します。
