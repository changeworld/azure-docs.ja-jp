...
* VM で実行されるサーバー ロールが sysprep をサポートしていることを確認します。詳しくは、「[Sysprep Support for Server Roles](https://msdn.microsoft.com/windows/hardware/commercialize/manufacture/desktop/sysprep-support-for-server-roles)」 (サーバー ロールに対する Sysprep サポート) をご覧ください。Sysprep を実行する前に、VM をバックアップします。
...
> [!NOTE]
> VM は、アップロードした VHD ファイルと同じストレージ アカウント内にある必要があります。
> 
> 
