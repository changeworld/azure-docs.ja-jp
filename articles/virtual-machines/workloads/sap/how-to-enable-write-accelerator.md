...
>[!NOTE]
> 現在、Azure 書き込みアクセラレータはパブリック プレビューであり、Azure サブスクリプション ID をホワイト リストに登録する必要があります

Azure 書き込みアクセラレータ機能は、次のリージョンでパブリック プレビューとして使用できます。
...
> [!IMPORTANT]
> VM の Azure オペレーティング システム ディスクで書き込みアクセラレータを有効にすると、VM が再起動します。 
...
- Premium ディスクのキャッシュを "なし" に設定する必要があります。 他のすべてのキャッシュ モードはサポートされていません。
...
次に、JSON ファイルを更新して、"log1" という名前のディスクで書き込みアクセラレータを有効にします。 これは、JSON ファイルでディスクのキャッシュ エントリの後に次の属性を追加することによって行うことができます。 
