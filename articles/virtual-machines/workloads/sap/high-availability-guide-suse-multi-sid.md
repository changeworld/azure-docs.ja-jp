...
この記事では、SUSE Linux Enterprise Server for SAP Applications を使用して、Azure VM の 2 ノード クラスターに複数の SAP NetWeaver 高可用性システム (つまりマルチ SID) をデプロイする方法について説明します。  
...
* [マルチ SID クラスターに対する SUSE のサポート](https://www.suse.com/c/sap-workloads-going-green/)
...
* フロントエンドの構成
  ...
  * NW2 の IP アドレス: 10.3.1.17
  * NW3 の IP アドレス: 10.3.1.19
* バックエンドの構成
  * (A)SCS/ERS クラスターに含める必要のあるすべての仮想マシンのプライマリ ネットワーク インターフェイスに接続済み
* プローブ ポート
...
8. **[1]** 新しくインストールした SAP システム用の SAP クラスター リソースを作成します。 
   ...
   ここで示す例は、SAP システム **NW2** と **NW3** に対するものです。エンキュー サーバー 1 アーキテクチャ (ENSA1) を使用しているものとします。
