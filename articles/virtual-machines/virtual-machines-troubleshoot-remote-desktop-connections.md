<properties 
	...
	description="Azure 仮想マシンへのリモート デスクトップ (RDP) 接続を復元する方法について、診断方法や問題の原因を特定する手順をまじえて説明します。"
...
このトピックでは、Windows ベースの Azure 仮想マシンへのリモート デスクトップ接続に関する、問題の修正や根本原因を突き止めるための系統的アプローチについて説明します。
...
表示されたエラー メッセージに合わせて、以下のセクションを参照してください。
...
一時的な回避策として、Azure 管理ポータルにある RDP ファイルのローカル コピーを保存し、Windows PowerShell コマンド プロンプトで次のコマンドを実行して接続します。
...
Azure 管理ポータルによって生成された RDP ファイルの例を次に示します。
...
- ローカルに格納された RDP ファイルを使用している場合は、正しいクラウド サービス名と仮想マシンのエンドポイント ポートが確実に含まれている、Azure 管理ポータルで生成された RDP ファイル使用してください。
...
- Azure 管理ポータルまたは Azure プレビュー ポータル内の仮想マシンの状態を確認します。
...
インターネットに直接接続されているコンピューターがない場合は、新しい Azure 仮想マシンをクラウド サービス内に簡単に作成して使用できます。詳細については、「[Azure 上で Windows を実行する仮想マシンの作成](virtual-machines-windows-tutorial.md)」を参照してください。テストの完了後に、その仮想マシンとクラウド サービスを削除します
...
問題のソースとなっている、または構成が正しくないという理由でクラウド サービス エンドポイントと ACL を排除する場合、 同じクラウド サービス内にある別の Azure 仮想マシンから自身の Azure 仮想マシンにリモート デスクトップ接続できるかを確認します。

![](./media/virtual-machines-troubleshoot-remote-desktop-connections/tshootrdp_3.png)
 
同じクラウド サービス内に別の仮想マシンがいない場合、新しい仮想マシンを簡単に作成することができます。詳細については、「[Azure 上で Windows を実行する仮想マシンの作成](virtual-machines-windows-tutorial.md)」を参照してください。テストの完了後に、追加した仮想マシンを削除してください。
...
詳細については、「[ネットワーク セキュリティ グループについて](https://msdn.microsoft.com/library/azure/dn848316.aspx)」をご覧ください。
...
これらの考えられる問題を修正するには、リモート PowerShell セッションを Azure 仮想マシンに使用します。最初に、仮想マシンをホストするクラウド サービスの証明書をインストールする必要があります。「[Azure 仮想マシンへの安全なリモート PowerShell アクセスを構成する](http://gallery.technet.microsoft.com/scriptcenter/Configures-Secure-Remote-b137f2fe)」を参照し、**InstallWinRMCertAzureVM.ps1** スクリプト ファイルを、ローカル コンピューター上のフォルダーにダウンロードします。
...
プロンプトの最初の部分は、ターゲットの仮想マシンを含むクラウド サービスに PowerShell コマンドを発行していることを示しています。cloudservice4testing.cloudapp.net はクラウド サービス名ではありません。

これで PowerShell コマンドを発行して、前述したその他の問題を調査して構成を修正することができます。
...
次のコマンドで、ポートが 3389 に変更されたことを確認します。

	Get-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" -Name "PortNumber"

次のコマンドで、リモートの PowerShell セッションを終了します。
