...
---
# <a name="using-load-balanced-sets-to-clusterize-mysql-on-linux"></a>負荷分散セットを使用して Linux の MySQL をクラスター化する
> [!IMPORTANT] 
> Azure には、リソースの作成と操作に関して、 [Resource Manager とクラシック](../azure-resource-manager/resource-manager-deployment-model.md)の 2 種類のデプロイメント モデルがあります。 この記事では、クラシック デプロイ モデルの使用方法について説明します。 最新のデプロイでは、リソース マネージャー モデルを使用することをお勧めします。 MySQL クラスターをデプロイするための Resource Manager テンプレートについては、[こちら](https://azure.microsoft.com/documentation/templates/mysql-replication/)を参照してください。
...
## <a name="setting-up-the-cluster"></a>クラスターのセットアップ
両方の Ubuntu VM に Corosync、Pacemaker、および DRBD をインストールするには、APT を使用する必要があります。 `apt-get`コマンドを使用します。

    sudo apt-get install corosync pacemaker drbd8-utils.

**** の MySQL のデータ ディレクトリを初期化します。 Debian および Ubuntu のインストール スクリプトは、`/var/lib/mysql` の MySQL のデータ ディレクトリを初期化します。しかし、このディレクトリは DRBD のファイルシステムに置き換わるため、MySQL は後でインストールする必要があります。
...
### <a name="setting-up-drbd"></a>DRBD のセットアップ
...
### <a name="mounting-the-drbd-resource"></a>DRBD リソースのマウント
...
## <a name="setting-up-mysql"></a>MySQL のセットアップ
...
### <a name="creating-the-mysql-load-balanced-set"></a>MySQL 負荷分散セットの作成
...
### <a name="testing-the-load-balanced-set"></a>負荷分散セットのテスト
...
## <a name="setting-up-corosync"></a>Corosync のセットアップ
...
> [!NOTE]
> 次に示す `transport: udpu` ディレクティブと手動で定義したノードの IP アドレス**。
...
## <a name="setting-up-pacemaker"></a>Pacemaker のセットアップ
