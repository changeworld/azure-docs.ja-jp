...
ms.locfileid: "35372941"
---
# <a name="tutorial-single-page-web-app"></a>チュートリアル: 単一ページの Web アプリ

これは、Bing Image Search に関する[単一ページ アプリのチュートリアル](tutorial-bing-image-search-single-page-app.md)で説明した完全なソース コードです。 アプリを実行するには、メモ帳または別のテキスト エディターにソース コードをコピーし、`bing.html` として保存します。 Microsoft Edge または他の一般的なブラウザーで、保存したファイルを開きます。
