...
Git Hub に [C# Bing Image Search SDK のサンプル ソース コード](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7/BingImageSearch)があります。

## <a name="application-dependencies"></a>アプリケーションの依存関係

Bing Image Search SDK を使用してコンソール アプリケーションを設定するには、Visual Studio のソリューション エクスプローラーで `Manage NuGet Packages` オプションを参照します。  `Microsoft.Azure.CognitiveServices.Search.ImageSearch` パッケージを追加します。

[NuGet Image Search パッケージ](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.Search.ImageSearch/1.2.0)をインストールすると、次の項目を含む依存関係もインストールされます。
* Microsoft.Rest.ClientRuntime
* Microsoft.Rest.ClientRuntime.Azure
* Newtonsoft.Json

## <a name="image-search-client"></a>Image Search クライアント
`ImageSearchAPI` クライアントのインスタンスを作成するには、次のディレクティブを追加します。
```
...
次のコンソール アプリケーションは、上記で定義されたクエリ "Canadian Rockies" を実行して結果を求め、最初の画像の分析情報トークン、サムネイル URL、画像コンテンツ URL を印刷します。
...
Bing 検索のサンプルでは、SDK のさまざまな機能について説明します。  以前に定義された `ImageSrchSDK` クラスに、次の関数を追加します。

### <a name="search-using-a-filter"></a>フィルターを使用して検索する

"studio ghibli" の画像を検索し、アニメーション Gif と広アスペクトで絞り込み、結果数を確認し、最初の結果の分析情報トークン、サムネイル URL、URL を印刷します。
...
### <a name="trending-images"></a>注目の画像

注目の画像を検索し、カテゴリとタイルを確認します。
...
### <a name="image-details"></a>画像詳細

"Degas" で画像を検索し、最初の画像の画像詳細を検索します。
```
