...
> [!NOTE]
> このガイドのコード スニペットでは、URL でアクセスされるリモート フォームが使用されます。 ローカル フォーム ドキュメントを代わりに処理する場合は、[リファレンス ドキュメント](https://docs.microsoft.com/javascript/api/overview/azure/formrecognizer?view=azure-node-preview)の関連するメソッドを参照してください。
...
この関数は、ヘルパー関数 `optionalToString` を使用します。 スクリプトのルートでこの関数を定義します。
...
**beginRecognizeFormsFromUrl** メソッドを使用します。 返される値は **RecognizedForm** オブジェクトのコレクションで、送信されたドキュメント内のページごとに 1 つです。
