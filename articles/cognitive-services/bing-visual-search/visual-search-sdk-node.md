...
Bing Visual Search SDK を使用してコンソール アプリケーションを設定するには:
* `npm install ms-rest-azure` を実行します。
* `npm install azure-cognitiveservices-search-visualSearch` を実行します。
