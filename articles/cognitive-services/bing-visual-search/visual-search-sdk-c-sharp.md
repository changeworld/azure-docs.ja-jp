...
* .NET コア SDK、.net コア 1.1 アプリを実行する機能。 https://www.microsoft.com/net/download/ から、Core、Framework、およびランタイムを 入手できます。
...
Bing 検索のサンプルでは、SDK のさまざまな機能について説明します。  以前に定義された `VisualSrchSDK` クラスに、次の関数を追加してください。

<a name="binary-crop"></a>
## <a name="image-binary-post-with-croparea"></a>cropArea を使ったイメージ バイナリ転記

次のコードは、転記要求の本文にあるイメージ バイナリを、cropArea オブジェクトと共に送信します。  その後、imageInsightsToken、タグの数、アクションの数、および最初の actionType を出力します。
...
次のコードは、`knowledgeRequest` パラメーター内のイメージ URL を、\"site:www.bing.com\" フィルターと共に送信します。  その後、`imageInsightsToken`、タグの数、アクションの数、および最初の actionType を出力します。
```
...
## <a name="tags-actions-and-actiontype"></a>タグ、アクション、および actionType
...
[Cognitive Services .NET SDK のサンプル](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7)。