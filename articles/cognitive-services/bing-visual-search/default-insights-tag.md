---
title: 既定の分析情報タグ - Bing Visual Search
...
既定の分析情報タグは、空の文字列が `displayName` フィールドに設定された分析情報タグです。 次の例は、考えられる既定の分析情報 (アクション) の一覧を示しています。 応答に含まれるアクションの一覧は、画像によって異なります。 さらに、アクションごとに、プロパティの一覧が画像によって異なる可能性があるため、プロパティを使用しようとする前に、そのプロパティが存在しているかどうかを確認してください。
...
## <a name="shoppingsources-insight"></a>ShoppingSources 分析情報

ShoppingSources 分析情報は、画像に表示されたアイテムをユーザーが購入できる Web サイトの一覧を示します。 提示された一覧には、ユーザーがそのアイテムを購入できる Web ページの URL、アイテムの価格、評価やレビューの詳細が含まれています。 使用例については、[ShoppingSources の例](./bing-insights-usage.md#shoppingsources-insight-example)を参照してください。
