...
## <a name="prerequisite-determine-the-application-id"></a>前提条件: アプリケーション ID を決定する 

Conversation Learner UI の外部でボットを実行するには、ボットが使用する Conversation Learner アプリケーション ID、つまり、Conversation Learner クラウドの機械学習モデルの ID を設定する必要があります   (これに対し、Conversation Learner UI 経由でボットを実行する場合は、UI がアプリケーション ID を選択します)。  

アプリケーション ID を取得する方法を次に示します。
...
3. ID を取得する Conversation Learner アプリケーションをクリックします。

4. 左側のナビゲーション バーにある [設定] をクリックします。

5. ページの上部付近に、"アプリ ID" の GUID が表示されます。
...
3. Azure Portal で、先ほど作成した Web App Bot リソースを編集します。
    ...
    2. 以下の設定を追加します
        ...
        CONVERSATION_LEARNER_APP_ID      | アプリケーション ID の GUID。アプリの [設定] 下にある Conversation Learner UI から取得する
        LUIS_AUTHORING_KEY               | このアプリの LUIS オーサリング キー
