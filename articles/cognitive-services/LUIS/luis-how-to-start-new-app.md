...
1. **[My Apps]/(マイ アプリ/)** ページで、自分のサブスクリプション、作成リソース、 **[+ Create]/(+ 作成/)** の順に選択します。 無料試用版キーを使用している場合、[作成リソースの作成方法](luis-how-to-azure-subscription.md#create-resources-in-the-azure-portal)について学習してください。
...
1. アプリケーション カルチャを選択し、 **[Done]/(完了/)** を選択します。 説明と予測リソースは現時点では任意です。 ポータルの **[Manage]/(管理/)** セクションでいつでも設定できます。

    > [!NOTE]
    > カルチャは、アプリケーションを作成した後に変更できません。 

    アプリが作成されると、LUIS ポータルに **[Intents]/(意図/)** 一覧が表示されます。この一任には、自動的に作成された `None` 意図が含まれています。 これで空のアプリが作成されました。 
