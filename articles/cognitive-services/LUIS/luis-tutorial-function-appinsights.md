...
* **[前のチュートリアル](luis-nodejs-tutorial-build-bot-framework-sample.md)** の Application insights を使用した LUIS Web アプリ ボットがオンになっている。 
...
LUIS エントリを参照するために Application Insights を開きます。 

1. ポータルで、**[すべてのリソース]** を選択し、Web アプリ ボット名でフィルター処理します。 **Application Insights** 型のリソースをクリックします。 Application Insights のアイコンは、電球マークです。 

    ![App Insights を検索する](./media/luis-tutorial-appinsights/search-for-app-insights.png)
...
3. 最上位のエントリを選択します。 新しいウィンドウに、右端にある LUIS クエリのカスタム データを含む詳細なデータが表示されます。 データには、最上位の意図とそのスコアが含まれています。
...
> [!Tip]
> 依存関係の一覧を保存して後でそれに戻る場合は、**[...詳細]** をクリックし、**[お気に入りの保存]** をクリックします。

## <a name="query-application-insights-for-intent-score-and-utterance"></a>Application Insights で意図、スコア、および発話のクエリを実行する
Application Insights を使用すると、[Kusto](https://docs.microsoft.com/azure/application-insights/app-insights-analytics#query-data-in-analytics) 言語を使用してデータを照会したり、[PowerBI](https://powerbi.microsoft.com) にエクスポートしたりすることができます。 

1. フィルター ボックスの上の依存関係の一覧の一番上にある **[Analytics]** をクリックします。 

    ![[Analytics] ボタン](./media/luis-tutorial-appinsights/analytics-button.png)

2. 新しいウィンドウが開き、上部のクエリ ウィンドウとその下のデータ テーブル ウィンドウが表示されます。 前にデータベースを使用している場合、この配置は使い慣れています。 クエリには、`LUIS-results` という名前で始まる過去 24 時間のすべての項目が含まれています。 **CustomDimensions** 列には、名前/値のペアとして LUIS クエリの結果が含まれています。
...
4. クエリを実行します。 データ テーブルの右端までスクロールします。 topIntent、スコア、および発話の新しい列を利用できます。 並べ替える topIntent 列をクリックします。

    ![Analytics の最上位の意図](./media/luis-tutorial-appinsights/app-insights-top-intent.png)


[Kusto クエリ言語](https://docs.loganalytics.io/docs/Learn/Getting-Started/Getting-started-with-queries)または [PowerBi へのデータのエクスポート](https://docs.microsoft.com/azure/application-insights/app-insights-export-power-bi)の詳細を参照してください。 

## <a name="next-steps"></a>次の手順

アプリ ID、バージョン ID、モデルの最終変更日、前回のトレーニング日、最後の公開日などの Application Insights データに追加したいその他の情報があります。 これらの値をエンドポイントの URL (アプリ ID とバージョン ID) から、または [Authoring API](https://westus.dev.cognitive.microsoft.com/docs/services/5890b47c39e2bb17b84a55ff/operations/5890b47c39e2bb052c5b9c3d) の呼び出しから取得して、Web アプリ ボットの設定に設定し、そこから取得できます。  

複数の LUIS アプリで同じエンドポイント サブスクリプションを使用している場合は、サブスクリプション ID および共有キーであることを示すプロパティも含める必要があります。 
