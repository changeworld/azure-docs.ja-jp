...
ms.locfileid: "73828258"
---
# <a name="tutorial-extract-structured-data-with-machine-learned-entities-in-language-understanding-luis"></a>チュートリアル:Language Understanding (LUIS) で機械学習エンティティを使用して構造化データを抽出する
...
ここでは、ピザ アプリの注文を表す機械学習エンティティを作成します。 注文には、注文を満たすために必要なすべてのパーツが含まれている必要があります。 まず、エンティティには、注文関連のすべてのテキスト、特にサイズと数量を含めます。 

`deliver one large cheese pizza` の発話の場合、発話全体を注文として抽出した後、`1` と `large` も抽出する必要があります。 

トッピングや生地など、さらに分解することが可能です。 このチュートリアルを完了したら、自信を持ってこれらのサブコンポーネントを自身の既存の `Order` エンティティに追加できるようになるはずです。
...
1. **[管理]** セクションの **[バージョン]** タブでバージョンを選択し、 **[複製]** を選択してそのバージョンを複製し、それに `mach-learn` という名前を付けます。 次に、 **[完了]** を選択して複製プロセスを完了します。 バージョン名は URL ルートの一部として使用されるため、URL 内で有効ではない文字を名前に含めることはできません。

    > [!TIP] 
    > ベスト プラクティスとして、アプリを変更する前に複製することをお勧めします。 あるバージョンが完成したら、そのバージョンを .json または .lu ファイルとしてエクスポートし、お使いのソース管理にチェックインします。

1. **[ビルド]** を選択してから **[Intents]\(意図\)** を選択し、LUIS アプリのメインの構成要素である意図を確認します。
...
1. 発話の例の一覧で、次の発話を選択します。 
    ...
    > [!NOTE]
    > エンティティが常に発話全体であるとは限りません。 この例では、`pickup` は注文の受け取り方法を示しているため、注文のラベル付けされたエンティティの一部である必要があります。 

1. **[Choose an entity type]\(エンティティ型の選択\)** ボックスで、 **[Add Structure]\(構造の追加\)** を選択し、 **[次へ]** を選択します。 構造は、サイズや数量などのサブコンポーネントを許可するために必要です。
...
1. 残りの発話で、Size エンティティと共に `Order` エンティティのラベルを付けます。 角かっこ内のテキストは、ラベル付けされた `Order` エンティティと、その内の `Size` エンティティを示しています。
    ...
    > [!CAUTION]
    > 文字 `a` が 1 枚のピザを暗示している場合など、暗黙的なデータはどのように扱えばよいのでしょうか。 または、ピザの受け取り方法を示す `pickup` や `delivery` が不足している場合や、 既定のサイズ、スモール、ミディアムなどを示すサイズが不足している場合などです。 暗黙的なデータの処理は、クライアント アプリケーションでビジネス ルールの一部として扱うことを検討してください。 

1. アプリをトレーニングするには、 **[トレーニング]** を選択します。 トレーニングにより、新しいエンティティやラベル付けされた発話などの変更が、アクティブなモデルに適用されます。

1. トレーニング後、発話の新しい例を追加すると、LUIS が機械学習エンティティをどの程度理解しているかがわかります。 
...
1. [change the prediction into a labeled entity]\(予測をラベル付けされたエンティティに変更\) で、行を選択し、 **[Confirm entity predictions]\(エンティティ予測を確定\)** を選択します。
    ...
    この時点で、機械学習エンティティは新しい発話例内でエンティティを見つけることができているため、正常に機能しています。 発話の例を追加したとき、エンティティが正しく予測されない場合は、エンティティとサブコンポーネントにラベルを付けます。 エンティティが正しく予測された場合は、必ず予測を確定してください。 

## <a name="add-prebuilt-number-to-app-to-help-extract-data"></a>データ抽出に役立つ事前構築済みの数をアプリに追加する
...
## <a name="label-example-utterance-with-subcomponent-for-quantity-to-teach-luis-about-the-entity"></a>数量のサブコンポーネントを使用して発話の例にラベルを付けて、LUIS にエンティティについて学習させる
