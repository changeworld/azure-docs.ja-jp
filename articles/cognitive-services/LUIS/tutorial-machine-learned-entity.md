...
1. **[管理]** セクションの **[バージョン]** タブでバージョンを選択し、 **[Clone]\(クローン\)** を選択してそのバージョンをクローンし、それに `mach-learn` という名前を付けます。次に、 **[完了]** を選択してクローン プロセスを終了します。 バージョン名は URL ルートの一部として使用されるため、URL 内で有効ではない文字を名前に含めることはできません。
