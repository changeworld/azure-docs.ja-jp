...
1. [プレビュー の LUIS ポータルl](https://preview.luis.ai)上で、アプリの **[ビルド]** セクション内にある **[+ 作成]** を選択します。 下の一覧に表示された新しい意図の名前を入力して、 **[完了]** を選択します。
...
1. アドレス バー上の URL の末尾に移動して、以下を入力します。
    ...
    最後のクエリ文字列パラメーターは `q` です。これは発話の**クエリ**です。 この発話は、どの発話例とも同じではありません。 これは適切なテストであり、最上位のスコアリング意図として `OrderPizza` 意図を返すはずです。 
    ...
    現在このアプリにはまったくエンティティがないため、エンティティの配列は空になっています。 
...
> [このアプリに事前構築済みの意図とエンティティを追加する](tutorial-machine-learned-entity.md)
