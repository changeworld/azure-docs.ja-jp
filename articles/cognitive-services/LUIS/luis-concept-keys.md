...
description: LUIS では、オーサリングとエンドポイントの 2 つのキーを使用します。 オーサリング キーは、LUIS アカウントの作成時に自動的に作成されます。 LUIS アプリを公開する準備ができたら、エンドポイント キーを作成して LUIS アプリに割り当て、これをエンドポイント クエリで使用する必要があります。
