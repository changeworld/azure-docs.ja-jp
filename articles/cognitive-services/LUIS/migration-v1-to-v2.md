...
サブスクリプション キー API は非推奨になり、410 GONE が返されます。
...
Azure portal で Azure [サブスクリプション キー](luis-how-to-azure-subscription.md)が生成されます。 そのキーは、**[[Publish]\(公開\)](manage-keys.md)** ページで LUIS アプリに割り当てます。 実際のキー値を確認する必要はありません。 LUIS では、サブスクリプション名を使用して、割り当てが行われます。 
...
LUIS では、モデルを強化する可能性がある既存の[エンドポイント発話](label-suggested-utterances.md)から、発話が提案されます。 以前のバージョンでは、その提案は **sample** でした。 新しいバージョンでは、名前が sample から **suggest** に変更されています。 これは、LUIS Web サイトでは**[エンドポイントの発話の確認](https://docs.microsoft.com/azure/cognitive-services/LUIS/label-suggested-utterances)** と呼ばれます。
...
[事前構築済みのエンティティ](Pre-builtEntities.md)は変更されました。 V2 の事前構築済みのエンティティが使用されていることを確認してください。 これには、datetime の代わりの [datetimeV2](pre-builtentities.md?#use-a-prebuilt-datetimev2-entity) の使用も含まれます。 
