...
## <a name="what-is-a-feature"></a>特徴とは

特徴とは、関数 f(x) = y として記述できる特徴的な特性のことです。 この特徴を使用して、特性を区別するために、発話の例のどこを調べるかが判別されます。 スキーマを作成するときに、特性を示す発話の例について何が判明しているでしょうか。 その回答が、特徴を作成するための最適なガイドとなります。

## <a name="types-of-features"></a>特徴の種類

 LUIS は、語句一覧とモデルの両方を特徴としてサポートしています。
* フレーズ リストの特徴
* 特徴としてのモデル (意図またはエンティティ)

特徴は、スキーマ設計の必要な部分と考える必要があります。

## <a name="how-you-find-features-in-your-example-utterances"></a>発話の例から特徴を見つける方法

LUIS は言語ベースのアプリケーションであるため、特徴はテキストベースになります。 区別する特性を示すテキストを選択します。 LUIS の場合、テキストベースの最小単位はトークンです。 英語の場合、トークンは、スペースや句読点を含まない、文字と数字の連続するスパンです。 スペースはトークンではありません。

スペースと句読点はトークンではないため、特徴として使用できるテキストのヒントに焦点を当てます。 次のような単語のバリエーションを含めることを忘れないでください。
* 複数形
* 動詞の時制
* 省略形
* スペルとスペルミス

このテキストは、特徴的な特性として、次を満たす必要があるかどうか。
* 完全な単語または語句に一致する - 正規表現エンティティを追加するか、またはリスト エンティティを特徴としてエンティティまたは意図に追加することを検討してください
* 日付、時刻、または人名などのよく知られた概念と一致する - 構築済みのエンティティを特徴としてエンティティまたは意図に使用します
* 時間の経過と共に新しい例を学習する - エンティティまたは意図に対する特徴としての概念の例の語句リストを使用します。

## <a name="combine-features"></a>特徴を結合する

特性の説明方法にはいくつかの選択肢があるため、その特性または概念を説明するのに役立つ複数の特徴を使用できます。 一般的な組み合わせとしては、語句一覧の特徴と、特徴として一般的に使用されるエンティティ型の 1 つ (事前構築されたエンティティ、正規表現エンティティ、またはリスト エンティティ) を使用します。

### <a name="ticket-booking-entity-example"></a>チケット予約エンティティの例

最初の例として、フライト予約の意図とチケット予約エンティティを使用してフライトを予約するアプリを考えてみましょう。

チケット予約エンティティは、機械学習されたフライト先のエンティティです。 場所を抽出するために、次の 2 つの特徴を使用します。
* `plane`、`flight`、`reservation`、`ticket` などの関連する単語の語句一覧
* エンティティに対する特徴として、事前構築された `geographyV2` エンティティ

### <a name="pizza-entity-example"></a>ピザ エンティティの例

別の例として、ピザの注文作成の意図とピザ エンティティを使用したピザ注文アプリを考えてみましょう。

ピザ エンティティは、ピザの詳細の機械学習されたエンティティです。 詳細を抽出するために、次の 2 つの特徴を使用します。
* `cheese`、`crust`、`pepperoni`、`pineapple` などの関連する単語の語句一覧
* エンティティに対する特徴として、事前構築された `number` エンティティ

## <a name="a-phrase-list-for-a-particular-concept"></a>特定の概念のフレーズ リスト

語句一覧は、特定の概念をカプセル化した単語または語句の一覧で、大文字小文字を区別しない一致としてトークン レベルで適用されます。
...
語句一覧は、大文字小文字に関係なく、トークン レベルで適用されます。 次のチャートは、`Ann` という単語を含む語句一覧が、その順序で同じ文字のバリエーションにどのように適用されるかを示しています。


| `Ann` のトークン バリエーション | トークンが見つかったときの語句一覧の一致 |
|--------------------------|---------------------------------------|
