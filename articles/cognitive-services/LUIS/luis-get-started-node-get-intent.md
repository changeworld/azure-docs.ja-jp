---
title: 'チュートリアル: Node.js を使用して Language Understanding Intelligent Service (LUIS) アプリを呼び出す方法 | Microsoft Docs'
...
ms.locfileid: "36263654"
---
# <a name="tutorial-call-a-luis-endpoint-using-nodejs"></a>チュートリアル: Node.js を使用して LUIS エンドポイントを呼び出す
...
## <a name="consume-a-luis-result-using-the-endpoint-api-with-nodejs"></a>Node.js でエンドポイント API を使用して LUIS の結果を利用する
...
2. コードのコメントで説明されているように `LUIS_APP_ID` 環境変数を設定します。 

3. `LUIS_SUBSCRIPTION_KEY` 環境変数を Cognitive Services サブスクリプション キーに設定します。

4. コードを実行します。 前の手順でブラウザー ウィンドウに表示されたものと同じ値が表示されます。
<!-- 
