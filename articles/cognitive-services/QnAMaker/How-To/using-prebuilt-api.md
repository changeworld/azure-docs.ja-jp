...
description: 事前構築済み API を使用してテキストを使って回答を取得する
...
ms.locfileid: "110382687"
---
# <a name="prebuilt-question-answering"></a>事前構築済みの質問と回答

事前構築済みの質問と回答を使用すると、ユーザーはナレッジベースを作成したり、質問と回答のペアのメンテナンスを行ったり、使用率が低いインフラストラクチャのコストを負担することなく、テキストによる質問に回答することができます。 この機能は API として提供され、質問と回答に関するニーズを満たすために使用できます。QnA Maker や追加のストレージに関する詳細を学習する必要はありません。
...
質問があった場合にその回答を取得したい、1 つまたは複数のテキスト ブロックがあるとします。 従来であれば、テキストのブロック数と同じ数のソースを作成する必要がありました。 しかし、事前構築済みの質問と回答を使用すれば、ナレッジ ベースでコンテンツ ソースを定義することなく、テキスト ブロックに対してクエリを実行できます。 

事前構築済み API を使用できるその他のシナリオを次に示します。
...
上記の要求本文では、1 つのテキスト ブロックに対してクエリを実行します。 上記のクエリに対して受信した応答の例は次のとおりです。
...
[構築済み API の制限](https://docs.microsoft.com/azure/cognitive-services/qnamaker/limits?#prebuilt-question-answering-limits)に関するドキュメントを参照してください 

## <a name="prebuilt-api-reference"></a>事前構築済み API のリファレンス
API の呼び出しに必要な入力と出力のパラメーターについては、[事前構築済み API のリファレンス](https://docs.microsoft.com/rest/api/cognitiveservices-qnamaker/qnamaker5.0preview2/prebuilt/generateanswer)に関するドキュメントを参照してください。
