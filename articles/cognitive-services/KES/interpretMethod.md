...
Name|値| 説明
----|----|----
クエリ    | テキスト文字列 | ユーザーが入力したクエリ。  complete が 1 に設定されている場合、クエリは、クエリのオートコンプリートの候補を生成するためのプレフィックスとして解釈されます。        
完了 | 0 (既定) または 1 | 1 は、文法およびインデックス データに基づいてオートコンプリートの候補が生成されることを意味します。         
