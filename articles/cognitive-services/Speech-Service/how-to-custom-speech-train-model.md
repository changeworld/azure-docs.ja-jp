...
6. **[Select training data]\(トレーニング データの選択\)** ページで、関連するテキスト データセット、またはトレーニングに使用するオーディオ + 人間によってラベル付けされた文字起こしデータセットを 1 つ以上選択します。 新しいモデルをトレーニングするときは、まず関連するテキストから開始します。オーディオ + 人間によってラベル付けされた文字起こしを使用したトレーニングには、かなり長い時間がかかる場合があります (最大[数日](how-to-custom-speech-evaluate-data.md#improve-model-recognition))。
