...
| [オーディオ + 人間というラベルが付いたトランスクリプト](#audio-and-human-labeled-transcript-data) | はい<br>精度を評価するために使用 | 0.5-5 時間のオーディオ | はい | 1 - 20 時間のオーディオ |

ファイルは、型別にデータセットにグループ化し、ZIP ファイルとしてアップロードする必要があります。 各データセットには、1 つのデータの種類のみを含めることができます。

> [!TIP]
> 新しいモデルをトレーニングするときには、[テキスト](#plain-text-data-for-training)から開始します。 このデータでは、特殊な用語や語句の認識が既に改善されています。 テキストを使用したトレーニングは、オーディオによるトレーニングよりもはるかに高速です (分単位と日単位)。
...
データをアップロードするには、<a href="https://speech.microsoft.com/customspeech" target="_blank">Custom Speech ポータル </a>に移動します。 プロジェクトを作成したら、 **[Speech データセット]** タブに移動し、 **[データのアップロード]** をクリックしてウィザードを起動し、最初のデータセットを作成します。 データのアップロードが許可される前に、データセットにより音声データ型を選択するように求められます。

まず、データセットを **トレーニング** または **テスト** に使用するかどうかを指定する必要 があります。 また、**トレーニング** または **テスト** にアップロードして使用できるデータには、複数の種類があります。 アップロードする各データセットでは、選択したデータの種類の要件が満たされている必要があります。 アップロードする前に、データの書式を正しく設定する必要があります。 データが正しくフォーマットされていれば、Custom Speech サービスによって正確に処理されます。 要件は、以降のセクションに示されています。
...
ドメイン関連の文を使用すると、製品名や業界固有の専門用語を認識する際の精度が向上します。 文は、1 つのテキスト ファイルとして提供できます。 正確性を高めるには、読み上げられる発話に近いテキスト データを使用します。 
...
発音は、1 つのテキスト ファイルとして指定する必要があります。 これには、音声発話の例と、それぞれのカスタム発音が含まれます。
...
## <a name="audio-and-human-labeled-transcript-data"></a>オーディオと人間によってラベル付けされた文字起こし データ
...
> [!TIP]
> 実際のオーディオがない場合は、 現在の精度を基本的に認識するために、**テスト** データの種類として **[Transcript (automatic audio synthesis)]\(トランスクリプト (自動オーディオ合成)\)** を選択してテキスト (.txt) ファイルをアップロードします。これにより、各発話のオーディオのペアが[テキスト読み上げ](text-to-speech.md)を使用して自動的に合成されます。 
> 
> 通常、合成されたオーディオを **トレーニング** データとして使用することはお勧め **しません**。
> 
> ファイル サイズは最大 500KB です。 行ごとに 1 つのオーディオが合成され、各行の最大サイズは 65535 バイトになります。
...
オーディオ データは、Microsoft の基準の音声テキスト変換モデルやカスタム モデルの精度をテストするのに最適です。 オーディオ データは、特定のモデルのパフォーマンスに関する音声の精度を検査するために使用されることに注意してください。 モデルの精度の定量化を検討している場合は、[オーディオ + 人間というラベルが付いた文字起こしデータ](#audio-and-human-labeled-transcript-data)を使用します。
