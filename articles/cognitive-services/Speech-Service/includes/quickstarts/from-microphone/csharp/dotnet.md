...
`SpeechRecognizer` オブジェクトを初期化するには、サブスクリプション キーとサブスクリプション リージョンを使用する構成を作成する必要があります ([リージョン](https://aka.ms/speech/sdkregion)の "Speech SDK パラメーター" を選択します)。 このコードを `RecognizeSpeechAsync()` メソッドに挿入します。
