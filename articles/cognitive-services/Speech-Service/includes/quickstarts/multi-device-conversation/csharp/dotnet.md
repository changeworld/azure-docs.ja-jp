...
1. 同じファイル内の文字列 `YourSubscriptionKey` を Cognitive Speech のサブスクリプション キーに置き換えます。
...
1. 話し終わったら、<kbd>Ctrl + C</kbd> キーを押してオーディオ キャプチャを停止し、会話を終了します。
...
4. Visual Studio に戻り、文字列 `YourConversationId` を、前の手順で作成した会話 ID に置き換えます。

5. メニュー バーから **[ビルド]**  >  **[ソリューションのビルド]** を選択してアプリケーションをビルドします。 これで、コードは、エラーなしでコンパイルされます。

6. **[デバッグ]**  >  **[デバッグの開始]** の順に選択するか、**F5** キーを押して、**helloworld** アプリケーションを起動します。
