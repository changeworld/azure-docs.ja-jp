...
最後に、各 WAV ファイルを対応する発話のテキスト バージョンに関連付ける *トランスクリプト* を作成します。 [カスタム音声フォントの作成](./how-to-custom-voice-create-voice.md)には、必要なフォーマットの詳細が含まれます。 テキストは台本から直接コピーできます。 次に、WAV ファイルとテキスト トランスクリプトの Zip ファイルを作成します。
