...
ms.locfileid: "72675621"
---
# <a name="quickstart-recognize-speech-by-using-a-cross-platform-xamarin-app-that-uses-the-speech-sdk"></a>クイック スタート:Speech SDK を使用するクロスプラットフォーム Xamarin アプリで音声を認識する
...
この記事では、ユニバーサル Windows プラットフォーム (UWP)、Android、および iOS 向けの Xamarin を使用して、Azure Cognitive Services [Speech SDK](speech-sdk.md) を使用するクロスプラットフォーム C# アプリケーションを開発します。 このプログラムは、デバイスのマイクからリアルタイムで音声をテキストに変換します。 このアプリケーションは、[Speech SDK NuGet パッケージ](https://aka.ms/csspeech/nuget)と Microsoft Visual Studio 2019 の任意のエディションを使用して作成します。
...
* iOS をターゲットにする場合: 
    * [開発に対応しており](https://docs.microsoft.com/xamarin/ios/get-started/installation/device-provisioning/)、マイクが動作する iOS デバイス (ARM64) または iOS シミュレーター (x64)。
...
1. 次に、Visual Studio 内の helloworld.UWP プロジェクトの下にある `Package.appxmanifest` ファイルをダブルクリックします。 **[Capabilities]\(機能\)** で、 **[マイク]** が選択されていることを確認し、ファイルを保存します。
   ...
   > [!NOTE] 
   > "証明書ファイルが存在しません: helloworld.UWP_TemporaryKey.pfx" という警告が表示された場合は、[音声テキスト変換](quickstart-csharp-uwp.md)のサンプルで詳細を確認してください。
