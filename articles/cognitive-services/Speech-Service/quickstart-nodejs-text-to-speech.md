...
* Speech Service 用の Azure サブスクリプション キー。 [1 つ無料で取得できます](get-started.md)。
...
> [!NOTE]
> これらのモジュールを使用していない場合は、プログラムを実行する前にこれらをインストールする必要があります。 これらのパッケージをインストールするには、`npm install request readline-sync` を実行します。

## <a name="set-the-subscription-key-and-create-a-prompt-for-tts"></a>サブスクリプション キーを設定して TTS のプロンプトを作成する

次のいくつかのセクションで、承認を処理し、Text to Speech API を呼び出し、応答を検証する関数を作成します。 サブスクリプション キーを追加し、テキスト入力用のプロンプトを作成することから始めましょう。
...
この関数では、Speech Services のサブスクリプション キーとコールバック関数という 2 つの引数を使用します。 この関数は、アクセス トークンを取得した後、その値をコールバック関数に渡します。 次のセクションで、Text to Speech API を呼び出し、合成された音声応答を保存する関数を作成します。

このサンプルでは、ご利用の Speech Service のサブスクリプションが米国西部リージョンにあることを想定しています。 別のリージョンを使用している場合は、`uri` の値を更新してください。 完全な一覧については、[リージョン](https://docs.microsoft.com/azure/cognitive-services/speech-service/regions#rest-apis)に関するセクションを参照してください。
...
ここでは、Text to Speech API への要求を作成し、音声応答を保存します。 このサンプルでは、米国西部のエンドポイントを使用していることを想定しています。 ご使用のリソースが別のリージョンに登録されている場合は、必ず `uri` を更新してください。 詳細については、[Speech Service のリージョン](https://docs.microsoft.com/azure/cognitive-services/speech-service/regions#text-to-speech)に関するセクションを参照してください。
...
最後に、サービスに対して要求を実行します。 要求が成功して状態コード 200 が返された場合、音声応答が `sample.wav` として書き込まれます。
...
あともう少しで終了です。 最後の手順は、`textToSpeech` 関数の呼び出しです。
