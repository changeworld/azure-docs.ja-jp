...
> [!TIP]
> トレーニング データとテスト データをアップロードする場合、.zip ファイルのサイズは 2 GB を超えることはできません。 より多くのデータがトレーニングとテストに必要な場合は、複数の .zip ファイルに分割し、個別にアップロードします。 その後、*複数の*データセットから選択してトレーニングとテストを行うことができます。
