...
1. プロジェクト ディレクトリを選択します。
   ...
   1. プロジェクト設定画面で、SDK へのパスを調整します。
      ...
      1. **[Search Paths]\(検索パス\)** という見出し下の **[Framework Search Paths]\(フレームワーク検索パス\)** にディレクトリとして「$(SRCROOT)/.. 」を追加します。

      ![[Framework Search Paths]\(フレームワーク検索パス\) の設定](media/sdk/qs-objectivec-framework-search-paths.png)
