...
* Azure アカウント (ない場合は[無料でサインアップ](https://azure.microsoft.com/free/ai/))。

* Speech サービスのサブスクリプション。 まだない場合は、[作成](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesSpeechServices)してください。

    ![パネルを作成する](media/custom-voice/create-panel.png)

サブスクリプションを作成した後、新しいサブスクリプションの [クイック スタート] パネルまたは [概要] パネルに 2 つのサブスクリプション キーが表示されます。 どちらのキーを使用してもかまいません。

最後に、次のようにしてサブスクリプションを Custom Voice ポータルに接続します。
...
4. 次に示すように、テーブルにサブスクリプション キーを貼り付けます。
...
これらのファイルは、どちらを先に準備してもかまいません。つまり、スクリプトを書いてから音声タレントに読んでもらうのでも、一般に利用可能なオーディオを使用してテキストに文字起こしするのでも、どちらでもかまいません。 後者の場合は、"うーん" のような口ごもり、その他の間つなぎ音、言葉の詰まり、聞き取りにくい単語、間違った発音を、オーディオ ファイルから編集します。

良質の音声フォントを作成するには、静かな部屋で高品質のマイクを使って録音を行うことが重要です。 一定の音量、速さ、ピッチ、および表現方法で話すことが、優れたデジタル音声の作成には不可欠です。 製品用の音声を作成するには、専門の録音スタジオと音声タレントを使うことをお勧めします。
...
文字起こしファイルは、プレーンな Unicode テキスト ファイル (UTF-16 リトル エンディアン) です。 文字起こしファイルの各行は、オーディオ ファイルの名前、タブ (コード ポイント 9) 文字、トランスクリプトという順序になっている必要があります。 空白行は許可されていません。
...
> [!NOTE]
> Free サブスクリプション ユーザーは、一度に 2 つの音声フォントをトレーニングできます。 Standard サブスクリプション ユーザーは、3 つの音声を同時にトレーニングできます。 制限に達した場合は、少なくとも 1 つの音声フォントのトレーニングが終わるまで待ってから、もう一度やり直します。
...
新しいカスタム エンドポイントを作成するには、ページ上部の [Custom Voice] メニューから **[エンドポイント]** を選択します。 [Deployment]\(展開\) ページが開き、存在する場合は現在のカスタム音声エンドポイントのテーブルが表示されます。

**[Deploy voices]\(音声の展開\)** ボタンをクリックして、新しいエンドポイントを作成します。 [Create Endpoint]\(エンドポイントの作成\) ページでは、現在のロケールがテーブルの最初の行に反映されています。 別の言語の展開を作成するには、表示されているロケールを変更します。 (展開している音声と一致する必要があります。)カスタム エンドポイントの名前と説明を入力します。
...
展開するモデルを選択した後、**[Create]\(作成\)** をクリックします。 [Deployment]\(展開\) ページが再び開きますが、今度は新しいエンドポイントのエントリが表示されます。 新しいエンドポイントのインスタンス化には、数分かかることがあります。 展開の状態が [Succeeded]\(成功\) の場合、エンドポイントは使用可能です。

![展開された音声](media/custom-voice/my-deployed-voices.png)

展開の状態が [Succeeded]\(成功\) の場合、展開済みの音声フォントのエンドポイントが、[My deployed voices]\(マイ展開済み音声\) テーブルに表示されます。 この URI を HTTP 要求内で直接使用できます。

Custom Voice ポータルを使用して、エンドポイントのオンライン テストを行うこともできます。 エンドポイントをテストするには、[Custom Voice] ドロップダウン メニューから **[Endpoints testing]\(エンドポイントのテスト\)** を選択します。 エンドポイントのテスト ページが表示されます。 展開した音声を選び、読み上げるテキストを (プレーンテキストまたは SSML 形式で) テキスト ボックスに入力します。
