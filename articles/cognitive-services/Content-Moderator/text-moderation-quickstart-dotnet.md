...
1. 必須の NuGet パッケージを入手します。 ソリューション エクスプローラーで目的のプロジェクトを右クリックし、 **[NuGet パッケージの管理]** を選択します。 **`Microsoft.Azure.CognitiveServices.ContentModerator` パッケージ**を見つけてインストールします。 または、ソリューション ディレクトリから次のコマンドを実行することもできます。
