...
REST API や SDK を通じて Content Moderator サービスを使用するには、API サブスクリプション キーが必要です。 [Azure portal](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesContentModerator) で Content Moderator サービスをサブスクライブすることによって取得します。
...
**Program** クラスに次のメソッドを追加します。 リストを更新するたびに、リストを使用して画像をスクリーニングする前に、検索インデックスを更新する必要があります。
