...
[Azure App Service の Web アプリ](http://go.microsoft.com/fwlink/?LinkId=529714) 内部設置型の環境またはパブリック インターネットのいずれからアクセスしているかにかかわらず、ユーザーのシングル サインオン (SSO) のサポートにより、企業の基幹業務アプリケーション シナリオを実現します。これは、内部の Active Directory (AD) ユーザーを認証し、適切に承認するために、[Azure Active Directory](http://azure.microsoft.com/services/active-directory/) (AAD)、または Active Directory フェデレーション サービス (AD FS) などの内部設置型トークン サービス (STS) と統合できます。

## 円滑な認証および承認 ##

ボタンの数回クリックすることにより、Web アプリの認証と承認有効にできます。すべての Azure Web アプリのチェックボックス形式の構成は、基幹業務 Web アプリの基本的なアクセス制御を提供します。これは、すべての Web アプリのコンテンツへのアクセスをユーザーに許可する前に、選択した Azure AD テナントに対して HTTPS および認証を強制することによって実現されます。詳細については、「[Web アプリの認証/承認](」を参照してください。http://azure.microsoft.com/blog/2014/11/13/azure-websites-authentication-authorization/)

>[AZURE.NOTE] この機能は、現在プレビュー段階にあります。
...
-	[Azure AD](web-sites-dotnet-lob-application-azure-ad.md) -Azure AD を使用して、Web アプリの認証と承認を実装できます。ID プロバイダーとして Azure AD を使用することには、次の特性があります。
	-	[OAuth 2.0](http://oauth.net/2/)、[OpenID Connect](http://openid.net/connect/)、および [SAML 2.0](http://en.wikipedia.org/wiki/SAML_2.0) など、一般的な認証プロトコルのサポート。サポートされているプロトコルの一覧については、「[Azure Active Directory の認証プロトコル](」を参照してください。http://msdn.microsoft.com/library/azure/dn151124.aspx)
	...
	-	[Azure AD によってサポートされているすべてのアプリケーションに SSO を提供](/marketplace/active-directory/)Azure、Office 365、Dynamics CRM Online、Windows InTune、および何千もの Microsoft 以外のクラウド アプリケーションが含まれます。 
...
## 変更内容
* Web サイトからアプリ サービス[Azure App Service and Its Impact on Existing Azure Services (Azure App Service と既存の Azure サービスへの影響)](http://go.microsoft.com/fwlink/?LinkId=529714)
