...
## <a name="resource-manager-template-for-a-simple-metric-alert"></a>簡単なメトリック アラート用の Resource Manager テンプレート
...
このチュートリアルでは､以下の JSON を simplemetricalert.json として保存します｡
...
以下の JSON を simplemetricalert.parameters.json として保存し､必要に応じて変更します｡
...
## <a name="resource-manager-template-for-a-more-advanced-metric-alert"></a>高度なメトリック アラート用の Resource Manager テンプレート

新しいメトリック アラートは､多次元メトリックでのアラートをサポートするばかりでなく､複数の基準もサポートしています｡ 次のテンプレートを使用して､多次元メトリックに高度なメトリック アラートを作成し､複数の条件を指定することができます｡

このチュートリアルでは､以下の JSON を advancedmetricalert.json として保存します｡
...
このチュートリアルでは､以下の JSON を advancedmetricalert.parameters.json として保存し､必要に応じて変更します｡
...
## <a name="resource-manager-template-for-metric-alert-that-monitors-multiple-resources"></a>複数のリソースを監視するメトリック アラートの Resource Manager テンプレート

前のセクションでは、1 つのリソースを監視するメトリック アラートを作成する Azure Resource Manager テンプレートのサンプルについて説明しました。 Azure Monitor は、1 つのメトリック アラート ルールによる複数リソースの監視をサポートするようになりました。 このプレビュー機能は現在、Azure Resource Manager テンプレートと REST API を介してのみ使用することができ、仮想マシンについてのみサポートされます。 このようなルールの作成と編集をサポートするポータルは、まもなく公開される予定です。
...
### <a name="all-virtual-machines-in-one-or-more-resource-groups"></a>1 つまたは複数のリソース グループ内のすべての仮想マシン

このテンプレートによって、1 つまたは複数のリソース グループ内 (1 つの Azure リージョン内) のすべての仮想マシンの CPU 使用率を監視するメトリック アラート ルールが作成されます。

このチュートリアルでは､以下の JSON を all-vms-in-resource-group.json として保存します｡
...
このチュートリアルでは､以下の JSON を all-vms-in-resource-group.parameters.json として保存および変更します｡
...
### <a name="all-virtual-machines-in-a-subscription"></a>サブスクリプション内のすべての仮想マシン

このテンプレートによって、1 つのサブスクリプション内 (1 つの Azure リージョン内) のすべての仮想マシンの CPU 使用率を監視するメトリック アラート ルールが作成されます。

このチュートリアルでは､以下の JSON を all-vms-in-subscription.json として保存します｡
...
上記のテンプレートは､以下のパラメーター ファイルと共に使用できます｡
このチュートリアルでは､以下の JSON を all-vms-in-subscription.parameters.json として保存および変更します｡
...
### <a name="a-list-of-virtual-machines"></a>仮想マシンの一覧

このテンプレートによって、1 つのサブスクリプション内 (1 つの Azure リージョン内) の、一覧に含まれる仮想マシンの CPU 使用率を監視するメトリック アラート ルールが作成されます。

このチュートリアルでは､以下の JSON を list-of-vms.json として保存します｡
...
上記のテンプレートは､以下のパラメーター ファイルと共に使用できます｡
このチュートリアルでは､以下の JSON を list-of-vms.parameters.json として保存および変更します｡
