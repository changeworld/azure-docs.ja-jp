...
- Azure StorSimple に格納されたデータの暗号化に使われる証明書の公開キーは、Microsoft Azure の管理ポータルに対して安全に公開され、秘密キーは StorSimple デバイス上に保持されます。StorSimple 仮想デバイスでは、公開キーと秘密キーの両方が Azure に格納されます。
...
- 仮想デバイスに対し、[Azure の仮想ネットワークを構成](https://msdn.microsoft.com/library/azure/jj156074.aspx)します。 
- 独自の DNS サーバー名を指定する代わりに、Azure に用意されている既定の DNS サーバーを使用できます。 
...
- 仮想ネットワークに [Azure Virtual Machines](https://msdn.microsoft.com/library/azure/jj156003.aspx) (ホスト サーバー) を作成します。これらのサーバーは次の要件を満たしている必要があります。 							
...
- 仮想デバイスのホスト サーバーとなる VM の[アクセス制御レコード](https://msdn.microsoft.com/library/1747f56e-858a-4cfe-a020-949d7db23b8b#sec02)を追加します。

- ご利用の[ストレージ アカウント](https://msdn.microsoft.com/library/1747f56e-858a-4cfe-a020-949d7db23b8b#sec01)が仮想デバイスと同じリージョンに存在することを確認します。ストレージ アカウントが異なるリージョンに存在すると、十分なパフォーマンスが得られない可能性があります。
...
以下の作業を始める前に、「[仮想デバイスの前提条件](https://msdn.microsoft.com/library/dn772572.aspx)」が満たされていることを確認してください。

以下の作業が完了した後、「[仮想デバイスの作業](https://msdn.microsoft.com/library/dn772527.aspx)」に進むことができます。
...
- **[仮想デバイスの作成] ダイアログ ボックス**で、次のように指定します。
...
- 仮想デバイスに格納するデータがマイクロソフトのデータセンターでホストされることに同意する旨のチェック ボックスをオンにします。物理デバイスのみを使用する場合は、暗号化キーがご利用のデバイスに保管されるため、マイクロソフトが暗号化を解除することはできません。仮想デバイスを使用する場合、暗号化キーと復号化キーの両方が Microsoft Azure に保管されます。詳細については、「[仮想デバイスを使用するためのセキュリティに関する考慮事項](https://msdn.microsoft.com/library/dn772561.aspx)」を参照してください。
...
1. ご利用のデバイスとして **[StorSimple 仮想デバイス]** を選択し、ダブルクリックして、[クイック スタート] にアクセスします。

- **[デバイスのセットアップの完了]** をクリックします。デバイスの構成ウィザードが起動します。

- 所定の場所に **[サービス データ暗号化キー]** を入力します。

- チェック マークをクリックして、仮想デバイスの初期構成と登録を完了します。[Snapshot Manager のパスワード] と [デバイス管理者のパスワード] にはあらかじめ既定値が設定されています。デバイスの登録後に変更してください。

### デバイスの構成設定の変更

次のセクションでは、StorSimple 仮想デバイスの構成に必要となるデバイスの構成設定について説明します。
...
#### StorSimple Snapshot Manager の構成
...
StorSimple 仮想デバイスで使用する場合、StorSimple Snapshot Manager を構成するには、次の手順を実行します。
...
StorSimple 仮想デバイスにデバイス管理者のパスワードを構成するには、次の手順を実行します。
...
- [ボリューム コンテナー](https://msdn.microsoft.com/library/dn757817.aspx)

- [ボリューム](https://msdn.microsoft.com/library/dn772417.aspx)

- [バックアップ ポリシー](https://msdn.microsoft.com/library/dn772382.aspx)
...
- **[自動更新]** – 自動更新をオンまたはオフにできます。自動更新をオンにすると、仮想デバイスが自動的にシャットダウンして (必要に応じて) 再起動し、リリースされた更新プログラムが適用されます。仮想デバイスには単一のインターフェイスしかないため、更新プログラムの適用時、サービスの中断はわずかで済みます。
...
- プロビジョニング中に作成されたホスト サービスと仮想ネットワークは保持されます。それらを使用していない場合は、手動で削除する必要があります。
...
「[Windows PowerShell を使用するリモート接続](https://msdn.microsoft.com/library/dn772393.aspx)」の手順に従って、仮想デバイスのリモート処理をセットアップしてください。
...
>[AZURE.NOTE]**仮想デバイスでサポートされるストレージの量は 30 TB です。**
...
仮想デバイスを削除またはシャットダウンすると、StorSimple Manager サービスの [デバイス] ページには **[オフライン]** として表示されます。仮想デバイスによって作成されたバックアップも一緒に削除する場合は、デバイスとして非アクティブにするか、削除するかを選択できます。詳細については、[非アクティブ化](https://msdn.microsoft.com/library/33b7811b-36ba-4609-b165-0796ad456435#BKMK_acis_deactivate)に関するトピックを参照してください。
