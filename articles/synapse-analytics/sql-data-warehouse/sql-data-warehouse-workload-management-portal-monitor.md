...
メトリック 2: "*Workload group allocation by cap resource percent (上限リソース割合別のワークロード グループの割り当て)* " (Avg 集計、`purple line`)<br>
