...
Synapse SQL の基本的な特徴は、[複数のディストリビューション](massively-parallel-processing-mpp-architecture.md#distributions)間でテーブルを格納して操作できる方法にあります。 Synapse SQL では、データを分散するための 3 つの方法 (ラウンド ロビン (既定値)、ハッシュ、レプリケート) がサポートされています。
