...
この効果は、状況をテストする場合や、ポリシー定義によって効果がパラメーター化されている場合に役立ちます。 効果の割り当てパラメーターを変更することで、ポリシーのすべての割り当てを無効にするのではなく、ポリシーの 1 つの割り当てを無効にできるようになります。
...
例 3: 配列 **value** の[別名](definition-structure.md#aliases)を使用して、ストレージ アカウントに IP 規則を設定する単一の **field/value** のペア。
...
- **roleDefinitionIds** [必須]
  - このプロパティは、サブスクリプションでアクセス可能なロールベースのアクセス制御ロール ID と一致する文字列の配列を含める必要があります。 詳細については、[修復 - ポリシー定義を構成する](../how-to/remediate-resources.md#configure-policy-definition)を参照してください。
- **Deployment** [必須]
