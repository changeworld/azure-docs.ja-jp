---
title: Azure Policy を使って準拠していないリソースを修復する
...
ms.locfileid: "47392697"
---
# <a name="remediate-non-compliant-resources-with-azure-policy"></a>Azure Policy を使って準拠していないリソースを修復する

**deployIfNotExists**に準拠していないリソースは､**修復**を使って準拠状態にすることができます。 修復は､既存のリソースに対して割り当てポリシーの **deployIfNotExists** 効果を実行するよう Policy に指示することによって行うことができます｡ このハウツーでは､この修復を行うために行う必要がある手順を示します｡
...
管理対象 ID は Policy によって各割り当てに自動的に作成されますが、それら管理対象 ID に付与するロールの詳細はユーザーが指定する必要があります。 管理対象 ID にロールが存在しない場合、そのポリシー､またはそのポリシーを含むイニシアチブの割り当て中にそのことが表示されます。 ポータルを使用している場合､割り当てが開始されると､Policy は示されているロールのすべてを自動的に管理対象 ID に付与します｡
...
最初の手順は、含まれているテンプレートのコンテンツを **deployIfNotExists** が正しくデプロイするために必要なロールをポリシー定義に定義することです｡ **details (詳細)** プロパティで **roleDefinitionIds** プロパティを追加します。 これは、環境にあるロールに合致する一連の文字列です。
[deployIfNotExists の例](../concepts/effects.md#deployifnotexists-example)に示す例を参照してください｡
...
1. リソース ページにある **アクセス制御 (IAM)** のリンクをクリックして､アクセス制御ページの上部にある **[+ 追加]** をクリックします｡
...
評価では､**deployIfNotExists** 効果を持つポリシー割り当てによって､準拠していないリソースがあるかどうかが判定されます｡ 準拠していないリソースが見つかった場合は、**修復**ページにその詳細が表示されます。 準拠していないリソースがあるポリシーの一覧には、**修復タスク** をトリガーするオプションがあります｡ これが､**deployIfNotExists**テンプレートからデプロイを作成するオプションです。
...
1. **新しい修復タスク** ページで **スコープ** 省略記号ボタンを使って､ポリシーが割り当てられている子リソースを選択することで､修復するリソースをフィルタ処理します (個々のリソース オブジェクトまでフィルタ可能) ｡ また、**[場所]** ドロップダウンを使って､リソースをさらにフィルター処理することもできます｡ 表に示されたリソースのみ修復されます。
...
1. ポリシー コンプライアンス ページの **[修復タスク]** をクリックすると､進行状況の詳細が表示されます｡ 修復されているリソースの一覧と共に､タスクに使用されたフィルター表示されます。
...
**[修復タスク]** を使ってデプロイされたリソースは､少し時間を置いてポリシー コンプライアンス ページの**デプロイしたリソース** タブに追加されます｡
...
- [コンプライアンス データの取得方法](getting-compliance-data.md)の詳細
