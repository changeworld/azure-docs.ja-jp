...
Azure Policy では、定義内の **type**、**name**、または **kind** フィールドを使用してリソースが一致かどうかを調べます。 リソースが一致している場合、適用可能と見なされ、**準拠**、**非準拠**、**適用除外** のいずれかの状態になります。 **name** または **kind** のいずれかが定義内の唯一のプロパティである場合、適用除外ではない、含まれているすべてのリソースが適用可能であると見なされ、評価されます。
