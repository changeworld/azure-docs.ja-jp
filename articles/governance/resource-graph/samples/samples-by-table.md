...
このページは、テーブル別にグループ化した Azure Resource Graph サンプル クエリのコレクションです。 特定の **テーブル** にジャンプするには、ページの右側にあるメニューを使用します。 それ以外では、<kbd>Ctrl</kbd>-<kbd>F</kbd> キーを押して、ブラウザーの検索機能を使用してください。 テーブルと関連する詳細の一覧については、「[Resource Graph テーブル](../concepts/query-language.md#resource-graph-tables)」を参照してください。
