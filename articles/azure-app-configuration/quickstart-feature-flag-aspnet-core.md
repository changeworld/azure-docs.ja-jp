...
7. **[操作]**  >  **[機能マネージャー]**  >  **[追加]** の順に選択して、*Beta* という機能フラグを追加します。
...
1. *Views/Shared/_Layout.cshtml* で、`<body>` > `<header>` の下の `<nav>` バー コードを次のマークアップに置き換えます。
    ...
    上記のマークアップでは、`<feature>` タグ ヘルパーが *Beta* リスト項目を囲んでいることに注意してください。
...
1. ブラウザー ページを最新の情報に更新して新しい構成設定を確認します。
    ...
    :::image type="content" source="media/quickstarts/aspnet-core-feature-flag-local-after.png" alt-text="変更前のローカル クイックスタート アプリ" border="true":::
