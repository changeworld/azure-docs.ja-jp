...
キーと値に対する具体的な名前付けスキームは、アプリケーション フレームワークにおける構成ストアの使われ方で決まる場合があります。 たとえば Java の Spring Cloud フレームワークでは、Spring アプリケーションに設定を提供する `Environment` リソースが、*<アプリケーション名>* と *<プロファイル>* を含む変数によってパラメーター化されるように定義しています。 通常、Spring Cloud に関連した構成データのキーは、区切り記号でつながれた、この 2 つの要素で始まります。
