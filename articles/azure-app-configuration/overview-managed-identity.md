...
システム割り当て ID は、Azure CLI で [az appconfig identity remove](/cli/azure/appconfig/identity#az_appconfig_identity_remove) コマンドを使用してその機能を無効にすることによって削除できます。 ユーザー割り当て ID は個別に削除することはできません。 この方法でシステム割り当て ID を削除すると、AAD からも削除されます。 システム割り当て ID は、アプリ リソースが削除されると、AAD からも自動的に削除されます。
