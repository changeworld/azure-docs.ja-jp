...
>[AZURE.NOTE] 以下の提案事項のほとんどは、いつでもアプリケーションに実装できます。そのため、"すばやく修正する方法" として最適です。 多くの場合、長期的にベストなソリューションには、クラウド向けのアプリケーション設計が必要です。  これら (設計関連) のチェック リストについては、「[可用性のチェックリスト](../best-practices-availability-checklist.md)」をご覧ください。
