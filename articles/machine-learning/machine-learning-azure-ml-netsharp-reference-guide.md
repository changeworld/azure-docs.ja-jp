...
-	ByRow の述語で、**s** は入力層 Pixels のノードの長方形配列へのインデックスを表すパラメーターです。一方、**d** は隠れ層 ByRow のノードの配列へのインデックスを表すパラメーターです。**s** と **d** の両方の型は、長さ 2 の整数のタプルです。概念的には、**s** の範囲は _0 <= s[0] < 10_ および _0 <= s[1] < 20_ である整数のすべてのペア、**d** の範囲は _0 <= d[0] < 10_ および _0 <= d[1] < 12_ である整数のすべてのペアです。 
