...
### <a name="add-labelers-to-workspace"></a>ワークスペースにラベラーを追加する

プロジェクトでデータのラベル付けを行うすべての担当者が含まれるようにワークスペースを設定します。  後で、これらのラベラーを特定のラベル付けプロジェクトに追加します。

1. 左側で **[データのラベル付け]** を選択します。

1. ページの上部で **[ラベラー]** を選択します。

1. **[ラベラーの追加]** を選択して、ラベラーのメール アドレスを追加します。

1. すべてのラベラーを追加するまで作業を続けます。
...
### <a name="add-labelers-to-your-project"></a>プロジェクトにラベラーを追加する

このプロジェクトに一部またはすべてのラベラーを追加します。

1. プロジェクト名を選択してプロジェクトを開きます。  

1. ページの上部で **[Teams]\(チーム\)** を選択します。

1. **[labeling_tutorial Default Team]\(labeling_tutorial 既定のチーム\)** リンクを選択します。

1. **[ラベラーの割り当て]** を使用して、このプロジェクトに参加させるラベラーを追加します。 

1. 前の手順で作成したラベラーのリストから選択します。  対象のラベラーをすべて選択したら、 **[ラベラーの割り当て]** を選択して既定のプロジェクト チームに追加します。

## <a name="start-labeling"></a>ラベル付けを開始する

これで Azure リソースの設定とデータ ラベル付けプロジェクトの構成が完了しました。 次に、データにラベルを追加します。

### <a name="notify-labelers"></a>ラベラーに通知する

多数の画像にラベルを付ける場合は、多数のラベラーでその作業を実行することになります。  ここでは、データにアクセスしてラベル付けを開始するための手順をラベラーに送信します。

1. [Machine Learning Studio](https://ml.azure.com) で、左側にある **[データのラベル付け]** を選択してプロジェクトを特定します。  

1. プロジェクト名のリンクを選択します。

1. ページの上部で **[詳細]** を選択します。  プロジェクトの概要が表示されます。

    ![プロジェクトの詳細](media/tutorial-labeling/project-details.png)

1. ラベラーに送信する **[Labeling portal URL]\(ラベル付けポータルの URL\)** リンクをコピーします。

1. 上部の **[チーム]** を選択して、ラベル付けチームを見つけます。  

1. チーム名のリンクを選択します。

1. ページの上部で、 **[Email team]\(チームにメールを送信\)** を選択して電子メールを開始します。  先ほどコピーしたラベル付けポータルの URL を貼り付けます。  

ラベラーがポータル URL にアクセスするたびに、キューが空になるまで、ラベルを付ける画像が表示されます。  

### <a name="tag-the-images"></a>画像にタグを付ける

チュートリアルのこのパートでは、ロールを "*プロジェクト管理者*" から "*ラベラー*" に切り替えます。  チームに送信した URL を使用します。  この URL でプロジェクトのラベル付けポータルにアクセスできます。  手順を追加した場合は、ページにアクセスすると、ここにその手順が表示されます。

1. ページの上部で、 **[Tasks]\(タスク\)** を選択してラベル付けを開始します。
