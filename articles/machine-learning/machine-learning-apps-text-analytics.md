...
##Batch API
Text Analytics サービスでは、センチメントとキー フレーズの抽出をバッチ モードで実行できます。GetSentimentBatch と GetKeyPhrasesBatch では、スコア付けされる各レコードがトランザクションとしてカウントされます。したがって、たとえば 1 回の呼び出しで 1000 個のレコードのセンチメントを取得する場合、1000 のトランザクションが推論されます。
