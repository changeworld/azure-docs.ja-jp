...
|| Excel|Azure ML (初期)|Azure ML (最小二乗法を使用)|
|---|:---:|:---:|:---:|
|ラベル付きの値 |実績作業期間 (数値)|同じ|同じ|
|学習者 |Excel -> データ分析 -> 回帰|線形回帰|線形回帰|
|学習者オプション|N/A|既定|通常の最小二乗法<br />L2 = 0.005|
|データ セット|26 行、3 特徴、1 ラベル。すべて数値。|同じ|同じ|
|分割: トレーニング|Excel は最初の 18 行でトレーニングし、最後の 8 行でテストしました。|同じ|同じ|
|分割: テスト|Excel の回帰式は最後の 8 行に適用されました。|同じ|同じ|
|**パフォーマンス**||||
|自由度調整済み決定係数|0.96|N/A||
|決定係数|N/A|0.78|0.952049|
...
|---|:---:|:---:|
|インターセプト/バイアス|19470209.88|19328500|
