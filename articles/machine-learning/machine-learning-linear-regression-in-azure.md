...
3.	[分割][split] モジュール (*相対式*モードで) をを使用して、Excel で実行されたのと同じトレーニング セットにデータを分割しました  
...
|| Excel|Azure ML (初期)|Azure ML (最小二乗法を使用)| |---|:---:|:---:|:---:| |ラベル付きの値 |実績作業期間 (数値)|同じ|同じ| |学習者 |Excel -> データ分析 -> 回帰|線形回帰|線形回帰| |学習者オプション|N/A|既定|通常の最小二乗法<br />L2 = 0.005| |データ セット|26 行、3 特徴、1 ラベル。すべて数値。|同じ|同じ| |分割: トレーニング|Excel は最初の 18 行でトレーニングし、最後の 8 行でテストしました。|同じ|同じ| |分割: テスト|Excel の回帰式は最後の 8 行に適用されました。|同じ|同じ| |**パフォーマンス**|||| |自由度調整済み決定係数|0.96|N/A|| |決定係数|N/A|0.78|0.952049| |平均絶対誤差 |$9.5M|$ 19.4M|$9.5M| |平均絶対誤差 (%) |<span style="background-color: 00FF00;"> 6.03%</span>|12.2 %|<span style="background-color: 00FF00;"> 6.03%</span>|

さらに、Excel の係数を Azure のトレーニング済みモデルの特徴の重みと十分に比較しました。

||Excel の係数|Azure の特徴の重み| |---|:---:|:---:| |インターセプト/バイアス|19470209.88|19328500| |特徴 A|0.832653063|0.834156| |特徴 B|11071967.08|11007300| |特徴 C|25383318.09|25140800|
