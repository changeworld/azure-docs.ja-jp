...
SVM の代わりのニューラル ネットワークのトレーニングは、`PARAMETERS.py` の変数 `classifier` を `svm` から `dnn` に変更して、行います。 次に、パート 1 で説明したように、データ準備 (手順 1) と SVM トレーニング (手順 3) を除くすべてのスクリプトを再度実行する必要があります。 DNN の調整には、GPU が必要です。 GPU が検出されないか、または GPU がロックされている (たとえば以前の CNTK 実行によって) 場合、スクリプト `2_refineDNN.py` はエラーをスローします。 DNN トレーニングでは一部の GPU でメモリ不足のエラーがスローされる可能性があります。これは、ミニバッチ サイズ (`PARAMETERS.py` 内の変数 `cntk_mb_size`) を小さくすることによって回避できます。
