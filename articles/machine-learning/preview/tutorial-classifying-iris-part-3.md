...
1. この Web サービスにはサンプル データが含まれています。 ローカル モードで実行している場合は、**az ml service show realtime** コマンドを呼び出すことができます。 この呼び出しにより、サービスのテストに役立つサンプル実行コマンドが取得されます。 また、独自のカスタム アプリにサービスを組み込む際に使用できるスコア付け URL も取得されます。
