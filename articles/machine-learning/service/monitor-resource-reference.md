...
| Category | ログ イベントの名前、AmlComputeClusterNodeEvent |
| JobId | 送信されたジョブの ID |
| ExperimentId | 実験の ID |
...
### <a name="amlcomputerclusterevents-table"></a>AmlComputerClusterEvents テーブル
...
次の表に、Azure Machine Learning に対して収集されたプラットフォーム メトリックを示します。すべてのメトリックは、名前空間 **[Azure Machine Learning Service ワークスペース]** に格納されます。
