...
> [Jupyter Notebook を取得する](https://aka.ms/aml-packages/forecasting/notebooks/financial_forecasting)
...
次に、テスト セットに対する予測エラーを計算できます。 ここでは、平均絶対パーセント誤差 (MAPE) を使用できます。 MAPE は、実際の売上値に対する平均絶対誤差です。 ```calc_error``` 関数には、誤差メトリックで一般的に使用されるいくつかの組み込み関数が用意されています。 カスタム誤差関数を定義して、それを ```err_fun``` 引数に渡すこともできます。
...
データ内の 249 の時系列のこれらの誤差の分布を確認することが大事です。 誤差の分布を表示するには、`calc_error` で `by` 引数を使用して、各系列の誤差を計算します。 その後、それらを視覚化する箱ひげ図を作成します。
...
総合的に、精度が低いいくつかの外れ値はありますが、ナイーブ モデルは、適切な予測を行っているように思われます。 

## <a name="build-machine-learning-models"></a>機械学習モデルを構築する
従来の一変量モデルだけでなく Azure Machine Learning Package for Forecasting を使用して、機械学習モデルを作成することもできます。 Y
...
[RegressionForecaster](https://docs.microsoft.com/python/api/ftk.models.regressionforecaster.regressionforecaster) 関数は、sklearn 回帰推定器をラップして、TimeSeriesDataFrame でトレーニングできるようにします。 ラップされた予測器も、各グループ (ここではストア) を同じモデル内に配置します。 予測器は、類似しているとみなされ、まとめてプールできる系列のグループに対する 1 つのモデルを学習できます。 多くの場合、系列のグループに対する 1 つのモデルは、長期の系列のデータを使用して、短期の系列の予測を向上させます。 `Lasso` と `RandomForest` モデルを `scikit-learn` から直接使用します。  回帰をサポートするライブラリ内のその他のモデルの代わりに、これらのモデルを使用できます。 
...
機械学習モデルでは、系列間の追加された特徴と類似性を活用して、より正確な予測を取得できました。

**クロス検証とパラメーターのスイープ**

パッケージでは、予測アプリケーション用にいくつかの従来の機械学習関数を適合させています。  [RollingOriginValidator](https://docs.microsoft.com/python/api/ftk.model_selection.cross_validation.rollingoriginvalidator) は、一時的にクロス検証を実行して、予測フレームワークで何が認識され、何が認識されていないかを考慮します。 
...
パイプラインは、配置用に適合されていません。 デプロイされた Web サービスをスコア付けすると、モデルが再度トレーニングされ、新しいデータで予測が生成されます。 
