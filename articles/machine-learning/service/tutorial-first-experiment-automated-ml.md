...
サービスの自動機械学習機能と Azure portal を使用して、自動機械学習プロセスを開始します。 アルゴリズムの選択とハイパーパラメーターのチューニングが自動的に行われます。 自動機械学習の手法では、アルゴリズムとハイパーパラメーターのさまざまな組み合わせが、条件に基づいて最適なモデルが見つかるまで反復処理されるため、コードを 1 行も記述する必要がありません。
...
* **bankmarketing_train.csv** データ ファイル。 [こちらでダウンロードできます](https://automlsamplenotebookdata.blob.core.windows.net/automl-sample-notebook-data/bankmarketing_train.csv)。
...
1. ワークスペースの左側のウィンドウに移動します。 **[作成 (プレビュー)]** セクションで、 **[自動 Machine Learning]** を選択します。
    ...
    これは、自動機械学習を使用する最初の実験であるため、 **[Welcome to Automated Machine Learning]\(自動 Machine Learning へようこそ\)** 画面が表示されます。 
...
1. このチュートリアルでは、新しいコンピューティングで作成された既定のストレージ アカウントとコンテナーを使用します。 フォームに自動的に入力されます。
...
この実験では、**VotingEnsemble** は **AUC_weighted** メトリックに基づいて最適なモデルと見なされます。 Azure portal で自動機械学習を使用すると、このモデルを Web サービスとしてデプロイして、新しいデータを予測することができます。 
...
1. **[デプロイ]** を選択します。 デプロイの完了には 20 分程度かかります。
...
+ [Web サービスを使用する](how-to-consume-web-service.md)方法を学習します。
