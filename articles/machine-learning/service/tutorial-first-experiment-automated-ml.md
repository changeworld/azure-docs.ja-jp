...
ワークスペース ランディング ページで、次の実験の設定を完了し、手順を実行します。ワークスペース ランディング ページは、あらゆるスキル レベルのデータ サイエンス実務者向けのデータ サイエンス シナリオを実行するための機械学習ツールを含む統合インターフェイスです。 Internet Explorer ブラウザーでは、ワークスペース ランディング ページはサポートされません。
...
1. **[詳細設定]** を展開し、次のようにフィールドを設定します。 これらは、トレーニング ジョブをより細かく制御するための設定です。 設定しない場合、実験の選択とデータに基づいて既定値が適用されます。
...
実験が進むにつれて、画面では、完了時に作成されたさまざまなイテレーション (モデル) で**イテレーション グラフ**と**イテレーション リスト**が更新されます。 既定では、イテレーションがメトリック スコア順に並べられています。 このチュートリアルでは、選択した **AUC_weighted** メトリックに基づいて最も高いスコアを獲得したモデルがリストの一番上に表示されます。
