...
ms.locfileid: "46989687"
---
#<a name="load-data-with-the-azure-machine-learning-data-prep-sdk"></a>Azure Machine Learning Data Prep SDK でデータを読み込む

[Azure Machine Learning Data Prep SDK](https://docs.microsoft.com/python/api/overview/azure/dataprep?view=azure-dataprep-py) を使用して、さまざまな種類の入力データを読み込むことができます。 データ ファイルの種類とパラメーターを指定することも、SDK スマート読み取り機能を使用してファイルの種類を自動検出することもできます。

## <a name="read-lines"></a>行の読み込み
...
## <a name="read-excel"></a>Excel の読み込み
Azure Machine Learning Data Prep SDK には、Excel ファイルを読み込むための `read_excel` 関数が含まれています。
```
...
Excel ファイルの最初のシートが読み込まれました。 読み込むシートの名前を明示的に指定することで、同じ結果を得ることができます。 代わりに、2 番目のシートを読み込むには、引数としてシートの名前を指定します。
```
...
ご覧のように、2 番目のシート内のテーブルにはヘッダーと 3 つの空の行があります。 したがって関数の引数を変更する必要があります。
```
...
## <a name="read-fixed-width-files"></a>固定幅ファイルの読み込み
固定幅ファイルの場合は、オフセットのリストを指定できます。 最初の列は常にオフセット 0 から始まると想定されます。

```
dataflow = dprep.read_fwf('./data/fixed_width_file.txt', offsets=[7, 13, 43, 46, 52, 58, 65, 73])
dataflow.head(5)
```
||010000|99999|BOGUS NORWAY|NO|NO_1|ENRS|列 7|列 8|列 9|
|------|------|------|-----|------|-----|-------|----|-----|----|
...
ファイルにヘッダーが存在しない場合は、最初の行をデータとして扱う必要があります。 ヘッダー キーワード引数に `PromoteHeadersMode.NONE` を渡すことで、ヘッダーの検出を回避し、正しいデータを取得できます。
...
|------|------|------|-----|------|-----|-------|----|-----|----|
|0|010000|99999|BOGUS NORWAY|NO|NO_1|ENRS|列 7|列 8|列 9|
...
データ ソース オブジェクトを作成したら、データの読み込みに進むことができます。
```
...
## <a name="read-from-azure-data-lake-storage"></a>Azure Data Lake Storage からの読み込み
...
Azure CLI を使用してサービス プリンシパルと対応する証明書を作成することができます。 この特定のサービス プリンシパルはリーダーとして構成され、その範囲は Azure Data Lake Storage アカウントの 'dpreptestfiles' のみに縮小されています
```
...
##### <a name="configure-an-azure-data-lake-storage-account-for-the-service-principal"></a>サービス プリンシパルの Azure Data Lake Storage アカウントを構成する
Azure Data Lake Storage ファイル システムの ACL を構成するには、ユーザーの objectId を使用するか、この例ではサービス プリンシパルを使用します。
```
az ad sp show --id "8dd38f34-1fcb-4ff9-accd-7cd60b757174" --query objectId
```
##### <a name="configure-read-and-execute-access-for-the-azure-data-lake-storage-file-system"></a>Azure Data Lake Storage ファイルシステムの読み取りアクセスおよび実行アクセスを構成します。
基になる HDFS ACL モデルは継承をサポートしていないため、フォルダーとファイルの ACL を個別に設定する必要があります。
```
