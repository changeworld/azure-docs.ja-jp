...
| `min_instances` | 整数 | クラスターで使用するノードの最小数。 ノードの最小数を `0` に設定すると、Azure ML はクラスターが使用されていないときに、クラスターをスケールダウンしてノードをゼロにすることができます。 `0` よりも大きい値を指定すると、クラスターが使用されていない場合でも、指定した数のノードが実行され続けます。 | | `0` |
| `max_instances` | 整数 | クラスターで使用するノードの最大数。 | | `1` |
| `idle_time_before_scale_down` | 整数 | クラスターをスケールダウンするまでのノードのアイドル時間 (秒)。 | | `120` |
