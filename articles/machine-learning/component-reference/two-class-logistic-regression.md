---
title: '2 クラス ロジスティック回帰: コンポーネント リファレンス'
titleSuffix: Azure Machine Learning
description: Azure Machine Learning の 2 クラス ロジスティック回帰コンポーネントを使用して、バイナリ分類器を作成する方法について説明します。
...
ms.locfileid: "131565702"
---
# <a name="two-class-logistic-regression-component"></a>2 クラス ロジスティック回帰コンポーネント
...
このコンポーネントでは、分類アルゴリズムが二値変数またはバイナリ変数に最適化されています。 複数の結果を分類する必要がある場合は、[多クラスのロジスティック回帰](./multiclass-logistic-regression.md)コンポーネントを使用してください。
...
1.  **2 クラスのロジスティック回帰** コンポーネントをパイプラインに追加します。  
...
+ 新しいデータについての予測を行うために、トレーニング済みのモデルと新しいデータを[モデルのスコア付け](./score-model.md)コンポーネントの入力として使用します。 
