<properties
	pageTitle="Machine Learning Studio での簡単な実験 | Microsoft Azure"
...
4. **[Begin With (次で始まる)]** の **[No columns (列なし)]** をクリックし、フィルター行で **[Include (含める)]** と**列名**を選択します。列名の一覧を入力します。これにより、モジュールでは、指定する列のみがフィルターされます。
	...
	> [AZURE.TIP] 実験を実行することで、データに対する列の定義が確実に、[見つからないデータのクリーンアップ][clean-missing-data]モジュールを介して元のデータセットから渡されるようにしました。つまり接続する他のモジュールにも、このデータセットから情報が渡されます。
