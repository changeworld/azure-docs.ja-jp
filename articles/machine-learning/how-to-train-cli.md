---
title: 2\.0 CLI を使用してモデルをトレーニングする (ジョブを作成する)
...
ms.locfileid: "110382645"
---
# <a name="train-models-create-jobs-with-the-20-cli-preview"></a>2\.0 CLI を使用してモデルをトレーニングする (ジョブを作成する) (プレビュー)

Machine Learning 用の Azure 2.0 CLI 拡張機能 (プレビュー) を使用すると、モデルのライフサイクルが追跡され監査可能な状態で Azure コンピューティングでのスケールアップとスケールアウトを行い、モデルのトレーニング プロセスを加速させることができます。
...
Azure Machine Learning CLI の場合、ジョブは YAML 形式で作成されます。 ジョブに集約されるもの:
...
これは、ログ ファイルに 1 行出力する以外は何も出力しない、サンプルのジョブです。 通常は、システムによって生成されたログに加えて、モデル バイナリや付随するメタデータなどの追加の成果物を生成します。
...
- 実行履歴を Azure Machine Learning スタジオに保存する
- リモート コンピューティング先での実行を再現する (スケールアップ、スケールアウト、ハイパーパラメーターのスイープ)
- ソース コードの git リポジトリやコミットなど、実行の送信の詳細を追跡する
- モデルのメトリック、メタデータ、および成果物を追跡する
- ローカル環境でのインストールとパッケージ管理を避ける
...
`cpu-cluster` を作成したら、モデルとそれに付随するメタデータを出力する基本的なトレーニング ジョブを実行できます。 ジョブ YAML ファイルの詳細を確認してみましょう。
