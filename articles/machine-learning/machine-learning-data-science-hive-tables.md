...
ユーザーは Web ブラウザーに URL (https://<Hadoop cluster name>.azurehdinsight.net/Home/HiveEditor) を入力することによって、[クエリ コンソール (ハイブ エディター)] を使用することもできます (ログインするために Hadoop クラスターの資格情報を入力するよう求められます)。または、[PowerShell を使用してハイブ ジョブを送信する](../hdinsight/hdinsight-submit-hadoop-jobs-programmatically.md)こともできます。 
