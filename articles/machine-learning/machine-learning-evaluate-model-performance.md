...
Azure Machine Learning では、[[モデルの評価]][evaluate-model] と [[モデルのクロス検証]][cross-validate-model] という 2 つの主な機械学習モジュールによってモデルの評価を行うことができます。 これらのモジュールを使用すれば、機械学習と統計情報でよく使用されるさまざまなメトリックの観点からモデルのパフォーマンスを確認できます。
