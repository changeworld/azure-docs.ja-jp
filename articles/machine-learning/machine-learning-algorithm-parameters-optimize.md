...
## トレーニング、評価、および比較を行う:  
同じ[調整モデル ハイパーパラメーター][tune-model-hyperparameters] モジュールで、パラメーターのセットに対応するすべてのモデルをトレーニングし、さまざまなメトリックを評価した後、ユーザーが選択したメトリックに基づいて最適なトレーニング済みのモデルを出力します。このモジュールには、次の 2 つの必須の入力があります。
