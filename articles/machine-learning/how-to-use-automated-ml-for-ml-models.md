...
1. (省略可能) 追加の構成設定の表示: トレーニング ジョブをより細かく制御するのに使用できる追加の設定です。 設定しない場合、実験の選択とデータに基づいて既定値が適用されます。 
    ...
    自動特徴付け| 自動化された機械学習によって行われる特徴量化を有効または無効にするように選択します。 自動特徴量化には、合成的特徴を生成するための自動データ クレンジング、準備、変換が含まれます。 時系列予測のタスクの種類ではサポートされていません。 [特徴量化についてさらに理解を深める](how-to-configure-auto-features.md#featurization)。 
...
1. **[Deploy model]\(モデルのデプロイ\)** ペインにデータを入力します。
    ...
    カスタム デプロイ アセットを使用する| 独自のスコアリング スクリプトと環境ファイルをアップロードする場合は、この特徴量を有効にします。 [スコアリング スクリプトの詳細を確認](how-to-deploy-and-where.md#script)してください。
...
1. **[デプロイ]** を選択します。 デプロイの完了には 20 分程度かかる場合があります。
    デプロイが開始されると、 **[モデルの詳細]** タブが表示されます。 **[プロパティ]** ウィンドウの **[デプロイ ステータス]** セクションで、デプロイの進行状況を確認します。 
