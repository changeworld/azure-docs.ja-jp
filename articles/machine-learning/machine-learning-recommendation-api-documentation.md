...
##7\.ビジネス ルールのモデル
サポートされているツールの種類は次のとおりです。 - <strong>BlockList</strong> - BlockList は推奨事項の結果の一部として返したくない項目の一覧を提供します。 - <strong>FeatureBlockList</strong> - Feature BlockList では、その機能の値に基づいて項目をブロックできます。 - <strong>Upsale</strong> - Upsale では、項目を強制的に推奨事項の結果の一部として返すことができます。 - <strong>WhiteList</strong> - White List では、項目の一覧からのみ推奨事項を提案できます。 - <strong>FeatureWhiteList</strong> - Feature White List では、特定の機能値を持つ項目のみを推奨できます。 - <strong>PerSeedBlockList</strong> - Per Seed Block List では、推奨事項の結果として返すことのできない項目の一覧を項目ごとに提供できます。
...
|FbtSimilarityFunction | ビルドで使用する類似関数を定義します。 | String | 共起、リフト、Jaccard (リフト) |
