...
1. Azure Stream Analytics (ASA) に Power BI 出力を追加します。
   ...
   * Azure Stream Analytics ジョブの出力を Power BI ダッシュボードとして設定するには、「[Azure Stream Analytics & Power BI: ストリーミング データをリアルタイムで表示するリアルタイム分析ダッシュボード](../stream-analytics/stream-analytics-power-bi-dashboard.md)」の手順に従う必要があります。
