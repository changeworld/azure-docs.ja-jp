...
4. *新しいエンドポイントを追加する* 上記の手順 2で発行したスコア付け Web サービスは、既定のエンドポイントで作成されました。既定のエンドポイントは元のトレーニングスコア付け実験との同期が維持されるため、既定のエンドポイントにおけるトレーニング済みのモデルを置き換えることはできません。更新できるエンドポイントを作成するには、Azure クラシック ポータルにアクセスし [エンドポイントの追加] をクリックします (詳細は[こちら](machine-learning-create-endpoint.md))。
