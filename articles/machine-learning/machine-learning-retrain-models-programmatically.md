...
2. *スコア付け実験を作成して Web サービスとして発行する*
	...
	実験の実行が完了したら、[予測実験の作成] をクリックします。これにより予測実験が作成され、モデルはトレーニング済みのモデルとして保存され、次に示すように "Web サービス入力" モジュールと "Web サービス出力" モジュールが追加されます。次に、[実行] をクリックします。

	実験の実行が完了した後で [Web サービスの発行] をクリックすると、予測実験が Web サービスとして発行され、既定のエンドポイントが作成されます。この Web サービスのトレーニング済みのモデルは更新できます。以下をご覧ください。これで、このエンドポイントの詳細が画面に表示されます。
...
4. *新しいエンドポイントを追加する* 上記の手順 2 で発行した予測 Web サービスは、既定のスコア付けエンドポイントです。既定のエンドポイントは元のトレーニングスコア付け実験との同期が維持されるため、既定のエンドポイントにおけるトレーニング済みのモデルを置き換えることはできません。更新できるモデルで新しいスコア付けエンドポイントを作成するには、Azure クラシック ポータルにアクセスし [エンドポイントの追加] をクリックします (詳細は[こちら](machine-learning-create-endpoint.md))。[こちら](https://github.com/raymondlaghaeian/AML_EndpointMgmt/blob/master/Program.cs)で提供されているサンプル コードを使用して、スコア付けエンドポイントを追加することもできます。

5. *新しいデータと BES によりモデルの再トレーニングを行う* モデルを再トレーニングするには、上記の手順 3 で作成した Web サービスの BES 関数を呼び出す必要があります。
...
6. *再トレーニングの結果を評価する* 上記の呼び出しの出力から、評価結果にアクセスする URL と SAS トークンを取得できます。

	上記の output2 の出力結果の BaseLocation、RelativeLocation、SasBlobToken を組み合わせてブラウザーのアドレス バーに完全な URL を貼り付けると、再トレーニング済みのモデルのパフォーマンス結果を確認できます。

	これにより、新しくトレーニングを行ったモデルが、既存のモデルに代わるのに十分なパフォーマンスを行うかどうかがわかります。

7. *追加のエンドポイントにおけるトレーニング済みのモデルを更新する* このプロセスを完了するには、上記の手順 4 で追加した新しいエンドポイントのトレーニング済みのモデルを更新する必要があります。

	- Azure ポータルを使用して新しいエンドポイントを追加した場合は、Azure ポータルで新しいエンドポイントの名前をクリックし、次に UpdateResource リンクをクリックして、エンドポイントのモデルを更新するときに必要な URL を入手できます。
	- サンプル コードを使用してエンドポイントを追加すると、その呼び出しの出力に HelpLocationURL が含まれます。この URL をコピーし、ブラウザーに貼り付けます。次に、[Update Resoruce (リソースの更新)] リンクをクリックします。PATCH 要求の POST URL をコピーします (たとえば、newendpoint2 の場合の PATCH URL は https://management.azureml.net/workspaces/00bf70534500b34rebfa1843d6/webservices/af3er32ad393852f9b30ac9a35b/endpoints/newendpoint2 です)

	前述の手順 5.a による BES 関数出力は、再トレーニング済みモデルの場所が含まれている output1 の再トレーニング結果を示します。ここで、このトレーニング済みのモデルを取得し、スコア付けエンドポイント (上記の手順 4 で作成) を更新する必要があります。サンプル コードを次に示します。
	...
	この呼び出しの "ApiKey" と "endpointUrl" は、エンドポイントのダッシュボードに表示されます。Resources の "Name" パラメーターは、予測実験で保存したトレーニング済みのモデルの名前と一致する必要があります。
	
	SAS トークンは 1 時間 (55 分) 後に有効期限が切れることに注意してください。ジョブ Id で GET を行って新しいトークンを取得する必要があります。

	この呼び出しが成功すると、新しいエンドポイントが約 15 ～ 30 秒以内に再トレーニング済みのモデルを使用して開始します。
