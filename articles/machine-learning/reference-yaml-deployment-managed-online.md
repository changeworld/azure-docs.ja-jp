...
| `instance_count` | 整数 (integer) | **必須。** デプロイに使用するインスタンスの数。 予想されるワークロードに基づいて値を指定します。 高可用性を確保するために、Microsoft では `3` 以上に設定することを推奨しています。 <br><br> `instance_count` は、`az ml online-deployment update` コマンドを使用して、デプロイの作成後に更新できます。 | | |
