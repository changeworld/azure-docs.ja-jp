...
`inferenceconfig.json` という名前でこのファイルを保存します。
...
`AZUREML_MODEL_DIR` 環境変数で、登録したモデルの場所を確認できます。 pip パッケージをいくつか追加したので、推論構成にもそれらのパッケージを追加する必要があります。
...
```
このファイルを `deploymentconfig.json` として保存します。
