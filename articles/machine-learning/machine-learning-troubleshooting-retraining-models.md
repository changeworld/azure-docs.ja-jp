...
6.  エンドポイントが追加されたら、エンドポイントの名前をクリックします。 次に、 **[リソースの更新]** をクリックしてパッチ適用のヘルプ ページを開きます。

>[AZURE.NOTE] 予測 Web サービスではなく、トレーニング Web サービスにエンドポイントを追加して、**[リソースの更新]** リンクをクリックすると、[Sorry, but this feature is not supported or available in this context (この機能はサポートされていないか、このコンテキストでは使用できません)] というエラーが表示されます。 この Web サービスには、更新可能なリソースがありません。 ご不便をおかけして申し訳ありません。現在、このワークフローの強化に取り組んでいます。
...
エンドポイントが存在する Web サービスは、Azure クラシック ポータルにアクセスして確認できます。 
