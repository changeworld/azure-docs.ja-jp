...
Web サービスでは、データは **Web サービスの入力**モジュールから[メタデータ エディター][metadata-editor] モジュールを経由して[スコア][score-model] モジュールに入力され、そこでスコア付けされます。結果は、Web サービスから **Web サービスの出力**を通して出力されます。

> [AZURE.TIP] 予測実験の構成方法に従って、[スコア モデル][score-model]から結果全体が返されます。これには、すべての入力データに加え、信用リスク値とスコア付け確率が含まれます。これとは別の結果を戻す場合、たとえば信用リスク値のみを戻す場合は、 [列の投影][project-columns]モジュールを[スコア モデル][score-model]と**Web サービスの主力**の間に挿入して、Web サービスから返さない列を排除します。
