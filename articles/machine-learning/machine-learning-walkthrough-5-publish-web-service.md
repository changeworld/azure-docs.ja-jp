...
Web サービスでは、データは **Web サービスの入力**モジュールから[メタデータの編集][edit-metadata]モジュールを経由して[モデルのスコア付け][score-model]モジュールに入力され、そこでスコア付けされます。結果は、Web サービスから **Web サービスの出力**を通して出力されます。
