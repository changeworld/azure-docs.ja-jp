...
# CRM API の概要
Dynamics CRM Online に接続して、新しいレコードの作成、項目の更新などを行います。

CRM Online API は PowerApps Enterprise とロジック アプリから使用できます。
...
| トリガー | アクション|
| --- | --- |
|ありません。| <ul><li>新しいレコードを作成する</li><li>レコードを取得する</li><li>レコードを削除する</li><li>レコードを取得する</li><li>エンティティを取得する</li><li>項目を更新する</li></ul>
...
### PowerApps に構成を追加する
PowerApps Enterprise に CRM Online を追加するときに、Dynamics CRM Online Azure Active Directory (AAD) アプリケーションの**クライアント ID** と**アプリ キー**の値を入力します。CRM Online アプリケーションには、**リダイレクト URL** 値も使用されます。アプリケーションがない場合は、次の手順でアプリケーションを作成できます。

1. [Azure ポータル](https://portal.azure.com)で **[Active Directory]** を開き、組織のテナント名を選択します。
2. [アプリケーション] タブで **[追加]** を選択します。**[アプリケーションの追加]** で次の操作を行います。  
...
4. 新しいアプリケーションで、**[構成]** を選択します。
5. _[OAuth 2]_ の **[応答 URL]** を、Azure ポータルで CRM Online API を追加すると表示されるリダイレクト URL 値に設定します。![Contoso AAD アプリケーションの構成][12]

**クライアント ID** 値と**アプリ キー**値をコピーして、Azure ポータルの CRM Online 構成に貼り付けます。

### ロジック アプリに構成を追加する
この API をロジック アプリに追加する場合、Dynamic CRM Online にサインインする必要があります。

接続を作成したら、テーブルやデータセットなど、CRM Online のプロパティを入力します。これらのプロパティについては、このトピックの **REST API リファレンス**を参照してください。
...
## 次のステップ
CRM Online API を PowerApps Enterprise に追加したら、この API をアプリで利用する[許可をユーザーに与えます](../power-apps/powerapps-manage-api-connection-user-access.md)。
