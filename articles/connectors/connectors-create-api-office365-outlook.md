...
### 既読としてマークする 
電子メール メッセージを既読としてマークします。```POST: /Mail/MarkAsRead/{messageId}```
...
### 新しい電子メールの着信時 
新しい電子メールが着信したときにフローをトリガーします。```GET: /Mail/OnNewEmail```
...
### 承認の電子メールを送信します 
承認の電子メールを送信し、宛先に指定した受信者からの返信を待ちます。```POST: /approvalmail/$subscriptions```
