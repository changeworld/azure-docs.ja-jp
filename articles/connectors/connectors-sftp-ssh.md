...
* SFTP-SSH アクションは、データを 1GB ピースではなく *15 MB ピース*で管理することで、*1 GB 以下*のファイルも読み書きできます。
...
* アクションは、SFTP コネクタと比較して*最大 1 GB* のファイルを読み書きできますが、データは 1 GB ピースではなく、15 MB ピースで処理します。 ファイルが 1 GB を超える場合、アクションで[メッセージ チャンク](../logic-apps/logic-apps-handle-large-messages.md)を使用することもできます。 現在、SFTP-SSH トリガーではチャンクはサポートされていません。
...
ファイルの内容を要求するときに、トリガーでは 15 MB より大きいファイルを取得しません。 15 MB より大きいファイルを取得するには、次のパターンに従います。 
