<properties urlDisplayName="How to configure" pageTitle="クラウド サービスの構成方法 - Azure" metaKeywords="クラウド サービスの構成" description="Azure のクラウド サービスの構成方法について説明します。クラウド サービスの構成の更新方法と、ロール インスタンスへのリモート アクセスの構成方法を紹介します。" metaCanonical="" services="cloud-services" documentationCenter="" title="How to Configure Cloud Services" authors="davidmu" solutions="" manager="timlt" editor="" />
...
2. **[監視]** で、監視レベルの設定 ([詳細] または [最小])、および詳細監視に必要な診断接続文字列の構成を行います。手順については、「[クラウド サービスの監視方法]」を参照してください(../how-to-monitor-a-cloud-service/)。
...
2. **[リモート]** をクリックします。
	...
	**警告:** 最初にリモート デスクトップを有効にして [OK] (チェックマーク) をクリックするにすると、すべてのロール インスタンスが再起動されます。再起動を防止するには、パスワードの暗号化に使用する証明書をロールにインストールする必要があります。
