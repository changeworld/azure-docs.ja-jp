...
オートパイロット モードでコンテナーとデータベースを構成するときは、超えてはならない最大スループット `Tmax` を指定する必要があります。 このようにすると、`0.1*Tmax < T < Tmax` の範囲内で、ワークロードのニーズに基づいて、コンテナーを瞬時にスケーリングできます。 つまり、コンテナーとデータベースは、構成した最大スループット値の 10% から、構成した最大スループット値までの範囲で、ワークロードのニーズに基づいて瞬時にスケーリングされます。 オートパイロットのデータベースまたはコンテナーでの最大スループット (Tmax) の設定は、いつでも変更できます。
