---
title: オートパイロット モードでスループットを使用して Azure Cosmos のコンテナーとデータベースを作成する
...
ms.locfileid: "73510112"
---
# <a name="create-azure-cosmos-containers-and-databases-with-provisioned-throughput-in-autopilot-mode-preview"></a>オートパイロット モードでプロビジョニング済みスループットを使用して Azure Cosmos のコンテナーとデータベースを作成する

Azure Cosmos DB を使用すると、手動モードまたはオートパイロット モードでコンテナーにスループットをプロビジョニングできます。 この記事では、オートパイロット モードの利点とユース ケースについて説明します。

> [!NOTE]
> 現在、オートパイロット モードはパブリック プレビューで利用できます。
...
オートパイロット モードでコンテナーとデータベースを構成するときは、超えてはならない最大スループット `Tmax` を指定する必要があります。 このようにすると、`0.1*Tmax < T < Tmax` の範囲内で、ワークロードのニーズに基づいて、コンテナーを瞬時にスケーリングできます。 つまり、コンテナーとデータベースは、構成されているスループット値の 10% から、構成されている最大値までの範囲で、ワークロードのニーズに基づいて瞬時にスケーリングされます。 オートパイロットのデータベースまたはコンテナーでの最大スループット (Tmax) の設定は、いつでも変更できます。
...
|---------|---------|---------|
| **プロビジョニング済みスループット** | 手動でプロビジョニングします | ワークロードの使用パターンに基づいて、事前および事後にスケーリングされます。 |
| **要求/操作のレート制限 (429)**  | 使用量がプロビジョニング済みの容量を超えた場合、発生する可能性があります。 | 発生しません。  |
...
1. **[新しいデータベース]** を選択し、データベースの名前を入力します。 **[Autopilot]\(オートパイロット\)** オプションで、 **[有効]** を選択し、オートパイロット オプション使用時にデータベースが超えてはならないスループットの最大値を指定します。

   ![オートパイロット モードでデータベースを作成する](./media/provision-throughput-autopilot/create-database-autopilot-mode.png)

1. **[OK]** を選択します。

同様の手順で、オートパイロット モードのプロビジョニング済みスループットを使用してコンテナーを作成することもできます。
