...
1. それは `posts` コンテナーに対して実行されているため、パフォーマンスがよくありません。このコンテナーは、次の値ではパーティション分割されていません `userId`
1. 言うまでもありませんが、このパフォーマンスの問題を解決するには、次の値によってパーティション分割されて "*いる*" コンテナーに対して要求を実行します `userId`
...
- [データベース、コンテナー、アイテムの操作](databases-containers-items.md)
