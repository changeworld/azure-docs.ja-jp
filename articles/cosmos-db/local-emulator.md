...
この記事の次のセクションで説明するように、システム要件に応じて、[Windows](#run-on-windows)、[Docker for Windows](local-emulator-on-docker-windows.md)、[Linux、または macOS](#run-on-linux-macos) 上でエミュレーターを実行できます。
