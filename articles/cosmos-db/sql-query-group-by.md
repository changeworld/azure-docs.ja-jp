...
さらに、クロスパーティション `GROUP BY` クエリには、最大で 21 の[集計システム関数](sql-query-aggregate-functions.md)を含めることができます。 

## <a name="examples"></a>例

これらの例では、[サンプルの栄養データ セット](https://github.com/AzureCosmosDB/labs/blob/master/dotnet/setup/NutritionData.json)を使用します。

次のクエリでは、各 foodGroup 内の項目の合計数が返されます。
