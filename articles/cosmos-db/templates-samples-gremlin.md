...
この記事に含まれているのは、Gremlin API アカウントの例のみです。他の種類の API アカウントでの例については、[Cassandra](templates-samples-cassandra.md)、[SQL](templates-samples-sql.md)、[MongoDB](templates-samples-mongodb.md)、[Table](templates-samples-table.md) 用の Azure Cosmos DB の API での Azure Resource Manager テンプレートの使用に関する記事を参照してください。
