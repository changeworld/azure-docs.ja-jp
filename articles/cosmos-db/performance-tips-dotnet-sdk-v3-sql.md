...
## <a name="sdk-usage"></a><a id="sdk-usage"></a> SDK の使用
...
Cosmos DB SDK は、多数の要求を同時に処理するように設計する必要があります。 非同期 API を使用すると、スレッドの小さなプールで、ブロック呼び出しを待たずに数千の同時要求を処理できます。 実行時間の長い同期タスクの完了を待機するのではなく、スレッドは別の要求を処理できます。

Cosmos DB SDK を使用するアプリでの一般的なパフォーマンス上の問題として、非同期である可能性のある呼び出しをブロックすることが挙げられます。 多くの場合、同期ブロック呼び出しを実行すると、[スレッド プールが枯渇](/archive/blogs/vancem/diagnosing-net-core-threadpool-starvation-with-perfview-why-my-service-is-not-saturating-all-cores-or-seems-to-stall)し、応答時間が長くなります。

**すべきではないこと**:

* [Task.Wait](/dotnet/api/system.threading.tasks.task.wait) または [Task.Result](/dotnet/api/system.threading.tasks.task-1.result) を呼び出して、非同期実行をブロックする。
* [Task.Run](/dotnet/api/system.threading.tasks.task.run) を使用して、同期 API を非同期にする。
* 共通コード パスでロックを取得する。 Cosmos DB .NET SDK は、並列でコードを実行するように設計されている場合に最もパフォーマンスが高くなります。
* [Task.Run](/dotnet/api/system.threading.tasks.task.run) を呼び出し、即時に待機する。 ASP.NET Core は通常のスレッド プールのスレッドでアプリのコードを既に実行しているため、Task.Run を呼び出しても、不要なスレッド プールのスケジューリングが追加されるだけです。 スケジュールされたコードによってスレッドがブロックされても、Task.Run はそれを回避しません。
* ブロック呼び出しを使用してクエリを同期的にドレインする `Container.GetItemLinqQueryable<T>()` で ToList() を使用しないでください。 [ToFeedIterator()](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/e2029f2f4854c0e4decd399c35e69ef799db9f35/Microsoft.Azure.Cosmos/src/Resource/Container/Container.cs#L1143) を使用して、クエリを非同期でドレインしてください。

**すべきこと**:

* Cosmos DB .NET API を非同期で呼び出します。
* [async/await](/dotnet/csharp/programming-guide/concepts/async/) パターンを活用するため、呼び出し履歴全体が非同期になっています。

[PerfView](https://github.com/Microsoft/perfview) などのプロファイラーを使用して、[スレッド プール](/windows/desktop/procthread/thread-pools)に頻繁に追加されるスレッドを見つけることができます。 `Microsoft-Windows-DotNETRuntime/ThreadPoolWorkerThread/Start` イベントは、スレッド プールに追加されたスレッドを示します。
...
## <a name="indexing-policy"></a><a id="indexing-policy"></a> インデックス作成ポリシー
