...
このチュートリアルでは、Azure Cosmos DB Table API と Azure Table Storage のシナリオで [.NET 用 Microsoft Azure Cosmos DB Table ライブラリ](https://www.nuget.org/packages/Microsoft.Azure.Cosmos.Table)を使用する方法を紹介したサンプルについて説明します。 テーブルの作成、データの挿入や更新、データの照会、テーブルの削除の方法を示した C# の例を使用して、これらのシナリオを考察しています。

このチュートリアルでは Cosmos DB 実装の詳細について説明します。同時に、Table Storage リソースを作成し、同じ NuGet パッケージと API を使用してリソースにアクセスできます (リソースの作成のみが異なります)。 選択するリソースの種類を問わず、作成した Azure サービスに固有の接続文字列を使用する必要があります。
...
>[Azure Cosmos DB Table API にデータを移行する](table-import.md)
