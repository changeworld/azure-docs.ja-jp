...
この記事に含まれているのは、Table API アカウントの例のみです。他の種類の API アカウントでの例については、[Cassandra](../cassandra/templates-samples.md)、[Gremlin](../templates-samples-gremlin.md)、[MongoDB](../mongodb/resource-manager-template-samples.md)、[SQL](../manage-with-templates.md) 用の Azure Cosmos DB の API での Azure Resource Manager テンプレートの使用に関する記事を参照してください。
