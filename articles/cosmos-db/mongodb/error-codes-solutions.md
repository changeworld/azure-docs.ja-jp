...
>[!Note]
> Azure Cosmos DB では、MongoDB エンジンをホストしません。 MongoDB [ワイヤ プロトコル バージョン 4.0](feature-support-40.md)、[3.6](feature-support-36.md) の実装と、[ワイヤ プロトコル バージョン 3.2](feature-support-32.md) 用のレガシ サポートを提供します。 そのため、これらのエラーの中には、Azure Cosmos DB の MongoDB 用 API でしか発生しないものもあります。
