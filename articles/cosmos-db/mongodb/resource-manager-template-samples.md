...
この記事に含まれているのは、MongoDB 用の Azure Cosmos DB の API の例のみです。他の種類の API アカウントでの例については、[Cassandra](../cassandra/templates-samples.md)、[Gremlin](../templates-samples-gremlin.md)、[SQL](../templates-samples-sql.md)、[Table](../table/resource-manager-templates.md) 用の Azure Cosmos DB の API での Azure Resource Manager テンプレートの使用に関する記事を参照してください。
