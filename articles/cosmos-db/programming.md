...
作成およびデータベース トリガー、ストアド プロシージャとカスタム クエリ演算子の実行はサポートされて、 [REST API](/rest/api/documentdb/)、 [Azure DocumentDB Studio](https://github.com/mingaliu/DocumentDBStudio/releases)、および[クライアント Sdk](documentdb-sdk-dotnet.md) .NET、Node.js、および JavaScript を含む多くのプラットフォームでします。
...
ストアド プロシージャが登録されているコレクションが単一パーティション コレクションである場合、トランザクションのスコープはそのコレクション内のすべてのドキュメントになります。 コレクションがパーティション分割されている場合、ストアド プロシージャは同一のパーティション キーをトランザクション スコープとして実行されます。 このとき、各ストアド プロシージャの実行には、トランザクションを実行するスコープに対応したパーティション キー値を含める必要があります。 詳細については、[Azure Cosmos DB パーティション分割](partition-data.md)に関するページをご覧ください。
