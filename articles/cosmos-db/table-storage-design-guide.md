...
このセクションでは、パフォーマンスとスケーラビリティを重視した設計に関連する Table サービスの主要機能を取り上げます。 初めて Azure Storage と Table サービスをご利用になる場合は、この文書を読み進める前に、「[Microsoft Azure Storage の概要](../storage/common/storage-introduction.md)」と「[.NET を使用して Azure Table Storage を使用する](table-storage-how-to-use-dotnet.md)」をお読みください。 このガイドで主に取り上げるのは Table サービスについてですが、Azure のキューや BLOB サービスと、それらのサービスをソリューションで Table サービスと共に使用する方法についても触れます。  
