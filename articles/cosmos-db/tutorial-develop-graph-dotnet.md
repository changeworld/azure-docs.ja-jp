...
ここでは前のセクションの Gremlin ステートメントについて詳しく説明します。 まず、Gremlin の `addV` メソッドを使用して頂点をいくつか追加します。 たとえば、次のスニペットでは、名、年齢のプロパティを含む、タイプが "person" の "Thomas Andersen" という頂点が作成されます。
