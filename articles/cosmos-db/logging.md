...
1. **[診断設定]** ページで以下の詳細情報をフォームに入力します。 
   ...
   * 次のデータを記録できます。
      ...
      * **PartitionKeyStatistics**: このログでは、パーティション キーの統計情報が報告されます。 現在、統計はパーティション キーのストレージ サイズ (KB) で表されます。 ログは、ほとんどのデータ ストレージを占有する最初の 3 つのパーティション キーに対して出力されます。
