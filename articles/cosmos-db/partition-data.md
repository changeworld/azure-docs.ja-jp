...
Azure Cosmos DB では、すべての API に対して次の種類のコンテナーをサポートしています。
...
* パーティション間でデータを均等に分散するパーティション キーを選択します。
...
* 可能な場合、パーティションの境界内でデータを取得するようにクエリを最適化します。 最適なパーティション分割方法を決める場合は、クエリ パターンに合わせます。 1 つのパーティションからデータを取得するクエリでは、最大限のパフォーマンスを実現できます。 フィルターの述語にパーティション キーを含めることで、コンカレンシー数の多いクエリ呼び出しが効率的にルーティングされるようにする。  
...
## <a name="prerequisites"></a>パーティション分割の前提条件

Azure Cosmos DB コンテナーは、固定または無制限として作成することができます。 固定サイズのコンテナーの上限は、容量が 10 GB で、スループットが毎秒 10,000 RU となります。 コンテナーを無制限として作成する場合は、スループットに最低でも 1,000 RU/秒を指定する必要があります。 スループットを共有するように、Azure Cosmos DB コンテナーを作成することもできます。 その場合、各コンテナーではパーティション キーを指定する必要があり、コンテナーは無制限に拡張可能です。 
...
* 初期スループットを 1,000 RU/秒以上にしてコンテナーを作成し、パーティション キーを指定した場合、コンテナーに対して何も変更を行わずに、無制限スケーリングを利用できます。 つまり、**固定**コンテナーを作成した場合でも、1,000 RU/秒以上のスループットで初期コンテナーを作成し、パーティション キーを指定すれば、コンテナーは無制限のコンテナーとして機能します。
...
## <a name="designing-for-partitioning"></a>パーティション キーを作成する 
...
## <a name="designing-for-scale"></a>スケールの設計
