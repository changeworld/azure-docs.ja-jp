...
データベース上のプロビジョニングされたスループットは、そのデータベース内のコンテナーで共有できます。 データベース レベルの共有スループットの新しい各コンテナーには、100 RU/秒が必要です。 共有データベース オファリングを使用してコンテナーをプロビジョニングするときは、次のようになります。

* 25 個のコンテナーごとに 1 つのパーティション セットにグループ化され、データベース スループット (D) はパーティション セット内のコンテナー間で共有されます。 データベース内のコンテナーの数が最大でも 25 個であり、どの時点でも 1 つのコンテナーしか使用されない場合は、そのコンテナーで最大 "D" のスループットを使用できます。

* 25 個のコンテナーの後で作成される新しいコンテナーごとに、新しいパーティション セットが作成され、新しく作成されたパーティション セットの間でデータベースのスループットが分割されます (2 個のパーティション セットの場合は D/2、3 個のパーティション セットの場合は D/3)。 どの時点でも、データベースから 1 つのコンテナーのみを使用する場合は、最大のスループット (D/2、D/3、D/4 ... ) をそれぞれ使用できます。 スループットが低下するので、1 つのデータベースには 25 個より多くのコンテナーを作成しないことをお勧めします。

**例**

* プロビジョニングされたスループット 10K RU/秒 で "MyDB" という名前のデータベースを作成する場合。

* "MyDB" の下に 25 個のコンテナーをプロビジョニングすると、すべてのコンテナーが 1 つのパーティション セットにグループ化されます。 どの時点でも、データベースから 1 つのコンテナーのみを使用する場合は、最大 10K RU/秒 (D) を使用できます。

* 26 番目のコンテナーをプロビジョニングすると、新しいパーティション セットが作成され、スループットは両方のパーティション セット間で均等に分割されます。 したがって、どの時点でも、データベースから 1 つのコンテナーのみを使用する場合は、最大 5K RU/秒 (D/2) を使用できます。 2 つのパーティション セットがあるため、スループット共有性係数は D/2 に分割されます。

   次の図は、前の例をグラフィカルに示したものです。

   ![データベース レベルのスループットでの共有性係数](./media/set-throughput/database-level-throughput-shareability-factor.png)
