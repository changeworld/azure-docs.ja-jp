...
DocumentDB API SQL の構文について詳しく説明する前に、Cosmos DB のインデックス作成に関する設計について説明します。 
...
### <a id="Joins"></a>結合
リレーショナル データベースでは、テーブル間を結合できることは重要です。 このことは、正規化されたスキーマの設計においては論理的必然です。 一方 DocumentDB API は、スキーマフリーのドキュメントの正規化されていないデータ モデルを扱います。 これは、論理的には "自己結合" と同義です。
...
従来の SQL とは異なり、DocumentDB API SQL では、値がデータベースから取得されるまで値の型は未知です。 クエリの実行を効率化するため、大部分の演算子には厳密な型の要件があります。 
...
`x-ms-documentdb-populatequerymetrics` ヘッダーを `True` に設定することによって、クエリ実行についての詳細なメトリックを取得できます。 詳細については、[Azure Cosmos DB DocumentDB API の SQL クエリ メトリック](documentdb-sql-query-metrics.md)に関するページをご覧ください。
