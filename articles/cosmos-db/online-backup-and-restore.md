...
ms.locfileid: "48902673"
---
# <a name="automatic-online-backup-and-restore-with-azure-cosmos-db"></a>Azure Cosmos DB での自動オンライン バックアップと復元
Azure Cosmos DB は、すべてのデータのバックアップを、一定の間隔で自動的に取得します。 自動バックアップは、データベース操作のパフォーマンスにも可用性にも影響を与えずに取得されます。 すべてのバックアップは別のストレージ サービスに保存され、これらのバックアップは、局地的な障害の発生時に回復機能を提供するためにグローバルにレプリケートされます。 自動バックアップは、Cosmos DB コンテナーを誤って削除した後でデータの回復が必要になるシナリオを想定しています。  
...
SQL API では、独自のスナップショットを保持する場合は、次のオプションを使用します。

* Azure Cosmos DB [データ移行ツール](import-data.md#export-to-json-file)の JSON へのエクスポート オプションを使用して、追加のバックアップをスケジュールします。

* [Azure Data Factory](../data-factory/connector-azure-cosmos-db.md) を使用して、定期的にデータを移動します。

* Azure Cosmos DB の[変更フィード](change-feed.md)を使用して、完全バックアップのデータを定期的に読み取り、変化の増分のデータは個別に読み取って、対象 BLOB に移動します。 

* ウォーム バックアップを管理するために、変更フィードから定期的にデータを読み取り、別のコレクションへの書き込みは遅延させることができます。 こうすることで、データを復元する必要がなくなり、すぐに問題のデータを調べることができます。 

> [!NOTE]
> "データベース レベルで一連のコンテナーのスループットをプロビジョニング" する場合、完全なデータベース アカウント レベルで復元が行われることに注意してください。 コンテナーを誤って削除した場合は、必ず 8 時間以内にサポート チームに連絡することも必要です。 8 時間以内にサポート チームに連絡しなかった場合は、データを復元できなくなります。
...
> [!NOTE]
> コレクションまたはデータベースは、お客様から明示的な要求があった場合にのみ復元することができます。 データの調整後、コンテナーまたはデータベースの削除は、お客様が直ちに行ってください。 復元されたデータベースまたはコレクションを削除しなかった場合は、要求ユニット、ストレージ、およびエグレスに対するコストが発生します。
...
破損が検出されたらすぐに破損したコンテナー (コレクション/グラフ/テーブル) を削除し、破損したデータでバックアップが上書きされないように保護してください。 最も重要なのは、Microsoft サポートに連絡し、重大度 2 で特定の要求のチケットを作成することです。 
...
このようなシナリオで復元が行われた場合、データは ("-restored" というサフィックスを使って) 別のアカウントとコンテナーに復元されます。 この復元は、お客様がデータの検証を実行し、必要に応じてデータを移動できるようにする目的で行われるものではありません。 復元されたコンテナーは、同じ RU とインデックス作成ポリシーを使用して、同じリージョン内に配置されます。 サブスクリプションの管理者または共同管理者であるユーザーは、この復元されたアカウントを見ることができます。


> [!NOTE]
> 破損の問題を解決するため、または単にテストのためにデータを復元する場合は、タスク終了後、すぐにそれらを削除するように計画してください。復元されたコンテナーまたはデータベースには、プロビジョニングされたスループットに基づいて、余分なコストがかかります。 
