...
## <a name="what-to-expect-during-a-region-outage"></a>リージョンの停止中に予期されること
...
| -- | -- | -- | -- |
| 単一の書き込みリージョン | 無効 | 読み取りリージョンで停止状態が発生した場合、すべてのクライアントは他のリージョンにリダイレクトされます。 読み取りまたは書き込みの可用性が失われることはありません。 データの損失はありません。 <p/> 書き込みリージョンで停止状態が発生した場合、クライアントで書き込みの可用性が失われる可能性があります。 データの損失は、選択した一貫性レベルによります。 <p/> 停止状態が終わると、Cosmos DB は書き込みの可用性を自動的に復元します。 | 停止状態が発生したときに、読み取りトラフィックをサポートするために、残りのリージョンに十分な容量がプロビジョニングされていることを確認します。 <p/> 成功しないからといって、停止状態発生時に手動フェールオーバーを *トリガーしないで* ください。 <p/> 停止状態が発生した場合は、必要に応じてプロビジョニング済みの容量を再調整します。 |
| 単一の書き込みリージョン | Enabled | 読み取りリージョンで停止状態が発生した場合、すべてのクライアントは他のリージョンにリダイレクトされます。 読み取りまたは書き込みの可用性が失われることはありません。 データの損失はありません。 <p/> 書き込みリージョンで停止状態が発生した場合、クライアントは、ユーザーの好みに応じて新しいリージョンを新しい書き込みリージョンとしてCosmos DB により自動的に選択するまで、書き込みの可用性が失われます。 データの損失は、選択した一貫性レベルによります。 | 停止状態が発生したときに、読み取りトラフィックをサポートするために、残りのリージョンに十分な容量がプロビジョニングされていることを確認します。 <p/> 成功しないからといって、停止状態発生時に手動フェールオーバーを *トリガーしないで* ください。 <p/> 停止状態が発生した場合は、障害が発生したリージョンのレプリケートされていないデータを [競合フィード](how-to-manage-conflicts.md#read-from-conflict-feed)から回復し、書き込みリージョンを元のリージョンに戻してから、必要に応じてプロビジョニング済みの容量を再調整することができます。 |
| 複数の書き込みリージョン | 該当なし | 読み取りまたは書き込みの可用性が失われることはありません。 <p/> 選択された整合性レベルに従ってデータ損失が発生しました。 | 停止状態が発生したときに、追加のトラフィックをサポートするために、残りのリージョンに十分な容量がプロビジョニングされていることを確認します。 <p/> 障害が発生した場合は、障害が発生したリージョンのレプリケートされていないデータを [競合フィード](how-to-manage-conflicts.md#read-from-conflict-feed) から回復し、必要に応じてプロビジョニング済みの容量を再調整することができます。 |
