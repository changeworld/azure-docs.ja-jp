...
- Azure リージョンが永久に回復不可能になるという、きわめてまれで不運な状況でも、複数リージョン Cosmos アカウントが *[厳密]* の既定の整合性レベルで構成されていれば、データ損失は発生しません。 永久に回復不可能な書き込みリージョンが発生した場合、有界整合性制約の整合性で構成された複数リージョン Cosmos アカウントでは、潜在的なデータ損失期間は整合性制約期間 (*K* または *T*) に制限されます。セッション、一貫性のあるプレフィックス、最終的整合性の各レベルの場合、潜在的なデータ損失期間は最大 5 秒に制限されます。
