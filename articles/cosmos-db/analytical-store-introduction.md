...
* スキーマの各入れ子レベルでの最大プロパティ数は 200 であり、入れ子の深さの最大値は 5 です。
  ...
  * 最上位レベルのプロパティが 201 の項目は、この制約を満たしていないため、分析ストアでは表されません。
  * スキーマ内の入れ子レベルが 5 を超える項目もこの制約を満たしていないため、分析ストアでは表されません。 たとえば、次の項目は要件を満たしていません。

     `{"level1": {"level2":{"level3":{"level4":{"level5":{"too many":12}}}}}}`

* プロパティ名は、大文字と小文字を区別せずに比べたときに一意になっている必要があります。 たとえば、次の項目はこの制約を満たしていないため、分析ストアでは表されません。

  `{"Name": "fred"} {"name": "john"}` – "Name" と "name" は、大文字と小文字を区別せずに比べると同じになっています。
...
* プロパティは、どのような項目でも常に同じ型にします。
  ...
  * たとえば、`{"a":123} {"a": "str"}` は、`"a"` が文字列の場合と数値の場合があるため、適切に定義されたスキーマではありません。 この場合、分析ストアでは、`“a”` のデータ型は、コンテナーの有効期間内で最初に検出された項目での `“a”` のデータ型として登録されます。 `“a”` のデータ型が異なる項目は、分析ストアに含まれません。
