...
複数のリージョンによる強固な整合性で構成された Azure Cosmos アカウントの場合、書き込み待機時間は、99 パーセンタイルで 2 つの最も遠いリージョンの間のラウンドトリップ時間 (RTT) の 2 倍 + 10 ミリ秒未満となるように保証されています。
...
|>1|シングルマスター|有界整合性制約|*K* & *T*|15 分未満|
|>1|シングルマスター|強固|0|15 分未満|
|>1|マルチマスター|セッション、一貫性のあるプレフィックス、最終的|15 分未満|0|
|>1|マルチマスター|有界整合性制約|*K* & *T*|0|
