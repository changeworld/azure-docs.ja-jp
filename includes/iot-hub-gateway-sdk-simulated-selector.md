...
> [!NOTE]
> モジュールが他のモジュールに直接メッセージを渡すことはありません。 下の図に示すように、モジュールが内部のブローカーにメッセージを発行し、メッセージ バスが他のモジュールにメッセージを配信します。これにはサブスクリプション メカニズムが使用されます。 詳細については、[Gateway SDK の使用][lnk-gw-getstarted]に関するページを参照してください。
> 
> 
