...
> [AZURE.NOTE] Azure の仮想マシンでは、オペレーティング システム ディスク、ローカル一時ディスク、オプションのデータ ディスクなど、さまざまな種類のディスクが使用されます。詳細については、「[About Disks and VHDs for Virtual Machines (Virtual Machines 用のディスクと VHD について)](../articles/virtual-machines/virtual-machines-linux-about-disks-vhds.md)」を参照してください。仮想マシンを削除しない限り、オペレーティング システム ディスクをデタッチすることはできません。
...
2. 	`azure vm disk list
	<virtual-machine-name>` を使用して、仮想マシンに接続されているディスクを確認します。
