...
| プレミアム | Premium レジストリは、高ボリュームのシナリオをサポートするストレージ機能の強化を含め、ストレージ、同時実行操作などの制約の上限が高く設定されています。 イメージ スループット容量の増加に加えて、Premium では、geo レプリケーションのような機能が追加され、複数のリージョン間で 1 つのレジストリが管理され、各デプロイにネットワーク上の近いレジストリが確保されます。 |
