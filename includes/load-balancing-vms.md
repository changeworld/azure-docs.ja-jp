<properties title="Azure インフラストラクチャ サービスの負荷分散" pageTitle="Azure インフラストラクチャ サービスの負荷分散" description="Traffic Manager とロード バランサーを利用して負荷分散を行う機能について説明します。" metaKeywords="" services="virtual-machines" solutions="" documentationCenter="" authors="cherylmc" videoId="" scriptId="" manager="adinah" />
...
詳細については、「[Traffic Manager での負荷分散方法について][Traffic Manager での負荷分散方法について]」を参照してください。
...
詳細については、「[Traffic Manager][Traffic Manager]」を参照してください。
...
詳細については、「[Azure ロード バランサー][Azure ロード バランサー]」を参照してください。負荷分散セットの作成手順については、「[負荷分散セットの構成][負荷分散セットの構成]」を参照してください。
...
詳細については、「[内部負荷分散][内部負荷分散]」を参照してください。負荷分散セットの作成手順については、「[内部負荷分散セットの構成][内部負荷分散セットの構成]」を参照してください。

<!-- LINKS -->

  [Traffic Manager での負荷分散方法について]: http://msdn.microsoft.com/ja-jp/library/azure/dn339010.aspx
  [負荷分散]: ./media/load-balancing-vms/TMSummary.png
  [Traffic Manager]: http://msdn.microsoft.com/ja-jp/library/azure/hh745750.aspx
  [1]: ./media/load-balancing-vms/LoadBalancing.png
  [Azure ロード バランサー]: http://msdn.microsoft.com/ja-jp/library/azure/dn655058.aspx
  [負荷分散セットの構成]: http://msdn.microsoft.com/ja-jp/library/azure/dn655055.aspx
  [2]: ./media/load-balancing-vms/LOBServers.png
  [内部負荷分散]: http://msdn.microsoft.com/ja-jp/library/azure/dn690121.aspx
  [内部負荷分散セットの構成]: http://msdn.microsoft.com/ja-jp/library/azure/dn690125.aspx
