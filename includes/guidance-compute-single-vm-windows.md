...
Azure で提供されるリソースおよびリソースの種類は多岐にわたるため、この参照アーキテクチャはさまざまな方法でデプロイすることができます。 上記の図に示すように、このアーキテクチャをデプロイするには、このドキュメントの最後にある「[ソリューションのデプロイ](#solution-deployment)」セクションで詳細を確認してください。 独自のリファレンス アーキテクチャを作成する場合は、推奨事項が当てはまらない特定の要件がない限り、これらの推奨事項に従ってください。
...
既存のワークロードを Azure に移動する場合は、オンプレミスのサーバーに最も適合性が高い VM サイズから開始します。 次に、CPU、メモリ、およびディスクの 1 秒あたりの入力/出力操作 (IOPS) について、実際のワークロードのパフォーマンスを測定し、必要に応じてサイズを調整します。 VM 用に複数の NIC が必要な場合は、使用可能な NIC の最大数が [VM サイズ][vm-size-tables]と関係することに注意してください。   

VM および他のリソースをプロビジョニングする際は、場所を指定する必要があります。 一般的に、内部ユーザーや顧客に最も近い場所を選択します。 ただし、すべての場所ですべての VM サイズを利用できるとは限りません。 詳細については、[リージョン別のサービス][services-by-region]に関するページを参照してください。 特定の場所で利用できる VM サイズを一覧を表示するには、次の Azure コマンド ライン インターフェイス (CLI) コマンドを実行します。
...
デプロイする予定の VM の数に対して十分なストレージ アカウントをプロビジョニングします。 各 VM には OS ディスクと一時ディスクが必要です。[ストレージ アカウントのディスク制限][storage-account-limits]に基づき、1 つの Premium Storage アカウントで 1 ～ 20 個の VM をサポートできます。 VM にデータ ディスクを追加する場合、データ ディスクは作成時にフォーマットされていません。 すべてのデータ ディスクをフォーマットするには、VM にログインする必要があります。
...
* 変化しない固定 IP アドレスが必要な場合 (たとえば、DNS に A レコードを作成する必要がある場合や IP アドレスをセーフ リストに追加する必要がある場合) は、[静的 IP アドレス][static-ip]を予約します。
...
VM の規模は、[VM サイズを変更する][vm-resize]ことでスケールアップまたはスケールダウンできます。 水平方向にスケール アウトするには、ロード バランサーの内側の可用性セットに 2 つ以上の VM を配置します。 詳細については、[Azure での複数の Windows VM の実行][multi-vm]に関するページを参照してください。
...
**VM の停止。** Azure では、"停止" 状態と "割り当て解除済み" 状態が区別されます。 VM が停止状態のときは課金されます。 VM が割り当て解除済み状態のときは課金されません。
...
Azure ポータルの **[停止]** ボタンを使用した場合も VM の割り当てが解除されます。 ただし、ログイン中に OS からシャットダウンした場合、VM は停止しますが、割り当て解除されない ** ため、引き続き課金されます。
...
これらのベスト プラクティスを実装する参照アーキテクチャの[デプロイ][github-folder]を利用できます。 このリファレンス アーキテクチャには、仮想ネットワーク (VNet)、ネットワーク セキュリティ グループ (NSG)、および単一の仮想マシン (VM) が含まれています。

このリファレンス アーキテクチャをデプロイする方法はいくつかあります。 最も簡単なのは、次の手順に従う方法です。 
...
このリファレンス アーキテクチャをデプロイするその他の方法については、GitHub の [guidance-compute-single-vm][github-folder] フォルダー内の readme ファイルを参照してください。 
