...
ビューアーを使用して、Azure Portal、Azure PowerShell、または Azure CLI を使って複数インスタンス構成で構成されている仮想マシンを報告します。 たとえば、Azure Portal を使用して、**[仮想マシン (クラシック)]** 参照ダイアログに _[可用性セット]_ を追加できます。 同じ可用性セットを報告する仮想マシンは、複数インスタンス構成の一部です。 次の例では、複数インスタンス構成は仮想マシン SQLContoso01 と SQLContoso02 で構成されています。
