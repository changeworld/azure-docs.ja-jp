...
仮想マシンの構成には、複数インスタンスと単一インスタンスの 2 種類があります。 複数インスタンス構成では、1 つの可用性セットに似たような仮想マシンが複数配置されます。
...
ビューアーを使用して、Azure Portal、Azure PowerShell、または Azure CLI を使って複数インスタンス構成で構成されている仮想マシンを報告します。 たとえば、Azure Portal を使用して、**仮想マシン (クラシック)** 参照ダイアログに _可用性セット_ を追加できます。 同じ可用性セットを報告する仮想マシンは、複数インスタンス構成の一部です。 次の例では、複数インスタンス構成は仮想マシン SQLContoso01 と SQLContoso02 で構成されています。
...
> [!NOTE]
> 可用性セットで 1 つの仮想マシン インスタンスしか実行していなくても、Azure プラットフォームではそのインスタンスを複数インスタンス構成の更新として処理します。
>
...
単一インスタンスと複数インスタンスの仮想マシンの構成に限り、Azure では計画的なメンテナンスが近づいていることを注意喚起するために電子メールを送信します (1 週間前)。 この電子メールは、サブスクリプション管理者と共同管理者の電子メール アカウントに送信されます。 この電子メールの例を次に示します。
