...
- 再起動を伴うメンテナンスの場合は、メンテナンスの予定日時が知らされます。 Azure により、都合の良いときに自分でメンテナンスを開始できる時間枠も与えられます。 セルフ メンテナンスの時間枠は、メンテナンスの実行が急務でない限り、通常は 30 日間です。 また、Azure では、計画されたプラットフォーム メンテナンスのために VM を再起動する必要があるケースを減らすための技術に投資しています。 
...
前述のとおり、プラットフォーム更新のほとんどはお客様の VM に影響を与えることなく完了します。 影響ゼロの更新が不可能であれば、Azure では、お客様の VM に対して最も影響の低い更新メカニズムを採用しています。 このような影響ゼロのメンテナンスの多くでは、VM に 10 秒以下の一時停止が発生します。 特定のケースでは、メモリ保持のメンテナンス メカニズムが使用されます。VM は最大で 30 秒間一時停止状態になり、RAM 内のメモリは保持されます。 VM が再開された後、クロックが自動的に同期されます。 メモリ保持メンテナンスは、G、M、N、H シリーズを除き、90% 以上の Azure VM で機能します。 Azure では、ライブ マイグレーション テクノロジの使用を拡大すると共に、一時停止期間を短縮するためのメモリ保持メンテナンス メカニズムの改善に努めています。  
