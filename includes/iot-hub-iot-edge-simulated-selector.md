...
このモジュールは、データをデバイスから受け取り、ゲートウェイを経由してクラウドに送信する流れの出発点となります。 サンプルのモジュールは次のことを行います。
...
上の図で "**Protocol X ingestion**" という名前のモジュールは、ソース コードでは "**Simulated device**" と呼ばれています。

### <a name="mac-lt-gt-iot-hub-id-module"></a>MAC &lt;-&gt; IoT Hub ID モジュール

このモジュールは、MAC アドレスのプロパティを持つメッセージをスキャンします。 サンプルでは、プロトコル インジェスト モジュールは MAC アドレスのプロパティを追加します。 モジュールは、このようなプロパティを検出すると、IoT Hub デバイス キーを含む別のプロパティをメッセージに追加します。 その後、モジュールはチェーン内の次のモジュールでメッセージを利用できるようにします。

開発者は、MAC アドレスと IoT Hub ID の間にマッピングを設定し、シミュレートされたデバイスを IoT Hub デバイスの ID と関連付けます。 開発者は、モジュール構成の一部として手動でマッピングを追加します。

> [!NOTE]
> このサンプルでは、MAC アドレスを一意のデバイス識別子として使用し、IoT Hub デバイス ID と関連付けていますが、 別の一意識別子を使用する独自のモジュールを記述することもできます。 たとえば、デバイスに設定されている一意のシリアル番号や、テレメトリ データに含まれる一意の埋め込みデバイス名などです。

### <a name="iot-hub-communication-module"></a>IoT Hub 通信モジュール

このモジュールは、前のモジュールによって割り当てられた、IoT Hub デバイス キー プロパティを含むメッセージを受け取ります。 モジュールは、HTTP プロトコルを使って IoT Hub にメッセージ コンテンツを送信します。 HTTP は、IoT Hub で認識される 3 つのプロトコルのうちの 1 つです。

シミュレートされたデバイスごとに接続を開く代わりに、このモジュールはゲートウェイから IoT Hub への 1 つの HTTP 接続を開きます。 その後、モジュールは、すべてのシミュレートされたデバイスからの接続をその接続に多重化します。 この方法では、1 つのゲートウェイで多くのデバイスに接続できます。
