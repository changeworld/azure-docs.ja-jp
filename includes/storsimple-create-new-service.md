...
3. 表示されるフォームで、次の手順を実行します。
   ...
   2. サービスの **[場所]** を指定します。一般的に、デバイスをデプロイする地理的リージョンに最も近い場所を選択します。あるいは次も考慮できます。 
      ...
      * StorSimple Manager サービスと Azure Storage は 2 つの別々の場所に置くことができます。 そのような場合、StorSimple Manager と Azure Storage のアカウントを別々に作成する必要があります。 Azure Storage アカウントを作成するには、管理ポータルで Azure Storage サービスを開き、[Azure Storage アカウントの作成に関するページ](../articles/storage/storage-create-storage-account.md#create-a-storage-account)に記載されている手順を実行してください。 このアカウントを作成したら、それを StorSimple Manager サービスに追加します。それには、[サービスの新しいストレージ アカウントの構成に関するページ](../articles/storsimple/storsimple-deployment-walkthrough.md#configure-a-new-storage-account-for-the-service) に記載されている手順を実行してください。
