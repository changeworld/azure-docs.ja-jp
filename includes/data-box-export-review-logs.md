...
ms.locfileid: "86208899"
---
データのコピーを始める前に、次のようにします。

1. "*コピー ログ*" をダウンロードします。 **[接続とコピー]** ページで、 **[ログのコピー]** を選択します。 メッセージが表示されたら、システムにログを保存します。 

    ![コピー ログのダウンロード 1](../articles/databox/media/data-box-deploy-export-copy-data/download-copy-log-1.png)

  
    手順を繰り返して、"*詳細ログ*" をダウンロードします。 

2. "*詳細ログ*" を確認します。 "*詳細ログ*" には、Azure Storage アカウントから正常にエクスポートされたすべてのファイルの一覧が含まれています。 ログには、ファイル サイズとチェックサムの計算も含まれています。
...
3. "*コピー ログ*" でエラーを確認します。 このログには、エラーのためにコピーできなかったファイルが示されます。

    次に示すのは、エラーが発生せず、Azure から Data Box デバイスへのデータ コピー中にすべてのファイルがコピーされた場合のコピー ログのサンプル出力です。
    ...
    ``` 
    次に示すのは、コピー ログ内にエラーがあり、一部のファイルを Azure からコピーできなかった場合のサンプル出力です。
    ...
    これらのファイルをエクスポートするには、次のオプションがあります。 

    - ネットワーク経由でコピーできなかったファイルを転送できます。 
    - データのサイズが使用可能なデバイスの容量よりも大きかった場合は、部分的なコピーが行われ、コピーされなかったすべてのファイルがこのログに一覧表示されます。 このログを入力 XML として使用して、新しい Data Box 注文を作成し、これらのファイルをコピーすることができます。
