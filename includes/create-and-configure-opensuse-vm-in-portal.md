...
**重要**: 仮想マシンが仮想ネットワークを使用する場合、仮想マシンの作成時に仮想ネットワークを指定する必要があります。仮想マシンが仮想ネットワークに接続するように設定できるのは、仮想マシンの作成時に限られています。仮想ネットワークの詳細については、「[仮想ネットワークの概要](http://go.microsoft.com/fwlink/p/?LinkID=294063)」を参照してください。
...
4. 最初の **[仮想マシンの構成]** ページで、設定を入力または確認します:
	...
	- 使用する **[認証]** の種類を決定します。一般的なパスワードのガイドラインについては、[強力なパスワード](http://msdn.microsoft.com/library/ms161962.aspx)に関するページを参照してください。
