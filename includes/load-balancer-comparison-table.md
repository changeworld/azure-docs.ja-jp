...
| [正常性プローブ ダウン動作](../articles/load-balancer/load-balancer-custom-probe-overview.md#probedown) | インスタンス プローブがダウンし、__かつ__ すべてのプローブがダウンしても TCP 接続は存続。 | インスタンス プローブがダウンしても TCP 接続は存続。 すべてのプローブがダウンすると、すべての TCP 接続が終了。 |
...
| 既定でのセキュリティ保護 | パブリック IP とロード バランサー エンドポイントに対して受信を拒否する既定設定を使用して、トラフィックが流れるようにネットワーク セキュリティ グループでホワイトリストに明示的に登録できる。 | 既定でオープン、ネットワーク セキュリティ グループは任意。 |
