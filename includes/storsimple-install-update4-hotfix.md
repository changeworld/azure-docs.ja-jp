...
5. 上の表で示されている他の修正プログラム (**4011841**) を検索し、バスケットに追加します。
6. **[バスケットの表示]**をクリックします。
...
>
> 修正プログラムは、3 つの異なるフォルダーにコピーする必要があります。 デバイス ソフトウェア更新プログラムは _FirstOrderUpdate_ フォルダーに、その他すべての中断なしの更新プログラムは _SecondOrderUpdate_ フォルダーに、メンテナンス モードの更新プログラムは _ThirdOrderUpdate_ フォルダーにコピーする必要があります。
...
3. 修正プログラムをインストールするには、コマンド プロンプトで次のように入力します。
    ...
    最初の注文の更新プログラムをインストールするためのサンプル出力を次に示します。
...
7. 2 番目の注文の更新プログラムをインストールするには、手順 3 から 5 を繰り返します。 `Start-HcsHotfix cmdlet` だけを実行し、2 番目の注文の更新プログラムが含まれているフォルダーをポイントすることで、複数の更新プログラムをインストールできます。 コマンドレットでは、フォルダー内で利用可能なすべての更新プログラムが実行されます。 更新プログラムが既にインストールされている場合、更新ロジックはそれを検出し、その更新プログラムを適用しません。 すべての修正プログラムがインストールされたら、`Get-HcsSystem` コマンドレットを使用します。 バージョンは次のようになります。
