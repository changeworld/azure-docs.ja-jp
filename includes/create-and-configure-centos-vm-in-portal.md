...
**注**: この記事では、仮想ネットワークに接続されていない仮想マシンを作成します。仮想マシンで仮想ネットワークを使用し、ホスト名による仮想マシンの直接接続またはクロスプレミス接続を設定できるようにするには、代わりに **[ギャラリーから]** を使用し、仮想マシンの作成時に仮想ネットワークを指定する必要があります。仮想ネットワークの詳細については、[Azure Virtual Network の概要](http://go.microsoft.com/fwlink/p/?LinkID=294063)に関するページを参照してください。

1. Azure アカウントを使用して、Azure 管理ポータルにサインインします。
...
4. **[仮想マシンの構成]** ページで、次の情報を指定します。
   ...
   * **[新しいパスワード]** ボックスに、[強力なパスワード](http://msdn.microsoft.com/library/ms161962.aspx)を入力します。
...
1. 管理ポータルで、**[Virtual Machines]**、新しい仮想マシンの名前、**[エンドポイント]** の順にクリックします。
...
2. **[ホスト名]** に「testlinuxvm.cloudapp.net」と入力し、**[ポート]** に「22」と入力します。![PuTTY の画面][Image6]
