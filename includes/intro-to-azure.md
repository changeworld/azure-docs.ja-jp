...
-   [モバイル][モバイル]
-   [バックアップ][バックアップ]
-   [メッセージングと統合][メッセージングと統合]
-   [コンピューティング支援][コンピューティング支援]
-   [パフォーマンス][パフォーマンス]
-   [大規模なコンピューティングとビッグ データ][大規模なコンピューティングとビッグ データ]
-   [メディア][メディア]
-   [コマース][コマース]
...
Azure では、管理ポータル内や、「[What Is Windows Azure Infographic (Windows Azure ポスター インフォグラフィックとは)][What Is Windows Azure Infographic (Windows Azure ポスター インフォグラフィックとは)]」などのさまざまな視覚的な説明の中で、サービスをいくつかのカテゴリに分類しています。管理ポータルは、Azure 内の大半のサービス (すべてではない) を管理するために使用するものです。
...
Azure には、管理者が Azure の機能の大半にアクセスして管理できる、[管理ポータル][1]と呼ばれる Web インターフェイスがあります。マイクロソフトは通常、以前のポータルを廃止する前に、より新しい UI ポータルをベータ版としてリリースします。より新しい UI ポータルは "[Azure プレビュー ポータル][Azure プレビュー ポータル]" と呼ばれます。
 ...
 **図: Azure Websites では、基になる Web サーバーを管理しなくても、クラウド内で Web サイト アプリケーションを実行できる。**
...
**図: Azure SQL Database は、クラウド内で管理されたリレーショナル データベース サービスを提供。**
...
**図: Azure Tables は、データを格納するためのフラットな NoSQL の手法を提供。**
...
**図: Azure Import/Export なら、Azure との間で物理ハード ドライブをやり取りすることで、データの一括インポートと一括エクスポートをお手頃な価格で高速に行うことができます。**
...
**図: Virtual Network は、クラウド内でプライベート ネットワークを提供。このネットワークを介してさまざまなサービスが相互通信できるほか、クロスプレミス接続に VPN 接続を設定している場合に内部設置型リソースにアクセスしたりできる。**
...
専用の仮想ネットワークの計画および作成の詳細については、「[Virtual Network][Virtual Network]」を参照してください。
...
ExpressRoute の詳細については、「[ExpressRoute の技術概要][ExpressRoute の技術概要]」を参照してください。
...
これらの SDK では、Azure アプリケーションを構築、展開、および管理できます。[www.microsoftazure.com][www.microsoftazure.com] または GitHub から入手でき、Visual Studio や Eclipse と共に使用できます。Azure には、Linux システムおよび Macintosh システムから Azure にアプリケーションを展開するためのツールなど、開発者がエディターや開発環境で使用できるコマンド ライン ツールも用意されています。
...
**図: Multi-Factor Authentication は、アプリケーションで複数の形式の ID を検証する機能を提供。**
...
**図: Azure Backup は、内部設置型の Windows Server からクラウドにデータをバックアップ。**

Azure Backup は、Windows Server が実行されている内部設置型のサーバーからクラウドにデータをバックアップします。Windows Server 2012、Windows Server 2012 Essentials、または System Center 2012 - Data Protection Manager 内のバックアップ ツールから直接バックアップを管理できます。代替として、特化したバックアップ エージェントを使用することもできます。
...
既に Windows Server または System Center を使用している場合、Azure Backup は、サーバー ファイル システム、仮想マシン、SQL Server データベースをバックアップするための最適なソリューションです。暗号化ファイル、スパース ファイル、および圧縮ファイルを処理します。いくつかの制限があるため、最初に [Azure Backup の前提条件を確認][Azure Backup の前提条件を確認]してください。
...
Azure は時間の経過と共に拡大するため、Azure Storage キューおよび Service Bus キューは類似のキュー サービスを提供しています。一方を他方より優先して使用する理由は、かなり技術的な資料である「[<link>][<link>]」で説明されています。多くのシナリオでは、どちらか一方が機能します。
...
Service Bus はリレー サービスを介した直接的な通信を実現するため、ファイアウォールを介して安全に対話する手段を確保できます。サービス バス リレーは、ローカルではなくクラウドでホストされるエンドポイントを介してメッセージを交換することによって、アプリケーションの通信ができます。
...
![HDInsight][HDInsight]
 **図: HDInsight は、大量のデータを一括処理する際に役立つ。**
...
Media Services の詳細に関するビジュアルな表示については、[Azure の Media Services ポスター][Azure の Media Services ポスター]をダウンロードしてください。

## <span id="commerce"></span></a>コマース

サービスとしてのソフトウェアの登場により、アプリケーションの作成方法が変わろうとしています。さらに、アプリケーションの販売方法も変わろうとしています。SaaS アプリケーションはクラウドに存在するため、当然ながら潜在的な顧客はソリューションをオンラインで探します。この変化は、アプリケーションだけでなくデータにも当てはまります。ユーザーは、商用データセットもクラウドで探すはずです。マイクロソフトは、[Azure Marketplace][Azure Marketplace] および [Azure ストア][Azure ストア]によってどちらの懸念事項にも対応しています。
...
これで全体像を把握できたため、次のステップは最初の Azure アプリケーションを記述することです。言語を選択して[適切な SDK を入手し][適切な SDK を入手し]、作成してみましょう。クラウド コンピューティングはこれからの基本です。今すぐ始めましょう。
  ...
  [ExpressRoute の技術概要]: http://msdn.microsoft.com/ja-jp/library/azure/dn606309.aspx
  [TrafficManager]: ./media/intro-to-azure/TrafficManagerIntroNew.png
  [www.microsoftazure.com]: http://azure.microsoft.com/ja-jp/downloads/
  ...
  [Azure Backup の前提条件を確認]: http://technet.microsoft.com/ja-jp/library/dn296608.aspx
