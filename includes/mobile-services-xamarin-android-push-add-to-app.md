...
3. **using** ステートメントと **namespace** 宣言の間に次のアクセス許可要求を追加します。
...
5. ToDoBroadcastReceiver.cs プロジェクト ファイルに、**PushHandlerService** クラスを定義する次のコードを追加します。
   ...
   > [!NOTE]
   > **GcmServiceBase** クラスでは、**OnRegistered()** メソッド、O**nUnRegistered()** メソッド、**OnMessage()** メソッド、および **OnError()** メソッドを実装しています。**PushHandlerService** クラスでは、これらのメソッドをオーバーライドする必要があります。
   > 
   > 
...
8. 次のコードで **OnUnRegistered()** メソッドと **OnError()** メソッドをオーバーライドします。
