...
5.	DocumentDB アカウントが作成されたら、オンライン ポータルの既定の設定で使用するための準備が整います。DocumentDB アカウントの既定の一貫性は **Session** に設定されることに注意してください。既定の整合性の設定は、**[要点] バー**の**キー アイコン**をクリックし、**[すべての設定]** ブレードの **[既定の整合性]** エントリをクリックすることで調整できます。
