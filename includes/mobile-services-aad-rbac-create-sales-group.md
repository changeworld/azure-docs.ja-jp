...
1. [Azure 管理ポータル]で、アプリに認証を追加するためのチュートリアルを完了したときに認証用に構成したディレクトリに移動します。
...
4. Mobile Services の構成ページに戻り、オブジェクト ID を **AAD_SALES_GROUP_ID** という名前のアプリ設定として追加します。このチュートリアルでは、グループ名に基づいて ID を検索する代わりに、グループのオブジェクト ID をアプリ設定として使用します。これは、グループ名は変更される場合がありますが、ID は同じままであるためです。
