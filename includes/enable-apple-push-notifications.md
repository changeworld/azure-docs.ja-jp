...
> [AZURE.NOTE]<strong>\[バンドル ID\]</strong> に <i>MobileServices.Quickstart</i> 以外の値を指定する場合は、Xcode プロジェクトでバンドル ID の値も更新する必要があります。クイック スタート プロジェクトで既に使用した正確なバンドル id の値を使用することをお勧めします。
...
## <a id="profile"></a>アプリのプロビジョニング プロファイルを作成する
