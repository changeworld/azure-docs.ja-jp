Bing の応答には、Bing が提供するサムネイル イメージへの URL を含むものがあります。 サムネイル イメージはサイズ変更とトリミングが可能です。 イメージのサイズを変更するには、サムネイルの URL に w (幅) および h (高さ) クエリ パラメーターを含めます。 幅と高さはピクセル単位で指定します。 次に例を示します。  
