...
> [!NOTE]
> [Search API の使用と表示の要件](/useanddisplayrequirements.md)に関するページで定められているように、サムネイルのサイズとトリミングでは検索シナリオを提供し、第三者の権利を尊重するようにしてください。


イメージのサイズを変更するには、サムネイルの URL に w (幅) および h (高さ) クエリ パラメーターを含めます。 幅と高さはピクセル単位で指定します。 次に例を示します。  
