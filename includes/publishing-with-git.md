...
この記事では、Git を使用してローカル コンピューターから Azure Websites に直接発行する方法について説明します (Azure では、この発行方法は**ローカル Git** と呼ばれます)。また、BitBucket、CodePlex、Dropbox、GitHub、Mercurial などのリポジトリ Web サイトからの継続的なデプロイを有効にする方法についても説明します。継続的な展開に TFS を使用する方法の詳細については、「[Continuous delivery to Azure using Visual Studio Online (Visual Studio Online を使用した Azure への継続的な配信)][Continuous delivery to Azure using Visual Studio Online (Visual Studio Online を使用した Azure への継続的な配信)]」を参照してください。

> [WACOM.NOTE] この記事に記載されている Git コマンドの多くは、[Mac および Linux 用 Azure コマンド ライン ツール][Mac および Linux 用 Azure コマンド ライン ツール]を使用して Web サイトを作成する際に自動的に実行されます。

このタスクの手順は次のとおりです。

-   [Git のインストール][Git のインストール]
-   [ローカル リポジトリの作成][ローカル リポジトリの作成]
-   [Web ページの追加][Web ページの追加]
-   [Web サイトのリポジトリの有効化][Web サイトのリポジトリの有効化]
-   [プロジェクトの展開][プロジェクトの展開]

    -   [ローカル ファイルの Azure へのプッシュ (ローカル Git)][ローカル ファイルの Azure へのプッシュ (ローカル Git)]
    -   [BitBucket、CodePlex、Dropbox、GitHub、Mercurial などのリポジトリ Web サイトからのファイルの展開][BitBucket、CodePlex、Dropbox、GitHub、Mercurial などのリポジトリ Web サイトからのファイルの展開]
    -   [BitBucket、CodePlex、Dropbox、GitHub、Mercurial からの Visual Studio ソリューションのデプロイ][BitBucket、CodePlex、Dropbox、GitHub、Mercurial からの Visual Studio ソリューションのデプロイ]
-   [トラブルシューティング][トラブルシューティング]

## <span id="Step2"></span></a>Git のインストール
...
## <span id="Step2"></span></a>ローカル リポジトリの作成
...
## <span id="Step3"></span></a>Web ページの追加

Azure Websites では、さまざまなプログラミング言語で作成されたアプリケーションをサポートしています。この例では、静的 .html ファイルを使用します。他のプログラミング言語の Web サイトを Azure に発行する方法の詳細については、[Azure デベロッパー センター][Azure デベロッパー センター]を参照してください。
...
## <span id="Step4"></span></a>Web サイト リポジトリの有効化

Azure ポータルを使用し、次に示す手順を実行して、Web サイトに対して Git リポジトリを有効にします。

1.  [Azure ポータル][Azure ポータル]にログインします。

2.  [新規] ボタンをクリックして、リポジトリを有効にする新しい Web サイトを作成します。

3.  **[Web サイト]** ビューで Web サイトの作成プロセスが終了するまで待ってから、Web サイトを選択します。

    ![選択した Web サイトが表示されている画像][選択した Web サイトが表示されている画像]

4.  **[ダッシュボード]** タブを選択します。

5.  **[概要]** セクションで、**[ソース管理からの展開の設定]** をクリックします。次の **[展開の設定]** ダイアログ ボックスが表示されます。

    ![git-WhereIsYourSourceCode][git-WhereIsYourSourceCode]

6.  **[ローカル Git]** を選択し、**[次へ]** 矢印をクリックします。
...
## <span id="Step5"></span></a>プロジェクトのデプロイ

### <span id="Step6"></span></a>ローカル ファイルの Azure へのプッシュ (ローカル Git)
...
2.  ポータルに表示されている手順 3. の git remote add コマンドをコピーします。そのコマンドは次のようになります。
    ...
    > [WACOM.NOTE] この **remote** コマンドは、名前付きの参照をリモート リポジトリに追加します。この例では、Azure Websites リポジトリに "azure" という名前の参照を作成しています。

3.  コマンド ラインで次のコマンドを使用して、ローカル リポジトリから "azure" リモートに現在のリポジトリのコンテンツをプッシュします。
    ...
    > [WACOM.NOTE] Azure Websites に対して作成したリポジトリでは、プッシュ要求のターゲットがそのリポジトリの **master** 分岐であると想定しています。そのため、この分岐が Web サイトのコンテンツとして使用されます。

4.  ポータルの下部にある **[参照]** をクリックして、**index.html** が展開されていることを確認します。"Hello Git!" を含むページが表示されます。

    !["Hello Git!" を含む Web ページ]["Hello Git!" を含む Web ページ]

5.  テキスト エディターを使用し、**index.html** ファイルに変更を加えて "Yay!" を追加します。次に、ファイルを保存します。

6.  コマンド ラインから次のコマンドを使用し、変更を**追加**して**コミット**します。その後で、変更をリモート リポジトリに**プッシュ**します。
    ...
    **push** コマンドが完了したら、ブラウザーを更新して (場合によっては、ブラウザーを正しく更新するには Ctrl + F5 キーを押す必要があります)、コミットした最新の変更がページのコンテンツに反映されていることを確認します。
    ...
    !["Yay!" を含む Web ページ]["Yay!" を含む Web ページ]

### <span id="Step7"></span></a>BitBucket、CodePlex、Dropbox、GitHub、Mercurial などのリポジトリ Web サイトからのファイルのデプロイ
...
GitHub、CodePlex、BitBucket からファイルを展開するには、これらのサービスのいずれかにローカル プロジェクトを発行している必要があります。これらのサービスにプロジェクトを発行する方法の詳細については、「[Create a Repo (GitHub) (Repo の作成 (GitHub))][Create a Repo (GitHub) (Repo の作成 (GitHub))]」、「[Using Git with CodePlex (CodePlex での Git の使用)][Using Git with CodePlex (CodePlex での Git の使用)]」、「[Create a Repo (BitBucket) (Repo の作成 (BitBucket))][Create a Repo (BitBucket) (Repo の作成 (BitBucket))]」、「[Using Dropbox to Share Git Repositories (Dropbox を使用した Git リポジトリの共有)][Using Dropbox to Share Git Repositories (Dropbox を使用した Git リポジトリの共有)]」、「[Quick Start - Mercurial (Mercurial のクイック スタート)][Quick Start - Mercurial (Mercurial のクイック スタート)]」を参照してください。
...
3.  継続的なデプロイに使用するソース管理方法を選択します。

4.  要求されたら、選択したサービスの資格情報を入力します。

5.  アカウントへのアクセスを Azure に許可すると、リポジトリの一覧が表示されます。

    ![git-ChooseARepositoryToDeploy][git-ChooseARepositoryToDeploy]

6.  Azure Websites と関連付けるリポジトリを選択します。チェックマークをクリックして続行します。

    > [WACOM.NOTE] GitHub または Bitbucket を使用して継続的なデプロイを有効にする場合、パブリック プロジェクトとプライベート プロジェクトの両方が表示されます。

7.  Azure によって、選択したリポジトリとの関連付けが作成され、master 分岐からファイルがプルされます。このプロセスが完了すると、**[展開]** ページの **[展開履歴]** に、次のような **[アクティブな展開]** メッセージが表示されます。

    ![git-githubdeployed][git-githubdeployed]

8.  この時点で、選択したリポジトリから Azure Websites にプロジェクトがデプロイされました。サイトがアクティブであることを確認するには、ポータルの下部にある **[参照]** リンクをクリックします。ブラウザーには Web サイトが表示されます。

9.  継続的なデプロイが行われていることを確認するには、プロジェクトに変更を加え、この Web サイトに関連付けたリポジトリに更新をプッシュします。リポジトリへのプッシュが完了すると、すぐに Web サイトは更新され、変更が反映されます。更新がプルされていることは、Web サイトの **[デプロイ]** ページで確認できます。

    ![git-GitHubDeployed-Updated][git-GitHubDeployed-Updated]

### <span id="Step75"></span></a>BitBucket、CodePlex、Dropbox、GitHub、Mercurial からの Visual Studio ソリューションのデプロイ

Visual Studio ソリューションを Azure Websites にプッシュすることは、単純な index.html ファイルをプッシュすることを同じくらい簡単です。Azure Websites のデプロイ プロセスでは、NuGet 依存関係の復元やアプリケーション バイナリの構築などのすべての詳細が合理化されます。Git リポジトリでコードのみを維持し、Azure Websites で残りを処理する、ソース管理のベスト プラクティスに従うことができます。

Visual Studio ソリューションを Azure Websites にプッシュする手順は、ソリューションとリポジトリを次のように構成するのであれば、[前のセクション][BitBucket、CodePlex、Dropbox、GitHub、Mercurial などのリポジトリ Web サイトからのファイルの展開]と同じです。

-   リポジトリ ルートで、`.gitignore` ファイルを追加してから、`Obj`、`Bin`、および `packages` フォルダーなど、リポジトリから除外するすべてのファイルやフォルダーを指定します (形式については、[gitignore ドキュメント][gitignore ドキュメント]を参照してください)。次に例を示します。
    ...
    > [WACOM.NOTE] GitHub を使用する場合、リポジトリの作成時に、必要に応じて Visual Studio 固有の .gitignore ファイルを生成できます。これには、すべての一般的な一時ファイル、ビルド結果などが含まれます。ファイルは、特定のニーズに合わせてカスタマイズできます。

-   .sln ファイルをリポジトリ ルートに入れて、ソリューションのディレクトリ ツリー全体をリポジトリに追加します。

-   Visual Studio ソリューションで、[NuGet パッケージの復元を有効にして][NuGet パッケージの復元を有効にして]、Visual Studio が自動的に失われたパッケージを復元できるよにします。
...
Azure Websites の Git デプロイ プロセスの背後にあるエンジンの詳細については、[プロジェクト Kudu][プロジェクト Kudu]を参照してください。
...
## <span id="Step8"></span></a>トラブルシューティング
...
-   [How to use PowerShell for Azure (Azure 用の PowerShell を使用する方法)][How to use PowerShell for Azure (Azure 用の PowerShell を使用する方法)]
-   [Mac および Linux 用 Azure コマンド ライン ツールの使用方法][Mac および Linux 用 Azure コマンド ライン ツール]
-   [Git に関するドキュメント][Git に関するドキュメント]
-   [プロジェクト Kudu][プロジェクト Kudu]
  ...
  [gitignore ドキュメント]: http://git-scm.com/docs/gitignore
  [NuGet パッケージの復元を有効にして]: http://docs.nuget.org/docs/workflows/using-nuget-without-committing-packages
