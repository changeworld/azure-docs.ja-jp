...
ブラウザーを使用してサインインを要求することにより、テナント 2 がアプリケーションへのアクセスに使用するサービス プリンシパルを作成します。 *<Tenant2 ID>* を、イメージ ギャラリーを共有するテナントのテナント ID に置き換えます。 *<アプリケーション (クライアント) ID>* を、作成したアプリの登録のアプリケーション ID に置き換えます。 置き換えが完了したら、URL をブラウザーに貼り付け、サインイン プロンプトに従ってテナント 2 にサインインします。
