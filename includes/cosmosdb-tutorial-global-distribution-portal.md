...
2 番目のリージョンを追加したら、ポータルの **[Replicate data locally (データをローカルにレプリケートする)]** ブレードで **[手動フェールオーバー]** オプションが有効になります。 このオプションは、フェールオーバー プロセスをテストしたり、プライマリ書き込みリージョンを変更したりするために使用できます。 3 番目のリージョンを追加すると、同じブレードの **[フェールオーバーの優先度]** オプションが有効になります。これで、読み取りのフェールオーバーの順序を変更できるようになります。  
