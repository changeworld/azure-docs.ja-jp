...
> [AZURE.NOTE] VM が同じクラウド サービス内にある場合、追加の作業を行うことで、仮想マシン間での負荷分散や高可用性の管理を行うこともできます。詳細については、次の情報を参照してください: [Azure インフラストラクチャ サービスの負荷分散](../../articles/load-balance-virtual-machines/) 、[仮想マシンの可用性管理](../../articles/manage-availability-virtual-machines/). 

まず、新しいクラウド サービスで仮想マシンを作成する必要があります。その後、同じクラウド サービスに配置される追加の仮想マシンを作成します。これを'接続'します。 

1. 最初の仮想マシンを作成します。参照: [カスタム仮想マシンの作成方法](../../articles/virtual-machines-create-custom/).
