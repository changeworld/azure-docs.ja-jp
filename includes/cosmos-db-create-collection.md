...
2. **[コレクションの追加]** ページで、新しいコレクションの設定を入力します。
    ...
    スループット|400 RU|スループットを 400 要求ユニット (RU/秒) に変更します。 スループットを 400 RU/秒に設定するには、記憶域容量を**固定 (10 GB)** に設定する必要があります。 待ち時間を短縮する場合、後でスループットをスケールアップできます。 
    パーティション キー|/category|パーティション キーとして「*/category*」と入力します。 パーティション キーは、データベース内の各パーティションに均等にデータを分散っせます。 パーティション分割について詳しくは、「[パーティション分割の設計](../articles/cosmos-db/partition-data.md#designing-for-partitioning)」をご覧ください。
