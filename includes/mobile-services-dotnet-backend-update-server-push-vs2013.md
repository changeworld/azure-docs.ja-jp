この手順では、アプリケーションにプッシュ通知を送信する新しいカスタム [ApiController][ApiController] を作成します。これと同じコードを [TableController][TableController] またはバックエンド サービスの他の場所に実装することができます。
...
2.  **[Azure モバイル サービス]** を展開し、**[Azure モバイル サービス カスタム コントローラー]** をクリックします。**[追加]** をクリックし、`NotifyAllUsersController` で **[コントローラー名]** を指定した後、**[追加]** をもう一度クリックします。
