...
Azure Redis Cache インスタンスの構成と管理は Microsoft によって行われるため、すべての Redis コマンドが Azure Redis Cache でサポートされるわけではありません。 詳細については、「[Azure Redis Cache でサポートされない Redis コマンド](../articles/redis-cache/cache-configure.md#redis-commands-not-supported-in-azure-redis-cache)」を参照してください。
