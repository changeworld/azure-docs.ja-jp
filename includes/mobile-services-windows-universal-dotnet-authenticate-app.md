...
3.  既存の **OnNavigatedTo** メソッドのオーバーライドで、**RefreshTodoItems** メソッドの呼び出しをコメントアウトするか、削除します。
    ...
    > [WACOM.NOTE]Windows Phone ストア 8.1 アプリから正常に認証するためには、**OnNavigated** メソッドが呼び出された後、ページの **Loaded** イベントが発生した後に LoginAsync を呼び出す必要があります。このチュートリアルでは、アプリに **[サイン イン]** ボタンを追加してこれを行います。
...
8.  App.xaml.cs プロジェクト ファイルに次のコードを追加します。
