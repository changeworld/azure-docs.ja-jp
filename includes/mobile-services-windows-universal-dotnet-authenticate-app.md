...
2. 既存の **OnNavigatedTo** メソッドのオーバーライドで、**RefreshTodoItems** メソッドの呼び出しをコメントアウトするか、削除します。
   ...
   > [!NOTE]
   > Windows Phone ストア 8.1 アプリから正常に認証するためには、**OnNavigated** メソッドが呼び出された後、ページの **Loaded** イベントが発生した後に LoginAsync を呼び出す必要があります。このチュートリアルでは、アプリに **[サイン イン]** ボタンを追加してこれを行います。
   > 
   > 
...
5. Windows Phone アプリ プロジェクトで、**[保存]** ボタンを定義する要素の直前に次の **Button** 要素を追加します。
   
        <Button Grid.Row ="1" Grid.Column="1" Name="ButtonLogin" Click="ButtonLogin_Click" 
                        Visibility="Visible" Margin="10, 0, 0, 0">Sign in</Button> 
6. 共有された App.xaml.cs プロジェクト ファイルを開き、次の using ステートメントが存在しない場合はこれを追加します。
   
        using Microsoft.WindowsAzure.MobileServices;  
7. App.xaml.cs プロジェクト ファイルに次のコードを追加します。
