...
**EventProcessorHost** は、永続的なチェックポイントの管理によって Event Hub のイベントの受信を簡素化し、並列して Event Hub から受信する .NET クラスです。**EventProcessorHost** を使用すると、さまざまなノードでホストされている場合でも、複数の受信側間でイベントを分割できます。この例では、受信側が単一の場合に **EventProcessorHost** を使用する方法を示します。[イベント処理のスケール アウトのサンプル]は、受信側が複数の場合に **EventProcessorHost** を使用する方法を示します。
	...
	次に、クラスの本文に次のコードを置き換えます。
		...
		class SimpleEventProcessor :IEventProcessor
	    {
	        ...
	        async Task IEventProcessor.ProcessEventsAsync(PartitionContext context, IEnumerable<EventData> messages)
	        {
	            ...
	            //5 分ごとにチェックポイントをコールします。これによって再起動した場合 5 分後に処理を再開できます。
...
9. **Program** クラスには、上部に次の  `using` ステートメントを追加します。
...
[Azure Storage アカウント]: http://azure.microsoft.com/documentation/articles/storage-create-storage-account/
...
[Event Hubs のプログラミング ガイド]: http://msdn.microsoft.com/library/azure/dn789972.aspx
