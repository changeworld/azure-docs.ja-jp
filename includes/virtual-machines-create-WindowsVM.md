1. Azure [旧ポータル](http://manage.windowsazure.com)にサインインします。まだサブスクリプションをお持ちでない場合は、[無料評価版](http://azure.microsoft.com/pricing/free-trial/)をお試しください。
...
6. 3 番目の画面では、リソースのネットワーク、ストレージ、可用性を構成できます。次に、フィールドの入力内容について補足説明します。 
	...
	>[AZURE.NOTE] 仮想マシンで仮想ネットワークを使用する場合は、仮想マシンを作成するときに、必ず仮想ネットワークを指定してください。仮想マシンの作成後に仮想マシンを仮想ネットワークに参加させることはできません。 For more information, see [Azure Virtual Network Overview](http://go.microsoft.com/fwlink/p/?LinkID=294063).

	- エンドポイントの構成の詳細については、「[仮想マシンに対してエンドポイントを設定する方法](../articles/virtual-machines-set-up-endpoints-classic-portal.md)」をご覧ください。
...
7. 4 番目の構成画面では、VM エージェントと、使用可能な一部の拡張機能を構成できます。チェック マークをクリックして、仮想マシンを作成します。
	...
	>[AZURE.NOTE] VM エージェントには、仮想マシンの操作と管理に役立つ拡張機能をインストールするための環境が用意されています。 For details, see [Using Extensions](http://go.microsoft.com/FWLink/p/?LinkID=390493).  
