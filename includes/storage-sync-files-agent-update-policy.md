...
2. 必要に応じて、新しいエージェント バージョンが使用可能になるとすぐにエージェントが自動アップグレードされるように選択できます (現在クラスター サーバーには適用できません)。 この更新は、選択されているメンテナンス期間内に行われ、新機能と機能強化が一般提供されるとすぐに、サーバーはそれらの恩恵を受けることができます。 これは安心して利用できる推奨設定であり、エージェントのメジャー バージョンだけでなく定期的な更新プログラムがサーバーに提供されます。 リリースされるすべてのエージェントは GA 品質です。 "新しいバージョンが利用可能になったときに自動的に更新する" オプションを選択した場合でも、リリース直後に更新プログラムが提供されないことがあります。 新しいエージェントは、最初は少数のサーバーに提供されます。その後、Microsoft は提供対象を段階的に拡張します。 フライト化が完了すると、エージェントは Microsoft Update および [Microsoft ダウンロード センター](https://go.microsoft.com/fwlink/?linkid=858257)でも利用可能になります。
