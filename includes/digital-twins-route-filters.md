...
| データ スキーマ | DTDL モデル ID | `dataschema = '<model-dtmi-ID>'` | **テレメトリの場合** : データ スキーマは、テレメトリを生成するツインまたはコンポーネントのモデル ID です。 たとえば、`dtmi:example:com:floor4;2` のように指定します。 <br>**通知の場合** : データ スキーマには、`$body.$metadata.$model` の通知本文でアクセスできます|
...
| 通知本文 | 通知の `data` フィールドの任意のプロパティを参照します | `$body.<property>` | 「 [*イベント データについて*](https://docs.microsoft.com/azure/digital-twins/how-to-interpret-event-data)」で通知の例を参照してください。 `data` フィールドの任意のプロパティは、`$body` を使用して参照できます
