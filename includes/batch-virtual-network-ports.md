...
**追加のネットワーク リソース** - VNET を含んでいるリソース グループには、Batch によって自動的に追加のネットワーク リソースが割り当てられます。 専用ノード 50 台ごと (低優先度のノードは 20 台ごと) に、1 つのネットワーク セキュリティ グループ (NSG)、1 つのパブリック IP アドレス、1 つのロード バランサーが Batch によって割り当てられます。 これらのリソースは、サブスクリプションの[リソース クォータ](../articles/azure-subscription-service-limits.md)によって制限されます。 大規模なプールでは、これらの 1 つ以上のリソースについて、クォータの引き上げの要求が必要になる場合があります。
...
* ポート 22 (Linux ノード) またはポート 3389 (Windows ノード) のインバウンド TCP トラフィック。リモート アクセスを許可するために使用されます。
...
サブネット レベルの NSG を自分で指定する必要はありません。これは、Batch によってその独自の NSG が構成されるためです。 ただし、指定したサブネットに、ネットワーク セキュリティ グループ (NSG) やファイアウォールが関連付けられている場合は、次の表に示すようにインバウンドとアウトバウンドのセキュリティ規則を構成します。 ポート 3389 (Windows) またはポート 22 (Linux) のインバウンド トラフィックは、プールの VM に対するリモート アクセスを許可する必要がある場合にのみ構成します。 プールの VM を使用可能な状態にするうえで必ずしも必要な作業ではありません。

**[受信セキュリティ規則]**

| ソース IP アドレス | ソース ポート | 変換先 | ターゲット ポート | プロトコル | アクションを表示します。 |
| --- | --- | --- | --- | --- | --- |
任意 <br /><br />実際上は "すべて許可" が必要ですが、Batch サービスは、Batch サービス以外の IP アドレスをすべてフィルターで除外する NSG を、Virtual Machine の構成の下で作成された各 VM 上のネットワーク インターフェイス レベルで適用します。 | * | 任意 | 29876 から 29877 | TCP | ALLOW |
| デバッグ目的でプールの VM にリモートでアクセスするために使用されるユーザーのマシン。 | * | 任意 |  3389 (Windows)、22 (Linux) | TCP | ALLOW |

**アウトバウンド セキュリティ規則**

| ソース | ソース ポート | 変換先 | 宛先サービス タグ | プロトコル | アクションを表示します。 |
| --- | --- | --- | --- | --- | --- |
| 任意 | 443 | [サービス タグ](../articles/virtual-network/security-overview.md#service-tags) | ストレージ (ご利用の Batch アカウントおよび VNET と同じリージョン内)  | 任意 | ALLOW |
...
NSG を指定する必要はありません。Batch IP アドレスからプールのノードへのみのインバウンド通信が Batch によって構成されるためです。 ただし、指定したサブネットに、NSG やファイアウォールが関連付けられている場合は、次の表に示すようにインバウンドとアウトバウンドのセキュリティ規則を構成します。 指定したサブネット内のコンピューティング ノードとの通信が NSG によって拒否された場合、Batch サービスによって、コンピューティング ノードの状態が**使用不可**に設定されます。
 ...
 ポート 3389 (Windows) またはポート 22 (Linux) のインバウンド トラフィックは、プールのノードに対するリモート アクセスを許可する必要がある場合にのみ構成します。 プールのノードを使用可能な状態にするうえで必ずしも必要な作業ではありません。
...
| ソース | ソース ポート | 変換先 | ターゲット ポート | プロトコル | アクションを表示します。 |
| --- | --- | --- | --- | --- | --- |
| 任意 | * | 任意 | 443  | 任意 | ALLOW |