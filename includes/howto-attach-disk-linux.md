...
2.  次のように「`azure vm disk attach-new`」と入力し、新しいディスクを作成して接続します。_TestVM_ は Linux 仮想マシンの名前で置き換え、ディスクのサイズを GB 単位で指定します (この例では 100 GB)。
...
4.  ここで、必要な VHD を仮想マシンに接続します。
	...
	_TestVM_ は仮想マシンの名前に、_myTestVhd_ は目的の VHD に置き換えてください。
