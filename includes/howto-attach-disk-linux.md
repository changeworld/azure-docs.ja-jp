...
2.  次のように `azure vm disk attach-new` を入力し、新しいディスクを作成して接続します。_ubuntuVMasm_ を Linux 仮想マシンの名前に置き換えます。その後、ディスクのサイズを GB 単位で指定します。この例では、30 GB を指定しています。

        azure vm disk attach-new ubuntuVMasm 30

3.	データ ディスクが作成され、接続されると、そのディスクは `azure vm disk list
...
3.  使用するディスクが見つからない場合は、`azure vm disk create` または `azure vm disk upload` を使用してローカル VHD をサブスクリプションにアップロードできます。`disk create` の例を次に示します。
	...
	`azure vm disk upload` を使用して、特定のストレージ アカウントに VHD をアップロードすることもできます。Azure Virtual Machine データ ディスクの管理用コマンドの詳細については、[こちら](virtual-machines-command-line-tools.md#commands-to-manage-your-azure-virtual-machine-data-disks)を参照してください。

4.  ここで、必要な VHD を仮想マシンに接続する必要があります。
...
5.	ディスクが仮想マシンに接続されたことを確認するには、`azure vm disk list
 	<virtual-machine-name>` を使用します。
