...
*    AMQP 1.0 サポートは、Azure のサービス バスと Windows Server のサービス バス (サービス バス 1.1) で利用できるようになりました。料金は、既存のプロトコルと同じです。
