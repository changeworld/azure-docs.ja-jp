...
2. `Console.ReadLine()` 行の直前の **Main** メソッドに次のメソッドを追加します。
    ...
    > [AZURE.NOTE]わかりやすくするために、このチュートリアルでは再試行ポリシーは実装しません。運用環境のコードでは、[一時的な障害処理]に関する MSDN の記事で推奨されているように、再試行ポリシー (指数関数的バックオフなど) を実装することをお勧めします。

<!-- Links -->
[一時的な障害処理]: https://msdn.microsoft.com/ja-JP/library/hh675232.aspx
