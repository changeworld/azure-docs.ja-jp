...
| [拡張機能が存在しない場合の監査](../articles/governance/policy/samples/audit-ext-not-exist.md) | 仮想マシンに拡張機能がデプロイされていない場合に監査が行われます。 デプロイされたかどうかを確認する拡張機能の公開元と種類を指定します。 |
| [許可されない VM 拡張機能](../articles/governance/policy/samples/not-allowed-vm-ext.md) | 指定された拡張機能の使用を禁止します。 禁止されている拡張機能の種類を格納している配列を指定します。 |