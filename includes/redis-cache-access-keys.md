キャッシュに接続するには、キャッシュ クライアントにホスト名、ポート、およびキーが必要です。一部のクライアントは、若干異なる名前で、これらの項目を参照できます。これらの項目を取得するには、[Azure ポータル](https://portal.azure.com)でキャッシュを[参照](../articles/redis-cache/cache-configure.md#configure-redis-cache-settings)し、**[設定]** または **[すべての設定]** をクリックします。
