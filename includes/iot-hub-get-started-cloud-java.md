...
10. **MessageReceiver** クラスに次の **run** メソッドを追加します。このメソッドでは、Event Hub パーティションから読み込む **EventHubReceiver** インスタンスが作成されます。これは **stopThread** が true になるまでループを続け、メッセージの詳細を出力します。
    ...
    > [!NOTE]
    > 受信側が実行開始後、IoT Hub に送信されたメッセージのみを読み込むように、このメソッドは受信側の構築時にフィルターを称します。現在の一連のメッセージを確認できるので、テスト環境ではこれは便利ですが、運用環境の場合、コードですべてのメッセージが処理されることを確認する必要があります。詳細については、「[IoT Hub のデバイスからクラウドへのメッセージを処理する方法][lnk-processd2c-tutorial]」チュートリアルを参照してください。
    > 
    > 
